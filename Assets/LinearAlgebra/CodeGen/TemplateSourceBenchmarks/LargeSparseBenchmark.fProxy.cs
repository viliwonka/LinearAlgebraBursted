...
namespace LinearAlgebra.Benchmarks
{
    ...
    [BurstCompile(CompileSynchronously = true, FloatMode = FloatMode.Default)]
    public struct SpCgJobFProxy : IJob { public fProxyBSR A; public fProxyN b, x, r, p, Ap; public int K;
        public void Execute() { for (int i = 0; i < x.N; i++) x[i] = 0f; Krylov.cg(in A, in b, ref x, ref r, ref p, ref Ap, K, 0f); } }

    [BurstCompile(CompileSynchronously = true, FloatMode = FloatMode.Default)]
    public struct SpPcgJobFProxy : IJob { public fProxyBSR A; public fProxyBlockJacobi M; public fProxyN b, x, r, p, Ap, z; public int K;
        public void Execute() { for (int i = 0; i < x.N; i++) x[i] = 0f; Krylov.pcg(in A, in M, in b, ref x, ref r, ref p, ref Ap, ref z, K, 0f); } }

    [BurstCompile(CompileSynchronously = true, FloatMode = FloatMode.Default)]
    public struct SpMinresJobFProxy : IJob { public fProxyBSR A; public fProxyN b, x, y, r1, r2, v, w, w1, w2; public int K;
    ...
    [BurstCompile(CompileSynchronously = true, FloatMode = FloatMode.Default)]
    public struct SpBicgJobFProxy : IJob { public fProxyBSR A; public fProxyN b, x, r, rHat0, p, v, t; public int K;
        public void Execute() { for (int i = 0; i < x.N; i++) x[i] = 0f; Krylov.biCGStab(in A, in b, ref x, ref r, ref rHat0, ref p, ref v, ref t, K, 0f); } }

    [BurstCompile(CompileSynchronously = true, FloatMode = FloatMode.Default)]
    public struct SpCglsJobFProxy : IJob { public fProxyBSR A; public fProxyN b, x, r, s, p, q; public int K;
        public void Execute() { for (int i = 0; i < x.N; i++) x[i] = 0f; Krylov.cgls(in A, in b, ref x, ref r, ref s, ref p, ref q, K, 0f); } }

    [BurstCompile(CompileSynchronously = true, FloatMode = FloatMode.Default)]
    public struct SpLsqrJobFProxy : IJob { public fProxyBSR A; public fProxyN b, x, u, v, w, tmpM, tmpN; public int K;
        public void Execute() { for (int i = 0; i < x.N; i++) x[i] = 0f; Krylov.lsqr(in A, in b, ref x, ref u, ref v, ref w, ref tmpM, ref tmpN, K, 0f); } }

    [BurstCompile(CompileSynchronously = true, FloatMode = FloatMode.Default)]
    public struct SpLsmrJobFProxy : IJob { public fProxyBSR A; public fProxyN b, x, u, v, h, hbar, tmpM, tmpN; public int K;
    ...
    public static partial class LargeSparseBenchmark
    {
        ...
        static void BenchKrylovFProxy(StringBuilder sb, int BR, int[] Ns, fProxy density, int K, int spmvReps)
        {
            foreach (int N in Ns)
            {
                ...
                sb.AppendLine(LargeSparseFmt.Row("fProxy", sz, "spMV x" + spmvReps, Bench.Time(() => spmvJob.Run()), 0.0));

                var x = arena.fProxyVec(N);
                var cgJob = new SpCgJobFProxy { A = A, b = b, x = x, r = arena.fProxyVec(N), p = arena.fProxyVec(N), Ap = arena.fProxyVec(N), K = K };
                sb.AppendLine(LargeSparseFmt.Row("fProxy", sz, "CG", Bench.Time(() => cgJob.Run()), Res(in A, in x, in b)));
                var xp = arena.fProxyVec(N);
                var pcgJob = new SpPcgJobFProxy { A = A, M = M, b = b, x = xp, r = arena.fProxyVec(N), p = arena.fProxyVec(N), Ap = arena.fProxyVec(N), z = arena.fProxyVec(N), K = K };
                sb.AppendLine(LargeSparseFmt.Row("fProxy", sz, "PCG-Jacobi", Bench.Time(() => pcgJob.Run()), Res(in A, in xp, in b)));
                var xm = arena.fProxyVec(N);
                var mrJob = new SpMinresJobFProxy { A = A, b = b, x = xm, y = arena.fProxyVec(N), r1 = arena.fProxyVec(N), r2 = arena.fProxyVec(N), v = arena.fProxyVec(N), w = arena.fProxyVec(N), w1 = arena.fProxyVec(N), w2 = arena.fProxyVec(N), K = K };
                sb.AppendLine(LargeSparseFmt.Row("fProxy", sz, "MINRES", Bench.Time(() => mrJob.Run()), Res(in A, in xm, in b)));
                ...
                var bicgJob = new SpBicgJobFProxy { A = An, b = bn, x = xn, r = arena.fProxyVec(N), rHat0 = arena.fProxyVec(N), p = arena.fProxyVec(N), v = arena.fProxyVec(N), t = arena.fProxyVec(N), K = K };
                sb.AppendLine(LargeSparseFmt.Row("fProxy", sz, "BiCGStab", Bench.Time(() => bicgJob.Run()), Res(in An, in xn, in bn)));
                ...
                var cglsJob = new SpCglsJobFProxy { A = At, b = bt, x = xc, r = arena.fProxyVec(m), s = arena.fProxyVec(N), p = arena.fProxyVec(N), q = arena.fProxyVec(m), K = K };
                sb.AppendLine(LargeSparseFmt.Row("fProxy", rsz, "CGLS", Bench.Time(() => cglsJob.Run()), Res(in At, in xc, in bt)));
                var xl = arena.fProxyVec(N);
                var lsqrJob = new SpLsqrJobFProxy { A = At, b = bt, x = xl, u = arena.fProxyVec(m), v = arena.fProxyVec(N), w = arena.fProxyVec(N), tmpM = arena.fProxyVec(m), tmpN = arena.fProxyVec(N), K = K };
                sb.AppendLine(LargeSparseFmt.Row("fProxy", rsz, "LSQR", Bench.Time(() => lsqrJob.Run()), Res(in At, in xl, in bt)));
                var xr = arena.fProxyVec(N);
                var lsmrJob = new SpLsmrJobFProxy { A = At, b = bt, x = xr, u = arena.fProxyVec(m), v = arena.fProxyVec(N), h = arena.fProxyVec(N), hbar = arena.fProxyVec(N), tmpM = arena.fProxyVec(m), tmpN = arena.fProxyVec(N), K = K };
                sb.AppendLine(LargeSparseFmt.Row("fProxy", rsz, "LSMR", Bench.Time(() => lsmrJob.Run()), Res(in At, in xr, in bt)));
