...
namespace BULA.Benchmarks
{
    ...
    // counterpart of LuSolveJobFProxy above (which uses the split L/U form; the compact form is the
    // one decompSolveTransA is promoted on, see LU.fProxy.cs).
    [BurstCompile(CompileSynchronously = true, FloatPrecision = FloatPrecision.High, FloatMode = FloatMode.Default)]
    public struct LuSolveTransAJobFProxy : IJob
    {
        public fProxyMxN A;     // receives Src via decompInPlace (compact factor, destructive)
        public fProxyMxN Src;
        public fProxyN b;       // receives bSrc, overwritten with the solution
        public fProxyN bSrc;
    ...
    public static partial class DirectSolveBenchmark
    {
        ...
        static string LuSolveTransAFProxy(int N)
        {
            ...
            var job = new LuSolveTransAJobFProxy { A = A, Src = Src, b = b, bSrc = bSrc };
            var stat = Bench.Time(() => job.Run());

            A.Dispose(); Src.Dispose(); b.Dispose(); bSrc.Dispose();
            return Bench.RowTime("LU fProxy (TransA)", N, stat);
        }
        ...
        static string QrSolveCacheFProxy(int N)
        {
            ...
            var bSrc = new fProxyN(N, Allocator.Persistent);
            var x = new fProxyN(N, Allocator.Persistent);
            var cache = new fProxyQRCache(N, N, Allocator.Persistent);
