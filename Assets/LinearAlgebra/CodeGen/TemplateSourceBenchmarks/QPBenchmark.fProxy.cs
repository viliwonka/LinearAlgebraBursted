...
namespace LinearAlgebra.Benchmarks
{
    ...
    [BurstCompile(CompileSynchronously = true, FloatPrecision = FloatPrecision.High, FloatMode = FloatMode.Default)]
    public struct QpCoreLoopJobFProxy : IJob
    {
        public fProxyMxN Q;
        public fProxyN c;
        public fProxyMxN A;
        ...
        public NativeArray<ConstraintSense> senses;
        public fProxyN xl, xu;
        public fProxyN x0;        // feasible start (read-only source; copied per rep)
        public fProxyN x;         // working solution (overwritten from x0 each Execute)
        public int maxIter;
    ...
    public static partial class QPBenchmark
    {
        ...
        static void SectionCoreLoopFProxy(StringBuilder sb)
        {
            ...
            foreach (var n in QPBenchmarkFmt.SizesN)
            {
                ...
                {
                    bool inc = variant == 0;
                    var job = new QpCoreLoopJobFProxy
                    {
                    ...
                    sb.AppendLine(QPCoreLoopFmt.Row("fProxy", inc ? "incr" : "batch", n, m, stat,
