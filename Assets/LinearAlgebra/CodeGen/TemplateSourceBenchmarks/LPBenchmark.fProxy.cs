    ...
    public static partial class LPBenchmark
    {
        ...
        static void SectionSolveFProxy(StringBuilder sb)
        {
            ...
            foreach (var n in LPBenchmarkFmt.SolveVarsN)
            {
                ...
                var jobR = new LpSolveJobFProxy { A = A, b = b, c = c, senses = senses, x = xR, method = LPMethod.RevisedSimplex, maxIter = 0 };
                var statR = Bench.Time(() => jobR.Run());
                sb.AppendLine(LPBenchmarkFmt.SolveRow("fProxy", n, m, "revised-primal", statR, infoR.iterations, objR));

                var xD = arena.fProxyVec(n);
                var infoD = LP.solve(in A, in b, in c, in senses, ref xD, out double objD, LPMethod.DualSimplex);
                var jobD = new LpSolveJobFProxy { A = A, b = b, c = c, senses = senses, x = xD, method = LPMethod.DualSimplex, maxIter = 0 };
                var statD = Bench.Time(() => jobD.Run());
                sb.AppendLine(LPBenchmarkFmt.SolveRow("fProxy", n, m, "dual-simplex", statD, infoD.iterations, objD));
