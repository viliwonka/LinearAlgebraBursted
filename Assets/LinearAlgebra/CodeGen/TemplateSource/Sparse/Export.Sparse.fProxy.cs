...
namespace BULA
{
    ...
    // Debug/Export.fProxy.cs. Lives in namespace BULA (not BULA.Sparse) so it
    // merges into the same `Print` partial class as the dense exporters and the Sparse Debug
    // overloads -- fProxyBSR is brought in via the `using BULA.Sparse;` above (same
    // pattern as Sparse/Debug.Sparse.fProxy.cs).
    public static partial class Print
    {
        /// <summary>
        /// Dense-ish preview: densifies via fProxyBSR.ToDense into scratch Temp memory (allocated and
        /// disposed internally -- the caller does not need to manage it) and reuses the existing
        /// dense Print.ToText(in fProxyMxN). For a preview of the STORAGE itself (not the expanded
        /// dense matrix), see ToCsv(in fProxyBSR)/SaveCsv, which write a block-level
        ...
        /// layout -- see fProxyBSR.cs). Avoids needing scratch memory (unlike ToText's ToDense route)
