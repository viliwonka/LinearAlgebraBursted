...
namespace LinearAlgebra.Sparse
{
    ...
    public readonly partial struct fProxyBlockJacobi : IfProxyPreconditioner, IDisposable
    {
        ...
        public unsafe UnsafeList<fProxy> DInv
        {
            get
            {
        ...
        {
            if (_rec != null && !ChunkedRecordTable<fProxyBlockJacobiRecord>.IsAliveFast(_rec))
                throw new InvalidOperationException("fProxyBlockJacobi.DInv: use of disposed/cleared arena allocation");
        }
#endif
        ...
        {
            this = new fProxyBlockJacobi(in A, allocator, out DirectSolveInfo info);
            if (!info.Solved)
        ...
        /// </summary>
        public unsafe fProxyBlockJacobi(in fProxyBSR A, Allocator allocator, out DirectSolveInfo info)
        {
            _rec = null;
            _inlineDInv = default;
        ...
        internal unsafe fProxyBlockJacobi(in fProxyBSR A, fProxyBlockJacobiRecord* rec, Allocator allocator, out DirectSolveInfo info) : this(in A, allocator, out info)
        {
            if (!info.Solved)
                return;
            rec->DInv = _inlineDInv;
