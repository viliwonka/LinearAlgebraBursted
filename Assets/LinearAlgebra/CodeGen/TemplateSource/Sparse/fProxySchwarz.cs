...
namespace LinearAlgebra.Sparse
{
    ...
    /// <see cref="LinearAlgebra.Krylov"/>.pcg AND pminres.
    ///
    ...
    public readonly struct fProxyAdditiveSchwarz : IfProxyPreconditioner
    {
        ...
        public unsafe void Apply(in fProxyN r, ref fProxyN z)
        {
            ...
            for (int i = 0; i < n; i++) z[i] = (fProxy)0;
        ...
        static void CholSolveInPlace(fProxyN factors, int fbase, int nn, fProxyN b)
        {
            for (int r = 0; r < nn; r++)
            {
                fProxy s = b[r];
    ...
    /// makes M NON-SYMMETRIC even for SPD A: never use it with pcg/pminres/CG (there is no such
    ...
    public readonly struct fProxyRestrictedSchwarz : IfProxyPreconditioner
    {
        ...
        static void LUSolveInPlace(fProxyN factors, int fbase, int nn, fProxyN b)
        {
            for (int r = 0; r < nn; r++)
            {
                fProxy s = b[r];
