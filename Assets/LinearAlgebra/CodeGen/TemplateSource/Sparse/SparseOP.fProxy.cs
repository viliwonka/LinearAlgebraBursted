    ...
    public static partial class BSR
    {
        ...
        public static void spMV(in fProxyBSR A, in fProxyN x, ref fProxyN y)
        {
            ...
            unsafe
            {
                ...
                fProxy* xPtr = x.Data.Ptr;
                fProxy* yPtr = y.Data.Ptr;
        ...
        // fProxyBSROperator.ApplyBlock looped `rows` scalar spMV calls through two Allocator.Temp
        // vectors). Same dispatch shape as spMV -- see bsrMatMat*/bsrMatMatSym* (UnsafeOP.Sparse.
        // fProxy.cs) for the per-kernel bit-identity argument. Guard: y must not alias x -- each
        // x-row feeds every block-row that stores a block in its column-block (same reason as spMV).
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static void spMM(in fProxyBSR A, in fProxyMxN Vrows, ref fProxyMxN AVrows, int rows)
        {
            ...
            unsafe
            {
                ...
                UnsafeUtility.MemClear(AVrows.Data.Ptr, (long)rows * AVrows.N_Cols * UnsafeUtility.SizeOf<fProxy>());

                int* rowPtr = A.RowPtr.Ptr;
                int* colInd = A.ColInd.Ptr;
                fProxy* values = A.Values.Ptr;
                fProxy* VPtr = Vrows.Data.Ptr;
                fProxy* AVPtr = AVrows.Data.Ptr;
        ...
        // (docs/draft-spec-krylov-optimization.md; see fProxyBSROperator.ApplyDot, the sole
        ...
        // contiguous 4-wide block to reinterpret as fProxy4 (row results arrive one at a time),
        // used two alternating SCALAR accumulators instead. That scalar fold is far slower than
        // simply calling the already-tuned SIMD vecDot separately (2x fProxy4, 8 lane-chains) --
        ...
        // known-worse code.
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static fProxy spMVDot(in fProxyBSR A, in fProxyN x, ref fProxyN y)
        {
        ...
        /// </summary>
        public static void sweepLower(in fProxyBSR A, in fProxyBlockJacobi Jacobi, fProxy diagScale, in fProxyN r, ref fProxyN y)
        {
            if (A.BlockRows != A.BlockCols || A.BR != A.BC)
                throw new ArgumentException("sweepLower: A must be square (BlockRows==BlockCols, BR==BC)");
            if (A.Symmetric)
                throw new ArgumentException("sweepLower: A must be full-storage BSR (Symmetric upper-block-triangle storage is not supported here -- mirror it first via Arena.fProxyBSRMirrorToFull)");
            ...
            unsafe
            {
                ...
                int* colInd = A.ColInd.Ptr;
                fProxy* values = A.Values.Ptr;
                fProxy* dInv = Jacobi.DInv.Ptr;
                fProxy* rPtr = r.Data.Ptr;
                fProxy* yPtr = y.Data.Ptr;
        ...
        public static void sweepLower(in fProxyBSR A, in fProxyBlockJacobi Jacobi, in fProxyN r, ref fProxyN y)
            => sweepLower(in A, in Jacobi, (fProxy)1, in r, ref y);
        ...
        /// </summary>
        public static void sweepUpper(in fProxyBSR A, in fProxyBlockJacobi Jacobi, fProxy diagScale, in fProxyN r, ref fProxyN y)
        {
            if (A.BlockRows != A.BlockCols || A.BR != A.BC)
                throw new ArgumentException("sweepUpper: A must be square (BlockRows==BlockCols, BR==BC)");
            if (A.Symmetric)
                throw new ArgumentException("sweepUpper: A must be full-storage BSR (Symmetric upper-block-triangle storage is not supported here -- mirror it first via Arena.fProxyBSRMirrorToFull)");
            if (Jacobi.BlockRows != A.BlockRows || Jacobi.BR != A.BR)
        ...
        public static void sweepUpper(in fProxyBSR A, in fProxyBlockJacobi Jacobi, in fProxyN r, ref fProxyN y)
            => sweepUpper(in A, in Jacobi, (fProxy)1, in r, ref y);
    }
}
