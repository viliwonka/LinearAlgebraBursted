...
namespace LinearAlgebra.Sparse
{
    ...
    /// §2.3.
    ///
    /// Lifecycle: build via fProxyBSRBuilder.ToBSR(arena). This type is the compressed,
    ...
    public partial struct fProxyBSR : IDisposable
    {
        ...
        // Arena-tracked path: a stable pointer into the arena's ChunkedRecordTable<fProxyBSRRecord>
        ...
        [NativeDisableUnsafePtrRestriction] private unsafe fProxyBSRRecord* _rec;
        ...
        private UnsafeList<fProxy> _inlineValues;

        // CSR-of-blocks index structure. Dual-mode: arena-tracked resolves through the record,
        // standalone keeps the inline field untouched -- mirrors fProxyN.Data (Arena/fProxyN.cs).
        ...
        // Unity Editor, including every test run, and compiles out of player builds). fProxyBSR has
        ...
        {
            if (_rec != null && (!ChunkedRecordTable<fProxyBSRRecord>.IsAliveFast(_rec) || ChunkedRecordTable<fProxyBSRRecord>.GenerationFast(_rec) != _gen))
                throw new InvalidOperationException("fProxyBSR: use of disposed/cleared arena allocation");
        }
#endif
        ...
        /// arena's record table by the caller (Arena.fProxyBSR) -- this ctor only fills in the
        /// record's RowPtr/ColInd/Values, it does not allocate or own the slot itself. Same
        /// pre-allocated-record contract as fProxyN's arena-tracked ctor (Arena/fProxyN.cs).
        /// </summary>
        internal unsafe fProxyBSR(int blockRows, int blockCols, int BR, int BC, int nnzb, fProxyBSRRecord* rec, Allocator allocator, bool uninit = false, bool symmetric = false)
        {
            _rec = rec;
            _inlineRowPtr = default;
            _inlineColInd = default;
        ...
        public unsafe void Dispose()
        {
            ...
            if (_rec != null)
            {
                ...
                // memory would be touched a second time -- see fProxyN.Dispose() (Arena/fProxyN.cs)
