...
namespace LinearAlgebra.Sparse
{
    ...
    /// blocks implicit), U upper; the nonsymmetric sibling of <see cref="fProxyIC0"/>, intended
    ...
    /// </summary>
    public readonly struct fProxyILU0 : IfProxyPreconditioner, IDisposable
    {
        public readonly fProxyBSR F;      // A's full pattern: L (strict lower), U_ii^{-1} (diag), U (upper)
        ...
        public fProxyILU0(in fProxyBSR a, ref Arena arena, out PreconditionerInfo info)
        {
            ...
            if (a.BR > 16)
                throw new ArgumentException("fProxyILU0: BR must be <= 16 (fixed-size pivot scratch)");
            ...
            for (int i = 0; i < nb; i++)
            {
                ...
                if (!hasDiag)
                    throw new ArgumentException("fProxyILU0: missing diagonal block in A");
            }
            if (diagMax <= (fProxy)0) diagMax = (fProxy)1;

            // Working copy with A's full pattern.
            var Fm = arena.fProxyBSR(nb, nb, BR, BR, A.Nnzb, true);
            {
        ...
        /// <summary>
        /// Standalone twin of <see cref="fProxyILU0(in fProxyBSR, ref Arena)"/>: allocates F from
        ...
        /// </summary>
        public unsafe fProxyILU0(in fProxyBSR a, Allocator allocator)
        {
            this = new fProxyILU0(in a, allocator, out PreconditionerInfo info);
            if (!info.Solved)
            {
                Dispose();
                throw new ArgumentException("fProxyILU0: factorization broke down at every diagonal shift — pivot blocks numerically singular");
        ...
        /// <summary>Standalone twin of <see cref="fProxyILU0(in fProxyBSR, ref Arena, out PreconditionerInfo)"/>.
        ...
        public unsafe fProxyILU0(in fProxyBSR a, Allocator allocator, out PreconditionerInfo info)
        {
            if (a.BlockRows != a.BlockCols || a.BR != a.BC)
                throw new ArgumentException("fProxyILU0: A must be square (BlockRows==BlockCols, BR==BC)");
            if (a.BR > 16)
            ...
            var Fm = new fProxyBSR(nb, nb, BR, BR, A.Nnzb, allocator, true);
            {
                var fRowPtr = Fm.RowPtr; var fColInd = Fm.ColInd;
                for (int i = 0; i <= nb; i++) fRowPtr[i] = A.RowPtr[i];
                for (int k = 0; k < A.Nnzb; k++) fColInd[k] = A.ColInd[k];
        ...
        static bool FactorizeInPlace(in fProxyBSR Fm, fProxy diagMax)
        {
            int nb = Fm.BlockRows;
            int BR = Fm.BR;
            int blockLen = BR * BR;
        ...
        static unsafe void BlockMulRight(Unity.Collections.LowLevel.Unsafe.UnsafeList<fProxy> v, int xOff, int mOff, int BR)
        {
            // Row buffer stackalloc'd ONCE (stackalloc frees at return, not loop exit).
            fProxy* tmp = stackalloc fProxy[16];
            for (int r = 0; r < BR; r++)
            {
                for (int c = 0; c < BR; c++)
                {
        ...
        static void BlockMulSubAB(Unity.Collections.LowLevel.Unsafe.UnsafeList<fProxy> v, int cOff, int aOff, int bOff, int BR)
        {
            for (int r = 0; r < BR; r++)
                for (int c = 0; c < BR; c++)
                {
        ...
        static bool InvertBlockInPlace(Unity.Collections.LowLevel.Unsafe.UnsafeList<fProxy> v, int off, int BR, fProxy pivotFloor)
        {
            unsafe
            {
                fProxy* m = stackalloc fProxy[16 * 16];
                ...
                for (int r = 0; r < BR; r++)
                    for (int c = 0; c < BR; c++)
                        inv[r * BR + c] = r == c ? (fProxy)1 : (fProxy)0;

                for (int col = 0; col < BR; col++)
                {
                    int piv = col;
                    fProxy best = math.abs(m[col * BR + col]);
                    for (int r = col + 1; r < BR; r++)
                    {
                        fProxy av = math.abs(m[r * BR + col]);
                    ...
                    fProxy d = (fProxy)1 / m[col * BR + col];
        ...
        {
            int n = Rows;
            if (r.N != n) throw new ArgumentException("fProxyILU0.Apply: r.N must equal Rows");
            if (z.N != n) throw new ArgumentException("fProxyILU0.Apply: z.N must equal Rows");
            if (z.Data.Ptr == r.Data.Ptr) throw new ArgumentException("fProxyILU0.Apply: z must not alias r");
            ...
            fProxy* w = stackalloc fProxy[16];   // stackalloc'd ONCE (stackalloc frees at return, not loop exit)
