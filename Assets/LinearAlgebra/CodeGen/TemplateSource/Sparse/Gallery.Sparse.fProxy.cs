...
namespace LinearAlgebra.Gallery
{
    ...
    public static partial class fProxyGallery
    {
        ...
        /// <summary>Standalone twin of <see cref="fProxyRandomSparseSPD(ref Arena, int, int, fProxy, uint)"/>:
        ...
        /// <summary>Standalone twin of <see cref="fProxyRandomSparse(ref Arena, int, int, int, fProxy, uint)"/>:
        /// builds entirely from Temp scratch and returns a result allocated from
        /// <paramref name="allocator"/> -- no arena needed. Same generation logic/RNG draws.</summary>
        ...
        public static fProxyBSR fProxyLaplacian2D(this ref Arena arena, int gridX, int gridY)
        {
            ...
            var builder = arena.fProxyBSRBuilder(nb, nb, BR, BR, nnzbEstimate);

            // Diagonal block: tridiag(-1, 4, -1) (x-neighbor coupling + the full 2D degree 4).
            var D = arena.fProxyMat(BR, BR);
            for (int r = 0; r < BR; r++)
                for (int c = 0; c < BR; c++)
                    D[r, c] = (fProxy)0;
            for (int r = 0; r < BR; r++)
        ...
        /// <summary>Standalone twin of <see cref="fProxyLaplacian2D(ref Arena, int, int)"/>: builds
        ...
        public static fProxyBSR fProxyPenalizedGrid3D(this ref Arena arena, int nx, int ny, int nz, fProxy EA, fProxy penalty)
        {
            if (nx < 1 || ny < 1 || nz < 1)
            ...
            var builder = arena.fProxyBSRBuilder(nb, nb, 3, 3, nb + 3 * barCount);
        ...
        /// <summary>Standalone twin of <see cref="fProxyPenalizedGrid3D(ref Arena, int, int, int, fProxy, fProxy)"/>:
        /// builds entirely from Temp scratch and returns a result allocated from
