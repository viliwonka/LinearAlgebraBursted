...
namespace LinearAlgebra
{
    ...
    // n = cols
    //
    // [StructLayout(Sequential)]: pins field order/packing explicitly (matches iProxyN's existing
    ...
    public partial struct iProxyMxN : IDisposable, IUnsafeiProxyArray {
        ...
        // Arena-tracked path -- see iProxyN.cs's `_rec` doc comment for the full rationale (same
        ...
        [NativeDisableUnsafePtrRestriction] private unsafe iProxyMatRecord* _rec;

        // Standalone-path backing store. Stays default(UnsafeList<iProxy>) whenever _rec != null.
        ...
        // TempCopy() and the cross-type allocation shortcuts (iProxyMxN.Shortcuts.cs) that used to
        ...
        // iProxyN's option, cannot tell "still the same allocation" from "a new one that reused this
        // slot number").
        ...
        /// arena's record table by the caller (Arena.iProxyMat/iProxyTempMat) -- this ctor only
        /// fills in the record's Data, it does not allocate or own the slot itself.
        /// </summary>
        internal unsafe iProxyMxN(int M_rows, int N_cols, iProxyMatRecord* rec, Allocator allocator, bool uninit = false)
        {
            _rec = rec;
            _inlineData = default;
        ...
        internal unsafe iProxyMxN(in iProxyMxN orig, iProxyMatRecord* rec, Allocator allocator)
        {
            _rec = rec;
            _inlineData = default;
        ...
        // doc comment). Unlike iProxyN (Alive-only), iProxyMxN also has
        ...
        {
            if (_rec != null && (!ChunkedRecordTable<iProxyMatRecord>.IsAliveFast(_rec) || ChunkedRecordTable<iProxyMatRecord>.GenerationFast(_rec) != _gen))
                throw new InvalidOperationException("iProxyMxN.Data: use of disposed/cleared arena allocation");
        }
#endif
        ...
        public unsafe iProxyMxN Copy()
        {
            ...
            return OwnerArena.iProxyMat(in this);
        }
        ...
        public unsafe iProxyMxN TempCopy()
        {
            ...
            return OwnerArena.iProxyTempMat(in this);
        }
