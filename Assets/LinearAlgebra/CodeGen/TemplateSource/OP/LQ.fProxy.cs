...
namespace LinearAlgebra
{
    public static partial class LQ {
        ...
        internal static void genHouseholderRow(ref fProxyMxN M, ref fProxyN v, int row, int colStart, fProxy zeroThreshold)
        {
            int n = M.N_Cols;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe fProxy dot4(fProxy* a, fProxy* b, int n) => UnsafeOP.vecDot(a, b, n);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe void applyHouseholderRightRows(ref fProxyMxN M, ref fProxyN v, int rowStart, int rowEnd, int colStart)
        {
            int cols = M.N_Cols;
            int L = cols - colStart;
            if (L <= 0) return;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal static unsafe void applyHouseholderRight(ref fProxyMxN M, ref fProxyN v, int rowStart, int colStart)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static void lqKernel(ref fProxyMxN W, ref fProxyMxN L, ref fProxyMxN Q, ref fProxyN v, fProxy zeroThreshold, bool reconstructQ)
        {
            int m = W.M_Rows;
            int n = W.N_Cols;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe void lqDecompositionBlockedCore(ref fProxyMxN W, ref fProxyMxN L, ref fProxyMxN Q,
            ref fProxyN v, ref fProxyN Vpanel, ref fProxyN Vt, ref fProxyN Tbuf, ref fProxyN Y, ref fProxyN tcolBuf,
            fProxy zeroThreshold, bool reconstructQ)
        {
            ...
            for (int p0 = 0; p0 < m; p0 += LQ_BLOCK)
            {
                ...
                {
                    int r = p0 + i;
                    fProxy* Vrow = Vp + (long)i * cn;
                    fProxy* Wrow = Wp + (long)r * n + p0;
                    for (int c = 0; c < cn; c++)
                        Vrow[c] = (c >= i) ? Wrow[c] : (fProxy)0;
                }
                ...
                if (rowsTrail > 0)
                {
                    fProxy* Cp = Wp + (long)(p0 + pb) * n + p0;
                    UnsafeUtility.MemClear(Yp, (long)rowsTrail * pb * UnsafeUtility.SizeOf<fProxy>());
            ...
            UnsafeUtility.MemClear(Q.Data.Ptr, (long)m * n * UnsafeUtility.SizeOf<fProxy>());
            for (int i = 0; i < m; i++)
                Q[i, i] = (fProxy)1;
            ...
            for (int p0 = lastP0; p0 >= 0; p0 -= LQ_BLOCK)
            {
                ...
                UnsafeUtility.MemClear(Yp, (long)rows * pb * UnsafeUtility.SizeOf<fProxy>());
        ...
        // rejected).
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static DirectSolveInfo decomp(in fProxyMxN A, ref fProxyMxN L, ref fProxyMxN Q)
        {
            // See lqDecompositionBlockedCore for why this is a method-local const, not a class field.
            const int LQ_BLOCK = 64;
            const int LQ_BLOCK_MIN_M = Consts.fProxyLqBlockMinM;   // float 256 / double 512 (see Consts)
            ...
            {
                var Vpanel = new fProxyN(LQ_BLOCK * n, Allocator.Temp, true);
                var Vt = new fProxyN(n * LQ_BLOCK, Allocator.Temp, true);
                var Tbuf = new fProxyN(LQ_BLOCK * LQ_BLOCK, Allocator.Temp, true);
                var Y = new fProxyN(m * LQ_BLOCK, Allocator.Temp, true);
                var tcolBuf = new fProxyN(LQ_BLOCK, Allocator.Temp, true);
        ...
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static DirectSolveInfo decomp(in fProxyMxN A, ref fProxyMxN L, ref fProxyMxN Q, ref fProxyLQCache ws)
        {
            int m = A.M_Rows;
            int n = A.N_Cols;
        ...
        static void lqFactorInPlace(ref fProxyMxN W, ref fProxyMxN L, ref fProxyN v, fProxy zeroThreshold)
        {
            const int LQ_BLOCK = 64;
            const int LQ_BLOCK_MIN_M = Consts.fProxyLqBlockMinM;   // float 256 / double 512 (see Consts)
            ...
            var Qnull = default(fProxyMxN);   // unused when reconstructQ is false
            if (m < LQ_BLOCK_MIN_M)
            {
        ...
        // internal (not private): shared with the row-pivoted LQRP class (see genHouseholderRow).
        internal static unsafe void applyQtFromReflectors(ref fProxyMxN W, ref fProxyN y, ref fProxyN x)
        {
            int m = W.M_Rows;
            int n = W.N_Cols;
            ...
            for (int d = m - 1; d >= 0; d--)
            {
                fProxy* vd = Wp + (long)d * n + d;   // reflector v_d over cols [d, n)
                fProxy* xd = xp + d;
                int len = n - d;
                fProxy dot = dot4(vd, xd, len);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static DirectSolveInfo minNormSolve(in fProxyMxN A, in fProxyN b, ref fProxyN x)
        {
            int m = A.M_Rows;
            int n = A.N_Cols;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static DirectSolveInfo minNormSolve(in fProxyMxN A, in fProxyN b, ref fProxyN x, ref fProxyLQMinNormCache ws)
        {
            int m = A.M_Rows;
            int n = A.N_Cols;
        ...
        public static DirectSolveInfo minNormSolveInPlace(ref fProxyMxN A, in fProxyN b, ref fProxyN x)
        {
            int m = A.M_Rows;
            int n = A.N_Cols;
        ...
        internal static unsafe void applyQtFromReflectors(ref fProxyMxN W, ref fProxyMxN Y, ref fProxyMxN X)
        {
            int m = W.M_Rows;
            int n = W.N_Cols;
            int k = Y.N_Cols;
            ...
            for (int j = 0; j < m; j++)
            {
                fProxy* Xr = Xp + (long)j * k;
                fProxy* Yr = Yp + (long)j * k;
                for (int c = 0; c < k; c++) Xr[c] = Yr[c];
            }
            for (int j = m; j < n; j++)
            {
                fProxy* Xr = Xp + (long)j * k;
                for (int c = 0; c < k; c++) Xr[c] = (fProxy)0;
            }
            ...
            for (int d = m - 1; d >= 0; d--)
            {
                fProxy* vd = Wp + (long)d * n;   // reflector v_d over cols [d, n): vd[c] = W[d,c]
        ...
        public static DirectSolveInfo minNormSolve(in fProxyMxN A, in fProxyMxN B, ref fProxyMxN X)
        {
            int m = A.M_Rows;
            int n = A.N_Cols;
            int k = B.N_Cols;
        ...
        /// <param name="B">Right-hand sides (m x k). Not modified.</param>
        /// <param name="X">Output only; prior contents ignored. Solution (n x k). Must not alias B.</param>
        public static DirectSolveInfo minNormSolveInPlace(ref fProxyMxN A, in fProxyMxN B, ref fProxyMxN X)
        {
            int m = A.M_Rows;
            int n = A.N_Cols;
            int k = B.N_Cols;
