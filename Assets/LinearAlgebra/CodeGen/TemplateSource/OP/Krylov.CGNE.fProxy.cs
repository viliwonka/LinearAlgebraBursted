...
namespace LinearAlgebra
{
    public static partial class Krylov {
        ...
        /// <see cref="IfProxyLinearOperator"/>. Solves A Aᵀ y = b by CG and forms x = Aᵀ y
            ...
            where TOp : struct, IfProxyLinearOperator
        {
            ...
            for (int k = 0; k < maxIter; k++)
            {
                ...
                if (!(pp > (fProxy)0)) // NaN-safe: pp is a norm, nonnegative
                ...
                A.Apply(in p, ref Ap);
                fProxy rrNew = Blas.updateXR(alpha, p, ref x, Ap, ref r);
        ...
        static LstsqInfo CGNEInfo<TOp>(IterativeSolveStatus status, int iterations, in TOp A, in fProxyN b,
        ...
        /// </summary>
        public static LstsqInfo cgne(in fProxyMxN A, in fProxyN b, ref fProxyN x,
                                ref fProxyN r, ref fProxyN p, ref fProxyN Ap, ref fProxyN tmpN,
                                int maxIter, fProxy tol)
        {
            return cgne(new fProxyDenseOperator(in A), in b, ref x, ref r, ref p, ref Ap, ref tmpN, maxIter, tol);
        }

        /// <summary>CGNE over a dense matrix -- allocates four scratch vectors from the arena.</summary>
        public static LstsqInfo cgne(in fProxyMxN A, in fProxyN b, ref fProxyN x, int maxIter, fProxy tol)
        {
        ...
        /// primitive. Forwards into <see cref="cgne{TOp}"/> via <c>fProxyBSROperator</c>:
        ...
        public static LstsqInfo cgne(in fProxyBSR A, in fProxyN b, ref fProxyN x,
                                ...
                                int maxIter, fProxy tol)
        {
            return cgne(new fProxyBSROperator(in A), in b, ref x, ref r, ref p, ref Ap, ref tmpN, maxIter, tol);
        }

        /// <summary>
        /// CGNE over a BSR matrix -- zero-alloc primitive variant that takes a CALLER-PROVIDED
        /// precomputed transpose AT (e.g. built once via <c>arena.fProxyBSRTranspose(in A)</c>
        ...
        /// </summary>
        public static LstsqInfo cgne(in fProxyBSR A, in fProxyBSR AT, in fProxyN b, ref fProxyN x,
                                ref fProxyN r, ref fProxyN p, ref fProxyN Ap, ref fProxyN tmpN,
                                int maxIter, fProxy tol)
        {
            return cgne(new fProxyBSROperator(in A, in AT), in b, ref x, ref r, ref p, ref Ap, ref tmpN, maxIter, tol);
        }

        /// <summary>
        /// CGNE over a BSR matrix -- allocates four scratch vectors AND materializes A^T ONCE via
        /// <c>arena.fProxyBSRTranspose</c>, then drives CGNE with the two-arg
        ...
        /// <see cref="cgne(in fProxyBSR, in fProxyBSR, in fProxyN, ref fProxyN, ref fProxyN, ref fProxyN, ref fProxyN, ref fProxyN, int, fProxy)"/>
        /// overload above with your own scratch vectors.
        /// </summary>
        public static LstsqInfo cgne(in fProxyBSR A, in fProxyN b, ref fProxyN x, int maxIter, fProxy tol)
        {
            fProxyN r    = b.fProxyTempVec(A.M_Rows);
            fProxyN p    = b.fProxyTempVec(A.N_Cols);
            fProxyN Ap   = b.fProxyTempVec(A.M_Rows);
