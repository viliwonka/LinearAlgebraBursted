...
namespace LinearAlgebra
{
    public static partial class LP
    {
        ...
        internal static LPInfo ladFrischNewtonCore(in fProxyMxN A, in fProxyN b, double tau,
            ...
            => ladFrischNewtonCore(new fProxyDenseLadDesign(in A), in b, tau, ref x, out objective, maxIter);

        /// <summary>
        /// Exact least-absolute-deviation (L1) regression over a block-sparse
        /// <see cref="fProxyBSR"/> design, by the same Frisch-Newton interior point as the dense
        /// <see cref="ladFN(in fProxyMxN, in fProxyN, ref fProxyN, out double, int)"/>. Every iteration
        ...
        /// <param name="objective">Output L1 residual ‖A x − b‖₁.</param>
        /// <param name="maxIter">Iteration budget; ≤0 picks the default (50).</param>
        public static LPInfo ladFN(in fProxyBSR A, in fProxyN b, ref fProxyN x, out double objective,
                                   int maxIter = 0)
        {
            ...
            return ladFrischNewtonCore(new fProxyBsrLadDesign(in A), in b, 0.5, ref x, out objective, maxIter);
        }

        /// <summary>
        /// Quantile regression over a block-sparse <see cref="fProxyBSR"/> design -- the sparse twin of
        /// <see cref="ladFN(in fProxyMxN, in fProxyN, double, ref fProxyN, out double, int)"/>.
        /// τ = 0.5 is median regression.
        /// </summary>
        /// <param name="tau">Quantile level, strictly inside (0, 1).</param>
        public static LPInfo ladFN(in fProxyBSR A, in fProxyN b, double tau, ref fProxyN x,
                                   out double objective, int maxIter = 0)
        {
            if (b.N != A.M_Rows) throw new ArgumentException("LP.ladFN(BSR): b.N must equal A.M_Rows");
            if (x.N != A.N_Cols) throw new ArgumentException("LP.ladFN(BSR): x.N must equal A.N_Cols");
        ...
        internal static LPInfo ladFrischNewtonCore<TDesign>(in TDesign design, in fProxyN b, double tau,
                                                            ref fProxyN x, out double objective, int maxIter)
            where TDesign : struct, IfProxyLadDesign
        {
            ...
            var bLP = new fProxyN(n, Allocator.Temp);         // dual equality RHS, Aᵀa at the initial a
