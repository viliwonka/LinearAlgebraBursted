...
namespace LinearAlgebra
{
    public static partial class LP
    {
        ...
        // max_it), duality-gap tolerance derived from Consts.fProxySqrtEps (double: ~1.49e-8, matching
                                                   ...
                                                   ref fProxyN x, out double objective, int maxIter)
        {
            ...
            while (status != LPStatus.Optimal && iters < budget)
            {
                ...
                for (int i = 0; i < m; i++) q[i] = (fProxy)1 / (z[i] / a[i] + w[i] / s[i]);
        ...
        static void BuildATQA(fProxyMxN A, fProxyN q, fProxyMxN M, int m, int n, fProxy reg)
        {
            for (int r = 0; r < n; r++)
                for (int c = r; c < n; c++)
                    M[r, c] = (fProxy)0;
