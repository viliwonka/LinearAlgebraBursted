...
namespace LinearAlgebra
{
    ...
    public static partial class Cholesky {
        ...
        /// </summary>
        public static DirectSolveInfo choleskyDecomposition(in fProxyMxN A, ref fProxyMxN L) {
            if (!A.IsSquare)
            ...
            unsafe
            {
                ...
                for (int i = 0; i < n; i++)
                {
                    ...
                    for (int j = i + 1; j < n; j++) L[i, j] = (fProxy)0;
                }

                var lj = new fProxyN(n, Allocator.Temp, false);
                fProxy* ljp = lj.Data.Ptr;
                ...
                if (n < CHOL_BLOCK_MIN_N)
                {
                    ...
                    for (int j = 0; j < n; j++)
                    {
                        ...
                        // sqrt.
                        fProxy d = L[j, j];
                        if (!(d > (fProxy)0)) { lj.Dispose(); return new DirectSolveInfo { status = DirectSolveStatus.NotPositiveDefinite }; }

                        fProxy Ljj = math.sqrt(d);
                        L[j, j] = Ljj;
                        fProxy inv = (fProxy)1 / Ljj;
                ...
                var PT = new fProxyN(CHOL_BLOCK * n, Allocator.Temp, false);
                fProxy* ptp = PT.Data.Ptr;
                ...
                for (int j0 = 0; j0 < n; j0 += CHOL_BLOCK)
                {
                    ...
                    {
                        fProxy d = L[j, j];
                        if (!(d > (fProxy)0)) { PT.Dispose(); lj.Dispose(); return new DirectSolveInfo { status = DirectSolveStatus.NotPositiveDefinite }; }
        ...
        /// always check Solved.
        /// </summary>
        public static DirectSolveInfo choleskySolve(ref fProxyMxN L, ref fProxyN b) {
            if (!L.IsSquare)
        ...
        /// </summary>
        public static RankRevealingInfo choleskyDecompositionPivot(in fProxyMxN A, ref fProxyMxN L, ref Pivot P,
                                                       ref fProxyCholeskyPivotCache ws) {
            if (!A.IsSquare)
        ...
        public static RankRevealingInfo choleskyDecompositionPivot(in fProxyMxN A, ref fProxyMxN L, ref Pivot P) {
            ...
            var ws = new fProxyCholeskyPivotCache
            {
        ...
        /// not re-verify them.
        /// </summary>
        public static DirectSolveInfo choleskyPivotSolve(ref fProxyMxN L, in Pivot P, int rank, ref fProxyN b,
                                              ref fProxyCholeskyPivotCache ws) {
            if (!L.IsSquare)
        ...
        /// </summary>
        public static RankRevealingInfo choleskyPivotSolve(in fProxyMxN A, ref fProxyMxN L, ref Pivot P, ref fProxyN b) {
