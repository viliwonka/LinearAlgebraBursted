...
namespace LinearAlgebra
{
    public static partial class LP
    {
        ...
        // using ONLY A·v / Aᵀ·v (matrix-free over any IfProxyLinearOperator) plus elementwise clamps --
        ...
        // applied matrix-free via fProxyRowColScaledOperator). A BSR overload + large-sparse benchmark is
        ...
        static double pdlpKkt<TOp>(in TOp A, in fProxyN x, in fProxyN y,
                                   in fProxyN lc, in fProxyN uc, in fProxyN lv, in fProxyN uv, in fProxyN c,
                                   ref fProxyN Ax, ref fProxyN Aty, double bScale, double cScale)
        ...
        static LPInfo pdlpCore<TOp>(in TOp A, in fProxyN lc, in fProxyN uc, in fProxyN lv, in fProxyN uv,
                                    in fProxyN c, ref fProxyN x, out double objective, int maxIter, double epsOpt)
            where TOp : struct, IfProxyLinearOperator
        {
            int m = A.Rows, n = A.Cols;

            var y    = new fProxyN(m, Allocator.Temp);   // dual (row multipliers); Temp => starts at 0
            var xnew = new fProxyN(n, Allocator.Temp);   // trial x⁺
            var ynew = new fProxyN(m, Allocator.Temp);   // trial y⁺
            var Ax   = new fProxyN(m, Allocator.Temp);   // A·x  (cached across iterations)
            var Axp  = new fProxyN(m, Allocator.Temp);   // A·x⁺ (trial; also kkt scratch)
            var Aty  = new fProxyN(n, Allocator.Temp);   // Aᵀ·y (reused across step-size retries)
            var sumX = new fProxyN(n, Allocator.Temp);   // running-average accumulators
            var sumY = new fProxyN(m, Allocator.Temp);
            var avgX = new fProxyN(n, Allocator.Temp);
            var avgY = new fProxyN(m, Allocator.Temp);
            var xPrev = new fProxyN(n, Allocator.Temp);  // previous restart point (drives the primal weight)
            var yPrev = new fProxyN(m, Allocator.Temp);
            var xBest = new fProxyN(n, Allocator.Temp);  // best (lowest-μ) average seen -- reported at the end
            ...
            fProxy normA;
            {
                fProxy inv = (fProxy)(1.0 / math.sqrt((double)n));
                ...
                for (int k = 0; k < 20; k++)
                {
                    ...
                    if (!(nrm > 0)) break;
                    lam = nrm;
                    fProxy s = (fProxy)(1.0 / nrm);
                    for (int j = 0; j < n; j++) xnew[j] = Aty[j] * s;
                }
                normA = (fProxy)math.sqrt(lam);
            }
            ...
            while (status != LPStatus.Optimal && iters < budget)
            {
                ...
                {
                    fProxy tau = (fProxy)(etaTry / omega), sigma = (fProxy)(etaTry * omega);
                    ...
                    for (int i = 0; i < m; i++)
                    {
                        fProxy aext = (fProxy)2 * Axp[i] - Ax[i];
                        fProxy w = y[i] + sigma * aext;
                ...
                if ((iters & 63) == 0)
                {
                    fProxy invc = (fProxy)(1.0 / (double)cnt);
                    ...
                    if (restart)
                    {
                        ...
                        for (int j = 0; j < n; j++) { x[j] = avgX[j]; xPrev[j] = avgX[j]; sumX[j] = (fProxy)0; }
                        for (int i = 0; i < m; i++) { y[i] = avgY[i]; yPrev[i] = avgY[i]; sumY[i] = (fProxy)0; }
        ...
        static void pdlpEquilibrateDense(in fProxyMxN A, ref fProxyN Dr, ref fProxyN Dc)
        {
            int m = A.M_Rows, n = A.N_Cols;
            for (int i = 0; i < m; i++) Dr[i] = (fProxy)1;
            for (int j = 0; j < n; j++) Dc[j] = (fProxy)1;
            ...
            for (int it = 0; it < 10; it++)
            {
                for (int i = 0; i < m; i++)                       // row inf-norms of the current Â
                {
                    ...
                    if (R > 1e-30) Dr[i] = (fProxy)((double)Dr[i] / math.sqrt(R));
                }
                for (int j = 0; j < n; j++)                       // column inf-norms (using the updated Dr)
                {
                    ...
                    if (C > 1e-30) Dc[j] = (fProxy)((double)Dc[j] / math.sqrt(C));
                }
            }
            ...
            for (int i = 0; i < m; i++)                           // Pock-Chambolle: 1/sqrt(row 1-norm of Â)
            {
                ...
                if (s > 1e-30) Dr[i] = (fProxy)((double)Dr[i] / math.sqrt(s));
            }
            for (int j = 0; j < n; j++)                           // 1/sqrt(column 1-norm of Â)
            {
                double s = 0;
                for (int i = 0; i < m; i++) s += math.abs((double)A[i, j]) * (double)Dr[i] * (double)Dc[j];
                if (s > 1e-30) Dc[j] = (fProxy)((double)Dc[j] / math.sqrt(s));
            }

            for (int i = 0; i < m; i++) Dr[i] = (fProxy)math.clamp((double)Dr[i], 1e-12, 1e12);
            for (int j = 0; j < n; j++) Dc[j] = (fProxy)math.clamp((double)Dc[j], 1e-12, 1e12);
        }

        // Solve the equilibrated problem on Â = Dr·A·Dc (matrix-free, via fProxyRowColScaledOperator) and
        ...
        static LPInfo pdlpScaledSolve<TOp>(in TOp inner, in fProxyN Dr, in fProxyN Dc,
                                           in fProxyN lc, in fProxyN uc, in fProxyN lv, in fProxyN uv, in fProxyN c,
                                           ref fProxyN x, out double objective, int maxIter, double epsOpt)
            where TOp : struct, IfProxyLinearOperator
        {
            int m = inner.Rows, n = inner.Cols;
            var lcS = new fProxyN(m, Allocator.Temp);
            var ucS = new fProxyN(m, Allocator.Temp);
            var lvS = new fProxyN(n, Allocator.Temp);
            var uvS = new fProxyN(n, Allocator.Temp);
            var cS  = new fProxyN(n, Allocator.Temp);
            var scratchN = new fProxyN(n, Allocator.Temp);
            var scratchM = new fProxyN(m, Allocator.Temp);

            for (int i = 0; i < m; i++)
            {
                lcS[i] = math.abs((double)lc[i]) >= 1e29 ? lc[i] : (fProxy)((double)lc[i] * (double)Dr[i]);
                ucS[i] = math.abs((double)uc[i]) >= 1e29 ? uc[i] : (fProxy)((double)uc[i] * (double)Dr[i]);
            }
            for (int j = 0; j < n; j++)
            {
                lvS[j] = math.abs((double)lv[j]) >= 1e29 ? lv[j] : (fProxy)((double)lv[j] / (double)Dc[j]);
                uvS[j] = math.abs((double)uv[j]) >= 1e29 ? uv[j] : (fProxy)((double)uv[j] / (double)Dc[j]);
                cS[j]  = (fProxy)((double)c[j] * (double)Dc[j]);
                x[j]   = (fProxy)((double)x[j] / (double)Dc[j]);          // x̂ = x / Dc
            }

            var op = new fProxyRowColScaledOperator<TOp>(in inner, in Dr, in Dc, in scratchN, in scratchM);
            var info = pdlpCore(in op, in lcS, in ucS, in lvS, in uvS, in cS, ref x, out double _, maxIter, epsOpt);

            objective = 0;
            for (int j = 0; j < n; j++)
            {
                x[j] = (fProxy)((double)Dc[j] * (double)x[j]);           // x = Dc · x̂
        ...
        /// with the solution.
        /// </summary>
        public static LPInfo pdlp(in fProxyMxN A, in fProxyN lc, in fProxyN uc, in fProxyN lv, in fProxyN uv,
                                  in fProxyN c, ref fProxyN x, out double objective, int maxIter = 0, double epsOpt = 1e-6)
        {
            int m = A.M_Rows, n = A.N_Cols;
            ...
            var Dr = new fProxyN(m, Allocator.Temp);
