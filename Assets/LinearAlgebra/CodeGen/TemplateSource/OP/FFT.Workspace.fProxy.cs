...
namespace LinearAlgebra
{
    ...
    {
        public fProxyN twReFull;   // length n:   cos(-2π·m/n), m = 0..n-1    (full circle)
        public fProxyN twImFull;   // length n:   sin(-2π·m/n); the half-circle uses (rfft/irfft
    ...
    public static partial class FFT
    {
        ...
        public static void rfft(in fProxyN real, ref fProxyN re, ref fProxyN im, in fProxyFFTCache ws)
        {
            ...
            for (int k = 1; k < M; k++)
            {
                ...
                fProxy curRe = twReFull[k];   // W_N^k real part (first half of full table)
                fProxy curIm = twImFull[k];   // W_N^k imaginary part
        ...
        public static void irfft(in fProxyN re, in fProxyN im, ref fProxyN real, in fProxyFFTCache ws)
        {
            ...
            for (int k = 1; k < M; k++)
            {
                ...
                fProxy cRe = twReFull[k];
                fProxy cIm = -twImFull[k];   // negate imaginary part for conjugate
