    ...
    public static partial class Cholesky
    {
                                                  ...
                                                  bool needW, bool needBt)
        {
            if (needW && (ws.W.M_Rows != n || ws.W.N_Cols != n))
                throw new ArgumentException("Cholesky: workspace W must be n x n (use Arena.fProxyCholeskyPivot_WS(n))");
            if (needBt && ws.bt.N != n)
                throw new ArgumentException("Cholesky: workspace bt must have length n (use Arena.fProxyCholeskyPivot_WS(n))");
        }
    }
