...
namespace LinearAlgebra
{
    ...
    public static partial class LP
    {
        ...
        // through UnsafeOP.matVecDot (two fProxy4 SIMD accumulators per row, [NoAlias] pointers); outv
        // is zeroed first since matVecDot ACCUMULATES.
        internal static unsafe void Mmul(fProxyMxN M, fProxyN v, fProxyN outv, int m, int N)
        {
        ...
        // an approximation.
        internal static unsafe void ApplyEtaForward(fProxyMxN etaAlpha, int row, int slot, fProxyN v, int m)
        {
        ...
        {
            fProxy* etaRow = etaAlpha.Data.Ptr + (long)slot * etaAlpha.N_Cols;
            fProxy full = UnsafeOP.vecDot(etaRow, v.Data.Ptr, m);
            fProxy t = full - etaRow[row] * v[row];
        ...
        internal static int SelectEntering(fProxyMxN M, fProxyN y, fProxyN cost, bool phase1,
                                           ...
                                           int N, int m, bool useBland, fProxy tol, fProxyN dWork, out int sigma, out fProxy dj)
        {
            sigma = 0; dj = (fProxy)0;
        ...
        internal static void RebuildXB(fProxyMxN M, fProxyN rhs, NativeArray<byte> status,
                                       ...
                                       int m, int N, fProxyN xB)
        {
            var valVec = new fProxyN(N, Allocator.Temp);
            for (int j = 0; j < N; j++)
                valVec[j] = status[j] == STATUS_BASIC ? (fProxy)0 : (status[j] == STATUS_AT_LOWER ? lower[j] : upper[j]);
        ...
        // forwards here. LP.DualSimplex.fProxy.cs also hands its own terminal basis to this core as a
                                                 ...
                                                 NativeArray<int> basis, NativeArray<byte> status)
        {
            ...
            var dWork = new fProxyN(N, Allocator.Temp);   // SelectEntering's per-iteration MTmul scratch
