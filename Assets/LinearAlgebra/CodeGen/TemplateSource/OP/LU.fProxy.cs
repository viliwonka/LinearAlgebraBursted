...
namespace BULA
{
    public static partial class LU {
        ...
        public static DirectSolveInfo decompInPlace(ref fProxyMxN A_to_LU, ref Pivot P) {
            ...
            const int LU_BLOCK_MIN_N = Consts.fProxyLuBlockMinN;
            ...
            unsafe
            {
                ...
                for (int k0 = 0; k0 < m - 1; k0 += LU_BLOCK) {
                    ...
                    if (rStart < m) {
                        ...
                        for (int r = 1; r < kb; r++) {
                            int Pr = P[k0 + r];
                            fProxy* uR = up + (long)Pr * m + rStart;
                            for (int p = 0; p < r; p++) {
                                fProxy Lrp = A_to_LU[Pr, k0 + p];
                                int Pp = P[k0 + p];
                                fProxy* uP = up + (long)Pp * m + rStart;
                                UnsafeOP.axpy(uR, uP, -Lrp, ntrail);
                            }
                        }
                        ...
                        for (int t = 0; t < ntrail; t++) {
                            ...
                            int Pt = P[rStart + t];
                            fProxy* crow = up + (long)Pt * m + rStart;
                            fProxy* Lrow = up + (long)Pt * m + k0;   // L21 multipliers for this row

                            int p = 0;
                            for (; p + 4 <= kb; p += 4) {            // one crow pass per FOUR panel columns
                                fProxy* w0 = ubufp + (long)p * ntrail;
        ...
        public static DirectSolveInfo decompSolve(ref fProxyMxN LU, in Pivot pivot, ref fProxyMxN B_to_X) {
        ...
        /// </summary>
        /// <param name="B_to_X">On entry B (n rows x k cols); on exit the solution X.</param>
        public static DirectSolveInfo decompSolve(ref fProxyMxN L, ref fProxyMxN U, in Pivot pivot, ref fProxyMxN B_to_X) {
        ...
        /// <param name="B_to_X">On entry B (n rows x k cols); on exit the solution X.</param>
        public static DirectSolveInfo solveInPlace(ref fProxyMxN A_to_LU, ref Pivot P, ref fProxyMxN B_to_X) {
            if (!A_to_LU.IsSquare)
                throw new System.ArgumentException("solveInPlace: A_to_LU needs to be square");
