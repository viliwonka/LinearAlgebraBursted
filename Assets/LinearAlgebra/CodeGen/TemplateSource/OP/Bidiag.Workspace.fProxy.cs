...
namespace LinearAlgebra
{
    public static partial class Bidiag
    {
        ...
        /// <see cref="bidiagonalizeValues"/>. Matches Arena.fProxyBidiagWorkspace(m, n).
        /// </summary>
        static void RequireBidiagWorkspace(in fProxyBidiagWorkspace ws, int m, int n, bool needLeftU, string who)
        {
            bool ok =
            ...
            if (!ok)
                throw new ArgumentException(who + ": workspace must be sized for m x n (use Arena.fProxyBidiagWorkspace(m, n))");
        }
    }

    /// <summary>
    /// Reusable scratch for Golub-Kahan-Householder bidiagonalization (Bidiag.bidiagonalize /
    /// bidiagonalizeValues). Allocate ONCE via Arena.fProxyBidiagWorkspace(m, n) and reuse it across
    ...
    /// </summary>
    public struct fProxyBidiagWorkspace
    {
        public fProxyMxN W;
    ...
    public partial struct Arena
    {
        ...
        /// </summary>
        public fProxyBidiagWorkspace fProxyBidiagWorkspace(int m, int n)
        {
            return new fProxyBidiagWorkspace
            {
                W = fProxyMat(m, n),
                leftU = fProxyMat(m, n),
                uVec = fProxyVec(m),
