...
namespace LinearAlgebra
{
    ...
    public static partial class Eigen
    {
            ...
            where TPre : struct, IfProxyPreconditioner
        {
            ...
            // Consts.fProxyEpsilon B-norm^2 is reseeded first (deterministic, distinct seed from the
            // other reseed sites).
            for (int i = 0; i < kWork; i++)
            {
                fProxy bn2 = (fProxy)0;
            ...
            for (int iter = 0; iter < maxIter; iter++)
            {
                ...
                // whose B-norm^2 is at/below Consts.fProxyEpsilon is reseeded first (deterministic,
        ...
        // <c>lobpcg(in A, in B, new fProxyIdentityPreconditioner(), ref ws, k, tol, maxIter)</c>.
        ...
        /// <c>fProxyBSROperator</c>. This is the preconditioned entry point the sparse-BSR
        /// eigensolver roadmap calls out (matvec + block-Jacobi, matching how
        /// <see cref="Krylov.cg{TPre}(in fProxyBSR, in TPre, in fProxyN, ref fProxyN)"/>
        /// consumes it).
        /// </summary>
        public static LOBPCGInfo lobpcg<TPre>(in fProxyBSR A, in TPre M, ref fProxyLOBPCGCache ws,
                                         int k, fProxy tol, int maxIter)
            where TPre : struct, IfProxyPreconditioner
        ...
        public static LOBPCGInfo lobpcg<TPre>(in fProxyBSR A, in TPre M, ref fProxyLOBPCGCache ws, int k, fProxy tol)
            where TPre : struct, IfProxyPreconditioner
            => lobpcg(in A, in M, ref ws, k, tol, 1000);

        /// <summary>lobpcg (BSR + preconditioner) with default tol (Consts.fProxySqrtEps) and maxIter (1000).</summary>
        public static LOBPCGInfo lobpcg<TPre>(in fProxyBSR A, in TPre M, ref fProxyLOBPCGCache ws, int k)
            where TPre : struct, IfProxyPreconditioner
            => lobpcg(in A, in M, ref ws, k, Consts.fProxySqrtEps, 1000);

        /// <summary>
        /// LOBPCG over a block-sparse pencil (A, B) -- GENERALIZED eigenproblem -- with ANY
        /// <see cref="IfProxyPreconditioner"/> (block-Jacobi) built from A. Forwards into
        /// <see cref="lobpcg{TOp,TBOp,TPre}"/> via <c>fProxyBSROperator</c>. Note the preconditioner
        ...
        /// </summary>
        public static LOBPCGInfo lobpcg<TPre>(in fProxyBSR A, in fProxyBSR B, in TPre M, ref fProxyLOBPCGCache ws,
                                         int k, fProxy tol, int maxIter)
            where TPre : struct, IfProxyPreconditioner
        ...
        public static LOBPCGInfo lobpcg<TPre>(in fProxyBSR A, in fProxyBSR B, in TPre M, ref fProxyLOBPCGCache ws, int k, fProxy tol)
            where TPre : struct, IfProxyPreconditioner
            => lobpcg(in A, in B, in M, ref ws, k, tol, 1000);

        /// <summary>lobpcg (generalized, BSR + preconditioner) with default tol (Consts.fProxySqrtEps) and maxIter (1000).</summary>
        public static LOBPCGInfo lobpcg<TPre>(in fProxyBSR A, in fProxyBSR B, in TPre M, ref fProxyLOBPCGCache ws, int k)
            where TPre : struct, IfProxyPreconditioner
            => lobpcg(in A, in B, in M, ref ws, k, Consts.fProxySqrtEps, 1000);

        /// <summary>lobpcg (allocating) over a BSR matrix with ANY <see cref="IfProxyPreconditioner"/>
        /// (block-Jacobi). See the dense overload's doc comment.</summary>
        public static fProxyN lobpcg<TPre>(ref Arena arena, in fProxyBSR A, in TPre M, int k,
        ...
        /// </summary>
        public static fProxyN lobpcg<TPre>(ref Arena arena, in fProxyBSR A, in fProxyBSR B, in TPre M, int k,
                                      out fProxyMxN eigenvectors, out LOBPCGInfo info, fProxy tol, int maxIter)
            where TPre : struct, IfProxyPreconditioner
        {
            var ws = arena.fProxyLOBPCGCache(A.M_Rows, k);
        ...
        static int OrthonormalizeBlockB(ref fProxyMxN V, ref fProxyMxN AV, ref fProxyMxN BV, int rows, int n,
                                         ref fProxyMxN Gram, ref fProxyMxN Z, ref fProxyMxN scratch)
        {
            if (rows <= 0) return 0;
            ...
            var D = new fProxyN(rows, Allocator.Temp);
            for (int i = 0; i < rows; i++)
            {
                fProxy gi = G[i, i];
                D[i] = gi > (fProxy)0 ? (fProxy)1 / math.sqrt(gi) : (fProxy)0;
            }
            ...
            var theta = new fProxyN(rows, Allocator.Temp);
            ...
            if (eigOk)
            {
                fProxy thetaMax = theta[0];
                if (thetaMax > (fProxy)0)
                {
                    fProxy dropFloor = thetaMax * ((fProxy)rows * Consts.fProxyEpsilon * (fProxy)10);
        ...
        static void SvqbAccumulate(in fProxyMxN Src, ref fProxyMxN Dst, in fProxyN D, in fProxyMxN Zv, in fProxyN theta,
                                    int rows, int kept, int n)
        {
            ...
            for (int j = 0; j < kept; j++)
            {
                fProxy invSqrtTheta = (fProxy)1 / math.sqrt(theta[j]);
                for (int i = 0; i < rows; i++)
                {
                    fProxy coef = D[i] * Zv[i, j] * invSqrtTheta;
                    for (int c = 0; c < n; c++)
        ...
        static unsafe void FillGramSub(ref fProxyMxN Gram, int rowOff, in fProxyMxN Vb, int rows,
                                 int colOff, in fProxyMxN Wb, int cols, int n, bool sameBlock)
        {
            fProxy* vp = Vb.Data.Ptr; int vnc = Vb.N_Cols;
            fProxy* wp = Wb.Data.Ptr; int wnc = Wb.N_Cols;
            for (int i = 0; i < rows; i++)
            {
                int jStart = sameBlock ? i : 0;
                for (int j = jStart; j < cols; j++)
                {
                    fProxy s = UnsafeOP.vecDot(vp + (long)i * vnc, wp + (long)j * wnc, n);
        ...
        static unsafe void FillHSub(ref fProxyMxN H, int rowOff, in fProxyMxN Vb, int rows,
                              int colOff, in fProxyMxN AWb, int cols, int n, bool sameBlock)
        {
            fProxy* vp = Vb.Data.Ptr; int vnc = Vb.N_Cols;
            fProxy* wp = AWb.Data.Ptr; int wnc = AWb.N_Cols;
            for (int i = 0; i < rows; i++)
            {
                int jStart = sameBlock ? i : 0;
                for (int j = jStart; j < cols; j++)
        ...
        // bit-for-bit.
        static unsafe void Deflate(ref fProxyMxN V, ref fProxyMxN AV, ref fProxyMxN BV, int activeCount,
                             in fProxyMxN Against, in fProxyMxN AgainstA, in fProxyMxN AgainstB, int againstStart, int againstCount, int n)
        {
            ...
            for (int pass = 0; pass < 2; pass++)
                for (int a = 0; a < activeCount; a++)
                    ...
                    {
                        fProxy coeff = UnsafeOP.vecDot(vpAB + (long)i * vncAB, vpV + (long)a * vncV, n);
        ...
        static void BuildProjected(ref fProxyMxN Gram, ref fProxyMxN H, int nx, int nw, int np, bool useP,
        ...
        static bool FactorGram(ref fProxyMxN Gram, ref fProxyMxN L, int m, fProxy tol, bool cubeRule)
        {
        ...
        // Seed / internal-block gate: linear pivot-ratio threshold Consts.fProxySqrtEps. A
        ...
        // CHO.fProxy.cs's CHOL_BLOCK comment).
        static bool FactorGram(ref fProxyMxN Gram, ref fProxyMxN L, int m)
            => FactorGram(ref Gram, ref L, m, Consts.fProxySqrtEps, false);
        ...
        static bool FactorGramCombined(ref fProxyMxN Gram, ref fProxyMxN L, int m)
            => FactorGram(ref Gram, ref L, m, Consts.fProxyEpsilon * (fProxy)10, true);

        static bool PivotGateOk(fProxy ratio, fProxy tol, bool cubeRule)
        ...
        static bool TryRayleighRitz(ref fProxyLOBPCGCache ws, int numActive, int nw, int np, bool useP, int n)
        {
            int npEff = useP ? np : 0;
        ...
        static void UpdateActiveBlock(ref fProxyLOBPCGCache ws, int numActive, int nw, int np, bool useP, int n, int k)
        {
            int npEff = useP ? np : 0;
            int m = numActive + nw + npEff;
