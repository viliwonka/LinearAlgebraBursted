...
namespace LinearAlgebra
{
    ...
    public static partial class Control
    {
        ...
        static void ValidateLQRDims(in fProxyMxN A, in fProxyMxN B, in fProxyMxN Q, in fProxyMxN R,
                                    string ctx, out int n, out int m)
        {
            ...
            {
                fProxy rd = R[i, i];
                if (!math.isfinite(rd) || rd < (fProxy)0)
                    throw new ArgumentException(ctx + ": R diagonal must be finite and non-negative");
            }
        }

        // ---- scalar control math (double locals only -- never stored back into fProxy) ----

        // internal (not private): reused by Kalman.fProxy.cs's steadyStateGain to data-scale the
        // filter DARE before handing it to SDACore (see that method's own comment for why).
        internal static double FrobeniusNorm(in fProxyMxN M)
        {
            double s = 0;
            unsafe
            {
        ...
        static double FrobeniusNormDiff(in fProxyMxN A, in fProxyMxN B)
        {
            double s = 0;
            unsafe
            {
                fProxy* pa = A.Data.Ptr;
                fProxy* pb = B.Data.Ptr;
        ...
        static double BlowupThreshold(in fProxyMxN Q, in fProxyMxN R) =>
            BLOWUP_FACTOR * (1.0 + FrobeniusNorm(in Q) + FrobeniusNorm(in R));

        // internal (not private): reused by Kalman.fProxy.cs's PredictCovarianceCore/UpdateCore --
        ...
        internal static RankInfo RiccatiStep(in fProxyMxN A, in fProxyMxN B, in fProxyMxN Q, in fProxyMxN R,
                                             in fProxyMxN S, ref fProxyMxN Snext, ref fProxyMxN K)
        {
            ...
            if (rinfo.Solved)
            {
                ...
                var BSATK = new fProxyMxN(n, n, Allocator.Temp);
        ...
        internal static LQRInfo SDACore(in fProxyMxN A, in fProxyMxN B, in fProxyMxN Q, in fProxyMxN R,
                                        ref fProxyMxN S, int maxIterations)
        {
            int n = A.M_Rows, m = B.N_Cols;

            var Ak = new fProxyMxN(in A, Allocator.Temp);
            var Gk = new fProxyMxN(n, n, Allocator.Temp);
            var Hk = new fProxyMxN(in Q, Allocator.Temp);
            ...
            var RL = new fProxyMxN(m, m, Allocator.Temp);
            ...
            else
            {
                ...
                var RinvBt = new fProxyMxN(m, n, Allocator.Temp);
                ...
                double tol = (double)Consts.fProxySqrtEps;
                int budget = maxIterations > 0 ? maxIterations : SDA_MAX_ITER;

                var GH = new fProxyMxN(n, n, Allocator.Temp);
                var HG = new fProxyMxN(n, n, Allocator.Temp);
                var X1 = new fProxyMxN(n, n, Allocator.Temp);
                var X2 = new fProxyMxN(n, n, Allocator.Temp);
                var X3 = new fProxyMxN(n, n, Allocator.Temp);
                var AkNext = new fProxyMxN(n, n, Allocator.Temp);
                var AG = new fProxyMxN(n, n, Allocator.Temp);
                var GkNext = new fProxyMxN(n, n, Allocator.Temp);
                var HkNext = new fProxyMxN(n, n, Allocator.Temp);
                ...
                while (iters < budget)
                {
                    ...
                    for (int i = 0; i < n; i++) { GH[i, i] += (fProxy)1; HG[i, i] += (fProxy)1; }
        ...
        static LQRInfo SolveCold(in fProxyMxN A, in fProxyMxN B, in fProxyMxN Q, in fProxyMxN R,
                                 ref fProxyMxN K, ref fProxyMxN S, int maxIterations)
        {
            var info = SDACore(in A, in B, in Q, in R, ref S, maxIterations);

            var Snext = new fProxyMxN(A.M_Rows, A.M_Rows, Allocator.Temp);
        ...
        internal static LQRInfo RiccatiIterate(in fProxyMxN A, in fProxyMxN B, in fProxyMxN Q, in fProxyMxN R,
                                               in fProxyMxN S0, ref fProxyMxN Sresult, ref fProxyMxN K, int maxIterations)
        {
            var Scur = new fProxyMxN(in S0, Allocator.Temp);
            var Snext = new fProxyMxN(A.M_Rows, A.M_Rows, Allocator.Temp);
            var Kscratch = new fProxyMxN(K.M_Rows, K.N_Cols, Allocator.Temp);
        ...
        public static LQRInfo lqr(in fProxyMxN A, in fProxyMxN B, in fProxyMxN Q, in fProxyMxN R,
                                  ref fProxyMxN K, int maxIterations = 0)
        {
        ...
        /// </summary>
        /// <param name="state">Must be constructed via <c>new fProxyLQRState(n, allocator)</c> before
        ...
        public static LQRInfo lqr(in fProxyMxN A, in fProxyMxN B, in fProxyMxN Q, in fProxyMxN R,
                                  ref fProxyMxN K, ref fProxyLQRState state, int maxIterations = 0)
        {
            ValidateLQRDims(in A, in B, in Q, in R, "Control.lqr", out int n, out int m);
            if (K.M_Rows != m || K.N_Cols != n)
                throw new ArgumentException("Control.lqr: K must be m x n");
        ...
        /// through the LQR/KF duality mapping (see Kalman.fProxy.cs's file header). The separation
        ...
        public static LQGInfo lqg(in fProxyMxN A, in fProxyMxN B, in fProxyMxN H,
                                  in fProxyMxN Qlqr, in fProxyMxN Rlqr,
                                  in fProxyMxN Qkf, in fProxyMxN Rkf,
                                  ref fProxyMxN Klqr, ref fProxyMxN Kkf, int maxIterations = 0)
        {
