...
namespace LinearAlgebra
{
    public static partial class Krylov {
        ...
        /// the preconditioner (<see cref="IfProxyPreconditioner"/>). Builds ONE shared block Krylov
            ...
            where TPre : struct, IfProxyPreconditioner
        {
            ...
            fProxyMxN Zt = default, Zcombo = default;
            fProxyN rowIn = default, rowOut = default;
            if (!M.IsIdentity)
            {
                Zt     = new fProxyMxN(s, n, Allocator.Temp, true);
                Zcombo = new fProxyMxN(s, n, Allocator.Temp, true);
                rowIn  = new fProxyN(n);
            ...
            while (total < maxIter)
            {
                ...
                // invariant: total < maxIter held at cycle entry, so the for-loop body runs at least once).
                {
                    ...
                    else
                    {
                        ...
                        BlockAdd(ref X, in Zcombo, (fProxy)1);
                    }
                }
        ...
        public static BlockSolveInfo bgmres<TOp>(in TOp A, in fProxyMxN B, ref fProxyMxN X,
            ...
            where TOp : struct, IfProxyLinearOperator
        {
            return bgmres(in A, default(fProxyIdentityPreconditioner), in B, ref X, restart, maxIter, tol);
        }

        /// <summary>Block GMRES(m) over a dense NON-symmetric <see cref="fProxyMxN"/> A, via
        ...
        public static BlockSolveInfo bgmres(in fProxyMxN A, in fProxyMxN B, ref fProxyMxN X,
                                        int restart, int maxIter, fProxy tol)
            => bgmres(new fProxyDenseOperatorGeneral(in A), in B, ref X, restart, maxIter, tol);
        ...
        public static BlockSolveInfo bgmres(in fProxyMxN A, in fProxyMxN B, ref fProxyMxN X)
            => bgmres(new fProxyDenseOperatorGeneral(in A), in B, ref X, math.min(30, A.M_Rows), A.M_Rows, Consts.fProxySqrtEps);
        ...
        public static BlockSolveInfo bgmres(in fProxyBSR A, in fProxyMxN B, ref fProxyMxN X,
                                        int restart, int maxIter, fProxy tol)
            => bgmres(new fProxyBSROperator(in A), in B, ref X, restart, maxIter, tol);

        /// <summary>Block GMRES over a BSR non-symmetric A with defaults (restart = min(30, A.M_Rows)).</summary>
        public static BlockSolveInfo bgmres(in fProxyBSR A, in fProxyMxN B, ref fProxyMxN X)
            => bgmres(new fProxyBSROperator(in A), in B, ref X, math.min(30, A.M_Rows), A.M_Rows, Consts.fProxySqrtEps);

        /// <summary>Right-preconditioned block GMRES(m) over a BSR non-symmetric A with ANY
        /// <see cref="IfProxyPreconditioner"/> (ILU0).</summary>
        public static BlockSolveInfo bgmres<TPre>(in fProxyBSR A, in TPre M, in fProxyMxN B, ref fProxyMxN X,
                                        int restart, int maxIter, fProxy tol)
            where TPre : struct, IfProxyPreconditioner
            => bgmres(new fProxyBSROperator(in A), in M, in B, ref X, restart, maxIter, tol);
        ...
        public static BlockSolveInfo bgmres<TPre>(in fProxyBSR A, in TPre M, in fProxyMxN B, ref fProxyMxN X)
            where TPre : struct, IfProxyPreconditioner
            => bgmres(new fProxyBSROperator(in A), in M, in B, ref X, math.min(30, A.M_Rows), A.M_Rows, Consts.fProxySqrtEps);
    }
}
