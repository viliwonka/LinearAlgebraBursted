...
namespace LinearAlgebra
{
    public static partial class Krylov {
        ...
        static void ExtractRowsAt(in fProxyMxN src, int rowOff, int rows, ref fProxyMxN dst)
        {
            int cols = dst.N_Cols;
            for (int a = 0; a < rows; a++)
        ...
        static void ZeroPrefix(ref fProxyMxN buf, int rows, int cols)
        {
            ...
            where TPre : struct, IfProxyPreconditioner
        {
            ...
            // Per-column thresholds tol^2 ||B[j]||^2, computed once (original RHS order -- rows never
            // reordered here, unlike bcg/bcgrq's live-column permutation).
            for (int j = 0; j < s; j++)
            {
                fProxy bb = (fProxy)0;
