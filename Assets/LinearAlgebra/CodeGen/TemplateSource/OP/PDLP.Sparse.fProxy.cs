...
namespace LinearAlgebra
{
    public static partial class LP
    {
        ...
        // PDLP.fProxy.cs); only the equilibration reads the sparse blocks directly. See docs/spec-pdlp.md.
        // ============================================================================================
        ...
        static unsafe void pdlpEquilibrateBSR(in fProxyBSR A, ref fProxyN Dr, ref fProxyN Dc)
        {
            ...
            fProxy* dr = Dr.Data.Ptr;
            fProxy* dc = Dc.Data.Ptr;

            var racc = new fProxyN(m, Allocator.Temp);
            var cacc = new fProxyN(n, Allocator.Temp);
            fProxy* ra = racc.Data.Ptr;
            fProxy* ca = cacc.Data.Ptr;
            long fsz = UnsafeUtility.SizeOf<fProxy>();
            ...
            for (int it = 0; it < 10; it++)
            {
                ...
                for (int bi = 0; bi < blockRows; bi++)
                    for (int k = rowPtr[bi]; k < rowPtr[bi + 1]; k++)
                    {
                        ...
                        fProxy* blk = values + (long)k * blockSize;
                        for (int r = 0; r < BR; r++)
                        {
                            ...
                            for (int c = 0; c < BC; c++)
                            {
                                ...
                                if (v > (double)ra[gr]) ra[gr] = (fProxy)v;
                            }
                        }
                    }
                for (int i = 0; i < m; i++) { double R = (double)dr[i] * (double)ra[i]; if (R > 1e-30) dr[i] = (fProxy)((double)dr[i] / math.sqrt(R)); }
                    ...
                    for (int k = rowPtr[bi]; k < rowPtr[bi + 1]; k++)
                    {
                        ...
                        for (int r = 0; r < BR; r++)
                        {
                            ...
                            for (int c = 0; c < BC; c++)
                            {
                                ...
                                if (v > (double)ca[gc]) ca[gc] = (fProxy)v;
                            }
                        }
                    }
                for (int j = 0; j < n; j++) { double C = (double)dc[j] * (double)ca[j]; if (C > 1e-30) dc[j] = (fProxy)((double)dc[j] / math.sqrt(C)); }
            }
            ...
            for (int bi = 0; bi < blockRows; bi++)
                for (int k = rowPtr[bi]; k < rowPtr[bi + 1]; k++)
                {
                    ...
                    for (int r = 0; r < BR; r++)
                    {
                        ...
                        {
                            int gc = colBase + c; if (gc >= n) continue;
                            ra[gr] = (fProxy)((double)ra[gr] + math.abs((double)blk[r * BC + c]) * (double)dc[gc]);
                        }
                    }
                }
            for (int i = 0; i < m; i++) { double s = (double)dr[i] * (double)ra[i]; if (s > 1e-30) dr[i] = (fProxy)((double)dr[i] / math.sqrt(s)); }
            ...
            for (int bi = 0; bi < blockRows; bi++)
                for (int k = rowPtr[bi]; k < rowPtr[bi + 1]; k++)
                {
                    ...
                    for (int r = 0; r < BR; r++)
                    {
                        ...
                        {
                            int gc = colBase + c; if (gc >= n) continue;
                            ca[gc] = (fProxy)((double)ca[gc] + math.abs((double)blk[r * BC + c]) * (double)dr[gr]);
                        }
                    }
                }
            for (int j = 0; j < n; j++) { double s = (double)dc[j] * (double)ca[j]; if (s > 1e-30) dc[j] = (fProxy)((double)dc[j] / math.sqrt(s)); }
        ...
        /// ℓ_v ≤ x ≤ u_v, with A a block-sparse <see cref="fProxyBSR"/>. Use ±<c>1e30</c> in a bound to mean
        ...
        /// <see cref="pdlp(in fProxyMxN, in fProxyN, in fProxyN, in fProxyN, in fProxyN, in fProxyN, ref fProxyN, out double, int, double)"/>.
        /// </summary>
        public static LPInfo pdlp(in fProxyBSR A, in fProxyN lc, in fProxyN uc, in fProxyN lv, in fProxyN uv,
