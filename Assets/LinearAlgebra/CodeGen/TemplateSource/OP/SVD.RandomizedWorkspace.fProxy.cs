...
namespace LinearAlgebra
{
    public static partial class SVD
    {
        ...
        /// Arena.fProxySvdRandomizedWorkspace(m, n, k, oversample).
        /// </summary>
        static void RequireSvdRandomizedWorkspace(in fProxySvdRandomizedWorkspace ws, int m, int n, int l)
        {
            bool ok =
            ...
            if (!ok)
                throw new ArgumentException(
                    ...
                    "Arena.fProxySvdRandomizedWorkspace(m, n, k, oversample) with the SAME k and oversample");
        }
    }
    ...
    /// Arena.fProxySvdRandomizedWorkspace(m, n, k, oversample) and reuse it across same-shape calls
    ...
    public partial struct Arena
    {
        ...
        /// </summary>
        public fProxySvdRandomizedWorkspace fProxySvdRandomizedWorkspace(int m, int n, int k, int oversample)
        {
            int l = math.min(k + oversample, n);
            return new fProxySvdRandomizedWorkspace
            {
                Omega = fProxyMat(n, l),
                Y = fProxyMat(m, l),
                R = fProxyMat(l, l),
        ...
        /// </summary>
        public fProxySvdRandomizedWorkspace fProxySvdRandomizedWorkspace(int m, int n, int k)
            => fProxySvdRandomizedWorkspace(m, n, k, 10);
    }
}
