...
namespace LinearAlgebra
{
    ...
    public static partial class LP
    {
                                           ...
                                           out int enterCol, out fProxy thetaD, out bool anyCandidate)
        {
            ...
            while (true)
            {
                ...
                for (int j = 0; j < N; j++)
                {
                    ...
                    if (best < 0 || ratio < bestRatio - (fProxy)1e-9 ||
                        (ratio <= bestRatio + (fProxy)1e-9 && math.abs(ah) > bestAbsA))
                                               ...
                                               NativeArray<int> basis, NativeArray<byte> status)
        {
            ...
            // for the same return-type-collision reason as the tolerances above (an fProxy-returning
            ...
            for (int j = 0; j < N; j++)
            {
                ...
                h ^= h >> 16;
                fProxy unit = (fProxy)(h * (1.0 / 4294967295.0) * 2.0 - 1.0);
                perturbedCost[j] = cost[j] + unit * (fProxy)1e-5 * ((fProxy)1 + math.abs(cost[j]));
            }
