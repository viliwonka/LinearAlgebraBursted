...
namespace LinearAlgebra
{
    ...
    // (LP.RevisedSimplex.fProxy.cs: Refactorize/Ftran/Btran/the eta-apply pair/RebuildXB). Where the
    ...
    // dual basis to the PRIMAL core (LP.RevisedSimplex.fProxy.cs) as a cleanup pass once real bounds are restored.
    //
    // Numerics: ordinary fProxy throughout, same as the primal revised simplex (reuses the library's own LU/Blas;
    // tolerances derived from Consts.fProxyZeroThreshold/fProxyEpsilon). The DSE weight floor (1e-4)
    ...
    // warm-overload, like LP.RevisedSimplex.fProxy.cs's RevisedPrimalCore.
    //
    // FACTOR/WEIGHT PERSISTENCE (fProxyLPCache in LP.Cache.fProxy.cs): the warm overload's pivot loop
    // is factored into DualSimplexPivotCore below, shared by a plain (Allocator.Temp scratch) wrapper
    // and a cache-aware wrapper that resumes B/P/eta/weight from a caller-persisted fProxyLPCache
    ...
    {
        // ---- fProxy-typed entry point: builds the computational form (reusing the revised simplex's
        ...
        static LPInfo dualSimplexCore(in fProxyMxN A, in fProxyN b, in fProxyN c,
        ...
        internal static void DualRatioTest(fProxyN alphaRow, fProxyN dj,
                                           NativeArray<byte> status, fProxyN lower, fProxyN upper,
                                           int N, bool needIncrease, fProxy pivTol, fProxy infeasR, fProxy feasTol,
                                           NativeArray<int> flipCols, out int flipCount,
                                           out int enterCol, out fProxy thetaD, out bool anyCandidate)
        {
            fProxy s = needIncrease ? (fProxy)(-1) : (fProxy)1;
            flipCount = 0; enterCol = -1; thetaD = (fProxy)0; anyCandidate = false;

            var considered = new NativeArray<bool>(N, Allocator.Temp);   // true = excluded from candidacy
            for (int j = 0; j < N; j++)
            {
                if (status[j] == STATUS_BASIC || upper[j] - lower[j] <= (fProxy)1e-13) { considered[j] = true; continue; }
                fProxy ah = s * alphaRow[j];
            ...
            fProxy ratioTieTol = (fProxy)/*+choose[1e-5f|1e-9]*/1e-5f/*-choose*/;

            fProxy remaining = infeasR;
            while (true)
            {
                int best = -1; fProxy bestRatio = (fProxy)0; fProxy bestAbsA = (fProxy)0;
                for (int j = 0; j < N; j++)
                {
                    if (considered[j]) continue;
                    fProxy ah = s * alphaRow[j];
                    fProxy djr = math.max(status[j] == STATUS_AT_LOWER ? dj[j] : -dj[j], (fProxy)0);
                    fProxy ratio = djr / math.abs(ah);
                ...
                bool boxed = upper[best] - lower[best] < (fProxy)1e29;
                fProxy fullAbsorb = bestAbsA * (upper[best] - lower[best]);
        ...
        // (mirrors LP.RevisedSimplex.fProxy.cs's RevisedPrimalCore split -- the fresh-start case is
        ...
        // Warm-start overload -- LP.solve's `ref LPBasis` re-solve entry point (LP.fProxy.cs; LPBasis
        ...
        // Cache-aware warm overload -- fProxyLPCache (LP.Cache.fProxy.cs) persists M/lower/upper/
        ...
        {
            // Same per-dtype tolerance expressions as LP.RevisedSimplex.fProxy.cs (see that file's header for why these are
            ...
            fProxy weightFloor = (fProxy)1e-4;
            ...
            fProxy artificialBound = (fProxy)(100.0 * dataScale);

            var xB = new fProxyN(m, Allocator.Temp);
            var perturbedCost = new fProxyN(N, Allocator.Temp);
            ...
            for (int j = 0; j < N; j++)
            {
                ...
                bool loInf = lower[j] <= (fProxy)(-1e29);
                bool upInf = upper[j] >= (fProxy)1e29;
                double xpert = (1.0 + r) * (math.abs((double)cost[j]) + 1.0) * colPerturbBase;
                if (loInf && upInf) perturbedCost[j] = cost[j];                                  // free
                else if (upInf) perturbedCost[j] = cost[j] + (fProxy)xpert;                      // lower-bounded
                else if (loInf) perturbedCost[j] = cost[j] - (fProxy)xpert;                      // upper-bounded
                else if (lower[j] != upper[j])                                                   // boxed
                    perturbedCost[j] = cost[j] + (cost[j] >= (fProxy)0 ? (fProxy)xpert : (fProxy)(-xpert));
            ...
            var rho = new fProxyN(m, Allocator.Temp);
            var tau = new fProxyN(m, Allocator.Temp);
            var alphaRow = new fProxyN(N, Allocator.Temp);
            var alphaCol = new fProxyN(m, Allocator.Temp);
            ...
            // resumeWeights (fProxyLPCache.weightsValid) AND didResumeFactors -- weight[] is already
            ...
            else
            {
                ...
                for (int j = 0; j < N; j++)
                {
                    ...
                    {
                        if (upper[j] < (fProxy)1e29) status[j] = STATUS_AT_UPPER;
                    ...
                    {
                        if (lower[j] > (fProxy)(-1e29)) status[j] = STATUS_AT_LOWER;
            ...
            while (resultStatus == LPStatus.Optimal)
            {
                ...
                int r = -1; fProxy bestScore = (fProxy)(-1); bool rNeedsIncrease = false; fProxy infeasR = (fProxy)0;
                for (int i = 0; i < m; i++)
                {
                    int v = basis[i];
                    fProxy xi = xB[i];
                    fProxy infeas; bool needInc;
                    if (xi < lower[v] - feasTol) { infeas = lower[v] - xi; needInc = true; }
                    else if (xi > upper[v] + feasTol) { infeas = xi - upper[v]; needInc = false; }
                    else continue;

                    fProxy score = infeas * infeas / math.max(weight[i], (fProxy)1e-12);
                ...
                for (int i = 0; i < m; i++) rho[i] = (i == r) ? (fProxy)1 : (fProxy)0;
                ...
                if (flipCount > 0)
                {
                    ...
                    {
                        int j = flipCols[f];
                        fProxy delta = status[j] == STATUS_AT_LOWER ? (upper[j] - lower[j]) : -(upper[j] - lower[j]);
                ...
                // LP.RevisedSimplex.fProxy.cs's primal core for the full rationale). Row selection already
                ...
                if (leavingNeedsIncrease && math.abs(lower[leavingVar]) >= (fProxy)1e29) leavingNeedsIncrease = false;
                else if (!leavingNeedsIncrease && math.abs(upper[leavingVar]) >= (fProxy)1e29) leavingNeedsIncrease = true;

                fProxy targetBound = leavingNeedsIncrease ? lower[leavingVar] : upper[leavingVar];
                fProxy safePivot = math.abs(pivotElt) < pivTolFloor ? (pivotElt >= (fProxy)0 ? pivTolFloor : -pivTolFloor) : pivotElt;
                fProxy thetaPrimal = (xB[r] - targetBound) / (sig * safePivot);
                if (thetaPrimal < (fProxy)0) thetaPrimal = (fProxy)0;

                fProxy enteringValue = (sigma > 0 ? lower[enterCol] : upper[enterCol]) + sig * thetaPrimal;
                ...
                fProxy wr = weight[r];
                fProxy newWr = wr / (safePivot * safePivot);
                for (int i = 0; i < m; i++)
                {
                    if (i == r) continue;
                    fProxy ratio = alphaCol[i] / safePivot;
                    fProxy wi = weight[i] - (fProxy)2 * ratio * tau[i] + ratio * ratio * wr;
