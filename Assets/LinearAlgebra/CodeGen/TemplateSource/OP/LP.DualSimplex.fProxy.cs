...
namespace LinearAlgebra
{
    ...
    // layer (LP.RevisedSimplex.fProxy.cs: Refactorize/Ftran/Btran/the eta-apply pair/RebuildXB) instead
    ...
    //
    // Numerics: ordinary fProxy throughout (matrix/vector storage, the ratio test, DSE weights), exactly
    // like stage 1 -- see that file's header for the full rationale (reuses the library's own LU/Blas;
    // tolerances derived from Consts.fProxyZeroThreshold/fProxyEpsilon, the SAME expressions stage 1
    // uses, computed INLINE in DualSimplexCore rather than via a shared helper method -- an fProxy-
    // returning helper with no fProxy-typed parameter differs only in return type between the float- and
    ...
    // split fresh-overload-forwards-to-warm-overload, exactly like LP.RevisedSimplex.fProxy.cs's
    ...
    {
        // ---- fProxy-typed entry point: builds the computational form (reusing stage 1's
        ...
        // Warm-start overload -- added for LP.solve's `ref LPBasis` re-solve entry point (LP.fProxy.cs;
        ...
        // allocates or disposes them), exactly like LP.RevisedSimplex.fProxy.cs's RevisedPrimalCore warm
