...
namespace LinearAlgebra
{
    ...
    /// Two rank-safe least-squares solves: <see cref="solveInPlace(ref fProxyMxN, ref fProxyN, ref fProxyN, ref fProxyMxN, ref Pivot, ref fProxyN, fProxy)"/>
    /// (the BASIC / truncated solution) and <see cref="minNormSolveInPlace(ref fProxyMxN, ref fProxyN, ref fProxyN, ref fProxyMxN, ref Pivot, ref fProxyN, fProxy)"/>
    ...
    public static partial class QRCP {
        ...
        public static RankInfo solveInPlace(ref fProxyMxN A_to_Q, ref fProxyN b, ref fProxyN x,
                                           ...
                                           ref fProxyN u, fProxy relativeTolerance)
        {
            int m = A_to_Q.M_Rows;
            int n = A_to_Q.N_Cols;
        ...
        public static RankInfo solveInPlace(ref fProxyMxN A_to_Q, ref fProxyN b, ref fProxyN x,
                                           ref fProxyMxN R, ref Pivot P,
                                           ref fProxyN u, ref fProxyQRCPCache cache, fProxy relativeTolerance)
        {
            int m = A_to_Q.M_Rows;
            int n = A_to_Q.N_Cols;
        ...
        static unsafe RankInfo solveInPlaceFusedDispatch(ref fProxyMxN A_to_Q, ref fProxyN b, ref fProxyN x,
                                                         ...
                                                         ref fProxyN vn1, ref fProxyN vn2, fProxy relativeTolerance)
        {
            // See decompInPlaceBlockedCore for why this is a method-local const, not a class field.
            const int QRCP_BLOCK = 32;
        ...
        static RankInfo solveInPlaceFinish(ref fProxyMxN A_to_Q, ref fProxyN b, ref fProxyN x,
                                            ref fProxyMxN R, ref Pivot P, ref fProxyN u, fProxy relativeTolerance)
        {
            int n = A_to_Q.N_Cols;
            ...
            fProxy tol = relativeTolerance * math.abs(R[0, 0]);
            int rank = 0;
            for (int i = 0; i < n; i++)
            {
        ...
        static unsafe void minNormFactor(ref fProxyMxN A_to_Q, ref fProxyN b, ref fProxyMxN R, ref Pivot P,
                                          ref fProxyN u, ref fProxyN vn1, ref fProxyN vn2)
        {
            if (A_to_Q.N_Cols >= Consts.fProxyQrcpBlockMinN)   // float/double split (see Consts); default 2*QRCP_BLOCK
                decompInPlaceBlockedCore(ref A_to_Q, ref R, ref P, ref u, ref vn1, ref vn2, b.Data.Ptr, 1, null, true);
        ...
        static unsafe RankInfo minNormCodFinish(ref fProxyN b, ref fProxyN x, ref fProxyMxN R, ref Pivot P,
                                                ref fProxyN u, int r)
        {
            int n = R.N_Cols;
            ...
            var M   = new fProxyMxN(r, n, Allocator.Temp, false);
            var Lc  = new fProxyMxN(r, r, Allocator.Temp, false);
            var vrow = new fProxyN(n, Allocator.Temp, false);
            var w   = new fProxyN(n, Allocator.Temp, false);

            UnsafeUtility.MemCpy(M.Data.Ptr, R.Data.Ptr, (long)r * n * UnsafeUtility.SizeOf<fProxy>());

            fProxy zeroThreshold = Consts.fProxyZeroThreshold * Norms.LInf(in M);
            for (int d = 0; d < r; d++)
            {
            ...
            for (int i = 0; i < r; i++)
            {
                ...
                for (int c = i + 1; c < r; c++) Lc[i, c] = (fProxy)0;
            }
            ...
            for (int i = r; i < n; i++)
                w[i] = (fProxy)0;                            // (applyQtFromReflectors only reads w[0..r), pad is cosmetic)
        ...
        /// result <see cref="SVD.pinvSolve(ref fProxyMxN, in fProxyN, ref fProxyN, fProxy, int)"/> gives,
        ...
        /// max(m,n)·Consts.fProxyZeroThreshold, matching SVD.pinvSolve / MatrixMetrics.rank).
        /// </summary>
        ...
        /// <remarks>R must not alias A_to_Q (unchecked) — see decompInPlace.</remarks>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static RankInfo minNormSolveInPlace(ref fProxyMxN A_to_Q, ref fProxyN b, ref fProxyN x,
                                                   ref fProxyMxN R, ref Pivot P, ref fProxyN u, fProxy relativeTolerance)
        {
            int m = A_to_Q.M_Rows;
            int n = A_to_Q.N_Cols;
        ...
        public static RankInfo minNormSolveInPlace(ref fProxyMxN A_to_Q, ref fProxyN b, ref fProxyN x,
                                                   ref fProxyMxN R, ref Pivot P, ref fProxyN u)
        {
            return minNormSolveInPlace(ref A_to_Q, ref b, ref x, ref R, ref P, ref u, (fProxy)(-1));
        }
        ...
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static RankInfo minNormSolveInPlace(ref fProxyMxN A_to_Q, ref fProxyN b, ref fProxyN x, fProxy relativeTolerance)
        {
            int m = A_to_Q.M_Rows;
            int n = A_to_Q.N_Cols;
        ...
        public static RankInfo minNormSolveInPlace(ref fProxyMxN A_to_Q, ref fProxyN b, ref fProxyN x)
        {
            return minNormSolveInPlace(ref A_to_Q, ref b, ref x, (fProxy)(-1));
        }
        ...
        static unsafe RankInfo finishFromQtB(ref fProxyMxN X, ref fProxyMxN R, in Pivot P, fProxy relativeTolerance, int n, int k)
        {
            fProxy tol = relativeTolerance * math.abs(R[0, 0]);
            int rank = 0;
            for (int i = 0; i < n; i++)
        ...
        /// (tol = relativeTolerance·|R[0,0]|; relativeTolerance &lt; 0 auto-selects max(m,n)·Consts.fProxyZeroThreshold). Only
        ...
        /// (max(m,n)·Consts.fProxyZeroThreshold). See the primitive.</summary>
        public static RankInfo decompSolve(ref fProxyMxN Q, ref fProxyMxN R, in Pivot P,
                                           ref fProxyMxN B, ref fProxyMxN X)
        {
            return decompSolve(ref Q, ref R, in P, ref B, ref X, (fProxy)(-1));
        }
        ...
        /// <see cref="minNormDecompSolve(ref fProxyMxN, ref fProxyMxN, in Pivot, ref fProxyN, ref fProxyN, fProxy)"/>).
        ...
        public static RankInfo solveInPlace(ref fProxyMxN A_to_Q, ref fProxyMxN B, ref fProxyMxN X, fProxy relativeTolerance)
        {
            int m = A_to_Q.M_Rows;
            int n = A_to_Q.N_Cols;
        ...
        static unsafe RankInfo minNormFinishFromQtB(ref fProxyMxN X, ref fProxyMxN R, in Pivot P, fProxy relativeTolerance, int n, int k)
        {
            fProxy tol = relativeTolerance * math.abs(R[0, 0]);
            int rank = 0;
            for (int i = 0; i < n; i++)
            ...
            fProxy* Xp = X.Data.Ptr;
            var W1 = new fProxyMxN(r, k, Allocator.Temp, false);
            fProxy* W1p = W1.Data.Ptr;
            for (int i = 0; i < r; i++)
            {
                fProxy* Wi = W1p + (long)i * k;
                fProxy* Xi = Xp + (long)i * k;               // C1 row i
                for (int c = 0; c < k; c++) Wi[c] = Xi[c];
                for (int j = 0; j < i; j++)
                    UnsafeOP.axpy(Wi, W1p + (long)j * k, -Lc[i, j], k);
                fProxy inv = (fProxy)1 / Lc[i, i];
                for (int c = 0; c < k; c++) Wi[c] *= inv;
            }

            // Y = Qzᵀ W1 (n×k) straight from M's reflectors, then un-permute rows into X: X[P[j],:] = Y[j,:].
            var Y = new fProxyMxN(n, k, Allocator.Temp, false);
            LQ.applyQtFromReflectors(ref M, ref W1, ref Y);
            fProxy* Yp = Y.Data.Ptr;
            for (int j = 0; j < n; j++)
            {
                fProxy* dst = Xp + (long)P[j] * k;
                fProxy* src = Yp + (long)j * k;
                for (int c = 0; c < k; c++) dst[c] = src[c];
            }
        ...
        /// factor-reuse form of <see cref="minNormSolveInPlace(ref fProxyMxN, ref fProxyN, ref fProxyN, ref fProxyMxN, ref Pivot, ref fProxyN, fProxy)"/>:
        ...
        public static unsafe RankInfo minNormDecompSolve(ref fProxyMxN Q, ref fProxyMxN R, in Pivot P,
                                                         ref fProxyMxN B, ref fProxyMxN X, fProxy relativeTolerance)
        {
            int m = Q.M_Rows;
            int n = Q.N_Cols;
        ...
        public static RankInfo minNormDecompSolve(ref fProxyMxN Q, ref fProxyMxN R, in Pivot P,
                                                  ref fProxyMxN B, ref fProxyMxN X)
        {
            return minNormDecompSolve(ref Q, ref R, in P, ref B, ref X, (fProxy)(-1));
        }
        ...
        /// the block form of the single-RHS <see cref="minNormSolveInPlace(ref fProxyMxN, ref fProxyN, ref fProxyN, ref fProxyMxN, ref Pivot, ref fProxyN, fProxy)"/>:
        ...
        /// <param name="P">Scratch: column Pivot of size n (reset internally).</param>
        /// <param name="u">Scratch: length EXACTLY m.</param>
        public static unsafe RankInfo minNormSolveInPlace(ref fProxyMxN A_to_Q, ref fProxyMxN B, ref fProxyMxN X,
                                                          ref fProxyMxN R, ref Pivot P, ref fProxyN u, fProxy relativeTolerance)
        {
            int m = A_to_Q.M_Rows;
            int n = A_to_Q.N_Cols;
        ...
        public static RankInfo minNormSolveInPlace(ref fProxyMxN A_to_Q, ref fProxyMxN B, ref fProxyMxN X,
                                                   ref fProxyMxN R, ref Pivot P, ref fProxyN u)
        {
            return minNormSolveInPlace(ref A_to_Q, ref B, ref X, ref R, ref P, ref u, (fProxy)(-1));
        }
        ...
        public static RankInfo minNormSolveInPlace(ref fProxyMxN A_to_Q, ref fProxyMxN B, ref fProxyMxN X, fProxy relativeTolerance)
        {
            int m = A_to_Q.M_Rows;
            int n = A_to_Q.N_Cols;
