...
namespace LinearAlgebra
{
    ...
    public static partial class MIP
    {
        ...
        public static MIPInfo solve(in fProxyMxN A, in fProxyN b, in fProxyN c,
                                    ...
                                    double absGap = 0.0, double relGap = 0.0)
        {
            ...
            var Aaug = new fProxyMxN(mAug, nY, Allocator.Temp);      // zero-initialized
            var bAug = new fProxyN(mAug, Allocator.Temp);
            var sensesAug = new NativeArray<ConstraintSense>(mAug, Allocator.Temp);
            var costY = new fProxyN(nY, Allocator.Temp);             // zero-initialized
            ...
            for (int j = 0; j < n; j++)
            {
                ...
                if (rowUB[j] >= 0)
                {
                    ...
                    Aaug[r, col[j]] = hiFiniteHere ? (fProxy)1 : (fProxy)0;
                    bAug[r] = hiFiniteHere ? (xu[j] - xl[j]) : (fProxy)0;
                    sensesAug[r] = ConstraintSense.LessEqual;
                }
                if (rowLB[j] >= 0)
                {
                    int r = rowLB[j];
                    Aaug[r, col[j]] = (fProxy)1;
                    bAug[r] = (fProxy)0;   // root bound == xl[j] -> shifted rhs 0
                    sensesAug[r] = ConstraintSense.GreaterEqual;
                }
            }

            var curLB = new fProxyN(math.max(n, 1), Allocator.Temp);
            var curUB = new fProxyN(math.max(n, 1), Allocator.Temp);
        ...
        internal static void UnshiftToX(fProxyN y, NativeArray<byte> kind, NativeArray<int> col,
                                        fProxyN xlRoot, fProxyN xuRoot, int n, fProxyN xOut)
        {
            for (int j = 0; j < n; j++)
            {
        ...
        internal static MIPInfo SearchCore(fProxyMxN Aaug, fProxyN bAug, fProxyN costY, NativeArray<ConstraintSense> sensesAug,
                                           ...
                                           fProxyN xlRoot, fProxyN xuRoot,
                                           fProxyN curLB, fProxyN curUB,
                                           NativeArray<int> rowLB, NativeArray<int> rowUB,
                                           fProxyMxN A, fProxyN b, NativeArray<ConstraintSense> senses, int m0,
                                           ...
                                           fProxyN xOut)
        {
            ...
            var xRound = new fProxyN(math.max(n, 1), Allocator.Temp);   // TryRoundingHeuristic scratch
            ...
            if (status == MIPStatus.Infeasible || status == MIPStatus.Unbounded)
            {
                ...
                for (int j = 0; j < n; j++) xOut[j] = (fProxy)0;
            }
            else
            {
        ...
        internal static void TryRoundingHeuristic(fProxyN xNode, NativeArray<byte> integrality, int n,
                                                   ...
                                                   fProxyMxN A, fProxyN b, NativeArray<ConstraintSense> senses, int m0,
                                                   fProxyN c, fProxyN xlRoot, fProxyN xuRoot, fProxyN xRound,
                                                   ref bool haveIncumbent, ref double incumbentObj, fProxyN incumbentX)
        {
            for (int j = 0; j < n; j++)
            {
