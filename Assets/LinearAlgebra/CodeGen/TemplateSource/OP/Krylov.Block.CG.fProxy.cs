...
namespace LinearAlgebra
{
    public static partial class Krylov {
        ...
        /// </summary>
        public static BlockSolveInfo bcg<TOp, TPre>(in TOp A, in TPre M, in fProxyMxN B, ref fProxyMxN X,
        ...
        /// <see cref="bcg{TOp, TPre}(in TOp, in TPre, in fProxyMxN, ref fProxyMxN, ref fProxyMxN, ref fProxyMxN, ref fProxyMxN, ref fProxyMxN, int, fProxy)"/>
        /// with the identity preconditioner (needs no Z block).</summary>
        public static BlockSolveInfo bcg<TOp>(in TOp A, in fProxyMxN B, ref fProxyMxN X,
                                        ref fProxyMxN R, ref fProxyMxN P, ref fProxyMxN Q,
                                        int maxIter, fProxy tol)
            where TOp : struct, IfProxyLinearOperator
        {
            fProxyMxN Z = default;
            return bcg(in A, default(fProxyIdentityPreconditioner), in B, ref X, ref R, ref P, ref Q, ref Z, maxIter, tol);
        }
        ...
        public static BlockSolveInfo bcg(in fProxyMxN A, in fProxyMxN B, ref fProxyMxN X, int maxIter, fProxy tol)
        {
            int s = B.M_Rows, n = A.M_Rows;
            fProxyMxN R = B.fProxyTempMat(s, n, true), P = B.fProxyTempMat(s, n, true), Q = B.fProxyTempMat(s, n, true);
            return bcg(new fProxyDenseOperator(in A), in B, ref X, ref R, ref P, ref Q, maxIter, tol);
        }

        /// <summary>Block-CG over a dense SPD A with default maxIter (A.M_Rows) and tol (sqrtEps).</summary>
        public static BlockSolveInfo bcg(in fProxyMxN A, in fProxyMxN B, ref fProxyMxN X)
            => bcg(in A, in B, ref X, A.M_Rows, Consts.fProxySqrtEps);

        /// <summary>Preconditioned block-CG over a dense SPD A. Allocates block scratch (incl. Z).</summary>
        public static BlockSolveInfo bcg<TPre>(in fProxyMxN A, in TPre M, in fProxyMxN B, ref fProxyMxN X, int maxIter, fProxy tol)
            where TPre : struct, IfProxyPreconditioner
        {
            int s = B.M_Rows, n = A.M_Rows;
            fProxyMxN R = B.fProxyTempMat(s, n, true), P = B.fProxyTempMat(s, n, true),
                      Q = B.fProxyTempMat(s, n, true), Z = B.fProxyTempMat(s, n, true);
            return bcg(new fProxyDenseOperator(in A), in M, in B, ref X, ref R, ref P, ref Q, ref Z, maxIter, tol);
        }

        /// <summary>Block-CG over a block-sparse (BSR) SPD A with an s x n block B. Allocates block
        /// scratch from the arena.</summary>
        public static BlockSolveInfo bcg(in fProxyBSR A, in fProxyMxN B, ref fProxyMxN X, int maxIter, fProxy tol)
        {
            int s = B.M_Rows, n = A.M_Rows;
            fProxyMxN R = B.fProxyTempMat(s, n, true), P = B.fProxyTempMat(s, n, true), Q = B.fProxyTempMat(s, n, true);
            return bcg(new fProxyBSROperator(in A), in B, ref X, ref R, ref P, ref Q, maxIter, tol);
        }

        /// <summary>Block-CG over a BSR SPD A with default maxIter (A.M_Rows) and tol (sqrtEps).</summary>
        public static BlockSolveInfo bcg(in fProxyBSR A, in fProxyMxN B, ref fProxyMxN X)
            => bcg(in A, in B, ref X, A.M_Rows, Consts.fProxySqrtEps);

        /// <summary>Preconditioned block-CG over a BSR SPD A. Allocates block scratch (incl. Z).</summary>
        public static BlockSolveInfo bcg<TPre>(in fProxyBSR A, in TPre M, in fProxyMxN B, ref fProxyMxN X, int maxIter, fProxy tol)
            where TPre : struct, IfProxyPreconditioner
        {
            int s = B.M_Rows, n = A.M_Rows;
            fProxyMxN R = B.fProxyTempMat(s, n, true), P = B.fProxyTempMat(s, n, true),
                      Q = B.fProxyTempMat(s, n, true), Z = B.fProxyTempMat(s, n, true);
            return bcg(new fProxyBSROperator(in A), in M, in B, ref X, ref R, ref P, ref Q, ref Z, maxIter, tol);
        }
