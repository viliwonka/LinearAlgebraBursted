...
namespace LinearAlgebra
{
    public static partial class LP
    {
        ...
        /// <see cref="ConstraintSense"/>), x ≥ 0, with A a block-sparse <see cref="fProxyBSR"/>. Matrix-
        /// free interior point (see the file header): each inequality row gets one non-negative
        /// slack/surplus, expressed by a <see cref="Sparse.fProxySlackAugmentedOperator"/> that never
        ...
        /// certificate; use the dense <see cref="solve(in fProxyMxN, in fProxyN, in fProxyN, in
        /// NativeArray{ConstraintSense}, ref fProxyN, out double, LPMethod, int)"/> simplex for those).
        /// </summary>
        public static LPInfo solve(in fProxyBSR A, in fProxyN b, in fProxyN c,
                                   ...
                                   ref fProxyN x, out double objective, int maxIter = 0)
        {
            ...
            var slackSign = new NativeArray<fProxy>(nSlack, Allocator.Temp);
            {
                int k = 0;
                for (int i = 0; i < m; i++)
                {
                    ...
                    slackRow[k] = i;
                    slackSign[k] = senses[i] == ConstraintSense.LessEqual ? (fProxy)1 : (fProxy)(-1);
                    k++;
                }
            }

            var opSp = new fProxyN(n, Allocator.Temp); var opAtr = new fProxyN(n, Allocator.Temp);
            var cvec = new fProxyN(nv, Allocator.Temp);
            var zBest = new fProxyN(nv, Allocator.Temp, false);
            for (int j = 0; j < n; j++) cvec[j] = c[j];   // slack costs stay 0

            var op = new fProxySlackAugmentedOperator(in A, nSlack, in slackRow, in slackSign, in opSp, in opAtr);
        ...
        // --------------------------------------------------------------------------------------------
        static LPInfo standardFormInterior<TOp>(in TOp aS, in fProxyN b, in fProxyN cvec,
                                                ref fProxyN zBest, int maxIter)
            where TOp : struct, IfProxyStandardFormOperator
        {
            ...
            var d = new fProxyN(nv, Allocator.Temp); var rc = new fProxyN(nv, Allocator.Temp);
            var g = new fProxyN(nv, Allocator.Temp); var tmpNV = new fProxyN(nv, Allocator.Temp);
            var normNV = new fProxyN(nv, Allocator.Temp);
            // length-m scratch
            var y = new fProxyN(m, Allocator.Temp); var dy = new fProxyN(m, Allocator.Temp);
            ...
            fProxy reg = math.max(Consts.fProxyZeroThreshold, (fProxy)1e-8);
            var Mop = new fProxyNormalOperator<TOp>(in aS, in d, in normNV, reg);
            var Jac = new fProxyNormalJacobi(in invDiag);
