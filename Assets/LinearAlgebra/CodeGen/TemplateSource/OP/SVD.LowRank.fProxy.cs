    ...
    public static partial class SVD {
                                        ...
                                        ref fProxySvdTruncatedWorkspace ws, out bool converged)
        {
            ...
            var rng = new Random(seed == 0 ? 0x9E3779B1u : seed);
            {
                ...
                for (int i = 0; i < n; i++)
                {
                    fProxy val = (fProxy)(rng.NextFloat() * 2f - 1f);
                    ws.VL[i, 0] = val;
                ...
                if (norm2 > 0)
                {
                    ...
                    for (int i = 0; i < n; i++)
                        ws.VL[i, 0] = (fProxy)((double)ws.VL[i, 0] * invNorm);
                }
                else
                {
                    // Degenerate: use e1
            ...
            fProxy scaleEps = Consts.fProxyEpsilon; // updated after first matvec
            int pDone = 0;
            ...
            for (int j = 0; j < p; j++)
            {
                ...
                // Compute alpha_j = ||uBuf||
                {
                    ...
                    ws.alpha[j] = (fProxy)math.sqrt(norm2);
                }
                ...
                for (int i = 0; i < n; i++)
                {
                    ...
                    ws.vBuf[i] = (fProxy)acc;
                }
                // Subtract alpha_j * v_j
                {
                    fProxy aJ = ws.alpha[j];
                    for (int i = 0; i < n; i++)
                ...
                // beta_j = ||vBuf||
                {
                    ...
                    ws.beta[j] = (fProxy)math.sqrt(norm2);
                }
            ...
            // triplets); ws.beta[pDone-1] is the previous step's coupling, NOT the residual.
            {
                ...
                {
                    fProxy res = math.abs(betaLast * ws.BsvdWs.U[pDone - 1, t]); // FIX 1: U not V
                                        ...
                                        int k, int oversample, uint seed, int maxIter, out bool converged)
        {
            ...
            {
                UL     = A.tempfProxyMat(m, p),
                VL     = A.tempfProxyMat(n, p + 1),
