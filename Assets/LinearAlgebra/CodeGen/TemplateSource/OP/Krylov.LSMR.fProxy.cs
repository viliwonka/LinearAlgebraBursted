...
namespace LinearAlgebra
{
    public static partial class Krylov {
        ...
        /// </summary>
        public static LstsqInfo lsmrRightPreOp<TPreN>(in fProxyMxN A, in TPreN N, in fProxyN b, ref fProxyN x, int maxIter, fProxy tol, fProxy damp)
            where TPreN : struct, IfProxyLinearOperator
            => LsmrRightPreOpCore(new fProxyDenseOperator(in A), in N, in b, ref x, maxIter, tol, damp);

        /// <summary>Undamped general right-preconditioned LSMR over a dense matrix (damp = 0).</summary>
        public static LstsqInfo lsmrRightPreOp<TPreN>(in fProxyMxN A, in TPreN N, in fProxyN b, ref fProxyN x, int maxIter, fProxy tol)
            where TPreN : struct, IfProxyLinearOperator
            => LsmrRightPreOpCore(new fProxyDenseOperator(in A), in N, in b, ref x, maxIter, tol, (fProxy)0);

        /// <summary>General right-preconditioned LSMR (dense), default maxIter (A.N_Cols) / tol (Consts.fProxySqrtEps).</summary>
        public static LstsqInfo lsmrRightPreOp<TPreN>(in fProxyMxN A, in TPreN N, in fProxyN b, ref fProxyN x)
            where TPreN : struct, IfProxyLinearOperator
            => LsmrRightPreOpCore(new fProxyDenseOperator(in A), in N, in b, ref x, A.N_Cols, Consts.fProxySqrtEps, (fProxy)0);

        /// <summary>General right-preconditioned damped LSMR over a BSR matrix (materializes Aᵀ once)
        /// -- same contract as the dense overload.</summary>
        public static LstsqInfo lsmrRightPreOp<TPreN>(in fProxyBSR A, in TPreN N, in fProxyN b, ref fProxyN x, int maxIter, fProxy tol, fProxy damp)
            where TPreN : struct, IfProxyLinearOperator
        {
            fProxyBSR AT = b.fProxyBSRTranspose(in A);
            return LsmrRightPreOpCore(new fProxyBSROperator(in A, in AT), in N, in b, ref x, maxIter, tol, damp);
        }

        /// <summary>Undamped general right-preconditioned LSMR over a BSR matrix (damp = 0, materializes Aᵀ once).</summary>
        public static LstsqInfo lsmrRightPreOp<TPreN>(in fProxyBSR A, in TPreN N, in fProxyN b, ref fProxyN x, int maxIter, fProxy tol)
            where TPreN : struct, IfProxyLinearOperator
            => lsmrRightPreOp(in A, in N, in b, ref x, maxIter, tol, (fProxy)0);

        /// <summary>General right-preconditioned LSMR (BSR), default maxIter (A.N_Cols) / tol (Consts.fProxySqrtEps).</summary>
        public static LstsqInfo lsmrRightPreOp<TPreN>(in fProxyBSR A, in TPreN N, in fProxyN b, ref fProxyN x)
            where TPreN : struct, IfProxyLinearOperator
            => lsmrRightPreOp(in A, in N, in b, ref x, A.N_Cols, Consts.fProxySqrtEps, (fProxy)0);
        ...
        static LstsqInfo LsmrRightPreOpCore<TOp, TPreN>(in TOp Aop, in TPreN N, in fProxyN b, ref fProxyN x, int maxIter, fProxy tol, fProxy damp)
            where TOp : struct, IfProxyLinearOperator
            where TPreN : struct, IfProxyLinearOperator
        {
            int m = Aop.Rows, n = Aop.Cols;
