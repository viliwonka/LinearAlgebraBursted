...
namespace LinearAlgebra
{
    public static partial class Krylov {
        ...
        static bool AnyZeroOrNaN(in fProxyN v, int s)
        {
            for (int i = 0; i < s; i++)
        ...
        static bool AnyNaN(in fProxyN v, int s)
        {
            for (int i = 0; i < s; i++)
            ...
            where TPre : struct, IfProxyPreconditioner
        {
            ...
            for (int k = 0; k < maxIter; k++)
            {
                ...
                else
                {
                    ...
                    for (int i = 0; i < s; i++) RowAddScaled(ref V, i, (fProxy)1, in AU, n);  // V += AU (now refreshed)
                }
            }
