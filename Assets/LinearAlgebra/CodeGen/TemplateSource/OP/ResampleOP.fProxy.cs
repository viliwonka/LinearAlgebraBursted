...
namespace BULA
{
    ...
    public static partial class Resample
    {
        ...
        private static fProxy sampleRowAt(in fProxyMxN m, int row, fProxy pos, Interp interp, EdgeMode edge)
        {
            int nCols = m.N_Cols;
            switch (interp)
            {
                ...
                case Interp.Linear:
                {
                    ...
                    fProxy frac = pos - (fProxy)i0;
                    fProxy v0 = m[row, idx(i0,     nCols, edge)];
                    fProxy v1 = m[row, idx(i0 + 1, nCols, edge)];
                    return math.lerp(v0, v1, frac);
                }
                default: // Interp.Cubic — Catmull-Rom
                {
                    int i0 = (int)math.floor(pos);
                    fProxy t  = pos - (fProxy)i0;
                    fProxy p0 = m[row, idx(i0 - 1, nCols, edge)];
                    fProxy p1 = m[row, idx(i0,     nCols, edge)];
                    fProxy p2 = m[row, idx(i0 + 1, nCols, edge)];
                    fProxy p3 = m[row, idx(i0 + 2, nCols, edge)];
                    return (fProxy)0.5 * (
                        (fProxy)2 * p1
                        + (-p0 + p2) * t
                        + ((fProxy)2 * p0 - (fProxy)5 * p1 + (fProxy)4 * p2 - p3) * t * t
                        + (-p0 + (fProxy)3 * p1 - (fProxy)3 * p2 + p3) * t * t * t);
                }
            }
        }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static fProxy sampleColAt(in fProxyMxN m, int col, fProxy pos, Interp interp, EdgeMode edge)
        {
            int nRows = m.M_Rows;
            switch (interp)
            {
                ...
                case Interp.Linear:
                {
                    int i0 = (int)math.floor(pos);
                    fProxy frac = pos - (fProxy)i0;
                    fProxy v0 = m[idx(i0,     nRows, edge), col];
                    fProxy v1 = m[idx(i0 + 1, nRows, edge), col];
                    return math.lerp(v0, v1, frac);
                }
                default: // Interp.Cubic — Catmull-Rom
                {
                    int i0 = (int)math.floor(pos);
                    fProxy t  = pos - (fProxy)i0;
                    fProxy p0 = m[idx(i0 - 1, nRows, edge), col];
                    fProxy p1 = m[idx(i0,     nRows, edge), col];
                    fProxy p2 = m[idx(i0 + 1, nRows, edge), col];
                    fProxy p3 = m[idx(i0 + 2, nRows, edge), col];
                    return (fProxy)0.5 * (
                        (fProxy)2 * p1
                        + (-p0 + p2) * t
                        + ((fProxy)2 * p0 - (fProxy)5 * p1 + (fProxy)4 * p2 - p3) * t * t
        ...
        /// data.N must be >= 1.
        /// </summary>
        public static fProxy sampleAt(in fProxyN data, fProxy pos, Interp interp, EdgeMode edge)
        {
            if (data.N < 1)
            ...
            switch (interp)
            {
                ...
                case Interp.Linear:
                {
                    int i0 = (int)math.floor(pos);
                    fProxy frac = pos - (fProxy)i0;
                    fProxy v0 = data[idx(i0,     n, edge)];
                    fProxy v1 = data[idx(i0 + 1, n, edge)];
                    return math.lerp(v0, v1, frac);
                }
                default: // Interp.Cubic — Catmull-Rom
                {
                    int i0 = (int)math.floor(pos);
                    fProxy t  = pos - (fProxy)i0;
                    fProxy p0 = data[idx(i0 - 1, n, edge)];
                    fProxy p1 = data[idx(i0,     n, edge)];
                    fProxy p2 = data[idx(i0 + 1, n, edge)];
                    fProxy p3 = data[idx(i0 + 2, n, edge)];
                    return (fProxy)0.5 * (
                        (fProxy)2 * p1
                        + (-p0 + p2) * t
                        + ((fProxy)2 * p0 - (fProxy)5 * p1 + (fProxy)4 * p2 - p3) * t * t
        ...
        /// </summary>
        public static void sampleAtInto(in fProxyN data, in fProxyN positions, ref fProxyN dest,
        ...
        /// </summary>
        public static void resampleInto(in fProxyN src, ref fProxyN dst, Interp interp, EdgeMode edge)
        {
            if (src.N < 1)
        ...
        /// </summary>
        public static void resample2DInto(in fProxyMxN src, ref fProxyMxN dst, Interp interp, EdgeMode edge)
        {
            ...
            var scratch = new fProxyMxN(srcM, dstN, Allocator.Temp);

            // ---- Pass 1: Horizontal (columns within each row, srcN → dstN) ----
            fProxy hScale = dstN > 1 ? (fProxy)(srcN - 1) / (fProxy)(dstN - 1) : (fProxy)0;
            ...
            fProxy vScale = dstM > 1 ? (fProxy)(srcM - 1) / (fProxy)(dstM - 1) : (fProxy)0;
