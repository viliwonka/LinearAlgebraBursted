...
namespace LinearAlgebra
{
    public static partial class Eigen
    {
        ...
        /// <c>Arena.fProxyLanczosCache(n, steps)</c>. Also validates the nested symmetric-eigenvalue
        /// workspace (sized to <paramref name="steps"/>, since the tridiagonal T is steps x steps).
        /// </summary>
        static void RequireLanczosWorkspace(in fProxyLanczosCache ws, int n, int steps)
        {
            if (ws.V.M_Rows != steps || ws.V.N_Cols != n)
                throw new ArgumentException("Eigen.lanczos: workspace V must be steps x n (use Arena.fProxyLanczosCache(n, steps))");

            if (ws.vCur.N != n || ws.w.N != n)
                throw new ArgumentException("Eigen.lanczos: workspace vCur/w must have length n (use Arena.fProxyLanczosCache(n, steps))");

            if (ws.alpha.N != steps || ws.beta.N != steps)
                throw new ArgumentException("Eigen.lanczos: workspace alpha/beta must have length steps (use Arena.fProxyLanczosCache(n, steps))");

            if (!ws.T.IsSquare || ws.T.M_Rows != steps)
                throw new ArgumentException("Eigen.lanczos: workspace T must be steps x steps (use Arena.fProxyLanczosCache(n, steps))");
    ...
    /// <see cref="Eigen.valuesSymmetricInPlace(ref fProxyMxN, ref fProxyN, ref fProxyEigenSymCache)"/>
    ...
    public static partial class ArenaExtensions
    {
        ...
        /// iterations. See <see cref="fProxyLanczosCache"/> for reuse guidance.
        /// </summary>
