...
namespace LinearAlgebra
{
    ...
    public static partial class CHOP {
        ...
        /// </summary>
        public static RankInfo decomp(in fProxyMxN A, ref fProxyMxN L, ref Pivot P,
                                                       ref fProxyCHOPCache ws) {
            if (!A.IsSquare)
            ...
            unsafe {
                ...
                var diagBuf = new fProxyN(n, Allocator.Temp, false);
                fProxy* diagp = diagBuf.Data.Ptr;
                ...
                for (int j0 = 0; j0 < n; j0 += CHOLP_BLOCK) {
                    ...
                    for (int k = j0; k < panelEnd; k++) {
                        ...
                        int q = k;
                        fProxy maxDiag = diagp[k] - dotp[k];
                        fProxy minDiag = maxDiag;
                        for (int i = k + 1; i < n; i++) {
                            fProxy d = diagp[i] - dotp[i];
                        ...
                        if (q != k) {
                            ...
                            { fProxy t = diagp[k]; diagp[k] = diagp[q]; diagp[q] = t; }
        ...
        public static RankInfo decomp(in fProxyMxN A, ref fProxyMxN L, ref Pivot P) {
            ...
            var ws = new fProxyCHOPCache
            {
                W = new fProxyMxN(n, n, Allocator.Temp),
                bt = default
            };
        ...
        public static DirectSolveInfo decompSolve(ref fProxyMxN L, in Pivot P, int rank, ref fProxyN b_to_x,
                                              ref fProxyCHOPCache ws) {
            if (!L.IsSquare)
        ...
        public static DirectSolveInfo decompSolve(ref fProxyMxN L, in Pivot P, int rank, ref fProxyN b_to_x) {
            ...
            {
                W = default,
                bt = new fProxyN(n, Allocator.Temp)
            };
        ...
        /// <param name="b_to_x">On entry b; on exit the solution x.</param>
        public static RankInfo solveInPlace(ref fProxyMxN A_to_L, ref Pivot P, ref fProxyN b_to_x) {
        ...
        public static unsafe DirectSolveInfo decompSolve(ref fProxyMxN L, in Pivot P, int rank, ref fProxyMxN B_to_X) {
            if (!L.IsSquare)
                throw new ArgumentException("decompSolve: L must be square");
            ...
            // X = A⁺B = 0 for the zero matrix.
            if (rank == 0) {
                fProxy* Xz = B_to_X.Data.Ptr;
                for (long e = 0; e < (long)n * k; e++) Xz[e] = (fProxy)0;
                return new DirectSolveInfo { status = DirectSolveStatus.Success };
            }

            // B̃[i,:] = B[P[i],:] (apply the symmetric permutation to the RHS rows).
            var Bt = new fProxyMxN(n, k, Allocator.Temp, false);
            {
                fProxy* Btp = Bt.Data.Ptr;
                fProxy* Xp = B_to_X.Data.Ptr;
                for (int i = 0; i < n; i++)
                {
                    fProxy* dst = Btp + (long)i * k;
                    fProxy* src = Xp + (long)P[i] * k;
            ...
            if (rank == n) {
                ...
                for (int i = 0; i < n; i++)
                {
                    fProxy* dst = Xp + (long)P[i] * k;
                    fProxy* src = Btp + (long)i * k;
                    for (int c = 0; c < k; c++) dst[c] = src[c];
                }
                Bt.Dispose();
                return new DirectSolveInfo { status = DirectSolveStatus.Success };
            ...
            var Grhs = new fProxyMxN(r, k, Allocator.Temp, false);
            {
                fProxy* Gp = Grhs.Data.Ptr;
                fProxy* Btp = Bt.Data.Ptr;
                for (int c = 0; c < r; c++)
                {
                    fProxy* Grow = Gp + (long)c * k;
                    for (int i = c; i < n; i++)
            ...
            {
                fProxy* Zp = Grhs.Data.Ptr;
                fProxy* Xp = B_to_X.Data.Ptr;
                for (int i = 0; i < n; i++)
                {
                    fProxy* dst = Xp + (long)P[i] * k;
                    for (int c = 0; c < k; c++) dst[c] = (fProxy)0;
        ...
        /// <param name="A_to_L">On entry A; on exit the lower-triangular factor L.</param>
        /// <param name="B_to_X">On entry B (n rows x k cols); on exit the solution X.</param>
        public static RankInfo solveInPlace(ref fProxyMxN A_to_L, ref Pivot P, ref fProxyMxN B_to_X) {
            if (!A_to_L.IsSquare)
                throw new ArgumentException("solveInPlace: A_to_L needs to be square");
