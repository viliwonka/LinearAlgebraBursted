...
namespace LinearAlgebra
{
    ...
    /// with a spare stamp (fProxyMxN/iProxyMxN/boolMxN/fProxyBSR) compare it against the table's
    /// current generation on every read to catch a stale handle into a since-recycled slot; types
    /// without one (fProxyN/iProxyN/boolN/fProxyBlockJacobi) check <see cref="IsAlive"/> alone.</para>
    /// </summary>
