...
{
    /// <summary>
    /// Arena-owned, pointer-stable allocation record backing an iProxyN (mirrors fProxyVecRecord,
    ...
    /// <see cref="Table"/>, declared on Arena.iProxy.cs's ArenaCore partial) and is addressed by
    /// iProxyN's private <c>iProxyVecRecord*</c> field. Never copied by user code: a struct copy
    /// of iProxyN just copies the pointer to this SAME record, so every copy resolves to the one
