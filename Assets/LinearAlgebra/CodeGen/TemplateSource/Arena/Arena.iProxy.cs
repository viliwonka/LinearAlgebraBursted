...
namespace LinearAlgebra
{
    ...
    {
        // Pointer-stable allocation-record tables: iProxyN/iProxyMxN hold a stable
        // iProxyVecRecord*/iProxyMatRecord* pointing INTO one of these tables (records never move
    ...
    public unsafe partial struct Arena {
        ...
        internal iProxyN iProxyVec(in iProxyN orig)
        {
            ...
            try
            {
                ...
                rec->Table = &_core->iProxyVecRecords;
                rec->SelfIndex = slot;
                return new iProxyN(in orig, rec, Allocator);   // persistent (backs Copy())
#if ENABLE_UNITY_COLLECTIONS_CHECKS
            }
            finally { _core->ExitMutation(); }
#endif
        ...
        {
            // forward to the (rows, cols) overload so the matrix is TRACKED in iProxyMatRecords. NOT
        ...
        internal iProxyMxN iProxyTempMat(int M_rows, int N_cols, bool uninit = false)
        {
            if (_core == null)
                throw new System.InvalidOperationException("Arena.iProxyVec/iProxyMat: arena is not initialized (default or disposed).");
#if ENABLE_UNITY_COLLECTIONS_CHECKS
