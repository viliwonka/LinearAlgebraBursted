...
{
    /// <summary>
    /// Arena-owned, pointer-stable allocation record backing a boolN (mirrors the analogous
    /// per-type record struct every other migrated family carries, e.g. floatVecRecord/
    /// intVecRecord). Lives inside one of the <see cref="ChunkedRecordTable{TRecord}"/> tables
    /// declared directly on ArenaCore in Arena.cs, and is addressed by boolN's private
    /// <c>boolVecRecord*</c> field. Never copied by user code: a struct copy of boolN just copies
    /// the pointer to this SAME record, so every copy resolves to the one authoritative
    /// <see cref="Data"/>.
    /// </summary>
