...
//alsoExpand[uint]// widens this file's int/short/long rotation to include uint, so `Hash` gets full
...
namespace LinearAlgebra
{
    public static partial class Hash
    {
        ...
        public static uint hash(in iProxyN v, uint seed = 0)
        {
            unsafe { return hash((byte*)v.Data.Ptr, v.Data.Length * sizeof(iProxy), seed); }
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static uint hash(in iProxyMxN v, uint seed = 0)
        {
            unsafe { return hash((byte*)v.Data.Ptr, v.Data.Length * sizeof(iProxy), seed); }
        }
        ...
        // always spelled via the real `iProxyN` placeholder token but immediately CHOSEN to the fixed
        ...
        /// </summary>
        public static void rowHashes(in iProxyMxN A, ref /*+choose[uintN|uintN|uintN|uintN]*/iProxyN/*-choose*/ dest, uint seed = 0)
        {
            if (dest.N != A.M_Rows)
            ...
            unsafe
            {
                int rowBytes = A.N_Cols * sizeof(iProxy);
        ...
        /// </summary>
        public static void colHashes(in iProxyMxN A, ref /*+choose[uintN|uintN|uintN|uintN]*/iProxyN/*-choose*/ dest, uint seed = 0)
        {
            if (dest.N != A.N_Cols)
            ...
            var col = A.iProxyTempVec(A.M_Rows);
            unsafe
            {
                int byteLen = A.M_Rows * sizeof(iProxy);
                uint* destPtr = (uint*)dest.Data.Ptr;
        ...
        // Hash.uint.cs, where the plain `iProxyN`/`iProxyVec` placeholder tokens below resolve to
        ...
        /// </summary>
        public static void rowHashes(in boolMxN A, ref iProxyN dest, uint seed = 0)
        {
            if (dest.N != A.M_Rows)
                throw new ArgumentException("Hash.rowHashes: dest.N must equal A.M_Rows");
        ...
        /// hash approach is what makes the "same result as a standalone vector" guarantee above hold.
        /// </summary>
        public static void colHashes(in boolMxN A, ref iProxyN dest, uint seed = 0)
        {
            if (dest.N != A.N_Cols)
                throw new ArgumentException("Hash.colHashes: dest.N must equal A.N_Cols");
