...
namespace LinearAlgebra
{
    ...
    // the bare `Stats` facade in Stats.iProxy.cs. Kept SPLIT (intStatsCore/shortStatsCore/
    ...
    internal static partial class iProxyStatsCore
    {
        ...
        // (see StatsTests.iProxy.cs's SumAccumulatorOwnOverflow, which pins this contrast: the
        // same 2-element/MaxValue-filled input is correct-and-widened for int/short but silently
        // wraps for long).
        public static long sum<T>(in T x) where T : unmanaged, IUnsafeiProxyArray {
        ...
        public static double mean<T>(in T x) where T : unmanaged, IUnsafeiProxyArray {
        ...
        // construction.
        public static double variance<T>(in T x) where T : unmanaged, IUnsafeiProxyArray {
            if (x.Data.Length == 0)
        ...
        public static double stdDev<T>(in T x) where T : unmanaged, IUnsafeiProxyArray {
        ...
        public static double varianceSample<T>(in T x) where T : unmanaged, IUnsafeiProxyArray {
            if (x.Data.Length == 0)
        ...
        public static double stdDevSample<T>(in T x) where T : unmanaged, IUnsafeiProxyArray {
            return math.sqrt(varianceSample(in x));
        }

        // Returns the index of the smallest element (first occurrence on ties). For a matrix
        // (IUnsafeiProxyArray over row-major data), the index is the linear index r*N_Cols+c.
        public static int argmin<T>(in T x) where T : unmanaged, IUnsafeiProxyArray {
            if (x.Data.Length == 0)
                throw new InvalidOperationException("Cannot compute argmin of an empty array.");

            iProxy best = x.Data[0];
            int bestIdx = 0;
        ...
        // as argmin above.
        public static int argmax<T>(in T x) where T : unmanaged, IUnsafeiProxyArray {
            if (x.Data.Length == 0)
        ...
        public static iProxy min<T>(in T x) where T : unmanaged, IUnsafeiProxyArray {
            if (x.Data.Length == 0)
        ...
        public static iProxy max<T>(in T x) where T : unmanaged, IUnsafeiProxyArray {
            if (x.Data.Length == 0)
        ...
        public static double median<T>(in T x) where T : unmanaged, IUnsafeiProxyArray {
            if (x.Data.Length == 0)
            ...
            var copy = new UnsafeList<iProxy>(x.Data.Length, Allocator.Temp, NativeArrayOptions.UninitializedMemory);
