...
namespace LinearAlgebra
{
    ...
    internal static partial class fProxyHistogramCore
    {
        // -------------------------------------------------------------------------
            ...
            where T : unmanaged, IUnsafefProxyArray
        {
            ...
            fProxy invNW = (fProxy)K / ((fProxy)N * (hi - lo));
            for (int b = 0; b < K; b++)
                dest[b] = (fProxy)scratch[b] * invNW;
            ...
            where T : unmanaged, IUnsafefProxyArray
        {
            ...
            {
                fProxy invTotal = (fProxy)1 / (fProxy)inRangeTotal;
                int cum = 0;
                for (int b = 0; b < K; b++)
                {
                    cum += scratch[b];
                    dest[b] = (fProxy)cum * invTotal;
                }
                dest[K - 1] = (fProxy)1;   // pin last bin to bit-exact 1 (cum*invTotal may be 1-ulp short)
            }
        ...
        public static void histogram2DInto<TX, TY>(
            ...
            fProxy loX, fProxy hiX,
            fProxy loY, fProxy hiY,
            ref fProxyMxN counts)
            where TX : unmanaged, IUnsafefProxyArray
            where TY : unmanaged, IUnsafefProxyArray
        {
            ...
            fProxy wX = (hiX - loX) / (fProxy)Kx;
            fProxy wY = (hiY - loY) / (fProxy)Ky;

            int n = dataX.Data.Length;
            for (int i = 0; i < n; i++)
            {
                fProxy x = dataX.Data[i];
                fProxy y = dataY.Data[i];
