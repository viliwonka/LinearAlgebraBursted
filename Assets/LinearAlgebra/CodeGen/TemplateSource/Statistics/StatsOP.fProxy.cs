...
namespace LinearAlgebra.Stats
{
    ...
    public static partial class fProxyStatsOP  {
        ...
        public static void rowMean(in fProxyMxN A, ref fProxyN dest)
        {
            ...
            fProxyOP.divInpl(dest, A.N_Cols);
        }
        ...
        public static void colMean(in fProxyMxN A, ref fProxyN dest)
        {
            ...
            fProxyOP.divInpl(dest, A.M_Rows);
        }
        ...
        // this every frame leaks nothing (unlike a tempfProxyVec, which lives until ClearTemp).
        ...
        //   return — callers on per-frame paths leak nothing (unlike tempfProxyVec / arena temp).
        //
        ...
        /// </summary>
        public static void normalizeColumns(ref fProxyMxN A, NormalizeMode mode)
        {
            if (A.M_Rows == 0 || A.N_Cols == 0)
            ...
            var s0 = new fProxyN(A.N_Cols, Allocator.Temp); // min  (MinMax) or mean   (ZScore)
            var s1 = new fProxyN(A.N_Cols, Allocator.Temp); // max  (MinMax) or stdDev (ZScore)
        ...
        /// </summary>
        public static void normalizeRows(ref fProxyMxN A, NormalizeMode mode)
        {
            if (A.M_Rows == 0 || A.N_Cols == 0)
                throw new System.InvalidOperationException("Cannot normalize an empty matrix.");

            // Two Temp scratch vectors; both freed before return (no arena leak).
            var s0 = new fProxyN(A.M_Rows, Allocator.Temp); // min  (MinMax) or mean   (ZScore)
            var s1 = new fProxyN(A.M_Rows, Allocator.Temp); // max  (MinMax) or stdDev (ZScore)
