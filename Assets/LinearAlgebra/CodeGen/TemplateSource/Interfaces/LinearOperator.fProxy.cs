namespace LinearAlgebra
{
    ...
    /// construction. The operator-shaped sibling of <see cref="fProxyIdentityPreconditioner"/> --
    ...
    /// <c>TOp</c>), so passing <c>fProxyColScaledOperator&lt;fProxyDenseOperator&gt;</c> (or the BSR
    ...
    /// guarantee this by allocating fresh). Readonly, like <see cref="fProxyDenseOperator"/>. NOTE:
    ...
    /// Pock–Chambolle equilibration used by <see cref="LP.pdlp(in fProxyMxN,in fProxyN,in fProxyN,in fProxyN,in fProxyN,in fProxyN,ref fProxyN,out double,int,double)"/>:
    ...
    /// to Apply/ApplyT. Readonly, like <see cref="fProxyDenseOperator"/>.
    /// </summary>
    public readonly struct fProxyRowColScaledOperator<TInner> : IfProxyLinearOperator
        where TInner : struct, IfProxyLinearOperator
    {
        public readonly TInner Inner;
        public readonly fProxyN Dr;        // length Inner.Rows: row (output) scale
        public readonly fProxyN Dc;        // length Inner.Cols: column (input) scale
        public readonly fProxyN ScratchN;  // length Inner.Cols: workspace for Apply (holds dc .* x)
        public readonly fProxyN ScratchM;  // length Inner.Rows: workspace for ApplyT (holds dr .* x)

        public fProxyRowColScaledOperator(in TInner inner, in fProxyN dr, in fProxyN dc,
                                          in fProxyN scratchN, in fProxyN scratchM)
        {
            if (dr.N != inner.Rows) throw new System.ArgumentException("fProxyRowColScaledOperator: dr.N must equal inner.Rows");
            if (dc.N != inner.Cols) throw new System.ArgumentException("fProxyRowColScaledOperator: dc.N must equal inner.Cols");
            if (scratchN.N != inner.Cols) throw new System.ArgumentException("fProxyRowColScaledOperator: scratchN.N must equal inner.Cols");
            if (scratchM.N != inner.Rows) throw new System.ArgumentException("fProxyRowColScaledOperator: scratchM.N must equal inner.Rows");
        ...
        public void ApplyBlock(in fProxyMxN Vrows, ref fProxyMxN AVrows, int rows)
        {
            int cols = Vrows.N_Cols;
            var rin = new fProxyN(cols, Unity.Collections.Allocator.Temp, false);
            var rout = new fProxyN(Rows, Unity.Collections.Allocator.Temp, false);
            for (int i = 0; i < rows; i++)
            {
                for (int c = 0; c < cols; c++) rin[c] = Vrows[i, c];
                Apply(in rin, ref rout);
