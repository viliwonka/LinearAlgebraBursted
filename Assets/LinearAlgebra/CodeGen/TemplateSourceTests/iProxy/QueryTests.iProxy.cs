...
public class iProxyQueryTests
{
    ...
    public struct TestJob : IJob
    {
        ...
        void ArgMaxMinAbsVector()
        {
            ...
            iProxyQueryOP.argMaxAbs(in v, out iProxy maxVal, out int maxIdx);
            AssertEqI(maxIdx, 1);                  // first of the two |5| entries
            AssertEqV(maxVal, (iProxy)5);

            iProxyQueryOP.argMinAbs(in v, out iProxy minVal, out int minIdx);
            ...
            one[0] = (iProxy)(-7);
            iProxyQueryOP.argMaxAbs(in one, out iProxy ov, out int oi);
            AssertEqI(oi, 0);
        ...
        void ArgMaxMinAbsMatrix()
        {
            ...
            iProxyQueryOP.argMaxAbs(in A, out iProxy maxVal, out int maxIdx);
            AssertEqI(maxIdx, 4);                  // (r,c) = (4/3, 4%3) = (1,1)
            AssertEqV(maxVal, (iProxy)4);

            // minAbs is the 0 at flat 3 (r1,c0).
            iProxyQueryOP.argMinAbs(in A, out iProxy minVal, out int minIdx);
        ...
        void RowColArgMinMax()
        {
            ...
            int nr = iProxyQueryOP.rowArgMin(in A, ref idxR, ref valR);
            AssertEqI(nr, 3);
            ...
            iProxyQueryOP.rowArgMin(in A, ref idxR2);
            AssertEqI(idxR2[0], 1); AssertEqI(idxR2[1], 1); AssertEqI(idxR2[2], 0);

            iProxyQueryOP.rowArgMax(in A, ref idxR, ref valR);
            ...
            iProxyQueryOP.rowArgMax(in A, ref idxR3);
            ...
            int nc = iProxyQueryOP.colArgMin(in A, ref idxC, ref valC);
            AssertEqI(nc, 3);
            ...
            iProxyQueryOP.colArgMin(in A, ref idxC2);
            AssertEqI(idxC2[0], 2); AssertEqI(idxC2[1], 0); AssertEqI(idxC2[2], 0);

            iProxyQueryOP.colArgMax(in A, ref idxC, ref valC);
            ...
            iProxyQueryOP.colArgMax(in A, ref idxC3);
        ...
        void ColArgStrided()
        {
            ...
            iProxyQueryOP.colArgMin(in A, ref idxC, ref valC);
        ...
        void ArgMaxRowNorm()
        {
            ...
            AssertEqI(iProxyQueryOP.argMaxRowNorm(in A, Norm.L1), 2);
            AssertEqI(iProxyQueryOP.argMaxRowNorm(in A, Norm.Linf), 0);
            ...
            AssertEqI(iProxyQueryOP.argMaxRowNorm(in T, Norm.L1), 0);
        ...
        void ArgMaxColNorm()
        {
            ...
            AssertEqI(iProxyQueryOP.argMaxColNorm(in A, Norm.L1), 0);
            AssertEqI(iProxyQueryOP.argMaxColNorm(in A, Norm.Linf), 2);
        ...
        void DistancesToRowAllMetrics()
        {
            ...
            iProxyQueryOP.distancesToRow(in A, in q, Metric.Manhattan, ref d);
            AssertEqV(d[0], (iProxy)7); AssertEqV(d[1], (iProxy)1);

            // SqEuclidean (unsquared, no sqrt): 25 ; 1
            iProxyQueryOP.distancesToRow(in A, in q, Metric.SqEuclidean, ref d);
            AssertEqV(d[0], (iProxy)25); AssertEqV(d[1], (iProxy)1);

            // Chebyshev: max(3,4)=4 ; max(1,0)=1
            iProxyQueryOP.distancesToRow(in A, in q, Metric.Chebyshev, ref d);
            AssertEqV(d[0], (iProxy)4); AssertEqV(d[1], (iProxy)1);

            // Dot with q=(0,0) is 0 for every row.
            iProxyQueryOP.distancesToRow(in A, in q, Metric.Dot, ref d);
            ...
            iProxyQueryOP.distancesToRow(in A, in q2, Metric.Dot, ref d);
        ...
        void DistancesToColumnAllMetrics()
        {
            ...
            // Manhattan: c0=4, c1=4
            iProxyQueryOP.distancesToColumn(in A, in q, Metric.Manhattan, ref d);
            AssertEqV(d[0], (iProxy)4); AssertEqV(d[1], (iProxy)4);

            // SqEuclidean: c0 = 9+1 = 10 ; c1 = 16+0 = 16
            iProxyQueryOP.distancesToColumn(in A, in q, Metric.SqEuclidean, ref d);
            AssertEqV(d[0], (iProxy)10); AssertEqV(d[1], (iProxy)16);

            // Chebyshev: c0=max(3,1)=3 ; c1=max(4,0)=4
            iProxyQueryOP.distancesToColumn(in A, in q, Metric.Chebyshev, ref d);
            AssertEqV(d[0], (iProxy)3); AssertEqV(d[1], (iProxy)4);

            // Dot with q=(0,0) is 0 for every column.
            iProxyQueryOP.distancesToColumn(in A, in q, Metric.Dot, ref d);
            AssertEqV(d[0], (iProxy)0); AssertEqV(d[1], (iProxy)0);
        ...
        void NearestFarthestDistance()
        {
            ...
            iProxyQueryOP.nearestRow(in A, in q, Metric.SqEuclidean, out int ni, out iProxy ns);
            AssertEqI(ni, 0); AssertEqV(ns, (iProxy)0);

            iProxyQueryOP.farthestRow(in A, in q, Metric.SqEuclidean, out int fi, out iProxy fs);
            AssertEqI(fi, 1); AssertEqV(fs, (iProxy)25);

            // Manhattan: distances 0, 7, 2. nearest=r0, farthest=r1 (score 7).
            iProxyQueryOP.nearestRow(in A, in q, Metric.Manhattan, out int ni2, out iProxy ns2);
            AssertEqI(ni2, 0); AssertEqV(ns2, (iProxy)0);
            iProxyQueryOP.farthestRow(in A, in q, Metric.Manhattan, out int fi2, out iProxy fs2);
        ...
        void NearestFarthestSimilarity()
        {
            ...
            iProxyQueryOP.nearestRow(in A, in q, Metric.Dot, out int ni, out iProxy ns);
            AssertEqI(ni, 1); AssertEqV(ns, (iProxy)10);

            iProxyQueryOP.farthestRow(in A, in q, Metric.Dot, out int fi, out iProxy fs);
            ...
            iProxyQueryOP.nearestColumn(in A, in q3, Metric.Dot, out int cni, out iProxy cns);
            AssertEqI(cni, 0); AssertEqV(cns, (iProxy)1);
            iProxyQueryOP.farthestColumn(in A, in q3, Metric.Dot, out int cfi, out iProxy cfs);
        ...
        void KNearestBruteForce()
        {
            ...
            int cnt = iProxyQueryOP.kNearestRows(in A, in q, k, Metric.SqEuclidean, ref idx, ref scores);
            AssertEqI(cnt, k);

            var all = arena.iProxyVec(M);
            iProxyQueryOP.distancesToRow(in A, in q, Metric.SqEuclidean, ref all);
            ...
            int cnt2 = iProxyQueryOP.kNearestRows(in A, in q, k, Metric.Dot, ref idx2, ref scores2);
            AssertEqI(cnt2, k);
            var allDot = arena.iProxyVec(M);
            iProxyQueryOP.distancesToRow(in A, in q, Metric.Dot, ref allDot);
            ...
            int cntC = iProxyQueryOP.kNearestColumns(in A, in qc, kc, Metric.SqEuclidean, ref idxC, ref scoresC);
            AssertEqI(cntC, kc);
            var allC = arena.iProxyVec(N);
            iProxyQueryOP.distancesToColumn(in A, in qc, Metric.SqEuclidean, ref allC);
        ...
        void KNearestClampAndZero()
        {
            ...
            int z = iProxyQueryOP.kNearestRows(in A, in q, 0, Metric.SqEuclidean, ref idx, ref scores);
            AssertEqI(z, 0);
        ...
        void KFarthest()
        {
            ...
            var idx = arena.Indices(k);
            var scores = arena.iProxyVec(k);
            int cnt = iProxyQueryOP.kFarthestRows(in A, in q, k, Metric.SqEuclidean, ref idx, ref scores);
            AssertEqI(cnt, k);
            ...
            var idxC = arena.Indices(kc);
            var scoresC = arena.iProxyVec(kc);
            int cntC = iProxyQueryOP.kFarthestColumns(in A, in qc, kc, Metric.SqEuclidean, ref idxC, ref scoresC);
            AssertEqI(cntC, kc);
            var allC = arena.iProxyVec(N);
            iProxyQueryOP.distancesToColumn(in A, in qc, Metric.SqEuclidean, ref allC);
        ...
        void WithinRadiusBoundary()
        {
            ...
            int cnt = iProxyQueryOP.rowsWithinRadius(in A, in q, (iProxy)25, Metric.SqEuclidean, ref idx);
            int ccnt = iProxyQueryOP.countWithinRadius(in A, in q, (iProxy)25, Metric.SqEuclidean);
            AssertEqI(cnt, 3); AssertEqI(ccnt, 3);
            AssertEqI(idx[0], 0); AssertEqI(idx[1], 1); AssertEqI(idx[2], 2);

            // radius 24 -> r1 (distance 25) excluded.
            int cnt2 = iProxyQueryOP.rowsWithinRadius(in A, in q, (iProxy)24, Metric.SqEuclidean, ref idx);
            AssertEqI(cnt2, 2);
            AssertEqI(idx[0], 0); AssertEqI(idx[1], 2);
            AssertEqI(iProxyQueryOP.countWithinRadius(in A, in q, (iProxy)24, Metric.SqEuclidean), 2);
            ...
            int cnt3 = iProxyQueryOP.rowsWithinRadius(in A, in q2, (iProxy)1, Metric.Dot, ref idx);
            AssertEqI(cnt3, 2);
            AssertEqI(idx[0], 1); AssertEqI(idx[1], 2);
            AssertEqI(iProxyQueryOP.countWithinRadius(in A, in q2, (iProxy)1, Metric.Dot), 2);
            ...
            var idxc = arena.Indices(2);
            int ccol = iProxyQueryOP.columnsWithinRadius(in A, in qcol, (iProxy)10, Metric.SqEuclidean, ref idxc);
            AssertEqI(ccol, 1); AssertEqI(idxc[0], 0);
            AssertEqI(iProxyQueryOP.countWithinColumnRadius(in A, in qcol, (iProxy)10, Metric.SqEuclidean), 1);
        ...
        void FindValue()
        {
            ...
            AssertEqI(iProxyQueryOP.findValue(in v, (iProxy)2, (iProxy)0), 1);
            // absent -> -1
            AssertEqI(iProxyQueryOP.findValue(in v, (iProxy)9, (iProxy)0), -1);
            // integer tol: target 4, tol 1 -> first element with |x-4| <= 1 is the 3 at index 3.
            AssertEqI(iProxyQueryOP.findValue(in v, (iProxy)4, (iProxy)1), 3);
            // tol 0 of an absent target -> no match.
            AssertEqI(iProxyQueryOP.findValue(in v, (iProxy)4, (iProxy)0), -1);

            // matrix overload (flat index). 2x2 = [5, 6; 7, 6]; first 6 at flat 1.
            var A = arena.iProxyMat(2, 2);
            A[0, 0] = (iProxy)5; A[0, 1] = (iProxy)6;
            A[1, 0] = (iProxy)7; A[1, 1] = (iProxy)6;
            AssertEqI(iProxyQueryOP.findValue(in A, (iProxy)6, (iProxy)0), 1);
        ...
        void NonzeroCountNonzero()
        {
            ...
            AssertEqI(iProxyQueryOP.countNonzero(in v, (iProxy)0), 3);
            var idx = arena.Indices(6);
            int c = iProxyQueryOP.nonzero(in v, (iProxy)0, ref idx);
            AssertEqI(c, 3);
            AssertEqI(idx[0], 1); AssertEqI(idx[1], 3); AssertEqI(idx[2], 4);

            // tol=1 (strict |x|>tol): |1| filtered out -> indices 1,3 -> count 2
            AssertEqI(iProxyQueryOP.countNonzero(in v, (iProxy)1), 2);
            int c2 = iProxyQueryOP.nonzero(in v, (iProxy)1, ref idx);
            AssertEqI(c2, 2);
            ...
            AssertEqI(iProxyQueryOP.countNonzero(in A, (iProxy)0), 1);
            var idxA = arena.Indices(4);
            int ca = iProxyQueryOP.nonzero(in A, (iProxy)0, ref idxA);
        ...
        void MinValueEdge()
        {
            ...
            iProxyQueryOP.argMaxAbs(in v, out iProxy mv, out int mi);
            AssertEqI(mi, 0);
            ...
            AssertEqI(iProxyQueryOP.findValue(in v, (iProxy)iProxy.MinValue, (iProxy)0), 0);
        ...
        void ArenaWrappers()
        {
            ...
            iProxyQueryOP.distancesToRow(in A, in q, Metric.SqEuclidean, ref drRef);
            AssertEqI(dr.N, M);
            ...
            iProxyQueryOP.distancesToColumn(in A, in qc, Metric.SqEuclidean, ref dcolRef);
            AssertEqI(dcol.N, N);
            ...
            int refCnt = iProxyQueryOP.nonzero(in A, (iProxy)0, ref refNz);
            ...
            int refRRcnt = iProxyQueryOP.rowsWithinRadius(in A, in q, radius, Metric.SqEuclidean, ref refRR);
            ...
            int refCRcnt = iProxyQueryOP.columnsWithinRadius(in A, in qc, (iProxy)40, Metric.SqEuclidean, ref refCR);
            ...
            int refCntK = iProxyQueryOP.kNearestRows(in A, in q, k, Metric.SqEuclidean, ref refIdxK, ref refScoresK);
            ...
            int refCntKC = iProxyQueryOP.kNearestColumns(in A, in qc, k, Metric.SqEuclidean, ref refIdxKC, ref refScoresKC);
            ...
            int refCntKF = iProxyQueryOP.kFarthestRows(in A, in q, k, Metric.SqEuclidean, ref refIdxKF, ref refScoresKF);
            ...
            int refCntKFC = iProxyQueryOP.kFarthestColumns(in A, in qc, k, Metric.SqEuclidean, ref refIdxKFC, ref refScoresKFC);
    ...
    public void NormL2ThrowsTest()
    {
        ...
        Assert.Throws<ArgumentException>(() => iProxyQueryOP.argMaxRowNorm(in A, Norm.L2));
        Assert.Throws<ArgumentException>(() => iProxyQueryOP.argMaxColNorm(in A, Norm.L2));
        // L1 / Linf must NOT throw.
        Assert.DoesNotThrow(() => iProxyQueryOP.argMaxRowNorm(in A, Norm.L1));
        Assert.DoesNotThrow(() => iProxyQueryOP.argMaxColNorm(in A, Norm.Linf));
    ...
    public void MetricEuclideanCosineThrowTest()
    {
        ...
        Assert.Throws<ArgumentException>(() =>
            iProxyQueryOP.distancesToRow(in A, in q, Metric.Euclidean, ref dest));
        Assert.Throws<ArgumentException>(() =>
            iProxyQueryOP.distancesToRow(in A, in q, Metric.Cosine, ref dest));
        Assert.Throws<ArgumentException>(() =>
            iProxyQueryOP.nearestRow(in A, in q, Metric.Euclidean, out int _, out iProxy _));
        Assert.Throws<ArgumentException>(() =>
            iProxyQueryOP.farthestRow(in A, in q, Metric.Cosine, out int _, out iProxy _));
        Assert.Throws<ArgumentException>(() =>
            iProxyQueryOP.kNearestRows(in A, in q, 2, Metric.Euclidean, ref idx, ref scores));
        Assert.Throws<ArgumentException>(() =>
            iProxyQueryOP.kFarthestRows(in A, in q, 2, Metric.Cosine, ref idx, ref scores));
        Assert.Throws<ArgumentException>(() =>
            iProxyQueryOP.rowsWithinRadius(in A, in q, (iProxy)1, Metric.Euclidean, ref idx));
        Assert.Throws<ArgumentException>(() =>
            iProxyQueryOP.countWithinRadius(in A, in q, (iProxy)1, Metric.Cosine));
    ...
    public void DimensionMismatchThrowTest()
    {
        ...
        Assert.Throws<ArgumentException>(() =>
            iProxyQueryOP.distancesToRow(in A, in qBadRow, Metric.SqEuclidean, ref destRow));
        Assert.Throws<ArgumentException>(() =>
            iProxyQueryOP.nearestRow(in A, in qBadRow, Metric.SqEuclidean, out int _, out iProxy _));

        var qBadCol = arena.iProxyVec(4);         // wrong for col ops
        var destCol = arena.iProxyVec(4);
        Assert.Throws<ArgumentException>(() =>
            iProxyQueryOP.distancesToColumn(in A, in qBadCol, Metric.SqEuclidean, ref destCol));
        Assert.Throws<ArgumentException>(() =>
            iProxyQueryOP.countWithinColumnRadius(in A, in qBadCol, (iProxy)1, Metric.SqEuclidean));
