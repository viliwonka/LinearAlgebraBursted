...
//production Arena.iProxy.cs (whose factories ALSO expand to uintVec/uintMat). Every sentinel used
...
public class iProxyArenaWiringTests
{
    ...
    public unsafe struct WiringTestJob : IJob
    {
        ...
        void MultiChunkVectors()
        {
            ...
            const int n = 300;
            const int len = 3;
            iProxyN* vecs = stackalloc iProxyN[n];

            for (int i = 0; i < n; i++)
            {
                iProxyN v = arena.iProxyVec(len, uninit: true);
                for (int j = 0; j < len; j++)
                    v[j] = (iProxy)(i * len + j);   // distinct sentinel per (alloc, element)
                vecs[i] = v;
            }
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                // == against a (iProxy)-cast expected, not Assert.AreEqual(int, (int)cell): the
                ...
                // same-typed values sidesteps it and matches the established iProxy test idiom.
                for (int j = 0; j < len; j++)
                    Assert.IsTrue(vecs[i][j] == (iProxy)(i * len + j));
            }
        ...
        // ---- multi-chunk: 300 persistent matrices, same argument on the iProxyMatRecords table ----
        void MultiChunkMatrices()
        {
            ...
            const int n = 300;
            const int R = 2, C = 2;
            iProxyMxN* mats = stackalloc iProxyMxN[n];

            for (int i = 0; i < n; i++)
            {
                iProxyMxN m = arena.iProxyMat(R, C, uninit: true);
                for (int r = 0; r < R; r++)
                    for (int c = 0; c < C; c++)
                        m[r, c] = (iProxy)(i * (R * C) + r * C + c);
                mats[i] = m;
            }
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                for (int r = 0; r < R; r++)
                    for (int c = 0; c < C; c++)
                        Assert.IsTrue(mats[i][r, c] == (iProxy)(i * (R * C) + r * C + c));
            }
        ...
        void TempRecyclingCycles()
        {
            ...
            const int N = 20;
            var seedV = arena.iProxyVec(4, (iProxy)0);
            var seedM = arena.iProxyMat(3, 3, (iProxy)0);
            ...
            for (int cycle = 0; cycle < 3; cycle++)
            {
                for (int i = 0; i < N; i++)
                {
                    ...
                    iProxyN tv = seedV.TempCopy();
                    tv[0] = (iProxy)(cycle * 1000 + i);
                    Assert.IsTrue(tv[0] == (iProxy)(cycle * 1000 + i));

                    iProxyMxN tm = seedM.TempCopy();
                    tm[0, 0] = (iProxy)(cycle * 1000 + i);
                    Assert.IsTrue(tm[0, 0] == (iProxy)(cycle * 1000 + i));
                }
        ...
        // iProxyN.Dispose(), and (b) that a following Clear()/Dispose() SKIPS the already-freed slot
        // (IsAlive guard) instead of double-freeing it -- no crash, no leak.
        void DirectDisposeCountsThenClearVectors()
        {
            var arena = new Arena(Allocator.Persistent);

            var a = arena.iProxyVec(4, (iProxy)1);
            var b = arena.iProxyVec(4, (iProxy)2);
            var c = arena.iProxyVec(4, (iProxy)3);
            ...
            Assert.IsTrue(a[0] == (iProxy)1);
            Assert.IsTrue(c[0] == (iProxy)3);
        ...
        void DirectDisposeCountsThenClearMatrices()
        {
            ...
            var a = arena.iProxyMat(2, 2, (iProxy)1);
            var b = arena.iProxyMat(2, 2, (iProxy)2);
            var c = arena.iProxyMat(2, 2, (iProxy)3);
            Assert.AreEqual(3, arena.AllocationsCount);

            b.Dispose();
            Assert.AreEqual(2, arena.AllocationsCount);

            Assert.IsTrue(a[0, 0] == (iProxy)1);
            Assert.IsTrue(c[0, 0] == (iProxy)3);
        ...
        void DisposeThenReallocateRecyclesSlot()
        {
            ...
            var v1 = arena.iProxyVec(4, uninit: true);
            for (int j = 0; j < 4; j++) v1[j] = (iProxy)(100 + j);   // sentinel written into v1's buffer
            Assert.AreEqual(1, arena.AllocationsCount);

            v1.Dispose();                                            // frees the slot back to the free-list
            Assert.AreEqual(0, arena.AllocationsCount);

            var v2 = arena.iProxyVec(4, uninit: false);              // should recycle v1's freed slot
            Assert.AreEqual(1, arena.AllocationsCount);
            Assert.AreEqual(4, v2.N);
            for (int j = 0; j < 4; j++)
                Assert.IsTrue(v2[j] == (iProxy)0);                    // zero-init: no stale sentinel bleed

            // And v2 is genuinely its own independent, writable buffer going forward.
            for (int j = 0; j < 4; j++) v2[j] = (iProxy)(200 + j);
            for (int j = 0; j < 4; j++) Assert.IsTrue(v2[j] == (iProxy)(200 + j));
    ...
    //
    // (An alias is REQUIRED to trip the guard: iProxyN.Dispose() nulls its own _rec, so calling
    ...
    public void AliasedDoubleDispose_Vector_Throws()
    {
        ...
        try
        {
            var v = arena.iProxyVec(8, (iProxy)0);
            var alias = v;                 // struct copy -- shares v's iProxyVecRecord*
            Assert.AreEqual(1, arena.AllocationsCount);
