    ...
    public struct TestJob : IJob
    {
        ...
        void FullRankSPD()
        {
            ...
            for (uint t = 0; t < 12; t++)
            {
                ...
                RecordBound(Norms.L2(in b), (fProxy)1E-3f);
        ...
        void MinNormCertificate()
        {
            ...
            for (uint t = 0; t < 12; t++)
            {
                ...
                fProxy xNorm = Norms.L2(in b);
                fProxy origNorm = Norms.L2(in xOrig);
        ...
        void BlockedVsUnblockedReference()
        {
            ...
            fProxy factorTol = ReconstructPrecision(in A);
            fProxy maxFactorDiff = 0;
            for (int i = 0; i < n; i++)
                for (int j = 0; j < n; j++)
        ...
        fProxy ReconstructPrecision(in fProxyMxN A)
        {
            int n = A.M_Rows;
            fProxy maxRowSum = 0;
            for (int i = 0; i < n; i++)
            ...
            return maxRowSum * (fProxy)n * Consts.fProxyEpsilon * (fProxy)8;
        }
        ...
        void RecordEqExact(fProxy got, fProxy expected)
        {
