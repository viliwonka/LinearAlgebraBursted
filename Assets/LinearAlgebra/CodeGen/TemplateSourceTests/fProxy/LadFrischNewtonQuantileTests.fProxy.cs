...
public class fProxyLadFrischNewtonQuantileTests
{
    ...
    public void TauHalfMatchesCore()
    {
        ...
        b[2] = 10f;
        var xFN = new fProxyN(2, Allocator.Temp);
        var xCore = new fProxyN(2, Allocator.Temp);
    ...
    {
        int m = 80;
        var A = new fProxyMxN(m, 2, Allocator.Temp);
    ...
    public void QuantileFitMatchesBruteForceOptimum()
    {
        ...
        int m = t.Length;
        double relTol = /*+choose[5e-3|1e-7]*/5e-3/*-choose*/;
        ...
        for (int q = 0; q < taus.Length; q++)
        {
            ...
            var A = new fProxyMxN(m, 2, Allocator.Temp);
            var b = new fProxyN(m, Allocator.Temp);
            for (int i = 0; i < m; i++) { A[i, 0] = (fProxy)1; A[i, 1] = (fProxy)t[i]; b[i] = (fProxy)bb[i]; }
            var x = new fProxyN(2, Allocator.Temp);
