...
// Acceptance battery for MPC.solve / fProxyMPCState, covering the properties verified in the design's
// numpy/scipy prototype before this feature was implemented (double integrator A=[[1,1],[0,1]],
// B=[[0],[1]], Q=I2, R=1 throughout -- the SAME system ControlLQRTests.fProxy.cs's own
...
// recorded into Fail[0..3], read back on the managed side -- same shape as ControlLQRTests.fProxy.cs /
// KalmanTests.fProxy.cs.
public class fProxyMPCTests
{
    ...
    public struct TestJob : IJob
    {
        ...
        static fProxy LqrU0Tol() => /*+choose[5e-3f|1e-6]*/5e-3f/*-choose*/;
        static fProxy OracleU0Tol() => /*+choose[1e-2f|1e-5]*/1e-2f/*-choose*/;
        static double SlackTol() => /*+choose[1e-2|1e-5]*/1e-2/*-choose*/;
        ...
        void UnconstrainedMatchesLQR()
        {
            ...
            var uLo = Vec1((fProxy)(-1e30)); var uHi = Vec1((fProxy)1e30);   // library's own "unbounded" sentinel (QP.solve's own INF convention)
            ...
            var reference = new fProxyN(2, Allocator.Temp);   // zero-initialized: track to the origin
            var u0 = new fProxyN(1, Allocator.Temp, true);
            var rng = new Unity.Mathematics.Random(0xA53Cu);

            for (int trial = 0; trial < 10; trial++)
            {
                var x0 = new fProxyN(2, Allocator.Temp, true);
                x0[0] = (fProxy)rng.NextFloat(-2f, 2f);
                x0[1] = (fProxy)rng.NextFloat(-2f, 2f);
                ...
                fProxy u0lqr = -(K[0, 0] * x0[0] + K[0, 1] * x0[1]);
        ...
        void SaturatedMatchesOracle()
        {
            ...
            var uLo = Vec1((fProxy)(-2)); var uHi = Vec1((fProxy)2);
            var mpc = new fProxyMPCState(2, 1, 8, Allocator.Temp, in A, in B, in Q, in R, in uLo, in uHi);

            var x0 = Vec2(3, (fProxy)1.9);
            var reference = new fProxyN(2, Allocator.Temp);   // zero-initialized: track to the origin
            var u0 = new fProxyN(1, Allocator.Temp, true);
            ...
            AssertClose(u0[0], (fProxy)(-2.0), OracleU0Tol());
        ...
        void SoftRowInactiveMatchesNoSoftBuild()
        {
            ...
            var mpcPlain = new fProxyMPCState(2, 1, 8, Allocator.Temp, in A, in B, in Q, in R, in uLo, in uHi);
            var C = WallRow(); var d = Vec1((fProxy)5);
            var mpcSoft = new fProxyMPCState(2, 1, 8, Allocator.Temp, in A, in B, in Q, in R, in uLo, in uHi, in C, in d);

            var x0 = new fProxyN(2, Allocator.Temp);   // zero-initialized: origin, far from the wall
            var reference = new fProxyN(2, Allocator.Temp);   // zero-initialized: track to the origin
            var u0a = new fProxyN(1, Allocator.Temp, true);
            var u0b = new fProxyN(1, Allocator.Temp, true);
        ...
        void SoftRowAvoidableMatchesSaturatedOracle()
        {
            ...
            var uLo = Vec1((fProxy)(-2)); var uHi = Vec1((fProxy)2);
            var C = WallRow(); var d = Vec1((fProxy)5);
            var mpc = new fProxyMPCState(2, 1, 8, Allocator.Temp, in A, in B, in Q, in R, in uLo, in uHi, in C, in d);
        ...
        void WarmIterationsBeatCold()
        {
            var A = Mat2(1, 1, 0, 1); var B = ColVec2(0, 1); var Q = Eye(2); var R = R1(1);
            var uLo = Vec1((fProxy)(-0.5)); var uHi = Vec1((fProxy)0.5);
            var mpcWarm = new fProxyMPCState(2, 1, 8, Allocator.Temp, in A, in B, in Q, in R, in uLo, in uHi);
            ...
            var xAtTarget = new fProxyN(2, Allocator.Temp, true);
            int warmIters = 0;
            ...
            var mpcCold = new fProxyMPCState(2, 1, 8, Allocator.Temp, in A, in B, in Q, in R, in uLo, in uHi);
            var u0c = new fProxyN(1, Allocator.Temp, true);
        ...
        void PrestabBindingBoundMatchesNonPrestab()
        {
            ...
            var Kstab = new fProxyMxN(1, 2, Allocator.Temp);
            var lqrInfo = LQR.lqr(in A, in B, in Q, in R, ref Kstab);
            AssertTrue(lqrInfo.status == RiccatiStatus.Converged);

            var mpcPlain = new fProxyMPCState(2, 1, 8, Allocator.Temp, in A, in B, in Q, in R, in uLo, in uHi);
            var mpcPrestab = new fProxyMPCState(2, 1, 8, Allocator.Temp, in A, in B, in Q, in R, in uLo, in uHi,
                                                default, default, default, default(fProxyN), (fProxy)0, (fProxy)0, in Kstab);

            var x0 = Vec2(3, (fProxy)1.9);
            var reference = new fProxyN(2, Allocator.Temp);   // zero-initialized: track to the origin
            var u0Plain = new fProxyN(1, Allocator.Temp, true);
            var u0Prestab = new fProxyN(1, Allocator.Temp, true);
            ...
            fProxy uRecon = -(mpcPrestab.Kstab[0, 0] * x0[0] + mpcPrestab.Kstab[0, 1] * x0[1]) + mpcPrestab.z[0];
            ...
            AssertClose(u0Prestab[0], (fProxy)(-2.0), OracleU0Tol());   // both saturate at uLo, per the prototype
        ...
        static fProxyMxN ColVec2(fProxy a, fProxy b)
        {
            var M = new fProxyMxN(2, 1, Allocator.Temp);
            M[0, 0] = a; M[1, 0] = b;
            return M;
        }
        ...
        static fProxyMxN WallRow()
        {
            var M = new fProxyMxN(1, 2, Allocator.Temp);
            M[0, 0] = (fProxy)1; M[0, 1] = (fProxy)0;
            return M;
        }

        static fProxyN Vec1(fProxy v)
        {
            var vec = new fProxyN(1, Allocator.Temp, true);
            vec[0] = v;
            return vec;
        }

        static fProxyN Vec2(fProxy a, fProxy b)
        {
            var vec = new fProxyN(2, Allocator.Temp, true);
            vec[0] = a; vec[1] = b;
            return vec;
        }
        ...
        void SetSoftBoundMatchesRebuild()
        {
            ...
            var uLo = Vec1((fProxy)(-2)); var uHi = Vec1((fProxy)2);
            var C = WallRow();
            var dNear = Vec1((fProxy)5); var dFar = Vec1((fProxy)9);

            var built = new fProxyMPCState(2, 1, 8, Allocator.Temp, in A, in B, in Q, in R, in uLo, in uHi, in C, in dNear);
            var moved = new fProxyMPCState(2, 1, 8, Allocator.Temp, in A, in B, in Q, in R, in uLo, in uHi, in C, in dFar);
            var stale = new fProxyMPCState(2, 1, 8, Allocator.Temp, in A, in B, in Q, in R, in uLo, in uHi, in C, in dFar);
            MPC.setSoftBound(ref moved, in dNear);

            var x0 = Vec2(3, (fProxy)2.3);
            var reference = new fProxyN(2, Allocator.Temp);   // zero-initialized: track to the origin
            var u0Built = new fProxyN(1, Allocator.Temp, true);
            var u0Moved = new fProxyN(1, Allocator.Temp, true);
            var u0Stale = new fProxyN(1, Allocator.Temp, true);
        ...
        void SetSoftBoundPerStageBindsPerStage()
        {
            var A = Mat2(1, 1, 0, 1); var B = ColVec2(0, 1); var Q = Eye(2); var R = R1(1);
            var uLo = Vec1((fProxy)(-2)); var uHi = Vec1((fProxy)2);
            var C = WallRow(); var dFar = Vec1((fProxy)9);
            var mpc = new fProxyMPCState(2, 1, 8, Allocator.Temp, in A, in B, in Q, in R, in uLo, in uHi, in C, in dFar);

            var x0 = Vec2(3, (fProxy)2.3);
            var reference = new fProxyN(2, Allocator.Temp);
            var u0 = new fProxyN(1, Allocator.Temp, true);

            // A per-stage array whose entries are all equal must reproduce the held form exactly.
            var flat = new fProxyN(8, Allocator.Temp, true);
            for (int k = 0; k < 8; k++) flat[k] = (fProxy)5;
            ...
            var staged = new fProxyN(8, Allocator.Temp, true);
            staged[0] = (fProxy)5;
            for (int k = 1; k < 8; k++) staged[k] = (fProxy)9;
        ...
        void SetSoftBoundSurvivesWarmStart()
        {
            var A = Mat2(1, 1, 0, 1); var B = ColVec2(0, 1); var Q = Eye(2); var R = R1(1);
            var uLo = Vec1((fProxy)(-0.5)); var uHi = Vec1((fProxy)0.5);
            var C = WallRow(); var d0 = Vec1((fProxy)12);
            var mpc = new fProxyMPCState(2, 1, 8, Allocator.Temp, in A, in B, in Q, in R, in uLo, in uHi, in C, in d0);
            ...
            double objRelTol = /*+choose[5e-6|1e-12]*/5e-6/*-choose*/;
            fProxy zTol = /*+choose[5e-3f|1e-5]*/5e-3f/*-choose*/;

            bool everForced = false, everMoved = false;
            for (int f = 0; f < 16; f++)
            {
                wall[0] = (fProxy)12 - (fProxy)f;
        ...
        void AssertRelCloseD(double a, double b, double relTol)
        {
            ...
            if (!(rel <= relTol) && Fail[0] == (fProxy)0) { Fail[0] = (fProxy)1; Fail[1] = (fProxy)a; Fail[2] = (fProxy)b; Fail[3] = (fProxy)rel; }
    ...
    {
        var A = new fProxyMxN(2, 2, Allocator.Temp); A[0, 0] = 1; A[0, 1] = 1; A[1, 1] = 1;
        var B = new fProxyMxN(2, 1, Allocator.Temp); B[1, 0] = 1;
        var Q = new fProxyMxN(2, 2, Allocator.Temp); Q[0, 0] = 1; Q[1, 1] = 1;
        var R = new fProxyMxN(1, 1, Allocator.Temp); R[0, 0] = 1;
        var uLo = new fProxyN(1, Allocator.Temp, true); uLo[0] = (fProxy)(-1);
        var uHi = new fProxyN(1, Allocator.Temp, true); uHi[0] = (fProxy)1;
        var mpc = new fProxyMPCState(2, 1, 4, Allocator.Temp, in A, in B, in Q, in R, in uLo, in uHi);

        var badX0 = new fProxyN(3, Allocator.Temp, true);
        var reference = new fProxyN(2, Allocator.Temp);   // zero-initialized: track to the origin
        var u0 = new fProxyN(1, Allocator.Temp, true);
    ...
    public void SolveThrowsOnBadReferenceLength()
    {
        ...
        var x0 = new fProxyN(2, Allocator.Temp, true);
        var badReference = new fProxyN(3, Allocator.Temp, true);   // neither n(2) nor N*n(8)
        var u0 = new fProxyN(1, Allocator.Temp, true);
    ...
    public void SetSoftBoundThrowsOnBadInput()
    {
        ...
        var uLo = new fProxyN(1, Allocator.Temp, true); uLo[0] = (fProxy)(-1);
        var uHi = new fProxyN(1, Allocator.Temp, true); uHi[0] = (fProxy)1;
        var C = new fProxyMxN(1, 2, Allocator.Temp); C[0, 0] = 1;
        var d = new fProxyN(1, Allocator.Temp, true); d[0] = (fProxy)5;

        var plain = new fProxyMPCState(2, 1, 4, Allocator.Temp, in A, in B, in Q, in R, in uLo, in uHi);
        var soft = new fProxyMPCState(2, 1, 4, Allocator.Temp, in A, in B, in Q, in R, in uLo, in uHi, in C, in d);

        var ok = new fProxyN(1, Allocator.Temp, true); ok[0] = (fProxy)3;
        var badLen = new fProxyN(3, Allocator.Temp, true);           // neither k(1) nor N*k(4)
        var nan = new fProxyN(1, Allocator.Temp, true); nan[0] = (fProxy)double.NaN;
    ...
    public void ConstructorThrowsOnUnstabilizablePair()
    {
        ...
        // ControlLQRTests.fProxy.cs's own FailureUnstabilizable job -- the auto-DARE terminal cost
        // cannot be computed, so construction must fail rather than silently produce a bad terminal cost.
        var A = new fProxyMxN(2, 2, Allocator.Temp); A[0, 0] = 2; A[1, 1] = (fProxy)0.5;
        var B = new fProxyMxN(2, 1, Allocator.Temp); B[1, 0] = 1;
        var Q = new fProxyMxN(2, 2, Allocator.Temp); Q[0, 0] = 1; Q[1, 1] = 1;
        var R = new fProxyMxN(1, 1, Allocator.Temp); R[0, 0] = 1;
        var uLo = new fProxyN(1, Allocator.Temp, true); uLo[0] = (fProxy)(-1);
        ...
        Assert.Catch<ArgumentException>(() => new fProxyMPCState(2, 1, 4, Allocator.Temp, in A, in B, in Q, in R, in uLo, in uHi));
    ...
    // ---- Job-struct-copy regression: fProxyMPCState.populated must survive a plain .Run() by-value
    // copy -- same bug class as fProxyLQRState.populated / LPBasis.populated (see DemoSmokeTests.cs's
    ...
    {
        var A = new fProxyMxN(2, 2, Allocator.TempJob); A[0, 0] = 1; A[0, 1] = 1; A[1, 1] = 1;
        var B = new fProxyMxN(2, 1, Allocator.TempJob); B[1, 0] = 1;
        var Q = new fProxyMxN(2, 2, Allocator.TempJob); Q[0, 0] = 1; Q[1, 1] = 1;
        var R = new fProxyMxN(1, 1, Allocator.TempJob); R[0, 0] = 1;
        var uLo = new fProxyN(1, Allocator.TempJob, true); uLo[0] = (fProxy)(-1);
        var uHi = new fProxyN(1, Allocator.TempJob, true); uHi[0] = (fProxy)1;
        var mpc = new fProxyMPCState(2, 1, 4, Allocator.TempJob, in A, in B, in Q, in R, in uLo, in uHi);

        var x0 = new fProxyN(2, Allocator.TempJob, true);
        var reference = new fProxyN(2, Allocator.TempJob);   // zero-initialized: track to the origin
        var u0 = new fProxyN(1, Allocator.TempJob, true);
        ...
        Assert.IsTrue(mpc.populated,
            "fProxyMPCState.populated did not survive the .Run() by-value copy -- a native-backed flag is what makes this pass; a plain bool would be lost");
