...
// Tests for fProxyResampleOP (data-vector interpolation + 1D/2D resizing).
//
...
public class fProxyResampleTests
{
    ...
    public struct TestJob : IJob
    {
        ...
        void CheckIntegerPos(in fProxyN data, int ix)
        {
            ...
            AssertClose(fProxyResampleOP.sampleAt(in data, pos, Interp.Nearest, EdgeMode.Clamp), data[ix], (fProxy)0);
            AssertClose(fProxyResampleOP.sampleAt(in data, pos, Interp.Linear,  EdgeMode.Clamp), data[ix], (fProxy)0);
            AssertClose(fProxyResampleOP.sampleAt(in data, pos, Interp.Cubic,   EdgeMode.Clamp), data[ix], (fProxy)0);
        }
        ...
        void LinearMidpointMean()
        {
            ...
            fProxy mid01 = fProxyResampleOP.sampleAt(in data, (fProxy)0.5, Interp.Linear, EdgeMode.Clamp);
            AssertClose(mid01, (fProxy)15, (fProxy)10 * Consts.fProxySqrtEps);   // (10+20)/2

            fProxy mid12 = fProxyResampleOP.sampleAt(in data, (fProxy)1.5, Interp.Linear, EdgeMode.Clamp);
        ...
        {
            fProxy got = fProxyResampleOP.sampleAt(in data, pos, Interp.Cubic, EdgeMode.Clamp);
        ...
        fProxy Near(in fProxyN data, int pos, EdgeMode edge) =>
            fProxyResampleOP.sampleAt(in data, (fProxy)pos, Interp.Nearest, edge);
        ...
        void SampleAtIntoGather()
        {
            ...
            fProxyResampleOP.sampleAtInto(in data, in positions, ref dest, Interp.Cubic, EdgeMode.Mirror);

            for (int j = 0; j < k; j++)
            {
                fProxy expected = fProxyResampleOP.sampleAt(in data, positions[j], Interp.Cubic, EdgeMode.Mirror);
        ...
        void ResampleIdentityAlignedGrid()
        {
            ...
            fProxyResampleOP.resampleInto(in src, ref dstL, Interp.Linear, EdgeMode.Clamp);
            for (int i = 0; i < n; i++) AssertClose(dstL[i], src[i], (fProxy)0);

            var dstC = arena.fProxyVec(n);
            fProxyResampleOP.resampleInto(in src, ref dstC, Interp.Cubic, EdgeMode.Clamp);
        ...
        void ResampleEndpointsPreserved()
        {
            ...
            fProxyResampleOP.resampleInto(in up, ref dstUp, Interp.Cubic, EdgeMode.Clamp);
            ...
            fProxyResampleOP.resampleInto(in down, ref dstDown, Interp.Linear, EdgeMode.Clamp);
        ...
        void ResampleLinearRampStaysLinear()
        {
            ...
            fProxyResampleOP.resampleInto(in src, ref dst, Interp.Linear, EdgeMode.Clamp);
        ...
        void CheckIdentity2D(ref Arena arena, in fProxyMxN src, Interp interp)
        {
            ...
            fProxyResampleOP.resample2DInto(in src, ref dst, interp, EdgeMode.Clamp);
        ...
        void Resample2DCornersPreserved()
        {
            ...
            fProxyResampleOP.resample2DInto(in src, ref dst, Interp.Cubic, EdgeMode.Clamp);
        ...
        void Resample2DBilinearPlanar()
        {
            ...
            int M2 = 7, N2 = 9;
            var dst = arena.fProxyMat(M2, N2);
            fProxyResampleOP.resample2DInto(in src, ref dst, Interp.Linear, EdgeMode.Clamp);
    ...
    public void SampleAtEmptyThrows()
    {
        ...
        try
        {
            ...
            Assert.Throws<ArgumentException>(
                () => fProxyResampleOP.sampleAt(in empty, (fProxy)0, Interp.Linear, EdgeMode.Clamp));
        }
    ...
    public void SampleAtIntoValidates()
    {
        ...
        try
        {
            ...
            Assert.Throws<ArgumentException>(
                () => fProxyResampleOP.sampleAtInto(in data, in positions, ref destBad, Interp.Linear, EdgeMode.Clamp));
            ...
            var ex = Assert.Throws<ArgumentException>(
                () => fProxyResampleOP.sampleAtInto(in emptyData, in positions, ref dest, Interp.Linear, EdgeMode.Clamp));
    ...
    public void ResampleIntoValidates()
    {
        ...
        try
        {
            ...
            Assert.Throws<ArgumentException>(
                () => fProxyResampleOP.resampleInto(in src, ref emptyDst, Interp.Linear, EdgeMode.Clamp));

            var emptySrc = arena.fProxyVec(0);
            var dst = arena.fProxyVec(4);
            Assert.Throws<ArgumentException>(
                () => fProxyResampleOP.resampleInto(in emptySrc, ref dst, Interp.Linear, EdgeMode.Clamp));
        }
        finally { arena.Dispose(); }
    }
    ...
    public void Resample2DIntoValidates()
    {
        ...
        try
        {
            ...
            Assert.Throws<ArgumentException>(
                () => fProxyResampleOP.resample2DInto(in src, ref dstNoRows, Interp.Linear, EdgeMode.Clamp));

            // dst with 0 cols
            var dstNoCols = arena.fProxyMat(3, 0);
            Assert.Throws<ArgumentException>(
                () => fProxyResampleOP.resample2DInto(in src, ref dstNoCols, Interp.Linear, EdgeMode.Clamp));

            // src 0x0 (validated before any scratch allocation)
            var emptySrc = arena.fProxyMat(0, 0);
            var dst = arena.fProxyMat(2, 2);
            Assert.Throws<ArgumentException>(
                () => fProxyResampleOP.resample2DInto(in emptySrc, ref dst, Interp.Linear, EdgeMode.Clamp));
        }
        finally { arena.Dispose(); }
    }
}
