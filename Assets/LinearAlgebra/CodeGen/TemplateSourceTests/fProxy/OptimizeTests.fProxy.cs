...
public class fProxyOptimizeTests
{

    // ----- Functor structs (Burst-legal: only numeric fields, no managed state) -----

    // (x - 3)^2 - 4, roots at x = 1 and x = 5.
    public struct ShiftedParabola : IfProxyScalarFunction
    {
        public fProxy Eval(fProxy x) => (x - (fProxy)3) * (x - (fProxy)3) - (fProxy)4;
    }

    // cos(x): root at pi/2, minimum at pi.
    public struct Cos : IfProxyScalarFunction
    {
        public fProxy Eval(fProxy x) => Unity.Mathematics.math.cos(x);
    }

    // x^2 - 2, root at sqrt(2); derivative 2x.
    public struct QuadraticDeriv : IfProxyScalarDerivativeFunction
    {
        public fProxy Eval(fProxy x) => x * x - (fProxy)2;
        public fProxy Derivative(fProxy x) => (fProxy)2 * x;
    }

    // Constant non-zero value with zero derivative everywhere -> Newton must report failure (flat).
    public struct FlatDeriv : IfProxyScalarDerivativeFunction
    {
        public fProxy Eval(fProxy x) => (fProxy)5;
        public fProxy Derivative(fProxy x) => (fProxy)0;
    }

    // (x - 2)^2 + 1, minimum at x = 2.
    public struct ParabolaMin : IfProxyScalarFunction
    {
        public fProxy Eval(fProxy x) => (x - (fProxy)2) * (x - (fProxy)2) + (fProxy)1;
    }
    ...
    public struct Bowl : IfProxyGradientFunction
    {
        public fProxy Target(int i)
        {
            switch (i)
            {
                case 0: return (fProxy)1;
                case 1: return (fProxy)2;
                case 2: return (fProxy)3;
                default: return (fProxy)4;
            }
        }

        public fProxy Eval(in fProxyN x)
        {
            fProxy sum = (fProxy)0;
            for (int i = 0; i < x.N; i++)
            {
                fProxy d = x[i] - Target(i);
                sum += d * d;
            }
            return sum;
        }

        public void Gradient(in fProxyN x, ref fProxyN g)
        {
            for (int i = 0; i < x.N; i++)
                g[i] = (fProxy)2 * (x[i] - Target(i));
        }
    }
    ...
    public struct TestJob : IJob
    {
        ...
        public void BisectionParabola()
        {
            ...
            bool ok = Optimize.bisection(ref fn, (fProxy)3, (fProxy)10, out fProxy root, (fProxy)1E-6f);

            Assert.IsTrue(ok);
            AssertFinite(root);
            AssertClose(root, (fProxy)5, 1E-4f);
        }
        ...
        public void BisectionNotBracketed()
        {
            ...
            AssertClose(root, (fProxy)6, 1E-6f);
        }
        ...
        public void BisectionCos()
        {
            ...
            bool ok = Optimize.bisection(ref fn, (fProxy)0, (fProxy)3, out fProxy root, (fProxy)1E-6f);

            Assert.IsTrue(ok);
            AssertFinite(root);
            AssertClose(root, (fProxy)(Unity.Mathematics.math.PI * 0.5), 1E-4f);
        }
        ...
        public void NewtonSqrt2()
        {
            ...
            Assert.IsTrue(ok);
            AssertFinite(root);
            AssertClose(root, (fProxy)1.4142135623730951, 1E-4f);
        ...
        public void GoldenParabolaMin()
        {
            ...
            bool ok = Optimize.goldenSection(ref fn, (fProxy)(-5), (fProxy)5, out fProxy xMin, (fProxy)1E-6f);

            Assert.IsTrue(ok);
            AssertFinite(xMin);
            AssertClose(xMin, (fProxy)2, 1E-3f);
        }
        ...
        public void GoldenCosMin()
        {
            ...
            bool ok = Optimize.goldenSection(ref fn, (fProxy)2, (fProxy)4, out fProxy xMin, (fProxy)1E-6f);

            Assert.IsTrue(ok);
            AssertFinite(xMin);
            AssertClose(xMin, (fProxy)Unity.Mathematics.math.PI, 1E-3f);
        }
