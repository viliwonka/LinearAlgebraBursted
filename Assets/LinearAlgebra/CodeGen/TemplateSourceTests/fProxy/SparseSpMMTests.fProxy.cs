...
public class fProxySparseSpMMTests
{
    ...
    public struct SparseSpMMTestJob : IJob
    {
        ...
        static void CheckSpMMAgainstSpMV(ref Arena arena, in fProxyBSR A, int rows, uint seed)
        {
            var V = arena.fProxyRandomMat(rows, A.N_Cols, (fProxy)(-1f), (fProxy)1f, seed);
            var AV = arena.fProxyMat(rows, A.M_Rows);
            BSR.spMM(in A, in V, ref AV, rows);

            var rowIn = arena.fProxyVec(A.N_Cols);
            var rowOut = arena.fProxyVec(A.M_Rows);
        ...
        void CheckRectangular()
        {
            ...
            builder.AddBlock(0, 0, arena.fProxyRandomMat(BR, BC, (fProxy)(-1f), (fProxy)1f, 121001));
            builder.AddBlock(0, 2, arena.fProxyRandomMat(BR, BC, (fProxy)(-1f), (fProxy)1f, 121002));
            builder.AddBlock(1, 1, arena.fProxyRandomMat(BR, BC, (fProxy)(-1f), (fProxy)1f, 121003));
            builder.AddBlock(2, 0, arena.fProxyRandomMat(BR, BC, (fProxy)(-1f), (fProxy)1f, 121004));
        ...
        void CheckApplyBlockForwards()
        {
            ...
            int rows = 4;
            var V = arena.fProxyRandomMat(rows, A.N_Cols, (fProxy)(-1f), (fProxy)1f, 131900u);
            var AVdirect = arena.fProxyMat(rows, A.M_Rows);
            BSR.spMM(in A, in V, ref AVdirect, rows);

            var AVop = arena.fProxyMat(rows, A.M_Rows);
