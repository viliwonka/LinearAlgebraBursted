...
// BSR SpMM -- BSR.spMM / fProxyBSROperator.
// fProxyBSROperator.ApplyBlock streams the matrix once and applies to k row-vectors together
...
//     bsrMatMat*/bsrMatMatSym* kernel (UnsafeOP.Sparse.fProxy.cs) is documented to preserve its
...
public class fProxySparseSpMMTests
{
    ...
    public struct SparseSpMMTestJob : IJob
    {
        ...
        // runtime inside a job (BC1028) -- same idiom as fProxyKrylovFusedKernelTests.Sizes.
        ...
        // ---- helpers (mirrors fProxySparseUnrollTests' BuildRandomSquare/BuildRandomSymmetric
        // recipe -- same 4x4 block grid shape, distinct seed range) ----------------------------

        static fProxyBSR BuildRandomSquare(int b, uint seedBase)
        {
            var builder = new fProxyBSRBuilder(4, 4, b, b, Allocator.Temp);
            builder.AddBlock(0, 0, GenerateOP.fProxyRandomMat(b, b, (fProxy)(-1f), (fProxy)1f, seedBase + 1u));
            builder.AddBlock(0, 2, GenerateOP.fProxyRandomMat(b, b, (fProxy)(-1f), (fProxy)1f, seedBase + 2u));
            builder.AddBlock(1, 1, GenerateOP.fProxyRandomMat(b, b, (fProxy)(-1f), (fProxy)1f, seedBase + 3u));
            builder.AddBlock(1, 3, GenerateOP.fProxyRandomMat(b, b, (fProxy)(-1f), (fProxy)1f, seedBase + 4u));
            builder.AddBlock(2, 0, GenerateOP.fProxyRandomMat(b, b, (fProxy)(-1f), (fProxy)1f, seedBase + 5u));
            builder.AddBlock(3, 3, GenerateOP.fProxyRandomMat(b, b, (fProxy)(-1f), (fProxy)1f, seedBase + 6u));
            return builder.ToBSR(Allocator.Temp);
        }

        static fProxyMxN SymDiagBlock(int b, uint seed)
        {
            var M = GenerateOP.fProxyRandomMat(b, b, (fProxy)(-1f), (fProxy)1f, seed);
        ...
        static fProxyBSR BuildRandomSymmetric(int b, uint seedBase)
        {
            var builder = new fProxyBSRBuilder(4, 4, b, b, Allocator.Temp);
            ...
            builder.AddBlock(1, 0, GenerateOP.fProxyRandomMat(b, b, (fProxy)(-1f), (fProxy)1f, seedBase + 5u));
            builder.AddBlock(3, 1, GenerateOP.fProxyRandomMat(b, b, (fProxy)(-1f), (fProxy)1f, seedBase + 6u));
            builder.AddBlock(3, 0, GenerateOP.fProxyRandomMat(b, b, (fProxy)(-1f), (fProxy)1f, seedBase + 7u));
            return builder.ToBSRSymmetric(Allocator.Temp);
        }

        // AV[r,:] must equal BSR.spMV(A, V[r,:]) EXACTLY for every r in [0, rows).
        static void CheckSpMMAgainstSpMV(in fProxyBSR A, int rows, uint seed)
        {
            var V = GenerateOP.fProxyRandomMat(rows, A.N_Cols, (fProxy)(-1f), (fProxy)1f, seed);
            var AV = new fProxyMxN(rows, A.M_Rows, Allocator.Temp);
            BSR.spMM(in A, in V, ref AV, rows);

            var rowIn = new fProxyN(A.N_Cols, Allocator.Temp);
            var rowOut = new fProxyN(A.M_Rows, Allocator.Temp);
        ...
        // fProxySparseUnrollTests.CheckRectangularSpMV's boundary case.
        void CheckRectangular()
        {
            const int BR = 2, BC = 3;
            var builder = new fProxyBSRBuilder(3, 3, BR, BC, Allocator.Temp);
            builder.AddBlock(0, 0, GenerateOP.fProxyRandomMat(BR, BC, (fProxy)(-1f), (fProxy)1f, 121001));
            builder.AddBlock(0, 2, GenerateOP.fProxyRandomMat(BR, BC, (fProxy)(-1f), (fProxy)1f, 121002));
            builder.AddBlock(1, 1, GenerateOP.fProxyRandomMat(BR, BC, (fProxy)(-1f), (fProxy)1f, 121003));
            builder.AddBlock(2, 0, GenerateOP.fProxyRandomMat(BR, BC, (fProxy)(-1f), (fProxy)1f, 121004));
            var A = builder.ToBSR(Allocator.Temp);
            for (int t = 0; t < Ks.Length; t++)
                CheckSpMMAgainstSpMV(in A, Ks[t], (uint)(121100 + t * 10));
        }

        // fProxyBSROperator.ApplyBlock is a one-line forward to BSR.spMM -- proves the wiring, not
        ...
        void CheckApplyBlockForwards()
        {
            ...
            int rows = 4;
            var V = GenerateOP.fProxyRandomMat(rows, A.N_Cols, (fProxy)(-1f), (fProxy)1f, 131900u);
            var AVdirect = new fProxyMxN(rows, A.M_Rows, Allocator.Temp);
            BSR.spMM(in A, in V, ref AVdirect, rows);

            var AVop = new fProxyMxN(rows, A.M_Rows, Allocator.Temp);
    ...
    // The "before" oracle: the exact scalar per-row loop fProxyBSROperator.ApplyBlock used before
    // BSR.spMM replaced it -- kept ONLY as an independent "pre-change" reference for the A/B test
    // below, not for production use.
    readonly struct OldStyleBSROperatorFProxy : IfProxyLinearOperator
    {
        public readonly fProxyBSR A;
        public OldStyleBSROperatorFProxy(in fProxyBSR a) { A = a; }
        public int Rows => A.M_Rows;
        public int Cols => A.N_Cols;
        public void Apply(in fProxyN x, ref fProxyN y) => BSR.spMV(in A, in x, ref y);
        public void ApplyT(in fProxyN x, ref fProxyN y) => BSR.spMVT(in A, in x, ref y);
    ...
    // Same 6x6 grid instance as fProxyLOBPCGSmokeTests.Laplacian2DGuardedMatchesAnalyticSmallest
    ...
    // fProxyLOBPCGSmokeTests' style -- this is a one-shot comparison, not a hot kernel.
    [Test]
    public void LOBPCGResultsUnchangedBySpMMKernelChange()
    {
        int g = 6;
        int n = g * g;
        var A = fProxyGallery.fProxyLaplacian2D(g, g);
        int k = 3;

        var wsOld = new fProxyLOBPCGCache(n, k, Allocator.Temp);
        var infoOld = Eigen.lobpcg(new OldStyleBSROperatorFProxy(in A), ref wsOld, k, Consts.fProxySqrtEps, 1000);

        var wsNew = new fProxyLOBPCGCache(n, k, Allocator.Temp);
        var infoNew = Eigen.lobpcg(in A, ref wsNew, k, Consts.fProxySqrtEps, 1000);
