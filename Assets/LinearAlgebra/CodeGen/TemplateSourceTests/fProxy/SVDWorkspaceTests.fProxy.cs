...
public class fProxySVDWorkspaceTests
{
    ...
    public struct WorkspaceEquivJob : IJob
    {
        ...
        void PinvEquiv(int m, int n)
        {
            ...
            var ws = arena.fProxySvdWorkspace(m, n);
        ...
        void WorkspaceReuse()
        {
            ...
            var ws = arena.fProxySvdWorkspace(m, n);   // allocated ONCE, reused below
    ...
    // Arena.fProxySvdWorkspace(m, n) must size S (k), M (k x k), and At (n x m only when wide).
    [Test]
