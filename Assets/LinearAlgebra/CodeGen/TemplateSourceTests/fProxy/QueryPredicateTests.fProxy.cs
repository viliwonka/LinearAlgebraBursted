...
public class fProxyQueryPredicateTests
{
    ...
    public struct TestJob : IJob
    {
        ...
        void MaskedNearest()
        {
            ...
            Query.nearestRow(in A, in q, Metric.SqEuclidean, out int ui, out fProxy us);
            AssertEqI(ui, 0); AssertClose(us, (fProxy)1, fEps());

            var pred = new RowSumAbove { t = (fProxy)1.5 };
            Query.nearestRowWhere(in A, in q, Metric.SqEuclidean, ref pred, out int mi, out fProxy ms);
        ...
        void AllPassEquivalence()
        {
            ...
            Query.nearestRowWhere(in A, in q, Metric.SqEuclidean, ref atr, out int wi, out fProxy ws);
            AssertEqI(wi, ni); AssertClose(ws, ns, (fProxy)0);

            Query.nearestRow(in A, in q, Metric.Dot, out int nid, out fProxy nsd);
            Query.nearestRowWhere(in A, in q, Metric.Dot, ref atr, out int wid, out fProxy wsd);
            ...
            Query.nearestRowWhere(in A, in q, Metric.SqEuclidean, ref afr, out int fi1, out fProxy fs1);
            AssertEqI(fi1, -1); AssertClose(fs1, fProxy.MaxValue, (fProxy)0);

            Query.nearestRowWhere(in A, in q, Metric.Manhattan, ref afr, out int fi2, out fProxy fs2);
            AssertEqI(fi2, -1); AssertClose(fs2, fProxy.MaxValue, (fProxy)0);

            Query.nearestRowWhere(in A, in q, Metric.Cosine, ref afr, out int fi3, out fProxy fs3);
            AssertEqI(fi3, -1); AssertClose(fs3, fProxy.MinValue, (fProxy)0);

            Query.nearestRowWhere(in A, in q, Metric.Dot, ref afr, out int fi4, out fProxy fs4);
        ...
        void GroupDScore()
        {
            ...
            Query.argMaxRowBy(in H, ref rs, out int mxi, out fProxy mxs);
            AssertEqI(mxi, 3); AssertClose(mxs, (fProxy)25, fEps());
            AssertEqI(mxi, Query.argMaxRowNorm(in H, Norm.L2));

            // argMinRowBy -> r0 (1).
            Query.argMinRowBy(in H, ref rs, out int mni, out fProxy mns);
            ...
            Query.argMaxColBy(in H, ref cs, out int cmi, out fProxy cms);
        ...
        void Symmetry()
        {
            ...
            Query.nearestColumnWhere(in A, in q, Metric.SqEuclidean, ref cpred, out int cni, out fProxy cns);
            Query.nearestRowWhere(in At, in q, Metric.SqEuclidean, ref rpred, out int rni, out fProxy rns);
            ...
            Query.argMaxColBy(in A, ref cscore, out int aci, out fProxy _);
    ...
    public void EmptyMatrixThrows()
    {
        ...
        Assert.Throws<InvalidOperationException>(() =>
            Query.argMaxRowBy(in A0, ref rs, out int _, out fProxy _));
