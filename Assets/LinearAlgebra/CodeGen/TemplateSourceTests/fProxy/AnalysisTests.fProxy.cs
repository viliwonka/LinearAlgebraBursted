    ...
    public struct AnalysisTestJob : IJob
    {
        ...
        void Determinant()
        {
            ...
            // matrix-in path agrees with the zero-alloc from-factor overload on the same A
            {
                ...
                fProxy viaFactor = Analysis.determinant(in lu, in P);
                P.Dispose();

                Assert.IsTrue(math.abs(viaMatrix - viaFactor) < (fProxy)1E-3f * (math.abs(viaFactor) + (fProxy)1));
            }
        ...
        void LogDeterminant()
        {
            ...
            // diagonal [2,-3,0.5,4] -> det = -12: sign = -1, sign*exp(logAbs) recovers det
            {
                ...
                fProxy recovered = sign * math.exp(logAbs);
                Assert.IsTrue(math.abs(recovered - (fProxy)(-12f)) < (fProxy)1E-3f);
            }
            ...
            // singular (row 1 == 2*row 0) -> sign = 0, log|det| = -infinity
            {
                ...
                A[2, 0] = 4f; A[2, 1] = 5f; A[2, 2] = 7f;
                fProxy logAbs = Analysis.logDeterminant(in A, out fProxy sign);
                Assert.IsTrue(math.abs(sign) < (fProxy)1E-6f);
                Assert.IsTrue(math.isinf(logAbs) && logAbs < (fProxy)0);
            }
            ...
            // 10*I at dim 400 -> det = 10^400, past both float (~1e38) and double (~1e308) range.
            {
                ...
                fProxy expected = (fProxy)dim * math.log((fProxy)10f);
