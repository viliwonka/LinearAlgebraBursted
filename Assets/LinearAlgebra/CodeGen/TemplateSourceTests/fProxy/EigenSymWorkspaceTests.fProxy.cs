...
// Workspace-overload tests for Eigen.eigenvaluesSymmetric and its workspace fProxyEigenSymWorkspace
// (Arena.fProxyEigenSymWorkspace(n)). eigenvaluesSymmetric DESTROYS its input matrix, so every call
...
public class fProxyEigenSymWorkspaceTests
{
    ...
    public struct WorkspaceJob : IJob
    {
        ...
        {
            var A = arena.fProxyRandomMatrix(n, n, (fProxy)(-5f), (fProxy)5f, seed);
            for (int i = 0; i < n; i++)
                for (int j = i + 1; j < n; j++)
                {
        ...
        void Equiv(int n, uint seed)
        {
            ...
            var ws = arena.fProxyEigenSymWorkspace(n);
        ...
        void WorkspaceReuse()
        {
            ...
            var ws = arena.fProxyEigenSymWorkspace(n);   // allocated ONCE
    ...
    public void EigenSym_BadWorkspace_Throws()
    {
        ...
        {
            int n = 5;
            var A = arena.fProxyIdentityMatrix(n);   // symmetric -> passes the symmetry guard
            var eig = arena.fProxyVec(n);
            var ws = arena.fProxyEigenSymWorkspace(n + 1);   // wrong n
    ...
    // Arena.fProxyEigenSymWorkspace(n): three length-n vectors.
    [Test]
    public void EigenSymWorkspace_Factory_SizesCorrectly()
    {
        ...
        try
        {
            var ws = arena.fProxyEigenSymWorkspace(7);
