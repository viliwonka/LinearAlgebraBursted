...
public class fProxyArenaWiringTests
{
    ...
    // InvalidOperationException instead of silently returning a dead/garbage buffer. fProxyN is
    // option (b) (Alive-only: the 32B struct has no spare padding for a generation stamp); fProxyMxN is
    ...
    // (b) VECTOR read-after-Dispose. fProxyN.Dispose() nulls the DISPOSER's OWN _rec (reading the
    ...
    public void Vector_ReadAfterDispose_Throws()
    {
        ...
        try
        {
            var v = arena.fProxyVec(4, (fProxy)0);
    ...
    public void Vector_ReadAfterClearTemp_Throws()
    {
        ...
        try
        {
            var seed = arena.fProxyVec(4, (fProxy)0);
    ...
    // fProxyMxN's Alive+generation guard. Same alias-for-dispose requirement (mutable struct nulls its
    // own _rec on Dispose).
    [Test]
    ...
    public void Matrix_ReadAfterClearTemp_Throws()
    {
        ...
        try
        {
            var seed = arena.fProxyMat(3, 3, (fProxy)0);
    ...
    // read true again for the new occupant and wave the stale read through, but fProxyMxN's _gen was
    ...
    public void Matrix_StaleGenerationAfterSlotRecycle_OldThrows_NewWorks()
    {
        ...
        try
        {
            var v1 = arena.fProxyMat(2, 3, (fProxy)0);
            ...
            var v2 = arena.fProxyMat(2, 3, (fProxy)0);  // recycles v1's EXACT slot; generation bumped
            Assert.AreEqual(1, arena.AllocationsCount);
