...
public class fProxySparseStructuralTests
{
    ...
    public struct SparseStructuralTestJob : IJob
    {
        ...
        void TraceAndDiagonal()
        {
            ...
            var b = new fProxyMxN(BR, BC, Allocator.Temp);
            ...
            var d = new fProxyN(A.M_Rows, Allocator.Temp);
        ...
        void DenseDiagonal()
        {
            var A = new fProxyMxN(4, 6, Allocator.Temp);
            for (int i = 0; i < 4; i++)
                for (int j = 0; j < 6; j++)
                    A[i, j] = (fProxy)(i * 10 + j);

            var d = new fProxyN(4, Allocator.Temp);   // min(4, 6)
            Analysis.diagonal(in A, ref d);
            for (int i = 0; i < 4; i++)
        ...
        void TransposeRectangular()
        {
            ...
            const int BR = 2, BC = 3;
            var builder = new fProxyBSRBuilder(2, 3, BR, BC, Allocator.Temp);
            var b = new fProxyMxN(BR, BC, Allocator.Temp);
        ...
        void TransposeIntoReuse()
        {
            ...
            var A = builder.ToBSR(Allocator.Temp);
            var At = new fProxyBSR(A.BlockCols, A.BlockRows, A.BC, A.BR, A.Nnzb, Allocator.Temp, true);
        ...
        {
            const int BR = 2;
            var builder = new fProxyBSRBuilder(2, 2, BR, BR, Allocator.Temp);
    ...
    public void RefillTopologyChangeThrows()
    {
        const int BR = 2, BC = 2;
        var builder = new fProxyBSRBuilder(2, 2, BR, BC, Allocator.Temp);
        var block = GenerateOP.fProxyMat(BR, BC, (fProxy)1);
    ...
    public void TransposeWrongDestinationThrows()
    {
        ...
        var wrongGrid = new fProxyBSR(2, 3, BR, BC, 1, Allocator.Temp, true);
        Assert.Throws<ArgumentException>(() => BSR.transpose(in A, ref wrongGrid));

        // Transposed grid but wrong nnzb -> throw.
        var wrongNnzb = new fProxyBSR(3, 2, BC, BR, 2, Allocator.Temp, true);
