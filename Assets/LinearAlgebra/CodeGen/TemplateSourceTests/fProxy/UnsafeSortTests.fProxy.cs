...
// key/value arrays -- LinearAlgebra.Internal.UnsafeOP, UnsafeOP.fProxy.cs). New Blas/UnsafeOP kernels
// get DIRECT tests against a plain scalar reference implementation, not just indirect coverage
// through callers. This kernel powers LP.ladBR's ratio-test candidate ordering
// (LP.BarrodaleRoberts.fProxy.cs), so before this file the kernel had ZERO direct coverage.
//
...
// raw pointers inside Burst jobs -- see QRCPDowndateTests.fProxy.cs / LUTests.fProxy.cs calling
...
public class fProxyUnsafeSortTests
{
    ...
    public unsafe struct TestJob : IJob
    {
        ...
        void RandomVariousN()
        {
            ...
            for (int s = 0; s < 5; s++)
            {
                ...
                var key = new fProxyN(n, Allocator.Temp);
                var val = new NativeArray<int>(n, Allocator.Temp);
                for (int i = 0; i < n; i++) { key[i] = rng.NextFProxy(-(fProxy)1000, (fProxy)1000); val[i] = i; }
        ...
        void DuplicateKeys()
        {
            ...
            var key = new fProxyN(n, Allocator.Temp);
            var val = new NativeArray<int>(n, Allocator.Temp);
            for (int i = 0; i < n; i++) { key[i] = (fProxy)rng.NextInt(0, 10); val[i] = i; }
        ...
        void AlreadySorted()
        {
            ...
            var key = new fProxyN(n, Allocator.Temp);
            var val = new NativeArray<int>(n, Allocator.Temp);
            for (int i = 0; i < n; i++) { key[i] = (fProxy)i; val[i] = i; }
        ...
        void ReverseSorted()
        {
            ...
            var key = new fProxyN(n, Allocator.Temp);
            var val = new NativeArray<int>(n, Allocator.Temp);
            for (int i = 0; i < n; i++) { key[i] = (fProxy)(n - i); val[i] = i; }
        ...
        {
            if (Fail[0] == (fProxy)0) { Fail[0] = (fProxy)1; Fail[1] = (fProxy)code; Fail[2] = (fProxy)a; Fail[3] = (fProxy)b; }
        }
    }
