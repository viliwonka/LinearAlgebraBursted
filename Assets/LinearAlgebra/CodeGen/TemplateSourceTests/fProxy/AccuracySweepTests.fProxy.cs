...
// kernels (the reconstruction products are recomputed in double from the fProxy factors), so the
// yardstick is ~1e-16 clean and never as noisy as the float thing being measured.
// ============================================================================================
public class fProxyAccuracySweepTests
{
    ...
    public struct AccuracyJob : IJob
    {
        ...
        double Eps() => (double)Consts.fProxyEpsilon;
        ...
        void RunLQ(int m, int n, uint seed, int mode)
        {
            ...
            if (mode == 1)
            {
                ...
                for (int i = 0; i < m; i++)
                {
                    fProxy s = (fProxy)math.pow(10.0, -3.0 * i / (m - 1));
                    for (int j = 0; j < n; j++)
                        A[i, j] *= s;
                }
            }
        ...
        static double FrobA(in fProxyMxN A)
        {
            double s = 0;
        ...
        static double ReconResidual2(in fProxyMxN A, in fProxyMxN Q, in fProxyMxN R)
        {
        ...
        static double ReconResidualLQ(in fProxyMxN A, in fProxyMxN L, in fProxyMxN Q)
        {
            int m = A.M_Rows, n = A.N_Cols;
            double num = 0, den = 0;
            for (int i = 0; i < m; i++)
        ...
        static double ReconResidualLLt(in fProxyMxN A, in fProxyMxN L)
        {
            int n = A.M_Rows;
            double num = 0, den = 0;
            for (int i = 0; i < n; i++)
        ...
        double ReconResidualPALU(in fProxyMxN A, in fProxyMxN L, in fProxyMxN U, in Pivot P)
        {
            int n = A.M_Rows;
            var PA = A.Copy();
        ...
        static double OrthoErrorCols(in fProxyMxN Q)
        {
        ...
        static double OrthoErrorRows(in fProxyMxN Q)
        {
            int m = Q.M_Rows, n = Q.N_Cols;
            double s = 0;
            for (int i = 0; i < m; i++)
        ...
        static double DiagCond(in fProxyMxN T, bool squared)
        {
        ...
        // Fail layout: [0]=flag, [1]=value, [2]=limit, [3]=excess. value/limit stored as fProxy for
        ...
        void AssertLE(double value, double limit)
        {
            if (!(value <= limit) && Fail[0] == (fProxy)0)
            {
                ...
                Fail[1] = (fProxy)value;
                Fail[2] = (fProxy)limit;
                Fail[3] = (fProxy)(value - limit);
            }
            Assert.IsTrue(value <= limit);
        }
    }

    // true only when fProxy expands to double.
    private static bool IsDouble() => (double)Consts.fProxyEpsilon < 1e-10;
