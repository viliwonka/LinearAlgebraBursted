...
public class fProxySelectRefTests
{
    ...
    public struct SelectRefTestJob : IJob
    {
        ...
        void VecCond()
        {
            ...
            var a = GenerateOP.fProxyRandomVec(N, -1f, 1f, 11111, allocator: Allocator.Temp);
            var b = GenerateOP.fProxyRandomVec(N, -1f, 1f, 22222, allocator: Allocator.Temp);
            ...
            var diff = new fProxyN(in R, Allocator.Temp); diff.subInPlace(D);
        ...
        void MatCond()
        {
            ...
            var a = GenerateOP.fProxyRandomMat(M, N, -1f, 1f, 44444, allocator: Allocator.Temp);
            var b = GenerateOP.fProxyRandomMat(M, N, -1f, 1f, 55555, allocator: Allocator.Temp);
            var c = GenerateOP.boolRandomMat(M, N, 66666, allocator: Allocator.Temp);

            var R = Select.select(a, b, c);

            var D = new fProxyMxN(M, N, Allocator.Temp);
            Select.select(in a, in b, in c, ref D);

            var diff = new fProxyMxN(in R, Allocator.Temp); diff.subInPlace(D);
            Assert.IsTrue(Analysis.isZero(diff, Tol()));
        }
        ...
        void VecScalarTrue()
        {
            ...
            var a = GenerateOP.fProxyRandomVec(N, -1f, 1f, 77777, allocator: Allocator.Temp);
            var b = GenerateOP.fProxyRandomVec(N, -1f, 1f, 88888, allocator: Allocator.Temp);

            var D = new fProxyN(N, Allocator.Temp);
            Select.select(in a, in b, true, ref D);

            var diff = new fProxyN(in b, Allocator.Temp); diff.subInPlace(D);
            Assert.IsTrue(Analysis.isZero(diff, Tol()));
        }
        ...
        void VecScalarFalse()
        {
            ...
            var a = GenerateOP.fProxyRandomVec(N, -1f, 1f, 99999, allocator: Allocator.Temp);
            var b = GenerateOP.fProxyRandomVec(N, -1f, 1f, 10101, allocator: Allocator.Temp);

            var D = new fProxyN(N, Allocator.Temp);
            Select.select(in a, in b, false, ref D);

            var diff = new fProxyN(in a, Allocator.Temp); diff.subInPlace(D);
            Assert.IsTrue(Analysis.isZero(diff, Tol()));
        }
        ...
        void MatScalarTrue()
        {
            ...
            var a = GenerateOP.fProxyRandomMat(M, N, -1f, 1f, 20202, allocator: Allocator.Temp);
            var b = GenerateOP.fProxyRandomMat(M, N, -1f, 1f, 30303, allocator: Allocator.Temp);

            var D = new fProxyMxN(M, N, Allocator.Temp);
            Select.select(in a, in b, true, ref D);

            var diff = new fProxyMxN(in b, Allocator.Temp); diff.subInPlace(D);
            Assert.IsTrue(Analysis.isZero(diff, Tol()));
        }
        ...
        void MatScalarFalse()
        {
            ...
            var a = GenerateOP.fProxyRandomMat(M, N, -1f, 1f, 40404, allocator: Allocator.Temp);
            var b = GenerateOP.fProxyRandomMat(M, N, -1f, 1f, 50505, allocator: Allocator.Temp);

            var D = new fProxyMxN(M, N, Allocator.Temp);
            Select.select(in a, in b, false, ref D);

            var diff = new fProxyMxN(in a, Allocator.Temp); diff.subInPlace(D);
            Assert.IsTrue(Analysis.isZero(diff, Tol()));
        }
        ...
        void VecAliasDest()
        {
            ...
            var a = GenerateOP.fProxyRandomVec(N, -1f, 1f, 60606, allocator: Allocator.Temp);
            var b = GenerateOP.fProxyRandomVec(N, -1f, 1f, 70707, allocator: Allocator.Temp);
            ...
            var diff = new fProxyN(in R, Allocator.Temp); diff.subInPlace(a);
            Assert.IsTrue(Analysis.isZero(diff, Tol()));
        }
    }
