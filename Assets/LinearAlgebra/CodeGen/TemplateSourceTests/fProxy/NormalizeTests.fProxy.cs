...
public class fProxyNormalizeTests
{
    ...
    public struct TestJob : IJob
    {
        ...
        static fProxy ZTol() => (fProxy)16 * (fProxy)Consts.fProxySqrtEps;
        ...
        void MinMaxColumns()
        {
            ...
            fProxyStatsOP.normalizeColumns(ref A, NormalizeMode.MinMax);

            // col0: min entry -> 0, max entry -> 1, interior -> exact fraction 0.5
            AssertClose(A[0, 0], (fProxy)0f, MinMaxTol);
            AssertClose(A[1, 0], (fProxy)0.5f, MinMaxTol);
            AssertClose(A[2, 0], (fProxy)1f, MinMaxTol);

            // col1 (negatives): min entry -> 0, max entry -> 1, interior -> 0.5
            AssertClose(A[0, 1], (fProxy)0f, MinMaxTol);
            AssertClose(A[1, 1], (fProxy)1f, MinMaxTol);
            AssertClose(A[2, 1], (fProxy)0.5f, MinMaxTol);
        ...
        void MinMaxRows()
        {
            ...
            fProxyStatsOP.normalizeRows(ref A, NormalizeMode.MinMax);

            AssertClose(A[0, 0], (fProxy)0f, MinMaxTol);
            AssertClose(A[0, 1], (fProxy)0.25f, MinMaxTol);
            AssertClose(A[0, 2], (fProxy)1f, MinMaxTol);

            AssertClose(A[1, 0], (fProxy)0f, MinMaxTol);
            AssertClose(A[1, 1], (fProxy)0.25f, MinMaxTol);
            AssertClose(A[1, 2], (fProxy)1f, MinMaxTol);
        ...
        void ZScoreColumns()
        {
            ...
            fProxyStatsOP.normalizeColumns(ref A, NormalizeMode.ZScore);

            // Specific entry oracle: [0,0] = (1-3)/sqrt(8/3); [2,1] = (60-30)/sqrt(1400/3).
            fProxy exp00 = ((fProxy)1f - (fProxy)3f) / math.sqrt((fProxy)(8f / 3f));
            fProxy exp21 = ((fProxy)60f - (fProxy)30f) / math.sqrt((fProxy)(1400f / 3f));
            AssertClose(A[0, 0], exp00, ZTol());
            AssertClose(A[2, 1], exp21, ZTol());

            // Property: each normalized column has mean ~ 0 and population std ~ 1.
            var mean = fProxyStatsOP.colMean(in A);
            var std = fProxyStatsOP.colStdDev(in A);
            for (int c = 0; c < A.N_Cols; c++)
            {
                AssertClose(mean[c], (fProxy)0f, ZTol());
                AssertClose(std[c], (fProxy)1f, ZTol());
            }
        ...
        void ZScoreRows()
        {
            ...
            fProxyStatsOP.normalizeRows(ref A, NormalizeMode.ZScore);

            fProxy exp00 = ((fProxy)1f - (fProxy)3f) / math.sqrt((fProxy)(8f / 3f));
            fProxy exp12 = ((fProxy)60f - (fProxy)30f) / math.sqrt((fProxy)(1400f / 3f));
            AssertClose(A[0, 0], exp00, ZTol());
            AssertClose(A[1, 2], exp12, ZTol());

            var mean = fProxyStatsOP.rowMean(in A);
            var std = fProxyStatsOP.rowStdDev(in A);
            for (int r = 0; r < A.M_Rows; r++)
            {
                AssertClose(mean[r], (fProxy)0f, ZTol());
                AssertClose(std[r], (fProxy)1f, ZTol());
            }
        ...
        void CheckConstantColumn(NormalizeMode mode)
        {
            ...
            fProxyStatsOP.normalizeColumns(ref A, mode);
            ...
            AssertGreater(maxAbs, (fProxy)0f);
        ...
        void CheckConstantRow(NormalizeMode mode)
        {
            ...
            fProxyStatsOP.normalizeRows(ref A, mode);
        ...
        void SingleRowMatrix()
        {
            ...
            for (int m = 0; m < 2; m++)
            {
                ...
                fProxyStatsOP.normalizeColumns(ref A, m == 0 ? NormalizeMode.MinMax : NormalizeMode.ZScore);
            ...
            var B = arena2.fProxyMat(1, 3);
            B[0, 0] = 4f;  B[0, 1] = 9f;  B[0, 2] = 2f;

            fProxyStatsOP.normalizeRows(ref B, NormalizeMode.MinMax);
            AssertClose(B[0, 0], (fProxy)(2f / 7f), MinMaxTol);
            AssertClose(B[0, 1], (fProxy)1f, MinMaxTol);
            AssertClose(B[0, 2], (fProxy)0f, MinMaxTol);
            arena2.Dispose();
        }
        ...
        void SingleColumnMatrix()
        {
            for (int m = 0; m < 2; m++)
            {
                ...
                fProxyStatsOP.normalizeRows(ref A, m == 0 ? NormalizeMode.MinMax : NormalizeMode.ZScore);
            ...
            var arena2 = new Arena(Allocator.Persistent);
            var B = arena2.fProxyMat(3, 1);
            B[0, 0] = 4f;  B[1, 0] = 9f;  B[2, 0] = 2f;

            fProxyStatsOP.normalizeColumns(ref B, NormalizeMode.MinMax);
            AssertClose(B[0, 0], (fProxy)(2f / 7f), MinMaxTol);
            AssertClose(B[1, 0], (fProxy)1f, MinMaxTol);
            AssertClose(B[2, 0], (fProxy)0f, MinMaxTol);
            arena2.Dispose();
        }
        ...
        void InPlaceMutation()
        {
            ...
            AssertClose(alias[1, 0], (fProxy)0.5f, MinMaxTol);
            AssertClose(alias[2, 0], (fProxy)1f, MinMaxTol);
            AssertClose(alias[0, 1], (fProxy)0f, MinMaxTol);
        ...
        void AssertExactZero(fProxy a)
        {
            if (!(a == (fProxy)0f) && Fail[0] == (fProxy)0)
            {
                Fail[0] = (fProxy)1;
                Fail[1] = a;
                Fail[2] = (fProxy)0f;
            ...
            Assert.IsTrue(a == (fProxy)0f);
        }

        void AssertNotNaN(fProxy a)
        {
            bool nan = math.isnan(a);
            if (nan && Fail[0] == (fProxy)0)
            {
                Fail[0] = (fProxy)1;
                Fail[1] = a;
                Fail[2] = (fProxy)(-999f);
        ...
        void AssertGreater(fProxy a, fProxy limit)
        {
            if (!(a > limit) && Fail[0] == (fProxy)0)
            {
                Fail[0] = (fProxy)1;
                Fail[1] = a;
                Fail[2] = limit;
    ...
    public void EmptyMatrixNormalizeColumnsThrows()
    {
        ...
        var A0 = arena.fProxyMat(0, 3); // 0 rows
        var A1 = arena.fProxyMat(3, 0); // 0 cols

        Assert.Throws<InvalidOperationException>(() => fProxyStatsOP.normalizeColumns(ref A0, NormalizeMode.MinMax));
        Assert.Throws<InvalidOperationException>(() => fProxyStatsOP.normalizeColumns(ref A0, NormalizeMode.ZScore));
        Assert.Throws<InvalidOperationException>(() => fProxyStatsOP.normalizeColumns(ref A1, NormalizeMode.MinMax));
        Assert.Throws<InvalidOperationException>(() => fProxyStatsOP.normalizeColumns(ref A1, NormalizeMode.ZScore));
    ...
    public void EmptyMatrixNormalizeRowsThrows()
    {
        ...
        Assert.Throws<InvalidOperationException>(() => fProxyStatsOP.normalizeRows(ref A0, NormalizeMode.MinMax));
        Assert.Throws<InvalidOperationException>(() => fProxyStatsOP.normalizeRows(ref A0, NormalizeMode.ZScore));
        Assert.Throws<InvalidOperationException>(() => fProxyStatsOP.normalizeRows(ref A1, NormalizeMode.MinMax));
        Assert.Throws<InvalidOperationException>(() => fProxyStatsOP.normalizeRows(ref A1, NormalizeMode.ZScore));
