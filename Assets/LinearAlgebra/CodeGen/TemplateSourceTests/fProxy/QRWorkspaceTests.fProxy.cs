...
public class fProxyQRWorkspaceTests
{
    ...
    public struct WorkspaceEquivJob : IJob
    {
        ...
        {
            var A = GenerateOP.fProxyRandomMat(M, N, -1f, 1f, 73101, allocator: Allocator.Temp);
            ...
            var QDiff = new fProxyMxN(in Qa, Allocator.Temp);
            QDiff.subInPlace(Qb);
            Assert.IsTrue(Analysis.isZero(QDiff, Tol()));

            var RDiff = new fProxyMxN(in Ra, Allocator.Temp);
            RDiff.subInPlace(Rb);
        ...
        {
            int dim = 16;
            var A0 = GenerateOP.fProxyRandomMat(dim, dim, -1f, 1f, 51237, allocator: Allocator.Temp);
            // make well-conditioned
            for (int d = 0; d < dim; d++)
                A0[d, d] += 5f;

            var xOrig = GenerateOP.fProxyRandomVec(dim, -3f, 3f, 99001, allocator: Allocator.Temp);

            // allocating reference (solveInPlace destroys A and b, so use fresh copies)
            var Aa = A0.Copy();
            var ba = Blas.dot(A0, xOrig);
            var xa = new fProxyN(dim, Allocator.Temp);
            QR.solveInPlace(ref Aa, ref ba, ref xa);

            // caller-scratch form
            var Ab = A0.Copy();
            var bb = Blas.dot(A0, xOrig);
            var xb = new fProxyN(dim, Allocator.Temp);
            var u = new fProxyN(dim, Allocator.Temp);
            QR.solveInPlace(ref Ab, ref bb, ref xb, ref u);

            var xDiff = new fProxyN(in xa, Allocator.Temp);
            xDiff.subInPlace(xb);
        ...
        {
            var A = GenerateOP.fProxyRandomMat(M, N, -1f, 1f, 33417, allocator: Allocator.Temp);
            for (int d = 0; d < N; d++)   // well-conditioned columns
                A[d, d] += 5f;

            var xOrig = GenerateOP.fProxyRandomVec(N, -3f, 3f, 60221, allocator: Allocator.Temp);
            ...
            var xVsOrig = new fProxyN(in x, Allocator.Temp);
            ...
            var xcVsX = new fProxyN(in xc, Allocator.Temp);
            xcVsX.subInPlace(x);
    ...
    public void QrDecomp_BadScratchSize_Throws()
    {
        ...
        var badU = new fProxyN(3, Allocator.Temp);   // must be length 6 (Q.M_Rows)
    ...
    public void QrDirectSolve_BadScratchSize_Throws()
    {
        ...
        var badU = new fProxyN(4, Allocator.Temp);   // must be length 6 (A.M_Rows)
    ...
    public void SolveQR_BadDestSize_Throws()
    {
        ...
        var badX = new fProxyN(3, Allocator.Temp);   // must be length 4 (Q.N_Cols)
