    ...
    public struct TestJob : IJob
    {
        ...
        void Radix4VsOracleOneSize(int N, uint seedRe, uint seedIm)
        {
            ...
            if (N <= 256)
            {
                ...
                fProxyFFT.fft(ref reRef, ref imRef);

                var reW = re0.Copy(); var imW = im0.Copy();
                fProxyFFT.fft(ref reW, ref imW, in ws);
        ...
        void FftVsDftOneSize(int N, uint seedRe, uint seedIm)
        {
            ...
            var dRe = arena.fProxyVec(N);
            var dIm = arena.fProxyVec(N);
            fProxyFFT.dft(in re0, in im0, ref dRe, ref dIm);
            ...
            fProxyFFT.fft(ref wRe, ref wIm, in ws);
        ...
        void ParsevalRfftOneSize(int N, uint seed)
        {
            ...
            var rRe = arena.fProxyVec(halfSpec);
            var rIm = arena.fProxyVec(halfSpec);
            fProxyFFT.rfft(in real, ref rRe, ref rIm, in ws);
        ...
        void FftLinearityOneSize(int N, uint sx, uint sy)
        {
            ...
            fProxyFFT.dft(in xr, in xi, ref Xr, ref Xi);
            fProxyFFT.dft(in yr, in yi, ref Yr, ref Yi);

            // LHS via three transforms.
            var Zno_r = zr.Copy(); var Zno_i = zi.Copy(); fProxyFFT.fft(ref Zno_r, ref Zno_i);
            var Zws_r = zr.Copy(); var Zws_i = zi.Copy(); fProxyFFT.fft(ref Zws_r, ref Zws_i, in ws);
            var Zdf_r = arena.fProxyVec(N); var Zdf_i = arena.fProxyVec(N);
            fProxyFFT.dft(in zr, in zi, ref Zdf_r, ref Zdf_i);
        ...
        void KnownAnalytics()
        {
            ...
            // --- impulse δ[0] -> flat spectrum (all ones) ---
            {
                ...
                fProxyFFT.dft(in inRe, in inIm, ref dRe, ref dIm);
                for (int k = 0; k < N; k++)
                {
        ...
        void WorkspaceReuseOneSize(int N, uint seed)
        {
            ...
            fProxyFFT.dft(in re0, in im0, ref dRe, ref dIm);
            var fRe = re0.Copy(); var fIm = im0.Copy();
            fProxyFFT.fft(ref fRe, ref fIm, in ws);
            fProxy relTol = (fProxy)1E-3f;
            for (int k = 0; k < N; k++)
            {
                AssertCloseRel(fRe[k], dRe[k], relTol);
            ...
            fProxyFFT.rfft(in real, ref oRe, ref oIm);   // no-ws oracle
            for (int k = 0; k <= N / 2; k++)
            {
            ...
            fProxyFFT.ifft(ref fRe, ref fIm, in ws);
            for (int i = 0; i < N; i++)
            {
