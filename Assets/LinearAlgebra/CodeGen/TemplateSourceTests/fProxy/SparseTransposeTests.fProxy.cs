...
// Milestone B test suite for the materialized-transpose optimization on fProxyBSM (block-CSR):
//   * Arena.fProxyBSMTranspose(in A)         -- builds A^T as its own compressed BSM
//   * fProxyBSMOperator(in A, in AT) two-arg  -- ApplyT forwards to spMV(AT, .) instead of spMVT(A, .)
//
...
public class fProxySparseTransposeTests
{
    ...
    public struct SparseTransposeTestJob : IJob
    {
        ...
        // fProxyBSMTranspose. Only 4 of the 6 block positions are filled (leaving (1,0) and (2,1)
        // empty) so genuine block sparsity -- not a dense grid -- is transposed.
        static fProxyBSM BuildTallRectBSM(ref Arena arena)
        {
            const int blockRows = 3, blockCols = 2, BR = 3, BC = 2;
            var b = arena.fProxyBSMBuilder(blockRows, blockCols, BR, BC, 4);
        ...
        static fProxyBSM BuildWideRectBSM(ref Arena arena)
        {
        ...
        // outer dimensions.
        static void CrossCheckTranspose(ref Arena arena, in fProxyBSM A, uint seed)
        {
            var AT = arena.fProxyBSMTranspose(in A);
        ...
        void OperatorApplyTParity()
        {
            ...
            var op1 = new fProxyBSMOperator(in A);        // one-arg: ApplyT == on-the-fly spMVT(A, .)
            var op2 = new fProxyBSMOperator(in A, in AT); // two-arg: ApplyT == spMV(AT, .)
        ...
        //
        // For symmetric upper-block storage A == A^T by construction, so fProxyBSMTranspose returns
