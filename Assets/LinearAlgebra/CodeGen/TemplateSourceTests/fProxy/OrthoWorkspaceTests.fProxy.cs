...
public class fProxyOrthoWorkspaceTests
{
    ...
    public struct WorkspaceEquivJob : IJob
    {
        ...
        void DecompEquiv(int M, int N)
        {
            ...
            var A = arena.fProxyRandomMatrix(M, N, -1f, 1f, 73101);
        ...
        void DirectSolveEquiv()
        {
            ...
            int dim = 16;
            var A0 = arena.fProxyRandomMatrix(dim, dim, -1f, 1f, 51237);
            // make well-conditioned
            for (int d = 0; d < dim; d++)
                A0[d, d] += 5f;

            var xOrig = arena.fProxyRandomVector(dim, -3f, 3f, 99001);
        ...
        void SolveQRSolve(int M, int N)
        {
            ...
            var A = arena.fProxyRandomMatrix(M, N, -1f, 1f, 33417);
