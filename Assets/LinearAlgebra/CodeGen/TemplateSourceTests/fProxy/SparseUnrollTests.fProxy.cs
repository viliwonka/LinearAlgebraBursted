...
public class fProxySparseUnrollTests
{
    ...
    public struct SparseUnrollTestJob : IJob
    {
        ...
        void CheckSquareSpMV(int b, uint seedBase)
        {
            ...
            var x = GenerateOP.fProxyRandomVec(A.N_Cols, (fProxy)(-1f), (fProxy)1f, seedBase + 900u);
        ...
        void CheckSquareSpMVT(int b, uint seedBase)
        {
            var A = BuildRandomSquare(b, seedBase);
            var dense = A.ToDense(Allocator.Temp);
            var xt = GenerateOP.fProxyRandomVec(A.M_Rows, (fProxy)(-1f), (fProxy)1f, seedBase + 900u);
            ...
            var diff = new fProxyN(in yt, Allocator.Temp);
            fProxyComp.subInPlace(diff, ytRef);
            Assert.IsTrue(Analysis.isZero(diff, Tol()));

            var yt2 = BSR.spMVT(in A, in xt);
            var diff2 = new fProxyN(in yt2, Allocator.Temp);
        ...
        void CheckRectangularSpMV()
        {
            ...
            builder.AddBlock(0, 0, GenerateOP.fProxyRandomMat(BR, BC, (fProxy)(-1f), (fProxy)1f, 71001));
            builder.AddBlock(0, 2, GenerateOP.fProxyRandomMat(BR, BC, (fProxy)(-1f), (fProxy)1f, 71002));
            builder.AddBlock(1, 1, GenerateOP.fProxyRandomMat(BR, BC, (fProxy)(-1f), (fProxy)1f, 71003));
            builder.AddBlock(2, 0, GenerateOP.fProxyRandomMat(BR, BC, (fProxy)(-1f), (fProxy)1f, 71004));
            var A = builder.ToBSR(Allocator.Temp);
            var dense = A.ToDense(Allocator.Temp);

            var x = GenerateOP.fProxyRandomVec(A.N_Cols, (fProxy)(-1f), (fProxy)1f, 71100);
            var y = new fProxyN(A.M_Rows, Allocator.Temp);
            BSR.spMV(in A, in x, ref y);
            var diff = new fProxyN(in y, Allocator.Temp);
        ...
        void CheckRectangularSpMVT()
        {
            const int BR = 2, BC = 3;
            var builder = new fProxyBSRBuilder(3, 3, BR, BC, Allocator.Temp);
            builder.AddBlock(0, 0, GenerateOP.fProxyRandomMat(BR, BC, (fProxy)(-1f), (fProxy)1f, 72001));
            builder.AddBlock(0, 2, GenerateOP.fProxyRandomMat(BR, BC, (fProxy)(-1f), (fProxy)1f, 72002));
            builder.AddBlock(1, 1, GenerateOP.fProxyRandomMat(BR, BC, (fProxy)(-1f), (fProxy)1f, 72003));
            builder.AddBlock(2, 0, GenerateOP.fProxyRandomMat(BR, BC, (fProxy)(-1f), (fProxy)1f, 72004));
            var A = builder.ToBSR(Allocator.Temp);
            var dense = A.ToDense(Allocator.Temp);

            var xt = GenerateOP.fProxyRandomVec(A.M_Rows, (fProxy)(-1f), (fProxy)1f, 72100);
            var yt = new fProxyN(A.N_Cols, Allocator.Temp);
            BSR.spMVT(in A, in xt, ref yt);
