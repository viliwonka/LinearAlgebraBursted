...
public class fProxyQRTests
{
    ...
    public struct TestJob : IJob
    {
        ...
        public void QRDecompIdentity()
        {
            ...
            var Q = GenerateOP.fProxyIdentityMat(dim, allocator: Allocator.Temp);
        ...
        public void QRDecompIdentityNonSquare()
        {
            ...
            var Q = new fProxyMxN(dim*2, dim, Allocator.Temp);
            var R = new fProxyMxN(dim, dim, Allocator.Temp);
        ...
        public void QRDecompRandomDiagonal()
        {
            ...
            var Q = GenerateOP.fProxyRandomDiagonalMat(dim, 1f, 3f, allocator: Allocator.Temp);
            var R = new fProxyMxN(dim, dim, Allocator.Temp);
        ...
        public void QRDecompRandom()
        {
            ...
            var R = new fProxyMxN(dim, dim, Allocator.Temp);
            var Q = GenerateOP.fProxyRandomMat(dim*2, dim, -0.5f, 0.5f, 94221, allocator: Allocator.Temp);
        ...
        public void QRDecompRandomLarge()
        {
            ...
            var R = new fProxyMxN(dim, dim, Allocator.Temp);
            var Q = GenerateOP.fProxyRandomMat(dim * 2, dim, -5f, 5f, 9612221, allocator: Allocator.Temp);
        ...
        public void QRDecompHilbert()
        {
            ...
            var Q = GenerateOP.fProxyHilbertMat(dim, allocator: Allocator.Temp);
            var R = new fProxyMxN(dim, dim, Allocator.Temp);
        ...
        public void QRDecompPermutation() {
            ...
            for (int i = 0; i < tests; i++) {
                ...
                var Q = GenerateOP.fProxyPermutationMat(dim, p0, p1, allocator: Allocator.Temp);
                ...
                Q = Blas.dot(GenerateOP.fProxyPermutationMat(dim, p0, p1, allocator: Allocator.Temp), Q);
        ...
        {
            int m = 10, n = 5;
            var Q = GenerateOP.fProxyRandomMat(m, n, -1f, 1f, 555123, allocator: Allocator.Temp);
            for (int r = 0; r < m; r++)
        ...
        void QRDecompBlockedNonAligned(int m, int n, uint seed)
        {
            ...
            var Q = GenerateOP.fProxyRandomMat(m, n, -5f, 5f, seed, allocator: Allocator.Temp);
            for (int d = 0; d < n; d++)
        ...
        {
            int m = 12, n = 6;
            var A = GenerateOP.fProxyRandomMat(m, n, -3f, 3f, 909090, allocator: Allocator.Temp);
        ...
        {
            int dim = 8;
            var A = GenerateOP.fProxyRandomMat(dim, dim, -5f, 5f, 13131, allocator: Allocator.Temp);
            for (int d = 0; d < dim; d++) A[d, d] += 10f;
            var xKnown = GenerateOP.fProxyRandomVec(dim, -3f, 3f, 24242, allocator: Allocator.Temp);
        ...
        void QRDecompPreservesABlocked()
        {
            ...
            var A = GenerateOP.fProxyRandomMat(m, n, -5f, 5f, 576512, allocator: Allocator.Temp);
            for (int d = 0; d < n; d++)
        ...
        public void Execute() {
            ...
            for (uint i = 0; i < tests; i++) {
                ...
                if(Type == TestType.RandomDiagonal)
                    A = GenerateOP.fProxyRandomDiagonalMat(dim, 1f, 3f, 21410 + i*i + i*7, allocator: Allocator.Temp);
                else
                    A = GenerateOP.fProxyRandomMat(dim*2, dim, -25f, +25f, 21410 + i*i + i*7, allocator: Allocator.Temp);
    ...
    public struct SolveSystemTestJob : IJob {
        ...
        void SquareFullRank() {
            ...
            for (uint i = 0; i < randomMatTests; i++) {
                ...
                fProxyMxN A = GenerateOP.fProxyRandomMat(systemDim, systemDim, -5, +5, 420 + i * 7, allocator: Allocator.Temp);

                for(int d = 0; d < systemDim; d++)
                    A[d, d] += 5.1f + 10f*random.NextFProxy();

                var Q = A.Copy();
                var R = new fProxyMxN(systemDim, systemDim, Allocator.Temp);

                QR.decompInPlace(ref Q, ref R);

                for(uint j = 0; j < randomVecTests; j++) {

                    fProxyN xOrig = GenerateOP.fProxyRandomVec(systemDim, -25, +25, 1337 + i * i + j * 5, allocator: Allocator.Temp);
        ...
        void OverdeterminedFullRank() {
            ...
            for (uint i = 0; i < randomMatTests; i++) {
                ...
                fProxyMxN A = GenerateOP.fProxyRandomMat(sysDimM, sysDimN, -5, +5, 420 + i * 7, allocator: Allocator.Temp);

                for (int d = 0; d < sysDimN; d++)
                    A[d, d] += 5.1f + 10f * random.NextFProxy();

                var Q = A.Copy();
                var R = new fProxyMxN(sysDimN, sysDimN, Allocator.Temp);

                QR.decompInPlace(ref Q, ref R);

                for (uint j = 0; j < randomVecTests; j++) {

                    fProxyN xOrig = GenerateOP.fProxyRandomVec(sysDimN, -25, +25, 1337 + i * i + j * 5, allocator: Allocator.Temp);
                    fProxyN b = Blas.dot(A, xOrig);
                    fProxyN y = Blas.dot(b, Q);
        ...
        void SquareFullRankDirect() {
            ...
            for (uint i = 0; i < randomMatTests; i++) {
                ...
                fProxyN xOrig = GenerateOP.fProxyRandomVec(systemDim, -25, +25, 1337 + i * i + i * 5, allocator: Allocator.Temp);
                fProxyN b = Blas.dot(A, xOrig);
                fProxyN x = new fProxyN(systemDim, Allocator.Temp);
        ...
        void OverdeterminedFullRankDirect() {
            ...
            for (uint i = 0; i < randomMatTests; i++) {
                ...
                fProxyN xOrig = GenerateOP.fProxyRandomVec(sysDimN, -25, +25, 1337 + i * i + i * 5, allocator: Allocator.Temp);
                fProxyN b = Blas.dot(A, xOrig);
                fProxyN x = new fProxyN(sysDimN, Allocator.Temp);
    ...
    public struct LQTestJob : IJob
    {
        ...
        {
            int dim = 8;
            var A    = GenerateOP.fProxyIdentityMat(dim, allocator: Allocator.Temp);
            var origA = A.Copy();
        ...
        {
            int dim = 8;
            var A    = GenerateOP.fProxyRandomMat(dim, dim, -0.5f, 0.5f, 77123, allocator: Allocator.Temp);
            var origA = A.Copy();
            var L    = new fProxyMxN(dim, dim, Allocator.Temp);
            var Q    = new fProxyMxN(dim, dim, Allocator.Temp);
            LQ.decomp(in A, ref L, ref Q);
        ...
        {
            int m = 4, n = 9;
            var A    = GenerateOP.fProxyRandomMat(m, n, -0.5f, 0.5f, 94221, allocator: Allocator.Temp);
            var origA = A.Copy();
        ...
        {
            int m = 8, n = 16;
            var A    = GenerateOP.fProxyRandomMat(m, n, -1f, 1f, 12345, allocator: Allocator.Temp);
            var origA = A.Copy();
            var L    = new fProxyMxN(m, m, Allocator.Temp);
            var Q    = new fProxyMxN(m, n, Allocator.Temp);
            LQ.decomp(in A, ref L, ref Q);
        ...
        private void AssertLQ(in fProxyMxN A, in fProxyMxN L, in fProxyMxN Q, fProxy precision)
        {
            ...
            fProxyMxN diff = new fProxyMxN(in A, Allocator.Temp);
    ...
    public struct LQMinNormTestJob : IJob
    {
        ...
        {
            int m = 3, n = 6;
            var A    = GenerateOP.fProxyRandomMat(m, n, -1f, 1f, 11111, allocator: Allocator.Temp);
            var c    = GenerateOP.fProxyRandomVec(m, -1f, 1f, 22222, allocator: Allocator.Temp);
        ...
        {
            int m = 4, n = 9;
            var A    = GenerateOP.fProxyRandomMat(m, n, -1f, 1f, 33333, allocator: Allocator.Temp);
            var c    = GenerateOP.fProxyRandomVec(m, -1f, 1f, 44444, allocator: Allocator.Temp);
            var xTrue = new fProxyN(n, Allocator.Temp);
            Blas.dot(in c, in A, ref xTrue);
            var b = new fProxyN(m, Allocator.Temp);
            Blas.dot(in A, in xTrue, ref b);
        ...
        {
            int m = 8, n = 16;
            var A    = GenerateOP.fProxyRandomMat(m, n, -1f, 1f, 55555, allocator: Allocator.Temp);
            var c    = GenerateOP.fProxyRandomVec(m, -1f, 1f, 66666, allocator: Allocator.Temp);
            var xTrue = new fProxyN(n, Allocator.Temp);
            Blas.dot(in c, in A, ref xTrue);
            var b = new fProxyN(m, Allocator.Temp);
            Blas.dot(in A, in xTrue, ref b);
        ...
        {
            int m = 5, n = 12;
            var A = GenerateOP.fProxyRandomMat(m, n, -2f, 2f, 77777, allocator: Allocator.Temp);
            var b = GenerateOP.fProxyRandomVec(m, -1f, 1f, 88888, allocator: Allocator.Temp);
            var x = new fProxyN(n, Allocator.Temp);
            LQ.minNormSolve(in A, in b, ref x);
            // residual = A x - b
        ...
        void ResidualCheckLargeBlocked()
        {
            ...
            var A = GenerateOP.fProxyRandomMat(m, n, -2f, 2f, 131313, allocator: Allocator.Temp);
            for (int d = 0; d < m; d++)
                A[d, d] += (fProxy)20f;
            var b = GenerateOP.fProxyRandomVec(m, -1f, 1f, 141414, allocator: Allocator.Temp);
            var x = new fProxyN(n, Allocator.Temp);
            LQ.minNormSolve(in A, in b, ref x);
            var Ax = new fProxyN(m, Allocator.Temp);
            Blas.dot(in A, in x, ref Ax);
        ...
        void ResidualCheckMidBlocked()
        {
            ...
            var A = GenerateOP.fProxyRandomMat(m, n, -2f, 2f, 151617, allocator: Allocator.Temp);
            for (int d = 0; d < m; d++)
                A[d, d] += (fProxy)20f;
            var b = GenerateOP.fProxyRandomVec(m, -1f, 1f, 181920, allocator: Allocator.Temp);
            var x = new fProxyN(n, Allocator.Temp);
            LQ.minNormSolve(in A, in b, ref x);
            var Ax = new fProxyN(m, Allocator.Temp);
            Blas.dot(in A, in x, ref Ax);
        ...
        {
            int m = 5, n = 12;
            var A = GenerateOP.fProxyRandomMat(m, n, -2f, 2f, 191919, allocator: Allocator.Temp);
            var b = GenerateOP.fProxyRandomVec(m, -1f, 1f, 292929, allocator: Allocator.Temp);
        ...
        {
            fProxyN diff = new fProxyN(in got, Allocator.Temp);
