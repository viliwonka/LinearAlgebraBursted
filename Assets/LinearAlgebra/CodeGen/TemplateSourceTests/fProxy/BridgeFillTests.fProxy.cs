...
public class fProxyBridgeFillTests
{
    ...
    {
        var buf = new fProxyMxN(4, 4, Allocator.Temp);
        for (int i = 0; i < 16; i++) buf[i] = (fProxy)i;

        var view = new fProxyMxN(in buf, 2, 2);
        ...
        Assert.Throws<ArgumentException>(() => { var bad = new fProxyMxN(in buf, 5, 4); });

        var full = new fProxyMxN(in buf, 4, 4);
