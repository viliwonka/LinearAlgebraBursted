...
// the sparse op against the DENSE reference: build a fProxyBSM via the builder, expand with
...
public class fProxySparseBSMTests
{
    [BurstCompile]
    public struct SparseBSMTestJob : IJob
    {
        ...
        void HandBuiltSmall()
        {
            ...
            const int BR = 3, BC = 3;
            var builder = arena.fProxyBSMBuilder(2, 2, BR, BC);
        ...
        static fProxyBSM BuildRandom(ref Arena arena)
        {
            const int BR = 3, BC = 3;
        ...
        void RectangularBlocks()
        {
            ...
            const int BR = 2, BC = 3;
            var builder = arena.fProxyBSMBuilder(2, 3, BR, BC); // 4 x 9 dense
        ...
        void DuplicateSummation()
        {
            ...
            const int BR = 2, BC = 2;
            var builder = arena.fProxyBSMBuilder(2, 2, BR, BC); // 4 x 4 dense
        ...
        void OutOfOrderTriplets()
        {
            ...
            const int BR = 2, BC = 2;
            var builder = arena.fProxyBSMBuilder(2, 3, BR, BC); // 4 x 6 dense
        ...
        void OneByOneBlocks()
        {
            ...
            var builder = arena.fProxyBSMBuilder(4, 4, 1, 1); // 4 x 4 scalar sparse
        ...
        static fProxyBSM BuildDenseGrown(ref Arena arena, in fProxyMxN refDense, out int tripletCount)
        {
            int N = refDense.M_Rows;
            var builder = arena.fProxyBSMBuilder(N, N, 1, 1); // DEFAULT capacityHint = 8
            for (int r = 0; r < N; r++)
                for (int c = 0; c < N; c++)
    ...
    //
    // fProxyBSM.ToDense and fProxyBSMBuilder.ToBSM used to take `in Arena arena`, but both call a
    // MUTATING arena allocator method internally (arena.fProxyMat / arena.fProxyBSM). Since those
    ...
    // Linear_OP.trans(dense).tempfProxyMat -- dereferenced the dangling pointer and threw
    ...
    public void AddBlock_BlockRowOutOfBounds_Throws()
    {
        ...
        try
        {
            var builder = arena.fProxyBSMBuilder(2, 2, 3, 3);
    ...
    public void AddValue_GlobalIndexOutOfBounds_Throws()
    {
        ...
        try
        {
            var builder = arena.fProxyBSMBuilder(2, 2, 3, 3); // 6 x 6
    ...
    static fProxyBSM BuildSquare(ref Arena arena)
    {
        const int BR = 2, BC = 2;
        var builder = arena.fProxyBSMBuilder(2, 2, BR, BC);
