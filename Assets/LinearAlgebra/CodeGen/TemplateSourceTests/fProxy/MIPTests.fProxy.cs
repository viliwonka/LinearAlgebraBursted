...
// stein/p0033 known-answer oracles). Templated (fProxy) so codegen emits a float and a double build; per the draft spec
...
// slots (flag / got / expected-or-limit / diff-or-extra) exactly like LPTests.fProxy.cs.
public class fProxyMIPTests
{
    ...
    public struct TestJob : IJob
    {
        ...
        void Knapsack3()
        {
            ...
            AssertClose(x[0], (fProxy)1, (fProxy)1e-3);   // unique optimum {0,2}
            AssertClose(x[1], (fProxy)0, (fProxy)1e-3);
            AssertClose(x[2], (fProxy)1, (fProxy)1e-3);
        ...
        void GomoryWolsey()
        {
            ...
            AssertTrue(info.status == MIPStatus.Optimal);
            AssertClose(x[0], (fProxy)5, (fProxy)1e-3);   // unique integer optimum (5,0)
            AssertClose(x[1], (fProxy)0, (fProxy)1e-3);
        ...
        void GomoryWolseyTrueInfiniteBound()
        {
            ...
            AssertClose(x[0], (fProxy)5, (fProxy)1e-3);
            AssertClose(x[1], (fProxy)0, (fProxy)1e-3);
            AssertCloseD(obj, -40.0, 1e-3);
        ...
        void RunEnumCase(int n, int m, uint seed)
        {
            ...
            if (!any && Fail[0] == (fProxy)0) { Fail[0] = (fProxy)1; Fail[1] = (fProxy)0; Fail[2] = (fProxy)1; Fail[3] = (fProxy)0; }
            Assert.IsTrue(any);
        ...
        void GeneralIntBounds()
        {
            ...
            AssertTrue(info.status == MIPStatus.Optimal);
            AssertClose(x[0], (fProxy)7, (fProxy)1e-3);
        ...
        void AssertPartialIncumbent(in MIPInfo info, double obj, in fProxyN x)
        {
            ...
            AssertClose(x[0], math.round(x[0]), (fProxy)1e-4);
            AssertClose(x[1], math.round(x[1]), (fProxy)1e-4);
        ...
        void SetTriple(fProxyMxN A, int r, int i, int j, int k)
        {
        ...
        // with fProxyLPCache's persisted DSE weights (same optimum, but pricing at a warm-started
        ...
        void LargeMagnitudeIntegrality()
        {
            ...
            for (int s = 0; s < cases; s++)
            {
                ...
                AssertTrue(info.status == MIPStatus.Optimal);
                AssertClose(x[0], (fProxy)750000, (fProxy)1e-3);
    ...
    //      LPTests.fProxy.cs's Solve/Lad dimension-mismatch tests) ----
