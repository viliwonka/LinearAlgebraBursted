// <auto-generated>
//   Generated from Assets/LinearAlgebra/CodeGen/TemplateSource/MG/AMG.Galerkin.fProxy.cs
...
namespace LinearAlgebra.Sparse
{
    public static partial class AMG
    {
        ...
        /// </summary>
        public static floatBSR galerkinRAP(in floatBSR A, in floatBSR T, in Indices aggId, int numAgg, ref Arena arena)
        {
            if (A.Symmetric)
            ...
            var builder = arena.floatBSRBuilder(numAgg, numAgg, m, m, math.max(1, A.Nnzb));
            var contrib = new floatMxN(m, m, Allocator.Temp, true);   // fully written each block
                ...
                for (int k = s; k < e; k++)
                {
                    ...
                    for (int p = 0; p < m; p++)
                        for (int q = 0; q < m; q++)
                        {
                            ...
                            for (int r = 0; r < BR; r++)
                            {
                                float tip = T.Values[ti + r * m + p];
                                if (tip == (float)0) continue;
                                float inner = 0;
                                for (int c = 0; c < BR; c++)
        ...
        /// <summary>Standalone twin of <see cref="galerkinRAP(in floatBSR, in floatBSR, in Indices, int, ref Arena)"/>:
        /// allocates the coarse operator from <paramref name="allocator"/> instead of an arena;
        /// caller owns disposing the returned <see cref="floatBSR"/>. Internal assembly scratch is
        /// Temp and disposed before returning.</summary>
        public static floatBSR galerkinRAP(in floatBSR A, in floatBSR T, in Indices aggId, int numAgg, Allocator allocator = Allocator.Temp)
        {
            if (A.Symmetric)
                throw new ArgumentException("AMG.galerkinRAP: A must be full storage (mirror a Symmetric BSR first)");
            if (A.BR != A.BC || A.BlockRows != A.BlockCols)
            ...
            var builder = new floatBSRBuilder(numAgg, numAgg, m, m, Allocator.Temp, math.max(1, A.Nnzb));
            var contrib = new floatMxN(m, m, Allocator.Temp, true);   // fully written each block
