...
namespace BULA
{
    ...
    /// The <c>in doubleBSR</c> overloads are the one-shot route: they build a hierarchy from
    ...
    public static partial class MG
    {
        ...
        /// <see cref="doubleAMG"/> yourself to amortize the setup over several solves or right-hand
        /// sides. A must be a square SPD BSR. The build's own result lands in <paramref name="setup"/>
        /// (levels, coarsest size, status), as with the <see cref="doubleAMG"/> constructors; pass
        ...
        /// </summary>
        public static SolveInfo solve(in doubleBSR A, in AMGOptions opts, out AMGSetupInfo setup,
                                      in doubleN b, ref doubleN x, int maxIter, double tol)
        {
            var amg = new doubleAMG(in A, in opts, out setup, Allocator.Temp);
            try
            {
                if (!setup.Solved)
        ...
        public static SolveInfo solve(in doubleBSR A, out AMGSetupInfo setup,
                                      in doubleN b, ref doubleN x, int maxIter, double tol)
        ...
        public static SolveInfo solve(in doubleBSR A, in AMGOptions opts, out AMGSetupInfo setup,
                                      in doubleN b, ref doubleN x)
            => solve(in A, in opts, out setup, in b, ref x, 50, Consts.doubleSqrtEps);

        /// <summary>One-shot AMG solve over a BSR with <see cref="AMGOptions.Default"/>, default
        /// maxIter (50 cycles) and tol (Consts.doubleSqrtEps).</summary>
        public static SolveInfo solve(in doubleBSR A, out AMGSetupInfo setup,
                                      in doubleN b, ref doubleN x)
            => solve(in A, AMGOptions.Default, out setup, in b, ref x, 50, Consts.doubleSqrtEps);
    }
}
