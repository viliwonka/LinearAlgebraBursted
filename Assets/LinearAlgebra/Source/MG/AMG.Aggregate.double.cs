...
namespace BULA.Sparse
{
    ...
    public static partial class AMG
    {
        ...
        static double BlockFrobenius(in doubleBSR A, int k)
        {
        ...
        static int DiagBlockIndex(in doubleBSR A, int i)
        {
        ...
        ///
        /// A must be FULL storage (not <see cref="doubleBSR.Symmetric"/>) with a structurally
        ...
        /// </summary>
        public static void aggregate(in doubleBSR A, double theta, ref Indices aggId, out int numAgg)
        {
            if (A.Symmetric)
            ...
            for (int i = 0; i < nb; i++)
            {
                ...
                diagNormF[i] = d < 0 ? (double)0 : BlockFrobenius(in A, d);
            }
            ...
            for (int i = 0; i < nb; i++)
            {
                ...
                for (int k = s; k < e; k++)
                {
                    ...
                    double str = BlockFrobenius(in A, k);
        ...
        static bool Strong(in doubleBSR A, int k, int i, int j, double theta, NativeArray<double> diagNormF)
        ...
        static bool StrongNorm(double nij, int i, int j, double theta, NativeArray<double> diagNormF)
        {
            if (theta <= (double)0) return nij > (double)0;
            double di = diagNormF[i], dj = diagNormF[j];
            ...
            if (di <= (double)0 || dj <= (double)0) return false;
