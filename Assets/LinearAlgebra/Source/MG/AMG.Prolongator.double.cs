...
namespace LinearAlgebra.Sparse
{
    public static partial class AMG
    {
        ...
        /// </summary>
        public static doubleBSR tentativeProlongator(in doubleBSR A, in Indices aggId, int numAgg,
            in doubleMxN B, ref Arena arena, out doubleMxN Bcoarse)
        {
            int nb = A.BlockRows;
            int BR = A.BR;
            int m = B.N_Cols;
            ...
            Bcoarse = arena.doubleMat(numAgg * m, m);   // arena.doubleMat clears (uninit=false)

            var builder = arena.doubleBSRBuilder(nb, numAgg, BR, m, math.max(1, nb));

            double relTol = Consts.doubleSqrtEps;
            ...
            for (int a = 0; a < numAgg; a++)
            {
                ...
                var L = new doubleMxN(rows, m, Allocator.Temp, true);
                for (int l = 0; l < k; l++)
                {
                ...
                // `clear`).
                var R = new doubleMxN(m, m, Allocator.Temp, false);
                ...
                for (int j = 0; j < m; j++)
                {
                    ...
                    if (nrm > relTol * initNorm && nrm > Consts.doubleEpsilon)
                    {
                        R[j, j] = nrm;
                        double inv = (double)1 / nrm;
                        for (int row = 0; row < rows; row++) L[row, j] *= inv;
                    }
                    else
                    {
                        R[j, j] = 0;
                        for (int row = 0; row < rows; row++) L[row, j] = (double)0;
                    }
                }

                // Scatter Q into T (one BR x m block per member row) and R into the coarse
                // near-nullspace block for aggregate a.
                var blk = new doubleMxN(BR, m, Allocator.Temp, true);
                for (int l = 0; l < k; l++)
                {
                    int gi = aggMembers[s + l];
                    for (int r = 0; r < BR; r++)
        ...
        /// <summary>Standalone twin of <see cref="tentativeProlongator(in doubleBSR, in Indices, int, in doubleMxN, ref Arena, out doubleMxN)"/>:
        ...
        public static doubleBSR tentativeProlongator(in doubleBSR A, in Indices aggId, int numAgg,
            in doubleMxN B, out doubleMxN Bcoarse, Allocator allocator = Allocator.Temp)
        {
            int nb = A.BlockRows;
            int BR = A.BR;
            int m = B.N_Cols;
            ...
            Bcoarse = new doubleMxN(numAgg * m, m, allocator);   // fresh alloc clears (uninit=false default)

            var builder = new doubleBSRBuilder(nb, numAgg, BR, m, Allocator.Temp, math.max(1, nb));
        ...
        /// </summary>
        public static doubleBSR tentativeProlongator(in doubleBSR A, in Indices aggId, int numAgg,
            ref Arena arena, out doubleMxN Bcoarse)
        {
            int n = A.M_Rows;
            var ones = arena.doubleMat(n, 1);
        ...
        public static doubleBSR tentativeProlongator(in doubleBSR A, in Indices aggId, int numAgg,
            out doubleMxN Bcoarse, Allocator allocator = Allocator.Temp)
        {
            int n = A.M_Rows;
