...
namespace LinearAlgebra
{
    ...
    public partial struct doubleN : IDisposable, IUnsafedoubleArray {
        ...
        // struct size is identical in both configs either way, since this adds no field). doubleN
        // has no spare bits to pack a generation
        // stamp into (32B = 8 _rec + 24 UnsafeList<double>, exactly -- see docs/dev/rfc-memory-model.md
        ...
        // stamp, which doubleMxN/doubleBSR carry in their own padding hole -- see those types).
        //
        ...
        // TempCopy() and the cross-type allocation shortcuts (doubleN.Shortcuts.cs) that used to
        ...
        public unsafe doubleN Copy()
        {
            ...
            return OwnerArena.doubleVec(in this);
        }
        ...
        public unsafe doubleN TempCopy()
        {
            ...
            return OwnerArena.doubleTempVec(in this);   // temp pool (was wrongly the persistent Copy path)
        }
