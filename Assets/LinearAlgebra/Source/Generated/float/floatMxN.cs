...
namespace LinearAlgebra
{
    ...
    // n = cols
    //
    // [StructLayout(Sequential)]: pins field order/packing explicitly (matches floatN's existing
    ...
    public partial struct floatMxN : IDisposable, IUnsafefloatArray {
        ...
        // Arena-tracked path -- see floatN.cs's `_rec` doc comment for the full rationale (same
        ...
        // TempCopy() and the cross-type allocation shortcuts (floatMxN.Shortcuts.cs) that used to
        ...
        // floatN's option, cannot tell "still the same allocation" from "a new one that reused this
        // slot number").
        ...
        // doc comment). Unlike floatN (Alive-only), floatMxN also has
