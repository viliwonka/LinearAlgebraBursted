...
namespace LinearAlgebra
{
    ...
    public partial struct floatN : IDisposable, IUnsafefloatArray {
        ...
        // Stays default(UnsafeList<float>) whenever _rec != null (arena-tracked).
        ...
        // stamp into (32B = 8 _rec + 24 UnsafeList<float>, exactly -- see docs/rfc-memory-model.md
        ...
        private unsafe void AssertRecordAlive()
        {
            if (_rec != null && !ChunkedRecordTable<floatVecRecord>.IsAliveFast(_rec))
                throw new InvalidOperationException("floatN.Data: use of disposed/cleared arena allocation");
        }
#endif
        ...
        /// arena's record table by the caller (Arena.floatVec/floatTempVec) -- this ctor only
        /// fills in the record's Data, it does not allocate or own the slot itself.
        /// </summary>
        internal unsafe floatN(int n, floatVecRecord* rec, Allocator allocator, bool uninit = false) {
        ...
        internal unsafe floatN(in floatN orig, floatVecRecord* rec, Allocator allocator) {
        ...
        public unsafe floatN Copy()
        {
            ...
            return OwnerArena.floatVec(in this);
        }
