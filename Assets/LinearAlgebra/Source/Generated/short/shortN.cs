...
namespace LinearAlgebra
{
    ...
    public partial struct shortN : IDisposable, IUnsafeshortArray {
        ...
        // struct size is identical in both configs either way, since this adds no field). shortN
        // has no spare bits to pack a generation
        // stamp into (32B = 8 _rec + 24 UnsafeList<short>, exactly -- see docs/dev/rfc-memory-model.md
        ...
        // TempCopy() and the cross-type allocation shortcuts (shortN.Shortcuts.cs) that used to
        ...
        public unsafe shortN TempCopy()
        {
            ...
            return OwnerArena.shortTempVec(in this);   // temp pool (was wrongly the persistent Copy path)
        }
