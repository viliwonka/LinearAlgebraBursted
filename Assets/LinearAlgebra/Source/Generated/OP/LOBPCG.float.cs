...
namespace LinearAlgebra
{
    ...
    /// <see cref="Eigen.eigenSymmetric(ref floatMxN, ref floatN, ref floatMxN)"/> solver for the
    ...
    /// solves it with <see cref="Eigen.eigenSymmetric(ref floatMxN, ref floatN, ref floatMxN)"/>,
    ...
    /// allocation). The one exception: <see cref="Eigen.eigenSymmetric(ref floatMxN, ref floatN, ref floatMxN)"/>
    ...
    public static partial class LOBPCG
    {
            ...
            for (int iter = 0; iter < maxIter; iter++)
            {
                ...
                for (int i = numActive - 1; i >= 0; i--)
                {
                    ...
                    for (int c = 0; c < n; c++)
                    {
                        ...
                        float rv = ws.AX[i, c] - ws.lambda[i] * ws.BX[i, c];
                        ws.R[i, c] = rv;
                        rn2 += rv * rv;
                    }
        ...
        /// <see cref="Solvers.pcg(in floatBSR, in floatBlockJacobi, in floatN, ref floatN)"/>
        /// consumes it).
        /// </summary>
        ...
        /// <c>floatBSROperator</c>/<c>floatBlockJacobi</c>. Note the preconditioner M is built from
        ...
        static unsafe void RequireDistinctBuffers(in floatLOBPCGCache ws)
        {
            ...
            for (int i = 0; i < count; i++)
                for (int j = i + 1; j < count; j++)
                    if (ptrs[i] == ptrs[j])
                        throw new ArgumentException("LOBPCG: workspace buffers must be distinct (use Arena.floatLOBPCGCache(n, k))");
        }

        // Same-buffer, smaller-shaped logical view: floatMxN.M_Rows/N_Cols are plain mutable
        ...
        static bool FactorGram(ref floatMxN Gram, ref floatMxN L, int m)
        {
            ...
            // (CS0102; see Cholesky.float.cs's CHOL_BLOCK comment).
            ...
            float scale = (float)0;
            for (int i = 0; i < m; i++) { float d = math.abs(Gram[i, i]); if (d > scale) scale = d; }
