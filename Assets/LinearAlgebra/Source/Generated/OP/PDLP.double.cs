...
namespace LinearAlgebra
{
    public static partial class LP
    {
        ...
        // using ONLY A·v / Aᵀ·v (matrix-free over any IdoubleLinearOperator) plus elementwise clamps --
        ...
        // applied matrix-free via doubleRowColScaledOperator). A BSR overload + large-sparse benchmark is
        ...
        static double pdlpKkt<TOp>(in TOp A, in doubleN x, in doubleN y,
                                   in doubleN lc, in doubleN uc, in doubleN lv, in doubleN uv, in doubleN c,
                                   ref doubleN Ax, ref doubleN Aty, double bScale, double cScale)
        ...
        static LPInfo pdlpCore<TOp>(in TOp A, in doubleN lc, in doubleN uc, in doubleN lv, in doubleN uv,
                                    in doubleN c, ref doubleN x, out double objective, int maxIter, double epsOpt)
            where TOp : struct, IdoubleLinearOperator
        {
            int m = A.Rows, n = A.Cols;

            var y    = new doubleN(m, Allocator.Temp);   // dual (row multipliers); Temp => starts at 0
            var xnew = new doubleN(n, Allocator.Temp);   // trial x⁺
            var ynew = new doubleN(m, Allocator.Temp);   // trial y⁺
            var Ax   = new doubleN(m, Allocator.Temp);   // A·x  (cached across iterations)
            var Axp  = new doubleN(m, Allocator.Temp);   // A·x⁺ (trial; also kkt scratch)
            var Aty  = new doubleN(n, Allocator.Temp);   // Aᵀ·y (reused across step-size retries)
            var sumX = new doubleN(n, Allocator.Temp);   // running-average accumulators
            var sumY = new doubleN(m, Allocator.Temp);
            var avgX = new doubleN(n, Allocator.Temp);
            var avgY = new doubleN(m, Allocator.Temp);
            var xPrev = new doubleN(n, Allocator.Temp);  // previous restart point (drives the primal weight)
            var yPrev = new doubleN(m, Allocator.Temp);
            var xBest = new doubleN(n, Allocator.Temp);  // best (lowest-μ) average seen -- reported at the end
            ...
            double normA;
            {
                double inv = (double)(1.0 / math.sqrt((double)n));
                ...
                for (int k = 0; k < 20; k++)
                {
                    ...
                    if (!(nrm > 0)) break;
                    lam = nrm;
                    double s = (double)(1.0 / nrm);
                    for (int j = 0; j < n; j++) xnew[j] = Aty[j] * s;
                }
                normA = (double)math.sqrt(lam);
            }
            ...
            while (status != LPStatus.Optimal && iters < budget)
            {
                ...
                {
                    double tau = (double)(etaTry / omega), sigma = (double)(etaTry * omega);
                    ...
                    for (int i = 0; i < m; i++)
                    {
                        double aext = (double)2 * Axp[i] - Ax[i];
                        double w = y[i] + sigma * aext;
                ...
                if ((iters & 63) == 0)
                {
                    double invc = (double)(1.0 / (double)cnt);
                    ...
                    if (restart)
                    {
                        ...
                        for (int j = 0; j < n; j++) { x[j] = avgX[j]; xPrev[j] = avgX[j]; sumX[j] = (double)0; }
                        for (int i = 0; i < m; i++) { y[i] = avgY[i]; yPrev[i] = avgY[i]; sumY[i] = (double)0; }
        ...
        static void pdlpEquilibrateDense(in doubleMxN A, ref doubleN Dr, ref doubleN Dc)
        {
            int m = A.M_Rows, n = A.N_Cols;
            for (int i = 0; i < m; i++) Dr[i] = (double)1;
            for (int j = 0; j < n; j++) Dc[j] = (double)1;
            ...
            for (int it = 0; it < 10; it++)
            {
                for (int i = 0; i < m; i++)                       // row inf-norms of the current Â
                {
                    ...
                    if (R > 1e-30) Dr[i] = (double)((double)Dr[i] / math.sqrt(R));
                }
                for (int j = 0; j < n; j++)                       // column inf-norms (using the updated Dr)
                {
                    ...
                    if (C > 1e-30) Dc[j] = (double)((double)Dc[j] / math.sqrt(C));
                }
            }
        ...
        // Solve the equilibrated problem on Â = Dr·A·Dc (matrix-free, via doubleRowColScaledOperator) and
        ...
        static LPInfo pdlpScaledSolve<TOp>(in TOp inner, in doubleN Dr, in doubleN Dc,
                                           in doubleN lc, in doubleN uc, in doubleN lv, in doubleN uv, in doubleN c,
                                           ref doubleN x, out double objective, int maxIter, double epsOpt)
            where TOp : struct, IdoubleLinearOperator
        {
            int m = inner.Rows, n = inner.Cols;
            var lcS = new doubleN(m, Allocator.Temp);
            var ucS = new doubleN(m, Allocator.Temp);
            var lvS = new doubleN(n, Allocator.Temp);
            var uvS = new doubleN(n, Allocator.Temp);
            var cS  = new doubleN(n, Allocator.Temp);
            var scratchN = new doubleN(n, Allocator.Temp);
            var scratchM = new doubleN(m, Allocator.Temp);

            for (int i = 0; i < m; i++)
            {
                lcS[i] = math.abs((double)lc[i]) >= 1e29 ? lc[i] : (double)((double)lc[i] * (double)Dr[i]);
                ucS[i] = math.abs((double)uc[i]) >= 1e29 ? uc[i] : (double)((double)uc[i] * (double)Dr[i]);
            }
            for (int j = 0; j < n; j++)
            {
                lvS[j] = math.abs((double)lv[j]) >= 1e29 ? lv[j] : (double)((double)lv[j] / (double)Dc[j]);
                uvS[j] = math.abs((double)uv[j]) >= 1e29 ? uv[j] : (double)((double)uv[j] / (double)Dc[j]);
                cS[j]  = (double)((double)c[j] * (double)Dc[j]);
                x[j]   = (double)((double)x[j] / (double)Dc[j]);          // x̂ = x / Dc
            }

            var op = new doubleRowColScaledOperator<TOp>(in inner, in Dr, in Dc, in scratchN, in scratchM);
            var info = pdlpCore(in op, in lcS, in ucS, in lvS, in uvS, in cS, ref x, out double _, maxIter, epsOpt);

            objective = 0;
            for (int j = 0; j < n; j++)
            {
                x[j] = (double)((double)Dc[j] * (double)x[j]);           // x = Dc · x̂
        ...
        /// with the solution.
        /// </summary>
        public static LPInfo pdlp(in doubleMxN A, in doubleN lc, in doubleN uc, in doubleN lv, in doubleN uv,
                                  in doubleN c, ref doubleN x, out double objective, int maxIter = 0, double epsOpt = 1e-6)
        {
            int m = A.M_Rows, n = A.N_Cols;
            ...
            var Dr = new doubleN(m, Allocator.Temp);
