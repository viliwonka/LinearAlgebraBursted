...
namespace LinearAlgebra
{
    ...
    /// Reuses the dense <see cref="Eigen.symmetric(ref doubleMxN, ref doubleN, ref doubleMxN)"/>
    /// solver for the small (&lt;= 3k) Rayleigh-Ritz sub-problem and
    /// <see cref="CHO.decomp(in doubleMxN, ref doubleMxN)"/> for orthogonalization and the
    ...
    /// allocated once via <c>Arena.doubleLOBPCGCache(n, k)</c> and reused across calls. The
    /// O(k)-scale Rayleigh-Ritz dense matrices are also cache fields. The only bounded
    /// <c>Allocator.Temp</c> allocations are inside
    /// <see cref="Eigen.symmetric(ref doubleMxN, ref doubleN, ref doubleMxN)"/> and the
    ...
    /// overloads forward into this same core with B = <see cref="doubleIdentityOperator"/>, giving a
    ...
    public static partial class Eigen
    {
        ...
        static bool WantedWithinTol(in doubleLOBPCGCache ws, int kWork, int k, double tol)
        {
            for (int i = 0; i < kWork; i++)
            {
                int rank = 0;
        ...
        static int ConvergedWithinTol(in doubleLOBPCGCache ws, int k, double tol)
        {
            int c = 0;
            for (int i = 0; i < k; i++)
            {
