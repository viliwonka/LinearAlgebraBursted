...
namespace LinearAlgebra
{
    public static partial class Bidiag
    {
        ...
        /// <see cref="bidiagonalizeValues"/>. Matches Arena.doubleBidiagWorkspace(m, n).
        /// </summary>
        static void RequireBidiagWorkspace(in doubleBidiagWorkspace ws, int m, int n, bool needLeftU, string who)
        {
            bool ok =
            ...
            if (!ok)
                throw new ArgumentException(who + ": workspace must be sized for m x n (use Arena.doubleBidiagWorkspace(m, n))");
        }
    }

    /// <summary>
    /// Reusable scratch for Golub-Kahan-Householder bidiagonalization (Bidiag.bidiagonalize /
    /// bidiagonalizeValues). Allocate ONCE via Arena.doubleBidiagWorkspace(m, n) and reuse it across
    ...
    /// </summary>
    public struct doubleBidiagWorkspace
    {
        public doubleMxN W;
    ...
    public partial struct Arena
    {
        ...
        /// </summary>
        public doubleBidiagWorkspace doubleBidiagWorkspace(int m, int n)
        {
            return new doubleBidiagWorkspace
            {
                W = doubleMat(m, n),
                leftU = doubleMat(m, n),
                uVec = doubleVec(m),
                vVec = doubleVec(n),
                wScratch = doubleVec(n)
            };
        }
    }
}
