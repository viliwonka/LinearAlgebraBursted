...
namespace LinearAlgebra
{
    public static partial class Eigen
    {
        ...
        /// (three length-n vectors) — the layout produced by Arena.floatEigenSymWorkspace(n).
        /// </summary>
        static void RequireEigenSymWorkspace(in floatEigenSymWorkspace ws, int n, string who = "eigenvaluesSymmetric")
        {
            if (ws.eVec.N != n || ws.vVec.N != n || ws.pVec.N != n)
                throw new ArgumentException(who + ": workspace must be sized for n (use Arena.floatEigenSymWorkspace(n))");
        }
    }
    ...
    /// rank-2-update vector p). Allocate ONCE via Arena.floatEigenSymWorkspace(n) and reuse it across
    /// same-size calls so repeated symmetric eigenvalue solves are zero-alloc.
    /// </summary>
    public struct floatEigenSymWorkspace
    {
        public floatN eVec;
        public floatN vVec;
        public floatN pVec;
    ...
    public partial struct Arena
    {
        ...
        /// </summary>
        public floatEigenSymWorkspace floatEigenSymWorkspace(int n)
        {
            return new floatEigenSymWorkspace
            {
                eVec = floatVec(n),
                vVec = floatVec(n),
                pVec = floatVec(n)
