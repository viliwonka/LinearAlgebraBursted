...
namespace LinearAlgebra
{
    ...
    public static partial class Eigen {
        ...
        ///   - Does not allocate.
        /// </summary>
        public static bool powerIteration<TOp>(in TOp A, ref doubleN v, ref doubleN w,
        ...
        /// <see cref="doubleMxN"/>. Forwards into <see cref="powerIteration{TOp}"/> via
        ...
        /// </summary>
        public static bool powerIteration(in doubleBSM A, ref doubleN v, ref doubleN w,
                                          out double lambda, double tol, int maxIter)
        {
        ...
        /// Does not allocate.
        /// </summary>
        public static bool lanczos<TOp>(in TOp A, ref doubleLanczos_WS ws, ref doubleN eigenvalues,
            ...
            where TOp : struct, IdoubleLinearOperator
        {
            ...
            double seedNormSq = (double)0;
            for (int i = 0; i < n; i++)
                seedNormSq += ws.V[0, i] * ws.V[0, i];

            if (seedNormSq == (double)0) {
                for (int i = 0; i < n; i++)
                    ws.V[0, i] = (double)(1 + (i & 3));
                seedNormSq = (double)0;
                for (int i = 0; i < n; i++)
                    seedNormSq += ws.V[0, i] * ws.V[0, i];
            }

            double invSeedNorm = (double)1 / math.sqrt(seedNormSq);
            for (int i = 0; i < n; i++)
            ...
            for (int jj = 0; jj < steps; jj++) {
                ...
                // vCur = V row jj (A.Apply requires a standalone doubleN distinct from its output).
                for (int i = 0; i < n; i++)
                    ws.vCur[i] = ws.V[jj, i];
            ...
            if (produced < steps) {
                ...
                double bound = (double)0;
                for (int i = 0; i < produced; i++) {
                    double radius = math.abs(ws.alpha[i]);
                ...
                double pad = bound > (double)1 ? bound : (double)1;
        ...
        public static bool lanczos<TOp>(in TOp A, ref doubleLanczos_WS ws, ref doubleN eigenvalues,
            ...
            where TOp : struct, IdoubleLinearOperator
            => lanczos(in A, ref ws, ref eigenvalues, out produced, steps, Consts.doubleZeroThreshold);

        /// <summary>
        /// Lanczos tridiagonalization of a SYMMETRIC dense <see cref="doubleMxN"/>. Forwards into
        /// <see cref="lanczos{TOp}"/> via <see cref="doubleDenseOperator"/> -- see that method for
        ...
        /// </summary>
        public static bool lanczos(in doubleMxN A, ref doubleLanczos_WS ws, ref doubleN eigenvalues,
                                   out int produced, int steps, double breakdownTol)
        {
            return lanczos(new doubleDenseOperator(in A), ref ws, ref eigenvalues, out produced, steps, breakdownTol);
        }
        ...
        /// </summary>
        public static doubleN lanczos(ref Arena arena, in doubleMxN A, int steps, out int produced,
                                      out bool converged, double breakdownTol)
        {
        ...
        /// <summary>lanczos (allocating) over a dense matrix with default breakdownTol (Consts.doubleZeroThreshold).</summary>
        public static doubleN lanczos(ref Arena arena, in doubleMxN A, int steps, out int produced, out bool converged)
        ...
        /// <see cref="lanczos(in doubleMxN, ref doubleLanczos_WS, ref doubleN, out int, int, double)"/>.
        /// Forwards into <see cref="lanczos{TOp}"/> via <c>doubleBSMOperator</c>.
        /// </summary>
        public static bool lanczos(in doubleBSM A, ref doubleLanczos_WS ws, ref doubleN eigenvalues,
                                   out int produced, int steps, double breakdownTol)
        {
            return lanczos(new doubleBSMOperator(in A), ref ws, ref eigenvalues, out produced, steps, breakdownTol);
        }
        ...
        /// </summary>
        public static doubleN lanczos(ref Arena arena, in doubleBSM A, int steps, out int produced,
                                      out bool converged, double breakdownTol)
        {
            var ws = arena.doubleLanczos_WS(A.M_Rows, steps);
            var eigenvalues = arena.doubleVec(steps);
        ...
        ///   - Does not allocate.
        /// </summary>
        /// <remarks>DEPRECATED: prefer <see cref="eigenSymmetric(ref doubleMxN, ref doubleN, ref doubleMxN)"/>
        /// (Householder tridiagonalization + QL, ~30x faster) for symmetric eigenpairs, or
        /// <see cref="eigenvaluesSymmetric(ref doubleMxN, ref doubleN)"/> for eigenvalues only. Retained for reference.</remarks>
...
#pragma warning restore 618
                                                 ...
                                                 ref doubleEigenSym_WS ws)
        {
            ...
            unsafe
            {
                ...
                {
                    double a = math.abs(ap[ii]);
                    if (a > matScale) matScale = a;
                }
                double belowNormTol = (double)n * Consts.doubleEpsilon * matScale;
            ...
            for (int i = 1; i < n; i++)
            {
                double rowSum = math.abs(eVec[i - 1]) + math.abs(eigenvalues[i]) + math.abs(eVec[i]);
