...
namespace LinearAlgebra
{
    public static partial class Eigen {
        ...
        static double InversePowerResidual(in doubleN Ap, in doubleN v, double lambda, int n)
        {
            double residual = (double)0;
            for (int i = 0; i < n; i++) {
                double ri = math.abs(Ap[i] - lambda * v[i]);
        ...
        public static LanczosInfo lanczosVectors<TOp>(in TOp A, ref doubleLanczosCache ws, ref doubleMxN Yt,
                                               ...
                                               int steps)
            where TOp : struct, IdoubleLinearOperator
            => lanczosVectors(in A, ref ws, ref Yt, ref eigenvalues, ref ritz, steps, Consts.doubleZeroThreshold);
                                             ...
                                             double breakdownTol)
        {
            ...
            info = lanczosVectors(new doubleDenseOperator(in A), ref ws, ref Yt, ref eigenvalues, ref ritz, steps, breakdownTol);
            return eigenvalues;
        }

        /// <summary>lanczosVectors (allocating) over a dense matrix with default breakdownTol.</summary>
        public static doubleN lanczosVectors(ref Arena arena, in doubleMxN A, int steps, out doubleMxN ritz, out LanczosInfo info)
        ...
        public static bool decompInPlace(ref doubleMxN A_to_D, ref doubleN eigenvalues,
                                              ref doubleMxN V, int maxSweeps, double eps)
        {
            ...
            for (int i = 0; i < n; i++) {
                for (int j = i + 1; j < n; j++) {
                    double aij = A_to_D[i, j];
                    double aji = A_to_D[j, i];
            ...
            for (int sweep = 0; sweep < maxSweeps; sweep++) {
                ...
                for (int p = 0; p < n - 1; p++) {
                    for (int q = p + 1; q < n; q++) {
                        ...
                        if (math.abs(apq) <= eps * (double)0.5 * (math.abs(A_to_D[p, p]) + math.abs(A_to_D[q, q])))
                            continue;

                        // Compute rotation angle: theta = (A[q,q] - A[p,p]) / (2 * A[p,q])
                        double theta = (A_to_D[q, q] - A_to_D[p, p]) / ((double)2 * apq);
                        ...
                        double app = A_to_D[p, p];
                        double aqq = A_to_D[q, q];
                        ...
                        for (int i = 0; i < n; i++) {
                            if (i == p || i == q)
                                continue;
                            double aip = A_to_D[i, p];
                            double aiq = A_to_D[i, q];
...
#pragma warning disable 618
        ...
        public static bool decompInPlace(ref doubleMxN A_to_D, ref doubleN eigenvalues,
            ...
            => decompInPlace(ref A_to_D, ref eigenvalues, ref V, maxSweeps, Consts.doubleZeroThreshold);

        /// <summary>decompInPlace with default maxSweeps (30) and eps (Consts.doubleZeroThreshold).</summary>
        [System.Obsolete("Prefer Eigen.symmetric (Householder tridiagonal + QL, ~30x faster) for symmetric eigenpairs, or Eigen.valuesSymmetric for eigenvalues only. This cyclic-Jacobi solver is retained for reference.", false)]
        public static bool decompInPlace(ref doubleMxN A_to_D, ref doubleN eigenvalues,
                                              ref doubleMxN V)
            => decompInPlace(ref A_to_D, ref eigenvalues, ref V, 30, Consts.doubleZeroThreshold);
