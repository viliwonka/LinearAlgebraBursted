...
namespace LinearAlgebra
{
    ...
    public static partial class Optimize {
                                     ...
                                     float delta, float xTol, int maxIter)
        {
            ...
            var prevx = new floatN(n, Allocator.Temp);
            ...
            while (it < maxIter)
            {
                for (int a = 0; a < n; a++) { rhs[a] = (float)0; for (int col = 0; col < n; col++) G[a, col] = (float)0; }
                ...
                for (int i = 0; i < m; i++)
                {
                    ...
                    float wi = (float)1 / math.max(math.abs(ri), delta);
                    for (int a = 0; a < n; a++)
                    {
                        float wa = wi * A[i, a];
                        rhs[a] += wa * b[i];
        ...
        /// (Consts.floatZeroThreshold) and maxIter (50). Pass a zeroed <paramref name="x"/> for a
        /// from-scratch solve.</summary>
        public static LPInfo ladIRLS(in floatMxN A, in floatN b, ref floatN x)
        {
            float mb = (float)0;
            for (int i = 0; i < b.N; i++) mb = math.max(mb, math.abs(b[i]));
            float delta = (float)1e-3 * ((float)1 + mb);
            return ladIRLS(in A, in b, ref x, delta, Consts.floatZeroThreshold, 50);
        }
    }
}
