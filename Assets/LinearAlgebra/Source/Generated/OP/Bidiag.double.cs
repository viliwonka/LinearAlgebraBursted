...
namespace LinearAlgebra
{
    ...
    public static partial class Bidiag
    {
        ...
        private static void genHouseholderCol(ref doubleMxN M, ref doubleN u, int k, double zeroThreshold)
        {
            ...
            if (math.abs(xNorm) > zeroThreshold)
            {
                for (int r = k; r < M.M_Rows; r++)
                    u[r] = u[r] / xNorm;
                u[k] = u[k] + sign(u[k]);
                double div = math.sqrt(math.abs(u[k]));
                for (int r = k; r < M.M_Rows; r++)
                    u[r] = u[r] / div;
            }
            else
        ...
        /// <see cref="doubleBidiagCache"/>); nothing is allocated internally.</para>
        /// </summary>
                                         ...
                                         ref doubleBidiagCache ws)
        {
            ...
            unsafe
            {
                UnsafeUtility.MemClear(V.Data.Ptr, (long)n * n * UnsafeUtility.SizeOf<double>());
                UnsafeUtility.MemClear(B.Data.Ptr, (long)n * n * UnsafeUtility.SizeOf<double>());
            }
            for (int i = 0; i < n; i++)
                V[i, i] = (double)1;
            ...
            unsafe
            {
                UnsafeUtility.MemClear(U.Data.Ptr, (long)m * n * UnsafeUtility.SizeOf<double>());
            }
            for (int j = 0; j < n; j++)
                U[j, j] = (double)1;
