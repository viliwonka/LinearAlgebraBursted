...
namespace LinearAlgebra
{
    public static partial class QR {
        ...
        public static DirectSolveInfo decompSolve(ref doubleMxN Q, ref doubleMxN R, ref doubleMxN B, ref doubleMxN X) {
        ...
        public static unsafe DirectSolveInfo solveInPlace(ref doubleMxN A, ref doubleMxN B, ref doubleMxN X) {
            if (A.M_Rows < A.N_Cols)
                throw new ArgumentException("QR.solveInPlace: Matrix A must be square or tall (more or equal rows than cols)");
            ...
            for (int r = 0; r < n; r++) {
                double* Xr = X.Data.Ptr + (long)r * k;
                double* Br = Bp + (long)r * k;
                for (int j = 0; j < k; j++)
                    Xr[j] = Br[j];
            }
