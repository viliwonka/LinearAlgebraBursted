...
namespace LinearAlgebra
{
    ...
    public static class RandomOP
    {
        ...
        {
            if (n <= 0)
                throw new ArgumentException("RandomOP.sampleKWithoutReplacementInpl: n must be > 0");
            int k = dest.N;
            if (k > n)
                throw new ArgumentException("RandomOP.sampleKWithoutReplacementInpl: dest.N must be <= n");
