...
namespace LinearAlgebra
{
    public static partial class SVD {
        ...
        //   - All arithmetic uses native float precision; stability comes from the two-pass DGKS
                                        ...
                                        ref floatSvdTruncatedWorkspace ws, out bool converged)
        {
            ...
            unsafe
            {
                ...
                float* uBuf_ptr = ws.uBuf.Data.Ptr; // m-vector scratch (also used as coeff temp in v-reortho)
                float* vBuf_ptr = ws.vBuf.Data.Ptr; // n-vector scratch (also used as coeff temp in u-reortho)
                float* A_ptr    = A.Data.Ptr;        // m×n row-major

                // --- Zero-initialize UL (p×m) and VL ((p+1)×n) ---
                UnsafeUtility.MemClear(UL_ptr, (long)ws.UL.Data.Length * UnsafeUtility.SizeOf<float>());
                UnsafeUtility.MemClear(VL_ptr, (long)ws.VL.Data.Length * UnsafeUtility.SizeOf<float>());
                ...
                for (int j = 0; j < p; j++)
                {
                    ...
                    if (j > 0)
                    {
                        ...
                        {
                            UnsafeUtility.MemClear(vBuf_ptr, (long)j * UnsafeUtility.SizeOf<float>());
                    ...
                    UnsafeUtility.MemCpy(UL_ptr + j * m, uBuf_ptr, (long)m * UnsafeUtility.SizeOf<float>());
                    ...
                    for (int pass = 0; pass < 2; pass++)
                    {
                        UnsafeUtility.MemClear(uBuf_ptr, (long)(j + 1) * UnsafeUtility.SizeOf<float>());
                    ...
                    UnsafeUtility.MemCpy(VL_ptr + (j + 1) * n, vBuf_ptr, (long)n * UnsafeUtility.SizeOf<float>());
                }
            }
