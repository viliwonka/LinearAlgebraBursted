    ...
    public static partial class SVD {
                                        ...
                                        bool partialReorth, ref floatSVDTruncatedCache ws, out bool converged)
        {
            ...
            unsafe
            {
                ...
                float seedNorm2 = Unsafe_OP.vecDot(v0, v0, n);
                if (seedNorm2 > (float)0)
                    Unsafe_OP.scalMul(v0, n, (float)1 / math.sqrt(seedNorm2));
                else
                    v0[0] = (float)1;
                ...
                if (partialReorth)
                {
                    ...
                    for (int j = 0; j < p; j++)
                    {
                        ...
                        if (j > 0 && ws.alpha[j] < gamma * ws.beta[j - 1])
                        {
                            ...
                            {
                                float t = Unsafe_OP.vecDot(UL_ptr + (j - 1) * m, uBuf_ptr, m);
                        ...
                        if (ws.beta[j] < gamma * ws.alpha[j])
                        {
                            ...
                            for (int it = 0; it < 4; it++)
                            {
                                float t = Unsafe_OP.vecDot(VL_ptr + j * n, vBuf_ptr, n);
