...
namespace LinearAlgebra
{
    ...
    // (MIP.Domain.float.cs's PushBoundChange/UndoToMarker), same as stage 2. A queue jump is not
    // generally to an ancestor, so it cannot replay/undo that stack -- instead each queued node carries
    // its own full length-n bound snapshot (floatMIPQueueNode) and a jump overwrites the live bound
    ...
    internal struct floatMIPQueueNode
    {
        public floatN L;          // full lower-bound snapshot, length n (owned -- Dispose on pop/drain)
        public floatN U;          // full upper-bound snapshot, length n (owned -- Dispose on pop/drain)
        ...
        public float fracValue;   // branchVar's fractional LP value at the parent
        public float newBound;    // the bound this node applies to branchVar (floor or ceil of fracValue)
    ...
    public static partial class MIP
    {
        ...
        internal static void HeapPush(ref UnsafeList<floatMIPQueueNode> heap, floatMIPQueueNode node)
        {
            heap.Add(node);
            int i = heap.Length - 1;
            while (i > 0)
            {
                ...
                floatMIPQueueNode tmp = heap[parent]; heap[parent] = heap[i]; heap[i] = tmp;
                i = parent;
            }
        }

        internal static floatMIPQueueNode HeapPopMin(ref UnsafeList<floatMIPQueueNode> heap)
        {
            floatMIPQueueNode top = heap[0];
            ...
            while (true)
            {
                ...
                if (smallest == i) break;
                floatMIPQueueNode tmp = heap[smallest]; heap[smallest] = heap[i]; heap[i] = tmp;
                i = smallest;
            }
            return top;
        }
                                    ...
                                    double absGap = 0.0, double relGap = 0.0)
        {
            ...
            var Aaug = new floatMxN(mAug, nY, Allocator.Temp);      // zero-initialized
            var bAug = new floatN(mAug, Allocator.Temp);
            var sensesAug = new NativeArray<ConstraintSense>(mAug, Allocator.Temp);
            var costY = new floatN(nY, Allocator.Temp);             // zero-initialized
            ...
            for (int j = 0; j < n; j++)
            {
                ...
                if (rowUB[j] >= 0)
                {
                    ...
                    Aaug[r, col[j]] = hiFiniteHere ? (float)1 : (float)0;
                    bAug[r] = hiFiniteHere ? (xu[j] - xl[j]) : (float)0;
                    sensesAug[r] = ConstraintSense.LessEqual;
                }
                if (rowLB[j] >= 0)
                {
                    int r = rowLB[j];
                    Aaug[r, col[j]] = (float)1;
                    bAug[r] = (float)0;   // root bound == xl[j] -> shifted rhs 0
                    sensesAug[r] = ConstraintSense.GreaterEqual;
                }
            }

            var curLB = new floatN(math.max(n, 1), Allocator.Temp);
            var curUB = new floatN(math.max(n, 1), Allocator.Temp);
        ...
        internal static void UnshiftToX(floatN y, NativeArray<byte> kind, NativeArray<int> col,
                                        floatN xlRoot, floatN xuRoot, int n, floatN xOut)
        {
            for (int j = 0; j < n; j++)
            {
        ...
        internal static MIPInfo SearchCore(floatMxN Aaug, floatN bAug, floatN costY, NativeArray<ConstraintSense> sensesAug,
                                           floatN c, NativeArray<byte> integrality,
                                           ...
                                           floatN xOut)
        {
            ...
            // UndoToMarker/ApplyNodeBounds's UB-row activation sites, MIP.Domain.float.cs); rhs-only
            // bound updates (the common plunge/strong-branch case) leave it alone, so most re-solves hit.
            var cache = new floatLPCache(nY, mAug, Allocator.Temp);
            var y = new floatN(nY, Allocator.Temp);
            var trialY = new floatN(nY, Allocator.Temp);
            ...
            var pcUpSum = new floatN(math.max(n, 1), Allocator.Temp);      // zero-initialized
            var pcDownSum = new floatN(math.max(n, 1), Allocator.Temp);    // zero-initialized
            ...
            int pendingVar = -1; float pendingFrac = (float)0, pendingNewBound = (float)0;
            ...
            while (true)
            {
                ...
                if (!prune)
                {
                    ...
                    else
                    {
                        ...
                        bool downFirst = (v - floorV) <= (float)0.5;
                        ...
                        var qL = new floatN(math.max(n, 1), Allocator.Temp);
                        var qU = new floatN(math.max(n, 1), Allocator.Temp);
                        for (int j = 0; j < n; j++) { qL[j] = curLB[j]; qU[j] = curUB[j]; }
                        if (downFirst) qL[branchVar] = ceilV; else qU[branchVar] = floorV;

                        HeapPush(ref heap, new floatMIPQueueNode
                        {
                            L = qL,
                            U = qU,
                            parentBound = nodeObj,
                ...
                while (heap.Length > 0)
                {
                    floatMIPQueueNode entry = HeapPopMin(ref heap);
            ...
            if (status == MIPStatus.Infeasible || status == MIPStatus.Unbounded)
            {
                ...
                for (int j = 0; j < n; j++) xOut[j] = (float)0;
            }
            else
            {
        ...
        internal static void TryRoundingHeuristic(floatN xNode, NativeArray<byte> integrality, int n,
                                                   ...
                                                   floatMxN A, floatN b, NativeArray<ConstraintSense> senses, int m0,
                                                   floatN c, floatN curLB, floatN curUB, floatN xRound,
