...
namespace LinearAlgebra
{
    ...
    public static partial class QP
    {
        ...
        internal static QPInfo eqpSolve(in floatMxN Q, in floatN c, in floatMxN A_W, in floatN b_W,
                                        ref floatN x, ref floatN lambda)
        {
        ...
        internal static QPInfo eqpNullSpaceStep(in floatMxN Q, in floatN c, in floatMxN A_W, in floatN b_W,
                                                ref floatN x, ref floatN lambda)
        {
            int n = Q.M_Rows;
            int k = A_W.M_Rows;
            ...
            var AWT = new floatMxN(n, k, Allocator.Temp, true);
            Blas.trans(in A_W, ref AWT);

            var R = new floatMxN(k, k, Allocator.Temp, true);
            var u = new floatN(n, Allocator.Temp, false);
            var w = new floatN(math.max(k, math.max(nz, 1)), Allocator.Temp, false);

            FactorWorkingSetTranspose(ref AWT, ref R, ref u, ref w, zeroThreshold);

            var Qx = new floatN(n, Allocator.Temp, false);
            var g = new floatN(n, Allocator.Temp, false);
            ...
            {
                var gz = new floatN(nz, Allocator.Temp, false);
                for (int j = 0; j < nz; j++) gz[j] = g[k + j];

                var Z = new floatMxN(n, nz, Allocator.Temp, true);
                FormNullSpaceBasis(ref AWT, ref Z, ref u, ref w, k);

                var QZ = new floatMxN(n, nz, Allocator.Temp, true);
                var Hz = new floatMxN(nz, nz, Allocator.Temp, true);
                var y = new floatN(nz, Allocator.Temp, false);
                ...
                {
                    var p = new floatN(n, Allocator.Temp, false);
            ...
            var Ax = new floatN(k, Allocator.Temp, false);
        ...
        internal static void ApplyWorkingSetQtForward(ref floatMxN AWT, ref floatN v, ref floatN u, int k)
        {
            int n = AWT.M_Rows;
        ...
        internal static void FormNullSpaceBasis(ref floatMxN AWT, ref floatMxN Z, ref floatN u, ref floatN w, int k)
        {
            int n = AWT.M_Rows;
            int nz = Z.N_Cols;

            unsafe { UnsafeUtility.MemClear(Z.Data.Ptr, (long)Z.Data.Length * UnsafeUtility.SizeOf<float>()); }
            for (int j = 0; j < nz; j++)
                Z[k + j, j] = (float)1;
        ...
        // length >= M.N_Cols.
        internal static unsafe void ApplyReflectorFullWidth(ref floatMxN M, ref floatN u, ref floatN w, int d)
        {
            int rows = M.M_Rows;
            int cols = M.N_Cols;
            if (cols <= 0) return;
            ...
            UnsafeUtility.MemClear(wp, (long)cols * UnsafeUtility.SizeOf<float>());
                                                                ...
                                                                ref floatN gz, ref floatN y, float normInfQ, out bool regularized)
        {
            ...
            {
                regularized = true;
                float delta = math.sqrt(Consts.floatEpsilon) * math.max(normInfQ, (float)1);
        ...
        internal static QPInfo qpActiveSetCore(in floatMxN Q, in floatN c, in floatMxN A, in floatN b,
                                               in NativeArray<ConstraintSense> senses,
                                               in floatN xl, in floatN xu,
                                               ref floatN x, out double objective, int maxIter)
        {
            ...
            while (true)
            {
                ...
                var R = new floatMxN(k, k, Allocator.Temp);
                var u = new floatN(n, Allocator.Temp, true);
                var w = new floatN(math.max(k, math.max(nz, 1)), Allocator.Temp, true);
                ...
                if (haveNullSpace)
                {
                    gz = new floatN(nz, Allocator.Temp, true);
                    for (int j = 0; j < nz; j++) gz[j] = g[k + j];
                    Z = new floatMxN(n, nz, Allocator.Temp, true);
                    FormNullSpaceBasis(ref AWT, ref Z, ref u, ref w, k);
                    QZ = new floatMxN(n, nz, Allocator.Temp, true);
                    Hz = new floatMxN(nz, nz, Allocator.Temp, true);
                    y = new floatN(nz, Allocator.Temp, true);
                    ...
                    {
                        p = new floatN(n, Allocator.Temp, true);
                        haveP = true;
                        Blas.dot(in Z, in y, ref p);
                ...
                float thetaSelf = (float)1;
                float pScale = (float)1;
                float alphaTake = (float)0;
                int addRow = -1; bool addUpper = false;
                bool doTakeStep = false;
                bool doMultiplierCheck = small;
                floatN Ax = default, Ap = default;
                ...
                if (exitStatus == null && !small)
                {
                    ...
                    Ax = new floatN(math.max(m, 1), Allocator.Temp, true);
                    Ap = new floatN(math.max(m, 1), Allocator.Temp, true);
            ...
            if (status == QPStatus.Optimal)
            {
                ...
                int nzf = n - kf;
                var Rf = new floatMxN(kf, kf, Allocator.Temp);
                var uf = new floatN(n, Allocator.Temp, true);
                var wf = new floatN(math.max(kf, math.max(nzf, 1)), Allocator.Temp, true);
                FactorWorkingSetTranspose(ref AWTf, ref Rf, ref uf, ref wf, zeroThresholdAW);

                var Qxf = new floatN(n, Allocator.Temp, true);
                var gf = new floatN(n, Allocator.Temp, true);
            ...
            {
                var Axf = new floatN(math.max(m, 1), Allocator.Temp, true);
        ...
        internal static void BuildRowBounds(in floatN b, in NativeArray<ConstraintSense> senses,
                                            in floatN xl, in floatN xu, int m, int n,
                                            ref floatN L, ref floatN U)
        {
            float INF = (float)1e30;
            for (int i = 0; i < m; i++)
            {
        ...
        internal static void SeedWorkingSet(in floatMxN A, in floatN L, in floatN U, int m, int n, int T,
                                            in floatN x0, in floatN Ax0, NativeArray<byte> wstatus,
                                            float feasTol, float zeroThresholdAW)
        {
        ...
        // EXACTLY the returned k -- no reshape/view capability exists for floatMxN (see this file's
        // header comment on why per-iteration re-assembly, not incremental update, is v1's design).
        internal static int AssembleWorkingSetTranspose(in floatMxN A, in floatN L, in floatN U, int m, int n, int T,
                                                         NativeArray<byte> wstatus, int extraT, WorkingSetStatus extraStatus,
                                                         NativeArray<int> rowOfCol, out floatMxN AWT, out floatN bW)
        {
            int k = 0;
            for (int t = 0; t < T; t++) if (wstatus[t] != (byte)WorkingSetStatus.Inactive) k++;
            if (extraT >= 0) k++;

            AWT = new floatMxN(n, k, Allocator.Temp, true);
            bW = new floatN(k, Allocator.Temp, true);
        ...
        internal static void WriteWorkingSetColumn(in floatMxN A, in floatN L, in floatN U, int m, int n,
                                                    int t, WorkingSetStatus status,
                                                    ref floatMxN AWT, ref floatN bW, int col)
        {
            float sign = status == WorkingSetStatus.ActiveUpper ? (float)(-1) : (float)1;
            if (t < m)
                for (int i = 0; i < n; i++) AWT[i, col] = sign * A[t, i];
            else
            {
                int j = t - m;
                for (int i = 0; i < n; i++) AWT[i, col] = (float)0;
                AWT[j, col] = sign;
            }
            bW[col] = sign > (float)0 ? L[t] : -U[t];
        }
        ...
        internal static bool TryAddToWorkingSet(in floatMxN A, in floatN L, in floatN U, int m, int n, int T,
                                                NativeArray<byte> wstatus, int t, WorkingSetStatus candStatus,
                                                float zeroThresholdAW)
        {
            var rowOfCol = new NativeArray<int>(math.max(n, 1), Allocator.Temp);
            ...
            var R = new floatMxN(k, k, Allocator.Temp);
            var u = new floatN(n, Allocator.Temp, true);
            var w = new floatN(math.max(k, 1), Allocator.Temp, true);
            FactorWorkingSetTranspose(ref AWT, ref R, ref u, ref w, zeroThresholdAW);
        ...
        internal static void RatioTest(in floatN L, in floatN U, int m, int n, int T,
                                       NativeArray<byte> wstatus, NativeArray<bool> excluded,
                                       in floatN Ax, in floatN Ap, in floatN x, in floatN p,
                                       float pScale, float thetaSelf, float feasTol, float pivTol,
                                       out float alpha, out int winnerRow, out bool winnerUpper)
        {
            ...
            for (int t = 0; t < T; t++)
            {
                ...
                float d = (t < m ? Ap[t] : p[t - m]) / pScale;
                if (math.abs(d) <= pivTol) continue;
                float act = t < m ? Ax[t] : x[t - m];

                if (d > (float)0)
                {
                    if (U[t] >= (float)1e29) continue;
                    float tcand = (U[t] + feasTol - act) / d;
                    if (tcand < (float)0) tcand = (float)0;
                    if (tcand < thetaRelaxed) thetaRelaxed = tcand;
                }
                else
                {
                    if (L[t] <= (float)(-1e29)) continue;
                    float tcand = (L[t] - feasTol - act) / d;
                    if (tcand < (float)0) tcand = (float)0;
                    if (tcand < thetaRelaxed) thetaRelaxed = tcand;
                }
            }
            ...
            int winner = -1; float winnerMag = (float)(-1); float winnerExact = (float)0; bool winnerUp = false;
            for (int t = 0; t < T; t++)
            {
                if (wstatus[t] != (byte)WorkingSetStatus.Inactive || excluded[t]) continue;
                float d = (t < m ? Ap[t] : p[t - m]) / pScale;
                ...
                float act = t < m ? Ax[t] : x[t - m];
                bool isUp = d > (float)0;
                if (isUp && U[t] >= (float)1e29) continue;
                if (!isUp && L[t] <= (float)(-1e29)) continue;
                float bound = isUp ? U[t] : L[t];

                float texact = (bound - act) / d;
