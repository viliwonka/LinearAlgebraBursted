            ...
            for (int bi = 0; bi < blockRows; bi++)
                for (int k = rowPtr[bi]; k < rowPtr[bi + 1]; k++)
                {
                    ...
                    for (int r = 0; r < BR; r++)
                    {
                        ...
                        {
                            int gc = colBase + c; if (gc >= n) continue;
                            ra[gr] = (float)((double)ra[gr] + math.abs((double)blk[r * BC + c]) * (double)dc[gc]);
                        }
                    }
                }
            for (int i = 0; i < m; i++) { double s = (double)dr[i] * (double)ra[i]; if (s > 1e-30) dr[i] = (float)((double)dr[i] / math.sqrt(s)); }
            ...
            for (int bi = 0; bi < blockRows; bi++)
                for (int k = rowPtr[bi]; k < rowPtr[bi + 1]; k++)
                {
                    ...
                    for (int r = 0; r < BR; r++)
                    {
                        ...
                        {
                            int gc = colBase + c; if (gc >= n) continue;
                            ca[gc] = (float)((double)ca[gc] + math.abs((double)blk[r * BC + c]) * (double)dr[gr]);
                        }
                    }
                }
            for (int j = 0; j < n; j++) { double s = (double)dc[j] * (double)ca[j]; if (s > 1e-30) dc[j] = (float)((double)dc[j] / math.sqrt(s)); }
