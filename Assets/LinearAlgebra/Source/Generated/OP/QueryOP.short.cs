...
{
    // shortQueryOP: integer-exact search & selection inside integer vectors / matrices.
    ...
    public static partial class shortQueryOP
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static short WorstScoreForNearest(Metric m)
            => IsSimilarityMetric(m) ? short.MinValue : short.MaxValue;

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static short WorstScoreForFarthest(Metric m)
            => IsSimilarityMetric(m) ? short.MaxValue : short.MinValue;

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static bool IsBetterForNearest(short a, short b, Metric m)
            => IsSimilarityMetric(m) ? a > b : a < b;

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static bool IsBetterForFarthest(short a, short b, Metric m)
            => IsSimilarityMetric(m) ? a < b : a > b;
        ...
        public static void nearestRow(in shortMxN A, in shortN q, Metric m, out int index, out short score)
        {
            ...
            short best = WorstScoreForNearest(m);
            int bestIdx = 0;
            for (int r = 0; r < A.M_Rows; r++)
            {
        ...
        public static void farthestRow(in shortMxN A, in shortN q, Metric m, out int index, out short score)
        {
            ...
            short worst = WorstScoreForFarthest(m);
            int worstIdx = 0;
            for (int r = 0; r < A.M_Rows; r++)
            {
                short s = RowScore(in A, r, in q, m);
        ...
        public static int kNearestRows(in shortMxN A, in shortN q, int k, Metric m, ref Indices idx, ref shortN scores)
        {
            ...
            for (int r = 0; r < A.M_Rows; r++)
            {
                short s = shortQueryOP.RowScore(in A, r, in q, m);
                if (count < clampedK)
                {
                    int ins = count;
        ...
        public static int kNearestColumns(in shortMxN A, in shortN q, int k, Metric m, ref Indices idx, ref shortN scores)
        {
            ...
            for (int c = 0; c < A.N_Cols; c++)
            {
                short s = shortQueryOP.ColScore(in A, c, in q, m);
                if (count < clampedK)
                {
                    int ins = count;
                    while (ins > 0 && (sim ? s > scores[ins - 1] : s < scores[ins - 1])) ins--;
        ...
        public static int rowsWithinRadius(in shortMxN A, in shortN q, short r, Metric m, ref Indices idx)
        {
            ...
            for (int row = 0; row < A.M_Rows; row++)
            {
                short s = shortQueryOP.RowScore(in A, row, in q, m);
