...
namespace LinearAlgebra
{
    ...
    // CHO, exactly the LAD Frisch-Newton precedent (LP.FrischNewton.float.cs).
    //
    ...
    //
    // float storage throughout; double is used only for local scalar control math (norms, residual
    // ratios), never stored back into an float buffer -- no double-only kernel. All scratch is
    // Allocator.Temp (job-safe) except floatLQRState's own carried S buffer.
    // ================================================================================================
    public static partial class Control
    {
        ...
        /// exact recurrences and source). Use the <c>ref floatLQRState</c> overload instead when K
    ...
    /// carries the n x n Riccati solution S across separate top-level calls, the LPBasis/floatLPCache
    ...
    /// <see cref="floatLPCache"/> -- standalone, user-allocated, no arena requirement (needs to persist
    /// across separate solve calls). MUST be constructed before being passed to the warm <c>lqr</c>
    /// overload (unlike <see cref="floatLPCache"/>'s optional-cache contract, this struct IS the only
