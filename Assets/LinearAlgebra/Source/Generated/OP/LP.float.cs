...
namespace LinearAlgebra
{
    ...
    public static partial class LP
    {
        ...
        /// <summary>
        /// Warm-started re-solve: like <see cref="solve(in floatMxN, in floatN, in floatN, in NativeArray{ConstraintSense}, ref floatN, out double, LPMethod, int)"/>
        ...
        /// LP.DualSimplex.float.cs) -- the other backends have no analogous "seed from an arbitrary
        ...
        /// since the cache was last built (see <see cref="floatLPCache"/>'s doc comment for the exact
        ...
        /// few pivots.
        ///
        /// <paramref name="cache"/> not <see cref="floatLPCache.IsCreated"/> (e.g. <c>default</c>):
        /// behaves BYTE-IDENTICALLY to the plain <see cref="LPBasis"/> overload above (no cache in play).
        /// Otherwise must be <see cref="floatLPCache.IsValid"/> for this problem's shape (mismatch
        ...
        /// <see cref="floatLPCache"/>'s doc comment for the invalidation contract.</param>
        /// <param name="maxIter">Pivot/iteration budget; ≤0 picks a size-based default.</param>
        public static LPInfo solve(in floatMxN A, in floatN b, in floatN c,
                                   in NativeArray<ConstraintSense> senses,
                                   ref floatN x, out double objective,
                                   ref LPBasis basis, ref floatLPCache cache, int maxIter = 0)
        {
            int m = A.M_Rows, n = A.N_Cols, N = n + m;
            ...
            if (useCache && !cache.IsValid(n, m))
                throw new ArgumentException("LP.solve: cache dimensions do not match A (use new floatLPCache(n, m, allocator))");
        ...
        static void VerifyLPCacheHit(in floatMxN A, in floatN b, in floatN c,
                                     in NativeArray<ConstraintSense> senses, floatLPCache cache,
                                     int m, int n, int N)
        {
            var M2 = new floatMxN(m, N, Allocator.Temp);
            var lower2 = new floatN(N, Allocator.Temp);
            var upper2 = new floatN(N, Allocator.Temp);
            var cost2 = new floatN(N, Allocator.Temp);
            var rhs2 = new floatN(m, Allocator.Temp);
                ...
                for (int j = 0; j < N; j++)
                    if (M2[i, j] != cache.M[i, j])
                        throw new InvalidOperationException(
                            "LP.solve: floatLPCache hit but M differs from a fresh rebuild -- matrixVersion was not bumped after a structural change");

            for (int j = 0; j < N; j++)
                if (lower2[j] != cache.lower[j] || upper2[j] != cache.upper[j] || cost2[j] != cache.cost[j])
                    throw new InvalidOperationException(
                        "LP.solve: floatLPCache hit but bounds/cost differ from a fresh rebuild -- matrixVersion was not bumped after a structural change");
        ...
        /// size (m = A.M_Rows) --
        /// <see cref="ladBR(in floatMxN, in floatN, ref floatN, out double, int)"/> (Barrodale-
        /// Roberts specialized simplex) at or below the per-dtype threshold in the dispatch below,
        /// <see cref="ladFN(in floatMxN, in floatN, ref floatN, out double, int)"/> (Frisch-Newton
        ...
        /// default <see cref="lad(in floatMxN, in floatN, ref floatN, out double, int)"/> overload
                                 ...
                                 LPMethod method, int maxIter = 0)
        {
            ...
            int nv = 2 * n + 2 * m;
            var Alad = new floatMxN(m, nv, Allocator.Temp);            // zero-initialized
            var blad = new floatN(m, Allocator.Temp);
            var clad = new floatN(nv, Allocator.Temp);
            var senses = new NativeArray<ConstraintSense>(m, Allocator.Temp); // default = Equal (0)
            var xstd = new floatN(nv, Allocator.Temp);
            ...
            for (int i = 0; i < m; i++)
            {
                ...
                Alad[i, 2 * n + i] = (float)(-1);         // −u_i
                Alad[i, 2 * n + m + i] = (float)1;        // +v_i
                blad[i] = b[i];
            }
                                  ...
                                  ref floatN xStruct, out double objective, int maxIter)
        {
            ...
            for (int i = 0; i < m; i++)
            {
                ...
                if (c1b != (float)0) for (int j = 0; j < nCols; j++) cost1[j] -= c1b * T[i, j];
                if (c2b != (float)0) for (int j = 0; j < nCols; j++) cost2[j] -= c2b * T[i, j];
            }
                          ...
                          NativeArray<int> basis, int m, int nCols, int prow, int pcol)
        {
            ...
            float f1 = cost1[pcol];
            if (f1 != (float)0) for (int j = 0; j < nCols; j++) cost1[j] -= f1 * T[prow, j];
            float f2 = cost2[pcol];
            if (f2 != (float)0) for (int j = 0; j < nCols; j++) cost2[j] -= f2 * T[prow, j];
