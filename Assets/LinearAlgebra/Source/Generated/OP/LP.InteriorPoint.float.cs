...
namespace LinearAlgebra
{
    public static partial class LP
    {
                                   ...
                                   ref floatN x, out double objective, int maxIter)
        {
            ...
            var colVal = new NativeArray<float>(nv, Allocator.Temp);   // that nonzero's value
            int nDense = 0;
            for (int k = 0; k < nv; k++)
            {
                int cnt = 0, row = -1; float val = (float)0;
                for (int i = 0; i < m && cnt < 2; i++)
                {
                    float a = As[i, k];
                    if (a != (float)0) { cnt++; row = i; val = a; }
                }
            ...
            var Ad = new floatMxN(m, math.max(nDense, 1), Allocator.Temp);           // compacted multi-nonzero columns
            var dd = new floatN(math.max(nDense, 1), Allocator.Temp);                // their d[k] slice
            {
                int cc = 0;
                for (int k = 0; k < nv; k++)
                {
            ...
            var zBest = new floatN(nv, Allocator.Temp);
        ...
        // -- exactly UnsafeOP.matVecDot's shape -- so it is routed through that kernel (two float4
        ...
        static unsafe void Amul(floatMxN A, floatN v, floatN outv, int m, int nv)
        {
        ...
        static unsafe void ATmul(floatMxN A, floatN v, floatN outv, int m, int nv)
        {
        ...
        static void BuildNormalStructured(floatMxN Ad, floatN d, floatN dd, NativeArray<int> denseK,
                                          NativeArray<int> colRow, NativeArray<float> colVal,
                                          floatMxN M, int m, int nv, int nDense, float reg)
        {
            for (int c = 0; c < nDense; c++) dd[c] = d[denseK[c]];
            BuildNormal(Ad, dd, M, m, nDense, (float)0);
            for (int k = 0; k < nv; k++)
            {
                int r = colRow[k];
