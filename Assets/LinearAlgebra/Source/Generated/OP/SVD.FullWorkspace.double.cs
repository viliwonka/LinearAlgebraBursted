...
namespace LinearAlgebra
{
    public static partial class SVD
    {
        ...
        /// V n x n) — the layout produced by Arena.doubleSvdFullWorkspace(m, n).
        /// </summary>
        static void RequireSvdFullWorkspace(in doubleSvdFullWorkspace ws, int m, int n, string who)
        {
            if (ws.U.M_Rows != m || ws.U.N_Cols != n || ws.S.N != n || ws.V.M_Rows != n || ws.V.N_Cols != n)
                throw new ArgumentException(who + ": workspace must be sized for m x n (use Arena.doubleSvdFullWorkspace(m, n))");
        }
    }
    ...
    /// Allocate ONCE (sized for the matrix shape) via Arena.doubleSvdFullWorkspace(m, n) and reuse it
    ...
    /// </summary>
    public struct doubleSvdFullWorkspace
    {
        public doubleMxN U;
        public doubleN S;
        public doubleMxN V;
    ...
    public partial struct Arena
    {
        ...
        /// </summary>
        public doubleSvdFullWorkspace doubleSvdFullWorkspace(int m, int n)
        {
            return new doubleSvdFullWorkspace
            {
                U = doubleMat(m, n),
                S = doubleVec(n),
                V = doubleMat(n, n)
