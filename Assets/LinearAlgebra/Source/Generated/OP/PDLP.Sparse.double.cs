...
namespace LinearAlgebra
{
    public static partial class LP
    {
        ...
        static unsafe void pdlpEquilibrateBSR(in doubleBSR A, ref doubleN Dr, ref doubleN Dc)
        {
            ...
            double* dr = Dr.Data.Ptr;
            double* dc = Dc.Data.Ptr;

            var racc = new doubleN(m, Allocator.Temp);
            var cacc = new doubleN(n, Allocator.Temp);
            double* ra = racc.Data.Ptr;
            double* ca = cacc.Data.Ptr;
            long fsz = UnsafeUtility.SizeOf<double>();
                    ...
                    for (int k = rowPtr[bi]; k < rowPtr[bi + 1]; k++)
                    {
                        ...
                        for (int r = 0; r < BR; r++)
                        {
                            ...
                            for (int c = 0; c < BC; c++)
                            {
                                ...
                                if (v > (double)ra[gr]) ra[gr] = (double)v;
                            }
                        }
                    }
                for (int i = 0; i < m; i++) { double R = (double)dr[i] * (double)ra[i]; if (R > 1e-30) dr[i] = (double)((double)dr[i] / math.sqrt(R)); }
                    ...
                    for (int k = rowPtr[bi]; k < rowPtr[bi + 1]; k++)
                    {
                        ...
                        for (int r = 0; r < BR; r++)
                        {
                            ...
                            for (int c = 0; c < BC; c++)
                            {
                                ...
                                if (v > (double)ca[gc]) ca[gc] = (double)v;
                            }
                        }
                    }
        ...
        /// ℓ_v ≤ x ≤ u_v, with A a block-sparse <see cref="doubleBSR"/>. Use ±<c>1e30</c> in a bound to mean
        ...
        /// <see cref="pdlp(in doubleMxN, in doubleN, in doubleN, in doubleN, in doubleN, in doubleN, ref doubleN, out double, int, double)"/>.
        /// </summary>
        public static LPInfo pdlp(in doubleBSR A, in doubleN lc, in doubleN uc, in doubleN lv, in doubleN uv,
