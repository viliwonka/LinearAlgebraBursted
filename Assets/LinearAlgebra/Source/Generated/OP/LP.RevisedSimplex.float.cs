...
namespace LinearAlgebra
{
    ...
    // m x (n+m) tableau every pivot (simplexCore, LP.float.cs -- left untouched), this keeps the
    ...
    // file, reduced costs, ratios -- is ordinary float, exactly like every other solver in this library
    // (simplexCore, interiorCore). Tolerances are derived from the SAME per-dtype Consts the tableau
    // simplex already uses (Consts.floatZeroThreshold / floatEpsilon), which resolve to the float or
    ...
    // a helper returning float with no float-typed parameter (e.g. `float Tol()`) differs only in its
    ...
    // new shared float-returning helper needs at least one float-typed parameter to stay a genuine
    ...
    // convention (its phase-1 infeasibility sum) and this file's float entry point (the final cᵀx
    // recompute).
    //
    ...
    public static partial class LP
    {
        ...
        // floatLPCache warm-resume path (docs/spec-lpbasis-persistence.md, LP.DualSimplex.float.cs) is
                                                 ...
                                                 NativeArray<int> basis, NativeArray<byte> status)
        {
            ...
            // shared named helper) because a helper returning float with no float-typed parameter
            ...
            // duplicate members of the same partial class. Stage 2 (LP.DualSimplex.float.cs) computes
