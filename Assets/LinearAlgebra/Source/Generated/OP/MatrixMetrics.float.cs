...
namespace LinearAlgebra
{
    public static partial class Linear_OP {
        ...
        /// <see cref="floatColScaledOperator{TInner}"/> / <c>buildJacobiScale</c>) -- without ever
        /// forming AᵀA or running n transpose-matvecs.
        /// </summary>
        public static void columnNormsSquared(in floatMxN A, ref floatN d2)
        {
            if (d2.N != A.N_Cols)
        ...
        /// allocation. Pairs with <see cref="columnNormsSquared(in floatMxN, ref floatN)"/> (or
        /// its BSM sibling) to right-precondition a least-squares solve via
        /// <see cref="floatColScaledOperator{TInner}"/>: solve (A·diag(d)) y = b, then x = diag(d)·y,
        /// which equilibrates the normal-equation diagonal so ill-conditioned LS converges faster.
        /// </summary>
        public static void buildJacobiScale(in floatN colNorm2, ref floatN d)
        {
            if (d.N != colNorm2.N)
