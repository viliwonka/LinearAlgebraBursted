...
namespace LinearAlgebra
{
    public static partial class LQ {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static void lqKernel(ref floatMxN W, ref floatMxN L, ref floatMxN Q, ref floatN v, float zeroThreshold, bool reconstructQ)
        {
            int m = W.M_Rows;
            int n = W.N_Cols;
        ...
        //
        // LQ_BLOCK_MIN_M (Consts.floatLqBlockMinM -> float 256 / double 512) is a measured, not
        ...
        public static DirectSolveInfo decomp(in floatMxN A, ref floatMxN L, ref floatMxN Q)
        {
            ...
            const int LQ_BLOCK = 64;
            const int LQ_BLOCK_MIN_M = Consts.floatLqBlockMinM;   // float 256 / double 512 (see Consts)
        ...
        /// <summary>
        /// decomp using a reusable workspace (Arena.floatLQCache(m, n)) — zero-alloc.
        ...
        static void lqFactorInPlace(ref floatMxN W, ref floatMxN L, ref floatN v, float zeroThreshold)
        {
            const int LQ_BLOCK = 64;
            const int LQ_BLOCK_MIN_M = Consts.floatLqBlockMinM;   // float 256 / double 512 (see Consts)
            ...
            var Qnull = default(floatMxN);   // unused when reconstructQ is false
            if (m < LQ_BLOCK_MIN_M)
            {
        ...
        internal static unsafe void applyQtFromReflectors(ref floatMxN W, ref floatN y, ref floatN x)
        {
            ...
            for (int d = m - 1; d >= 0; d--)
            {
                float* vd = Wp + (long)d * n + d;   // reflector v_d over cols [d, n)
                float* xd = xp + d;
                int len = n - d;
                float dot = dot4(vd, xd, len);
        ...
        /// <summary>
        /// minNormSolve using a reusable workspace (Arena.floatLQMinNormCache(m, n)) —
        ...
        internal static unsafe void applyQtFromReflectors(ref floatMxN W, ref floatMxN Y, ref floatMxN X)
        {
            ...
            for (int j = 0; j < m; j++)
            {
                float* Xr = Xp + (long)j * k;
                float* Yr = Yp + (long)j * k;
                for (int c = 0; c < k; c++) Xr[c] = Yr[c];
            }
            for (int j = m; j < n; j++)
            {
                float* Xr = Xp + (long)j * k;
                for (int c = 0; c < k; c++) Xr[c] = (float)0;
            }
            ...
            for (int d = m - 1; d >= 0; d--)
            {
                float* vd = Wp + (long)d * n;   // reflector v_d over cols [d, n): vd[c] = W[d,c]
