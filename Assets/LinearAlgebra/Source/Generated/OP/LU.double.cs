...
namespace LinearAlgebra
{
    public static partial class LU {
        ...
        public static DirectSolveInfo decompNoPivot(in doubleMxN A, ref doubleMxN L, ref doubleMxN U)
        {
            if (!A.IsSquare)
        ...
        /// <param name="U">Output only; prior contents ignored; safe to allocate with uninit: true. Receives the upper-triangular factor.</param>
        public static DirectSolveInfo decomp(in doubleMxN A, ref doubleMxN L, ref doubleMxN U, ref Pivot P) {
            if (!A.IsSquare)
            ...
            if (m < LU_BLOCK_MIN_N) {
                ...
                for (int k = 0; k < m - 1; k++) {
                    ...
                    for(int r = k + 1; r < m; r++) {
                        double absValue = math.abs(U[r, k]);
        ...
        public static DirectSolveInfo decompInPlace(ref doubleMxN A_to_LU, ref Pivot P) {
            ...
            const int LU_BLOCK_MIN_N = Consts.doubleLuBlockMinN;
            ...
            unsafe
            {
                ...
                for (int k0 = 0; k0 < m - 1; k0 += LU_BLOCK) {
                    ...
                    if (rStart < m) {
                        ...
                        for (int r = 1; r < kb; r++) {
                            int Pr = P[k0 + r];
                            double* uR = up + (long)Pr * m + rStart;
                            for (int p = 0; p < r; p++) {
                                double Lrp = A_to_LU[Pr, k0 + p];
                                int Pp = P[k0 + p];
                                double* uP = up + (long)Pp * m + rStart;
                                UnsafeOP.axpy(uR, uP, -Lrp, ntrail);
                            }
                        }
                        ...
                        for (int t = 0; t < ntrail; t++) {
                            ...
                            int Pt = P[rStart + t];
                            double* crow = up + (long)Pt * m + rStart;

                            for (int p = 0; p < kb; p++) {
                                double Lip = A_to_LU[Pt, k0 + p];
                                double* wp = ubufp + (long)p * ntrail;
        ...
        /// <param name="A_to_LU">On entry A; on exit the compact LU factor (L below the diagonal, U on/above it).</param>
        /// <param name="b_to_x">On entry b; on exit the solution x.</param>
        public static DirectSolveInfo solveInPlace(ref doubleMxN A_to_LU, ref Pivot P, ref doubleN b_to_x) {
        ...
        /// </summary>
        /// <param name="b_to_x">On entry b; on exit the solution x.</param>
        public static DirectSolveInfo decompSolveTransA(ref doubleMxN LU, in Pivot pivot, ref doubleN b_to_x) {
        ...
        /// </summary>
        /// <param name="B_to_X">On entry B (n rows x k cols); on exit the solution X.</param>
        public static DirectSolveInfo decompSolveTransA(ref doubleMxN LU, in Pivot pivot, ref doubleMxN B_to_X) {
        ...
        /// <param name="A_to_LU">On entry A; on exit the compact LU factor (L below the diagonal, U on/above it).</param>
        /// <param name="b_to_x">On entry b; on exit the solution x.</param>
        public static DirectSolveInfo solveInPlaceTransA(ref doubleMxN A_to_LU, ref Pivot P, ref doubleN b_to_x) {
        ...
        /// <param name="A_to_LU">On entry A; on exit the compact LU factor.</param>
        /// <param name="B_to_X">On entry B (n rows x k cols); on exit the solution X.</param>
        public static DirectSolveInfo solveInPlaceTransA(ref doubleMxN A_to_LU, ref Pivot P, ref doubleMxN B_to_X) {
            if (!A_to_LU.IsSquare)
                throw new System.ArgumentException("solveInPlaceTransA: A_to_LU needs to be square");
