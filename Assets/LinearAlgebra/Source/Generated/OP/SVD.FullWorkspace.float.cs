...
namespace LinearAlgebra
{
    ...
    {
        /// <summary>Throws unless <paramref name="ws"/> matches Arena.floatSVDFull_WS(m, n) sizing.</summary>
        static void RequireSvdFullWorkspace(in floatSVDFull_WS ws, int m, int n)
        {
            if (ws.U.M_Rows != m || ws.U.N_Cols != n || ws.S.N != n || ws.V.M_Rows != n || ws.V.N_Cols != n)
                throw new ArgumentException("SVD: workspace must be sized for m x n (use Arena.floatSVDFull_WS(m, n))");
        }
    }
    ...
    /// ONCE via Arena.floatSVDFull_WS(m, n) and reuse across same-shape calls to avoid per-call temp
    /// allocations.
    ///
    ...
    public static partial class ArenaExtensions
    {
        ...
        /// <see cref="floatSVDFull_WS"/> for layout. Persistent in this arena; pass to the workspace
