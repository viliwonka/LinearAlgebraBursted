...
namespace LinearAlgebra
{
    public static partial class SVD {
        ...
        //
        // The workspace (doubleSVDTruncated_WS) bundles all scratch; allocate ONCE via
        // Arena.doubleSVDTruncated_WS(m, n, k, oversample) and reuse across same-shape calls.
                                        ...
                                        bool partialReorth, ref doubleSVDTruncated_WS ws, out bool converged)
        {
            ...
            unsafe
            {
                ...
                double seedNorm2 = Unsafe_OP.vecDot(v0, v0, n);
                if (seedNorm2 > (double)0)
                    Unsafe_OP.scalMul(v0, n, (double)1 / math.sqrt(seedNorm2));
                else
                    v0[0] = (double)1;
                ...
                if (partialReorth)
                {
                    ...
                    for (int j = 0; j < p; j++)
                    {
                        ...
                        if (j > 0 && ws.alpha[j] < gamma * ws.beta[j - 1])
                        {
                            ...
                            {
                                double t = Unsafe_OP.vecDot(UL_ptr + (j - 1) * m, uBuf_ptr, m);
                        ...
                        if (ws.beta[j] < gamma * ws.alpha[j])
                        {
                            ...
                            for (int it = 0; it < 4; it++)
                            {
                                double t = Unsafe_OP.vecDot(VL_ptr + j * n, vBuf_ptr, n);
