...
namespace LinearAlgebra
{
    ...
    public static partial class Eigen {
        ...
        /// loop — the concrete dense (<c>powerIteration(in floatMxN, ...)</c>) and BSM
        /// (<c>powerIteration(in floatBSM, ...)</c>) overloads below are thin forwarders that
        ...
        /// (<c>inversePowerIteration(in floatMxN, ...)</c>) and BSM
        /// (<c>inversePowerIteration(in floatBSM, ...)</c>) overloads below are thin forwarders
        /// that wrap their matrix in <see cref="floatDenseOperator"/> / <c>floatBSMOperator</c>
                                                 ...
                                                 float tol, int maxIter, int cgMaxIter, float cgTol)
        {
            ...
            floatN r  = v.tempfloatVec(A.M_Rows);
            floatN p  = v.tempfloatVec(A.M_Rows);
            floatN Ap = v.tempfloatVec(A.M_Rows);
        ...
        public static EigenSolveInfo inversePowerIteration(in floatBSM A, ref floatN v, ref floatN y,
                                                 ...
                                                 float tol, int maxIter, int cgMaxIter, float cgTol)
        {
            return inversePowerIteration(new floatBSMOperator(in A), ref v, ref y, ref r, ref p, ref Ap, out lambda, tol, maxIter, cgMaxIter, cgTol);
        }
        ...
        /// Lanczos loop -- the concrete dense (<c>lanczos(in floatMxN, ...)</c>) and BSM
        /// (<c>lanczos(in floatBSM, ...)</c>) overloads below are thin forwarders that wrap their
        /// matrix in <see cref="floatDenseOperator"/> / <c>floatBSMOperator</c> and call this
        /// method.
        ///
        ...
        /// <see cref="eigenvaluesSymmetric(ref floatMxN, ref floatN, ref floatEigenSym_WS)"/> on
        ...
        ///
        /// Workspace (see <see cref="floatLanczos_WS"/>, allocate via
        /// <c>Arena.floatLanczos_WS(A.Rows, steps)</c>): <c>ws.V</c> (steps x n) accumulates the
        ...
        static void lanczosTridiag<TOp>(in TOp A, ref floatLanczos_WS ws, out int produced,
                                        int steps, float breakdownTol)
            where TOp : struct, IfloatLinearOperator
        {
            if (A.Rows != A.Cols)
