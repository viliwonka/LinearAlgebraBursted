...
namespace LinearAlgebra
{
    public static partial class Eigen {
        ...
        /// <c>LinearAlgebra.Gallery.floatGallery.floatLaplacian1D</c> qualifies), then normalizes
        /// y into v.
        ///
        ...
        public static EigenSolveInfo inversePowerIteration(in floatMxN A, ref floatN v, ref floatN y,
                                                 ...
                                                 float tol, int maxIter, int cgMaxIter, float cgTol)
        {
            return inversePowerIteration(new floatDenseOperator(in A), ref v, ref y, ref r, ref p, ref Ap, out lambda, tol, maxIter, cgMaxIter, cgTol);
        }
        ...
        /// maxIter (1000), cgMaxIter (A.M_Rows) and cgTol (Consts.floatSqrtEps). tol defaults to
        /// a multiple of cgTol (NOT the much tighter Consts.floatZeroThreshold) on purpose: the
        ...
        public static EigenSolveInfo inversePowerIteration(in floatBSR A, ref floatN v, ref floatN y,
                                                 ...
                                                 float tol, int maxIter, int cgMaxIter, float cgTol)
        {
            return inversePowerIteration(new floatBSROperator(in A), ref v, ref y, ref r, ref p, ref Ap, out lambda, tol, maxIter, cgMaxIter, cgTol);
        }
        ...
        /// (10 * Consts.floatSqrtEps), maxIter (1000), cgMaxIter (A.M_Rows) and cgTol
        ...
        /// <see cref="valuesSymmetric(ref floatMxN, ref floatN, ref floatEigenSymCache)"/> on
        ...
        /// eigendecomposes T with <see cref="symmetric(ref floatMxN, ref floatN, ref floatMxN)"/>
        ...
        /// <see cref="symmetric(ref floatMxN, ref floatN, ref floatMxN)"/> allocates three
        ...
        public static bool decompInPlace(ref floatMxN A_to_D, ref floatN eigenvalues,
                                              ref floatMxN V, int maxSweeps, float eps)
        {
            ...
            for (int i = 0; i < n; i++) {
                for (int j = i + 1; j < n; j++) {
                    float aij = A_to_D[i, j];
                    float aji = A_to_D[j, i];
            ...
            for (int sweep = 0; sweep < maxSweeps; sweep++) {
                ...
                for (int p = 0; p < n - 1; p++) {
                    for (int q = p + 1; q < n; q++) {
                        ...
                        if (math.abs(apq) <= eps * (float)0.5 * (math.abs(A_to_D[p, p]) + math.abs(A_to_D[q, q])))
                            continue;

                        // Compute rotation angle: theta = (A[q,q] - A[p,p]) / (2 * A[p,q])
                        float theta = (A_to_D[q, q] - A_to_D[p, p]) / ((float)2 * apq);
                        ...
                        float app = A_to_D[p, p];
                        float aqq = A_to_D[q, q];
                        ...
                        for (int i = 0; i < n; i++) {
                            if (i == p || i == q)
                                continue;
                            float aip = A_to_D[i, p];
                            float aiq = A_to_D[i, q];
...
#pragma warning disable 618
        /// <summary>decompInPlace with default eps (Consts.floatZeroThreshold).</summary>
        [System.Obsolete("Prefer Eigen.symmetric (Householder tridiagonal + QL, ~30x faster) for symmetric eigenpairs, or Eigen.valuesSymmetric for eigenvalues only. This cyclic-Jacobi solver is retained for reference.", false)]
        public static bool decompInPlace(ref floatMxN A_to_D, ref floatN eigenvalues,
                                              ref floatMxN V, int maxSweeps)
            => decompInPlace(ref A_to_D, ref eigenvalues, ref V, maxSweeps, Consts.floatZeroThreshold);

        /// <summary>decompInPlace with default maxSweeps (30) and eps (Consts.floatZeroThreshold).</summary>
        [System.Obsolete("Prefer Eigen.symmetric (Householder tridiagonal + QL, ~30x faster) for symmetric eigenpairs, or Eigen.valuesSymmetric for eigenvalues only. This cyclic-Jacobi solver is retained for reference.", false)]
        public static bool decompInPlace(ref floatMxN A_to_D, ref floatN eigenvalues,
                                              ref floatMxN V)
            => decompInPlace(ref A_to_D, ref eigenvalues, ref V, 30, Consts.floatZeroThreshold);
#pragma warning restore 618
        ...
        /// </summary>
        public static bool valuesSymmetric(ref floatMxN A, ref floatN eigenvalues, int maxIterPerEig, float eps,
        ...
        /// <summary>valuesSymmetric (ref workspace) with default maxIterPerEig (30) and eps (Consts.floatZeroThreshold).</summary>
        public static bool valuesSymmetric(ref floatMxN A, ref floatN eigenvalues, ref floatEigenSymCache ws)
            => valuesSymmetric(ref A, ref eigenvalues, 30, Consts.floatZeroThreshold, ref ws);
        ...
        /// <summary>valuesSymmetric with default maxIterPerEig (30) and eps (Consts.floatZeroThreshold).</summary>
        public static bool valuesSymmetric(ref floatMxN A, ref floatN eigenvalues)
            => valuesSymmetric(ref A, ref eigenvalues, 30, Consts.floatZeroThreshold);
        ...
        /// </summary>
        public static bool symmetric(ref floatMxN A, ref floatN eigenvalues, ref floatMxN V,
        ...
        /// <summary>symmetric with default maxIterPerEig (30) and eps (Consts.floatZeroThreshold).</summary>
        public static bool symmetric(ref floatMxN A, ref floatN eigenvalues, ref floatMxN V)
            => symmetric(ref A, ref eigenvalues, ref V, 30, Consts.floatZeroThreshold);
        ...
        /// </summary>
        public static unsafe bool valuesQR(ref floatMxN A, ref floatN eigenvaluesReal,
        ...
        public static bool valuesQR(ref floatMxN A, ref floatN eigenvaluesReal,
