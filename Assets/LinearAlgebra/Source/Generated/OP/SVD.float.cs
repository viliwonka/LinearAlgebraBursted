                ...
                for (int sweep = 0; sweep < maxSweeps; sweep++) {
                    ...
                    for (int p = 0; p < n - 1; p++) {
                        for (int q = p + 1; q < n; q++) {
                            ...
                            Unsafe_OP.gram2x2(rowPU, rowQU, out float alpha, out float beta, out float gamma, m);
...
#pragma warning disable 618
        /// <summary>svdDecomposition with default eps (Consts.floatZeroThreshold).</summary>
        [System.Obsolete("Prefer SVD.svdThin (Golub-Kahan, ~3x faster) for the full SVD, or SVD.svdValues for singular values only. This one-sided Jacobi SVD is retained for reference.", false)]
        public static bool svdDecomposition(ref floatMxN U, ref floatN S, ref floatMxN V,
                                            int maxSweeps)
            => svdDecomposition(ref U, ref S, ref V, maxSweeps, Consts.floatZeroThreshold);

        /// <summary>svdDecomposition with default maxSweeps (30) and eps (Consts.floatZeroThreshold).</summary>
        [System.Obsolete("Prefer SVD.svdThin (Golub-Kahan, ~3x faster) for the full SVD, or SVD.svdValues for singular values only. This one-sided Jacobi SVD is retained for reference.", false)]
        public static bool svdDecomposition(ref floatMxN U, ref floatN S, ref floatMxN V)
            => svdDecomposition(ref U, ref S, ref V, 30, Consts.floatZeroThreshold);
#pragma warning restore 618
        ...
        /// <summary>svdValues with default maxIter (75) and eps (Consts.floatZeroThreshold).</summary>
        public static bool svdValues(in floatMxN A, ref floatN S)
            => svdValues(in A, ref S, 75, Consts.floatZeroThreshold);

        /// <summary>
        /// svdValues using a reusable workspace (Arena.floatSVDValuesCache(m, n)) — zero-alloc.
        /// Semantics identical to the allocating overload; see that one for full documentation.
        /// </summary>
        public static bool svdValues(in floatMxN A, ref floatN S, ref floatSVDValuesCache ws,
        ...
        /// <summary>svdThin with default eps (Consts.floatZeroThreshold).</summary>
        public static bool svdThin(in floatMxN A, ref floatMxN U, ref floatN S, ref floatMxN V,
                                   int maxIter)
            => svdThin(in A, ref U, ref S, ref V, maxIter, Consts.floatZeroThreshold);
        ...
        /// <summary>
        /// svdThin using a reusable workspace (Arena.floatSVDThinCache(m, n)) — zero-alloc (including
