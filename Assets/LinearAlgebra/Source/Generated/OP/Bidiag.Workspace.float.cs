...
namespace LinearAlgebra
{
    public static partial class Bidiag
    {
        ...
        /// <see cref="bidiagonalizeValues"/>. Matches Arena.floatBidiagWorkspace(m, n).
        /// </summary>
        static void RequireBidiagWorkspace(in floatBidiagWorkspace ws, int m, int n, bool needLeftU, string who)
        {
            bool ok =
            ...
            if (!ok)
                throw new ArgumentException(who + ": workspace must be sized for m x n (use Arena.floatBidiagWorkspace(m, n))");
        }
    }

    /// <summary>
    /// Reusable scratch for Golub-Kahan-Householder bidiagonalization (Bidiag.bidiagonalize /
    /// bidiagonalizeValues). Allocate ONCE via Arena.floatBidiagWorkspace(m, n) and reuse it across
    ...
    /// </summary>
    public struct floatBidiagWorkspace
    {
        public floatMxN W;
    ...
    public partial struct Arena
    {
        ...
        /// </summary>
        public floatBidiagWorkspace floatBidiagWorkspace(int m, int n)
        {
            return new floatBidiagWorkspace
            {
                W = floatMat(m, n),
                leftU = floatMat(m, n),
                uVec = floatVec(m),
