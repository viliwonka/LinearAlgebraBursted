...
namespace LinearAlgebra
{
    ...
    /// float-only (float / double generated variants); use the existing integer ops for
    /// index-space resampling.
    /// </summary>
    public static partial class floatResample_OP
    {
        // ---- private helpers ----
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static float sampleRowAt(in floatMxN m, int row, float pos, Interp interp, EdgeMode edge)
        {
            int nCols = m.N_Cols;
            switch (interp)
            {
                ...
                case Interp.Linear:
                {
                    ...
                    float v0 = m[row, idx(i0,     nCols, edge)];
                    float v1 = m[row, idx(i0 + 1, nCols, edge)];
                ...
                default: // Interp.Cubic — Catmull-Rom
                {
                    ...
                    float p0 = m[row, idx(i0 - 1, nCols, edge)];
                    float p1 = m[row, idx(i0,     nCols, edge)];
                    float p2 = m[row, idx(i0 + 1, nCols, edge)];
                    float p3 = m[row, idx(i0 + 2, nCols, edge)];
                    return (float)0.5 * (
                        (float)2 * p1
                        + (-p0 + p2) * t
                        + ((float)2 * p0 - (float)5 * p1 + (float)4 * p2 - p3) * t * t
                        + (-p0 + (float)3 * p1 - (float)3 * p2 + p3) * t * t * t);
                }
            }
        }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static float sampleColAt(in floatMxN m, int col, float pos, Interp interp, EdgeMode edge)
        {
            int nRows = m.M_Rows;
            switch (interp)
            {
                ...
                case Interp.Linear:
                {
                    int i0 = (int)math.floor(pos);
                    float frac = pos - (float)i0;
                    float v0 = m[idx(i0,     nRows, edge), col];
                    float v1 = m[idx(i0 + 1, nRows, edge), col];
                    return math.lerp(v0, v1, frac);
                }
                default: // Interp.Cubic — Catmull-Rom
                {
                    int i0 = (int)math.floor(pos);
                    float t  = pos - (float)i0;
                    float p0 = m[idx(i0 - 1, nRows, edge), col];
                    float p1 = m[idx(i0,     nRows, edge), col];
                    float p2 = m[idx(i0 + 1, nRows, edge), col];
                    float p3 = m[idx(i0 + 2, nRows, edge), col];
                    return (float)0.5 * (
                        (float)2 * p1
                        + (-p0 + p2) * t
                        + ((float)2 * p0 - (float)5 * p1 + (float)4 * p2 - p3) * t * t
        ...
        /// data.N must be >= 1.
        /// </summary>
        public static float sampleAt(in floatN data, float pos, Interp interp, EdgeMode edge)
        {
            if (data.N < 1)
            ...
            switch (interp)
            {
                ...
                case Interp.Linear:
                {
                    int i0 = (int)math.floor(pos);
                    float frac = pos - (float)i0;
                    float v0 = data[idx(i0,     n, edge)];
                    float v1 = data[idx(i0 + 1, n, edge)];
                    return math.lerp(v0, v1, frac);
                }
                default: // Interp.Cubic — Catmull-Rom
                {
                    int i0 = (int)math.floor(pos);
                    float t  = pos - (float)i0;
                    float p0 = data[idx(i0 - 1, n, edge)];
                    float p1 = data[idx(i0,     n, edge)];
                    float p2 = data[idx(i0 + 1, n, edge)];
                    float p3 = data[idx(i0 + 2, n, edge)];
                    return (float)0.5 * (
                        (float)2 * p1
                        + (-p0 + p2) * t
                        + ((float)2 * p0 - (float)5 * p1 + (float)4 * p2 - p3) * t * t
        ...
        /// </summary>
        public static void sampleAtInto(in floatN data, in floatN positions, ref floatN dest,
        ...
        /// </summary>
        public static void resampleInto(in floatN src, ref floatN dst, Interp interp, EdgeMode edge)
        {
            if (src.N < 1)
        ...
        /// </summary>
        public static void resample2DInto(in floatMxN src, ref floatMxN dst, Interp interp, EdgeMode edge)
        {
            ...
            var scratch = new floatMxN(srcM, dstN, Allocator.Temp);

            // ---- Pass 1: Horizontal (columns within each row, srcN → dstN) ----
            float hScale = dstN > 1 ? (float)(srcN - 1) / (float)(dstN - 1) : (float)0;
            ...
            float vScale = dstM > 1 ? (float)(srcM - 1) / (float)(dstM - 1) : (float)0;
