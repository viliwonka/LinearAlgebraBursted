...
namespace LinearAlgebra
{
    ...
    // (simplexCore, LP.double.cs -- left untouched), this keeps the ORIGINAL constraint matrix and an
    ...
    //
    // Every kernel function below is `internal static` so LP.DualSimplex.double.cs (stage 2) can call
    ...
    // via a shared helper: an double-returning helper with no double-typed parameter would differ only
    ...
    public static partial class LP
    {
        ...
        // LP.DualSimplex.double.cs routes through MTmul, which reshapes the sweep into
        // UnsafeOP.vecMatDot's row-major form (i outer, j inner unit-stride: outv[j] += v[i]*M[i,j],
        // zeroed first) -- mirrors LP.InteriorPoint.double.cs's ATmul (kept as an independent copy
        ...
        // doubleLPCache warm-resume path (LP.DualSimplex.double.cs) is the one caller that can arrive
