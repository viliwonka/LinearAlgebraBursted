...
namespace LinearAlgebra
{
    ...
    public static partial class Select
    {
        ...
        public static void select(in longN a, in longN b, in boolN c, ref longN dest)
        {
            ...
            unsafe
            {
                UnsafeSelectOP.selectlong(a.Data.Ptr, b.Data.Ptr, c.Data.Ptr, dest.Data.Ptr, a.N);
            }
        }

        public static longN select(in longN a, in longN b, in boolN c)
        {
            longN res = a.longTempVec(a.N, true);
            select(in a, in b, in c, ref res);
            return res;
        }

        // ref-dest primitive. No alias guard: elementwise op.
        public static void select(in longMxN a, in longMxN b, in boolMxN c, ref longMxN dest)
        {
            Assume.SameDim(in a, in b);
            Assume.SameDim(in a, in c);
            ...
            unsafe
            {
                UnsafeSelectOP.selectlong(a.Data.Ptr, b.Data.Ptr, c.Data.Ptr, dest.Data.Ptr, a.M_Rows * a.N_Cols);
            }
        }

        public static longMxN select(in longMxN a, in longMxN b, in boolMxN c)
        {
            longMxN res = a.longTempMat(a.M_Rows, a.N_Cols, true);
            select(in a, in b, in c, ref res);
            return res;
        }

        // ref-dest primitive. No alias guard: scalar bool selects the whole source unchanged.
        public static void select(in longN a, in longN b, in bool c, ref longN dest)
        {
            Assume.SameDim(in a, in b);
        ...
        public static longN select(in longN a, in longN b, in bool c)
        {
            return c ? b.TempCopy() : a.TempCopy();
        }

        // Matrix analog of the scalar-bool vector overload above (same no-alias reasoning).
        public static void select(in longMxN a, in longMxN b, in bool c, ref longMxN dest)
        {
            Assume.SameDim(in a, in b);
        ...
        public static longMxN select(in longMxN a, in longMxN b, in bool c)
        {
            return c ? b.TempCopy() : a.TempCopy();
        }
    }
...
namespace LinearAlgebra.Internal
{
    ...
    {
        public static void selectlong([NoAlias] long* a, [NoAlias] long* b, [NoAlias] bool* c, long* target, int n)
        {
            for (int i = 0; i < n; i++)
