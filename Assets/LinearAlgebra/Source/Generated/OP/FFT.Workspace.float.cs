...
{
    /// <summary>
    /// Precomputed twiddle table for floatFFT_OP. One size-n table serves every radix-2 transform of
    ...
    /// Arena.floatFFT_WS(n) and reuse across many transforms of the same size.
    ///
    ...
    public struct floatFFT_WS
    {
        ...
        public floatN cz;         // length n/2: even-sample packing scratch for rfft/irfft
    ...
    public static partial class ArenaExtensions
    {
        ...
        /// drift); see <see cref="floatFFT_WS"/> for the table layout and full-circle-table rationale.
        ///
    ...
    public static partial class floatFFT_OP
    {
        ...
        /// produced by Arena.floatFFT_WS(n): ws.n == n and table lengths == n/2.
        /// </summary>
        static void RequireFftWorkspace(in floatFFT_WS ws, int n, string who)
        {
            if (ws.n != n || ws.twRe.N != n >> 1 || ws.twIm.N != n >> 1 ||
                ws.cz.N != n >> 1 || ws.sz.N != n >> 1 || ws.visited.N != n)
                throw new ArgumentException(
                    who + ": workspace must be sized for an n-point FFT (use Arena.floatFFT_WS(n))");
        }
        ...
        /// </summary>
        static void RequireRadix4Workspace(in floatFFT_WS ws, int n, string who)
        {
            RequireFftWorkspace(in ws, n, who);
            if (ws.twReFull.N != n || ws.twImFull.N != n)
                throw new ArgumentException(
                    who + ": workspace must have full-circle twiddle table (use Arena.floatFFT_WS(n))");
        }
        ...
        /// ws must be sized for re.N (build via Arena.floatFFT_WS(N)); it must contain the
        ...
        /// </summary>
        public static void fft(ref floatN re, ref floatN im, in floatFFT_WS ws)
        {
            int n = re.N;
        ...
        /// </summary>
        public static void ifft(ref floatN re, ref floatN im, in floatFFT_WS ws)
        {
            int n = re.N;
        ...
        /// </summary>
        public static void rfft(in floatN real, ref floatN re, ref floatN im, in floatFFT_WS ws)
        {
            int n = real.N;
            ...
            var cz = ws.cz;   // floatN of length n/2 = M
            var sz = ws.sz;   // floatN of length n/2 = M
            for (int j = 0; j < M; j++)
            {
                cz[j] = real[2 * j];
                sz[j] = real[2 * j + 1];
            ...
            for (int k = 1; k < M; k++)
            {
                ...
                float curRe = twRe[k];   // W_N^k real part (half-table)
                float curIm = twIm[k];   // W_N^k imaginary part (half-table)
        ...
        /// </summary>
        public static void irfft(in floatN re, in floatN im, ref floatN real, in floatFFT_WS ws)
        {
            int halfSpec = re.N;
            if (im.N != halfSpec)
        ...
        static unsafe void FftCoreRadix4Ptr(
            ...
            [NoAlias] float* twr, [NoAlias] float* twi, int n, int tableN)
        {
        ...
        //
        // In-place implementation — no floatN Temp allocations.
        ...
        // visited is a workspace scratch of length >= size (0/1 flags, float-typed).
                                              ...
                                              int tableN, bool inverse)
        {
            ...
            {
                if (visited[s] != (float)0) continue;
                visited[s] = (float)1;

                int j = (s & 1) == 0 ? (s >> 1) : (M + (s >> 1));
                if (j == s) continue;   // fixed point (s==0 or s==size-1)

                float carryRe = re[s], carryIm = im[s];
                while (j != s)
                {
                    float tmpRe = re[j], tmpIm = im[j];
                    re[j] = carryRe;
                    im[j] = carryIm;
                    visited[j] = (float)1;
                    carryRe = tmpRe;
            ...
            for (int k = 0; k < M; k++)
            {
                float wr = twReFull[k * combineStep];
                float wi = twImFull[k * combineStep];
                float er  = re[k],     ei  = im[k];
                float or_ = re[M + k], oi_ = im[M + k];
