...
namespace LinearAlgebra
{
    ...
    /// Arena.floatFFTCache(n) and reuse across many transforms of the same size.
    ///
    ...
    public static partial class ArenaExtensions
    {
        ...
        /// drift); see <see cref="floatFFTCache"/> for the table layout and full-circle-table rationale.
        ///
    ...
    public static partial class floatFFT_OP
    {
        ...
        /// produced by Arena.floatFFTCache(n): ws.n == n and table lengths == n/2.
        /// </summary>
        static void RequireFftWorkspace(in floatFFTCache ws, int n, string who)
        {
            if (ws.n != n || ws.twRe.N != n >> 1 || ws.twIm.N != n >> 1 ||
                ws.cz.N != n >> 1 || ws.sz.N != n >> 1 || ws.visited.N != n)
                throw new ArgumentException(
                    who + ": workspace must be sized for an n-point FFT (use Arena.floatFFTCache(n))");
        }
        ...
        /// </summary>
        static void RequireRadix4Workspace(in floatFFTCache ws, int n, string who)
        {
            RequireFftWorkspace(in ws, n, who);
            if (ws.twReFull.N != n || ws.twImFull.N != n)
                throw new ArgumentException(
                    who + ": workspace must have full-circle twiddle table (use Arena.floatFFTCache(n))");
        }
        ...
        /// ws must be sized for re.N (build via Arena.floatFFTCache(N)); it must contain the
        ...
        /// </summary>
        public static void irfft(in floatN re, in floatN im, ref floatN real, in floatFFTCache ws)
        {
            int halfSpec = re.N;
            if (im.N != halfSpec)
