...
{
    /// <summary>
    /// Warm-solve factor/weight persistence cache for <see cref="LP.solve(in doubleMxN, in doubleN, in doubleN, in NativeArray{ConstraintSense}, ref doubleN, out double, ref LPBasis, ref doubleLPCache, int)"/>
    ...
    /// separate solve calls, not arena-scoped). <c>new doubleLPCache(n, m, allocator)</c> +
    ...
    public struct doubleLPCache
    {
        ...
        /// </summary>
        public doubleLPCache(int n, int m, Allocator allocator)
        {
            int N = n + m;
            M = new doubleMxN(m, N, allocator);
            rhs = new doubleN(m, allocator);
            lower = new doubleN(N, allocator);
            upper = new doubleN(N, allocator);
            cost = new doubleN(N, allocator);
            B = new doubleMxN(m, m, allocator);
            P = new Pivot(m, allocator);
            etaAlpha = new doubleMxN(LP.REFACTOR_INTERVAL, m, allocator);
            etaRow = new NativeArray<int>(LP.REFACTOR_INTERVAL, allocator);
            etaCount = 0;
            weight = new doubleN(m, allocator);
            matrixVersion = 0;
