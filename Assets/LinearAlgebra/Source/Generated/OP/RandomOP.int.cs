...
namespace LinearAlgebra
{
    ...
    public static partial class Rand
    {
        ...
        public static void nextUniformInPlace(ref Random rng, ref intN dest, int min, int max)
        {
        ...
        /// <summary>
        /// Matrix overload of <see cref="nextUniformInPlace(ref Random, ref intN, int, int)"/>;
        /// advances <paramref name="rng"/> by <c>dest.Length</c> steps.
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static void nextUniformInPlace(ref Random rng, ref intMxN dest, int min, int max)
        {
            if (min < int.MinValue || max > int.MaxValue)
                throw new ArgumentException("Rand: min/max must be within int range for random generation");
            if (!(min <= max))
