...
namespace LinearAlgebra
{
    ...
    public static partial class CHOP {
        ...
        public static RankInfo decomp(in doubleMxN A, ref doubleMxN L, ref Pivot P,
            ...
            const int CHOLP_BLOCK_MIN_N = Consts.doubleCholPivotBlockMinN;
            ...
            unsafe {
                ...
                var dotBuf = new doubleN(n, Allocator.Temp, false);
                double* dotp = dotBuf.Data.Ptr;
                ...
                var QT = new doubleN(CHOLP_BLOCK * n, Allocator.Temp, false);
                double* qtp = QT.Data.Ptr;
                ...
                for (int j0 = 0; j0 < n; j0 += CHOLP_BLOCK) {
                    ...
                    for (int k = j0; k < panelEnd; k++) {
                        ...
                        if (k > j0) {
                            double* prevRow = wp + (long)(k - 1) * n;
                            for (int i = k; i < n; i++) dotp[i] += prevRow[i] * prevRow[i];
                        }

                        int q = k;
                        double maxDiag = W[k, k] - dotp[k];
                        double minDiag = maxDiag;
                        for (int i = k + 1; i < n; i++) {
                            double d = W[i, i] - dotp[i];
                        ...
                        // explicit write.
                        if (q != k) {
                            double demoted = W[k, k];
                            for (int i = 0; i < k; i++)     { double t = W[i, k]; W[i, k] = W[i, q]; W[i, q] = t; }
                            for (int j = q + 1; j < n; j++) { double t = W[k, j]; W[k, j] = W[q, j]; W[q, j] = t; }
                            for (int m = k + 1; m < q; m++) { double t = W[k, m]; W[k, m] = W[m, q]; W[m, q] = t; }
                            W[q, q] = demoted;
                            { double t = dotp[k]; dotp[k] = dotp[q]; dotp[q] = t; }
                            Swap.Rows(ref L, k, q, 0, k);    // permute the already-computed factor rows
                            P.Swap(k, q);
                        }

                        double Ukk = math.sqrt(maxDiag);
                        L[k, k] = Ukk;
                    ...
                    if (rStart < n) {
                        int ntrail = n - rStart;
                        for (int p = 0; p < jb; p++) {
                            double* rowp = wp + (long)(j0 + p) * n + rStart;
        ...
        public static unsafe DirectSolveInfo decompSolve(ref doubleMxN L, in Pivot P, int rank, ref doubleMxN B_to_X) {
            if (!L.IsSquare)
                throw new ArgumentException("decompSolve: L must be square");
            ...
            // X = A⁺B = 0 for the zero matrix.
            if (rank == 0) {
                double* Xz = B_to_X.Data.Ptr;
                for (long e = 0; e < (long)n * k; e++) Xz[e] = (double)0;
                return new DirectSolveInfo { status = DirectSolveStatus.Success };
            }

            // B̃[i,:] = B[P[i],:] (apply the symmetric permutation to the RHS rows).
            var Bt = new doubleMxN(n, k, Allocator.Temp, false);
            {
                double* Btp = Bt.Data.Ptr;
                double* Xp = B_to_X.Data.Ptr;
                for (int i = 0; i < n; i++)
                {
                    double* dst = Btp + (long)i * k;
                    double* src = Xp + (long)P[i] * k;
            ...
            if (rank == n) {
                ...
                for (int i = 0; i < n; i++)
                {
                    double* dst = Xp + (long)P[i] * k;
                    double* src = Btp + (long)i * k;
                    for (int c = 0; c < k; c++) dst[c] = src[c];
                }
                Bt.Dispose();
                return new DirectSolveInfo { status = DirectSolveStatus.Success };
            ...
            var Grhs = new doubleMxN(r, k, Allocator.Temp, false);
            {
                double* Gp = Grhs.Data.Ptr;
                double* Btp = Bt.Data.Ptr;
                for (int c = 0; c < r; c++)
                {
                    double* Grow = Gp + (long)c * k;
                    for (int i = c; i < n; i++)
            ...
            {
                double* Zp = Grhs.Data.Ptr;
                double* Xp = B_to_X.Data.Ptr;
                for (int i = 0; i < n; i++)
                {
                    double* dst = Xp + (long)P[i] * k;
                    for (int c = 0; c < k; c++) dst[c] = (double)0;
        ...
        /// <param name="A_to_L">On entry A; on exit the lower-triangular factor L.</param>
        /// <param name="B_to_X">On entry B (n rows x k cols); on exit the solution X.</param>
        public static RankInfo solveInPlace(ref doubleMxN A_to_L, ref Pivot P, ref doubleMxN B_to_X) {
            if (!A_to_L.IsSquare)
                throw new ArgumentException("solveInPlace: A_to_L needs to be square");
