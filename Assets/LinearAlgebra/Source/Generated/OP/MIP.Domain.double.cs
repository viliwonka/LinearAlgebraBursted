...
{
    // Bound-change undo stack for MIP's search (MIP.double.cs's SearchCore): push a tightening, undo
    // back to a marker. Used for the current plunge's dive step, throwaway strong-branch trials
    // (MIP.Pseudocost.double.cs), and stage 4's PropagateFixpoint below, which pushes every tightening
    ...
    // rhs internally (see MIP.double.cs's header for the shift).
    //
    ...
    internal struct doubleBoundChange
    {
    ...
    public static partial class MIP
    {
                                             ...
                                             NativeArray<byte> integrality, int n)
        {
            for (int j = 0; j < n; j++)
            {
                ...
                Aaug[rowUB[j], col[j]] = hiFinite ? (double)1 : (double)0;
                bAug[rowUB[j]] = hiFinite ? (U[j] - xlRoot[j]) : (double)0;
            }
        }
        ...
        /// was reached).</returns>
        internal static bool PropagateFixpoint(doubleMxN A, doubleN b, NativeArray<ConstraintSense> senses,
