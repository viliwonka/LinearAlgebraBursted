...
namespace LinearAlgebra
{
    ...
    /// IdoubleScalarFunction functor), convolution kernels, DSP windows, and rank-1 (1D×1D)
    ...
    public static partial class doubleGenOP
    {
        ...
        /// </summary>
        public static void linspace(ref doubleN dest, double a, double b)
        {
            int N = dest.N;
            if (N == 0)
        ...
        public static void arange(ref doubleN dest, double start, double step)
        {
            int N = dest.N;
            if (N == 0)
        ...
        /// Works with the built-in <c>doubleEasing</c>/<c>doubleWave</c> functors or a caller's own struct.
        /// </summary>
        public static void sample<F>(ref F f, ref doubleN dest, double t0, double t1)
        ...
        public static void sample<F>(ref F f, ref doubleN dest)
            where F : struct, IdoubleScalarFunction
            => sample(ref f, ref dest, (double)0, (double)1);
        ...
        /// </summary>
        public static void gaussianKernel(ref doubleN dest, double sigma)
        {
            int N = dest.N;
            if (N == 0)
            ...
            double c = (double)(N - 1) * (double)0.5;
            double inv2s2 = (double)1 / ((double)2 * sigma * sigma);
        ...
        /// N==1 yields {1}.
        /// </summary>
        public static void tentKernel(ref doubleN dest)
        {
            int N = dest.N;
            if (N == 0)
            ...
            for (int i = 0; i < N; i++)
            {
                double w = (c + (double)1) - math.abs((double)i - c);
                dest[i] = w;
                sum += w;
            }
        ...
        /// </summary>
        public static void window(ref doubleN dest, WindowType type)
        {
            int N = dest.N;
            if (N == 0)
            ...
            if (N == 1) { dest[0] = (double)1; return; }

            double twoPiOverNm1 = (double)(2.0 * System.Math.PI) / (double)(N - 1);
            double fourPiOverNm1 = (double)(4.0 * System.Math.PI) / (double)(N - 1);
            ...
            for (int i = 0; i < N; i++)
            {
                ...
                switch (type)
                {
                    case WindowType.Hann:
                        w = (double)0.5 * ((double)1 - math.cos(twoPiOverNm1 * i));
                        break;
                    case WindowType.Hamming:
                        w = (double)0.54 - (double)0.46 * math.cos(twoPiOverNm1 * i);
                        break;
                    case WindowType.Blackman:
                        w = (double)0.42
                            - (double)0.5 * math.cos(twoPiOverNm1 * i)
                            + (double)0.08 * math.cos(fourPiOverNm1 * i);
                        break;
                    default:
                        w = (double)1;
                        break;
        ...
        /// </summary>
        public static void gaussianKernel2D(ref doubleMxN dest, double sigma)
        {
