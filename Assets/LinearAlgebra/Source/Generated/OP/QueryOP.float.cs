...
namespace LinearAlgebra
{
    ...
    public static partial class floatQueryOP
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static float WorstScoreForNearest(Metric m)
            => IsSimilarityMetric(m) ? float.MinValue : float.MaxValue;

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static float WorstScoreForFarthest(Metric m)
            => IsSimilarityMetric(m) ? float.MaxValue : float.MinValue;

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static bool IsBetterForNearest(float a, float b, Metric m)
            => IsSimilarityMetric(m) ? a > b : a < b;

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static bool IsBetterForFarthest(float a, float b, Metric m)
            => IsSimilarityMetric(m) ? a < b : a > b;
        ...
        public static void farthestRow(in floatMxN A, in floatN q, Metric m, out int index, out float score)
        {
            ...
            float worst = WorstScoreForFarthest(m);
            int worstIdx = 0;
            for (int r = 0; r < A.M_Rows; r++)
            {
                float s = RowScore(in A, r, in q, m);
