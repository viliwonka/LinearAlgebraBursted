...
namespace LinearAlgebra
{
    public static partial class LP
    {
        ...
        // (LP.ladFN, LP.FrischNewton.double.cs) -- see docs/spec-lad-barrodale-roberts.md.
        //
        // ---- Source + verification ----
        ...
        // worked for LP.FrischNewton.double.cs's source). Cross-checked against the R wrapper
        ...
        //    simplex's own ratio test (LP.double.cs's RatioTest: max(Consts.doubleZeroThreshold,
        ...
        /// <see cref="ladFN(in doubleMxN, in doubleN, ref doubleN, out double, int)"/>: a primal
        ...
        public static LPInfo ladBR(in doubleMxN A, in doubleN b, ref doubleN x, out double objective, int maxIter = 0)
        {
        ...
        /// <see cref="ladBR(in doubleMxN, in doubleN, ref doubleN, out double, int)"/>). Exact and
        ...
        public static LPInfo ladBR(in doubleMxN A, in doubleN b, double tau, ref doubleN x,
        ...
        // supported entry.
        internal static LPInfo ladBarrodaleRobertsCore(in doubleMxN A, in doubleN b, double tau,
                                                        ref doubleN x, out double objective, int maxIter)
        {
            ...
            var candRatio = new doubleN(m, Allocator.Temp);
            ...
            for (int j = 0; j < n; j++)
            {
                ...
                costT *= 2.0;
                cost[j] = (double)(costConst + costT * tau);
            }
            ...
            while (true)
            {
                ...
                else
                {
                    ...
                    {
                        for (int i = 0; i < m; i++) T[i, enter] = -T[i, enter];
                        cost[enter] = -cost[enter] - (double)2;
                        colTag[enter] = -colTag[enter];
                    }
                }
                ...
                // exercises for BR (<=192, per LPTests.double.cs's LadBRvsOracleM192/LadBRStackloss/
                ...
                if (leave < 0)
                {
                    if (stage)
                    {
                        ...
                        for (int i = 0; i < m; i++) { double tmp = T[i, kr]; T[i, kr] = T[i, enter]; T[i, enter] = tmp; }
                        double tc = cost[kr]; cost[kr] = cost[enter]; cost[enter] = tc;
                ...
                else
                {
                    ...
                    if (stage)
                    {
                        if (leave != kount - 1)
                        {
                            for (int j = kr; j < n; j++) { double tmp = T[leave, j]; T[leave, j] = T[kount - 1, j]; T[kount - 1, j] = tmp; }
                            double tr = rhs[leave]; rhs[leave] = rhs[kount - 1]; rhs[kount - 1] = tr;
            ...
            for (int i = 0; i < kl; i++)
            {
                ...
                double sgn = rowTag[i] < 0 ? -(double)1 : (double)1;
                x[k] = rhs[i] * sgn;
            }
        ...
        static bool TryPivotOrFold(doubleMxN T, doubleN rhs, doubleN cost, NativeArray<int> rowTag,
                                   int n, int kr, int enter, int cand, double pivTol)
        {
            double pivot = T[cand, enter];
        ...
        static unsafe void BRPivot(doubleMxN T, doubleN rhs, doubleN cost, NativeArray<int> rowTag, NativeArray<int> colTag,
                            int m, int n, int kr, int leave, int enter)
        {
            double pivot = T[leave, enter];
            double* Tp = T.Data.Ptr;
            double* leaveRow = Tp + (long)leave * n;
            ...
            double dCost = cost[enter];
            double* costP = cost.Data.Ptr;
