...
namespace LinearAlgebra.Internal
{
    public static unsafe partial class UnsafeOP {
        ...
        public static void matVecDot([NoAlias] float* mat, [NoAlias] float* x, [NoAlias] float* y, int m, int n)
        {
            ...
            // this kernel uses two explicit, independent width-4 SIMD accumulators (float4 ->
        ...
        [MethodImpl(MethodImplOptions.NoInlining)]
        public static void syrkUpperSub([NoAlias] float* Wp, int n, int rStart, int j0, int jb, [NoAlias] float* QT)
        {
            int ntrail = n - rStart;
            for (int ip = 0; ip < ntrail; ip++)
            {
                int i = rStart + ip;
                float* Wrow = Wp + (long)i * n;
                float* Qi = QT + (long)ip * jb;
                for (int p = 0; p < jb; p++)
                {
                    float temp = Qi[p];
                    float* Up = Wp + (long)(j0 + p) * n + rStart;
        ...
        // ratio-test scan (LP.BarrodaleRoberts.float.cs), which sorts once up front (O(k log k)) and
        ...
        // LP.BarrodaleRoberts.float.cs's own gate for the rationale.
        //
