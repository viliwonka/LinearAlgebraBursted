...
{
    // intQuery_OP: integer-exact search & selection inside integer vectors / matrices.
    ...
    public static partial class intQuery_OP
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static int WorstScoreForNearest(Metric m)
            => IsSimilarityMetric(m) ? int.MinValue : int.MaxValue;

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static int WorstScoreForFarthest(Metric m)
            => IsSimilarityMetric(m) ? int.MaxValue : int.MinValue;

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static bool IsBetterForNearest(int a, int b, Metric m)
            => IsSimilarityMetric(m) ? a > b : a < b;

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static bool IsBetterForFarthest(int a, int b, Metric m)
            => IsSimilarityMetric(m) ? a < b : a > b;
        ...
        public static void nearestRow(in intMxN A, in intN q, Metric m, out int index, out int score)
        {
            ...
            int best = WorstScoreForNearest(m);
            int bestIdx = 0;
            for (int r = 0; r < A.M_Rows; r++)
            {
        ...
        public static void farthestRow(in intMxN A, in intN q, Metric m, out int index, out int score)
        {
            ...
            int worst = WorstScoreForFarthest(m);
            int worstIdx = 0;
            for (int r = 0; r < A.M_Rows; r++)
            {
                int s = RowScore(in A, r, in q, m);
        ...
        public static int kNearestRows(in intMxN A, in intN q, int k, Metric m, ref Indices idx, ref intN scores)
        {
            ...
            for (int r = 0; r < A.M_Rows; r++)
            {
                int s = intQuery_OP.RowScore(in A, r, in q, m);
                if (count < clampedK)
                {
                    int ins = count;
        ...
        /// </summary>
        public static int nonzero<T>(in T x, int tol, ref Indices idx)
            where T : unmanaged, IUnsafeintArray
        {
        ...
        /// </para>
        /// </summary>
        public static int findValue<T>(in T x, int target, int tol)
            where T : unmanaged, IUnsafeintArray
        {
            for (int i = 0; i < x.Data.Length; i++)
            {
        ...
        /// </summary>
        public static int countNonzero<T>(in T x, int tol)
            where T : unmanaged, IUnsafeintArray
        {
            int count = 0;
            for (int i = 0; i < x.Data.Length; i++)
