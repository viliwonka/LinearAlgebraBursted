...
namespace LinearAlgebra
{
    public static partial class LP
    {
        ...
        public static LPInfo ladFN(in floatMxN A, in floatN b, ref floatN x, out double objective, int maxIter = 0)
        {
        ...
        /// τ-less <see cref="ladFN(in floatMxN, in floatN, ref floatN, out double, int)"/>);
                                                   ...
                                                   ref floatN x, out double objective, int maxIter)
        {
            ...
            float tauC = (float)math.clamp(tau, 1e-6, 1.0 - 1e-6);
            float oneMinusTau = (float)1 - tauC;

            var a = new floatN(m, Allocator.Temp);
            var s = new floatN(m, Allocator.Temp);
            var y = new floatN(n, Allocator.Temp);          // dual multipliers; x_coef = -y (see file header)
            var z = new floatN(m, Allocator.Temp);
            var w = new floatN(m, Allocator.Temp);
            var q = new floatN(m, Allocator.Temp);
            var zw = new floatN(m, Allocator.Temp);         // z - w, recomputed each iteration
            var Av = new floatN(m, Allocator.Temp);         // scratch: A * (n-vector), reused per use
            var tmpN = new floatN(n, Allocator.Temp);        // scratch: CHOP.decompSolve in/out (n-length)
            var rhs = new floatN(n, Allocator.Temp);         // affine-predictor RHS, kept for the corrector
            var rhs2 = new floatN(n, Allocator.Temp);
            var dyAff = new floatN(n, Allocator.Temp);
            var daAff = new floatN(m, Allocator.Temp);
            var dsAff = new floatN(m, Allocator.Temp);
            var dzAff = new floatN(m, Allocator.Temp);
            var dwAff = new floatN(m, Allocator.Temp);
            var dy = new floatN(n, Allocator.Temp);
            var da = new floatN(m, Allocator.Temp);
            var ds = new floatN(m, Allocator.Temp);
            var dz = new floatN(m, Allocator.Temp);
            var dw = new floatN(m, Allocator.Temp);
            var dadz = new floatN(m, Allocator.Temp);
            var dsdw = new floatN(m, Allocator.Temp);
            var xi = new floatN(m, Allocator.Temp);
            var qCorr = new floatN(m, Allocator.Temp);       // q .* (dadz - dsdw - xi), corrector RHS term
            var M = new floatMxN(n, n, Allocator.Temp);
            var L = new floatMxN(n, n, Allocator.Temp);
            var yBest = new floatN(n, Allocator.Temp);
            var bLP = new floatN(n, Allocator.Temp);         // Aᵀ((1-tau)·1), the dual LP's constraint RHS
            ...
            for (int i = 0; i < m; i++)
            {
                ...
                {
                    z[i] = math.max(r, (float)0) + zwFloor;
                    w[i] = math.max(-r, (float)0) + zwFloor;
                }
                else
                {
                    z[i] = math.max(r, (float)0);
                    w[i] = math.max(-r, (float)0);
                }
            }
            ...
            while (status != LPStatus.Optimal && iters < budget)
            {
                ...
                for (int i = 0; i < m; i++)
                {
                    ...
                    dsAff[i] = -daAff[i];
                    dzAff[i] = -z[i] * ((float)1 + daAff[i] / a[i]);
                    dwAff[i] = -w[i] * ((float)1 + dsAff[i] / s[i]);
                }

                float fa = math.min(MaxStep(a, daAff, m, BIG), MaxStep(s, dsAff, m, BIG));
                float fd = math.min(MaxStep(w, dwAff, m, BIG), MaxStep(z, dzAff, m, BIG));
                fa = math.min(beta * fa, (float)1);
                fd = math.min(beta * fd, (float)1);

                if (math.min(fa, fd) < (float)1)
                {
                    ...
                    for (int i = 0; i < m; i++)
                    {
                        ...
                        ds[i] = -da[i];
                        dz[i] = (float)(muTarget / (double)a[i]) - z[i] - (z[i] / a[i]) * da[i] - dadz[i];
                        dw[i] = (float)(muTarget / (double)s[i]) - w[i] - (w[i] / s[i]) * ds[i] - dsdw[i];
                    }
        ...
        static double DualityGap(floatN b, floatN a, floatN y, floatN bLP, floatN w, int m, int n)
        {
            double cx = 0;
