...
namespace LinearAlgebra
{
    ...
    // overload.
    //
    // FACTOR/WEIGHT PERSISTENCE (docs/spec-lpbasis-persistence.md, floatLPCache in LP.Cache.float.cs):
    ...
    // caller-persisted floatLPCache instead of allocating+Refactorize+w=1 every call. See
    ...
    public static partial class LP
    {
        ...
        // Cache-aware warm overload (docs/spec-lpbasis-persistence.md) -- floatLPCache
        // (LP.Cache.float.cs) persists M/lower/upper/cost/rhs and the basis factorization (B/P/eta) and
        // DSE weight[] across separate top-level solve calls (MIP.SearchCore's per-node/strong-branch-
        // trial re-solves; MIP.float.cs). `M`/`lower`/`upper`/`cost`/`rhs` here are ALREADY the cache's
        // own buffers (LP.solve, LP.float.cs, builds or patches them before calling this) -- this
        ...
        // factors were built (floatLPCache's own doc comment has the full invalidation contract,
                                                     ...
                                                     bool resumeFactors, bool resumeWeights, out bool factorsUsable)
        {
            ...
            // HEkkDual.cpp's solve setup): (1) resumeWeights (floatLPCache.weightsValid) AND
