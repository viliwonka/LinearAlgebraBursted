...
namespace LinearAlgebra
{
    ...
    // (LP.RevisedSimplex.float.cs: Refactorize/Ftran/Btran/the eta-apply pair/RebuildXB). Where the
    ...
    // dual basis to the PRIMAL core (LP.RevisedSimplex.float.cs) as a cleanup pass once real bounds are restored.
    //
    // Numerics: ordinary float throughout, same as the primal revised simplex (reuses the library's own LU/Blas;
    // tolerances derived from Consts.floatZeroThreshold/floatEpsilon). The DSE weight floor (1e-4)
    ...
    // warm-overload, like LP.RevisedSimplex.float.cs's RevisedPrimalCore.
    //
    // FACTOR/WEIGHT PERSISTENCE (floatLPCache in LP.Cache.float.cs): the warm overload's pivot loop
    // is factored into DualSimplexPivotCore below, shared by a plain (Allocator.Temp scratch) wrapper
    // and a cache-aware wrapper that resumes B/P/eta/weight from a caller-persisted floatLPCache
    ...
    {
        // ---- float-typed entry point: builds the computational form (reusing the revised simplex's
        ...
        // Warm-start overload -- LP.solve's `ref LPBasis` re-solve entry point (LP.float.cs; LPBasis
        ...
        // Cache-aware warm overload -- floatLPCache (LP.Cache.float.cs) persists M/lower/upper/
                                               ...
                                               ref floatLPCache cache)
        {
            ...
            // update formula, never a correctness hazard -- floatLPCache's doc comment), unlike B/P/eta
        ...
        // persisted floatLPCache buffers directly. This method never allocates or disposes any of the
        ...
        // (see the exit branch).
        internal static LPInfo DualSimplexPivotCore(floatMxN M, floatN lower, floatN upper,
                                                     floatN cost, floatN rhs, int m, int n, int N,
                                                     int maxIter, floatN xFull,
                                                     NativeArray<int> basis, NativeArray<byte> status,
                                                     floatMxN B, ref Pivot P, floatMxN etaAlpha, NativeArray<int> etaRow,
                                                     ref int etaCount, floatN weight,
                                                     bool resumeFactors, bool resumeWeights, out bool factorsUsable)
        {
            // Same per-dtype tolerance expressions as LP.RevisedSimplex.float.cs (see that file's header for why these are
            ...
            // Entry factorization: resumeFactors (floatLPCache hit) resumes the caller-supplied
            ...
            // resumeWeights (floatLPCache.weightsValid) AND didResumeFactors -- weight[] is already
            ...
            if (!(resumeWeights && didResumeFactors)) for (int i = 0; i < m; i++) weight[i] = (float)1;
            ...
            else if (resultStatus == LPStatus.Optimal && iters == 0 && !anyArtificial)
            {
                ...
                // with status = Optimal. Composes with a resumeFactors cache hit (floatLPCache): the
