...
namespace LinearAlgebra
{
    ...
    public static partial class LP
    {
        ...
        internal static void PriceRow(floatMxN M, floatN rho, NativeArray<byte> status, int N, int m, floatN alphaRow)
        {
            for (int j = 0; j < N; j++)
            {
                if (status[j] == STATUS_BASIC) { alphaRow[j] = (float)0; continue; }
                float s = (float)0;
        ...
        internal static void PriceReducedCosts(floatMxN M, floatN y, floatN cost, NativeArray<byte> status, int N, int m, floatN dj)
        {
            for (int j = 0; j < N; j++)
            {
                if (status[j] == STATUS_BASIC) { dj[j] = (float)0; continue; }
                float d = cost[j];
                                           ...
                                           out int enterCol, out float thetaD, out bool anyCandidate)
        {
            ...
            while (true)
            {
                ...
                for (int j = 0; j < N; j++)
                {
                    ...
                    if (best < 0 || ratio < bestRatio - (float)1e-9 ||
                        (ratio <= bestRatio + (float)1e-9 && math.abs(ah) > bestAbsA))
                                               ...
                                               NativeArray<int> basis, NativeArray<byte> status)
        {
            ...
            // for the same return-type-collision reason as the tolerances above (an float-returning
            ...
            for (int j = 0; j < N; j++)
            {
                ...
                h ^= h >> 16;
                float unit = (float)(h * (1.0 / 4294967295.0) * 2.0 - 1.0);
                perturbedCost[j] = cost[j] + unit * (float)1e-5 * ((float)1 + math.abs(cost[j]));
            }
