...
{
    // Burst-safe block-structure printing for floatBSM (block-CSR sparse matrix), mirroring
    // Print.Spy/Print.Log for dense matrices (Debug.float.cs) but at BLOCK granularity: one char
    // per BR x BC block instead of per scalar. See Sparse/floatBSM.cs for the RowPtr/ColInd/Values
    ...
    // overloads -- floatBSM is brought in via the `using LinearAlgebra.Sparse;` above.
    public static partial class Print
    {
        // Scans ColInd[RowPtr[row]..RowPtr[row+1)) for `col`. ColInd is ascending within a row
        // (see floatBSM.cs), so this early-exits once it passes col instead of scanning the whole
        ...
        static bool BsmBlockStored(in floatBSM m, int row, int col)
        {
        ...
        // type-agnostic const in an float template would emit into BOTH the float and double Print
        // partials and collide (CS0102).

        // Appends the header + block-sparsity grid ('X' stored / '.' absent) into str. One char per
        // block; for Symmetric matrices (only the upper block-triangle is stored, see floatBSM.cs)
        ...
        static bool AppendBsmSpyGrid(in floatBSM m, ref FixedString4096Bytes str)
        {
            int mb = m.BlockRows;
            int nb = m.BlockCols;
            int nnzb = m.Nnzb;
        ...
        /// </summary>
        public static void Spy(in floatBSM m)
        {
        ...
        /// </summary>
        public static void Log(in floatBSM m)
        {
            FixedString4096Bytes str = new FixedString4096Bytes();
