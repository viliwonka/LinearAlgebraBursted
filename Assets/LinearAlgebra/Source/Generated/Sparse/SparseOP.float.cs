...
namespace LinearAlgebra.Sparse
{
    ...
    public static partial class BSR
    {
        ...
        // floatBSROperator.ApplyBlock looped `rows` scalar spMV calls through two Allocator.Temp
        // vectors). Same dispatch shape as spMV -- see bsrMatMat*/bsrMatMatSym* (UnsafeOP.Sparse.
        // float.cs) for the per-kernel bit-identity argument. Guard: y must not alias x -- each
        ...
        public static void sweepLower(in floatBSR A, in floatBlockJacobi Jacobi, float diagScale, in floatN r, ref floatN y)
        {
            ...
            if (A.Symmetric)
                throw new ArgumentException("sweepLower: A must be full-storage BSR (Symmetric upper-block-triangle storage is not supported here -- mirror it first via Arena.floatBSRMirrorToFull)");
        ...
        public static void sweepUpper(in floatBSR A, in floatBlockJacobi Jacobi, float diagScale, in floatN r, ref floatN y)
        {
            ...
            if (A.Symmetric)
                throw new ArgumentException("sweepUpper: A must be full-storage BSR (Symmetric upper-block-triangle storage is not supported here -- mirror it first via Arena.floatBSRMirrorToFull)");
            if (Jacobi.BlockRows != A.BlockRows || Jacobi.BR != A.BR)
