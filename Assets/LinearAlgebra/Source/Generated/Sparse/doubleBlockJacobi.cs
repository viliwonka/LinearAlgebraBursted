...
namespace LinearAlgebra.Sparse
{
    ...
    ///
    /// Built ONCE from a compressed <see cref="doubleBSM"/> (an O(nb * BR^3) one-time cost via
    ...
    /// The two IdoubleLinearOperator wrappers (doubleBSMOperator/doubleDenseOperator) are
    ...
    /// z)` call inside pcg -- undermining the zero-cost-dispatch claim. See doubleBSMOperator's
    ...
    public readonly partial struct doubleBlockJacobi : IdoublePreconditioner, IDisposable
    {
        ...
        /// </summary>
        public unsafe doubleBlockJacobi(in doubleBSM A, Allocator allocator)
        {
            _arenaPtr = null;
        ...
        /// `in` here because -- unlike doubleBSM.ToDense/doubleBSMBuilder.ToBSM -- this
        ...
        /// proofing) `_arenaPtr` field, matching the doubleBSM/doubleBSMBuilder constructor
        /// pair's own `in Arena` constructor. The arena-OWNING factory that actually registers
        /// this instance for disposal is `Arena.doubleBlockJacobi(in doubleBSM)`, which takes
        /// `ref Arena` per the Phase-1 dangling-pointer lesson (see doubleBSM.ToDense).
        /// </summary>
        public unsafe doubleBlockJacobi(in doubleBSM A, in Arena arena) : this(in A, arena.Allocator)
        {
        ...
        public void Dispose()
        {
            ...
            for (int i = 0; i < DInv.Length; i++) DInv[i] = double.NaN;
#endif
            DInv.Dispose();
        }
    }
