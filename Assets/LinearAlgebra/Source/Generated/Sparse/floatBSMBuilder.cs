...
{
    /// <summary>
    /// COO-of-blocks assembly builder for floatBSM. Accumulates (blockRow, blockCol, BR x BC
    ...
    ///
    /// MUTABLE-STATE INDIRECTION (fixes a use-after-free): the arena's floatBSMBuilder(...)
    /// factory (see Arena.Sparse.float.cs) registers a VALUE COPY of this struct in its own
    ...
    /// floatBSMBuilder shares (the pointer VALUE is copied around; there is exactly one
    /// pointee) -- mirrors the Arena* idiom already used by floatMxN/floatBSM/etc, except the
    ...
    /// the arena's.
    /// </summary>
    public partial struct floatBSMBuilder : IDisposable
    {
        // Heap-owned, single-identity mutable state -- see the type doc above. Every
        // floatBSMBuilder value-copy (this instance, the arena's tracked copy, the caller's
        ...
        public unsafe floatBSMBuilder(int blockRows, int blockCols, int BR, int BC, Allocator allocator, int capacityHint = 8)
        {
            _arenaPtr = null;
        ...
        public unsafe floatBSMBuilder(int blockRows, int blockCols, int BR, int BC, in Arena arena, int capacityHint = 8)
        {
        ...
        ///
        /// Takes the arena by `ref`, NOT `in`: it calls the mutating arena.floatBSM(...)
        /// allocator internally, and `Arena`'s allocator methods are not `readonly` -- see the
        /// matching comment on floatBSM.ToDense for why an `in Arena` parameter here would
        /// produce a dangling internal arena pointer on the returned floatBSM.
        /// </summary>
        public unsafe floatBSM ToBSM(ref Arena arena)
        {
            int n = TripletCount;
            int blockLen = BR * BC;
            ...
            var bsm = arena.floatBSM(BlockRows, BlockCols, BR, BC, nnzb, true);
        ...
        /// value returned by arena.floatBSMBuilder(...) themselves, same as floatMxN/
        /// floatBSM/etc).
        /// </summary>
