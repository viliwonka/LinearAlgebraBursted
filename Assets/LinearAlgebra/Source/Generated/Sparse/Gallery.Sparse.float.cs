...
namespace LinearAlgebra.Gallery
{
    ...
    public static partial class floatGallery
    {
        ...
        public static floatBSR floatRandomSparseSPD(this ref Arena arena, int blockRows, int BR, float density, uint seed)
        {
            ...
            var offBound = arena.floatVec(blockRows);
            for (int i = 0; i < blockRows; i++) offBound[i] = (float)0;

            int nnzbEstimate = blockRows + blockRows * deg * 2;
            var builder = arena.floatBSRBuilder(blockRows, blockRows, BR, BR, nnzbEstimate);

            var blk  = arena.floatMat(BR, BR);
            var blkT = arena.floatMat(BR, BR);
            float blockRowBound = (float)floatSparseOffScale * BR; // max scalar-row L1 of one off-diag block
        ...
        public static floatBSR floatRandomSparse(this ref Arena arena, int blockRows, int blockCols, int BR, float density, uint seed)
        {
            ...
            var builder = arena.floatBSRBuilder(blockRows, blockCols, BR, BR, nnzbEstimate);
        ...
        public static floatBSR floatLaplacian2D(this ref Arena arena, int gridX, int gridY)
        {
            ...
            var builder = arena.floatBSRBuilder(nb, nb, BR, BR, nnzbEstimate);

            // Diagonal block: tridiag(-1, 4, -1) (x-neighbor coupling + the full 2D degree 4).
            var D = arena.floatMat(BR, BR);
            for (int r = 0; r < BR; r++)
                for (int c = 0; c < BR; c++)
                    D[r, c] = (float)0;
            for (int r = 0; r < BR; r++)
