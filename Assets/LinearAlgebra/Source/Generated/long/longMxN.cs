...
namespace LinearAlgebra
{
    ...
    // n = cols
    //
    // [StructLayout(Sequential)]: pins field order/packing explicitly (matches longN's existing
    ...
    public partial struct longMxN : IDisposable, IUnsafelongArray {
        ...
        // Arena-tracked path -- see longN.cs's `_rec` doc comment for the full rationale (same
        ...
        // TempCopy() and the cross-type allocation shortcuts (longMxN.Shortcuts.cs) that used to
        ...
        // longN's option, cannot tell "still the same allocation" from "a new one that reused this
        // slot number").
        ...
        // doc comment). Unlike longN (Alive-only), longMxN also has
