...
namespace LinearAlgebra
{
    ...
    public partial struct longN : IDisposable, IUnsafelongArray {
        ...
        // Arena-tracked path: a stable pointer into the arena's ChunkedRecordTable<longVecRecord>
        ...
        [NativeDisableUnsafePtrRestriction] private unsafe longVecRecord* _rec;

        // Standalone-path backing store -- the ONLY thing that changes for a non-arena vector.
        // Stays default(UnsafeList<long>) whenever _rec != null (arena-tracked).
        ...
        // struct size is identical in both configs either way, since this adds no field). longN
        // has no spare bits to pack a generation
        // stamp into (32B = 8 _rec + 24 UnsafeList<long>, exactly -- see docs/dev/rfc-memory-model.md
        ...
        {
            if (_rec != null && !ChunkedRecordTable<longVecRecord>.IsAliveFast(_rec))
                throw new InvalidOperationException("longN.Data: use of disposed/cleared arena allocation");
        }
#endif

        // Reconstructs a live Arena handle from this record's owner core -- used by Copy()/
        // TempCopy() and the cross-type allocation shortcuts (longN.Shortcuts.cs) that used to
        ...
        /// arena's record table by the caller (Arena.longVec/longTempVec) -- this ctor only
        /// fills in the record's Data, it does not allocate or own the slot itself.
        /// </summary>
        internal unsafe longN(int n, longVecRecord* rec, Allocator allocator, bool uninit = false) {
        ...
        internal unsafe longN(in longN orig, longVecRecord* rec, Allocator allocator) {
        ...
        public unsafe longN Copy()
        {
            ...
            return OwnerArena.longVec(in this);
        }
        ...
        public unsafe longN TempCopy()
        {
            ...
            return OwnerArena.longTempVec(in this);   // temp pool (was wrongly the persistent Copy path)
        }
