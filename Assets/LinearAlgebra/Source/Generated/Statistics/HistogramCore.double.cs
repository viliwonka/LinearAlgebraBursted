...
namespace LinearAlgebra
{
    ...
    //
    // Generic constraint IUnsafedoubleArray lets these ops work over both doubleN and
    // doubleMxN (a matrix histograms its flat row-major data, identical to StatsOP reductions).
    internal static partial class doubleHistogramCore
    {
        // -------------------------------------------------------------------------
        ...
        /// </summary>
        public static void histogramInto<T>(in T data, double lo, double hi, ref Indices counts)
        ...
        /// <para>Same bin rule as <see cref="histogramInto{T}(in T, double, double, ref Indices)"/>.
        ...
        /// </summary>
        public static void densityInto<T>(in T data, double lo, double hi, ref doubleN dest)
            where T : unmanaged, IUnsafedoubleArray
        {
            int K = dest.N;
            int N = data.Data.Length;
            ...
            double invNW = (double)K / ((double)N * (hi - lo));
            for (int b = 0; b < K; b++)
                dest[b] = (double)scratch[b] * invNW;
        ...
        /// arguments are validated before any allocation.</para>
        /// </summary>
        public static void cdfInto<T>(in T data, double lo, double hi, ref doubleN dest)
            where T : unmanaged, IUnsafedoubleArray
        {
            int K = dest.N;
            ...
            {
                double invTotal = (double)1 / (double)inRangeTotal;
                int cum = 0;
                for (int b = 0; b < K; b++)
                {
                    cum += scratch[b];
                    dest[b] = (double)cum * invTotal;
                }
                dest[K - 1] = (double)1;   // pin last bin to bit-exact 1 (cum*invTotal may be 1-ulp short)
            }
        ...
        public static void histogram2DInto<TX, TY>(
            ...
            double loX, double hiX,
            double loY, double hiY,
            ref doubleMxN counts)
            where TX : unmanaged, IUnsafedoubleArray
            where TY : unmanaged, IUnsafedoubleArray
        {
            ...
            double wX = (hiX - loX) / (double)Kx;
            double wY = (hiY - loY) / (double)Ky;

            int n = dataX.Data.Length;
            for (int i = 0; i < n; i++)
            {
                double x = dataX.Data[i];
                double y = dataY.Data[i];
