...
namespace LinearAlgebra
{
    ...
    // the bare `Stats` facade in Stats.int.cs. Kept SPLIT (intStatsCore/shortStatsCore/
    // longStatsCore -- one distinct type per generated type) because every method here is
    // generic over T with only a CONSTRAINT (IUnsafeintArray) distinguishing int/short/long --
    ...
    internal static partial class intStatsCore
    {
        ...
        // (see StatsTests.int.cs's SumAccumulatorOwnOverflow, which pins this contrast: the
        // same 2-element/MaxValue-filled input is correct-and-widened for int/short but silently
        // wraps for long).
        public static long sum<T>(in T x) where T : unmanaged, IUnsafeintArray {
        ...
        public static double mean<T>(in T x) where T : unmanaged, IUnsafeintArray {
        ...
        // construction.
        public static double variance<T>(in T x) where T : unmanaged, IUnsafeintArray {
            if (x.Data.Length == 0)
        ...
        public static double stdDev<T>(in T x) where T : unmanaged, IUnsafeintArray {
        ...
        public static double varianceSample<T>(in T x) where T : unmanaged, IUnsafeintArray {
            if (x.Data.Length == 0)
        ...
        public static double stdDevSample<T>(in T x) where T : unmanaged, IUnsafeintArray {
            return math.sqrt(varianceSample(in x));
        }

        // Returns the index of the smallest element (first occurrence on ties). For a matrix
        // (IUnsafeintArray over row-major data), the index is the linear index r*N_Cols+c.
        public static int argmin<T>(in T x) where T : unmanaged, IUnsafeintArray {
            if (x.Data.Length == 0)
        ...
        // as argmin above.
        public static int argmax<T>(in T x) where T : unmanaged, IUnsafeintArray {
            if (x.Data.Length == 0)
        ...
        public static int min<T>(in T x) where T : unmanaged, IUnsafeintArray {
            if (x.Data.Length == 0)
        ...
        public static int max<T>(in T x) where T : unmanaged, IUnsafeintArray {
            if (x.Data.Length == 0)
        ...
        public static double median<T>(in T x) where T : unmanaged, IUnsafeintArray {
            if (x.Data.Length == 0)
            ...
            var copy = new UnsafeList<int>(x.Data.Length, Allocator.Temp, NativeArrayOptions.UninitializedMemory);
