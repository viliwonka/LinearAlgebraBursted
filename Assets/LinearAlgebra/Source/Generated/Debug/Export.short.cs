...
namespace LinearAlgebra
{
    ...
    // only the cast off the short PROXY struct (which -- like fProxy, see proxyStructs.cs -- only
    ...
    // at the proxy-compile stage (short defines an implicit conversion to int, so the int choice
    ...
    {
        public static string ToText(in shortMxN m)
        {
            ...
            for (int r = 0; r < m.M_Rows; r++)
            {
                ...
                {
                    if (c > 0) sb.Append(' ');
                    sb.Append(((short)m[r, c]).ToString(CultureInfo.InvariantCulture));
                }
                sb.Append('\n');
            }
            return sb.ToString();
        ...
        public static string ToText(in shortN v)
        {
            var sb = new StringBuilder();
            for (int i = 0; i < v.N; i++)
            {
                if (i > 0) sb.Append('\n');
                sb.Append(((short)v[i]).ToString(CultureInfo.InvariantCulture));
            }
            return sb.ToString();
        }

        public static string ToCsv(in shortMxN m)
        {
            var sb = new StringBuilder();
            for (int r = 0; r < m.M_Rows; r++)
            {
        ...
        public static string ToCsv(in shortN v)
        {
            var sb = new StringBuilder();
            for (int i = 0; i < v.N; i++)
            {
        ...
        public static void SaveCsv(in shortMxN m, string path) => File.WriteAllText(path, ToCsv(in m));
        public static void SaveCsv(in shortN v, string path) => File.WriteAllText(path, ToCsv(in v));
    }
}
