...
namespace LinearAlgebra
{
    ...
    // only the cast off the uint PROXY struct (which -- like fProxy, see proxyStructs.cs -- only
    ...
    // at the proxy-compile stage (uint defines an implicit conversion to int, so the int choice
    ...
    {
        public static string ToText(in uintMxN m)
        {
            ...
            for (int r = 0; r < m.M_Rows; r++)
            {
                ...
                {
                    if (c > 0) sb.Append(' ');
                    sb.Append(((uint)m[r, c]).ToString(CultureInfo.InvariantCulture));
                }
                sb.Append('\n');
            }
            return sb.ToString();
        ...
        public static string ToText(in uintN v)
        {
            var sb = new StringBuilder();
            for (int i = 0; i < v.N; i++)
            {
                if (i > 0) sb.Append('\n');
                sb.Append(((uint)v[i]).ToString(CultureInfo.InvariantCulture));
            }
            return sb.ToString();
        }

        public static string ToCsv(in uintMxN m)
        {
            var sb = new StringBuilder();
            for (int r = 0; r < m.M_Rows; r++)
            {
        ...
        public static string ToCsv(in uintN v)
        {
            var sb = new StringBuilder();
            for (int i = 0; i < v.N; i++)
            {
        ...
        public static void SaveCsv(in uintMxN m, string path) => File.WriteAllText(path, ToCsv(in m));
        public static void SaveCsv(in uintN v, string path) => File.WriteAllText(path, ToCsv(in v));
    }
}
