...
namespace LinearAlgebra
{
    ...
    public partial struct uintN : IDisposable, IUnsafeuintArray {
        ...
        // Arena-tracked path: a stable pointer into the arena's ChunkedRecordTable<uintVecRecord>
        ...
        [NativeDisableUnsafePtrRestriction] private unsafe uintVecRecord* _rec;

        // Standalone-path backing store -- the ONLY thing that changes for a non-arena vector.
        // Stays default(UnsafeList<uint>) whenever _rec != null (arena-tracked).
        ...
        // struct size is identical in both configs either way, since this adds no field). uintN
        // has no spare bits to pack a generation
        // stamp into (32B = 8 _rec + 24 UnsafeList<uint>, exactly -- see docs/dev/rfc-memory-model.md
        ...
        {
            if (_rec != null && !ChunkedRecordTable<uintVecRecord>.IsAliveFast(_rec))
                throw new InvalidOperationException("uintN.Data: use of disposed/cleared arena allocation");
        }
#endif

        // Reconstructs a live Arena handle from this record's owner core -- used by Copy()/
        // TempCopy() and the cross-type allocation shortcuts (uintN.Shortcuts.cs) that used to
        ...
        /// </summary>
        public unsafe uintN(int n, Allocator allocator = Allocator.Invalid, bool uninit = false)
        {
            _rec = null;
            _inlineData = default;
        ...
        /// </summary>
        public unsafe uintN(in uintN orig, Allocator allocator = Allocator.Invalid) {
        ...
        /// arena's record table by the caller (Arena.uintVec/uintTempVec) -- this ctor only
        /// fills in the record's Data, it does not allocate or own the slot itself.
        /// </summary>
        internal unsafe uintN(int n, uintVecRecord* rec, Allocator allocator, bool uninit = false) {
        ...
        internal unsafe uintN(in uintN orig, uintVecRecord* rec, Allocator allocator) {
        ...
        public unsafe uintN Copy()
        {
            ...
            return OwnerArena.uintVec(in this);
        }
        ...
        public unsafe uintN TempCopy()
        {
            ...
            return OwnerArena.uintTempVec(in this);   // temp pool (was wrongly the persistent Copy path)
        }
