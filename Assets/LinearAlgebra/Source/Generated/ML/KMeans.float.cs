...
namespace LinearAlgebra.ML
{
    ...
    public static partial class KMeans
    {
            ...
            ref floatKMeansCache ws)
        {
            ...
            {
                float s = (float)0;
                for (int f = 0; f < D; f++) { float v = X[n, f]; s += v * v; }
                ws.PointNormSq[n] = s;
            }
            ...
            for (int iter = 0; iter < maxIter; iter++)
            {
                ...
                for (int j = 0; j < k; j++)
                {
                    for (int f = 0; f < D; f++) ws.NewCentroids[j, f] = (float)0;
                ...
                for (int j = 0; j < k; j++)
                {
                    ...
                    ws.D2Weights[farthestPt] = (float)(-1); // exclude from subsequent scans
                }

                // 5.4.10  Divide accumulators -> new centroids
                for (int j = 0; j < k; j++)
                {
                    float invN = (float)1 / (float)ws.ClusterCounts[j];
        ...
        /// <summary>
        /// Calls <see cref="kmeans(in floatMxN,int,uint,int,KMeansInit,ref floatMxN,ref Indices,out float,out int,ref floatKMeansCache)"/>
        ...
        /// <summary>
        /// Calls <see cref="kmeans(ref Arena,in floatMxN,int,uint,int,KMeansInit,out floatMxN,out Indices,out float,out int)"/>
        /// with <c>init = KMeansInit.KMeansPlusPlus</c>.
        /// </summary>
        public static void kmeans(
            ref Arena arena,
            ...
            ref floatKMeansCache ws)
        {
            ...
            for (int n = 0; n < N; n++)
            {
                ...
                for (int f = 0; f < D; f++)
                {
                    float diff = X[n, f] - centroids[0, f];
                    d2 += diff * diff;
                }
                ws.D2Weights[n] = d2;
            }
            ...
            for (int ci = 1; ci < k; ci++)
            {
                ...
                if (!(total > (float)0))
                {
                    ...
                    nextIdx = math.min((int)(rng.NextFloat() * (float)N), N - 1);
                }
                else
                {
                    // Rand.weightedPick validates + draws from D² distribution.
                    nextIdx = Rand.weightedPick(in ws.D2Weights, ref rng);
                }
                ...
                if (ci < k - 1)
                {
                    for (int n = 0; n < N; n++)
                    {
                        ...
                        for (int f = 0; f < D; f++)
                        {
                            float diff = X[n, f] - centroids[ci, f];
                            d2 += diff * diff;
                        }
            ...
            ref floatMxN centroids)
        {
            ...
            for (int n = 0; n < N && needed > 0; n++)
            {
                ...
                float r = rng.NextFloat() * (float)(N - n);
                if (r < (float)needed)
                {
