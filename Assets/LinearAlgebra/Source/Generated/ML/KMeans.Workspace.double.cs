...
namespace LinearAlgebra.ML
{
    ...
    /// Allocate ONCE (sized for the data shape) via <c>Arena.doubleKMeansWorkspace(N, D, k)</c>
    ...
    /// </summary>
    public struct doubleKMeansWorkspace
    {
...
namespace LinearAlgebra
{
    public partial struct Arena
    {
        ...
        /// </summary>
        public LinearAlgebra.ML.doubleKMeansWorkspace doubleKMeansWorkspace(int N, int D, int k)
        {
            return new LinearAlgebra.ML.doubleKMeansWorkspace
            {
                Gram           = doubleMat(N, k),
                Ct             = doubleMat(D, k),
                PointNormSq    = doubleVec(N),
                CentNormSq     = doubleVec(k),
                PrevAssignment = Indices(N),
                NewCentroids   = doubleMat(k, D),
                ClusterCounts  = Indices(k),
                D2Weights      = doubleVec(N)
            };
        }
    }
}
