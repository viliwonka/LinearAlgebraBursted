...
namespace LinearAlgebra.ML
{
    ...
    /// </summary>
    public static partial class doubleKMeansOP
    {
        // =========================================================================
        ...
        /// <paramref name="ws"/>         Pre-allocated workspace — Arena.doubleKMeansWorkspace(N, D, k).
        /// </summary>
        public static void kmeans(
            ...
            int maxIter,
            KMeansInit init,
            ref doubleMxN centroids,
            ref Indices assignment,
            out double inertia,
            out int iters,
            ref doubleKMeansWorkspace ws)
        {
            // ---- input guards ----
            if (X.M_Rows == 0 || X.N_Cols == 0)
                throw new InvalidOperationException("doubleKMeansOP.kmeans: X is empty");
            if (k <= 0)
                throw new ArgumentException("doubleKMeansOP.kmeans: k must be >= 1");
            if (maxIter < 1)
                throw new ArgumentException("doubleKMeansOP.kmeans: maxIter must be >= 1");
            ...
            if (centroids.M_Rows != k || centroids.N_Cols != D)
                throw new ArgumentException(
                    "doubleKMeansOP.kmeans: centroids must be k×D after clamping k to min(k,N)");
            if (assignment.N != N)
                throw new ArgumentException(
                    "doubleKMeansOP.kmeans: assignment.N must equal X.M_Rows (N)");

            // ---- workspace shape checks ----
            if (ws.Gram.M_Rows != N || ws.Gram.N_Cols != k)
                throw new ArgumentException(
                    "doubleKMeansOP.kmeans: ws.Gram must be N×k");
            if (ws.Ct.M_Rows != D || ws.Ct.N_Cols != k)
                throw new ArgumentException(
                    "doubleKMeansOP.kmeans: ws.Ct must be D×k");
            if (ws.PointNormSq.N != N)
                throw new ArgumentException(
                    "doubleKMeansOP.kmeans: ws.PointNormSq.N must equal N");
            if (ws.CentNormSq.N != k)
                throw new ArgumentException(
                    "doubleKMeansOP.kmeans: ws.CentNormSq.N must equal k");
            if (ws.PrevAssignment.N != N)
                throw new ArgumentException(
                    "doubleKMeansOP.kmeans: ws.PrevAssignment.N must equal N");
            if (ws.NewCentroids.M_Rows != k || ws.NewCentroids.N_Cols != D)
                throw new ArgumentException(
                    "doubleKMeansOP.kmeans: ws.NewCentroids must be k×D");
            if (ws.ClusterCounts.N != k)
                throw new ArgumentException(
                    "doubleKMeansOP.kmeans: ws.ClusterCounts.N must equal k");
            if (ws.D2Weights.N != N)
                throw new ArgumentException(
                    "doubleKMeansOP.kmeans: ws.D2Weights.N must equal N");

            // ---- precompute point squared norms (once, before the Lloyd loop) ----
            for (int n = 0; n < N; n++)
            {
                double s = (double)0;
                for (int f = 0; f < D; f++) { double v = X[n, f]; s += v * v; }
                ws.PointNormSq[n] = s;
            }
            ...
            iters = 0;
            inertia = (double)0; // overwritten in final sync below
            ...
            for (int iter = 0; iter < maxIter; iter++)
            {
                ...
                {
                    double s = (double)0;
                    for (int f = 0; f < D; f++) { double v = centroids[j, f]; s += v * v; }
                    ws.CentNormSq[j] = s;
                }

                // 5.4.2  Transpose centroids (k×D) -> ws.Ct (D×k)
                doubleOP.trans(in centroids, ref ws.Ct);

                // 5.4.3  GEMM: ws.Gram = X * ws.Ct  (N×k); dot zero-clears before accumulating.
                doubleOP.dot(in X, in ws.Ct, ref ws.Gram);
                ...
                doubleQueryOP.rowArgMin(in ws.Gram, ref assignment);
        ...
        /// <summary>
        /// Calls <see cref="kmeans(in doubleMxN,int,uint,int,KMeansInit,ref doubleMxN,ref Indices,out double,out int,ref doubleKMeansWorkspace)"/>
            ...
            out int iters)
        {
            ...
            var ws     = arena.doubleKMeansWorkspace(N, D, kk);
            ...
            ref doubleKMeansWorkspace ws)
        {
            ...
            int firstIdx = math.min((int)(rng.NextDouble() * (double)N), N - 1);
            ...
            for (int n = 0; n < N; n++)
            {
                ...
                for (int f = 0; f < D; f++)
                {
                    double diff = X[n, f] - centroids[0, f];
                    d2 += diff * diff;
                }
                ws.D2Weights[n] = d2;
            }
            ...
            for (int ci = 1; ci < k; ci++)
            {
                ...
                if (!(total > (double)0))
                {
                    ...
                    nextIdx = math.min((int)(rng.NextDouble() * (double)N), N - 1);
                }
                else
                {
                    // doubleRandomOP.weightedPick validates + draws from D² distribution.
                    nextIdx = doubleRandomOP.weightedPick(in ws.D2Weights, ref rng);
                }
                ...
                if (ci < k - 1)
                {
                    for (int n = 0; n < N; n++)
                    {
                        ...
                        for (int f = 0; f < D; f++)
                        {
                            double diff = X[n, f] - centroids[ci, f];
                            d2 += diff * diff;
                        }
            ...
            ref doubleMxN centroids)
        {
            ...
            for (int n = 0; n < N && needed > 0; n++)
            {
                ...
                double r = rng.NextDouble() * (double)(N - n);
                if (r < (double)needed)
                {
