...
namespace LinearAlgebra.ML
{
    ...
    /// </summary>
    public static partial class doubleKMeans_OP
    {
        // =========================================================================
        ...
        /// <paramref name="ws"/>         Pre-allocated workspace — Arena.doubleKMeans_WS(N, D, k).
        /// </summary>
        public static void kmeans(
            ...
            out double inertia,
            out int iters,
            ref doubleKMeans_WS ws)
        {
            // ---- input guards ----
            if (X.M_Rows == 0 || X.N_Cols == 0)
                throw new InvalidOperationException("doubleKMeans_OP.kmeans: X is empty");
            if (k <= 0)
                throw new ArgumentException("doubleKMeans_OP.kmeans: k must be >= 1");
            if (maxIter < 1)
                throw new ArgumentException("doubleKMeans_OP.kmeans: maxIter must be >= 1");
            ...
            if (centroids.M_Rows != k || centroids.N_Cols != D)
                throw new ArgumentException(
                    "doubleKMeans_OP.kmeans: centroids must be k×D after clamping k to min(k,N)");
            if (assignment.N != N)
                throw new ArgumentException(
                    "doubleKMeans_OP.kmeans: assignment.N must equal X.M_Rows (N)");

            // ---- workspace shape checks ----
            if (ws.Gram.M_Rows != N || ws.Gram.N_Cols != k)
                throw new ArgumentException(
                    "doubleKMeans_OP.kmeans: ws.Gram must be N×k");
            if (ws.Ct.M_Rows != D || ws.Ct.N_Cols != k)
                throw new ArgumentException(
                    "doubleKMeans_OP.kmeans: ws.Ct must be D×k");
            if (ws.PointNormSq.N != N)
                throw new ArgumentException(
                    "doubleKMeans_OP.kmeans: ws.PointNormSq.N must equal N");
            if (ws.CentNormSq.N != k)
                throw new ArgumentException(
                    "doubleKMeans_OP.kmeans: ws.CentNormSq.N must equal k");
            if (ws.PrevAssignment.N != N)
                throw new ArgumentException(
                    "doubleKMeans_OP.kmeans: ws.PrevAssignment.N must equal N");
            if (ws.NewCentroids.M_Rows != k || ws.NewCentroids.N_Cols != D)
                throw new ArgumentException(
                    "doubleKMeans_OP.kmeans: ws.NewCentroids must be k×D");
            if (ws.ClusterCounts.N != k)
                throw new ArgumentException(
                    "doubleKMeans_OP.kmeans: ws.ClusterCounts.N must equal k");
            if (ws.D2Weights.N != N)
                throw new ArgumentException(
                    "doubleKMeans_OP.kmeans: ws.D2Weights.N must equal N");
            ...
            for (int iter = 0; iter < maxIter; iter++)
            {
                ...
                double_OP.trans(in centroids, ref ws.Ct);

                // 5.4.3  GEMM: ws.Gram = X * ws.Ct  (N×k); dot zero-clears before accumulating.
                double_OP.dot(in X, in ws.Ct, ref ws.Gram);
        ...
        /// <summary>
        /// Calls <see cref="kmeans(in doubleMxN,int,uint,int,KMeansInit,ref doubleMxN,ref Indices,out double,out int,ref doubleKMeans_WS)"/>
            ...
            out int iters)
        {
            ...
            var ws     = arena.doubleKMeans_WS(N, D, kk);
            ...
            ref doubleKMeans_WS ws)
        {
            ...
            for (int ci = 1; ci < k; ci++)
            {
                ...
                {
                    // doubleRandom_OP.weightedPick validates + draws from D² distribution.
                    nextIdx = doubleRandom_OP.weightedPick(in ws.D2Weights, ref rng);
                }
