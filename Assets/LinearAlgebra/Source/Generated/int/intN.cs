...
namespace LinearAlgebra
{
    ...
    public partial struct intN : IDisposable, IUnsafeintArray {
        ...
        [NativeDisableUnsafePtrRestriction] private unsafe intVecRecord* _rec;

        // Standalone-path backing store -- the ONLY thing that changes for a non-arena vector.
        // Stays default(UnsafeList<int>) whenever _rec != null (arena-tracked).
        private UnsafeList<int> _inlineData;
        ...
        {
            if (_rec != null && !ChunkedRecordTable<intVecRecord>.IsAliveFast(_rec))
                throw new InvalidOperationException("intN.Data: use of disposed/cleared arena allocation");
        }
#endif

        // Reconstructs a live Arena handle from this record's owner core -- used by Copy()/
        // TempCopy() and the cross-type allocation shortcuts (intN.Shortcuts.cs) that used to
        ...
        /// arena's record table by the caller (Arena.intVec/intTempVec) -- this ctor only
        /// fills in the record's Data, it does not allocate or own the slot itself.
        /// </summary>
        internal unsafe intN(int n, intVecRecord* rec, Allocator allocator, bool uninit = false) {
        ...
        internal unsafe intN(in intN orig, intVecRecord* rec, Allocator allocator) {
        ...
        public unsafe intN Copy()
        {
            ...
            return OwnerArena.intVec(in this);
        }
        ...
        public unsafe intN TempCopy()
        {
            ...
            return OwnerArena.intTempVec(in this);   // temp pool (was wrongly the persistent Copy path)
        }
