...
namespace LinearAlgebra
{
    ...
    {
        // Pointer-stable allocation-record tables: floatN/floatMxN hold a stable
        // floatVecRecord*/floatMatRecord* pointing INTO one of these tables (records never move
    ...
    public unsafe partial struct Arena {
        ...
        internal floatN floatVec(in floatN orig)
        {
            ...
            try
            {
                ...
                rec->Table = &_core->floatVecRecords;
                rec->SelfIndex = slot;
                return new floatN(in orig, rec, Allocator);   // persistent (backs Copy())
#if ENABLE_UNITY_COLLECTIONS_CHECKS
            }
            finally { _core->ExitMutation(); }
#endif
