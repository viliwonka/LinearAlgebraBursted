...
namespace BULA
{
    ...
    public static partial class Fit
    {
        ...
        /// <see cref="QRCP.solveInPlace(ref doubleMxN, ref doubleN, ref doubleN, ref doubleMxN, ref Pivot, ref doubleN, double)"/>
        /// or <see cref="SVD.pinvSolve"/>. False means the solve failed.
        /// </summary>
        public static bool linear(in doubleMxN A, in doubleN b, ref doubleN x)
        {
        ...
        /// <see cref="LP.lad(in doubleMxN, in doubleN, ref doubleN, out double, int)"/> -- a finite
        /// algorithm that reaches the true optimum rather than converging toward it.
        /// </summary>
        public static bool linear<TLoss>(in doubleMxN A, in doubleN b, ref doubleN x,
            ...
            where TLoss : struct, IdoubleRobustLoss
        {
            ...
            bool warm = false;
            for (int j = 0; j < n && !warm; j++) warm = x[j] != (double)0;
            ...
            else for (int i = 0; i < m; i++) w[i] = (double)1;

            bool ok = false;
            for (int it = 0; it < maxIter; it++)
            {
                // A REDESCENDING loss (doubleTukeyLoss) drives the weight to exactly zero past its
                ...
                // doubleSubspaceIrls guards the same way for the geometric fits.
                double sw = (double)0;
                ...
                for (int i = 0; i < m; i++)
                {
                    double s = math.sqrt(math.max(w[i], (double)0));
        ...
        /// both cases.
        /// </summary>
        public static bool total(in doubleMxN A, in doubleN b, ref doubleN x)
        {
            int m = A.M_Rows, n = A.N_Cols;
            ...
            int nc = n + 1;
            var C = new doubleMxN(m, nc, Allocator.Temp);
            for (int i = 0; i < m; i++)
            {
            ...
            var U = new doubleMxN(m, nc, Allocator.Temp);
            var S = new doubleN(nc, Allocator.Temp);
            var V = new doubleMxN(nc, nc, Allocator.Temp);
