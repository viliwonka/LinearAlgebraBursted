...
namespace LinearAlgebra
{
    ...
    public static partial class Query
    {
        ...
        internal static unsafe void RowArgMinScan(double* row, int n, out int bestC, out double bestVal)
        {
            if (n < 4)
            {
                double s = row[0]; int si = 0;
                for (int c = 1; c < n; c++) if (row[c] < s) { s = row[c]; si = c; }
                bestC = si; bestVal = s; return;
            }
            double m0 = row[0], m1 = row[1], m2 = row[2], m3 = row[3];
            int i0 = 0, i1 = 1, i2 = 2, i3 = 3;
            int c4 = 4;
            for (; c4 + 4 <= n; c4 += 4)
            {
                double v0 = row[c4], v1 = row[c4 + 1], v2 = row[c4 + 2], v3 = row[c4 + 3];
        ...
        internal static unsafe void RowArgMaxScan(double* row, int n, out int bestC, out double bestVal)
        {
            if (n < 4)
            {
                double s = row[0]; int si = 0;
        ...
        public static int rowArgMin(in doubleMxN A, ref Indices colIndexPerRow, ref doubleN valPerRow)
        {
            ...
            unsafe
            {
                ...
                {
                    RowArgMinScan(ap + (long)r * nc, nc, out int bestC, out double best);
        ...
        public static int rowArgMin(in doubleMxN A, ref Indices colIndexPerRow)
        {
            ...
            unsafe
            {
                ...
                for (int r = 0; r < A.M_Rows; r++)
                {
                    RowArgMinScan(ap + (long)r * nc, nc, out int bestC, out double _);
                    colIndexPerRow[r] = bestC;
                }
            }
            return A.M_Rows;
        ...
        public static int rowArgMax(in doubleMxN A, ref Indices colIndexPerRow, ref doubleN valPerRow)
        {
            ...
            unsafe
            {
                ...
                for (int r = 0; r < A.M_Rows; r++)
                {
                    RowArgMaxScan(ap + (long)r * nc, nc, out int bestC, out double best);
                    colIndexPerRow[r] = bestC;
                    valPerRow[r] = best;
                }
            }
        ...
        public static int rowArgMax(in doubleMxN A, ref Indices colIndexPerRow)
        {
            ...
            unsafe
            {
                ...
                for (int r = 0; r < A.M_Rows; r++)
                {
                    RowArgMaxScan(ap + (long)r * nc, nc, out int bestC, out double _);
                    colIndexPerRow[r] = bestC;
                }
            }
            return A.M_Rows;
        ...
        internal static void AllColScores(in doubleMxN A, in doubleN q, Metric m, double normQ, ref doubleN dest)
        {
            int nc = A.N_Cols, mR = A.M_Rows;
            unsafe
            {
                double* ap = A.Data.Ptr; double* qp = q.Data.Ptr; double* dp = dest.Data.Ptr;
                for (int c = 0; c < nc; c++) dp[c] = (double)0;

                if (m == Metric.Manhattan)
                {
                    for (int r = 0; r < mR; r++)
                    {
                        double* row = ap + (long)r * nc; double qr = qp[r];
                ...
                else if (m == Metric.Euclidean || m == Metric.SqEuclidean)
                {
                    ...
                    {
                        double* row = ap + (long)r * nc; double qr = qp[r];
                        for (int c = 0; c < nc; c++) { double d = row[c] - qr; dp[c] += d * d; }
                    }
                ...
                {
                    var normAv = new doubleN(nc, Allocator.Temp);
                    double* nap = normAv.Data.Ptr;
                    for (int c = 0; c < nc; c++) nap[c] = (double)0;
                    for (int r = 0; r < mR; r++)
                    {
                        double* row = ap + (long)r * nc; double qr = qp[r];
                    ...
                    for (int c = 0; c < nc; c++)
                    {
                        double denom = math.sqrt(nap[c] * normQ);
                        dp[c] = denom > (double)0 ? dp[c] / denom : (double)0;
                    }
                    normAv.Dispose();
                }
            }
        ...
        public static void nearestColumn(in doubleMxN A, in doubleN q, Metric m, out int index, out double score)
        {
            ...
            double normQ = m == Metric.Cosine ? QueryNormSq(in q) : (double)0;
            var scores = new doubleN(A.N_Cols, Allocator.Temp);
            AllColScores(in A, in q, m, normQ, ref scores);
            double best = doubleQueryCore.WorstScoreForNearest(m);
            int bestIdx = 0;
            for (int c = 0; c < A.N_Cols; c++)
                if (doubleQueryCore.IsBetterForNearest(scores[c], best, m)) { best = scores[c]; bestIdx = c; }
            scores.Dispose();
            index = bestIdx;
            score = best;
        }
        ...
        public static void farthestColumn(in doubleMxN A, in doubleN q, Metric m, out int index, out double score)
        {
            ...
            int worstIdx = 0;
            for (int c = 0; c < A.N_Cols; c++)
                if (doubleQueryCore.IsBetterForFarthest(scores[c], worst, m)) { worst = scores[c]; worstIdx = c; }
            scores.Dispose();
            index = worstIdx;
            score = worst;
        }
