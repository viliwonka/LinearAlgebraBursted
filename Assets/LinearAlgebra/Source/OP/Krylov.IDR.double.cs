...
namespace BULA
{
    public static partial class Krylov {
        ...
        /// </summary>
        public static SolveInfo idr<TOp, TPre>(in TOp A, in TPre M, in doubleN b, ref doubleN x,
            ...
            where TPre : struct, IdoublePreconditioner
        {
            ...
            var P = new UnsafeList<doubleN>(s, Allocator.Temp);
            var G = new UnsafeList<doubleN>(s, Allocator.Temp);
            var U = new UnsafeList<doubleN>(s, Allocator.Temp);
            for (int i = 0; i < s; i++)
            {
                P.Add(new doubleN(n, Allocator.Temp, true));
                G.Add(new doubleN(n));
                U.Add(new doubleN(n));
            }
            ...
            {
                doubleN Pi = P[i];
                for (int col = 0; col < n; col++) Pi[col] = rng.NextDouble();
            }

            var R = new doubleN(n);
            var V = new doubleN(n);
            var Q = new doubleN(n);
            doubleN VHat = default;
            if (!M.IsIdentity) VHat = new doubleN(n);

            var Msys = new doubleMxN(s, s, Allocator.Temp, false);
            for (int i = 0; i < s; i++) Msys[i, i] = (double)1;
            var f = new doubleN(s);
            var c = new doubleN(s);
            ...
            while (iter < maxIter && !done)
            {
                ...
                for (int k = 0; k < s && iter < maxIter; k++)
                {
                    ...
                    for (int i = k; i < s && !breakdown; i++)
                    {
                        ...
                        double dii = Msys[i, i];
                        if (dii == (double)0 || math.isnan(dii)) { breakdown = true; break; }
                        c[i] = sum / dii;
                    }
                    ...
                    V.scaleAddInPlace((double)(-1), R);   // V = R - V
                    ...
                    for (int i = 0; i < k && !breakdown; i++)
                    {
                        double dii = Msys[i, i];
                        if (dii == (double)0 || math.isnan(dii)) { breakdown = true; break; }
                        double alpha = Blas.dot(P[i], Gk) / dii;
                    ...
                    double dkk = Msys[k, k];
                    if (dkk == (double)0 || math.isnan(dkk))
                    {
                    ...
                    if (rr <= threshold)
                    {
                        ...
                        double trueRR = VerifyTrueResidual(in A, in b, in x, ref V, ref Q);
                        R.CopyFrom(in Q);
                        rr = trueRR;
        ...
        /// <see cref="idr{TOp, TPre}(in TOp, in TPre, in doubleN, ref doubleN, int, int, double, uint)"/>
        /// with the identity preconditioner (whose IsIdentity fold strips the M⁻¹ applies and the
        /// VHat workspace).
        /// </summary>
        public static SolveInfo idr<TOp>(in TOp A, in doubleN b, ref doubleN x, int s, int maxIter, double tol, uint seed = 0x9E3779B1u)
            where TOp : struct, IdoubleLinearOperator
        {
            return idr(in A, default(doubleIdentityPreconditioner), in b, ref x, s, maxIter, tol, seed);
        }

        /// <summary>IDR(s) over a dense <see cref="doubleMxN"/>. Forwards via <see cref="doubleDenseOperator"/>.</summary>
        public static SolveInfo idr(in doubleMxN A, in doubleN b, ref doubleN x, int s, int maxIter, double tol, uint seed = 0x9E3779B1u)
            => idr(new doubleDenseOperator(in A), in b, ref x, s, maxIter, tol, seed);

        /// <summary>IDR(s) over a dense matrix with defaults (s = 4, maxIter = A.M_Rows, tol = Consts.doubleSqrtEps, seed = default).</summary>
        public static SolveInfo idr(in doubleMxN A, in doubleN b, ref doubleN x)
            => idr(in A, in b, ref x, 4, A.M_Rows, Consts.doubleSqrtEps);

        /// <summary>IDR(s) over a block-sparse (BSR) matrix. Forwards via <c>doubleBSROperator</c>.</summary>
        public static SolveInfo idr(in doubleBSR A, in doubleN b, ref doubleN x, int s, int maxIter, double tol, uint seed = 0x9E3779B1u)
            => idr(new doubleBSROperator(in A), in b, ref x, s, maxIter, tol, seed);

        /// <summary>IDR(s) over a BSR matrix with defaults (s = 4, maxIter = A.M_Rows, tol = Consts.doubleSqrtEps, seed = default).</summary>
