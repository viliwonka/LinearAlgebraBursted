...
namespace LinearAlgebra
{
    public static partial class Krylov {
        ...
        // stored at index i of a length-s floatN; these helpers apply one such scalar to a single row
        // of an s x n block.

        // Y[row,:] += coeff * T[row,:].
        static void RowAddScaled(ref floatMxN Y, int row, float coeff, in floatMxN T, int n)
        {
            for (int c = 0; c < n; c++) Y[row, c] += coeff * T[row, c];
        }

        // Y[row,:] = coeff*Y[row,:] + T[row,:].
        static void RowScaleAddSelf(ref floatMxN Y, int row, float coeff, in floatMxN T, int n)
        {
            for (int c = 0; c < n; c++) Y[row, c] = coeff * Y[row, c] + T[row, c];
        }

        // Y[row,:] *= scale.
        static void RowScaleInPlace(ref floatMxN Y, int row, float scale, int n)
        {
            for (int c = 0; c < n; c++) Y[row, c] *= scale;
        }

        // dot(P[row,:], Q[row,:]).
        static float RowDot(in floatMxN P, in floatMxN Q, int row, int n)
        {
            float acc = (float)0;
        ...
        static int CountConvergedByBound(in floatN bound, in floatN thresh, int s, out double maxRnorm)
        {
        ...
        /// the preconditioner (<see cref="IfloatPreconditioner"/>). No published true (subspace-mixing)
        ...
        public static BlockSolveInfo btfqmr<TOp, TPre>(in TOp A, in TPre M, in floatMxN B, ref floatMxN X,
                                         ...
                                         ref floatMxN AU, ref floatMxN D, ref floatMxN UHat,
            ...
            where TPre : struct, IfloatPreconditioner
        {
            ...
            float bbAll = BlockFrobDot(in B, in B);
            if (bbAll == (float)0)
            {
            ...
            for (int i = 0; i < s; i++)
                for (int c = 0; c < n; c++) D[i, c] = (float)0;

            var theta = new floatN(s);
            var eta = new floatN(s);
            var alpha = new floatN(s);
            var rho = new floatN(s);
            var rhoLast = new floatN(s);
            for (int i = 0; i < s; i++)
            {
                theta[i] = (float)0; eta[i] = (float)0; alpha[i] = (float)0;
                rho[i] = tau[i] * tau[i];       // <Rhat0[i], Rhat0[i]> == tau[i]^2 (shadow == r0)
                rhoLast[i] = rho[i];
            }

            var vtrstar = new floatN(s);
            var rhoNew = new floatN(s);
            var beta = new floatN(s);
            ...
            for (int k = 0; k < maxIter; k++)
            {
                ...
                for (int i = 0; i < s; i++)
                {
                    ...
                    float dCoeff = (theta[i] * theta[i] / alpha[i]) * eta[i];
                    if (M.IsIdentity) RowScaleAddSelf(ref D, i, dCoeff, in U, n);
                    else              RowScaleAddSelf(ref D, i, dCoeff, in UHat, n);

                    float thetaI = math.sqrt(RowDot(in W, in W, i, n)) / tau[i];
                    float c = (float)1 / math.sqrt((float)1 + thetaI * thetaI);
                    ...
                    bound[i] = tau[i] * math.sqrt((float)(k + 1));
                }
        ...
        public static BlockSolveInfo btfqmr<TOp>(in TOp A, in floatMxN B, ref floatMxN X,
                                         ref floatMxN Rhat0, ref floatMxN U, ref floatMxN W, ref floatMxN V,
                                         ref floatMxN AU, ref floatMxN D,
                                         int maxIter, float tol)
            where TOp : struct, IfloatLinearOperator
        {
            floatMxN UHat = default;
            return btfqmr(in A, default(floatIdentityPreconditioner), in B, ref X, ref Rhat0, ref U, ref W, ref V, ref AU, ref D, ref UHat, maxIter, tol);
        }

        /// <summary>Pseudo-block TFQMR over a dense NON-symmetric <see cref="floatMxN"/> A, via
        ...
        public static BlockSolveInfo btfqmr(in floatMxN A, in floatMxN B, ref floatMxN X,
                                    ref floatMxN Rhat0, ref floatMxN U, ref floatMxN W, ref floatMxN V,
                                    ref floatMxN AU, ref floatMxN D,
                                    int maxIter, float tol)
        {
            return btfqmr(new floatDenseOperatorGeneral(in A), in B, ref X, ref Rhat0, ref U, ref W, ref V, ref AU, ref D, maxIter, tol);
        }

        /// <summary>Pseudo-block TFQMR over a dense non-symmetric A -- allocates block scratch from the
        /// arena.</summary>
        public static BlockSolveInfo btfqmr(in floatMxN A, in floatMxN B, ref floatMxN X, int maxIter, float tol)
        {
            int s = B.M_Rows, n = A.M_Rows;
            floatMxN Rhat0 = B.floatTempMat(s, n, true), U = B.floatTempMat(s, n, true), W = B.floatTempMat(s, n, true),
                      V = B.floatTempMat(s, n, true), AU = B.floatTempMat(s, n, true), D = B.floatTempMat(s, n, true);
        ...
        /// tol (Consts.floatSqrtEps).</summary>
        public static BlockSolveInfo btfqmr(in floatMxN A, in floatMxN B, ref floatMxN X)
        ...
        /// Forwards into <see cref="btfqmr{TOp}"/> via <c>floatBSROperator</c>.</summary>
        public static BlockSolveInfo btfqmr(in floatBSR A, in floatMxN B, ref floatMxN X,
                                    ref floatMxN Rhat0, ref floatMxN U, ref floatMxN W, ref floatMxN V,
                                    ref floatMxN AU, ref floatMxN D,
                                    int maxIter, float tol)
        {
            return btfqmr(new floatBSROperator(in A), in B, ref X, ref Rhat0, ref U, ref W, ref V, ref AU, ref D, maxIter, tol);
        }

        /// <summary>Pseudo-block TFQMR over a BSR non-symmetric A -- allocates block scratch from the
        /// arena.</summary>
        public static BlockSolveInfo btfqmr(in floatBSR A, in floatMxN B, ref floatMxN X, int maxIter, float tol)
        {
            int s = B.M_Rows, n = A.M_Rows;
            floatMxN Rhat0 = B.floatTempMat(s, n, true), U = B.floatTempMat(s, n, true), W = B.floatTempMat(s, n, true),
                      V = B.floatTempMat(s, n, true), AU = B.floatTempMat(s, n, true), D = B.floatTempMat(s, n, true);
                               ...
                               int maxIter, float tol)
        {
            ...
            floatMxN Rhat0 = B.floatTempMat(s, n, true), U = B.floatTempMat(s, n, true), W = B.floatTempMat(s, n, true),
                      V = B.floatTempMat(s, n, true), AU = B.floatTempMat(s, n, true), D = B.floatTempMat(s, n, true),
                      UHat = B.floatTempMat(s, n, true);
            return btfqmr(new floatBSROperator(in A), in M, in B, ref X,
