...
namespace LinearAlgebra
{
    ...
    /// require a twiddle-table workspace (built once via <c>arena.doubleFFTCache(n)</c>); see the workspace
    ...
    /// divides by N. Typical DSP pipeline: window (Hann) → rfft → powerSpectrum. double-only.
    /// </summary>
        ...
        static void DftCore(in doubleN inRe, in doubleN inIm, ref doubleN outRe, ref doubleN outIm, bool inverse)
        {
            ...
            for (int k = 0; k < n; k++)
            {
                ...
                for (int t = 0; t < n; t++)
                {
                    ...
                    double c = DetMath.Cos(ang);
                    double s = DetMath.Sin(ang);
                    double xr = inRe[t];
                    double xi = inIm[t];
                    // (xr + i·xi)·(c + i·s)
                    sumRe += xr * c - xi * s;
