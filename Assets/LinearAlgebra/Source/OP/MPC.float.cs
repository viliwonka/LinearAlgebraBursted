...
namespace BULA.Control
{
    ...
    public static partial class MPC
    {
        ...
        public static void setSoftBound(ref floatMPCState s, in floatN d)
        {
            if (!s.IsCreated)
                throw new ArgumentException("MPC.setSoftBound: state must be constructed via new floatMPCState(...)");
            if (!s.hasSoftRows)
            ...
            if (s.hasSoftRows)
            {
                ...
                for (int k = 0; k < N; k++)
                {
                    for (int si = 0; si < s.nSoftPerStage; si++)
                    {
                        ...
                        float viol = act - s.rowConstRHS[rowIdx];
