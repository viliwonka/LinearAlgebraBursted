    ...
    public static unsafe partial class UnsafeOP {
        ...
        public static float sumAbs([NoAlias] float* a, int n)
        {
            ...
            
            {
                ...
                for (; q + 2 <= nW; q += 2)
                {
                    acc0 += floatW.Abs(floatW.Load(a, q));
                    acc1 += floatW.Abs(floatW.Load(a, q + 1));
                }
                if (q < nW) acc0 += floatW.Abs(floatW.Load(a, q));
        ...
        public static float sum([NoAlias] float* a, int n)
        {
            ...
            
            {
                ...
                for (; q + 2 <= nW; q += 2)
                {
                    acc0 += floatW.Load(a, q);
                    acc1 += floatW.Load(a, q + 1);
                }
                if (q < nW) acc0 += floatW.Load(a, q);
                head = floatW.HSum(acc0 + acc1);
                i = nW * floatW.Width;
            }
        ...
        public static float maxAbs([NoAlias] float* a, int n)
        {
            ...
            
            {
                ...
                for (; q + 2 <= nW; q += 2)
                {
                    acc0 = floatW.Max(acc0, floatW.Abs(floatW.Load(a, q)));
                    acc1 = floatW.Max(acc1, floatW.Abs(floatW.Load(a, q + 1)));
                }
                if (q < nW) acc0 = floatW.Max(acc0, floatW.Abs(floatW.Load(a, q)));
                head = floatW.HMax(floatW.Max(acc0, acc1));
                i = nW * floatW.Width;
            }
        ...
        public static float max([NoAlias] float* a, int n)
        {
...

            {
                ...
                if (nW > 0)
                {
                    floatW acc0 = floatW.Load(a, 0), acc1 = acc0;
                    int q = 1;
                    for (; q + 2 <= nW; q += 2)
                    {
                        acc0 = floatW.Max(acc0, floatW.Load(a, q));
                        acc1 = floatW.Max(acc1, floatW.Load(a, q + 1));
                    }
                    if (q < nW) acc0 = floatW.Max(acc0, floatW.Load(a, q));
                    m = math.max(m, floatW.HMax(floatW.Max(acc0, acc1)));
                    i = nW * floatW.Width;
                }
            }
            ...
            for (; i + 8 <= n; i += 8)
            {
                qacc0 = math.max(qacc0, *(float4*)(a + i));
                qacc1 = math.max(qacc1, *(float4*)(a + i + 4));
            }
            if (i + 4 <= n) { qacc0 = math.max(qacc0, *(float4*)(a + i)); i += 4; }
            float4 qacc = math.max(qacc0, qacc1);
        ...
        public static float min([NoAlias] float* a, int n)
        {
...

            {
                ...
                if (nW > 0)
                {
                    ...
                    for (; q + 2 <= nW; q += 2)
                    {
                        acc0 = floatW.Min(acc0, floatW.Load(a, q));
                        acc1 = floatW.Min(acc1, floatW.Load(a, q + 1));
                    }
                    if (q < nW) acc0 = floatW.Min(acc0, floatW.Load(a, q));
                    m = math.min(m, floatW.HMin(floatW.Min(acc0, acc1)));
                    i = nW * floatW.Width;
                }
            }
            ...
            for (; i + 8 <= n; i += 8)
            {
                qacc0 = math.min(qacc0, *(float4*)(a + i));
                qacc1 = math.min(qacc1, *(float4*)(a + i + 4));
            }
            if (i + 4 <= n) { qacc0 = math.min(qacc0, *(float4*)(a + i)); i += 4; }
            float4 qacc = math.min(qacc0, qacc1);
        ...
        public static void matVecDot([NoAlias] float* mat, [NoAlias] float* x, [NoAlias] float* y, int m, int n)
        {
            ...
            for (int r = 0; r < m; r++)
            {
                ...
                if (n >= 128)
                {
                    ...
                    for (; q + 2 <= nW; q += 2)
                    {
                        acc0 += floatW.Load(row, q)     * floatW.Load(x, q);
                        acc1 += floatW.Load(row, q + 1) * floatW.Load(x, q + 1);
                    }
                    if (q < nW) acc0 += floatW.Load(row, q) * floatW.Load(x, q);
                    head = floatW.HSum(acc0 + acc1);
                    i = nW * floatW.Width;
                }
                ...
                for (; i + 8 <= n; i += 8)
                {
                    qacc0 += *(float4*)(row + i)     * *(float4*)(x + i);
                    qacc1 += *(float4*)(row + i + 4) * *(float4*)(x + i + 4);
                }
                if (i + 4 <= n)
                {
                    qacc0 += *(float4*)(row + i) * *(float4*)(x + i);
                ...
                float4 qacc = qacc0 + qacc1;
                float sum = head + ((qacc.x + qacc.y) + (qacc.z + qacc.w));
        ...
        public static void matTrans([NoAlias] float* matA, [NoAlias] float* matB, int m, int n)
        {
            ...
            // matB = n x m, out
            const int TB = 16;
            float* buf = stackalloc float[TB * TB];
            ...
            for (int rb = 0; rb < m; rb += TB)
            {
                ...
                for (int cb = 0; cb < n; cb += TB)
                {
                    ...
                    for (int c = cb; c < cEnd; c++)
                    {
                        float* Brow = matB + (long)c * m + rb;
                        float* bufRow = buf + (c - cb) * TB;
        ...
        [MethodImpl(MethodImplOptions.NoInlining)]
        public static void matMatDotPacked([NoAlias] float* matA, [NoAlias] float* matB, [NoAlias] float* matC, int m, int n, int k)
        {
            const int MR = 8;
            const int NR = 16;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static void matMatDotTransACore(float* matA, float* matB, float* matC, int m, int n, int k, bool symUpper)
        {
        ...
        // Rows are mutually independent, so groups of floatW.Width rows solve simultaneously —
        ...
        public static void trsmLowerPanel([NoAlias] float* L11, int Lld, [NoAlias] float* B, int Bld, int nrows, int jb)
        {
            ...
            const int W = floatW.Width;
            float* tile = stackalloc float[jb * W];

            for (; t + W <= nrows; t += W)
            {
                for (int l = 0; l < W; l++)
                {
                    float* Brow = B + (long)(t + l) * Bld;
                    for (int k = 0; k < jb; k++) tile[k * W + l] = Brow[k];
                }

                for (int p = 0; p < jb; p++)
                {
                    floatW v = floatW.Load(tile, p);
                    float* L11row = L11 + (long)p * Lld;
                    for (int k = 0; k < p; k++)
                        v -= floatW.Splat(L11row[k]) * floatW.Load(tile, k);
                    floatW.Store(tile, p, v / floatW.Splat(L11row[p]));
                }
        ...
        public static void syrkLowerSub([NoAlias] float* Lp, int n, int rStart, int j0, int jb, [NoAlias] float* PT)
        {
            ...
            {
                int i = rStart + ip;
                float* Lrow = Lp + (long)i * n + rStart; // write cols [rStart, i], relative [0, ip]
        ...
        public static void syrkUpperSub([NoAlias] float* Wp, int n, int rStart, int j0, int jb, [NoAlias] float* QT)
        {
            ...
            {
                int i = rStart + ip;
                float* Wrow = Wp + (long)i * n + i;     // write cols [i, n)
                float* Qi = QT + (long)ip * jb;
                int len = n - i;
                int p = 0;
                for (; p + 4 <= jb; p += 4)              // one Wrow pass per FOUR panel rows
                {
                    float* u0 = Wp + (long)(j0 + p) * n + rStart + ip;
        ...
        public static void wyVtC([NoAlias] float* Vp, int Vld, [NoAlias] float* Cp, int Cld, int rows, int pb, int cw, [NoAlias] float* W)
        {
            ...
            {
                float* V0 = Vp + (long)t * Vld;
                float* C0 = Cp + (long)t * Cld;
                for (int i = 0; i < pb; i++)
        ...
        [MethodImpl(MethodImplOptions.NoInlining)]
        public static void signFlipInPlace([NoAlias] float* target, int n) {
        ...
        [MethodImpl(MethodImplOptions.NoInlining)]
        public static void axpy4([NoAlias] float* y,
            [NoAlias] float* x0, [NoAlias] float* x1, [NoAlias] float* x2, [NoAlias] float* x3,
            float a0, float a1, float a2, float a3, int n) {
        ...
        [MethodImpl(MethodImplOptions.NoInlining)]
        public static void compMul([NoAlias] float* target, [NoAlias] float* from, int n)
        {
            for (int i = 0; i < n; i++)
                target[i] *= from[i];
        }
        ...
        // target[i] = s
        [MethodImpl(MethodImplOptions.NoInlining)]
        public static void fill([NoAlias] float* target, int n, float s)
        {
            for (int i = 0; i < n; i++)
                target[i] = s;
        }
