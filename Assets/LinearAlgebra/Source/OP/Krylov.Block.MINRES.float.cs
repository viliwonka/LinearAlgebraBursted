...
namespace LinearAlgebra
{
    public static partial class Krylov {
        ...
        static void CopyRowsAt(in floatMxN src, ref floatMxN dst, int rowOffset, int rows, int cols)
        {
        ...
        static void CopyColsAt(in floatMxN src, ref floatMxN dst, int colOffset, int rows, int cols)
        {
            for (int r = 0; r < rows; r++)
                for (int c = 0; c < cols; c++)
        ...
        static void CopyBlockAt(in floatMxN src, int srcRowOff, int srcColOff, ref floatMxN dst, int rows, int cols)
        {
            for (int r = 0; r < rows; r++)
                for (int c = 0; c < cols; c++)
        ...
        {
            int s2 = 2 * s;
            var Y = new floatMxN(s2, s, Allocator.Temp, true);
            CopyRowsAt(in Gbar, ref Y, 0, s, s);
            for (int r = 0; r < s; r++)
                for (int c = 0; c < s; c++)
                    Y[s + r, c] = Beta[c, r];   // Beta^T

            var Qy = new floatMxN(s2, s, Allocator.Temp, true);
            QR.decomp(in Y, ref Qy, ref Gamma);

            bool gammaOk = true;
            for (int i = 0; i < s; i++)
                if (math.abs(Gamma[i, i]) <= Consts.floatZeroThreshold) { gammaOk = false; break; }

            var Z0 = new floatMxN(s2, s, Allocator.Temp, true);
            var T = new floatMxN(s, s, Allocator.Temp, true);
            var QyT = new floatMxN(s2, s, Allocator.Temp, true);
            var Z1 = new floatMxN(s2, s, Allocator.Temp, true);
            var Qperp = new floatMxN(s2, s, Allocator.Temp, true);
            ...
            where TPre : struct, IfloatPreconditioner
        {
            ...
            bool allZero = true;
            for (int j = 0; j < s; j++) if (thr[j] != (float)0) { allZero = false; break; }
            if (allZero)
            {
