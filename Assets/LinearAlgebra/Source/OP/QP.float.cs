// <auto-generated>
//   Generated from Assets/LinearAlgebra/CodeGen/TemplateSource/OP/QP.fProxy.cs
...
namespace BULA
{
    ...
    public static partial class QP
    {
        ...
        /// scratch: reaches a feasible point via <see cref="LQ.minNormSolve(in floatMxN, in floatN, ref floatN)"/>
                                               ...
                                               bool useIncrementalReduced = true)
        {
            ...
            var wsf = floatQPFactorState.Create(n);
            ...
            var red = useIncrementalReduced ? floatQPReducedState.Create(n) : default;
        ...
        /// </summary>
        internal static QPInfo qpActiveSetCoreWarmPersistent(in floatMxN Q, in floatN c, in floatMxN A, in floatN b,
                                                             in NativeArray<ConstraintSense> senses,
                                                             in floatN xl, in floatN xu,
                                                             ref floatN x, out double objective, int maxIter,
                                                             NativeArray<byte> wstatusPersist, out int workingSetChanges,
                                                             ref floatQPFactorState wsf, ref floatQPReducedState red,
                                                             NativeArray<int> qpMeta)
        {
            ...
            bool incremental = factorValid
                               && wsf.deadCount + numDrops < floatQPFactorState.DeadCap
                               && numDrops + numAdds <= floatQPFactorState.DeadCap;
        ...
        internal static void ComputeTargetStatus(in floatN L, in floatN U, int m, int n, int T,
                                                 in floatN x0, in floatN Ax0, NativeArray<byte> wstatusPrev,
                                                 NativeArray<byte> wstatus, float feasTol)
        {
        ...
        internal static QPInfo qpActiveSetLoop(in floatMxN Q, in floatN c, in floatMxN A,
                                               ...
                                               NativeArray<byte> wstatus, ref floatQPFactorState wsf,
                                               ref floatQPReducedState red, bool useIncrementalReduced,
                                               ref floatN x, int maxIter,
            ...
            while (true)
            {
                ...
                if (haveNullSpace)
                {
                    ...
                    if (useIncrementalReduced)
                    {
                        ...
                        if (red.stale || red.changeCount >= floatQPReducedState.RebuildCap)
                ...
                else if (exitStatus == null && doMultiplierCheck)
                {
                    ...
                    else
                    {
                        ...
                        if (wsf.deadCount >= floatQPFactorState.DeadCap)
                        {
        ...
        //     floatQPFactorState.Create allocates: reflectors <= n + DeadCap, rotations <=
        //     DeadCap·(n-1). The refactor cost O(nk²) amortizes to O(nk²/DeadCap) per drop.
        //
        // The state struct itself (floatQPFactorState, with Create/Dispose) lives at namespace level
        ...
        internal static void RefactorWorkingSet(in floatMxN A, int m, int n, int T, NativeArray<byte> wstatus,
                                                ref floatQPFactorState s, float zeroThresholdAW)
        {
        ...
        internal static void ApplyFactorQtForward(ref floatQPFactorState s, ref floatN v)
        {
            int n = s.n;
            for (int op = 0; op < s.opCount; op++)
            {
                ...
                else
                {
                    int gi = s.opArg[op];
                    int i = s.rotRow[gi];
                    float cc = s.rotC[gi], sn = s.rotS[gi];
                    float t1 = v[i], t2 = v[i + 1];
                    v[i] = cc * t1 + sn * t2;
                    v[i + 1] = cc * t2 - sn * t1;
                }
            }
        ...
        internal static bool TryAddToFactor(in floatMxN A, int m, int n, int t, WorkingSetStatus candStatus,
                                            ref floatQPFactorState s, float zeroThresholdAW)
        {
            int k = s.k;

            // candidate column of A_Wᵀ -- WriteWorkingSetColumn's sign orientation, vector form
            float sign = candStatus == WorkingSetStatus.ActiveUpper ? (float)(-1) : (float)1;
            if (t < m)
                for (int i = 0; i < n; i++) s.vCol[i] = sign * A[t, i];
            else
            {
                for (int i = 0; i < n; i++) s.vCol[i] = (float)0;
                s.vCol[t - m] = sign;
            }

            ApplyFactorQtForward(ref s, ref s.vCol);

            float xNorm = Norms.L2Range(in s.vCol, k, n);
            ...
            float div = math.sqrt(math.abs(s.uRefl[k]));
        ...
        // once deadCount reaches floatQPFactorState.DeadCap (the log-capacity contract -- see Create).
        internal static void DropFromFactor(int col, ref floatQPFactorState s)
        {
            int k = s.k;
            ...
            {
                float a = s.R[i, i], bsub = s.R[i + 1, i];
                if (bsub == (float)0) continue;   // already triangular here -- no rotation needed
                float r2 = Helpers.pythag(a, bsub);
                float cc = a / r2, sn = bsub / r2;
                s.R[i, i] = r2;
                s.R[i + 1, i] = (float)0;
                for (int c2 = i + 1; c2 < k - 1; c2++)
                {
                    float t1 = s.R[i, c2], t2 = s.R[i + 1, c2];
        ...
        internal static void FormNullSpaceBasisFromFactor(ref floatQPFactorState s, ref floatMxN Z)
        {
        ...
        internal static void SolveFactorRUpper(ref floatQPFactorState s, ref floatN lam)
        {
            int k = s.k;
            for (int i = k - 1; i >= 0; i--)
            {
        ...
        //
        // The state struct (floatQPReducedState, Create/Dispose) lives at namespace level below, next
        // to floatQPFactorState, for the same dtype-collision reason.
        // ============================================================================================
        ...
        internal static void RebuildReduced(in floatMxN Q, ref floatQPFactorState s, ref floatQPReducedState red)
        {
            ...
            var Zt = new floatMxN(n, nz, Allocator.Temp, true);
            FormNullSpaceBasisFromFactor(ref s, ref Zt);
            var QZt = new floatMxN(n, nz, Allocator.Temp, true);
            Blas.dot(in Q, in Zt, ref QZt);
            var Hzt = new floatMxN(nz, nz, Allocator.Temp, true);
        ...
        // fully overwritten.
        internal static void FormNullSpaceColumn(ref floatQPFactorState s, int seedRow, ref floatN col)
        {
            int n = s.n;
            for (int i = 0; i < n; i++) col[i] = (float)0;
            col[seedRow] = (float)1;
        ...
        internal static void UpdateReducedOnAdd(ref floatQPFactorState s, ref floatQPReducedState red)
        {
            if (red.stale) return;
            int n = s.n;
            int kOld = s.k - 1;
            ...
            for (int i = 0; i < n; i++)
            {
                float acc1 = (float)0, acc2 = (float)0;
            ...
            for (int i = 0; i < nzOld; i++) red.zv[i] -= (float)0.5 * beta * red.u[i];
        ...
        internal static void UpdateReducedOnDrop(in floatMxN Q, ref floatQPFactorState s, ref floatQPReducedState red)
        {
            if (red.stale) return;
            int n = s.n, kNew = s.k;
            int nzOld = n - kNew - 1;
        ...
        internal static DirectSolveInfo SolveReducedNewtonStepCached(ref floatQPReducedState red, int nz,
                                                                     ref floatN gz, ref floatN y, float normInfQ, out bool regularized)
        {
            var Hs = new floatMxN(nz, nz, Allocator.Temp, true);
            for (int i = 0; i < nz; i++)
    ...
    internal struct floatQPFactorState
    {
        // Drops allowed between full refactors; also sizes Create's log capacities.
        public const int DeadCap = 8;

        public floatMxN V;                 // reflector store, n x (n + DeadCap); column r holds reflector r's vector on rows [reflStart[r], n)
        public floatMxN R;                 // n x n buffer; the leading k x k upper triangle is R
        ...
        public floatN rotC, rotS;          // rotation g's cosine / sine
        public floatN vCol, uRefl, wApply; // scratch (candidate column / staged reflector / apply accumulator), all length n
        ...
        public static floatQPFactorState Create(int n, Allocator alloc = Allocator.Temp)
        {
            int reflCap = n + DeadCap;
            int rotCap = DeadCap * math.max(n, 1);
            return new floatQPFactorState
            {
                n = n,
                V = new floatMxN(n, reflCap, alloc, true),
                R = new floatMxN(n, n, alloc, true),
                ...
                rotC = new floatN(rotCap, alloc, false),
                rotS = new floatN(rotCap, alloc, false),
                vCol = new floatN(n, alloc, false),
                uRefl = new floatN(n, alloc, false),
                wApply = new floatN(n, alloc, false),
            };
        }
    ...
    // the same dtype-collision reason as floatQPFactorState (proxy-free-signature members cannot live
    // on the shared partial QP class).
    internal struct floatQPReducedState
    {
        // Incremental changes allowed before a from-scratch rebuild (roundoff-drift bound).
        public const int RebuildCap = 16;

        public floatMxN Z;          // null-space basis, live block n x nz
        public floatMxN QZ;         // Q·Z, live block n x nz
        public floatMxN Hz;         // ZᵀQZ, live block nz x nz
        public floatN u, zv, qzv;   // scratch, length n (reflector tail / Z·û then H_Z·û,r / QZ·û)
        ...
        public static floatQPReducedState Create(int n, Allocator alloc = Allocator.Temp)
        {
            return new floatQPReducedState
            {
                n = n,
                stale = true,
                ...
                Z = new floatMxN(n, n, alloc, true),
                QZ = new floatMxN(n, n, alloc, true),
                Hz = new floatMxN(n, n, alloc, true),
                u = new floatN(n, alloc, true),
                zv = new floatN(n, alloc, true),
                qzv = new floatN(n, alloc, true),
            };
        }
