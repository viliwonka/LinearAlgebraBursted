...
namespace LinearAlgebra
{
    ...
    /// table via CosQ at build time. Build once via Arena.floatFFTCache(n) and reuse across many
    ...
    {
        public floatN twQuarter;  // length n/4+1: cos(2π·j/n), j = 0..n/4  (first quadrant, incl. 0)
        ...
        public floatN cw1re, cw1im;

        /// <summary>
        /// Standalone allocation sized and populated identically to <c>Arena.floatFFTCache(n)</c>
        /// (same twiddle-table construction, on standalone buffers). Pair with <see cref="Dispose"/>.
        /// </summary>
        public unsafe floatFFTCache(int n, Allocator allocator)
        {
            if (n < 2 || (n & (n - 1)) != 0)
            ...
            int half = n >> 1;
            int Q    = n >> 2;
            twQuarter = new floatN(Q + 1, allocator);
            ...
            for (int m = 0; m < Q; m++)
                twQuarter[m] = (float)dre[m];
            twQuarter[Q] = (Q >= 1) ? (float)0 : (float)1;
            UnsafeUtility.Free(dre, Allocator.Persistent);
            UnsafeUtility.Free(dim, Allocator.Persistent);

            cz      = new floatN(half, allocator, uninit: true);
            sz      = new floatN(half, allocator, uninit: true);
            visited = new floatN(n,    allocator, uninit: true);

            float* cq = twQuarter.Data.Ptr;
            swLen = 0;
            for (int qq = 1; 4 * qq <= n; qq <<= 2)
                swLen += qq;
            int swAlloc = swLen > 0 ? swLen : 1;
            sw1re = new floatN(swAlloc, allocator, uninit: true);
            sw1im = new floatN(swAlloc, allocator, uninit: true);
            {
                int off = 0;
                for (int qq = 1; 4 * qq <= n; qq <<= 2)
                {
                    ...
                    {
                        int t1 = j * step;
                        FFT.WQ(cq, t1, n, out float wr, out float wi);
            ...
            cw1re = new floatN(cwLen, allocator, uninit: true);
            cw1im = new floatN(cwLen, allocator, uninit: true);
            for (int k = 0; k < cwLen; k++)
            {
                FFT.WQ(cq, k * cwStep, n, out float wr, out float wi);
    ...
    public static partial class ArenaExtensions
    {
        ...
        /// no recurrence drift); see <see cref="floatFFTCache"/> for the quarter-circle table layout.
        ///
        ...
        public static unsafe floatFFTCache floatFFTCache(this ref Arena arena, int n)
        {
            ...
            var twQuarter = arena.floatVec(Q + 1);   // cos(2π·j/n), j = 0..Q
            ...
            // float once. Only the first QUADRANT [0, n/4) is built (doubling stops two stages early,
            ...
            for (int m = 0; m < Q; m++)
                twQuarter[m] = (float)dre[m];
            twQuarter[Q] = (Q >= 1) ? (float)0 : (float)1;   // cos(π/2)=0 (n>=4); cos(0)=1 (n=2)
            UnsafeUtility.Free(dre, Allocator.Persistent);
            UnsafeUtility.Free(dim, Allocator.Persistent);
            ...
            // floatW stages and the small scalar stages) gets its W^1 = (Re,Im)(W^(j·step)),
            ...
            int cwLen = (P & 1) == 0 ? (n >> 2) : half;
            int cwStep = (P & 1) == 0 ? 2 : 1;
            var cw1re = arena.floatVec(cwLen, uninit: true);
            var cw1im = arena.floatVec(cwLen, uninit: true);
            for (int k = 0; k < cwLen; k++)
            {
                FFT.WQ(cq, k * cwStep, n, out float wr, out float wi);
                cw1re[k] = wr; cw1im[k] = wi;
    ...
    public static partial class FFT
    {
        ...
        static void RequireRadix4Workspace(in floatFFTCache ws, int n, string who)
        {
            ...
            if (ws.twQuarter.N != (n >> 2) + 1)
                throw new ArgumentException(
                    who + ": workspace must have quarter-circle twiddle table (use Arena.floatFFTCache(n))");
        }
        ...
        internal static unsafe float CosQ(float* c, int idx, int tableN)
        {
            int Q = tableN >> 2;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal static unsafe void WQ(float* c, int idx, int tableN, out float wr, out float wi)
        {
            ...
            wi = tableN >= 4 ? CosQ(c, idx + (tableN >> 2), tableN) : (float)0;
        }
        ...
        /// (build via Arena.floatFFTCache(N)); it must contain the quarter-circle twiddle table required
        ...
        /// </summary>
        public static unsafe void rfft(in floatN real, ref floatN re, ref floatN im, in floatFFTCache ws)
        {
            int n = real.N;
            ...
            var cz = ws.cz;   // floatN of length n/2 = M (packed real parts)
            var sz = ws.sz;   // floatN of length n/2 = M (packed imag parts)
            var twQuarter = ws.twQuarter;
            float* cqPtr = twQuarter.Data.Ptr;   // used by the unpack below
            float* czp   = cz.Data.Ptr;
            float* szp   = sz.Data.Ptr;
            float* realp = real.Data.Ptr;
            float* s1rp  = ws.sw1re.Data.Ptr;
            float* s1ip  = ws.sw1im.Data.Ptr;
            ...
            else
            {
                ...
                float* cw1rp = ws.cw1re.Data.Ptr;
                float* cw1ip = ws.cw1im.Data.Ptr;
                int deHalf = M >> 1;
                for (int j = 0; j < M; j++)
                {
        ...
        /// </summary>
        public static unsafe void irfft(in floatN re, in floatN im, ref floatN real, in floatFFTCache ws)
        {
            int halfSpec = re.N;
            if (im.N != halfSpec)
            ...
            //   Y[k] = E[k] + i·O[k]
            var twQuarter    = ws.twQuarter;
            float* cqPtr = twQuarter.Data.Ptr;
            var sw1re    = ws.sw1re;
            var sw1im    = ws.sw1im;
            var cw1re    = ws.cw1re;
            var cw1im    = ws.cw1im;
            ...
            for (int k = 1; k < half; k++)
            {
                ...
                float cIm = -cImPos;   // negate imaginary part for conjugate
        ...
        // in-register — no runtime twiddle reconstruction. Stages with quarter-stride q >= floatW.Width
        ...
        static unsafe void FftCoreRadix4Ptr(
            ...
            [NoAlias] float* s1r, [NoAlias] float* s1i, int n)
        {
            ...
            for (int q = 1; q < n; q <<= 2)
            {
                ...
                if (q >= W)
                {
                    ...
                    for (int base_ = 0; base_ < n; base_ += len)
                    {
                        for (int j = 0; j < q; j += W)
                        {
                            ...
                            floatW Are = floatW.Load(re + i0, 0), Aim = floatW.Load(im + i0, 0);
                            ...
                            floatW w2r = w1r * w1r - w1i * w1i, w2i = w1r * w1i + w1i * w1r;
                            floatW w3r = w1r * w2r - w1i * w2i, w3i = w1r * w2i + w1i * w2r;

                            floatW x1r = floatW.Load(re + i1, 0), x1i = floatW.Load(im + i1, 0);
                            floatW Bre = w1r * x1r - w1i * x1i;
                            floatW Bim = w1r * x1i + w1i * x1r;

                            floatW x2r = floatW.Load(re + i2, 0), x2i = floatW.Load(im + i2, 0);
                            floatW Cre = w2r * x2r - w2i * x2i;
                            floatW Cim = w2r * x2i + w2i * x2r;

                            floatW x3r = floatW.Load(re + i3, 0), x3i = floatW.Load(im + i3, 0);
                            ...
                            floatW.Store(re + i0, 0, T0re + T2re); floatW.Store(im + i0, 0, T0im + T2im);
                            floatW.Store(re + i2, 0, T0re - T2re); floatW.Store(im + i2, 0, T0im - T2im);
                            floatW.Store(re + i1, 0, T1re + T3im); floatW.Store(im + i1, 0, T1im - T3re);
                            floatW.Store(re + i3, 0, T1re - T3im); floatW.Store(im + i3, 0, T1im + T3re);
                        }
                    }
                }
                else
                {
                    ...
                    for (int j = 0; j < q; j++)
                    {
                        float w1r = s1r[stageOff + j], w1i = s1i[stageOff + j];
                        float w2r = w1r * w1r - w1i * w1i, w2i = w1r * w1i + w1i * w1r;
        ...
        static unsafe void FftCoreRadix4(ref floatN re, ref floatN im,
                                         ref floatN sw1re, ref floatN sw1im,
                                         bool inverse)
        {
            int size = re.N;
            if (size == 1) return;
        ...
        static unsafe void FftCoreRadix4Mixed(ref floatN re, ref floatN im,
                                              ref floatN sw1re, ref floatN sw1im,
                                              ref floatN cw1re, ref floatN cw1im,
                                              floatN visited,
                                              bool inverse)
        {
            int size = re.N;
        ...
        static unsafe void MixedDeinterleave(float* re, float* im, float* visited, int size)
        {
            int M = size >> 1;
                                                  ...
                                                  int size, bool inverse, float* interleaveOut)
        {
            ...
            for (; k + Wc <= M; k += Wc)
            {
                floatW wr = floatW.Load(cw1r + k, 0), wi = floatW.Load(cw1i + k, 0);
                floatW er  = floatW.Load(rePtr + k, 0),     ei  = floatW.Load(imPtr + k, 0);
                floatW or_ = floatW.Load(rePtr + M + k, 0), oi_ = floatW.Load(imPtr + M + k, 0);
                floatW tr  = wr * or_ - wi * oi_;
                floatW ti  = wr * oi_ + wi * or_;
                floatW.Store(rePtr + k, 0,     er + tr); floatW.Store(imPtr + k, 0,     ei + ti);
                floatW.Store(rePtr + M + k, 0, er - tr); floatW.Store(imPtr + M + k, 0, ei - ti);
            }
            for (; k < M; k++)
            {
                float wr = cw1r[k], wi = cw1i[k];
                float er  = rePtr[k],     ei  = imPtr[k];
                float or_ = rePtr[M + k], oi_ = imPtr[M + k];
