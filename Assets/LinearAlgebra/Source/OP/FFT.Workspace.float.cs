...
namespace LinearAlgebra
{
    ...
    {
        public floatN twReFull;   // length n:   cos(-2π·m/n), m = 0..n-1    (full circle)
        public floatN twImFull;   // length n:   sin(-2π·m/n); the half-circle uses (rfft/irfft
        ...
        // Contiguous per-stage W^1 twiddle table for the wide (floatW) radix-4 butterfly: stages
        // with quarter-stride q >= floatW.Width (8 float / 4 double lanes), concatenated in stage
    ...
    public static partial class ArenaExtensions
    {
        ...
        public static unsafe floatFFTCache floatFFTCache(this ref Arena arena, int n)
        {
            ...
            // so error stays O(log n · ε). Kept in a double scratch and cast to float once, so the
            ...
            for (int m = 0; m < n; m++)
            {
                twReFull[m] = (float)dre[m];
                twImFull[m] = (float)dim[m];
            }
            ...
            // (floatW) radix-4 butterfly. Every power-of-two n gets one: the wide butterfly drives
            ...
            // qq < floatW.Width stay scalar and are skipped here.
            int swLen = 0;
            ...
            if ((P & 1) == 0)
            {
                ...
                cw1re = arena.floatVec(cwLen, uninit: true);
                cw1im = arena.floatVec(cwLen, uninit: true);
    ...
    public static partial class FFT
    {
        ...
        public static void rfft(in floatN real, ref floatN re, ref floatN im, in floatFFTCache ws)
        {
            ...
            for (int k = 1; k < M; k++)
            {
                ...
                float curRe = twReFull[k];   // W_N^k real part (first half of full table)
                float curIm = twImFull[k];   // W_N^k imaginary part
        ...
        public static void irfft(in floatN re, in floatN im, ref floatN real, in floatFFTCache ws)
        {
            ...
            for (int k = 1; k < M; k++)
            {
                ...
                float cRe = twReFull[k];
                float cIm = -twImFull[k];   // negate imaginary part for conjugate
        ...
        // q >= floatW.Width vectorize across j (Width consecutive j give contiguous wide re/im loads
        ...
        static unsafe void FftCoreRadix4Ptr(
            ...
            [NoAlias] float* twr, [NoAlias] float* twi,
            [NoAlias] float* s1r, [NoAlias] float* s1i, int n, int tableN)
        {
