...
namespace BULA
{
    ...
    /// over <c>TDesign : struct, IfloatLadDesign</c>, so each call compiles to a direct call and one
    /// core body serves both dense and block-sparse designs.
    ///
    /// Deliberately NOT <see cref="IfloatLinearOperator"/>: that interface also requires ApplyDot and
    ...
    /// <see cref="Sparse.IfloatStandardFormOperator"/> being its own interface.
    /// </summary>
    public interface IfloatLadDesign
    {
        ...
        void ApplyT(in floatN y, ref floatN x);
        ...
        /// </summary>
        void WeightedGram(in floatN q, ref floatMxN M);
    }

    /// <summary>Dense <see cref="floatMxN"/> design.</summary>
    public readonly struct floatDenseLadDesign : IfloatLadDesign
    {
        readonly floatMxN A;

        public floatDenseLadDesign(in floatMxN a) { A = a; }

        public int Rows => A.M_Rows;
        public int Cols => A.N_Cols;

        public unsafe void Apply(in floatN x, ref floatN y)
        {
            int m = A.M_Rows, n = A.N_Cols;
            float* Ap = A.Data.Ptr; float* xp = x.Data.Ptr; float* yp = y.Data.Ptr;
            UnsafeUtility.MemClear(yp, (long)m * UnsafeUtility.SizeOf<float>());
            for (int i = 0; i < m; i++) yp[i] = UnsafeOP.vecDot(Ap + (long)i * n, xp, n);
        }

        public unsafe void ApplyT(in floatN y, ref floatN x)
        {
            int m = A.M_Rows, n = A.N_Cols;
            float* Ap = A.Data.Ptr; float* yp = y.Data.Ptr; float* xp = x.Data.Ptr;
            UnsafeUtility.MemClear(xp, (long)n * UnsafeUtility.SizeOf<float>());
            for (int i = 0; i < m; i++)
            {
                float v = yp[i];
        ...
        public unsafe void WeightedGram(in floatN q, ref floatMxN M)
        {
            int m = A.M_Rows, n = A.N_Cols;
            for (int r = 0; r < n; r++)
                for (int c = r; c < n; c++)
    ...
    /// <summary>
    /// Block-sparse <see cref="floatBSR"/> design. A x and Aᵀ y go through the library's block spMV /
    ...
    /// is cheap here.
    /// </summary>
    public readonly struct floatBsrLadDesign : IfloatLadDesign
    {
        readonly floatBSR A;

        public floatBsrLadDesign(in floatBSR a) { A = a; }

        public int Rows => A.M_Rows;
        public int Cols => A.N_Cols;

        public void Apply(in floatN x, ref floatN y) => BSR.spMV(in A, in x, ref y);
        public void ApplyT(in floatN y, ref floatN x) => BSR.spMVT(in A, in y, ref x);
        ...
        public unsafe void WeightedGram(in floatN q, ref floatMxN M)
        {
            ...
            for (int br = 0; br < A.BlockRows; br++)
            {
                ...
                {
                    float qi = q[br * BR + r];
                    if (qi == (float)0) continue;

                    for (int ka = k0; ka < k1; ka++)
                    {
                        float* rowA = vals + (long)ka * blockSize + (long)r * BC;
                        ...
                        for (int ca = 0; ca < BC; ca++)
                        {
                            ...
                            {
                                float* rowB = vals + (long)kb * blockSize + (long)r * BC;
