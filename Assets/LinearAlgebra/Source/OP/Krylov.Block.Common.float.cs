...
namespace BULA
{
    public static partial class Krylov {
        ...
        // (blsmr/bcraig/bcraigmr).
        static void BlockLstsqExit<TOp>(in TOp A, in floatMxN X, in floatMxN B, ref floatMxN Rfinal,
                                        IterativeSolveStatus status, int s, int m,
                                        ref floatN rowN, ref floatN rowM, out double maxr, out int converged)
        ...
        static void BuildColumnThresholds(in floatMxN B, ref floatN thr, int s, int n, float tol)
        {
            ...
            float absScale = (float)64 * Consts.floatEpsilon;
            float floorSq = absScale * absScale * bbFrob;
            float t2 = tol * tol;
            for (int j = 0; j < s; j++)
        ...
        static void BuildColumnThresholdsPlain(in floatMxN B, ref floatN thr, int s, int n, float tol)
        {
            for (int j = 0; j < s; j++)
            {
                float bb = (float)0;
        ...
        // LQRPRank exactly.
        static int LQRPRankFloored(in floatMxN L, int m, int nGlobal, float scale)
        {
            if (!math.isfinite(L[0, 0])) return 0;
            float relTol = (float)math.max(m, nGlobal) * Consts.floatZeroThreshold;
            float tol = math.max(relTol * math.abs(L[0, 0]), relTol * scale);
            int rank = 0;
            for (int i = 0; i < m; i++)
            {
                if (math.abs(L[i, i]) > tol) rank++;
        ...
        static void CopyBlockFromTransposed(in floatMxN Src, int rowOff, int colOff, ref floatMxN Dst, int size)
        {
        ...
        static void CopyBlockFrom(in floatMxN Src, int rowOff, int colOff, ref floatMxN Dst, int rows, int cols)
        {
            for (int i = 0; i < rows; i++)
        ...
        static void CopyBlockInto(ref floatMxN Dst, int rowOff, int colOff, in floatMxN Src, int rows, int cols)
        {
            for (int i = 0; i < rows; i++)
                for (int j = 0; j < cols; j++)
        ...
        static void AddBlockInto(ref floatMxN dst, int rowOff, int colOff, in floatMxN src, int rows, int cols)
        {
        ...
        static void CopyRowsFrom(in floatMxN src, int rowOff, int rows, ref floatMxN dst)
        {
            int cols = dst.N_Cols;
            for (int a = 0; a < rows; a++)
        ...
        static void BlockResidual<TOp>(in TOp A, in floatMxN X, in floatMxN B, ref floatMxN applied, ref floatMxN R, int s, int n)
            where TOp : struct, IfloatLinearOperator
        {
        ...
        static void BlockResidual<TOp>(in TOp A, in floatMxN X, in floatMxN B, ref floatMxN R, int s, int n)
            where TOp : struct, IfloatLinearOperator
        {
        ...
        static unsafe int RowOrthoRankFloored(in floatMxN W, ref floatMxN Qout, int m, int nGlobal, float scale)
        {
            if (m <= 0) return 0;
            int n = W.N_Cols;
            float* q = Qout.Data.Ptr;
            UnsafeUtility.MemCpy(q, W.Data.Ptr, (long)m * n * UnsafeUtility.SizeOf<float>());
            ...
            int pivotRow = 0;
            float pivotNormSq = UnsafeOP.vecDot(q, q, n);
            for (int i = 1; i < m; i++)
            {
                float ns = UnsafeOP.vecDot(q + (long)i * n, q + (long)i * n, n);
            ...
            for (int d = 0; d < m; d++)
            {
                ...
                float pivotNorm = math.sqrt(pivotNormSq);
                if (d == 0)
                {
                ...
                float tol = math.max(relTol * diag0, relTol * scale);
                if (!(pivotNorm > tol))
                    break;
                rank = d + 1;

                float* qd = q + (long)d * n;
                float inv = (float)1 / pivotNorm;
                ...
                int nextPivot = d + 1;
                float nextNormSq = (float)(-1);
                for (int i = d + 1; i < m; i++)
                {
                    float* qi = q + (long)i * n;
                    float dot = UnsafeOP.vecDot(qi, qd, n);
                    float ns = UnsafeOP.axpyNormSq(qi, qd, -dot, n);
        ...
        static int BlockArnoldiMGS2Step(ref floatMxN Wj, in UnsafeList<floatMxN> V, ref Indices w, ref Indices off,
                                         ref floatMxN Hbuf, ref floatMxN HijBuf, ref floatMxN Tbuf,
        ...
        static bool BlockLSResolveAndCheck(in floatMxN Hbuf, in Indices off, int k,
                                            ...
                                            in floatN thr, int s, out bool lsBreakdown)
        {
