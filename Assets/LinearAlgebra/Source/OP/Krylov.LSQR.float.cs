...
namespace LinearAlgebra
{
    public static partial class Krylov {
        ...
        /// </summary>
        public static LstsqInfo lsqrRightPreOp<TPreN>(in floatMxN A, in TPreN N, in floatN b, ref floatN x, int maxIter, float tol, float damp)
            where TPreN : struct, IfloatLinearOperator
            => LsqrRightPreOpCore(new floatDenseOperator(in A), in N, in b, ref x, maxIter, tol, damp);

        /// <summary>Undamped general right-preconditioned LSQR over a dense matrix (damp = 0).</summary>
        public static LstsqInfo lsqrRightPreOp<TPreN>(in floatMxN A, in TPreN N, in floatN b, ref floatN x, int maxIter, float tol)
            where TPreN : struct, IfloatLinearOperator
            => LsqrRightPreOpCore(new floatDenseOperator(in A), in N, in b, ref x, maxIter, tol, (float)0);

        /// <summary>General right-preconditioned LSQR (dense), default maxIter (A.N_Cols) / tol (Consts.floatSqrtEps).</summary>
        public static LstsqInfo lsqrRightPreOp<TPreN>(in floatMxN A, in TPreN N, in floatN b, ref floatN x)
            where TPreN : struct, IfloatLinearOperator
            => LsqrRightPreOpCore(new floatDenseOperator(in A), in N, in b, ref x, A.N_Cols, Consts.floatSqrtEps, (float)0);

        /// <summary>General right-preconditioned damped LSQR over a BSR matrix (materializes Aᵀ once)
        /// -- same contract as the dense overload.</summary>
        public static LstsqInfo lsqrRightPreOp<TPreN>(in floatBSR A, in TPreN N, in floatN b, ref floatN x, int maxIter, float tol, float damp)
            where TPreN : struct, IfloatLinearOperator
        {
            floatBSR AT = b.floatBSRTranspose(in A);
            return LsqrRightPreOpCore(new floatBSROperator(in A, in AT), in N, in b, ref x, maxIter, tol, damp);
        }

        /// <summary>Undamped general right-preconditioned LSQR over a BSR matrix (damp = 0, materializes Aᵀ once).</summary>
        public static LstsqInfo lsqrRightPreOp<TPreN>(in floatBSR A, in TPreN N, in floatN b, ref floatN x, int maxIter, float tol)
            where TPreN : struct, IfloatLinearOperator
            => lsqrRightPreOp(in A, in N, in b, ref x, maxIter, tol, (float)0);

        /// <summary>General right-preconditioned LSQR (BSR), default maxIter (A.N_Cols) / tol (Consts.floatSqrtEps).</summary>
        public static LstsqInfo lsqrRightPreOp<TPreN>(in floatBSR A, in TPreN N, in floatN b, ref floatN x)
            where TPreN : struct, IfloatLinearOperator
            => lsqrRightPreOp(in A, in N, in b, ref x, A.N_Cols, Consts.floatSqrtEps, (float)0);
        ...
        static LstsqInfo LsqrRightPreOpCore<TOp, TPreN>(in TOp Aop, in TPreN N, in floatN b, ref floatN x, int maxIter, float tol, float damp)
            where TOp : struct, IfloatLinearOperator
            where TPreN : struct, IfloatLinearOperator
        {
            int m = Aop.Rows, n = Aop.Cols;
