...
namespace LinearAlgebra
{
    ...
    public static partial class LP
    {
        ...
        /// <see cref="lad(in doubleMxN, in doubleN, ref doubleN, out double, int)"/> for the
        /// size-routed hybrid default, and
        /// <see cref="ladFN(in doubleMxN, in doubleN, ref doubleN, out double, int)"/> for the
        ...
        public static LPInfo ladBR(in doubleMxN A, in doubleN b, ref doubleN x, out double objective,
                                   int maxIter = 0)
        {
        ...
        /// classical LAD's symmetric [-0.5, 0.5]). tau = 0.5 reproduces <see cref="ladBR(in doubleMxN, in doubleN, ref doubleN, out double, int)"/>.
        /// </summary>
        /// <param name="A">Design matrix, m x n (m observations, n coefficients).</param>
        /// <param name="b">Observations, length m.</param>
                                                       ...
                                                       ref doubleN x, out double objective, int maxIter)
        {
            ...
            double oneMinusTau = (double)1 - tauF;
            ...
            double tol = (double)math.max(math.sqrt((double)Consts.doubleEpsilon), 1e-7);
            double pivTolFloor = math.max(Consts.doubleZeroThreshold, (double)1e-9);
            double zeroResidTol = pivTolFloor;
            ...
            var val = new doubleN(n, Allocator.Temp);
            var adjustedG = new doubleN(n, Allocator.Temp);
            var R = new doubleN(m, Allocator.Temp);
            var tcol = new doubleN(m, Allocator.Temp);
            var candBreak = new doubleN(m, Allocator.Temp);
            ...
            bool useBland = false;
            int cyclingSince = -1;
            var bestX = new doubleN(n, Allocator.Temp);
            double bestChk = 0;
            bool haveBest = false;
            ...
            while (iters < maxIter)
            {
                ...
                doubleMxN Bmat = default;
                Pivot piv = default;
                bool haveFactor = false;

                if (k > 0)
                {
                    Bmat = new doubleMxN(k, k, Allocator.Temp);
                    for (int a = 0; a < k; a++)
                        for (int c = 0; c < k; c++)
                    ...
                    var rhs = new doubleN(k, Allocator.Temp);
                    for (int a = 0; a < k; a++) rhs[a] = b[srows[a]];
                    LU.decompSolve(ref Bmat, in piv, ref rhs);
                    for (int a = 0; a < k; a++) x[placedCols[a]] = rhs[a];
                    rhs.Dispose();
                }
                for (int j = 0; j < n; j++) if (colRow[j] < 0) x[j] = (double)0;

                for (int j = 0; j < n; j++) g[j] = (double)0;
                double chk = 0;
                for (int i = 0; i < m; i++)
                {
                    ...
                    double r = b[i] - dot;
                    R[i] = r;
                    chk += r >= (double)0 ? tau * (double)r : (tau - 1.0) * (double)r;
                    double sg = r > zeroResidTol ? tauF : (r < -zeroResidTol ? tauF - (double)1 : (double)0);
                    if (sg != (double)0)
                ...
                for (int j = 0; j < n; j++) val[j] = (double)0;
                if (k > 0)
                {
                    var gp = new doubleN(k, Allocator.Temp);
                ...
                for (int j = 0; j < n; j++)
                {
                    if (colRow[j] >= 0) { adjustedG[j] = (double)0; continue; }
                    double adj = g[j];
                ...
                for (int j = 0; j < n; j++)
                {
                    ...
                    if (colRow[j] < 0)
                    {
                        double v = math.abs(adjustedG[j]);
                        if (v > bestViol)
                        {
                            bestViol = v; bestCol = j; bestDir = adjustedG[j] > (double)0 ? -1 : 1;
                            if (useBland) break;   // ascending j -> first hit is smallest index
                        }
                    }
                    else
                    {
                        double lo = -tauF, hi = oneMinusTau;
                        double vv = val[j];
                        if (vv < lo - tol)
                        {
                            double v = lo - vv;
                ...
                if (bestCol < 0)
                {
                    ...
                    bestCol = bc;
                    bestDir = adjustedG[bc] <= (double)0 ? 1 : -1;
                }
                ...
                if (placingNew)
                {
                    if (k > 0)
                    {
                        var scolVals = new doubleN(k, Allocator.Temp);
                ...
                else
                {
                    ...
                    var e = new doubleN(k, Allocator.Temp);
                    for (int a = 0; a < k; a++) e[a] = a == slot ? (double)1 : (double)0;
                ...
                for (int i = 0; i < m; i++)
                {
                    ...
                    candBreak[nCand] = math.max(ti, (double)0);
                    candRow[nCand] = i;
                    nCand++;
                }
                ...
                if (nCand == 0)
                {
                    ...
                    {
                        int forceRow = -1;
                        double forceMag = (double)0;
                        for (int i = 0; i < m; i++)
                        {
                            if (rowUsed[i]) continue;
                            double mag = math.abs(tcol[i]);
                ...
                else
                {
                    unsafe
                    {
                        UnsafeOP.sortByKeyAscending((double*)candBreak.Data.Ptr,
                                                    (int*)candRow.GetUnsafePtr(), nCand);
                    }

                    double cum = initSlope;
                    double stopWinTheta = (double)0;
                    for (int idx = 0; idx < nCand; idx++)
                    {
                        int row = candRow[idx];
                        double rate = dir * tcol[row];
                        cum += math.abs(rate);
                    ...
                    double tieTol = tol * ((double)1 + math.abs(stopWinTheta));
                    if (useBland)
                    {
                    ...
                    else
                    {
                        ...
                        for (int idx = 0; idx < nCand; idx++)
                        {
                            ...
                            {
                                double mag = math.abs(dir * tcol[row]);
                ...
                if (repeated)
                {
                    ...
                    else if (zeroResidTol < tol) zeroResidTol = math.min(zeroResidTol * (double)10, tol);
                }
            ...
            if (!acceptedBest)
            {
                ...
                if (k > 0)
                {
                    var Bmat2 = new doubleMxN(k, k, Allocator.Temp);
                    for (int a = 0; a < k; a++)
                        for (int c = 0; c < k; c++)
                    ...
                    {
                        var rhs2 = new doubleN(k, Allocator.Temp);
