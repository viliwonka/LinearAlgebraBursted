...
namespace BULA
{
    ...
    public static partial class Fit
    {
        ...
        ///
        /// Needs <see cref="IfloatWeighted3"/> rather than merely estimable, because the inner step is
        /// a weighted refit. <paramref name="innerIter"/> &lt;= 0 uses 4, which is where the classic
        /// result flattens out.
        /// </summary>
        public static RansacInfo ransacLo<TModel>(NativeArray<float3> points, ref TModel model,
            ...
            where TModel : struct, IfloatWeighted3
        {
            ...
            if (!(threshold > (float)0)) throw new ArgumentException("Fit.ransacLo: threshold must be positive");
        ...
        /// </summary>
        public static RansacInfo magsac<TModel>(NativeArray<float3> points, ref TModel model,
                                                float sigmaMax, int maxIter = 0, uint seed = 0,
            ...
            where TModel : struct, IfloatEstimable3
        {
            ...
            if (!(sigmaMax > (float)0)) throw new ArgumentException("Fit.magsac: sigmaMax must be positive");
            ...
            if (ok)
            {
                var keep = new NativeArray<float3>(n, Allocator.Temp);
                int cnt = 0;
                double t2 = smax * smax;
                for (int i = 0; i < n; i++)
                {
        ...
        static double MsacScore<TModel>(NativeArray<float3> points, in TModel model, double t2,
                                        out int inliers)
        ...
        /// <summary>2D <see cref="MsacScore{TModel}(NativeArray{float3}, in TModel, double, out int)"/>.</summary>
        static double MsacScore<TModel>(NativeArray<float2> points, in TModel model, double t2,
                                        out int inliers)
            where TModel : struct, IfloatShape2
        {
            var s = model;
        ...
        // ---- 2D solver overloads -------------------------------------------------------------------
        //
        // Same drivers, different point type. Duplicated because C# cannot abstract over float2 vs
        // float3 in a Burst-compatible way -- see Fit.Shapes2 for why. The SHAPES do not duplicate.
        ...
        public static RansacInfo ransac<TModel>(NativeArray<float2> points, ref TModel model,
            ...
            where TModel : struct, IfloatEstimable2
        {
            int n = points.Length;
            int m = model.MinimalSamples;
            ...
            var sample = new NativeArray<float2>(m, Allocator.Temp);
            var idx = new NativeArray<int>(m, Allocator.Temp);
            var best = new NativeArray<float2>(n, Allocator.Temp);
