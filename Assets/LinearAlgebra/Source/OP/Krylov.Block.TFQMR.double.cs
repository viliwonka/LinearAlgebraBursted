// <auto-generated>
//   Generated from Assets/LinearAlgebra/CodeGen/TemplateSource/OP/Krylov.Block.TFQMR.fProxy.cs
...
namespace LinearAlgebra
{
    public static partial class Krylov {
        ...
        // stored at index i of a length-s doubleN; these helpers apply one such scalar to a single row
        // of an s x n block.

        // Y[row,:] += coeff * T[row,:].
        static void RowAddScaled(ref doubleMxN Y, int row, double coeff, in doubleMxN T, int n)
        {
            for (int c = 0; c < n; c++) Y[row, c] += coeff * T[row, c];
        }

        // Y[row,:] = coeff*Y[row,:] + T[row,:].
        static void RowScaleAddSelf(ref doubleMxN Y, int row, double coeff, in doubleMxN T, int n)
        {
            for (int c = 0; c < n; c++) Y[row, c] = coeff * Y[row, c] + T[row, c];
        }

        // Y[row,:] *= scale.
        static void RowScaleInPlace(ref doubleMxN Y, int row, double scale, int n)
        {
            for (int c = 0; c < n; c++) Y[row, c] *= scale;
        }

        // dot(P[row,:], Q[row,:]).
        static double RowDot(in doubleMxN P, in doubleMxN Q, int row, int n)
        {
            double acc = (double)0;
        ...
        static bool AnyZeroOrNaNActive(in doubleN v, in NativeArray<bool> frozen, int s)
        {
            for (int i = 0; i < s; i++)
            {
                if (frozen[i]) continue;
                if (v[i] == (double)0 || math.isnan(v[i])) return true;
            }
            return false;
        }

        // True iff any NON-FROZEN entry in [0,s) is NaN.
        static bool AnyNaNActive(in doubleN v, in NativeArray<bool> frozen, int s)
        {
            for (int i = 0; i < s; i++)
            {
                if (frozen[i]) continue;
        ...
        static int CountConvergedByBound(in doubleN bound, in doubleN thresh, int s, out double maxRnorm)
        {
        ...
        /// the preconditioner (<see cref="IdoublePreconditioner"/>). No published true (subspace-mixing)
        ...
        /// </summary>
        public static BlockSolveInfo btfqmr<TOp, TPre>(in TOp A, in TPre M, in doubleMxN B, ref doubleMxN X,
                                         ref doubleMxN Rhat0, ref doubleMxN U, ref doubleMxN W, ref doubleMxN V,
                                         ref doubleMxN AU, ref doubleMxN D, ref doubleMxN UHat,
            ...
            where TPre : struct, IdoublePreconditioner
        {
            ...
            double bbAll = BlockFrobDot(in B, in B);
            if (bbAll == (double)0)
            {
            ...
            for (int i = 0; i < s; i++)
                for (int c = 0; c < n; c++) D[i, c] = (double)0;

            var theta = new doubleN(s);
            var eta = new doubleN(s);
            var alpha = new doubleN(s);
            var rho = new doubleN(s);
            var rhoLast = new doubleN(s);
            for (int i = 0; i < s; i++)
            {
                theta[i] = (double)0; eta[i] = (double)0; alpha[i] = (double)0;
                rho[i] = tau[i] * tau[i];       // <Rhat0[i], Rhat0[i]> == tau[i]^2 (shadow == r0)
                rhoLast[i] = rho[i];
            }

            var vtrstar = new doubleN(s);
            var rhoNew = new doubleN(s);
            var beta = new doubleN(s);
            ...
            for (int k = 0; k < maxIter; k++)
            {
                ...
                for (int i = 0; i < s; i++)
                {
                    ...
                    double dCoeff = (theta[i] * theta[i] / alpha[i]) * eta[i];
                    if (M.IsIdentity) RowScaleAddSelf(ref D, i, dCoeff, in U, n);
                    else              RowScaleAddSelf(ref D, i, dCoeff, in UHat, n);

                    double thetaI = math.sqrt(RowDot(in W, in W, i, n)) / tau[i];
                    double c = (double)1 / math.sqrt((double)1 + thetaI * thetaI);
                    ...
                    bound[i] = tau[i] * math.sqrt((double)(k + 1));
                }
                ...
                else
                {
                    ...
                    for (int i = 0; i < s; i++) { if (frozen[i]) continue; RowAddScaled(ref V, i, (double)1, in AU, n); }  // V += AU (now refreshed)
                }
            }
        ...
        public static BlockSolveInfo btfqmr<TOp>(in TOp A, in doubleMxN B, ref doubleMxN X,
                                         ref doubleMxN Rhat0, ref doubleMxN U, ref doubleMxN W, ref doubleMxN V,
                                         ref doubleMxN AU, ref doubleMxN D,
                                         int maxIter, double tol)
            where TOp : struct, IdoubleLinearOperator
        {
            doubleMxN UHat = default;
            return btfqmr(in A, default(doubleIdentityPreconditioner), in B, ref X, ref Rhat0, ref U, ref W, ref V, ref AU, ref D, ref UHat, maxIter, tol);
        }

        /// <summary>Pseudo-block TFQMR over a dense NON-symmetric <see cref="doubleMxN"/> A, via
        ...
        public static BlockSolveInfo btfqmr(in doubleMxN A, in doubleMxN B, ref doubleMxN X,
                                    ref doubleMxN Rhat0, ref doubleMxN U, ref doubleMxN W, ref doubleMxN V,
                                    ref doubleMxN AU, ref doubleMxN D,
                                    int maxIter, double tol)
        {
        ...
        public static BlockSolveInfo btfqmr(in doubleMxN A, in doubleMxN B, ref doubleMxN X, int maxIter, double tol)
        {
            ...
            doubleMxN Rhat0 = B.doubleTempMat(s, n, true), U = B.doubleTempMat(s, n, true), W = B.doubleTempMat(s, n, true),
                      V = B.doubleTempMat(s, n, true), AU = B.doubleTempMat(s, n, true), D = B.doubleTempMat(s, n, true);
        ...
        /// tol (Consts.doubleSqrtEps).</summary>
        public static BlockSolveInfo btfqmr(in doubleMxN A, in doubleMxN B, ref doubleMxN X)
            => btfqmr(in A, in B, ref X, A.M_Rows, Consts.doubleSqrtEps);

        /// <summary>Pseudo-block TFQMR over a block-sparse (BSR) non-symmetric A -- zero-alloc primitive.
        /// Forwards into <see cref="btfqmr{TOp}"/> via <c>doubleBSROperator</c>.</summary>
        public static BlockSolveInfo btfqmr(in doubleBSR A, in doubleMxN B, ref doubleMxN X,
                                    ref doubleMxN Rhat0, ref doubleMxN U, ref doubleMxN W, ref doubleMxN V,
                                    ref doubleMxN AU, ref doubleMxN D,
                                    int maxIter, double tol)
        {
        ...
        /// A with ANY <see cref="IdoublePreconditioner"/> (ILU0) -- forwards into
        /// <see cref="btfqmr{TOp,TPre}"/> via <c>doubleBSROperator</c>.</summary>
        public static BlockSolveInfo btfqmr<TPre>(in doubleBSR A, in TPre M, in doubleMxN B, ref doubleMxN X,
                               int maxIter, double tol)
            where TPre : struct, IdoublePreconditioner
        {
            int s = B.M_Rows, n = A.M_Rows;
            doubleMxN Rhat0 = B.doubleTempMat(s, n, true), U = B.doubleTempMat(s, n, true), W = B.doubleTempMat(s, n, true),
                      V = B.doubleTempMat(s, n, true), AU = B.doubleTempMat(s, n, true), D = B.doubleTempMat(s, n, true),
                      UHat = B.doubleTempMat(s, n, true);
            return btfqmr(new doubleBSROperator(in A), in M, in B, ref X,
                             ref Rhat0, ref U, ref W, ref V, ref AU, ref D, ref UHat,
                             maxIter, tol);
        }

        /// <summary>Preconditioned pseudo-block TFQMR over BSR with ANY <see cref="IdoublePreconditioner"/>
        /// (ILU0), with default maxIter (A.M_Rows) and tolerance (Consts.doubleSqrtEps).</summary>
        public static BlockSolveInfo btfqmr<TPre>(in doubleBSR A, in TPre M, in doubleMxN B, ref doubleMxN X)
            where TPre : struct, IdoublePreconditioner
            => btfqmr(in A, in M, in B, ref X, A.M_Rows, Consts.doubleSqrtEps);
    }
}
