// <auto-generated>
//   Generated from Assets/LinearAlgebra/CodeGen/TemplateSource/OP/Krylov.CGNE.fProxy.cs
//   DO NOT EDIT BY HAND - edit the template and run Tools/regen.ps1.
// </auto-generated>
using System;
using Unity.Mathematics;
using LinearAlgebra.Sparse;

namespace LinearAlgebra
{
    public static partial class Krylov {

        /// <summary>
        /// Zero-alloc CGNE (Conjugate Gradient on the Normal Equations, second kind; Saad,
        /// "Iterative Methods for Sparse Linear Systems," Algorithm 8.5 -- CGNE / Craig's method)
        /// solver for UNDERDETERMINED consistent systems A x = b (A Rows &lt;= Cols), generic over
        /// <see cref="IfloatLinearOperator"/>. Solves A Aᵀ y = b by CG and forms x = Aᵀ y
        /// implicitly, MATRIX-FREE: A Aᵀ is never assembled -- each iteration costs exactly one
        /// A.Apply and one A.ApplyT. Requires A to have full row rank and b ∈ range(A) (a
        /// consistent system) -- on a square nonsingular A it recovers the unique solution.
        ///
        /// Every search direction p_k is built as an Aᵀ-image (p_0 = Aᵀr_0, p_{k+1} = Aᵀr_{k+1} +
        /// beta_k p_k), so x = Σ alpha_k p_k stays in row(A) at every iteration by construction --
        /// the same minimum-Euclidean-norm characterization x* = Aᵀ(AAᵀ)⁻¹b that
        /// <see cref="craig{TOp}"/> computes via Golub-Kahan bidiagonalization, here via a direct
        /// CG recurrence on the (never-formed) AAᵀ -- cheaper and simpler per step, at CG's usual
        /// κ² conditioning sensitivity. O(m+n) memory and per-iteration cost.
        ///
        /// No warm start (unlike lsqr/lsmr): the min-norm characterization only holds starting
        /// from x₀ = 0, so x is zeroed internally regardless of its incoming contents.
        ///
        /// Converges when ‖b-Ax‖ &lt;= tol*‖b‖. Returns an <see cref="LstsqInfo"/> -- see that
        /// struct for the implicit-bool/status/undefined-x contract; rnorm/Arnorm/xnorm are filled
        /// by a certified-exact <see cref="lstsqResidual{TOp}"/> audit (one extra Apply + ApplyT)
        /// rather than the tracked CG residual, since the recurrence's r can drift from the true
        /// ‖b-Ax‖ under accumulated rounding. A claimed Converged is reconciled against that
        /// certified rnorm (no extra matvec beyond the audit already paid for) before being
        /// reported -- falls through to another iteration if the tracked estimate turned out
        /// optimistic. Breakdown when p collapses (r numerically orthogonal to row(A)) before
        /// ‖b-Ax‖ reaches tolerance -- A lacks full row rank (or, on the very first step, b lies
        /// outside its row space).
        /// </summary>
        public static LstsqInfo cgne<TOp>(in TOp A, in floatN b, ref floatN x,
                                     ref floatN r, ref floatN p, ref floatN Ap, ref floatN tmpN,
                                     int maxIter, float tol)
            where TOp : struct, IfloatLinearOperator
        {
            if (A.Rows > A.Cols) throw new ArgumentException("cgne: A must be square or underdetermined (Rows <= Cols)");
            if (b.N != A.Rows) throw new ArgumentException("cgne: b.N must equal A.Rows");
            if (x.N != A.Cols) throw new ArgumentException("cgne: x.N must equal A.Cols");
            if (r.N != A.Rows) throw new ArgumentException("cgne: r.N must equal A.Rows");
            if (p.N != A.Cols) throw new ArgumentException("cgne: p.N must equal A.Cols");
            if (Ap.N != A.Rows) throw new ArgumentException("cgne: Ap.N must equal A.Rows");
            if (tmpN.N != A.Cols) throw new ArgumentException("cgne: tmpN.N must equal A.Cols");

            if (maxIter < 1)
                throw new ArgumentException("cgne: maxIter must be >= 1");

            unsafe
            {
                long* ptrs = stackalloc long[6];
                ptrs[0] = (long)r.Data.Ptr; ptrs[1] = (long)p.Data.Ptr;
                ptrs[2] = (long)Ap.Data.Ptr; ptrs[3] = (long)tmpN.Data.Ptr;
                ptrs[4] = (long)x.Data.Ptr; ptrs[5] = (long)b.Data.Ptr;
                RequireDistinctBuffers("cgne: r/p/Ap/tmpN/x/b must be distinct", ptrs, 6);
            }

            // No warm start: the min-norm characterization x = Aᵀ(AAᵀ)⁻¹b requires x₀ = 0.
            for (int i = 0; i < x.N; i++) x[i] = (float)0;

            float bb = Blas.dot(b, b);
            if (bb == (float)0)
                // b = 0: the min-norm solution is trivially x = 0.
                return LstsqInfoAudited(IterativeSolveStatus.Converged, 0, in A, in b, ref x, ref Ap, ref tmpN);

            // r_0 = b - A x_0 = b (x_0 = 0), so ‖r_0‖² == bb -- no extra dot needed.
            r.CopyFrom(in b);
            float rr = bb;

            float threshold = tol * tol * bb;
            if (rr <= threshold)
                return LstsqInfoAudited(IterativeSolveStatus.Converged, 0, in A, in b, ref x, ref Ap, ref tmpN);

            // p_0 = Aᵀ r_0 -- an Aᵀ-image, so every later p_k (built below as Aᵀr_k + beta·p_{k-1})
            // stays in row(A) by induction, keeping x = Σ alpha_k p_k row(A)-minimal throughout.
            A.ApplyT(in r, ref p);

            for (int k = 0; k < maxIter; k++)
            {
                float pp = Blas.dot(p, p);
                if (!(pp > (float)0)) // NaN-safe: pp is a norm, nonnegative
                    // p collapsed before ‖r‖ reached tolerance: r is (numerically) orthogonal to
                    // row(A) -- A lacks full row rank (or b escaped range(A)). x is the previous
                    // iterate (undefined per the Breakdown contract).
                    return LstsqInfoAudited(IterativeSolveStatus.Breakdown, k, in A, in b, ref x, ref Ap, ref tmpN);

                float alpha = rr / pp;

                // x += alpha p ; r -= alpha (A p) ; rr = ‖r‖², fused into one pass.
                A.Apply(in p, ref Ap);
                float rrNew = Blas.updateXR(alpha, p, ref x, Ap, ref r);

                if (rrNew <= threshold)
                {
                    // Verify-at-exit: the CG recurrence's tracked r can drift from the true
                    // residual over many iterations (accumulated rounding in the axpy chain).
                    // CGNEInfo already pays for a certified-exact ‖b-Ax‖ audit (lstsqResidual: one
                    // Apply + one ApplyT) for every exit regardless of status -- reuse THAT instead
                    // of a redundant matvec: only commit to Converged if the certified residual
                    // also clears the threshold. Ap/tmpN are safe to reuse either way (fully
                    // overwritten by the audit before their next read, so a fall-through loses
                    // nothing).
                    var info = LstsqInfoAudited(IterativeSolveStatus.Converged, k + 1, in A, in b, ref x, ref Ap, ref tmpN);
                    if (info.rnorm * info.rnorm <= threshold)
                        return info;
                }

                // beta_k = ‖r_{k+1}‖² / ‖r_k‖² ; p_{k+1} = Aᵀ r_{k+1} + beta_k p_k.
                float beta = rrNew / rr;
                A.ApplyT(in r, ref tmpN);
                p.scaleAddInPlace(beta, tmpN);

                rr = rrNew;
            }

            return LstsqInfoAudited(IterativeSolveStatus.MaxIterations, maxIter, in A, in b, ref x, ref Ap, ref tmpN);
        }

        /// <summary>
        /// Tikhonov-DAMPED CGNE: minimum-norm solve of the regularized underdetermined system,
        /// x = Aᵀ(A Aᵀ + damp²·I)⁻¹ b (ridge-regularized least-norm; equivalently the least-norm
        /// solution of the augmented system [A | damp·I]·(x,s) = b). Reduces to plain
        /// <see cref="cgne{TOp}(in TOp, in floatN, ref floatN, ref floatN, ref floatN, ref floatN, ref floatN, int, float)"/>
        /// when damp == 0 (delegated, bit-identical -- ps/s unused). MATRIX-FREE: runs CGNE on the
        /// augmented operator [A | damp·I], so A Aᵀ + damp²·I is never assembled; each iteration
        /// costs exactly one A.Apply + one A.ApplyT, as undamped.
        ///
        /// Unlike undamped CGNE, damping does NOT require A to have full row rank or b ∈ range(A):
        /// A Aᵀ + damp²·I is SPD for any A when damp ≠ 0, so the iteration is always well-posed.
        /// Needs two extra Rows-length scratch vectors over the undamped primitive: ps (the
        /// Rows-space component of the augmented search direction) and s (the auxiliary variable,
        /// used only for the fresh augmented-residual convergence check).
        ///
        /// Convergence is decided on the AUGMENTED residual ‖b - A x - damp·s‖ ≤ tol·‖b‖ (which →0
        /// at the regularized solution), verified fresh at exit. In the returned
        /// <see cref="LstsqInfo"/>, rnorm is the UNDAMPED residual ‖b - A x‖ (= damp·‖s‖ at the
        /// optimum -- legitimately NONZERO, mirroring damped lsqr/lsmr), so a Converged exit here
        /// carries a nonzero rnorm; Arnorm is the Tikhonov gradient ‖Aᵀr - damp²x‖, which DOES →0 at
        /// the optimum. Read the implicit bool / status (or Arnorm), not rnorm, for success.
        /// </summary>
        public static LstsqInfo cgne<TOp>(in TOp A, in floatN b, ref floatN x,
                                     ref floatN r, ref floatN p, ref floatN Ap, ref floatN tmpN,
                                     ref floatN ps, ref floatN s,
                                     int maxIter, float tol, float damp)
            where TOp : struct, IfloatLinearOperator
        {
            if (damp == (float)0)
                return cgne(in A, in b, ref x, ref r, ref p, ref Ap, ref tmpN, maxIter, tol);

            if (A.Rows > A.Cols) throw new ArgumentException("cgne: A must be square or underdetermined (Rows <= Cols)");
            if (b.N != A.Rows) throw new ArgumentException("cgne: b.N must equal A.Rows");
            if (x.N != A.Cols) throw new ArgumentException("cgne: x.N must equal A.Cols");
            if (r.N != A.Rows) throw new ArgumentException("cgne: r.N must equal A.Rows");
            if (p.N != A.Cols) throw new ArgumentException("cgne: p.N must equal A.Cols");
            if (Ap.N != A.Rows) throw new ArgumentException("cgne: Ap.N must equal A.Rows");
            if (tmpN.N != A.Cols) throw new ArgumentException("cgne: tmpN.N must equal A.Cols");
            if (ps.N != A.Rows) throw new ArgumentException("cgne: ps.N must equal A.Rows");
            if (s.N != A.Rows) throw new ArgumentException("cgne: s.N must equal A.Rows");
            if (maxIter < 1) throw new ArgumentException("cgne: maxIter must be >= 1");

            unsafe
            {
                long* ptrs = stackalloc long[8];
                ptrs[0] = (long)r.Data.Ptr;  ptrs[1] = (long)p.Data.Ptr;
                ptrs[2] = (long)Ap.Data.Ptr; ptrs[3] = (long)tmpN.Data.Ptr;
                ptrs[4] = (long)ps.Data.Ptr; ptrs[5] = (long)s.Data.Ptr;
                ptrs[6] = (long)x.Data.Ptr;  ptrs[7] = (long)b.Data.Ptr;
                RequireDistinctBuffers("cgne: r/p/Ap/tmpN/ps/s/x/b must be distinct", ptrs, 8);
            }

            float lam = damp;

            // x0 = 0, s0 = 0 (the min-norm characterization requires the zero start).
            for (int i = 0; i < x.N; i++) x[i] = (float)0;
            for (int i = 0; i < s.N; i++) s[i] = (float)0;

            float bb = Blas.dot(b, b);
            if (bb == (float)0)
                return LstsqInfoAudited(IterativeSolveStatus.Converged, 0, in A, in b, ref x, ref Ap, ref tmpN, damp);

            r.CopyFrom(in b);   // r_0 = b - [A|λI](x0,s0) = b
            float rr = bb;
            float threshold = tol * tol * bb;
            if (rr <= threshold)
                return LstsqInfoAudited(IterativeSolveStatus.Converged, 0, in A, in b, ref x, ref Ap, ref tmpN, damp);

            // Initial augmented search direction [A|λI]ᵀ r0 = (Aᵀ r0, λ r0).
            A.ApplyT(in r, ref p);            // p_c = Aᵀ r
            Blas.scaledCopy(lam, r, ref ps);  // p_s = λ r

            for (int k = 0; k < maxIter; k++)
            {
                float pp = Blas.dot(p, p) + Blas.dot(ps, ps);   // ‖(p_c,p_s)‖²
                if (!(pp > (float)0))
                    return LstsqInfoAudited(IterativeSolveStatus.Breakdown, k, in A, in b, ref x, ref Ap, ref tmpN, damp);

                float alpha = rr / pp;

                A.Apply(in p, ref Ap);            // Ap = A p_c
                Ap.addScaledInPlace(lam, ps);     // Ap = A p_c + λ p_s = [A|λI](p_c,p_s)
                float rrNew = Blas.updateXR(alpha, p, ref x, Ap, ref r);  // x += α p_c ; r -= α Ap ; ‖r‖²
                s.addScaledInPlace(alpha, ps);    // s += α p_s

                if (rrNew <= threshold)
                {
                    // Fresh augmented residual ‖b - A x - λ s‖² (tracked r can drift). Ap is free
                    // scratch here (recomputed as A p_c at the top of the next iter on fall-through).
                    A.Apply(in x, ref Ap);        // Ap = A x
                    float augSq = (float)0;
                    for (int i = 0; i < b.N; i++)
                    {
                        float e = b[i] - Ap[i] - lam * s[i];
                        augSq += e * e;
                    }
                    if (augSq <= threshold)
                        return LstsqInfoAudited(IterativeSolveStatus.Converged, k + 1, in A, in b, ref x, ref Ap, ref tmpN, damp);
                }

                float beta = rrNew / rr;
                A.ApplyT(in r, ref tmpN);         // tmpN = Aᵀ r
                p.scaleAddInPlace(beta, tmpN);    // p_c = β p_c + Aᵀ r
                ps.mulInPlace(beta);              // p_s = β p_s + λ r
                ps.addScaledInPlace(lam, r);

                rr = rrNew;
            }

            return LstsqInfoAudited(IterativeSolveStatus.MaxIterations, maxIter, in A, in b, ref x, ref Ap, ref tmpN, damp);
        }

        /// <summary>
        /// CGNE over a dense <see cref="floatMxN"/> (Rows ≤ Cols) -- zero-alloc primitive.
        /// Forwards into <see cref="cgne{TOp}"/> via <see cref="floatDenseOperator"/>.
        /// </summary>
        public static LstsqInfo cgne(in floatMxN A, in floatN b, ref floatN x,
                                ref floatN r, ref floatN p, ref floatN Ap, ref floatN tmpN,
                                int maxIter, float tol)
        {
            return cgne(new floatDenseOperator(in A), in b, ref x, ref r, ref p, ref Ap, ref tmpN, maxIter, tol);
        }

        /// <summary>CGNE over a dense matrix -- allocates four scratch vectors from the arena.</summary>
        public static LstsqInfo cgne(in floatMxN A, in floatN b, ref floatN x, int maxIter, float tol)
        {
            floatN r    = b.floatTempVec(A.M_Rows);
            floatN p    = b.floatTempVec(A.N_Cols);
            floatN Ap   = b.floatTempVec(A.M_Rows);
            floatN tmpN = b.floatTempVec(A.N_Cols);
            return cgne(in A, in b, ref x, ref r, ref p, ref Ap, ref tmpN, maxIter, tol);
        }

        /// <summary>CGNE over a dense matrix with default maxIter (A.M_Rows -- the CG recurrence on
        /// the m×m AAᵀ system terminates within m = Rows steps in exact arithmetic, unlike
        /// lsqr/lsmr's N_Cols-bounded search) and tol (Consts.floatSqrtEps).</summary>
        public static LstsqInfo cgne(in floatMxN A, in floatN b, ref floatN x)
        {
            return cgne(in A, in b, ref x, A.M_Rows, Consts.floatSqrtEps);
        }

        /// <summary>
        /// CGNE over a (possibly rectangular, Rows ≤ Cols) block-sparse (BSR) matrix -- zero-alloc
        /// primitive. Forwards into <see cref="cgne{TOp}"/> via <c>floatBSROperator</c>:
        /// matrix-free minimum-norm solve over a sparse operator, never forming AAᵀ.
        /// </summary>
        public static LstsqInfo cgne(in floatBSR A, in floatN b, ref floatN x,
                                ref floatN r, ref floatN p, ref floatN Ap, ref floatN tmpN,
                                int maxIter, float tol)
        {
            return cgne(new floatBSROperator(in A), in b, ref x, ref r, ref p, ref Ap, ref tmpN, maxIter, tol);
        }

        /// <summary>
        /// CGNE over a BSR matrix -- zero-alloc primitive variant that takes a CALLER-PROVIDED
        /// precomputed transpose AT (e.g. built once via <c>arena.floatBSRTranspose(in A)</c>
        /// outside a hot loop) and routes every ApplyT call through the resulting cache-friendly
        /// forward spMV(AT, x) instead of the scatter-heavy on-the-fly spMVT(A, x) -- see
        /// <see cref="floatBSROperator"/>'s two-arg ctor. Caller is responsible for AT actually
        /// being A's transpose; this overload does not verify it.
        /// </summary>
        public static LstsqInfo cgne(in floatBSR A, in floatBSR AT, in floatN b, ref floatN x,
                                ref floatN r, ref floatN p, ref floatN Ap, ref floatN tmpN,
                                int maxIter, float tol)
        {
            return cgne(new floatBSROperator(in A, in AT), in b, ref x, ref r, ref p, ref Ap, ref tmpN, maxIter, tol);
        }

        /// <summary>
        /// CGNE over a BSR matrix -- allocates four scratch vectors AND materializes A^T ONCE via
        /// <c>arena.floatBSRTranspose</c>, then drives CGNE with the two-arg
        /// <see cref="floatBSROperator"/> so every ApplyT call routes through a cache-friendly
        /// forward spMV(A^T, x) instead of scatter-heavy spMVT(A, x) every iteration. For a
        /// build-free zero-alloc path, build A^T yourself once and call the zero-alloc
        /// <see cref="cgne(in floatBSR, in floatBSR, in floatN, ref floatN, ref floatN, ref floatN, ref floatN, ref floatN, int, float)"/>
        /// overload above with your own scratch vectors.
        /// </summary>
        public static LstsqInfo cgne(in floatBSR A, in floatN b, ref floatN x, int maxIter, float tol)
        {
            floatN r    = b.floatTempVec(A.M_Rows);
            floatN p    = b.floatTempVec(A.N_Cols);
            floatN Ap   = b.floatTempVec(A.M_Rows);
            floatN tmpN = b.floatTempVec(A.N_Cols);
            floatBSR AT = b.floatBSRTranspose(in A);
            return cgne(new floatBSROperator(in A, in AT), in b, ref x, ref r, ref p, ref Ap, ref tmpN, maxIter, tol);
        }

        /// <summary>CGNE over a BSR matrix with default maxIter (A.M_Rows) and tol
        /// (Consts.floatSqrtEps) -- see the dense default overload for the M_Rows rationale.</summary>
        public static LstsqInfo cgne(in floatBSR A, in floatN b, ref floatN x)
        {
            return cgne(in A, in b, ref x, A.M_Rows, Consts.floatSqrtEps);
        }

        // ===== Tikhonov-damped overloads: x = Aᵀ(A Aᵀ + damp²·I)⁻¹ b (arena-allocated) =====

        /// <summary>
        /// Damped CGNE over a dense matrix (Rows ≤ Cols) -- allocates six scratch vectors from the
        /// arena. damp == 0 falls through to the undamped path. See the damped primitive for the
        /// regularized-least-norm semantics and the nonzero-rnorm-at-convergence contract.
        /// </summary>
        public static LstsqInfo cgne(in floatMxN A, in floatN b, ref floatN x, int maxIter, float tol, float damp)
        {
            floatN r    = b.floatTempVec(A.M_Rows);
            floatN p    = b.floatTempVec(A.N_Cols);
            floatN Ap   = b.floatTempVec(A.M_Rows);
            floatN tmpN = b.floatTempVec(A.N_Cols);
            floatN ps   = b.floatTempVec(A.M_Rows);
            floatN s    = b.floatTempVec(A.M_Rows);
            return cgne(new floatDenseOperator(in A), in b, ref x, ref r, ref p, ref Ap, ref tmpN, ref ps, ref s, maxIter, tol, damp);
        }

        /// <summary>
        /// Damped CGNE over a BSR matrix (Rows ≤ Cols) -- allocates six scratch vectors AND
        /// materializes Aᵀ once (cache-friendly ApplyT), then drives the damped primitive. damp == 0
        /// falls through to the undamped path.
        /// </summary>
        public static LstsqInfo cgne(in floatBSR A, in floatN b, ref floatN x, int maxIter, float tol, float damp)
        {
            floatN r    = b.floatTempVec(A.M_Rows);
            floatN p    = b.floatTempVec(A.N_Cols);
            floatN Ap   = b.floatTempVec(A.M_Rows);
            floatN tmpN = b.floatTempVec(A.N_Cols);
            floatN ps   = b.floatTempVec(A.M_Rows);
            floatN s    = b.floatTempVec(A.M_Rows);
            floatBSR AT = b.floatBSRTranspose(in A);
            return cgne(new floatBSROperator(in A, in AT), in b, ref x, ref r, ref p, ref Ap, ref tmpN, ref ps, ref s, maxIter, tol, damp);
        }
    }
}
