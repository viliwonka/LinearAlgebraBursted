...
namespace LinearAlgebra
{
    public static partial class LP
    {
        ...
        // library's dense row-major doubleMxN/doubleN types and CHOP-based normal-equation solve; see
        ...
        /// ordinary LAD case). See <see cref="lad(in doubleMxN, in doubleN, ref doubleN, out double, int)"/>
        ...
        public static LPInfo ladFN(in doubleMxN A, in doubleN b, ref doubleN x, out double objective,
                                   int maxIter = 0)
        {
        ...
        /// public <see cref="ladFN(in doubleMxN, in doubleN, ref doubleN, out double, int)"/> overloads
                                                   ...
                                                   ref doubleN x, out double objective, int maxIter)
        {
            ...
            var As = new doubleMxN(m, n, Allocator.Temp);
            var colScale = new doubleN(n, Allocator.Temp);
            for (int j = 0; j < n; j++) colScale[j] = (double)0;
            for (int i = 0; i < m; i++)
                for (int j = 0; j < n; j++)
                    colScale[j] = math.max(colScale[j], math.abs(A[i, j]));
            for (int j = 0; j < n; j++)
                if (!(colScale[j] > (double)0)) colScale[j] = (double)1;
            for (int i = 0; i < m; i++)
                for (int j = 0; j < n; j++)
                    As[i, j] = A[i, j] / colScale[j];

            // ---- constant RHS of the dual equality constraint: bLp = (1-tau) As^T e ----
            var onesM = new doubleN(m, Allocator.Temp);
            for (int i = 0; i < m; i++) onesM[i] = (double)1;
            var bLp = new doubleN(n, Allocator.Temp);
            ATmul(As, onesM, bLp, m, n);
            double oneMinusTau = (double)(1.0 - tau);
            ...
            var dPlus = new doubleN(m, Allocator.Temp);
            var dMinus = new doubleN(m, Allocator.Temp);
            for (int i = 0; i < m; i++)
            {
                double c = -b[i];
                dPlus[i] = math.max(c, (double)0);
                dMinus[i] = math.max(-c, (double)0);
            }
            double U = 0;
            ...
            double epsFloor = (double)1e-8;
            double alpha0F = (double)alpha0;
            for (int i = 0; i < m; i++)
            {
            ...
            var r1 = new doubleN(m, Allocator.Temp);
            var r2 = new doubleN(n, Allocator.Temp);
            var qinv = new doubleN(m, Allocator.Temp);
            var qr = new doubleN(m, Allocator.Temp);
            var Av = new doubleN(m, Allocator.Temp);
            var ATa = new doubleN(n, Allocator.Temp);

            var dxAff = new doubleN(m, Allocator.Temp);
            ...
            var aPred = new doubleN(m, Allocator.Temp);
            var sPred = new doubleN(m, Allocator.Temp);
            var zPred = new doubleN(m, Allocator.Temp);
            var wPred = new doubleN(m, Allocator.Temp);
            var yPred = new doubleN(n, Allocator.Temp);

            var r1Hat = new doubleN(m, Allocator.Temp);
            var dxCor = new doubleN(m, Allocator.Temp);
            var dsCor = new doubleN(m, Allocator.Temp);
            var dzCor = new doubleN(m, Allocator.Temp);
            var dwCor = new doubleN(m, Allocator.Temp);
            var dyCor = new doubleN(n, Allocator.Temp);
            ...
            {
                for (int i = 0; i < m; i++) qinv[i] = (double)1;
            ...
            while (iters < budget)
            {
                ...
                for (int i = 0; i < m; i++) qinv[i] = (double)1 / (z[i] / a[i] + w[i] / s[i]);
                ...
                double alphaPAff = math.min((double)1, backoff * math.min(MaxStep(a, dxAff, m, BIG), MaxStep(s, dsAff, m, BIG)));
                double alphaDAff = math.min((double)1, backoff * math.min(MaxStep(z, dzAff, m, BIG), MaxStep(w, dwAff, m, BIG)));
                ...
                double muTarg = (double)(sigma * mu / m);

                // ---- centering corrector ----
                for (int i = 0; i < m; i++)
                    r1Hat[i] = muTarg * ((double)1 / s[i] - (double)1 / a[i]) + (dxAff[i] * dzAff[i]) / a[i] - (dsAff[i] * dwAff[i]) / s[i];
                ...
                double alphaPCor = math.min((double)1, backoff * math.min(MaxStep(aPred, dxCor, m, BIG), MaxStep(sPred, dsCor, m, BIG)));
                double alphaDCor = math.min((double)1, backoff * math.min(MaxStep(zPred, dzCor, m, BIG), MaxStep(wPred, dwCor, m, BIG)));
        ...
        static void BuildWeightedNormal(doubleMxN A, doubleN qinv, doubleMxN M, int m, int n)
        {
            for (int i = 0; i < n; i++)
                for (int j = i; j < n; j++)
                    M[i, j] = (double)0;
