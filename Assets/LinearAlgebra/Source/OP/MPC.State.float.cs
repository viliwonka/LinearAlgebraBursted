...
namespace LinearAlgebra.Control
{
    ...
    // see floatMPCState.Kstab's own doc comment) followed by [s_0; ...; s_{N-1}] soft-row slacks
    ...
    // ================================================================================================
    public partial struct floatMPCState
    {
        ...
        /// or <see cref="MPCStatus.MaxIterations"/>) completes -- mirrors <see cref="floatLQRState.populated"/>.</summary>
        ...
        /// <see cref="MPC.State.float.cs"/>'s caller supplied an explicit terminal P.</summary>
        public floatMxN Kinf;

        /// <summary>Prestabilization gain, m x n. <see cref="floatMxN.IsCreated"/> false if disabled.</summary>
        public floatMxN Kstab;
        ...
        /// <c>Rcross @ x0</c> every <see cref="MPC.solve"/> call (see <see cref="MPC.float.cs"/>'s
        /// BuildGradient). <see cref="floatMxN.IsCreated"/> false if not prestabilized.</summary>
        public floatMxN Rcross;

        /// <summary>DeltaU penalty weight, m x m. <see cref="floatMxN.IsCreated"/> false if disabled.</summary>
        public floatMxN S;

        /// <summary>Soft-row coefficients, nSoftPerStage x n. <see cref="floatMxN.IsCreated"/> false if
        /// disabled.</summary>
        public floatMxN C;
        ...
        internal floatQPFactorState qpFactor;
        ...
        internal floatQPReducedState qpReduced;
        ...
        /// <summary>
        /// Full constructor -- every optional knob explicit. Pass <c>default(floatMxN)</c> for
        ...
        /// <param name="P">Terminal state cost, n x n, or <c>default(floatMxN)</c> for the infinite-
        /// horizon DARE solution (via <see cref="LQR.lqr"/>).</param>
        /// <param name="S">DeltaU penalty weight, m x m, or <c>default(floatMxN)</c> to disable.</param>
        /// <param name="C">Soft-row coefficients, k x n (k &gt;= 1), or <c>default(floatMxN)</c> to
        ...
        /// <param name="Kstab">Prestabilization gain, m x n, or <c>default(floatMxN)</c> to disable.
        /// NOT supported together with a created <paramref name="S"/> (throws).</param>
        public floatMPCState(int n, int m, int N, Allocator allocator,
                              in floatMxN A, in floatMxN B, in floatMxN Q, in floatMxN R,
                              in floatN uLo, in floatN uHi,
                              in floatMxN P, in floatMxN S,
                              in floatMxN C, in floatN d, float rho1, float rho2,
                              in floatMxN Kstab)
        {
            if (n <= 0 || m <= 0 || N <= 0)
                throw new ArgumentException("floatMPCState: n, m, N must all be >= 1");
            if (!A.IsSquare || A.M_Rows != n) throw new ArgumentException("floatMPCState: A must be n x n");
            if (B.M_Rows != n || B.N_Cols != m) throw new ArgumentException("floatMPCState: B must be n x m");
            if (!Q.IsSquare || Q.M_Rows != n) throw new ArgumentException("floatMPCState: Q must be n x n");
            if (!R.IsSquare || R.M_Rows != m) throw new ArgumentException("floatMPCState: R must be m x m");
            if (uLo.N != m || uHi.N != m) throw new ArgumentException("floatMPCState: uLo/uHi must have length m");
            for (int i = 0; i < m; i++)
                if (uLo[i] > uHi[i]) throw new ArgumentException("floatMPCState: uLo must be <= uHi componentwise");

            bool hasP = P.IsCreated;
            if (hasP && (!P.IsSquare || P.M_Rows != n)) throw new ArgumentException("floatMPCState: P must be n x n");
            bool hasDeltaU = S.IsCreated;
            if (hasDeltaU && (!S.IsSquare || S.M_Rows != m)) throw new ArgumentException("floatMPCState: S must be m x m");
            bool hasSoftRows = C.IsCreated;
            int nSoftPerStageLocal = 0;
            if (hasSoftRows)
            {
                if (C.N_Cols != n || C.M_Rows < 1) throw new ArgumentException("floatMPCState: C must be k x n, k >= 1");
                if (d.N != C.M_Rows) throw new ArgumentException("floatMPCState: d.N must equal C.M_Rows");
                if (!math.isfinite(rho1) || rho1 < (float)0 || !math.isfinite(rho2) || rho2 < (float)0)
                    throw new ArgumentException("floatMPCState: rho1/rho2 must be finite and non-negative");
                nSoftPerStageLocal = C.M_Rows;
            }
            bool hasPrestab = Kstab.IsCreated;
            if (hasPrestab && (Kstab.M_Rows != m || Kstab.N_Cols != n)) throw new ArgumentException("floatMPCState: Kstab must be m x n");
            if (hasDeltaU && hasPrestab)
                throw new ArgumentException("floatMPCState: deltaU weighting (S) and prestabilization (Kstab) cannot both be enabled");
            ...
            this.A = new floatMxN(in A, allocator);
            this.B = new floatMxN(in B, allocator);
            this.uLo = new floatN(in uLo, allocator);
            ...
            Kinf = new floatMxN(m, n, allocator);
            var lqrState = new floatLQRState(n, Allocator.Temp);
            ...
            floatMxN Pterm = hasP ? P : lqrState.S;

            // ---- closed-loop (or plain) condensing dynamics ----
            var Acond = new floatMxN(n, n, Allocator.Temp);
            if (hasPrestab)
            {
                var BK = new floatMxN(n, n, Allocator.Temp);
            ...
            Phi = new floatMxN(N * n, n, allocator, true);
            var Ak = new floatMxN(n, n, Allocator.Temp);
            ...
            Gamma = new floatMxN(N * n, nu, allocator);   // zero-initialized
            var prevBlock = new floatMxN(n, m, Allocator.Temp);
            var newBlock = new floatMxN(n, m, Allocator.Temp);
            for (int k = 0; k < N; k++)
            {
                for (int i = 0; i < n; i++)
                    for (int j = 0; j < m; j++)
            ...
            var QGamma = new floatMxN(N * n, nu, Allocator.Temp);
            for (int k = 0; k < N; k++)
            {
                bool terminal = k == N - 1;
                for (int i = 0; i < n; i++)
            ...
            floatMxN M = default, KPhiPre = default;
            ...
            else
            {
                ...
                M = new floatMxN(nu, nu, Allocator.Temp);   // zero-initialized
                for (int i = 0; i < nu; i++) M[i, i] = (float)1;
                KPhiPre = new floatMxN(nu, n, Allocator.Temp);   // zero-initialized
                ...
                var RM = new floatMxN(nu, nu, Allocator.Temp);   // zero-initialized
                unsafe
                {
                    float* Mp = M.Data.Ptr;
                    float* RMp = RM.Data.Ptr;
                    for (int k = 0; k < N; k++)
                        for (int i = 0; i < m; i++)
                        {
                            float* dst = RMp + (long)(k * m + i) * nu;
                            for (int p = 0; p < m; p++)
                                UnsafeOP.axpy(dst, Mp + (long)(k * m + p) * nu, R[i, p], nu);
                        }
                }

                var MtRbarM = new floatMxN(nu, nu, Allocator.Temp);
                Blas.dotSym(in M, in RM, ref MtRbarM);                    // M^T @ (Rbar M), symmetric
                for (int i = 0; i < nu; i++)
                    for (int j = 0; j < nu; j++)
                        H_UU[i, j] += MtRbarM[i, j];

                Rcross = new floatMxN(nu, n, allocator);
                Blas.dot(in RM, in KPhiPre, ref Rcross, transposeA: true); // (Rbar M)^T @ KPhiPre = M^T Rbar KPhiPre
                for (int i = 0; i < nu; i++)
                    for (int j = 0; j < n; j++)
                        Rcross[i, j] = (float)(-2) * Rcross[i, j];
            ...
            if (hasDeltaU)
            {
                for (int k = 0; k < N; k++)
                {
                    float scale = k < N - 1 ? (float)2 : (float)1;
                    for (int i = 0; i < m; i++)
                        for (int j = 0; j < m; j++)
            ...
            H = new floatMxN(nz, nz, allocator);   // zero-initialized
            for (int i = 0; i < nu; i++)
                for (int j = 0; j < nu; j++)
                    H[i, j] = (float)2 * H_UU[i, j];
            if (hasSoftRows)
                for (int j = 0; j < nSlack; j++)
                    H[nu + j, nu + j] = (float)2 * rho2;
            H_UU.Dispose();
            ...
            Arows = new floatMxN(nGeneral, nz, allocator);
            qCoupling = new floatMxN(nGeneral, n, allocator);
            rowConstRHS = new floatN(nGeneral, allocator);
            ...
            if (hasSoftRows)
            {
                for (int k = 0; k < N; k++)
                {
                    for (int si = 0; si < nSoftPerStage; si++)
                    {
                        ...
                        Arows[rowIdx, nu + rowIdx] = (float)(-1);
                        for (int q = 0; q < n; q++)
                        {
                            float sum = (float)0;
            ...
            // ---- box bounds ----
            xl = new floatN(nz, allocator);
            xu = new floatN(nz, allocator);
            ...
            if (hasSoftRows)
                for (int j = 0; j < nSlack; j++) { xl[nu + j] = (float)0; xu[nu + j] = INF; }

            // ---- persistent scratch / warm-start carry ----
            z = new floatN(nz, allocator);
            wstatus = new NativeArray<byte>(nGeneral + nz, allocator);
            cScratch = new floatN(nz, allocator);
            ...
            bScratch = new floatN(nGeneral, allocator);
            uPlan = new floatN(nu, allocator);
            xTrajScratch = new floatN(N * n, allocator);
            ...
            qpFactor = floatQPFactorState.Create(nz, allocator);
            qpReduced = floatQPReducedState.Create(nz, allocator);
        ...
        public floatMPCState(int n, int m, int N, Allocator allocator,
            ...
            : this(n, m, N, allocator, in A, in B, in Q, in R, in uLo, in uHi,
                   default, default, default, default(floatN), (float)0, (float)0, default)
        {
        }
        ...
        public floatMPCState(int n, int m, int N, Allocator allocator,
                              in floatMxN A, in floatMxN B, in floatMxN Q, in floatMxN R,
                              in floatN uLo, in floatN uHi, in floatMxN P)
            : this(n, m, N, allocator, in A, in B, in Q, in R, in uLo, in uHi,
                   in P, default, default, default(floatN), (float)0, (float)0, default)
        {
        }
        ...
        public floatMPCState(int n, int m, int N, Allocator allocator,
                              in floatMxN A, in floatMxN B, in floatMxN Q, in floatMxN R,
                              in floatN uLo, in floatN uHi, in floatMxN C, in floatN d)
            : this(n, m, N, allocator, in A, in B, in Q, in R, in uLo, in uHi,
                   default, default, in C, in d, (float)MPC.DEFAULT_RHO1, (float)MPC.DEFAULT_RHO2, default)
        {
        }
        ...
        public floatMPCState(int n, int m, int N, Allocator allocator,
                              in floatMxN A, in floatMxN B, in floatMxN Q, in floatMxN R,
                              in floatN uLo, in floatN uHi, in floatMxN C, in floatN d, float rho1, float rho2)
            : this(n, m, N, allocator, in A, in B, in Q, in R, in uLo, in uHi,
        ...
        // provided -- both would need an extra floatMxN-typed parameter (S / Kstab) in the SAME
