...
using BULA.Internal;   // doubleW (8-lane AVX helper) for the wide radix-4 butterfly
...
namespace BULA
{
    ...
    public static partial class FFT
    {
        ...
        /// produced by the doubleFFTCache(n, allocator) constructor: ws.n == n and table lengths == n/2.
        /// </summary>
        static void RequireFftWorkspace(in doubleFFTCache ws, int n, string who)
        {
            if (ws.n != n ||
                ...
                throw new ArgumentException(
                    who + ": workspace must be sized for an n-point FFT (use new doubleFFTCache(n, allocator))");
        }
        ...
        static void RequireRadix4Workspace(in doubleFFTCache ws, int n, string who)
        {
            ...
            if (ws.twQuarter.N != (n >> 2) + 1)
                throw new ArgumentException(
                    who + ": workspace must have quarter-circle twiddle table (use new doubleFFTCache(n, allocator))");
        }
        ...
        /// (build via new doubleFFTCache(N, allocator)); it must contain the quarter-circle twiddle table required
                                         ...
                                         bool inverse)
        {
            ...
            for (int i = 0; i < size; i++)
            {
                ...
                if (j > i)
                {
                    double tr = rePtr[i]; rePtr[i] = rePtr[j]; rePtr[j] = tr;
                    double ti = imPtr[i]; imPtr[i] = imPtr[j]; imPtr[j] = ti;
                }
            }
        ...
        static unsafe void FftCoreRadix4Core(double* rePtr, double* imPtr,
                                             double* s1rPtr, double* s1iPtr,
                                             int size, bool inverse, double* interleaveOut)
        {
