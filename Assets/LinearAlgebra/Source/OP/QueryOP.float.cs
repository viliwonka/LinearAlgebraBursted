...
namespace BULA
{
    ...
    public static partial class Query
    {
        ...
        // argmin/argmax over one CONTIGUOUS row of length n via a width-4 float4 (aliased to
        ...
        internal static unsafe void RowArgMaxScan(float* row, int n, out int bestC, out float bestVal)
        {
            if (n < 4)
            {
                float s = row[0]; int si = 0;
        ...
        public static int rowArgMin(in floatMxN A, ref Indices colIndexPerRow, ref floatN valPerRow)
        {
            ...
            unsafe
            {
                ...
                {
                    RowArgMinScan(ap + (long)r * nc, nc, out int bestC, out float best);
        ...
        public static int rowArgMin(in floatMxN A, ref Indices colIndexPerRow)
        {
            ...
            unsafe
            {
                ...
                for (int r = 0; r < A.M_Rows; r++)
                {
                    RowArgMinScan(ap + (long)r * nc, nc, out int bestC, out float _);
                    colIndexPerRow[r] = bestC;
                }
            }
            return A.M_Rows;
        ...
        public static int rowArgMax(in floatMxN A, ref Indices colIndexPerRow, ref floatN valPerRow)
        {
            ...
            unsafe
            {
                ...
                for (int r = 0; r < A.M_Rows; r++)
                {
                    RowArgMaxScan(ap + (long)r * nc, nc, out int bestC, out float best);
                    colIndexPerRow[r] = bestC;
                    valPerRow[r] = best;
                }
            }
        ...
        public static int rowArgMax(in floatMxN A, ref Indices colIndexPerRow)
        {
            ...
            unsafe
            {
                ...
                for (int r = 0; r < A.M_Rows; r++)
                {
                    RowArgMaxScan(ap + (long)r * nc, nc, out int bestC, out float _);
                    colIndexPerRow[r] = bestC;
                }
            }
            return A.M_Rows;
        ...
        public static int argMaxColNorm(in floatMxN A, Norm n)
        {
            ...
            var acc = new floatN(nc, Allocator.Temp);
            int bestCol = 0;
            float bestNorm = (float)0;
            unsafe
            {
                float* ap = A.Data.Ptr; float* accp = acc.Data.Ptr;
                for (int c = 0; c < nc; c++) accp[c] = (float)0;
        ...
        internal static void AllColScores(in floatMxN A, in floatN q, Metric m, float normQ, ref floatN dest)
        {
            int nc = A.N_Cols, mR = A.M_Rows;
            unsafe
            {
                float* ap = A.Data.Ptr; float* qp = q.Data.Ptr; float* dp = dest.Data.Ptr;
                for (int c = 0; c < nc; c++) dp[c] = (float)0;

                if (m == Metric.Manhattan)
                {
                    for (int r = 0; r < mR; r++)
                    {
                        float* row = ap + (long)r * nc; float qr = qp[r];
                ...
                else if (m == Metric.Euclidean || m == Metric.SqEuclidean)
                {
                    ...
                    {
                        float* row = ap + (long)r * nc; float qr = qp[r];
                        for (int c = 0; c < nc; c++) { float d = row[c] - qr; dp[c] += d * d; }
                    }
                ...
                {
                    var normAv = new floatN(nc, Allocator.Temp);
                    float* nap = normAv.Data.Ptr;
                    for (int c = 0; c < nc; c++) nap[c] = (float)0;
                    for (int r = 0; r < mR; r++)
                    {
                        float* row = ap + (long)r * nc; float qr = qp[r];
                    ...
                    for (int c = 0; c < nc; c++)
                    {
                        float denom = math.sqrt(nap[c] * normQ);
                        dp[c] = denom > (float)0 ? dp[c] / denom : (float)0;
                    }
                    normAv.Dispose();
                }
            }
        ...
        public static void nearestColumn(in floatMxN A, in floatN q, Metric m, out int index, out float score)
        {
            ...
            float normQ = m == Metric.Cosine ? QueryNormSq(in q) : (float)0;
            var scores = new floatN(A.N_Cols, Allocator.Temp);
            AllColScores(in A, in q, m, normQ, ref scores);
            float best = floatQueryCore.WorstScoreForNearest(m);
            int bestIdx = 0;
            for (int c = 0; c < A.N_Cols; c++)
                if (floatQueryCore.IsBetterForNearest(scores[c], best, m)) { best = scores[c]; bestIdx = c; }
            scores.Dispose();
            index = bestIdx;
            score = best;
        }
        ...
        public static void farthestColumn(in floatMxN A, in floatN q, Metric m, out int index, out float score)
        {
            ...
            int worstIdx = 0;
            for (int c = 0; c < A.N_Cols; c++)
                if (floatQueryCore.IsBetterForFarthest(scores[c], worst, m)) { worst = scores[c]; worstIdx = c; }
            scores.Dispose();
            index = worstIdx;
            score = worst;
        }
