...
namespace LinearAlgebra
{
    public static partial class SVD {
        ...
        /// the floatSVDTruncatedCache(m, n, k, oversample, allocator) ctor using the SAME k and oversample.
        /// </summary>
                                        ...
                                        bool partialReorth, ref floatSVDTruncatedCache ws)
        {
            ...
            unsafe
            {
                ...
                for (int i = 0; i < n; i++)
                    v0[i] = (float)(rng.NextDouble() * 2.0 - 1.0);   // full-precision seed for the double variant
                ...
                if (partialReorth)
                {
                    ...
                    float epsFloor = (float)1.5 * eps;                          // reset level for orthogonalized ω
                    ...
                    for (int j = 0; j < p; j++)
                    {
                        ...
                        anorm = math.max(anorm, (float)1.01 * svdAnormBlock(ws.alpha[j], j > 0 ? ws.beta[j - 1] : (float)0));
                        ...
                        anorm = math.max(anorm, (float)1.01 * svdAnormBlock(ws.alpha[j], ws.beta[j]));
        ...
        /// Pass a workspace from the floatSVDTruncatedCache(m, n, k, allocator) ctor (no oversample overload)
        ...
        /// the floatSVDFullCache(m, n, allocator) ctor.
        /// </summary>
