...
namespace LinearAlgebra
{
    ...
    /// <c>Consts.doubleEpsilon</c> is reseeded first). W and P are B-orthonormalized
    ...
    public static partial class Eigen
    {
            ...
            where TPre : struct, IdoublePreconditioner
        {
            ...
            // Consts.doubleEpsilon B-norm^2 is reseeded first (deterministic, distinct seed from the
            // other reseed sites).
            for (int i = 0; i < kWork; i++)
            {
                double bn2 = (double)0;
            ...
            double normAEst, normBEst;
            {
                double af2 = (double)0, bf2 = (double)0;
                for (int i = 0; i < kWork; i++)
                    for (int c = 0; c < n; c++)
                    {
                ...
                normAEst = math.sqrt(af2 / (double)kWork);
                normBEst = math.sqrt(bf2 / (double)kWork);
            }
            ...
            for (int iter = 0; iter < maxIter; iter++)
            {
                ...
                {
                    double rn2 = (double)0, xn2 = (double)0, xb2 = (double)0;
                    for (int c = 0; c < n; c++)
                    {
                        ...
                        double xv = ws.X[i, c];
                        double bxv = ws.BX[i, c];
                        double rv = ws.AX[i, c] - ws.lambda[i] * bxv;
                        ws.R[i, c] = rv;
                        rn2 += rv * rv;
                        xn2 += xv * xv;
                        xb2 += xv * bxv;
                    ...
                    double xB = math.sqrt(math.max(xb2, (double)0));
                    double lamAbs = math.abs(ws.lambda[i]);
                    double scaleMag = lamAbs;
                    if (scaleMag < (double)1) scaleMag = (double)1;
                    double scale = math.min(normAEst * math.sqrt(xn2) + lamAbs * normBEst * xB, scaleMag * xB);
                    ws.resScale[i] = scale;
                    ws.xBnorm[i] = xB;
                ...
                // whose B-norm^2 is at/below Consts.doubleEpsilon is reseeded first (deterministic,
        ...
        static unsafe void RestoreBufferIdentity(ref doubleMxN cur, ref doubleMxN other, in doubleMxN entry, int rows, int cols)
        {
        ...
        static int OrthonormalizeBlockB(ref doubleMxN V, ref doubleMxN AV, ref doubleMxN BV, int rows, int n,
                                         ref doubleMxN Gram, ref doubleMxN Z, ref doubleMxN scratch)
        {
            if (rows <= 0) return 0;
            ...
            var D = new doubleN(rows, Allocator.Temp);
            for (int i = 0; i < rows; i++)
            {
                double gi = G[i, i];
                D[i] = gi > (double)0 ? (double)1 / math.sqrt(gi) : (double)0;
            }
            ...
            var theta = new doubleN(rows, Allocator.Temp);
            ...
            if (eigOk)
            {
                ...
                if (thetaMax > (double)0)
                {
                    double dropFloor = thetaMax * ((double)rows * Consts.doubleEpsilon * (double)10);
        ...
        static void SvqbAccumulate(in doubleMxN Src, ref doubleMxN Dst, in doubleN D, in doubleMxN Zv, in doubleN theta,
                                    int rows, int kept, int n)
        {
            ...
            for (int j = 0; j < kept; j++)
            {
                double invSqrtTheta = (double)1 / math.sqrt(theta[j]);
                for (int i = 0; i < rows; i++)
                {
                    double coef = D[i] * Zv[i, j] * invSqrtTheta;
                    for (int c = 0; c < n; c++)
        ...
        static unsafe void FillGramSub(ref doubleMxN Gram, int rowOff, in doubleMxN Vb, int rows,
                                 int colOff, in doubleMxN Wb, int cols, int n, bool sameBlock)
        {
            double* vp = Vb.Data.Ptr; int vnc = Vb.N_Cols;
            double* wp = Wb.Data.Ptr; int wnc = Wb.N_Cols;
            for (int i = 0; i < rows; i++)
            {
                int jStart = sameBlock ? i : 0;
                for (int j = jStart; j < cols; j++)
                {
                    double s = UnsafeOP.vecDot(vp + (long)i * vnc, wp + (long)j * wnc, n);
        ...
        static unsafe void FillHSub(ref doubleMxN H, int rowOff, in doubleMxN Vb, int rows,
                              int colOff, in doubleMxN AWb, int cols, int n, bool sameBlock)
        {
            double* vp = Vb.Data.Ptr; int vnc = Vb.N_Cols;
            double* wp = AWb.Data.Ptr; int wnc = AWb.N_Cols;
            for (int i = 0; i < rows; i++)
            {
                int jStart = sameBlock ? i : 0;
                for (int j = jStart; j < cols; j++)
        ...
        // bit-for-bit.
        static unsafe void Deflate(ref doubleMxN V, ref doubleMxN AV, ref doubleMxN BV, int activeCount,
                             in doubleMxN Against, in doubleMxN AgainstA, in doubleMxN AgainstB, int againstStart, int againstCount, int n)
        {
            double* vpV = V.Data.Ptr; int vncV = V.N_Cols;
            double* vpAB = AgainstB.Data.Ptr; int vncAB = AgainstB.N_Cols;
            for (int pass = 0; pass < 2; pass++)
                for (int a = 0; a < activeCount; a++)
                    for (int i = againstStart; i < againstStart + againstCount; i++)
                    {
                        double coeff = UnsafeOP.vecDot(vpAB + (long)i * vncAB, vpV + (long)a * vncV, n);
        ...
        static unsafe void DeflateRow(ref doubleMxN V, ref doubleMxN AV, ref doubleMxN BV, int row,
                             in doubleMxN Against, in doubleMxN AgainstA, in doubleMxN AgainstB, int againstStart, int againstCount, int n)
        {
            double* vpV = V.Data.Ptr; int vncV = V.N_Cols;
            double* vpAB = AgainstB.Data.Ptr; int vncAB = AgainstB.N_Cols;
            for (int pass = 0; pass < 2; pass++)
                for (int i = againstStart; i < againstStart + againstCount; i++)
                {
                    double coeff = UnsafeOP.vecDot(vpAB + (long)i * vncAB, vpV + (long)row * vncV, n);
        ...
        static void BuildProjected(ref doubleMxN Gram, ref doubleMxN H, int nx, int nw, int np, bool useP,
        ...
        static bool FactorGram(ref doubleMxN Gram, ref doubleMxN L, int m, double tol, bool cubeRule)
        {
            ...
            for (int i = 0; i < m; i++) { double d = math.abs(Gram[i, i]); scale = math.max(scale, d); }
        ...
        // Seed / internal-block gate: linear pivot-ratio threshold Consts.doubleSqrtEps. A
        ...
        // CHO.double.cs's CHOL_BLOCK comment).
        static bool FactorGram(ref doubleMxN Gram, ref doubleMxN L, int m)
            => FactorGram(ref Gram, ref L, m, Consts.doubleSqrtEps, false);
        ...
        static bool FactorGramCombined(ref doubleMxN Gram, ref doubleMxN L, int m)
            => FactorGram(ref Gram, ref L, m, Consts.doubleEpsilon * (double)10, true);

        static bool PivotGateOk(double ratio, double tol, bool cubeRule)
        ...
        static bool TryRayleighRitz(ref doubleLOBPCGCache ws, int numActive, int nw, int np, bool useP, int n)
        {
            int npEff = useP ? np : 0;
        ...
        static void UpdateActiveBlock(ref doubleLOBPCGCache ws, int numActive, int nw, int np, bool useP, int n, int k)
        {
            int npEff = useP ? np : 0;
            int m = numActive + nw + npEff;
        ...
        static double MaxRelResidual(in doubleLOBPCGCache ws, int k)
        {
            ...
            for (int i = 0; i < k; i++)
            {
                double scale = ws.resScale[i];
                double rel = scale > (double)0
                    ? (double)(ws.residual[i] / scale)
                    : (ws.residual[i] > (double)0 ? double.PositiveInfinity : 0.0);
        ...
        static bool WantedWithinTol(in doubleLOBPCGCache ws, int kWork, int k, double tol, double normFloor)
        {
            for (int i = 0; i < kWork; i++)
            {
                int rank = 0;
        ...
        // frozen at lock time).
        static int ConvergedWithinTol(in doubleLOBPCGCache ws, int k, double tol, double normFloor)
        {
            int c = 0;
            for (int i = 0; i < k; i++)
