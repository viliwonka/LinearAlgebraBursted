// <auto-generated>
//   Generated from Assets/LinearAlgebra/CodeGen/TemplateSource/OP/Krylov.TFQMR.fProxy.cs
...
namespace LinearAlgebra
{
    public static partial class Krylov {
        ...
        /// systems A x = b, generic over BOTH the operator (<see cref="IfloatLinearOperator"/>)
        /// and the preconditioner (<see cref="IfloatPreconditioner"/>) -- the SINGLE body behind
        ...
        ///
        /// With <see cref="floatIdentityPreconditioner"/> the IsIdentity fold makes uHat = u, so
        ...
        /// </summary>
        public static SolveInfo tfqmr<TOp, TPre>(in TOp A, in TPre M, in floatN b, ref floatN x,
                                         ref floatN rHat0, ref floatN u, ref floatN w, ref floatN v,
                                         ref floatN au, ref floatN d, ref floatN uHat,
            ...
            where TPre : struct, IfloatPreconditioner
        {
            ...
            rHat0.CopyFrom(in b);
            rHat0.addScaledInPlace((float)(-1), v);

            float tau = math.sqrt(Blas.dot(rHat0, rHat0));
            if (tau <= thresh)
            ...
            v.CopyFrom(in au);
            for (int i = 0; i < A.Rows; i++) d[i] = (float)0;

            float theta = (float)0, eta = (float)0, alpha = (float)0;
            float rho = tau * tau;                        // <rHat0, rHat0> == tau^2 (shadow == r0)
            float rhoLast = rho;
            float bound = tau;
            ...
            for (int k = 0; k < maxIter; k++)
            {
                ...
                if (even)
                {
                    float vtrstar = Blas.dot(rHat0, v);
                    if (vtrstar == (float)0 || math.isnan(vtrstar))
                        return MakeSolveInfo(IterativeSolveStatus.Breakdown, k, bound); // r orthogonal to shadow

                    alpha = rho / vtrstar;
                    if (alpha == (float)0 || math.isnan(alpha))
                ...
                {
                    float rhoNew = Blas.dot(rHat0, w);
                    if (rhoLast == (float)0 || math.isnan(rhoLast))
                        return MakeSolveInfo(IterativeSolveStatus.Breakdown, k + 1, bound); // beta undefined
                    float beta = rhoNew / rhoLast;
                    if (math.isnan(beta))
        ...
        /// <see cref="tfqmr{TOp, TPre}(in TOp, in TPre, in floatN, ref floatN, ref floatN, ref floatN, ref floatN, ref floatN, ref floatN, ref floatN, ref floatN, int, float)"/>
        /// with the identity preconditioner (whose IsIdentity fold strips the uHat traffic), so
        /// this needs no uHat buffer.
        /// </summary>
        public static SolveInfo tfqmr<TOp>(in TOp A, in floatN b, ref floatN x,
                                         ref floatN rHat0, ref floatN u, ref floatN w, ref floatN v,
                                         ref floatN au, ref floatN d,
                                         int maxIter, float tol)
            where TOp : struct, IfloatLinearOperator
        {
            floatN uHat = default;
            return tfqmr(in A, default(floatIdentityPreconditioner), in b, ref x, ref rHat0, ref u, ref w, ref v, ref au, ref d, ref uHat, maxIter, tol);
        }

        /// <summary>
        /// TFQMR over a dense <see cref="floatMxN"/> -- zero-alloc primitive. Forwards into
        /// <see cref="tfqmr{TOp}"/> via <see cref="floatDenseOperator"/>.
        /// </summary>
        public static SolveInfo tfqmr(in floatMxN A, in floatN b, ref floatN x,
                                    ref floatN rHat0, ref floatN u, ref floatN w, ref floatN v,
                                    ref floatN au, ref floatN d,
                                    int maxIter, float tol)
        {
        ...
        public static SolveInfo tfqmr(in floatMxN A, in floatN b, ref floatN x, int maxIter, float tol)
        {
            ...
            floatN au    = b.floatTempVec(A.M_Rows);
            floatN d     = b.floatTempVec(A.M_Rows);
            return tfqmr(in A, in b, ref x, ref rHat0, ref u, ref w, ref v, ref au, ref d, maxIter, tol);
        }

        /// <summary>TFQMR over a dense matrix with default maxIter (A.M_Rows) and tol (Consts.floatSqrtEps).</summary>
        public static SolveInfo tfqmr(in floatMxN A, in floatN b, ref floatN x)
        {
            return tfqmr(in A, in b, ref x, A.M_Rows, Consts.floatSqrtEps);
        }

        /// <summary>
        /// TFQMR over a block-sparse (BSR) matrix -- zero-alloc primitive. Forwards into
        /// <see cref="tfqmr{TOp}"/> via <c>floatBSROperator</c>.
        /// </summary>
        public static SolveInfo tfqmr(in floatBSR A, in floatN b, ref floatN x,
                                    ref floatN rHat0, ref floatN u, ref floatN w, ref floatN v,
                                    ref floatN au, ref floatN d,
                                    int maxIter, float tol)
        {
        ...
        /// <summary>TFQMR over a BSR matrix with default maxIter (A.M_Rows) and tol (Consts.floatSqrtEps).</summary>
        public static SolveInfo tfqmr(in floatBSR A, in floatN b, ref floatN x)
        {
            return tfqmr(in A, in b, ref x, A.M_Rows, Consts.floatSqrtEps);
        }
        ...
        /// <see cref="IfloatPreconditioner"/> (ILU0) -- forwards into <see cref="tfqmr{TOp,TPre}"/>
        /// via <c>floatBSROperator</c>.
        /// </summary>
        public static SolveInfo tfqmr<TPre>(in floatBSR A, in TPre M, in floatN b, ref floatN x,
                               int maxIter, float tol)
            where TPre : struct, IfloatPreconditioner
        {
            floatN rHat0 = b.floatTempVec(A.M_Rows);
            ...
            floatN au    = b.floatTempVec(A.M_Rows);
            floatN d     = b.floatTempVec(A.M_Rows);
            floatN uHat  = b.floatTempVec(A.M_Rows);
            return tfqmr(new floatBSROperator(in A), in M, in b, ref x,
                             ref rHat0, ref u, ref w, ref v, ref au, ref d, ref uHat,
                             maxIter, tol);
        }

        /// <summary>Preconditioned TFQMR over BSR with ANY <see cref="IfloatPreconditioner"/> (ILU0),
        /// with default maxIter (A.M_Rows) and tolerance (Consts.floatSqrtEps).</summary>
        public static SolveInfo tfqmr<TPre>(in floatBSR A, in TPre M, in floatN b, ref floatN x)
            where TPre : struct, IfloatPreconditioner
        {
            return tfqmr(in A, in M, in b, ref x, A.M_Rows, Consts.floatSqrtEps);
        }
    }
}
