...
namespace BULA
{
    public static partial class Krylov {
        ...
        static void BlockResidual<TOp>(in TOp A, in doubleMxN X, in doubleMxN B, ref doubleMxN applied, ref doubleMxN R, int s, int n)
            where TOp : struct, IdoubleLinearOperator
        {
        ...
        static void BlockResidual<TOp>(in TOp A, in doubleMxN X, in doubleMxN B, ref doubleMxN R, int s, int n)
            where TOp : struct, IdoubleLinearOperator
        {
        ...
        static unsafe int RowOrthoRankFloored(in doubleMxN W, ref doubleMxN Qout, int m, int nGlobal, double scale)
        {
            if (m <= 0) return 0;
            int n = W.N_Cols;
            double* q = Qout.Data.Ptr;
            UnsafeUtility.MemCpy(q, W.Data.Ptr, (long)m * n * UnsafeUtility.SizeOf<double>());
            ...
            int pivotRow = 0;
            double pivotNormSq = UnsafeOP.vecDot(q, q, n);
            for (int i = 1; i < m; i++)
            {
                double ns = UnsafeOP.vecDot(q + (long)i * n, q + (long)i * n, n);
                if (ns > pivotNormSq) { pivotNormSq = ns; pivotRow = i; }
            }

            double relTol = (double)math.max(m, nGlobal) * Consts.doubleZeroThreshold;
            double diag0 = (double)0;
            int rank = 0;
            ...
            for (int d = 0; d < m; d++)
            {
                ...
                double pivotNorm = math.sqrt(pivotNormSq);
                if (d == 0)
                {
                ...
                double tol = math.max(relTol * diag0, relTol * scale);
                if (!(pivotNorm > tol))
                    break;
                rank = d + 1;

                double* qd = q + (long)d * n;
                double inv = (double)1 / pivotNorm;
                ...
                int nextPivot = d + 1;
                double nextNormSq = (double)(-1);
                for (int i = d + 1; i < m; i++)
                {
                    double* qi = q + (long)i * n;
                    double dot = UnsafeOP.vecDot(qi, qd, n);
                    double ns = UnsafeOP.axpyNormSq(qi, qd, -dot, n);
        ...
        static int BlockArnoldiMGS2Step(ref doubleMxN Wj, in UnsafeList<doubleMxN> V, ref Indices w, ref Indices off,
                                         ref doubleMxN Hbuf, ref doubleMxN HijBuf, ref doubleMxN Tbuf,
