...
namespace BULA.Control
{
    ...
    public static partial class MPC
    {
        ...
        public static void setSoftBound(ref doubleMPCState s, in doubleN d)
        {
            if (!s.IsCreated)
                throw new ArgumentException("MPC.setSoftBound: state must be constructed via new doubleMPCState(...)");
            if (!s.hasSoftRows)
            ...
            if (s.hasSoftRows)
            {
                ...
                for (int k = 0; k < N; k++)
                {
                    for (int si = 0; si < s.nSoftPerStage; si++)
                    {
                        ...
                        double viol = act - s.rowConstRHS[rowIdx];
