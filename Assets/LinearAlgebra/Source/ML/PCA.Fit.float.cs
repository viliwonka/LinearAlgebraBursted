...
namespace LinearAlgebra.ML
{
    ...
    // without assembling a floatPCAModel yourself. Always uses PCAScaling.Covariance (never
    // Correlation, which would rescale axes independently and break the perpendicular-distance
    // meaning of a geometric fit). Zero-copy: each NativeArray<float2/3/4> point cloud is reinterpreted
    // as a flat NativeArray<float> and viewed as an n x {2,3,4} floatMxN (row-major AoS == the
    ...
    public static partial class PCA
    {
        ...
        /// <see cref="fitCov(in floatMxN, ref floatPCAModel)"/>); direction is undefined when false.
        /// </summary>
        public static bool fitLine(NativeArray<float2> points, out float2 centroid, out float2 direction)
        {
            ...
            var model = new floatPCAModel(2, 2, Allocator.Temp);
            bool converged = fitCov(in X, ref model);

            centroid = new float2(model.mean[0], model.mean[1]);
            direction = converged ? new float2(model.components[0, 0], model.components[1, 0]) : default;
        ...
        /// <see cref="fitCov(in floatMxN, ref floatPCAModel)"/>); direction is undefined when false.
        /// </summary>
        public static bool fitLine(NativeArray<float3> points, out float3 centroid, out float3 direction)
        {
            const string method = "PCA.fitLine";
            if (points.Length < 2)
                throw new ArgumentException(method + ": points.Length must be >= 2");
            ...
            var model = new floatPCAModel(3, 3, Allocator.Temp);
            bool converged = fitCov(in X, ref model);

            centroid = new float3(model.mean[0], model.mean[1], model.mean[2]);
            direction = converged
                ? new float3(model.components[0, 0], model.components[1, 0], model.components[2, 0])
                : default;
        ...
        /// <see cref="fitCov(in floatMxN, ref floatPCAModel)"/>); direction is undefined when false.
        /// </summary>
        public static bool fitLine(NativeArray<float4> points, out float4 centroid, out float4 direction)
        {
            const string method = "PCA.fitLine";
            if (points.Length < 2)
                throw new ArgumentException(method + ": points.Length must be >= 2");
            ...
            var model = new floatPCAModel(4, 4, Allocator.Temp);
            bool converged = fitCov(in X, ref model);

            centroid = new float4(model.mean[0], model.mean[1], model.mean[2], model.mean[3]);
            direction = converged
                ? new float4(model.components[0, 0], model.components[1, 0], model.components[2, 0], model.components[3, 0])
                : default;
        ...
        /// eigensolve's convergence flag (see <see cref="fitCov(in floatMxN, ref floatPCAModel)"/>);
        /// normal is undefined when false.
        /// </summary>
        public static bool fitPlane(NativeArray<float3> points, out float3 centroid, out float3 normal)
        {
            ...
            centroid = new float3(model.mean[0], model.mean[1], model.mean[2]);
            normal = converged
                ? new float3(model.components[0, 2], model.components[1, 2], model.components[2, 2])
                : default;
