    ...
    public static partial class KMeans
    {
            ...
            ref doubleKMeansCache ws)
        {
            ...
            unsafe
            {
                ...
                for (int n = 0; n < N; n++)
                {
                    double* xr = xp + (long)n * D;
                    double s = (double)0;
                    for (int f = 0; f < D; f++) { double v = xr[f]; s += v * v; }
                    ws.PointNormSq[n] = s;
                }
            }
            ...
            for (int iter = 0; iter < maxIter; iter++)
            {
                ...
                unsafe
                {
                    double* cp = centroids.Data.Ptr;
                    for (int j = 0; j < k; j++)
                    {
                        double* cr = cp + (long)j * D;
                        double s = (double)0;
                        for (int f = 0; f < D; f++) { double v = cr[f]; s += v * v; }
                        ws.CentNormSq[j] = s;
                    }
                }
                ...
                unsafe
                {
                    double* gp = ws.Gram.Data.Ptr; double* cn = ws.CentNormSq.Data.Ptr;
                    for (int n = 0; n < N; n++)
                    {
                        double* gr = gp + (long)n * k;
                        for (int j = 0; j < k; j++)
                            gr[j] = cn[j] - (double)2 * gr[j];
                    }
                }
                ...
                unsafe
                {
                    double* ncp = ws.NewCentroids.Data.Ptr;
                    for (int j = 0; j < k; j++)
                    {
                        double* ncr = ncp + (long)j * D;
                        for (int f = 0; f < D; f++) ncr[f] = (double)0;
                ...
                unsafe
                {
                    double* ncp = ws.NewCentroids.Data.Ptr; double* xp = X.Data.Ptr;
                    for (int n = 0; n < N; n++)
                    {
                        int j = assignment[n];
                        ws.ClusterCounts[j]++;
                        double* ncr = ncp + (long)j * D; double* xr = xp + (long)n * D;
                ...
                unsafe
                {
                    double* cp = centroids.Data.Ptr; double* ncp = ws.NewCentroids.Data.Ptr;
                    for (int j = 0; j < k; j++)
                    {
                        double invN = (double)1 / (double)ws.ClusterCounts[j];
                        double* cr = cp + (long)j * D; double* ncr = ncp + (long)j * D;
            ...
            Allocator allocator = Allocator.Temp)
        {
            ...
            int D = X.N_Cols;
            int kk = math.min(k, N);  // match the primary overload's clamp
            centroids  = new doubleMxN(kk, D, allocator);
            assignment = new Indices(N, allocator);
            var ws     = new doubleKMeansCache(N, D, kk, Allocator.Temp);
            fit(in X, k, seed, maxIter, init, ref centroids, ref assignment,
                   out inertia, out iters, ref ws);
        }
        ...
        /// <summary>
        /// Calls <see cref="fit(in doubleMxN,int,uint,int,KMeansInit,out doubleMxN,out Indices,out double,out int,Allocator)"/>
        /// with <c>init = KMeansInit.KMeansPlusPlus</c>.
        /// </summary>
        public static void fit(
            in doubleMxN X,
            ...
            ref doubleKMeansCache ws)
        {
            ...
            unsafe
            {
                double* xp = X.Data.Ptr; double* c0 = centroids.Data.Ptr;
                for (int n = 0; n < N; n++)
                {
                    double* xr = xp + (long)n * D;
                    double d2 = (double)0;
            ...
            for (int ci = 1; ci < k; ci++)
            {
                ...
                if (ci < k - 1)
                {
                    unsafe
                    {
                        double* xp = X.Data.Ptr; double* cci = centroids.Data.Ptr + (long)ci * D;
                        for (int n = 0; n < N; n++)
                        {
                            double* xr = xp + (long)n * D;
                            double d2 = (double)0;
