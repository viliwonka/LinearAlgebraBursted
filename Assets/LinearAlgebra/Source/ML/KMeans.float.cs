// <auto-generated>
//   Generated from Assets/LinearAlgebra/CodeGen/TemplateSource/ML/KMeans.fProxy.cs
    ...
    public static partial class KMeans
    {
            ...
            ref floatKMeansCache ws)
        {
            ...
            unsafe
            {
                ...
                for (int n = 0; n < N; n++)
                {
                    float* xr = xp + (long)n * D;
                    float s = (float)0;
                    for (int f = 0; f < D; f++) { float v = xr[f]; s += v * v; }
                    ws.PointNormSq[n] = s;
                }
            }
            ...
            for (int iter = 0; iter < maxIter; iter++)
            {
                ...
                unsafe
                {
                    float* cp = centroids.Data.Ptr;
                    for (int j = 0; j < k; j++)
                    {
                        float* cr = cp + (long)j * D;
                        float s = (float)0;
                        for (int f = 0; f < D; f++) { float v = cr[f]; s += v * v; }
                        ws.CentNormSq[j] = s;
                    }
                }
                ...
                unsafe
                {
                    float* gp = ws.Gram.Data.Ptr; float* cn = ws.CentNormSq.Data.Ptr;
                    for (int n = 0; n < N; n++)
                    {
                        float* gr = gp + (long)n * k;
                        for (int j = 0; j < k; j++)
                            gr[j] = cn[j] - (float)2 * gr[j];
                    }
                }
                ...
                unsafe
                {
                    float* ncp = ws.NewCentroids.Data.Ptr;
                    for (int j = 0; j < k; j++)
                    {
                        float* ncr = ncp + (long)j * D;
                        for (int f = 0; f < D; f++) ncr[f] = (float)0;
                ...
                unsafe
                {
                    float* ncp = ws.NewCentroids.Data.Ptr; float* xp = X.Data.Ptr;
                    for (int n = 0; n < N; n++)
                    {
                        int j = assignment[n];
                        ws.ClusterCounts[j]++;
                        float* ncr = ncp + (long)j * D; float* xr = xp + (long)n * D;
                ...
                unsafe
                {
                    float* cp = centroids.Data.Ptr; float* ncp = ws.NewCentroids.Data.Ptr;
                    for (int j = 0; j < k; j++)
                    {
                        float invN = (float)1 / (float)ws.ClusterCounts[j];
                        float* cr = cp + (long)j * D; float* ncr = ncp + (long)j * D;
            ...
            Allocator allocator = Allocator.Temp)
        {
            ...
            int D = X.N_Cols;
            int kk = math.min(k, N);  // match the primary overload's clamp
            centroids  = new floatMxN(kk, D, allocator);
            assignment = new Indices(N, allocator);
            var ws     = new floatKMeansCache(N, D, kk, Allocator.Temp);
            fit(in X, k, seed, maxIter, init, ref centroids, ref assignment,
                   out inertia, out iters, ref ws);
        }
        ...
        /// <summary>
        /// Calls <see cref="fit(in floatMxN,int,uint,int,KMeansInit,out floatMxN,out Indices,out float,out int,Allocator)"/>
        /// with <c>init = KMeansInit.KMeansPlusPlus</c>.
        /// </summary>
        public static void fit(
            in floatMxN X,
            ...
            ref floatKMeansCache ws)
        {
            ...
            unsafe
            {
                float* xp = X.Data.Ptr; float* c0 = centroids.Data.Ptr;
                for (int n = 0; n < N; n++)
                {
                    float* xr = xp + (long)n * D;
                    float d2 = (float)0;
            ...
            for (int ci = 1; ci < k; ci++)
            {
                ...
                if (ci < k - 1)
                {
                    unsafe
                    {
                        float* xp = X.Data.Ptr; float* cci = centroids.Data.Ptr + (long)ci * D;
                        for (int n = 0; n < N; n++)
                        {
                            float* xr = xp + (long)n * D;
                            float d2 = (float)0;
