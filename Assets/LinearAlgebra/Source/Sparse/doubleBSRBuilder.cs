...
namespace LinearAlgebra.Sparse
{
    ...
    /// separate, supported path: BuildAssemblyCache + Refill (doubleBSRAssembly.double.cs).
    ///
    ...
    public partial struct doubleBSRBuilder : IDisposable
    {
        ...
        /// <summary>
        /// Same as ToBSR, but builds SYMMETRIC lower-block storage (see doubleBSR.Symmetric / spec
        ...
        public unsafe doubleBSR ToBSR(Allocator allocator) => ToBSRCore(allocator, symmetric: false);
        ...
        public unsafe doubleBSR ToBSRSymmetric(Allocator allocator)
        {
            if (BR != BC || BlockRows != BlockCols)
                throw new ArgumentException("ToBSRSymmetric: requires BR==BC and BlockRows==BlockCols (square blocks on a square block grid)");
        ...
        // the final doubleBSR allocation source differs (allocator instead of an arena record).
        private unsafe doubleBSR ToBSRCore(Allocator allocator, bool symmetric)
        {
            int n = TripletCount;
            int blockLen = BR * BC;
            ...
            var bsm = new doubleBSR(BlockRows, BlockCols, BR, BC, nnzb, allocator, true, symmetric);
