...
namespace LinearAlgebra.Sparse
{
    ...
    /// A's own row pattern) -- the nonsymmetric sibling of <see cref="floatIC0"/>/<see
    /// cref="floatFSAI"/>'s counterpart <see cref="floatILU0"/>, intended for
    ...
    /// Dispose().
    /// </summary>
    public readonly struct floatSPAI : IfloatPreconditioner
    {
        /// <summary>M's storage: A's own full block pattern (MVP default).</summary>
        public readonly floatBSR M;
        ...
        public floatSPAI(in floatBSR a, ref Arena arena, in SaiOptions opts, out PreconditionerInfo info)
        {
            ...
            for (int i = 0; i < nb; i++)
            {
                ...
                var Nbase = new floatMxN(nJ, nJ, Allocator.Temp, true);
                ...
                var RHSbase = new floatMxN(nJ, BR, Allocator.Temp, true);
                for (int r = 0; r < nJ; r++)
                    for (int c = 0; c < BR; c++)
                        RHSbase[r, c] = Ahat[r, iLocal * BR + c];

                var Nwork = new floatMxN(nJ, nJ, Allocator.Temp, true);
                var RHSwork = new floatMxN(nJ, BR, Allocator.Temp, true);
        ...
        public unsafe void Apply(in floatN r, ref floatN z)
        {
            int n = Rows;
            if (r.N != n)
                throw new ArgumentException("floatSPAI.Apply: r.N must equal Rows");
            if (z.N != n)
                throw new ArgumentException("floatSPAI.Apply: z.N must equal Rows");
            if (z.Data.Ptr == r.Data.Ptr)
                throw new ArgumentException("floatSPAI.Apply: z must not alias r");
