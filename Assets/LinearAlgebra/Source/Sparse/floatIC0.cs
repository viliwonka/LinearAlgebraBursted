...
namespace LinearAlgebra.Sparse
{
    ...
    public readonly struct floatIC0 : IfloatPreconditioner
    {
        ...
        {
            this = new floatIC0(in a, ref arena, out DirectSolveInfo info);
            if (!info.Solved)
        ...
        /// </summary>
        public floatIC0(in floatBSR a, ref Arena arena, out DirectSolveInfo info)
        {
        ...
        static void CopyLowerFromA(in floatBSR A, in floatBSR Lm, float shift)
        {
            int nb = A.BlockRows;
            int BR = A.BR;
            int blockLen = BR * BR;
        ...
        static bool FactorizeInPlace(in floatBSR Lm, float diagMax)
        {
            int nb = Lm.BlockRows;
            int BR = Lm.BR;
            int blockLen = BR * BR;
        ...
        static void BlockMulSubABt(Unity.Collections.LowLevel.Unsafe.UnsafeList<float> v, int cOff, int aOff, int bOff, int BR)
        {
            for (int r = 0; r < BR; r++)
                for (int c = 0; c < BR; c++)
                {
        ...
        static void ForwardSolveRow(Unity.Collections.LowLevel.Unsafe.UnsafeList<float> v, int rowOff, int ljjOff, int BR)
        {
            for (int c = 0; c < BR; c++)
            {
                float sum = v[rowOff + c];
                for (int t = 0; t < c; t++)
        ...
        static bool CholBlockLower(Unity.Collections.LowLevel.Unsafe.UnsafeList<float> v, int dOff, int BR, float pivotFloor)
        {
            for (int r = 0; r < BR; r++)
            {
                for (int c = 0; c <= r; c++)
                {
                    float sum = v[dOff + r * BR + c];
                    for (int t = 0; t < c; t++)
        ...
        public unsafe void Apply(in floatN r, ref floatN z)
        {
            int n = Rows;
            if (r.N != n)
                throw new ArgumentException("floatIC0.Apply: r.N must equal Rows");
            if (z.N != n)
                throw new ArgumentException("floatIC0.Apply: z.N must equal Rows");
            if (z.Data.Ptr == r.Data.Ptr)
                throw new ArgumentException("floatIC0.Apply: z must not alias r");
