...
namespace LinearAlgebra.Sparse
{
    ...
    /// <see cref="doubleBSRBuilder.BuildAssemblyCache"/>; every later frame:
    /// builder.Clear(), re-Add the SAME blocks in the SAME order with new values, then
    /// <see cref="doubleBSRBuilder.Refill"/> — no sorting, no allocation, O(nnz) scatter-add.
    /// Buffers are arena-owned (disposed with the arena).
    /// </summary>
    public struct doubleBSRAssemblyCache
    {
    ...
    public partial struct doubleBSRBuilder
    {
        /// <summary>
        /// Builds the triplet-to-slot map for the CURRENT triplet set (same sort + duplicate
        /// merge as ToBSR). Pair it with the doubleBSR that the same triplet set produced.
        /// </summary>
        public unsafe doubleBSRAssemblyCache BuildAssemblyCache(ref Arena arena)
        {
            int n = TripletCount;
            if (n == 0)
                return new doubleBSRAssemblyCache { nnzb = 0, tripletCount = 0 };

            SortTriplets(out var order, out var rowStart);

            var cache = new doubleBSRAssemblyCache
            {
        ...
        public unsafe doubleBSRAssemblyCache BuildAssemblyCache(Allocator allocator = Allocator.Temp)
        {
            int n = TripletCount;
            if (n == 0)
                return new doubleBSRAssemblyCache { nnzb = 0, tripletCount = 0 };
        ...
        /// </summary>
        public unsafe void Refill(in doubleBSRAssemblyCache cache, in doubleBSR dst)
        {
            int n = TripletCount;
            ...
            UnsafeUtility.MemClear(values.Ptr, (long)values.Length * UnsafeUtility.SizeOf<double>());
