...
namespace LinearAlgebra.Sparse
{
    ...
    public readonly struct doubleSPAI : IdoublePreconditioner
    {
        ...
        /// </summary>
        public doubleSPAI(in doubleBSR a, ref Arena arena)
        {
            this = new doubleSPAI(in a, ref arena, out PreconditionerInfo info);
            if (!info.Solved)
        ...
        public doubleSPAI(in doubleBSR a, ref Arena arena, out PreconditionerInfo info)
        {
            this = new doubleSPAI(in a, ref arena, SaiOptions.Default, out info);
        }

        /// <summary>Builds SPAI with the given <see cref="SaiOptions"/>. Same throw contract as the
        /// options-less overload.</summary>
        public doubleSPAI(in doubleBSR a, ref Arena arena, in SaiOptions opts)
        {
            this = new doubleSPAI(in a, ref arena, in opts, out PreconditionerInfo info);
            if (!info.Solved)
                throw new ArgumentException("doubleSPAI: a row's local least-squares solve broke down at every Tikhonov shift");
        }
        ...
        /// </summary>
        public doubleSPAI(in doubleBSR a, ref Arena arena, in SaiOptions opts, out PreconditionerInfo info)
        {
            ...
            var Mm = arena.doubleBSR(nb, nb, BR, BR, A.Nnzb, true);
            ...
            for (int i = 0; i < nb; i++)
            {
                ...
                var Nbase = new doubleMxN(nJ, nJ, Allocator.Temp, true);
                ...
                var RHSbase = new doubleMxN(nJ, BR, Allocator.Temp, true);
                for (int r = 0; r < nJ; r++)
                    for (int c = 0; c < BR; c++)
                        RHSbase[r, c] = Ahat[r, iLocal * BR + c];

                var Nwork = new doubleMxN(nJ, nJ, Allocator.Temp, true);
                var RHSwork = new doubleMxN(nJ, BR, Allocator.Temp, true);
