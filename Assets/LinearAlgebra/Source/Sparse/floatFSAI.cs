...
namespace LinearAlgebra.Sparse
{
    ...
    /// pattern, diagonal included -- same as <see cref="floatIC0"/>'s) such that M = Gᵀ G ≈ A⁻¹.
    ...
    public readonly struct floatFSAI : IfloatPreconditioner
    {
        ...
        /// non-throwing build.
        /// </summary>
        public floatFSAI(in floatBSR a, ref Arena arena)
        {
            this = new floatFSAI(in a, ref arena, out PreconditionerInfo info);
            if (!info.Solved)
        ...
        public floatFSAI(in floatBSR a, ref Arena arena, out PreconditionerInfo info)
        {
            this = new floatFSAI(in a, ref arena, SaiOptions.Default, out info);
        }

        /// <summary>Builds FSAI with the given <see cref="SaiOptions"/>. Same throw contract as the
        /// options-less overload.</summary>
        public floatFSAI(in floatBSR a, ref Arena arena, in SaiOptions opts)
        {
            this = new floatFSAI(in a, ref arena, in opts, out PreconditionerInfo info);
            if (!info.Solved)
                throw new ArgumentException("floatFSAI: a row's local solve broke down at every diagonal shift — is A symmetric positive definite?");
        }
        ...
        /// </summary>
        public floatFSAI(in floatBSR a, ref Arena arena, in SaiOptions opts, out PreconditionerInfo info)
        {
            ...
            var Gm = arena.floatBSR(nb, nb, BR, BR, nnzbG, true);
            ...
            G = Gm;
            Gt = arena.floatBSRTranspose(in Gm);
            Scratch = arena.floatVec(nb * BR, true);
            Shift = worstShift;
        ...
        static bool BelowDropTol(in floatBSR A, int k, int blockLen, float diagNormI, float diagNormJ, float dropTol)
        {
            int off = k * blockLen;
            float normSq = 0;
            for (int t = 0; t < blockLen; t++)
            ...
            float threshold = dropTol * math.sqrt(diagNormI * diagNormJ);
        ...
        static void FillLowerGather(in floatBSR A, in floatBSR G, int rowStart, int m, int BR, float shift, ref floatMxN Ahat)
        {
            for (int aI = 0; aI < m; aI++)
            {
                ...
                for (int bI = 0; bI <= aI; bI++)
                {
                    ...
                    GatherBlockInto(in A, ga, gb, ref Ahat, aI * BR, bI * BR, aI == bI ? shift : (float)0);
                }
            }
        }

        // E (n x BR): zero except an I_BR block in the LAST block-row slot.
        static void FillIdentityLastBlock(ref floatMxN X, int m, int BR)
        {
            int n = X.M_Rows;
            for (int r = 0; r < n; r++)
                for (int c = 0; c < BR; c++)
        ...
        static void CopyLastBlock(in floatMxN X, int m, int BR, ref floatMxN D)
        {
            int last = (m - 1) * BR;
            for (int r = 0; r < BR; r++)
                for (int c = 0; c < BR; c++)
        ...
        static void TransposeInto(in floatMxN X, ref floatMxN Xt)
        {
        ...
        static void ScatterRow(in floatMxN Xt, int m, int BR, UnsafeList<float> gValues, int rowStart)
        {
            int blockLen = BR * BR;
            for (int aI = 0; aI < m; aI++)
            {
        ...
        // ---- shared gather primitives (also used by floatSPAI) ----
        ...
        internal static int FindBlockIndex(in floatBSR A, int row, int col)
        {
        ...
        internal static void GatherBlockInto(in floatBSR A, int gr, int gc, ref floatMxN dst, int rowOff, int colOff, float diagShift)
        {
            int BR = A.BR, BC = A.BC;
        ...
        public unsafe void Apply(in floatN r, ref floatN z)
        {
            int n = Rows;
            if (r.N != n)
                throw new ArgumentException("floatFSAI.Apply: r.N must equal Rows");
            if (z.N != n)
                throw new ArgumentException("floatFSAI.Apply: z.N must equal Rows");
            if (z.Data.Ptr == r.Data.Ptr)
                throw new ArgumentException("floatFSAI.Apply: z must not alias r");
            if (z.Data.Ptr == Scratch.Data.Ptr)
                throw new ArgumentException("floatFSAI.Apply: z must not alias Scratch");
            if (r.Data.Ptr == Scratch.Data.Ptr)
                throw new ArgumentException("floatFSAI.Apply: r must not alias Scratch");

            floatN scratch = Scratch;   // local alias sharing Scratch's buffer -- Scratch itself is
