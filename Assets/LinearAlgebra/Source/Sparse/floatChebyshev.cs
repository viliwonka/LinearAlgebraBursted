// <auto-generated>
//   Generated from Assets/LinearAlgebra/CodeGen/TemplateSource/Sparse/fProxyChebyshev.cs
//   DO NOT EDIT BY HAND - edit the template and run Tools/regen.ps1.
// </auto-generated>
using System;
using LinearAlgebra;
using Unity.Mathematics;
using Unity.Collections;

namespace LinearAlgebra.Sparse
{
    /// <summary>Setup/Apply knobs for <see cref="floatChebyshev"/>.</summary>
    public struct floatChebyshevOptions
    {
        /// <summary>spMVs per Apply; must be &gt;= 1.</summary>
        public int degree;

        /// <summary>Lo = Hi / kappa; must be &gt; 1.</summary>
        public float kappa;

        /// <summary>Pinned Lanczos steps used to estimate Hi; must be &gt;= 1.</summary>
        public int eigSteps;

        /// <summary>Hi = safety * (Lanczos estimate of the scaled operator's largest eigenvalue); must be &gt;= 1.</summary>
        public float safety;

        /// <summary>degree=3, kappa=30, eigSteps=10, safety=1.1.</summary>
        public static floatChebyshevOptions Default => new floatChebyshevOptions
        {
            degree = 3,
            kappa = (float)30,
            eigSteps = 10,
            safety = (float)1.1,
        };
    }

    /// <summary>
    /// Symmetric Jacobi-scaled wrapper S·A·S (S = D^(-1/2), D = diag(A)) -- same spectrum as
    /// D⁻¹A but symmetric, so a symmetric Lanczos tridiagonalization applies to it. Used only by
    /// <see cref="floatChebyshev"/>'s setup; not part of the public API.
    /// </summary>
    internal readonly struct floatJacobiScaledBSROperator : IfloatLinearOperator
    {
        public readonly floatBSR A;
        public readonly floatN InvSqrtD;   // D^(-1/2), length n
        public readonly floatN Scratch;    // length n

        public floatJacobiScaledBSROperator(in floatBSR a, in floatN invSqrtD, in floatN scratch)
        {
            A = a;
            InvSqrtD = invSqrtD;
            Scratch = scratch;
        }

        public int Rows => A.M_Rows;
        public int Cols => A.N_Cols;

        // y = S A S x: s = InvSqrtD . x ; y = A s ; y = InvSqrtD . y.
        public void Apply(in floatN x, ref floatN y)
        {
            int n = InvSqrtD.N;
            for (int i = 0; i < n; i++) Scratch[i] = InvSqrtD[i] * x[i];
            BSR.spMV(in A, in Scratch, ref y);
            for (int i = 0; i < n; i++) y[i] = InvSqrtD[i] * y[i];
        }

        public void ApplyT(in floatN x, ref floatN y) => Apply(in x, ref y);   // symmetric

        // Composes Apply + a plain dot; no fused kernel here.
        public float ApplyDot(in floatN x, ref floatN y)
        {
            Apply(in x, ref y);
            return Blas.dot(x, y);
        }

        // Per-row fallback via Apply (floatColScaledOperator's Temp-buffer pattern). Lanczos
        // never calls this -- present only to satisfy the interface.
        public void ApplyBlock(in floatMxN Vrows, ref floatMxN AVrows, int rows)
        {
            int n = InvSqrtD.N;
            var rin = new floatN(n, Unity.Collections.Allocator.Temp, false);
            var rout = new floatN(n, Unity.Collections.Allocator.Temp, false);
            for (int i = 0; i < rows; i++)
            {
                for (int c = 0; c < n; c++) rin[c] = Vrows[i, c];
                Apply(in rin, ref rout);
                for (int c = 0; c < n; c++) AVrows[i, c] = rout[c];
            }
            rout.Dispose();
            rin.Dispose();
        }
    }

    /// <summary>
    /// Chebyshev polynomial preconditioner over a square SPD BSR: z = q(D⁻¹A)·D⁻¹r, q a degree-
    /// <see cref="Degree"/> Chebyshev polynomial on [<see cref="Lo"/>, <see cref="Hi"/>] bracketing
    /// the upper spectrum of the point-Jacobi-scaled operator D⁻¹A (D = diag(A), scalar diagonal).
    /// Apply is dot-free: exactly <see cref="Degree"/> spMVs, Degree+1 diagonal scales, ~2·Degree
    /// axpys, zero triangular solves.
    ///
    /// Requires A square (BlockRows==BlockCols, BR==BC), every diagonal block stored, and every
    /// scalar diagonal entry A[i,i] &gt; 0. The induced M⁻¹ is SPD as long as no eigenvalue of
    /// D⁻¹A exceeds <see cref="Hi"/>: setup estimates Hi from a pinned-eigSteps Lanczos run on the
    /// symmetrically-scaled operator D^(-1/2)·A·D^(-1/2), scaled by
    /// <see cref="floatChebyshevOptions.safety"/>; Lo = Hi / <see cref="floatChebyshevOptions.kappa"/>.
    /// Underestimating Hi breaks the SPD guarantee; overestimating only weakens the preconditioner.
    ///
    /// Accepts either Symmetric (lower-block-only) or full storage -- <see cref="BSR.spMV"/>
    /// handles both natively, so unlike <see cref="floatSSOR"/> no mirror-to-full copy is needed.
    ///
    /// Built via a ref Arena ctor, InvDiag/Scratch1-3 are arena-tracked and the arena owns disposal
    /// -- do not call Dispose() on that instance. Built via an Allocator ctor, this instance owns
    /// those buffers standalone and Dispose() must be called when done (A itself is never owned --
    /// see Dispose()'s own doc). All fields are readonly, set once at construction: IJob-struct-copy-safe.
    /// </summary>
    public readonly struct floatChebyshev : IfloatPreconditioner, IDisposable
    {
        public readonly floatBSR A;

        /// <summary>1 / diag(A), the scalar (point) Jacobi diagonal. Length Rows.</summary>
        public readonly floatN InvDiag;

        /// <summary>Final interval bracketing the upper spectrum of D⁻¹A (diagnostics -- already
        /// folded into Theta/Delta/Sigma below).</summary>
        public readonly float Lo, Hi;

        /// <summary>Chebyshev recurrence coefficients: Theta=(Hi+Lo)/2, Delta=(Hi-Lo)/2, Sigma=Theta/Delta.</summary>
        public readonly float Theta, Delta, Sigma;

        /// <summary>spMVs per Apply.</summary>
        public readonly int Degree;

        /// <summary>Apply's owned scratch (d, rk, t of the recurrence). Length Rows.</summary>
        public readonly floatN Scratch1, Scratch2, Scratch3;

        public int Rows => A.M_Rows;

        /// <summary>
        /// Builds InvDiag, then Hi/Lo from a Lanczos run on the symmetrically-scaled operator (the
        /// step count is clamped to A.Rows, so a system smaller than opt.eigSteps builds fine), then
        /// the Chebyshev recurrence coefficients. Throws ArgumentException if A is not square
        /// (BlockRows==BlockCols, BR==BC), a diagonal block is missing, any scalar diagonal entry
        /// A[i,i] &lt;= 0, the Lanczos eigen-estimate fails to converge or yields a non-positive
        /// largest eigenvalue ("is A symmetric positive definite?"), opt.degree &lt; 1, opt.kappa
        /// &lt;= 1, opt.eigSteps &lt; 1, or opt.safety &lt; 1.
        /// </summary>
        public floatChebyshev(in floatBSR a, in floatChebyshevOptions opt, ref Arena arena)
        {
            if (a.BlockRows != a.BlockCols || a.BR != a.BC)
                throw new ArgumentException("floatChebyshev: A must be square (BlockRows==BlockCols, BR==BC)");
            if (opt.degree < 1)
                throw new ArgumentException("floatChebyshev: opt.degree must be >= 1");
            if (!(opt.kappa > (float)1))
                throw new ArgumentException("floatChebyshev: opt.kappa must be > 1");
            if (opt.eigSteps < 1)
                throw new ArgumentException("floatChebyshev: opt.eigSteps must be >= 1");
            if (!(opt.safety >= (float)1))
                throw new ArgumentException("floatChebyshev: opt.safety must be >= 1");

            A = a;
            int n = a.M_Rows;
            int BR = a.BR;
            int blockLen = BR * BR;

            // ---- InvDiag: 1 / A[i,i] over the stored scalar diagonal (both storage modes store
            // the diagonal block) -- same block-row scan floatSSOR's ctor uses to find it. ----
            var invDiag = arena.floatVec(n);
            for (int i = 0; i < a.BlockRows; i++)
            {
                int s = a.RowPtr[i], e = a.RowPtr[i + 1];
                int found = -1;
                for (int k = s; k < e; k++)
                {
                    int col = a.ColInd[k];
                    if (col == i) { found = k; break; }
                    if (col > i) break;
                }
                if (found < 0)
                    throw new ArgumentException("floatChebyshev: missing diagonal block in A");

                int off = found * blockLen;
                int rowBase = i * BR;
                for (int r = 0; r < BR; r++)
                {
                    float dv = a.Values[off + r * BR + r];
                    if (!(dv > (float)0))
                        throw new ArgumentException("floatChebyshev: A has a non-positive diagonal entry -- is A symmetric positive definite?");
                    invDiag[rowBase + r] = (float)1 / dv;
                }
            }
            InvDiag = invDiag;

            // ---- Hi/Lo: safety * lambdaMax(D^-1 A), via a pinned Lanczos run on the
            // symmetrically-scaled S.A.S, S = D^(-1/2) (same spectrum as D^-1 A, symmetric). ----
            var invSqrtD = arena.floatVec(n);
            for (int i = 0; i < n; i++)
                invSqrtD[i] = math.sqrt(invDiag[i]);
            var scaledScratch = arena.floatVec(n);
            var scaledOp = new floatJacobiScaledBSROperator(in A, in invSqrtD, in scaledScratch);

            // Lanczos needs steps in [1, n]; clamp so a system smaller than opt.eigSteps still
            // builds (fewer steps only coarsens the estimate, never invalidates it).
            int eigSteps = math.min(opt.eigSteps, n);
            var ws = arena.floatLanczosCache(n, eigSteps);
            var ritz = arena.floatVec(eigSteps);
            var lInfo = Eigen.lanczos(in scaledOp, ref ws, ref ritz, eigSteps);

            float lambdaMax = ritz[0];
            for (int i = 1; i < lInfo.produced; i++)
                if (ritz[i] > lambdaMax) lambdaMax = ritz[i];

            // A failed eigen-estimate (non-converged Lanczos, or a non-positive largest eigenvalue)
            // makes Hi/Sigma garbage and the induced M^-1 indefinite/NaN -- signal a bad SPD build.
            if (!(lInfo.status == IterativeSolveStatus.Converged) || !(lambdaMax > (float)0))
                throw new ArgumentException("floatChebyshev: Lanczos produced no positive largest-eigenvalue estimate for D^-1 A -- is A symmetric positive definite? (raise opt.eigSteps or check A)");

            float hi = opt.safety * lambdaMax;
            float lo = hi / opt.kappa;
            Hi = hi;
            Lo = lo;

            Theta = (hi + lo) / (float)2;
            Delta = (hi - lo) / (float)2;
            Sigma = Theta / Delta;

            Degree = opt.degree;

            Scratch1 = arena.floatVec(n);
            Scratch2 = arena.floatVec(n);
            Scratch3 = arena.floatVec(n);
        }

        /// <summary>floatChebyshev with floatChebyshevOptions.Default (degree=3, kappa=30, eigSteps=10, safety=1.1).</summary>
        public floatChebyshev(in floatBSR a, ref Arena arena) : this(in a, floatChebyshevOptions.Default, ref arena) { }

        /// <summary>
        /// Standalone twin of <see cref="floatChebyshev(in floatBSR, in floatChebyshevOptions, ref Arena)"/>:
        /// allocates InvDiag/Scratch1-3 from <paramref name="allocator"/> instead of an arena; the
        /// Lanczos eigen-estimate's own workspace is Temp-allocated and disposed before returning
        /// (it is setup-only, not part of the built preconditioner). Same validation/throw contract.
        /// Dispose the result with <see cref="Dispose"/> -- only call Dispose on an instance built
        /// via this ctor, never on one built via the ref Arena ctor (that instance is arena-owned).
        /// </summary>
        public unsafe floatChebyshev(in floatBSR a, in floatChebyshevOptions opt, Allocator allocator)
        {
            if (a.BlockRows != a.BlockCols || a.BR != a.BC)
                throw new ArgumentException("floatChebyshev: A must be square (BlockRows==BlockCols, BR==BC)");
            if (opt.degree < 1)
                throw new ArgumentException("floatChebyshev: opt.degree must be >= 1");
            if (!(opt.kappa > (float)1))
                throw new ArgumentException("floatChebyshev: opt.kappa must be > 1");
            if (opt.eigSteps < 1)
                throw new ArgumentException("floatChebyshev: opt.eigSteps must be >= 1");
            if (!(opt.safety >= (float)1))
                throw new ArgumentException("floatChebyshev: opt.safety must be >= 1");

            A = a;
            int n = a.M_Rows;
            int BR = a.BR;
            int blockLen = BR * BR;

            // ---- InvDiag: 1 / A[i,i], same block-row scan as the ref Arena ctor. ----
            var invDiag = new floatN(n, allocator);
            for (int i = 0; i < a.BlockRows; i++)
            {
                int s = a.RowPtr[i], e = a.RowPtr[i + 1];
                int found = -1;
                for (int k = s; k < e; k++)
                {
                    int col = a.ColInd[k];
                    if (col == i) { found = k; break; }
                    if (col > i) break;
                }
                if (found < 0)
                {
                    invDiag.Dispose();
                    throw new ArgumentException("floatChebyshev: missing diagonal block in A");
                }

                int off = found * blockLen;
                int rowBase = i * BR;
                for (int r = 0; r < BR; r++)
                {
                    float dv = a.Values[off + r * BR + r];
                    if (!(dv > (float)0))
                    {
                        invDiag.Dispose();
                        throw new ArgumentException("floatChebyshev: A has a non-positive diagonal entry -- is A symmetric positive definite?");
                    }
                    invDiag[rowBase + r] = (float)1 / dv;
                }
            }
            InvDiag = invDiag;

            // ---- Hi/Lo: safety * lambdaMax(D^-1 A) via a pinned Lanczos run on S.A.S. All of this
            // section's buffers are setup-only scratch (Temp-allocated, disposed below); none of
            // them are struct fields. ----
            var invSqrtD = new floatN(n, Allocator.Temp);
            for (int i = 0; i < n; i++)
                invSqrtD[i] = math.sqrt(invDiag[i]);
            var scaledScratch = new floatN(n, Allocator.Temp);
            var scaledOp = new floatJacobiScaledBSROperator(in A, in invSqrtD, in scaledScratch);

            int eigSteps = math.min(opt.eigSteps, n);
            var ws = new floatLanczosCache
            {
                V = new floatMxN(eigSteps, n, Allocator.Temp),
                vCur = new floatN(n, Allocator.Temp),
                w = new floatN(n, Allocator.Temp),
                alpha = new floatN(eigSteps, Allocator.Temp),
                beta = new floatN(eigSteps, Allocator.Temp),
                T = new floatMxN(eigSteps, eigSteps, Allocator.Temp),
                symWs = new floatEigenSymCache
                {
                    eVec = new floatN(eigSteps, Allocator.Temp),
                    vVec = new floatN(eigSteps, Allocator.Temp),
                    pVec = new floatN(eigSteps, Allocator.Temp),
                },
            };
            var ritz = new floatN(eigSteps, Allocator.Temp);
            var lInfo = Eigen.lanczos(in scaledOp, ref ws, ref ritz, eigSteps);

            float lambdaMax = ritz[0];
            for (int i = 1; i < lInfo.produced; i++)
                if (ritz[i] > lambdaMax) lambdaMax = ritz[i];

            bool lanczosOk = lInfo.status == IterativeSolveStatus.Converged && lambdaMax > (float)0;
            float hi = default, lo = default;
            if (lanczosOk)
            {
                hi = opt.safety * lambdaMax;
                lo = hi / opt.kappa;
            }

            ritz.Dispose();
            ws.symWs.pVec.Dispose();
            ws.symWs.vVec.Dispose();
            ws.symWs.eVec.Dispose();
            ws.T.Dispose();
            ws.beta.Dispose();
            ws.alpha.Dispose();
            ws.w.Dispose();
            ws.vCur.Dispose();
            ws.V.Dispose();
            scaledScratch.Dispose();
            invSqrtD.Dispose();

            // A failed eigen-estimate (non-converged Lanczos, or a non-positive largest eigenvalue)
            // makes Hi/Sigma garbage and the induced M^-1 indefinite/NaN -- signal a bad SPD build.
            if (!lanczosOk)
            {
                invDiag.Dispose();
                throw new ArgumentException("floatChebyshev: Lanczos produced no positive largest-eigenvalue estimate for D^-1 A -- is A symmetric positive definite? (raise opt.eigSteps or check A)");
            }

            Hi = hi;
            Lo = lo;

            Theta = (hi + lo) / (float)2;
            Delta = (hi - lo) / (float)2;
            Sigma = Theta / Delta;

            Degree = opt.degree;

            Scratch1 = new floatN(n, allocator);
            Scratch2 = new floatN(n, allocator);
            Scratch3 = new floatN(n, allocator);
        }

        /// <summary>floatChebyshev with floatChebyshevOptions.Default, allocated from <paramref name="allocator"/>.</summary>
        public unsafe floatChebyshev(in floatBSR a, Allocator allocator) : this(in a, floatChebyshevOptions.Default, allocator) { }

        /// <summary>
        /// Disposes InvDiag and the Apply scratch (Scratch1/2/3). A is never disposed -- Chebyshev
        /// never copies A, it always aliases the caller's own matrix. Only call on an instance built
        /// via the Allocator ctor -- an instance built via the ref Arena ctor is arena-owned and must
        /// not be disposed directly.
        /// </summary>
        public unsafe void Dispose()
        {
            InvDiag.Dispose();
            Scratch1.Dispose();
            Scratch2.Dispose();
            Scratch3.Dispose();
        }

        /// <summary>z = q(D⁻¹A)·D⁻¹r via the degree-Degree Chebyshev recurrence
        /// (<see cref="BSR.chebyApply"/>) over the struct's owned scratch. z must not alias r.</summary>
        public bool IsIdentity => false;
        public bool IsSpd => true;
        public bool IsConstant => true;

        public unsafe void Apply(in floatN r, ref floatN z)
        {
            int n = Rows;
            if (r.N != n)
                throw new ArgumentException("floatChebyshev.Apply: r.N must equal Rows");
            if (z.N != n)
                throw new ArgumentException("floatChebyshev.Apply: z.N must equal Rows");
            if (z.Data.Ptr == r.Data.Ptr)
                throw new ArgumentException("floatChebyshev.Apply: z must not alias r");

            var d = Scratch1;
            var rk = Scratch2;
            var t = Scratch3;
            BSR.chebyApply(in A, in InvDiag, Theta, Delta, Sigma, Degree, in r, ref z, ref d, ref rk, ref t);
        }
    }

    public static partial class BSR
    {
        /// <summary>
        /// Zero-initial-guess degree-<paramref name="degree"/> Chebyshev recurrence on D⁻¹A over
        /// [lo, hi] (theta=(hi+lo)/2, delta=(hi-lo)/2, sigma=theta/delta -- caller-supplied):
        /// z = q(D⁻¹A)·(invDiag ∘ r). Explicit scratch d/rk/t (length A.M_Rows, distinct from
        /// r/z/each other) so callers (e.g. an AMG smoother) can supply their own buffers. Exactly
        /// <paramref name="degree"/> spMVs; no reductions.
        /// </summary>
        public static unsafe void chebyApply(in floatBSR A, in floatN invDiag, float theta, float delta, float sigma,
            int degree, in floatN r, ref floatN z, ref floatN d, ref floatN rk, ref floatN t)
        {
            int n = A.M_Rows;
            float invTheta = (float)1 / theta;

            float* rp = r.Data.Ptr;
            float* invDp = invDiag.Data.Ptr;
            float* dp = d.Data.Ptr;
            float* zp = z.Data.Ptr;
            float* rkp = rk.Data.Ptr;

            for (int i = 0; i < n; i++)
            {
                float di = invTheta * (invDp[i] * rp[i]);
                dp[i] = di;
                zp[i] = di;
                rkp[i] = rp[i];
            }

            float rhoPrev = (float)1 / sigma;

            for (int k = 0; k < degree; k++)
            {
                spMV(in A, in d, ref t);              // t = A d
                rk.addScaledInPlace((float)(-1), t); // rk -= t

                float rho = (float)1 / ((float)2 * sigma - rhoPrev);
                float a = rho * rhoPrev;
                float b = ((float)2 * rho) / delta;

                for (int i = 0; i < n; i++)
                {
                    float dNew = a * dp[i] + b * (invDp[i] * rkp[i]);
                    dp[i] = dNew;
                    zp[i] += dNew;
                }

                rhoPrev = rho;
            }
        }
    }
}
