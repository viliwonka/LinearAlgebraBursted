...
namespace BULA.Realtime
{
    ...
    /// separate Run()/Schedule() calls. float-only.
    /// </summary>
    public struct floatRollingWindow : IDisposable
    {
        ...
        internal floatRollingWindow(in floatMxN buffer, int capacity, int features, Allocator allocator)
        {
            _buffer = buffer;
            _capacity = capacity;
            _features = features;
        ...
        public floatRollingWindow(int capacity, int features, Allocator allocator)
        {
            ...
            _buffer = new floatMxN(capacity, features, allocator);
            _capacity = capacity;
            _features = features;
            _ring = new NativeArray<int>(2, allocator);   // zero-initialised => head=0, count=0
        }
