...
namespace BULA
{
    ...
    internal static partial class doubleStatsCore {
        ...
        public static void colSum(in doubleMxN A, ref doubleN dest)
        {
            ...
            unsafe
            {
                double* ap = A.Data.Ptr; double* dp = dest.Data.Ptr;
                int nc = A.N_Cols;
                for (int c = 0; c < nc; c++)
                    dp[c] = 0f;
        ...
        // doubleTempVec, which is never explicitly disposed).
        public static void colVariance(in doubleMxN A, ref doubleN dest)
        {
            ...
            unsafe
            {
                double* ap = A.Data.Ptr; double* mp = means.Data.Ptr; double* dp = dest.Data.Ptr;
                int nc = A.N_Cols;
                for (int r = 0; r < A.M_Rows; r++)
                {
                    double* row = ap + (long)r * nc;
        ...
        public static void covarianceInto(in doubleMxN A, ref doubleMxN C)
        {
            ...
            unsafe
            {
                double* ap = A.Data.Ptr; double* mp = means.Data.Ptr; double* cp = centered.Data.Ptr;
                for (int r = 0; r < M; r++)
                {
                    double* row = ap + (long)r * N;
                    for (int c = 0; c < N; c++)
                        mp[c] += row[c];
                }
                for (int c = 0; c < N; c++)
                ...
                for (int r = 0; r < M; r++)
                {
                    double* arow = ap + (long)r * N; double* crow = cp + (long)r * N;
                    for (int c = 0; c < N; c++)
                        crow[c] = arow[c] - mp[c];
                }
            }
        ...
        // return — callers on per-frame paths leak nothing (unlike doubleTempVec, which is never
        // explicitly disposed).
        //
        ...
        public static void standardizeRows(ref doubleMxN A)
        {
            ...
            unsafe
            {
                ...
                for (int r = 0; r < A.M_Rows; r++)
                {
                    ...
                    double mu = rsum / nc;
                    double varAcc = (double)0;
                    for (int c = 0; c < nc; c++) { double d = row[c] - mu; varAcc += d * d; }
                    double sd = math.sqrt(varAcc / nc);
                    if (!(sd > (double)0)) { for (int c = 0; c < nc; c++) row[c] = (double)0; }
        ...
        public static void standardizeColumns(ref doubleMxN A)
        {
            ...
            unsafe
            {
                double* ap = A.Data.Ptr; double* s0p = s0.Data.Ptr; double* s1p = s1.Data.Ptr;
                int nc = A.N_Cols;
                for (int c = 0; c < nc; c++)
                    s1p[c] = (double)0;
                for (int r = 0; r < A.M_Rows; r++)
                ...
                for (int r = 0; r < A.M_Rows; r++)
                {
                    ...
                    for (int c = 0; c < nc; c++)
                    {
                        double mu = s0p[c], sd = s1p[c];
                        if (!(sd > (double)0)) row[c] = (double)0;
        ...
        public static void rescaleRows(ref doubleMxN A, double lo, double hi)
        {
            ...
            unsafe
            {
                ...
                for (int r = 0; r < A.M_Rows; r++)
                {
                    ...
                    if (!(rng > (double)0)) { for (int c = 0; c < nc; c++) row[c] = lo; }
                    else
                    {
                        double sc = hi - lo;
        ...
        public static void rescaleColumns(ref doubleMxN A, double lo, double hi)
        {
            ...
            unsafe
            {
                ...
                for (int r = 0; r < A.M_Rows; r++)
                {
                    ...
                    for (int c = 0; c < nc; c++)
                    {
                        double mn = s0p[c], rng = s1p[c] - mn;
                        if (!(rng > (double)0)) row[c] = lo;
        ...
        public static void centerColumns(ref doubleMxN A)
        {
            ...
            unsafe
            {
                double* ap = A.Data.Ptr; double* s0p = s0.Data.Ptr;
                int nc = A.N_Cols;
                for (int r = 0; r < A.M_Rows; r++)
                {
                    double* row = ap + (long)r * nc;
        ...
        public static void maxAbsColumns(ref doubleMxN A)
        {
            ...
            unsafe
            {
                double* ap = A.Data.Ptr; double* mp = mAbsArr.Data.Ptr;
                int nc = A.N_Cols;
                for (int r = 0; r < A.M_Rows; r++)
                {
                    double* row = ap + (long)r * nc;
        ...
        public static void softmaxColumns(ref doubleMxN A)
        {
            ...
            doubleN colMax = A.doubleTempVec(nc);
            doubleN colSum = A.doubleTempVec(nc);
            unsafe
            {
                double* ap = A.Data.Ptr;
                double* mp = colMax.Data.Ptr;
                double* sp = colSum.Data.Ptr;

                // Pass 1: per-column max (init from row 0; strict > so NaN never displaces).
                for (int c = 0; c < nc; c++) { mp[c] = ap[c]; sp[c] = (double)0; }
                for (int r = 1; r < nr; r++)
                {
                    double* row = ap + (long)r * nc;
                ...
                {
                    double* row = ap + (long)r * nc;
                    for (int c = 0; c < nc; c++) { double e = DetMath.Exp(row[c] - mp[c]); row[c] = e; sp[c] += e; }
                }
