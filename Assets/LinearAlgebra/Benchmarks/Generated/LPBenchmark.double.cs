    ...
    public static partial class LPBenchmark
    {
        ...
        static void SectionSolveDouble(StringBuilder sb)
        {
            ...
            foreach (var n in LPBenchmarkFmt.SolveVarsN)
            {
                ...
                var jobS = new LpSolveJobDouble { A = A, b = b, c = c, senses = senses, x = xS, method = LPMethod.Simplex, maxIter = 0 };
                var statS = Bench.Time(() => jobS.Run());
                sb.AppendLine(LPBenchmarkFmt.SolveRow("double", n, m, "simplex", statS, infoS.iterations, objS));

                var xI = arena.doubleVec(n);
                var infoI = LP.solve(in A, in b, in c, in senses, ref xI, out double objI, LPMethod.InteriorPoint);
                var jobI = new LpSolveJobDouble { A = A, b = b, c = c, senses = senses, x = xI, method = LPMethod.InteriorPoint, maxIter = 0 };
                var statI = Bench.Time(() => jobI.Run());
                sb.AppendLine(LPBenchmarkFmt.SolveRow("double", n, m, "interior-point", statI, infoI.iterations, objI));

                var xR = arena.doubleVec(n);
                var infoR = LP.solve(in A, in b, in c, in senses, ref xR, out double objR, LPMethod.RevisedSimplex);
                var jobR = new LpSolveJobDouble { A = A, b = b, c = c, senses = senses, x = xR, method = LPMethod.RevisedSimplex, maxIter = 0 };
                var statR = Bench.Time(() => jobR.Run());
                sb.AppendLine(LPBenchmarkFmt.SolveRow("double", n, m, "revised-primal", statR, infoR.iterations, objR));

                var xD = arena.doubleVec(n);
                var infoD = LP.solve(in A, in b, in c, in senses, ref xD, out double objD, LPMethod.DualSimplex);
                var jobD = new LpSolveJobDouble { A = A, b = b, c = c, senses = senses, x = xD, method = LPMethod.DualSimplex, maxIter = 0 };
                var statD = Bench.Time(() => jobD.Run());
                sb.AppendLine(LPBenchmarkFmt.SolveRow("double", n, m, "dual-simplex", statD, infoD.iterations, objD));
        ...
        {
            sb.AppendLine();
            sb.AppendLine("--- 2. LAD (L1) regression, gross outliers: exact LP.lad (simplex/interior) vs fast ladIRLS [double] ---");
            ...
            foreach (var m in LPBenchmarkFmt.LadRowsM)
            {
                ...
                var jobLs = new LadJobDouble { A = A, b = b, x = xLs, method = LPMethod.Simplex };
                var statLs = Bench.Time(() => jobLs.Run());
                sb.AppendLine(LPBenchmarkFmt.LadRow("double", m, n, "LP.lad-simplex", statLs, infoLs.iterations, objLs));

                var xLi = arena.doubleVec(n);
                var infoLi = LP.lad(in A, in b, ref xLi, out double objLi, LPMethod.InteriorPoint);
                var jobLi = new LadJobDouble { A = A, b = b, x = xLi, method = LPMethod.InteriorPoint };
                var statLi = Bench.Time(() => jobLi.Run());
                sb.AppendLine(LPBenchmarkFmt.LadRow("double", m, n, "LP.lad-interior", statLi, infoLi.iterations, objLi));

                var xIr = arena.doubleVec(n);
                var infoIr = Optimize.ladIRLS(in A, in b, ref xIr);
                var jobIr = new IrlsJobDouble { A = A, b = b, x = xIr };
                var statIr = Bench.Time(() => jobIr.Run());
                sb.AppendLine(LPBenchmarkFmt.LadRow("double", m, n, "ladIRLS", statIr, infoIr.iterations, infoIr.objective));
        ...
        static void SectionSparseLadDouble(StringBuilder sb)
        {
            ...
            foreach (var m in LPBenchmarkFmt.SparseLadRowsM)
            {
                ...
                if (m <= LPBenchmarkFmt.SparseLadDenseCap)
                {
                    ...
                    var jobD = new LadJobDouble { A = Ad, b = b, x = xd, method = LPMethod.InteriorPoint };
                    var statD = Bench.Time(() => jobD.Run());
                    sb.AppendLine(LPBenchmarkFmt.LadRow("double", m, n, "dense LP.lad-ip", statD, infoD.iterations, objD));
                }

                var xs = arena.doubleVec(n);
                var infoS = LP.lad(in As, in b, ref xs, out double objS, 0);
                var jobS = new SparseLadJobDouble { A = As, b = b, x = xs };
                var statS = Bench.Time(() => jobS.Run());
                sb.AppendLine(LPBenchmarkFmt.LadRow("double", m, n, "sparse LP.lad", statS, infoS.iterations, objS));
        ...
        static void SectionPdlpDenseDouble(StringBuilder sb)
        {
            ...
            foreach (var n in LPBenchmarkFmt.SolveVarsN)
            {
                ...
                var jobP = new PdlpJobDouble { A = A, lc = lc, uc = uc, lv = lv, uv = uv, c = c, x = xP, maxIter = LPBenchmarkFmt.PdlpMaxIter, eps = LPBenchmarkFmt.PdlpEps };
                var statP = Bench.Time(() => jobP.Run());
                sb.AppendLine(LPBenchmarkFmt.SolveRow("double", n, m, "pdlp", statP, infoP.iterations, objP));
        ...
        static void SectionPdlpSparseDouble(StringBuilder sb)
        {
            ...
            foreach (var m in LPBenchmarkFmt.PdlpSparseM)
            {
                ...
                var jobI = new LpSolveSparseJobDouble { A = As, b = b, c = c, senses = senses, x = xI };
                var statI = Bench.Time(() => jobI.Run());
                sb.AppendLine(LPBenchmarkFmt.SolveRow("double", n, m, "sparse ip", statI, infoI.iterations, objI));
                ...
                var jobP = new PdlpSparseJobDouble { A = As, lc = lc, uc = uc, lv = lv, uv = uv, c = c, x = xP, maxIter = LPBenchmarkFmt.PdlpMaxIter, eps = LPBenchmarkFmt.PdlpEps };
                var statP = Bench.Time(() => jobP.Run());
                sb.AppendLine(LPBenchmarkFmt.SolveRow("double", n, m, "sparse pdlp", statP, infoP.iterations, objP));
