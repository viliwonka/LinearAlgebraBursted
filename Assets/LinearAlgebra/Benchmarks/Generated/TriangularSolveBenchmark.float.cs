...
namespace LinearAlgebra.Benchmarks
{
    ...
    public struct TriSolveFactorJobFloat : IJob
    {
        public floatMxN A;     // receives Src, factored in place into the compact LU form
        public floatMxN Src;
        public Pivot P;
    ...
    [BurstCompile(CompileSynchronously = true, FloatPrecision = FloatPrecision.High, FloatMode = FloatMode.Default)]
    public struct TriSolveFwdVecJobFloat : IJob
    {
        public floatMxN A;     // pre-factored compact LU (read-only here)
    ...
    [BurstCompile(CompileSynchronously = true, FloatPrecision = FloatPrecision.High, FloatMode = FloatMode.Default)]
    public struct TriSolveTransAVecJobFloat : IJob
    {
        public floatMxN A;
        public Pivot P;
        public floatN b;
    ...
    [BurstCompile(CompileSynchronously = true, FloatPrecision = FloatPrecision.High, FloatMode = FloatMode.Default)]
    public struct TriSolveFwdMatJobFloat : IJob
    {
        public floatMxN A;
        public Pivot P;
        public floatMxN BX;
        public floatMxN BXsrc;
    ...
    [BurstCompile(CompileSynchronously = true, FloatPrecision = FloatPrecision.High, FloatMode = FloatMode.Default)]
    public struct TriSolveTransAMatJobFloat : IJob
    {
        public floatMxN A;
        public Pivot P;
        public floatMxN BX;
    ...
    [BurstCompile(CompileSynchronously = true, FloatPrecision = FloatPrecision.High, FloatMode = FloatMode.Default)]
    public struct TriSolveKernelFwdVecJobFloat : IJob
    {
        public floatMxN A;
        public Pivot P;
        public floatN b;
    ...
    [BurstCompile(CompileSynchronously = true, FloatPrecision = FloatPrecision.High, FloatMode = FloatMode.Default)]
    public struct TriSolveKernelTransAVecJobFloat : IJob
    {
        public floatMxN A;
        public Pivot P;
        public floatN b;
    ...
    {
        static string TriSolveFloat(int n)
        {
            ...
            // in LU.float.cs for the same convention).
            const int K = 8;
            ...
            for (int i = 0; i < n; i++)
                bSrc[i] = rng.NextFloat(-1f, 1f);
            for (int i = 0; i < n; i++)
                for (int c = 0; c < K; c++)
                    BXsrc[i, c] = rng.NextFloat(-1f, 1f);

            // Factor ONCE, outside every timed region below -- shared read-only by every job that follows.
            new TriSolveFactorJobFloat { A = A, Src = Src, P = P }.Run();

            var fwdVec = Bench.Time(() => new TriSolveFwdVecJobFloat { A = A, P = P, b = b, bSrc = bSrc }.Run());
            var transAVec = Bench.Time(() => new TriSolveTransAVecJobFloat { A = A, P = P, b = b, bSrc = bSrc }.Run());
            var fwdMat = Bench.Time(() => new TriSolveFwdMatJobFloat { A = A, P = P, BX = BX, BXsrc = BXsrc }.Run());
            var transAMat = Bench.Time(() => new TriSolveTransAMatJobFloat { A = A, P = P, BX = BX, BXsrc = BXsrc }.Run());
            var kernelFwdVec = Bench.Time(() => new TriSolveKernelFwdVecJobFloat { A = A, P = P, b = b, bSrc = bSrc }.Run());
            var kernelTransAVec = Bench.Time(() => new TriSolveKernelTransAVecJobFloat { A = A, P = P, b = b, bSrc = bSrc }.Run());

            P.Dispose();
            arena.Dispose();

            return TriSolveFmt.RowKernel("float", "LU solve fwd (vec)", n, fwdVec)
                 + "\n" + TriSolveFmt.RowKernel("float", "LU solve TransA (vec)", n, transAVec)
                 + "\n" + TriSolveFmt.RowKernel("float", "LU solve fwd (mat k=8)", n, fwdMat)
                 + "\n" + TriSolveFmt.RowKernel("float", "LU solve TransA (mat k=8)", n, transAMat)
                 + "\n" + TriSolveFmt.RowKernel("float", "Blas triUpperLU (vec)", n, kernelFwdVec)
                 + "\n" + TriSolveFmt.RowKernel("float", "Blas triUpperLUTransA (vec)", n, kernelTransAVec);
        }
    }
}
