    ...
    public static partial class LPBenchmark
    {
        ...
        static void SectionPdlpDenseFloat(StringBuilder sb)
        {
            ...
            foreach (var n in LPBenchmarkFmt.PdlpDenseVarsN)
            {
                ...
                var jobP = new PdlpJobFloat { A = A, lc = lc, uc = uc, lv = lv, uv = uv, c = c, x = xP, maxIter = LPBenchmarkFmt.PdlpMaxIter, eps = LPBenchmarkFmt.PdlpEps, objOut = objOut, itersOut = itersOut };
                var statP = Bench.Time(() => jobP.Run());
                sb.AppendLine(LPBenchmarkFmt.SolveRow("float", n, m, "pdlp", statP, itersOut[0], objOut[0]));
        ...
        static void SectionPdlpSparseFloat(StringBuilder sb)
        {
            ...
            foreach (var m in LPBenchmarkFmt.PdlpSparseM)
            {
                ...
                var xI = arena.floatVec(n);
                var jobI = new LpSolveSparseJobFloat { A = As, b = b, c = c, senses = senses, x = xI, objOut = objOut, itersOut = itersOut, statusOut = statusOut };
                var statI = Bench.Time(() => jobI.Run());
                sb.AppendLine(LPBenchmarkFmt.SolveRow("float", n, m, "sparse ip", statI, itersOut[0], objOut[0]));
                ...
                for (int j = 0; j < n; j++) { lv[j] = (float)0; uv[j] = INF; }
                var xP = arena.floatVec(n);
                var jobP = new PdlpSparseJobFloat { A = As, lc = lc, uc = uc, lv = lv, uv = uv, c = c, x = xP, maxIter = LPBenchmarkFmt.PdlpMaxIter, eps = LPBenchmarkFmt.PdlpEps, objOut = objOut, itersOut = itersOut };
                var statP = Bench.Time(() => jobP.Run());
                sb.AppendLine(LPBenchmarkFmt.SolveRow("float", n, m, "sparse pdlp", statP, itersOut[0], objOut[0]));
