...
namespace BULA.Benchmarks
{
    ...
    public struct FitQRJobFloat : IJob
    {
        public floatMxN A;     // m x n; DESTROYED by solveInPlace (restored from Src each sample)
        public floatMxN Src;
        public floatN b;       // length m; DESTROYED (restored from bSrc each sample)
        public floatN bSrc;
        public floatN x;       // length n, coefficients
    ...
    [BurstCompile(CompileSynchronously = true, FloatPrecision = FloatPrecision.High, FloatMode = FloatMode.Default)]
    public struct FitLadJobFloat : IJob
    {
        public floatMxN A;     // m x n; NOT modified
        public floatN b;       // length m; NOT modified
        public floatN x;       // length n, coefficients
    ...
    [BurstCompile(CompileSynchronously = true, FloatPrecision = FloatPrecision.High, FloatMode = FloatMode.Default)]
    public struct FitIRLSJobFloat : IJob
    {
        public floatMxN A;     // m x n; NOT modified
        public floatN b;       // length m; NOT modified
        public floatN x;       // length n, coefficients
    ...
    public static partial class FittingBenchmark
    {
        static string FitFloat(int m, int n)
        {
            ...
            for (int r = 0; r < m; r++)
            {
                ...
                for (int c = 0; c < n; c++)
                {
                    float a = rng.NextFloat(-1f, 1f);
                    Src[r, c] = a;
                    dot += a * xTrue[c];
                }
                float noise = rng.NextFloat(-0.05f, 0.05f);
                float outlier = rng.NextFloat(0f, 1f) < (float)0.05 ? rng.NextFloat(-20f, 20f) : (float)0;
                bSrc[r] = dot + noise + outlier;
            }

            var qr = new FitQRJobFloat { A = A, Src = Src, b = b, bSrc = bSrc, x = x };
            ...
            var lad = new FitLadJobFloat { A = A, b = b, x = x };
            ...
            sb.Append(FittingFmt.Fmt("float", "Optimize.ladIRLS L1", m, n, sIRLS));
            return sb.ToString();
        }
    }
}
