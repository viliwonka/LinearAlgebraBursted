...
namespace LinearAlgebra.Benchmarks
{
    ...
    public struct DetIntFamilyCoreJobLong : IJob
    {
        public longN a, b;
        public Pivot pivot;
        public longN randDest;

        public NativeArray<uint> HashOut; // 3 slots

        public unsafe void Execute()
        {
            long dotResult = Blas.dot(a, b);
            HashOut[0] = Hash.hash((byte*)&dotResult, sizeof(long));
    ...
    [BurstCompile(CompileSynchronously = true, FloatPrecision = FloatPrecision.High, FloatMode = FloatMode.Strict)]
    public struct DetIntFamilyNormsStatsJobLong : IJob
    {
        public longN vec;
    ...
    {
        public static (string id, uint hash)[] Case_IntFamilyCoreLong()
        {
            var rng = new Random(2654435761u ^ 0x0019u);

            const int n = 48;
            // Non-negative range: this case runs for uint too (unlike the norms/stats job below,
            // which is skipped for uint), and a negative long literal wraps to a huge value under
            // unsigned arithmetic, which would violate Rand.nextUniformInPlace's min <= max contract.
            var a = new longN(n, Allocator.Persistent);
            var b = new longN(n, Allocator.Persistent);
            ...
            // Rand.nextUniformInPlace has no uint instantiation (RandomOP.long.cs does not opt
            // into the uint alsoExpand widening) -- fall back to Unity.Mathematics.Random.NextUInt
            // directly for that one slot.
            
            Rand.nextUniformInPlace(ref rng, ref a, (long)0, (long)10);
            Rand.nextUniformInPlace(ref rng, ref b, (long)0, (long)10);
            
            

            var pivot = new Pivot(8, Allocator.Persistent);

            var randDest = new longN(n, Allocator.Persistent);
            
            Rand.nextUniformInPlace(ref rng, ref randDest, (long)0, (long)1000);
            
            

            var hashOut = new NativeArray<uint>(3, Allocator.Persistent);
            var job = new DetIntFamilyCoreJobLong { a = a, b = b, pivot = pivot, randDest = randDest, HashOut = hashOut };
            job.Run();

            var result = new[]
            {
                ("int-family/dot.long.n48", hashOut[0]),
                ("int-family/pivot-roundtrip.long", hashOut[1]),
                ("int-family/randomFill.long.n48", hashOut[2]),
            };
            hashOut.Dispose();
            pivot.Dispose();
        ...
        public static (string id, uint hash)[] Case_IntFamilyNormsStatsLong()
        {
            var rng = new Random(2654435761u ^ 0x001Au);

            const int n = 48;
            var vec = new longN(n, Allocator.Persistent);
            Rand.nextUniformInPlace(ref rng, ref vec, (long)(-50), (long)50);

            var hashOut = new NativeArray<uint>(1, Allocator.Persistent);
            var job = new DetIntFamilyNormsStatsJobLong { vec = vec, HashOut = hashOut };
            job.Run();

            var result = new[] { ("int-family/norms-stats.long.n48", hashOut[0]) };
            hashOut.Dispose();
            vec.Dispose();
            return result;
        }
