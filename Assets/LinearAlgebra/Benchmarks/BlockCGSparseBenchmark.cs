...
{
    // Block-CG vs the scalar loop of s independent cg solves, over a BSR 2D-Poisson operator (5-point
    // stencil, ~5 nonzeros/row, n = grid²), solved to sqrt(eps) — SPARSE is block-CG's real use case.
    // A "spMM x50 / spMVx s x50" pair per row is a matvec-only probe: 50 block spMM(s) calls vs 50*s
    ...
    // `iters` is block iterations for block-CG, summed column iterations for the scalar loop, and the
    // rep count for the probes. `minActive` is the smallest active block width block-CG reached.
    //
