...
// Concretely-long tests for the long-only out-of-int-range guard in longRandom_OP.nextUniformInpl.
//
// WHY THIS FILE IS NOT A TEMPLATE: longRandom_OP.nextUniformInpl draws via Random.NextInt (int
...
// the firstpass proxy test assembly cannot reference the generated longRandom_OP, so this test is
...
public class RandomLongRangeTests
{
    ...
    public void MinBelowIntRangeThrows()
    {
        ...
        Assert.Throws<ArgumentException>(() => longRandom_OP.nextUniformInpl(ref rng, ref v, badMin, 0L));

        var M = arena.longMat(3, 3);
        Assert.Throws<ArgumentException>(() => longRandom_OP.nextUniformInpl(ref rng, ref M, badMin, 0L));
    ...
    public void MaxAboveIntRangeThrows()
    {
        ...
        var v = arena.longVec(8);
        Assert.Throws<ArgumentException>(() => longRandom_OP.nextUniformInpl(ref rng, ref v, 0L, badMax));

        var M = arena.longMat(3, 3);
        Assert.Throws<ArgumentException>(() => longRandom_OP.nextUniformInpl(ref rng, ref M, 0L, badMax));
    ...
    public void InIntRangeFillsAndDoesNotThrow()
    {
        ...
        longRandom_OP.nextUniformInpl(ref rng, ref c, 7L, 7L);
        for (int i = 0; i < c.N; i++)
    ...
    public void MinGreaterThanMaxStillThrows()
    {
        ...
        var v = arena.longVec(8);
        Assert.Throws<ArgumentException>(() => longRandom_OP.nextUniformInpl(ref rng, ref v, 5L, 1L));
