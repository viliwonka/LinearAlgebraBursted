...
// Stage-2 test suite for the inequality-constrained active-set QP core (QP.qpActiveSetCore,
// docs/draft-spec-qp.md Stage 2). INTERNAL entry point (the public surface is the future QP.solve
// facade, stage 3), so this is HAND-WRITTEN in SourceTests/ -- InternalsVisibleTo
// ("BurstLinearAlgebra.Tests") reaches internals here but not the template "-firstpass" compile-check
// assembly -- same route as QPEqpTests.cs (stage 1) / LadFrischNewtonQuantileTests.cs.
//
// Burst execution (shipped-feature spec pillar 3): compute runs inside
// [BurstCompile(CompileSynchronously = true)] IJob structs; NUnit Assert.IsTrue with == only inside the
// job, first-failure recorded into a Fail[] diagnostic array read back on the managed side -- the exact
// pattern QPEqpTests.cs / LPTests.fProxy.cs use.
//
// ---- Acceptance oracles (per draft-spec-qp.md Stage 2 + the implementation agent's handoff) ----
//
...
//     library's phase-1-free Stage-2 contract for every problem -- HS21/HS52's do not) -- see each job.
//
...
//     stage 1's QP.eqpSolve on the selected unit rows -- exactly the kernel qpActiveSetCore itself is
//     built from), checks FULL-box feasibility (not just the active bounds), and keeps the minimum
//     feasible objective. The true optimum's active set is necessarily one of the 3^n combinations
//     enumerated, so this minimum IS the global box-constrained optimum -- an oracle independent of
//     qpActiveSetCore's own machinery.
//
// (c) LP limit: Q = 0 reproduces LP.solve's objective on a Section-1-style random LP (see
//     LPBenchmark.fProxy.cs "Section 1: LP.solve, random dense feasible LP" -- A m x n nonnegative in
//     [0,1] with m = n/2, x0 random in [0,1], b = A x0 + slack (0.1..1, so x0 is STRICTLY feasible),
//     c random in [-1,1], all rows LessEqual, x >= 0). x0 doubles as the Stage-2 feasible start.
//
...
//     SeedWorkingSet's rank-guarded seeding on a redundant row (draft-spec-qp.md requirement 6d).
//
...
//     kernel's own logic (see docs/draft-spec-qp.md Stage 2 handoff): x0 = (0,0) seeds with x2's lower
//     bound tight; the first EQP step there is already optimal in x1 (its multiplier sign is wrong, so
//     it is dropped); the SECOND step (k = 0, full space) hits Q's singularity directly, regularizes,
//     finds zero curvature, an unblocked ray, and gp &lt; 0 -- the real Stage-2 Unbounded condition.
public class QPActiveSetTests
{
    // ================================================================================================
    ...
    public struct HS21FloatJob : IJob
    {
        ...
        public void Execute()
        {
            ...
            var x = arena.floatVec(2); x[0] = 5f; x[1] = 10f;
    ...
    [BurstCompile(CompileSynchronously = true, FloatPrecision = FloatPrecision.High, FloatMode = FloatMode.Default)]
    public struct HS21DoubleJob : IJob
    {
        public NativeArray<double> Fail;
        public void Execute()
        {
            ...
            var x = arena.doubleVec(2); x[0] = 5.0; x[1] = 10.0;
    ...
    [BurstCompile(CompileSynchronously = true, FloatPrecision = FloatPrecision.High, FloatMode = FloatMode.Default)]
    public struct HS35FloatJob : IJob
    {
        public NativeArray<double> Fail;
        public void Execute()
        {
            ...
            var x = arena.floatVec(3); x[0] = 0.5f; x[1] = 0.5f; x[2] = 0.5f;
    ...
    [BurstCompile(CompileSynchronously = true, FloatPrecision = FloatPrecision.High, FloatMode = FloatMode.Default)]
    public struct HS35DoubleJob : IJob
    {
        public NativeArray<double> Fail;
        public void Execute()
        {
            ...
            var x = arena.doubleVec(3); x[0] = 0.5; x[1] = 0.5; x[2] = 0.5;
    ...
    [BurstCompile(CompileSynchronously = true, FloatPrecision = FloatPrecision.High, FloatMode = FloatMode.Default)]
    public struct HS52FloatJob : IJob
    {
        public NativeArray<double> Fail;
        public void Execute()
        {
            ...
            var x = arena.floatVec(5);   // origin: all three equalities are homogeneous (RHS 0)
    ...
    [BurstCompile(CompileSynchronously = true, FloatPrecision = FloatPrecision.High, FloatMode = FloatMode.Default)]
    public struct HS52DoubleJob : IJob
    {
        public NativeArray<double> Fail;
        public void Execute()
        {
    ...
    [BurstCompile(CompileSynchronously = true, FloatPrecision = FloatPrecision.High, FloatMode = FloatMode.Default)]
    public struct HS76FloatJob : IJob
    {
        public NativeArray<double> Fail;
        public void Execute()
        {
            ...
            var x = arena.floatVec(4); for (int i = 0; i < 4; i++) x[i] = 0.5f;
    ...
    [BurstCompile(CompileSynchronously = true, FloatPrecision = FloatPrecision.High, FloatMode = FloatMode.Default)]
    public struct HS76DoubleJob : IJob
    {
        public NativeArray<double> Fail;
        public void Execute()
        {
            ...
            var x = arena.doubleVec(4); for (int i = 0; i < 4; i++) x[i] = 0.5;
    ...
    [Test] public void HS21_Float() => H.Run(fail => new HS21FloatJob { Fail = fail }.Run());
    [Test] public void HS21_Double() => H.Run(fail => new HS21DoubleJob { Fail = fail }.Run());
    [Test] public void HS35_Float() => H.Run(fail => new HS35FloatJob { Fail = fail }.Run());
    [Test] public void HS35_Double() => H.Run(fail => new HS35DoubleJob { Fail = fail }.Run());
    [Test] public void HS52_Float() => H.Run(fail => new HS52FloatJob { Fail = fail }.Run());
    [Test] public void HS52_Double() => H.Run(fail => new HS52DoubleJob { Fail = fail }.Run());
    [Test] public void HS76_Float() => H.Run(fail => new HS76FloatJob { Fail = fail }.Run());
    [Test] public void HS76_Double() => H.Run(fail => new HS76DoubleJob { Fail = fail }.Run());
    ...
    // ================================================================================================
    [BurstCompile(CompileSynchronously = true, FloatPrecision = FloatPrecision.High, FloatMode = FloatMode.Default)]
    public struct BruteForceFloatJob : IJob
    {
        public int N, Seed;
        public NativeArray<double> Fail;
        public void Execute()
        {
            ...
            var xEq = arena.floatVec(n, true);
            var Qcopy = arena.floatMat(n, n, true);
            var rhs = arena.floatVec(n, true);
            ...
            for (int combo = 0; combo < combos; combo++)
            {
                ...
                {
                    var AWv = new floatMxN(k, n, Allocator.Temp);   // k x n, zero-init
                    var bWv = new floatN(k, Allocator.Temp);
                    var lamWv = new floatN(k, Allocator.Temp);      // eqpSolve requires lambda.N == A_W.M_Rows == k
                    int kk = 0;
                    for (int i = 0; i < n; i++)
                    {
                ...
                {
                    for (int i = 0; i < n; i++)
                        if ((double)xEq[i] < (double)xl[i] - 1e-4 || (double)xEq[i] > (double)xu[i] + 1e-4)
            ...
            H.AssertLE(Fail, 3, math.abs(obj - bestObj), 2e-3 * (1.0 + math.abs(bestObj)));
    ...
    [BurstCompile(CompileSynchronously = true, FloatPrecision = FloatPrecision.High, FloatMode = FloatMode.Default)]
    public struct BruteForceDoubleJob : IJob
    {
        public int N, Seed;
        public NativeArray<double> Fail;
        public void Execute()
        {
            ...
            for (int i = 0; i < n; i++) { xl[i] = rng.NextDouble(-3.0, -1.0); xu[i] = rng.NextDouble(1.0, 3.0); }
            var x0 = arena.doubleVec(n);
            for (int i = 0; i < n; i++) x0[i] = 0.5 * (xl[i] + xu[i]);
            ...
            double bestObj = double.PositiveInfinity;
            var digit = new NativeArray<int>(n, Allocator.Temp);
            var xEq = arena.doubleVec(n, true);
            var Qcopy = arena.doubleMat(n, n, true);
            var rhs = arena.doubleVec(n, true);
            ...
            for (int combo = 0; combo < combos; combo++)
            {
                ...
                {
                    var AWv = new doubleMxN(k, n, Allocator.Temp);
                    var bWv = new doubleN(k, Allocator.Temp);
                    var lamWv = new doubleN(k, Allocator.Temp);   // eqpSolve requires lambda.N == A_W.M_Rows == k
                    int kk = 0;
                    for (int i = 0; i < n; i++)
                    {
                        if (digit[i] == 0) continue;
                ...
                {
                    for (int i = 0; i < n; i++)
                        if (xEq[i] < xl[i] - 1e-9 || xEq[i] > xu[i] + 1e-9)
                        { feasibleCandidate = false; break; }
                }
                ...
                if (feasibleCandidate)
                {
                    ...
                    {
                        double qxi = 0;
                        for (int j = 0; j < n; j++) qxi += Q[i, j] * xEq[j];
                        o += 0.5 * xEq[i] * qxi + c[i] * xEq[i];
                    }
                    if (o < bestObj) bestObj = o;
                }
            }
            ...
            H.AssertTrue(Fail, 2, !double.IsPositiveInfinity(bestObj));
            H.AssertLE(Fail, 3, math.abs(obj - bestObj), 1e-8 * (1.0 + math.abs(bestObj)));
    ...
    public void BruteForce_Float(int n, int seed) => H.Run(fail => new BruteForceFloatJob { N = n, Seed = seed, Fail = fail }.Run());

    [TestCaseSource(nameof(BruteForceCases))]
    public void BruteForce_Double(int n, int seed) => H.Run(fail => new BruteForceDoubleJob { N = n, Seed = seed, Fail = fail }.Run());

    // ================================================================================================
    // (c) LP limit: Q = 0 reproduces LP.solve's objective on a Section-1-style random LP.
    // ================================================================================================
    [BurstCompile(CompileSynchronously = true, FloatPrecision = FloatPrecision.High, FloatMode = FloatMode.Default)]
    public struct LpLimitFloatJob : IJob
    {
        public int N, Seed;
        public NativeArray<double> Fail;
        public void Execute()
        {
            ...
            var x0 = arena.floatVec(n);
            for (int i = 0; i < n; i++) x0[i] = rng.NextFloat(0f, 1f);
            var Ax0 = arena.floatVec(m);
            Blas.dot(in A, in x0, ref Ax0);
            var b = arena.floatVec(m);
            for (int i = 0; i < m; i++) b[i] = Ax0[i] + rng.NextFloat(0.1f, 1f);
            var c = arena.floatVec(n);
            for (int i = 0; i < n; i++) c[i] = rng.NextFloat(-1f, 1f);
            var senses = new NativeArray<ConstraintSense>(m, Allocator.Temp);
            for (int i = 0; i < m; i++) senses[i] = ConstraintSense.LessEqual;

            var Q = arena.floatMat(n, n);   // zero
            var xl = arena.floatVec(n);
            var xu = arena.floatVec(n); for (int i = 0; i < n; i++) xu[i] = 1e30f;

            var xQp = arena.floatVec(n);
            ...
            H.AssertLE(Fail, 3, math.abs(qpObj - lpObj), 5e-3 * (1.0 + math.abs(lpObj)));
    ...
    [BurstCompile(CompileSynchronously = true, FloatPrecision = FloatPrecision.High, FloatMode = FloatMode.Default)]
    public struct LpLimitDoubleJob : IJob
    {
        public int N, Seed;
        public NativeArray<double> Fail;
        public void Execute()
        {
            ...
            var x0 = arena.doubleVec(n);
            for (int i = 0; i < n; i++) x0[i] = rng.NextDouble(0.0, 1.0);
            ...
            var Q = arena.doubleMat(n, n);
            var xl = arena.doubleVec(n);
            var xu = arena.doubleVec(n); for (int i = 0; i < n; i++) xu[i] = 1e30;

            var xQp = arena.doubleVec(n);
            for (int i = 0; i < n; i++) xQp[i] = x0[i];
            var qpInfo = QP.qpActiveSetCore(in Q, in c, in A, in b, senses, in xl, in xu, ref xQp, out double qpObj, 0);

            var xLp = arena.doubleVec(n);
            var lpInfo = LP.solve(in A, in b, in c, senses, ref xLp, out double lpObj);

            H.AssertTrue(Fail, 1, qpInfo.status == QPStatus.Optimal);
            H.AssertTrue(Fail, 2, lpInfo.Solved);
            H.AssertLE(Fail, 3, math.abs(qpObj - lpObj), 1e-6 * (1.0 + math.abs(lpObj)));
    ...
    public void LpLimit_Float(int n, int seed) => H.Run(fail => new LpLimitFloatJob { N = n, Seed = seed, Fail = fail }.Run());

    [TestCaseSource(nameof(LpLimitCases))]
    public void LpLimit_Double(int n, int seed) => H.Run(fail => new LpLimitDoubleJob { N = n, Seed = seed, Fail = fail }.Run());

    // ================================================================================================
    // (d) Degenerate: a verbatim-duplicated tight constraint row must not change the answer.
    // ================================================================================================
    [BurstCompile(CompileSynchronously = true, FloatPrecision = FloatPrecision.High, FloatMode = FloatMode.Default)]
    public struct DegenerateFloatJob : IJob
    {
        public int Seed;
        public NativeArray<double> Fail;
        public void Execute()
        {
            ...
            for (int i = 0; i < n; i++) c[i] = rng.NextFloat(-1f, 1f);
            var xl = arena.floatVec(n);
            var xu = arena.floatVec(n); for (int i = 0; i < n; i++) xu[i] = 5f;
            var x0 = arena.floatVec(n); x0[0] = 2f; x0[1] = 2f; x0[2] = 2f;   // sum = 6, tight below

            // single copy
            var A1 = arena.floatMat(1, n); A1[0, 0] = 1f; A1[0, 1] = 1f; A1[0, 2] = 1f;
            var b1 = arena.floatVec(1); b1[0] = 6f;
            var senses1 = new NativeArray<ConstraintSense>(1, Allocator.Temp); senses1[0] = ConstraintSense.LessEqual;
            var x1 = arena.floatVec(n); x1[0] = x0[0]; x1[1] = x0[1]; x1[2] = x0[2];
            ...
            var A2 = arena.floatMat(2, n);
            A2[0, 0] = 1f; A2[0, 1] = 1f; A2[0, 2] = 1f;
            A2[1, 0] = 1f; A2[1, 1] = 1f; A2[1, 2] = 1f;
            var b2 = arena.floatVec(2); b2[0] = 6f; b2[1] = 6f;
            ...
            var x2 = arena.floatVec(n); x2[0] = x0[0]; x2[1] = x0[1]; x2[2] = x0[2];
            ...
            H.AssertLE(Fail, 3, math.abs(obj1 - obj2), 5e-4 * (1.0 + math.abs(obj1)));
            for (int i = 0; i < n; i++)
                H.AssertLE(Fail, 4 + i, math.abs((double)x1[i] - (double)x2[i]), 5e-3);
    ...
    [BurstCompile(CompileSynchronously = true, FloatPrecision = FloatPrecision.High, FloatMode = FloatMode.Default)]
    public struct DegenerateDoubleJob : IJob
    {
        public int Seed;
        public NativeArray<double> Fail;
        public void Execute()
        {
            ...
            int n = 3;
            var Q = arena.doubleMat(n, n);
            Rand.spdInPlace(ref rng, ref Q, 1.0, 5.0);
            var c = arena.doubleVec(n);
            for (int i = 0; i < n; i++) c[i] = rng.NextDouble(-1.0, 1.0);
            var xl = arena.doubleVec(n);
            var xu = arena.doubleVec(n); for (int i = 0; i < n; i++) xu[i] = 5.0;
            var x0 = arena.doubleVec(n); x0[0] = 2.0; x0[1] = 2.0; x0[2] = 2.0;

            var A1 = arena.doubleMat(1, n); A1[0, 0] = 1.0; A1[0, 1] = 1.0; A1[0, 2] = 1.0;
            var b1 = arena.doubleVec(1); b1[0] = 6.0;
            var senses1 = new NativeArray<ConstraintSense>(1, Allocator.Temp); senses1[0] = ConstraintSense.LessEqual;
            var x1 = arena.doubleVec(n); x1[0] = x0[0]; x1[1] = x0[1]; x1[2] = x0[2];
            var info1 = QP.qpActiveSetCore(in Q, in c, in A1, in b1, senses1, in xl, in xu, ref x1, out double obj1, 0);

            var A2 = arena.doubleMat(2, n);
            A2[0, 0] = 1.0; A2[0, 1] = 1.0; A2[0, 2] = 1.0;
            A2[1, 0] = 1.0; A2[1, 1] = 1.0; A2[1, 2] = 1.0;
            var b2 = arena.doubleVec(2); b2[0] = 6.0; b2[1] = 6.0;
            var senses2 = new NativeArray<ConstraintSense>(2, Allocator.Temp);
            senses2[0] = ConstraintSense.LessEqual; senses2[1] = ConstraintSense.LessEqual;
            var x2 = arena.doubleVec(n); x2[0] = x0[0]; x2[1] = x0[1]; x2[2] = x0[2];
            var info2 = QP.qpActiveSetCore(in Q, in c, in A2, in b2, senses2, in xl, in xu, ref x2, out double obj2, 0);

            H.AssertTrue(Fail, 1, info1.status == QPStatus.Optimal);
            H.AssertTrue(Fail, 2, info2.status == QPStatus.Optimal);
            H.AssertLE(Fail, 3, math.abs(obj1 - obj2), 1e-9 * (1.0 + math.abs(obj1)));
            for (int i = 0; i < n; i++)
                H.AssertLE(Fail, 4 + i, math.abs(x1[i] - x2[i]), 1e-6);
    ...
    public void Degenerate_Float(int seed) => H.Run(fail => new DegenerateFloatJob { Seed = seed, Fail = fail }.Run());

    [TestCaseSource(nameof(DegenerateCases))]
    public void Degenerate_Double(int seed) => H.Run(fail => new DegenerateDoubleJob { Seed = seed, Fail = fail }.Run());
    ...
    // ================================================================================================
    [BurstCompile(CompileSynchronously = true, FloatPrecision = FloatPrecision.High, FloatMode = FloatMode.Default)]
    public struct UnboundedFloatJob : IJob
    {
        public NativeArray<double> Fail;
        public void Execute()
        {
            var arena = new Arena(Allocator.Persistent);
            var Q = arena.floatMat(2, 2); Q[0, 0] = 1f; Q[1, 1] = 0f;
            var c = arena.floatVec(2); c[1] = -1f;
            var A = arena.floatMat(0, 2);
            var b = arena.floatVec(0);
            var senses = new NativeArray<ConstraintSense>(0, Allocator.Temp);
            var xl = arena.floatVec(2); xl[0] = -1e30f; xl[1] = 0f;
            var xu = arena.floatVec(2); xu[0] = 1e30f; xu[1] = 1e30f;
            var x = arena.floatVec(2);   // (0,0), feasible
    ...
    [BurstCompile(CompileSynchronously = true, FloatPrecision = FloatPrecision.High, FloatMode = FloatMode.Default)]
    public struct UnboundedDoubleJob : IJob
    {
        public NativeArray<double> Fail;
        public void Execute()
        {
            var arena = new Arena(Allocator.Persistent);
            var Q = arena.doubleMat(2, 2); Q[0, 0] = 1.0; Q[1, 1] = 0.0;
            var c = arena.doubleVec(2); c[1] = -1.0;
            var A = arena.doubleMat(0, 2);
            var b = arena.doubleVec(0);
            var senses = new NativeArray<ConstraintSense>(0, Allocator.Temp);
            var xl = arena.doubleVec(2); xl[0] = -1e30; xl[1] = 0.0;
            var xu = arena.doubleVec(2); xu[0] = 1e30; xu[1] = 1e30;
    ...
    [Test] public void Unbounded_Float() => H.Run(fail => new UnboundedFloatJob { Fail = fail }.Run());
    [Test] public void Unbounded_Double() => H.Run(fail => new UnboundedDoubleJob { Fail = fail }.Run());

    // ---- shared test-side helpers (Fail[]-array Burst diagnostic pattern, see QPEqpTests.cs) ----
    static class H
    {
