...
// Property tests for the integer Hash surface (int / short / long / uint) over uintN / uintMxN.
...
public class uintHashTests
{
    ...
    public struct HashTestJob : IJob
    {
        ...
        uintN MakeVec(ref Arena arena, int n)
        {
            var v = arena.uintVec(n);
            for (int i = 0; i < n; i++)
                v[i] = (uint)((i * 3 + 1) % 50);
            return v;
        }

        uintMxN MakeMat(ref Arena arena, int m, int n)
        {
            var A = arena.uintMat(m, n);
            for (int r = 0; r < m; r++)
                for (int c = 0; c < n; c++)
                    A[r, c] = (uint)((r * 7 + c * 2 + 1) % 50);
            return A;
        }
        ...
        void AvalancheElement(ref Arena arena)
        {
            ...
            v[3] = (uint)(v[3] + (uint)1); // perturb one element (cast: short+short widens to int)
        ...
        {
            var v2 = arena.uintVec(2);
            v2[0] = (uint)1; v2[1] = (uint)2;

            var v3 = arena.uintVec(3);
            v3[0] = (uint)1; v3[1] = (uint)2; v3[2] = (uint)0;
        ...
        void RowHashesConsistency(ref Arena arena)
        {
            ...
            var row = arena.uintVec(A.N_Cols);
        ...
        void ColHashesConsistency(ref Arena arena)
        {
            ...
            var col = arena.uintVec(A.M_Rows);
            for (int c = 0; c < A.N_Cols; c++)
            {
                for (int r = 0; r < A.M_Rows; r++)
                    col[r] = A[r, c];
