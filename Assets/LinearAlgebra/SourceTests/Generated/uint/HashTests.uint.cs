...
public class uintHashTests
{
    ...
    public struct HashTestJob : IJob
    {
        ...
        uintN MakeVec(int n)
        {
            var v = new uintN(n, Allocator.Temp);
            for (int i = 0; i < n; i++)
                v[i] = (uint)((i * 3 + 1) % 50);
            return v;
        }

        uintMxN MakeMat(int m, int n)
        {
            var A = new uintMxN(m, n, Allocator.Temp);
            for (int r = 0; r < m; r++)
                for (int c = 0; c < n; c++)
        ...
        {
            var v2 = new uintN(2, Allocator.Temp);
            v2[0] = (uint)1; v2[1] = (uint)2;

            var v3 = new uintN(3, Allocator.Temp);
        ...
        {
            var e = new uintN(0, Allocator.Temp);
        ...
        void RowHashesConsistency()
        {
            ...
            var row = new uintN(A.N_Cols, Allocator.Temp);
        ...
        void ColHashesConsistency()
        {
            ...
            var col = new uintN(A.M_Rows, Allocator.Temp);
            for (int c = 0; c < A.N_Cols; c++)
            {
                for (int r = 0; r < A.M_Rows; r++)
                    col[r] = A[r, c];
    ...
    {
        var A = new uintMxN(3, 4, Allocator.Temp);
        for (int r = 0; r < 3; r++)
            for (int c = 0; c < 4; c++)
                A[r, c] = (uint)(r + c);
    ...
    {
        var A = new uintMxN(4, 0, Allocator.Temp);
    ...
    {
        var A = new uintMxN(0, 4, Allocator.Temp);
