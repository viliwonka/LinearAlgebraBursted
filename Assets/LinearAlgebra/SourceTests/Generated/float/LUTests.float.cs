...
public class floatLUTests
{
    ...
    public struct TestJob : IJob
    {
        ...
        public void LUDecompSingularStatus()
        {
            ...
            int dim = 8;
            var A = arena.floatMat(dim, dim); // zero matrix -> singular
            var L = arena.floatIdentityMat(dim);
            var U = arena.floatMat(dim, dim);
        ...
        void LUDecompVariantsPreserveA()
        {
            ...
            {
                var A = arena.floatRandomMat(dim, dim, -5f, 5f, 424242);
            ...
            {
                var A = arena.floatRandomMat(dim, dim, -5f, 5f, 535353);
                for (int d = 0; d < dim; d++) A[d, d] += 15f;
                float checksumBefore = Checksum(in A);
        ...
        void LUSolveInPlaceExitIsUsableFactor()
        {
            ...
            var xKnown2 = arena.floatRandomVec(dim, 1f, 5f, 222);
        ...
        void LUDecompSolveTransA()
        {
            ...
            var At = arena.floatMat(dim, dim);
            for (int r = 0; r < dim; r++)
                for (int c = 0; c < dim; c++)
                    At[r, c] = A[c, r];

            var xKnown = arena.floatRandomVec(dim, 1f, 5f, 777);
        ...
        void LUDecompSolveTransAMatrix()
        {
            ...
            var B = arena.floatRandomMat(dim, rhsCount, -3f, 3f, 13131);
        ...
        void LUSolveInPlaceTransAMatchesDecompSolve()
        {
            ...
            // ---- vector ----
            var b = arena.floatRandomVec(dim, -4f, 4f, 13579);
            ...
            // ---- matrix ----
            int rhsCount = 3;
            var B = arena.floatRandomMat(dim, rhsCount, -4f, 4f, 24680);
        ...
        void LUSolveInPlaceTransASingularParity()
        {
            ...
            var b = arena.floatRandomVec(dim, -3f, 3f, 135791);
            ...
            // ---- matrix ----
            var A2 = arena.floatRandomMat(dim, dim, 1f, 10f, 8821);
            for (int d = 0; d < dim; d++) A2[d, d] += 20f;
            for (int c = 0; c < dim; c++) A2[5, c] = A2[2, c];

            var B = arena.floatRandomMat(dim, 3, -3f, 3f, 246813);
            var Bsnapshot = B.Copy();
