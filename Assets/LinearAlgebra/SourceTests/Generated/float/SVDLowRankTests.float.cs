...
public class floatSVDLowRankTests
{
    ...
    public struct TestJob : IJob
    {
        ...
        void RandomTall12x5()
        {
            ...
            var A = arena.floatRandomMat(12, 5, (float)(-2f), (float)2f, 555111);
        ...
        {
            var arena = new Arena(Allocator.Persistent);
            var A = arena.floatRandomMat(8, 8, (float)(-3f), (float)3f, 909090);
            var fullS = Spectrum(in A, ref arena, out float normA2);
            CheckAtK(in A, in fullS, 1, normA2, ref arena);
        ...
        void LowRank10x6r3()
        {
            var arena = new Arena(Allocator.Persistent);
            int m = 10, n = 6, r = 3;
            var B = arena.floatRandomMat(m, r, (float)(-2f), (float)2f, 121212);
            var C = arena.floatRandomMat(r, n, (float)(-2f), (float)2f, 343434);
            var A = float_OP.dot(B, C);   // rank 3
            var fullS = Spectrum(in A, ref arena, out float normA2);
        ...
        void GklVsThin_40x12()
        {
            var arena = new Arena(Allocator.Persistent);
            int m = 40, n = 12;
            var A = arena.floatRandomMat(m, n, (float)(-2f), (float)2f, 777111);
        ...
        void GklExactLowRank_24x8r4()
        {
            var arena = new Arena(Allocator.Persistent);
            int m = 24, n = 8, r = 4;
            var B = arena.floatRandomMat(m, r, (float)(-2f), (float)2f, 8881);
            var C = arena.floatRandomMat(r, n, (float)(-2f), (float)2f, 9992);
            var A = float_OP.dot(B, C);   // exactly rank 4
        ...
        void GklReconBound_30x10()
        {
            var arena = new Arena(Allocator.Persistent);
            int m = 30, n = 10;
            var A = arena.floatRandomMat(m, n, (float)(-3f), (float)3f, 313131);
        ...
        void GklOrthonormal_20x7()
        {
            var arena = new Arena(Allocator.Persistent);
            int m = 20, n = 7;
            var A = arena.floatRandomMat(m, n, (float)(-4f), (float)4f, 202020);
                             ...
                             out floatMxN Ularge, out floatMxN Vmat)
        {
            ...
            floatRandomMatrix_OP.randomOrthogonalInpl(ref rng, ref Ularge);
            floatRandomMatrix_OP.randomOrthogonalInpl(ref rng, ref Vmat);
        }
        ...
        void GklConvergedFalse_RankDeficient()
        {
            ...
            var B = arena.floatRandomMat(m, r, (float)(-2f), (float)2f, 0xBADC0DE0u);
            var C = arena.floatRandomMat(r, n, (float)(-2f), (float)2f, 0xBADC0DE1u);
            var A = float_OP.dot(B, C);  // exactly rank 3
        ...
        void GklConvergedFalse_TooFewSteps()
        {
            ...
            var A = arena.floatRandomMat(m, n, (float)(-2f), (float)2f, 0x55AA77BBu);
        ...
        void GklConvergedFalse_MaxIter1()
        {
            ...
            var A = arena.floatRandomMat(m, n, (float)(-3f), (float)3f, 0xDEADBEEFu);
        ...
        // whose singular values are prescribed. svTol scales with the type via Consts.floatSqrtEps
        ...
        void GklGeometricTrunc_80x30()
        {
            ...
            double s = 1.0;
            for (int i = 0; i < n; i++) { sigma[i] = (float)s; s *= 0.5; }   // 1, 0.5, 0.25, ...
        ...
        void GklFlatCliffTrunc_70x25()
        {
            ...
            CheckTruncVsThin(in A, 3, 6, 0x2222BBBBu, m, n,
                             (float)8 * Consts.floatSqrtEps * (sigma[0] + (float)1),
                             (float)0.01f, (float)1E-3f, ref arena);
            arena.Dispose();
        }
        ...
        void GklOneSmallTrunc_50x20()
        {
            ...
            var sigma = arena.floatVec(n);
            for (int i = 0; i < n; i++) sigma[i] = (i == n - 1) ? (float)1E-4f : (float)1;
            ...
            float svTol = (float)8 * Consts.floatSqrtEps * (sigma[0] + (float)1);
            for (int t = 0; t < k; t++) AssertClose(Sk[t], (float)1, svTol);
            AssertOrthoCols(in Uk, m, k, (float)1E-3f);
            AssertOrthoCols(in Vk, n, k, (float)1E-3f);
        ...
        void AssertProjectorMatch(in floatMxN B, in floatMxN Ref, int rows, int k, float tol)
        {
            for (int i = 0; i < rows; i++)
                for (int j = 0; j < rows; j++)
                {
                    float pb = (float)0, pr = (float)0;
        ...
        void RunTruncWithReorth(in floatMxN A, in floatN sigmaTrue, int k, int oversample, uint seed,
                                bool partialReorth, ref floatN SkOut, int m, int n,
                                float svTol, ref Arena arena)
        {
            var Uk = arena.floatMat(m, k);
            var Sk = arena.floatVec(k);
            var Vk = arena.floatMat(n, k);
            ...
            AssertLE(math.abs(err2 - tail), (float)1E-2f * (tail + (float)1));
        }
        ...
        void CheckPartialReorth(in floatMxN A, in floatN sigmaTrue, int k, int oversample, uint seed,
                                int m, int n, float svTol, ref Arena arena)
        {
            var Uk = arena.floatMat(m, k);
            var Sk = arena.floatVec(k);
            var Vk = arena.floatMat(n, k);
            ...
            for (int t = 0; t < k; t++)
            {
                float best = math.abs(Sk[t] - sigmaTrue[0]);
                for (int i = 1; i < n; i++)
                {
                    float d = math.abs(Sk[t] - sigmaTrue[i]);
                    if (d < best) best = d;
                }
                AssertLE(best, svTol);
            }
        ...
        void PartialVsFull_Geometric_80x30()
        {
            ...
            var sigma = arena.floatVec(n);
            double s = 1.0;
            for (int i = 0; i < n; i++) { sigma[i] = (float)s; s *= 0.5; }
            ...
            var SkP = arena.floatVec(k);
            var SkF = arena.floatVec(k);
        ...
        void PartialOrthonormal_70x25()
        {
            ...
            var sigma = arena.floatVec(n);
            for (int i = 0; i < n; i++)
                sigma[i] = (i == 0) ? (float)100 : (i == 1) ? (float)80 : (i == 2) ? (float)60 :
                           (i == 3) ? (float)40 : (float)1E-2f;
            ...
            for (int t = 0; t < k; t++)
            {
                ...
                if (t > 0)
                {
                    bool desc = Sk[t] <= Sk[t - 1] + (float)1E-4f;
                    if (!desc) Record(Sk[t], Sk[t - 1], Sk[t] - Sk[t - 1]);
                    Assert.IsTrue(desc);
                }
            }
        ...
        void PartialClustered_50x30()
        {
            ...
            CheckPartialReorth(in A, in sigma, k, 12, 0xC1057E12u, m, n, (float)0.05f, ref arena);
        ...
        void PartialIllConditioned_60x20()
        {
            ...
            var sigma = arena.floatVec(n);
            for (int i = 0; i < n; i++)
                sigma[i] = (i == 0) ? (float)1000 : (i == 1) ? (float)500 : (i == 2) ? (float)200 : (float)0.1f;
            ...
            CheckPartialReorth(in A, in sigma, k, 4, 0x111C0AD3u, m, n, (float)5f, ref arena);
        ...
        void PartialLargeP_120x60()
        {
            ...
            var sigma = arena.floatVec(n);
            for (int i = 0; i < n; i++)
                sigma[i] = (i < 15) ? (float)(100.0 - 2.0 * i) : (float)1E-2f;
        ...
        void PartialCloseClusterSeeds_64x24()
        {
            ...
            var sigma = arena.floatVec(n);
            for (int i = 0; i < n; i++)
                sigma[i] = (i < 3) ? (float)(10.0 - 0.3 * i) : (float)(0.5 / (i - 1));
