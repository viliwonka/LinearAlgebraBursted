...
// criterion -- the caller sees the final X written through the ref floatMxN -- is exercised by
// construction.
public class floatBlockTFQMRTests
{
    ...
    public struct BlockTfqmrTestJob : IJob
    {
        ...
        {
            var A = GenerateOP.floatRandomMat(n, n, (float)(-1f), (float)1f, seed, Allocator.Temp);
        ...
        static bool BlockResidualDenseOK(in floatMxN A, in floatMxN X, in floatMxN B, int s, int n, float tol)
        {
            var AX = new floatMxN(s, n, Allocator.Temp);
            new floatDenseOperatorGeneral(in A).ApplyBlock(in X, ref AX, s);
            for (int j = 0; j < s; j++)
            {
                float num = 0, den = 0;
                for (int c = 0; c < n; c++) { float d = AX[j, c] - B[j, c]; num += d * d; den += B[j, c] * B[j, c]; }
                if (math.sqrt(num) / math.sqrt(math.max(den, (float)1e-30)) > tol) return false;
            }
            return true;
        }

        static bool BlockResidualBSROK(in floatBSR A, in floatMxN X, in floatMxN B, int s, int n, float tol)
        {
            var AX = new floatMxN(s, n, Allocator.Temp);
        ...
        void SolvesDenseNonsym()
        {
            ...
            var B = GenerateOP.floatRandomMat(s, n, (float)(-1f), (float)1f, 0x1F02u, Allocator.Temp);
        ...
        void SolvesBSRNonsym()
        {
            int n = 80, s = 3;
            var A = ConvDiff1D(n);
            var B = GenerateOP.floatRandomMat(s, n, (float)(-1f), (float)1f, 0x1F12u, Allocator.Temp);
        ...
        void MatchesScalarTfqmrPerColumn()
        {
            ...
            var B = GenerateOP.floatRandomMat(s, n, (float)(-1f), (float)1f, 0x1F22u, Allocator.Temp);
            ...
            for (int j = 0; j < s; j++)
            {
                ...
                for (int c = 0; c < n; c++) xj[c] = (float)0;
        ...
        void KnownSolutionRecovered()
        {
            ...
            var Xk = GenerateOP.floatRandomMat(s, n, (float)(-1f), (float)1f, 0x1F32u, Allocator.Temp);   // known solution
        ...
        void IdentityFoldBitIdentical()
        {
            ...
            var B = GenerateOP.floatRandomMat(s, n, (float)(-1f), (float)1f, 0x1F42u, Allocator.Temp);
            int maxIter = 30 * n;
            float tol = ResTol();

            var opA = new floatDenseOperatorGeneral(in A);   // readonly struct -- construct once, reuse

            var X1     = new floatMxN(s, n, Allocator.Temp);
            var Rhat01 = new floatMxN(s, n, Allocator.Temp);
            var U1     = new floatMxN(s, n, Allocator.Temp);
            var W1     = new floatMxN(s, n, Allocator.Temp);
            var V1     = new floatMxN(s, n, Allocator.Temp);
            var AU1    = new floatMxN(s, n, Allocator.Temp);
            var D1     = new floatMxN(s, n, Allocator.Temp);
            var UHat1  = new floatMxN(s, n, Allocator.Temp);
            var info1 = Krylov.btfqmr<floatDenseOperatorGeneral, floatIdentityPreconditioner>(
            ...
            var Rhat02 = new floatMxN(s, n, Allocator.Temp);
            var U2     = new floatMxN(s, n, Allocator.Temp);
            var W2     = new floatMxN(s, n, Allocator.Temp);
            var V2     = new floatMxN(s, n, Allocator.Temp);
            var AU2    = new floatMxN(s, n, Allocator.Temp);
            var D2     = new floatMxN(s, n, Allocator.Temp);
            var info2 = Krylov.btfqmr<floatDenseOperatorGeneral>(
        ...
        void DuplicateRHSRowsBitIdentical()
        {
            ...
            var B = GenerateOP.floatRandomMat(s, n, (float)(-1f), (float)1f, 0x1F52u, Allocator.Temp);
        ...
        void MaxIterBudgetHonestStatus()
        {
            ...
            var B = GenerateOP.floatRandomMat(s, n, (float)(-1f), (float)1f, 0x1F62u, Allocator.Temp);

            var X = new floatMxN(s, n, Allocator.Temp);
            var info = Krylov.btfqmr(in A, in B, ref X, 1, Consts.floatSqrtEps);   // deliberately tiny budget
        ...
        void PreconditionedILU0()
        {
            int n = 120, s = 3;
            var A = ConvDiff1D(n);
            var B = GenerateOP.floatRandomMat(s, n, (float)(-1f), (float)1f, 0x1F82u, Allocator.Temp);
        ...
        void ZeroRhs()
        {
            ...
            var B = new floatMxN(s, n, Allocator.Temp);
            for (int i = 0; i < s; i++)
                for (int c = 0; c < n; c++) B[i, c] = (float)0;

            var X = new floatMxN(s, n, Allocator.Temp);
            for (int i = 0; i < s; i++)
                for (int c = 0; c < n; c++) X[i, c] = (float)5;
