// <auto-generated>
//   Generated from Assets/LinearAlgebra/CodeGen/TemplateSourceTests/fProxy/GCRODRTests.fProxy.cs
...
public class floatGCRODRTests
{
    ...
    public struct GcrodrTestJob : IJob
    {
        ...
        static floatMxN SmallIsolatedEig(int n, uint seed)
        {
            ...
            A[2, 2] = (float)0.06;
            for (int i = 3; i < n; i++)
                A[i, i] = (float)(4.0 + 6.0 * (i - 3) / (double)(n - 4));   // cluster in [4, 10]

            var rnd = new Unity.Mathematics.Random(seed);
            for (int i = 0; i < n; i++)
                for (int j = i + 1; j < n; j++)
                    A[i, j] = (float)(rnd.NextFloat(-1f, 1f) * 0.02f);   // mild nonsymmetry, near-normal
            return A;
        }
        ...
        void RecyclingBeatsGmres()
        {
            ...
            var b = GenerateOP.floatRandomVec(n, -1f, 1f, 0x6C0Eu);   // excites all eigen-components

            var xG = new floatN(n, Allocator.Temp);     // plain gmres(m)
            var giG = Krylov.gmres(in A, in b, ref xG, m, maxIter, tol);

            var xR = new floatN(n, Allocator.Temp);     // gcrodr(m, recycle=k)
        ...
        void MatchesLUOracle()
        {
            ...
            var xTrue = GenerateOP.floatRandomVec(n, -1f, 1f, 0x6C12u);
            ...
            for (int i = 0; i < n; i++)
            {
                Assert.IsTrue(math.abs(x[i] - xLU[i]) <= Tol() * ((float)1 + math.abs(xLU[i])));
                Assert.IsTrue(math.abs(x[i] - xTrue[i]) <= Tol() * ((float)1 + math.abs(xTrue[i])));
            }
        }

        // ---- Identity fold, recycle > 0: the no-preconditioner entry point and the explicit
        // floatIdentityPreconditioner path share the exact IsIdentity-folded body, so x, iteration
        ...
        void IdentityFoldBitExact()
        {
            ...
            var b = GenerateOP.floatRandomVec(n, -1f, 1f, 0x6C22u);
            var op = new floatDenseOperator(in A);

            var xImplicit = new floatN(n, Allocator.Temp);
            var i0 = Krylov.gcrodr(in op, in b, ref xImplicit, m, k, 8 * n, Tol());

            var xExplicit = new floatN(n, Allocator.Temp);
            var i1 = Krylov.gcrodr(in op, default(floatIdentityPreconditioner), in b, ref xExplicit, m, k, 8 * n, Tol());
        ...
        void RecycleZeroMatchesGmres()
        {
            ...
            var b = GenerateOP.floatRandomVec(n, -1f, 1f, 0x6C32u);
        ...
        void Deterministic()
        {
            ...
            var b = GenerateOP.floatRandomVec(n, -1f, 1f, 0x6C42u);
        ...
        void ZeroRhs()
        {
            ...
            var b = new floatN(n, Allocator.Temp);   // all zeros
        ...
        {
            int n = 8;
            var A = new floatMxN(n, n, Allocator.Temp);   // all zeros -> singular, A x == 0 for every x
            var b = GenerateOP.floatRandomVec(n, 1f, 2f, 0x6C61u);   // nonzero, not in range(A)
