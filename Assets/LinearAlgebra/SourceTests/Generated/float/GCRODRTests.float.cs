...
public class floatGCRODRTests
{
    ...
    public struct GcrodrTestJob : IJob
    {
        ...
        void RecyclingBeatsGmres()
        {
            ...
            var b = arena.floatRandomVec(n, -1f, 1f, 0x6C0Eu);   // excites all eigen-components

            var xG = arena.floatVec(n);     // plain gmres(m)
            var giG = Krylov.gmres(in A, in b, ref xG, m, maxIter, tol);

            var xR = arena.floatVec(n);     // gcrodr(m, recycle=k)
        ...
        void MatchesLUOracle()
        {
            ...
            var xTrue = arena.floatRandomVec(n, -1f, 1f, 0x6C12u);
        ...
        void IdentityFoldBitExact()
        {
            ...
            var b = arena.floatRandomVec(n, -1f, 1f, 0x6C22u);
            var op = new floatDenseOperator(in A);

            var xImplicit = arena.floatVec(n);
            var i0 = Krylov.gcrodr(in op, in b, ref xImplicit, m, k, 8 * n, Tol());

            var xExplicit = arena.floatVec(n);
        ...
        void RecycleZeroMatchesGmres()
        {
            ...
            var b = arena.floatRandomVec(n, -1f, 1f, 0x6C32u);
        ...
        void Deterministic()
        {
            ...
            var b = arena.floatRandomVec(n, -1f, 1f, 0x6C42u);
        ...
        void ZeroRhs()
        {
            ...
            var b = arena.floatVec(n);   // all zeros
        ...
        void SingularBreakdown()
        {
            var arena = new Arena(Allocator.Persistent);
            int n = 8;
            var A = arena.floatMat(n, n);   // all zeros -> singular, A x == 0 for every x
            var b = arena.floatRandomVec(n, 1f, 2f, 0x6C61u);   // nonzero, not in range(A)
