...
// "Tests" section items 1-7). The coder's smoke tests live in ControlTests.float.cs; this file is the
...
// public S buffer. Templated (float) so codegen emits a float and a double build; per-dtype tolerances
// via choose-markers (float | double). Every numeric assertion routes through Fail[0..3] like the smoke file.
public class floatControlLQRTests
{
    ...
    public struct TestJob : IJob
    {
        ...
        static float LitKTol() => 5e-3f;   // published gain, |K|~O(1)
        static float OracleFloor() => 1e-4f; // oracle-loop stop
        static float CompareTol() => 5e-3f;   // SDA-vs-oracle rel Frob
        static float DareTol() => 5e-3f;      // DARE residual on S
        static float PsdEigFloor() => -3e-3f; // min eig(S) >= floor
        static float SymTol() => 1e-4f;
        ...
        void LiteratureDoubleIntegrator()
        {
            ...
            AssertClose(K[0, 1], (float)1.24392885, LitKTol());
            // S recovered from the warm state matches the published Riccati solution
            AssertClose(state.S[0, 0], (float)2.94712297, (float)5e-2);
            AssertClose(state.S[0, 1], (float)2.36920541, (float)5e-2);
            AssertClose(state.S[1, 1], (float)4.61313426, (float)5e-2);
            AssertLess(ClosedLoopSpecRad(in A, in B, in K), (float)1);
        ...
        void LiteratureScalarAnalytic()
        {
            ...
            float sExact = (float)(2.0 + sqrt5);
            float kExact = (float)((1.0 + sqrt5) / 2.0);
            ...
            for (int ni = 0; ni < 4; ni++)
            {
                ...
                for (int mi = 0; mi < 3; mi++)
                {
                    ...
                    if (info.status == LQRStatus.Converged)
                    {
                        ...
                        var Sor = new floatMxN(n, n, Allocator.Temp);
                        var Snx = new floatMxN(n, n, Allocator.Temp);
                        var Kor = new floatMxN(m, n, Allocator.Temp);
                        ...
                        var Sstep = new floatMxN(n, n, Allocator.Temp);
            ...
            for (int s = 0; s < 8; s++)
            {
                ...
                if (info.status == LQRStatus.Converged)
                {
                    ...
                    for (int i = 0; i < n; i++)
                        for (int j = i + 1; j < n; j++)
                        {
                            float d = math.abs(state.S[i, j] - state.S[j, i]);
                            if (d > maxAsym) maxAsym = d;
                        }
                    AssertClose(maxAsym, (float)0, SymTol());
        ...
        void ScheduleHandStepGeneralM()
        {
            ...
            var Shand = new floatMxN(n, n, Allocator.Temp);
            var Khand = new floatMxN(m, n, Allocator.Temp);
            RiccatiStepPublic(in A, in B, in Q, in R, in Q, ref Shand, ref Khand);   // one step from S=Qf=Q

            for (int i = 0; i < m; i++)
                for (int j = 0; j < n; j++)
                    AssertClose(Ksched[i, j], Khand[i, j], (float)5e-3);
        ...
        void ScheduleApproachesInfiniteHorizon()
        {
            ...
            for (int i = 0; i < m; i++)
                for (int j = 0; j < n; j++)
                    AssertClose(Ksched[i, j], Kinf[i, j], (float)5e-3);
        ...
        void FailureRankDeficientR()
        {
            ...
            B[0, 0] = (float)0; B[1, 0] = (float)1; B[0, 1] = (float)0; B[1, 1] = (float)1;   // duplicate column
            var Q = Eye(2);
        ...
        void DeterminismRandom()
        {
            ...
            for (int i = 0; i < m; i++)
                for (int j = 0; j < n; j++)
                    AssertClose(K1[i, j], K2[i, j], (float)0);   // exact
        ...
        void WarmDeterminism()
        {
            ...
            AssertEqInt(it1, it2);
            for (int j = 0; j < 2; j++) AssertClose(Kw1[0, j], Kw2[0, j], (float)0);

            Kw1.Dispose(); Kw2.Dispose(); A.Dispose(); B.Dispose(); Q.Dispose(); R.Dispose();
        }

        floatMxN WarmSequence(in floatMxN A, in floatMxN B, in floatMxN Q, in floatMxN R, out int warmIters)
        {
            var state = new floatLQRState(2, Allocator.Temp);
            var Kcold = new floatMxN(1, 2, Allocator.Temp);
                                      ...
                                      in floatMxN S, ref floatMxN Snext, ref floatMxN K)
        {
            ...
            var AtSBK = new floatMxN(n, n, Allocator.Temp);
        ...
        {
            A = new floatMxN(n, n, Allocator.Temp);
            for (int i = 0; i < n; i++)
                for (int j = 0; j < n; j++) A[i, j] = (float)rng.NextFloat(-1f, 1f);
            float sr = SpectralRadius(in A);
            float scale = sr > (float)1e-6 ? (float)1.05 / sr : (float)1;
            for (int i = 0; i < n; i++)
                for (int j = 0; j < n; j++) A[i, j] *= scale;

            B = new floatMxN(n, m, Allocator.Temp);
            for (int i = 0; i < n; i++)
                for (int j = 0; j < m; j++) B[i, j] = (float)rng.NextFloat(-1f, 1f);
            Q = Eye(n);
            R = new floatMxN(m, m, Allocator.Temp);
        ...
        static float SpectralRadius(in floatMxN M)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                float mg = math.sqrt(er[i] * er[i] + ei[i] * ei[i]);
                if (mg > mx) mx = mg;
            }
        ...
        static float ClosedLoopSpecRad(in floatMxN A, in floatMxN B, in floatMxN K)
        {
            int n = A.M_Rows;
            var Acl = new floatMxN(n, n, Allocator.Temp);
            Blas.dot(in B, in K, ref Acl);                       // B*K
            for (int i = 0; i < n; i++)
                for (int j = 0; j < n; j++) Acl[i, j] = A[i, j] - Acl[i, j];
            float sr = SpectralRadius(in Acl);
            Acl.Dispose();
            return sr;
        }
        ...
        static double FrobNorm(in floatMxN M)
        {
            double s = 0;
        ...
        static double FrobDiff(in floatMxN A, in floatMxN B)
        {
            double s = 0;
        ...
        // ---- Fail[0..3] diagnostic asserts (same shape as ControlTests.float.cs) ----
        ...
        void AssertGE(float got, float limit)
        {
            if (!(got >= limit) && Fail[0] == (float)0) { Fail[0] = (float)1; Fail[1] = got; Fail[2] = limit; Fail[3] = got - limit; }
            Assert.IsTrue(got >= limit);
        }
    ...
    // ---- extra managed-thread validation throws not already in ControlTests.float.cs ----
    ...
    public void LqrThrowsOnNonFiniteR()
    {
        ...
        var B = new floatMxN(2, 1, Allocator.Temp); B[1, 0] = (float)1;
