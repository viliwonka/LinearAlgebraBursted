// <auto-generated>
//   Generated from Assets/LinearAlgebra/CodeGen/TemplateSourceTests/fProxy/MPCTests.fProxy.cs
//   DO NOT EDIT BY HAND - edit the template and run Tools/regen.ps1.
// </auto-generated>
using System;

using BULA;
using BULA.Control;

using NUnit.Framework;
using Unity.Burst;
using Unity.Collections;
using Unity.Jobs;
using Unity.Mathematics;

// Acceptance battery for MPC.solve / floatMPCState, covering the properties verified in the design's
// numpy/scipy prototype before this feature was implemented (double integrator A=[[1,1],[0,1]],
// B=[[0],[1]], Q=I2, R=1 throughout -- the SAME system ControlLQRTests.float.cs's own
// LiteratureDoubleIntegrator job uses, so K/P below match that job's published literals):
//
// (a) UnconstrainedMatchesLQR -- wide input bounds reproduce LQR.lqr's OWN gain exactly (a
//     stationary terminal-P receding-horizon solve equals infinite-horizon LQR for ANY N >= 1 -- the
//     correctness anchor).
// (b) SaturatedMatchesOracle -- x0=(3,1.9), bounds [-2,2], N=8: independently solved via
//     scipy.optimize.minimize(method='trust-constr') on the SAME condensed QP (Gamma^T Qbar Gamma + Rbar
//     from the DARE terminal P), giving u0 = -1.999999998939275 (essentially exactly saturated).
// (c) Soft-row exact penalty (Kerrigan & Maciejowski 2000): inactive (x0=origin) reproduces the no-
//     soft-row build exactly with zero slack; active-avoidable (x0=(3,1.9), same as (b) -- the wall at
//     x[0]<=5 never actually binds there) reproduces the SAME saturated u0 with ~zero slack; active-
//     unavoidable (x0=(3,2.3)) matches a hand-derived closed form: stage 0's predicted position
//     x0[0]+x0[1]=5.3 exceeds the wall (5) REGARDLESS of any control (a double integrator's control only
//     affects position two steps later), forcing a minimum violation of 0.3 there; stage 1's predicted
//     position under maximum braking (u0 at its -2 lower bound) is x0[0]+2*x0[1]-2=5.6, forcing 0.6 --
//     the worst-case slack over the horizon.
// (d) Warm-start health: active-set churn (activeSetChanges) collapses to <= 3 after a short initial
//     transient in a receding-horizon run, and a warm sequence's iteration count at a given frame never
//     exceeds a freshly-constructed (cold) state solving the identical point.
//
// (e) Prestabilization is a pure change of coordinates and must reproduce the identical physical answer.
// (f) MPC.setSoftBound -- moving the wall equals rebuilding at that wall EXACTLY (nothing but the RHS
//     may depend on d), the per-stage form binds one stage at a time (which the constructor's shared d
//     cannot express), and a wall closing in every frame still leaves the warm path on the true
//     minimizer of the QP it was handed.
//
// Burst execution (house pattern): NUnit Assert.IsTrue with == only inside the job, first-failure
// recorded into Fail[0..3], read back on the managed side -- same shape as ControlLQRTests.float.cs /
// KalmanTests.float.cs.
public class floatMPCTests
{
    [BurstCompile(CompileSynchronously = true, FloatPrecision = FloatPrecision.High, FloatMode = FloatMode.Default)]
    public struct TestJob : IJob
    {
        public enum TestType
        {
            UnconstrainedMatchesLQR,
            SaturatedMatchesOracle,
            SoftRowInactiveMatchesNoSoftBuild,
            SoftRowAvoidableMatchesSaturatedOracle,
            SoftRowUnavoidableMinimalViolation,
            WarmStartChurnBound,
            WarmIterationsBeatCold,
            WarmPersistentMatchesColdEachFrame,
            PrestabBindingBoundMatchesNonPrestab,
            SetSoftBoundMatchesRebuild,
            SetSoftBoundPerStageBindsPerStage,
            SetSoftBoundSurvivesWarmStart,
        }

        public TestType Type;
        public NativeArray<float> Fail;   // [0]=flag [1]=got [2]=expected/limit [3]=diff/extra

        public void Execute()
        {
            switch (Type)
            {
                case TestType.UnconstrainedMatchesLQR: UnconstrainedMatchesLQR(); break;
                case TestType.SaturatedMatchesOracle: SaturatedMatchesOracle(); break;
                case TestType.SoftRowInactiveMatchesNoSoftBuild: SoftRowInactiveMatchesNoSoftBuild(); break;
                case TestType.SoftRowAvoidableMatchesSaturatedOracle: SoftRowAvoidableMatchesSaturatedOracle(); break;
                case TestType.SoftRowUnavoidableMinimalViolation: SoftRowUnavoidableMinimalViolation(); break;
                case TestType.WarmStartChurnBound: WarmStartChurnBound(); break;
                case TestType.WarmIterationsBeatCold: WarmIterationsBeatCold(); break;
                case TestType.WarmPersistentMatchesColdEachFrame: WarmPersistentMatchesColdEachFrame(); break;
                case TestType.PrestabBindingBoundMatchesNonPrestab: PrestabBindingBoundMatchesNonPrestab(); break;
                case TestType.SetSoftBoundMatchesRebuild: SetSoftBoundMatchesRebuild(); break;
                case TestType.SetSoftBoundPerStageBindsPerStage: SetSoftBoundPerStageBindsPerStage(); break;
                case TestType.SetSoftBoundSurvivesWarmStart: SetSoftBoundSurvivesWarmStart(); break;
            }
        }

        // ---- per-dtype tolerances (calibrated from the float32 prototype run, with margin) ----
        static float LqrU0Tol() => 5e-3f;
        static float OracleU0Tol() => 1e-2f;
        static double SlackTol() => 1e-2;

        // ============================ (a) unconstrained == infinite-horizon LQR ============================
        void UnconstrainedMatchesLQR()
        {
            var A = Mat2(1, 1, 0, 1);
            var B = ColVec2(0, 1);
            var Q = Eye(2); var R = R1(1);
            var uLo = Vec1((float)(-1e30)); var uHi = Vec1((float)1e30);   // library's own "unbounded" sentinel (QP.solve's own INF convention)

            var K = new floatMxN(1, 2, Allocator.Temp);
            var lqrInfo = LQR.lqr(in A, in B, in Q, in R, ref K);
            AssertTrue(lqrInfo.status == RiccatiStatus.Converged);

            var mpc = new floatMPCState(2, 1, 8, Allocator.Temp, in A, in B, in Q, in R, in uLo, in uHi);
            var reference = new floatN(2, Allocator.Temp);   // zero-initialized: track to the origin
            var u0 = new floatN(1, Allocator.Temp, true);
            var rng = new Unity.Mathematics.Random(0xA53Cu);

            for (int trial = 0; trial < 10; trial++)
            {
                var x0 = new floatN(2, Allocator.Temp, true);
                x0[0] = (float)rng.NextFloat(-2f, 2f);
                x0[1] = (float)rng.NextFloat(-2f, 2f);

                var info = MPC.solve(ref mpc, in x0, in reference, ref u0);
                AssertTrue(info.status == MPCStatus.Optimal || info.status == MPCStatus.MaxIterations);

                float u0lqr = -(K[0, 0] * x0[0] + K[0, 1] * x0[1]);
                AssertClose(u0[0], u0lqr, LqrU0Tol());

                x0.Dispose();
            }

            u0.Dispose(); reference.Dispose(); mpc.Dispose(); K.Dispose();
            A.Dispose(); B.Dispose(); Q.Dispose(); R.Dispose(); uLo.Dispose(); uHi.Dispose();
        }

        // ============================ (b) input-saturated vs scipy oracle ============================
        void SaturatedMatchesOracle()
        {
            var A = Mat2(1, 1, 0, 1); var B = ColVec2(0, 1); var Q = Eye(2); var R = R1(1);
            var uLo = Vec1((float)(-2)); var uHi = Vec1((float)2);
            var mpc = new floatMPCState(2, 1, 8, Allocator.Temp, in A, in B, in Q, in R, in uLo, in uHi);

            var x0 = Vec2(3, (float)1.9);
            var reference = new floatN(2, Allocator.Temp);   // zero-initialized: track to the origin
            var u0 = new floatN(1, Allocator.Temp, true);

            var info = MPC.solve(ref mpc, in x0, in reference, ref u0);
            AssertTrue(info.status == MPCStatus.Optimal || info.status == MPCStatus.MaxIterations);

            AssertClose(u0[0], (float)(-2.0), OracleU0Tol());

            x0.Dispose(); reference.Dispose(); u0.Dispose(); mpc.Dispose();
            A.Dispose(); B.Dispose(); Q.Dispose(); R.Dispose(); uLo.Dispose(); uHi.Dispose();
        }

        // ============================ (c) soft-row exact penalty ============================

        void SoftRowInactiveMatchesNoSoftBuild()
        {
            var A = Mat2(1, 1, 0, 1); var B = ColVec2(0, 1); var Q = Eye(2); var R = R1(1);
            var uLo = Vec1((float)(-2)); var uHi = Vec1((float)2);

            var mpcPlain = new floatMPCState(2, 1, 8, Allocator.Temp, in A, in B, in Q, in R, in uLo, in uHi);
            var C = WallRow(); var d = Vec1((float)5);
            var mpcSoft = new floatMPCState(2, 1, 8, Allocator.Temp, in A, in B, in Q, in R, in uLo, in uHi, in C, in d);

            var x0 = new floatN(2, Allocator.Temp);   // zero-initialized: origin, far from the wall
            var reference = new floatN(2, Allocator.Temp);   // zero-initialized: track to the origin
            var u0a = new floatN(1, Allocator.Temp, true);
            var u0b = new floatN(1, Allocator.Temp, true);

            var infoA = MPC.solve(ref mpcPlain, in x0, in reference, ref u0a);
            var infoB = MPC.solve(ref mpcSoft, in x0, in reference, ref u0b);
            AssertTrue(infoA.status == MPCStatus.Optimal);
            AssertTrue(infoB.status == MPCStatus.Optimal);

            AssertClose(u0a[0], u0b[0], OracleU0Tol());
            AssertLEd(infoB.maxSlackViolation, SlackTol());

            x0.Dispose(); reference.Dispose(); u0a.Dispose(); u0b.Dispose();
            mpcPlain.Dispose(); mpcSoft.Dispose(); C.Dispose(); d.Dispose();
            A.Dispose(); B.Dispose(); Q.Dispose(); R.Dispose(); uLo.Dispose(); uHi.Dispose();
        }

        // x0=(3,1.9): the SAME point as SaturatedMatchesOracle -- the wall (x[0]<=5) never actually
        // binds along this trajectory (predicted positions peak at ~4.9), so the soft-row build must
        // reproduce the SAME oracle u0 with ~zero slack.
        void SoftRowAvoidableMatchesSaturatedOracle()
        {
            var A = Mat2(1, 1, 0, 1); var B = ColVec2(0, 1); var Q = Eye(2); var R = R1(1);
            var uLo = Vec1((float)(-2)); var uHi = Vec1((float)2);
            var C = WallRow(); var d = Vec1((float)5);
            var mpc = new floatMPCState(2, 1, 8, Allocator.Temp, in A, in B, in Q, in R, in uLo, in uHi, in C, in d);

            var x0 = Vec2(3, (float)1.9);
            var reference = new floatN(2, Allocator.Temp);   // zero-initialized: track to the origin
            var u0 = new floatN(1, Allocator.Temp, true);

            var info = MPC.solve(ref mpc, in x0, in reference, ref u0);
            AssertTrue(info.status == MPCStatus.Optimal || info.status == MPCStatus.MaxIterations);

            AssertClose(u0[0], (float)(-2.0), OracleU0Tol());
            AssertLEd(info.maxSlackViolation, SlackTol());

            x0.Dispose(); reference.Dispose(); u0.Dispose(); mpc.Dispose(); C.Dispose(); d.Dispose();
            A.Dispose(); B.Dispose(); Q.Dispose(); R.Dispose(); uLo.Dispose(); uHi.Dispose();
        }

        // x0=(3,2.3): stage 0's predicted position (x0[0]+x0[1] = 5.3) exceeds the wall (5) with NO
        // dependence on any control (B's effect on position lags one step for a double integrator),
        // forcing a minimum 0.3 violation there regardless of the solve. Stage 1's predicted position
        // under maximum braking (u0 pinned at its -2 lower bound, the best any control can do) is
        // x0[0]+2*x0[1]+uLo = 3+4.6-2 = 5.6, forcing 0.6 -- the worst-case (reported) slack.
        void SoftRowUnavoidableMinimalViolation()
        {
            var A = Mat2(1, 1, 0, 1); var B = ColVec2(0, 1); var Q = Eye(2); var R = R1(1);
            var uLo = Vec1((float)(-2)); var uHi = Vec1((float)2);
            var C = WallRow(); var d = Vec1((float)5);
            var mpc = new floatMPCState(2, 1, 8, Allocator.Temp, in A, in B, in Q, in R, in uLo, in uHi, in C, in d);

            var x0 = Vec2(3, (float)2.3);
            var reference = new floatN(2, Allocator.Temp);   // zero-initialized: track to the origin
            var u0 = new floatN(1, Allocator.Temp, true);

            var info = MPC.solve(ref mpc, in x0, in reference, ref u0);
            AssertTrue(info.status == MPCStatus.Optimal || info.status == MPCStatus.MaxIterations);
            AssertCloseD(info.maxSlackViolation, 0.6, SlackTol());

            x0.Dispose(); reference.Dispose(); u0.Dispose(); mpc.Dispose(); C.Dispose(); d.Dispose();
            A.Dispose(); B.Dispose(); Q.Dispose(); R.Dispose(); uLo.Dispose(); uHi.Dispose();
        }

        // ============================ (d) warm-start health ============================

        // Receding-horizon run from a displaced start (x0=(5,0), tight input bounds [-0.5,0.5], N=8):
        // active-set churn must collapse to a small bound after a short initial transient.
        void WarmStartChurnBound()
        {
            var A = Mat2(1, 1, 0, 1); var B = ColVec2(0, 1); var Q = Eye(2); var R = R1(1);
            var uLo = Vec1((float)(-0.5)); var uHi = Vec1((float)0.5);
            var mpc = new floatMPCState(2, 1, 8, Allocator.Temp, in A, in B, in Q, in R, in uLo, in uHi);

            var x = Vec2(5, 0);
            var reference = new floatN(2, Allocator.Temp);   // zero-initialized: track to the origin
            var u0 = new floatN(1, Allocator.Temp, true);

            const int steadyFrame = 8;
            const int totalFrames = 20;
            for (int f = 0; f < totalFrames; f++)
            {
                var info = MPC.solve(ref mpc, in x, in reference, ref u0);
                AssertTrue(info.status == MPCStatus.Optimal || info.status == MPCStatus.MaxIterations);
                if (f >= steadyFrame) AssertLE(info.activeSetChanges, 3);

                float x0n = x[0] + x[1];
                float x1n = x[1] + u0[0];
                x[0] = x0n; x[1] = x1n;
            }

            x.Dispose(); reference.Dispose(); u0.Dispose(); mpc.Dispose();
            A.Dispose(); B.Dispose(); Q.Dispose(); R.Dispose(); uLo.Dispose(); uHi.Dispose();
        }

        // At a fixed later frame, a warm-started sequence's own iteration count must not exceed a
        // freshly-constructed (cold) state solving the IDENTICAL point (same optimum either way -- a
        // strictly convex QP's solution does not depend on warm-starting, only the pivot count does).
        void WarmIterationsBeatCold()
        {
            var A = Mat2(1, 1, 0, 1); var B = ColVec2(0, 1); var Q = Eye(2); var R = R1(1);
            var uLo = Vec1((float)(-0.5)); var uHi = Vec1((float)0.5);
            var mpcWarm = new floatMPCState(2, 1, 8, Allocator.Temp, in A, in B, in Q, in R, in uLo, in uHi);

            var x = Vec2(5, 0);
            var reference = new floatN(2, Allocator.Temp);   // zero-initialized: track to the origin
            var u0 = new floatN(1, Allocator.Temp, true);

            const int targetFrame = 10;
            var xAtTarget = new floatN(2, Allocator.Temp, true);
            int warmIters = 0;
            for (int f = 0; f <= targetFrame; f++)
            {
                if (f == targetFrame) xAtTarget.CopyFrom(x);
                var info = MPC.solve(ref mpcWarm, in x, in reference, ref u0);
                if (f == targetFrame) warmIters = info.iterations;

                float x0n = x[0] + x[1];
                float x1n = x[1] + u0[0];
                x[0] = x0n; x[1] = x1n;
            }

            var mpcCold = new floatMPCState(2, 1, 8, Allocator.Temp, in A, in B, in Q, in R, in uLo, in uHi);
            var u0c = new floatN(1, Allocator.Temp, true);
            var coldInfo = MPC.solve(ref mpcCold, in xAtTarget, in reference, ref u0c);
            AssertTrue(coldInfo.status == MPCStatus.Optimal || coldInfo.status == MPCStatus.MaxIterations);

            AssertLE(warmIters, coldInfo.iterations);

            u0c.Dispose(); mpcCold.Dispose(); xAtTarget.Dispose();
            x.Dispose(); reference.Dispose(); u0.Dispose(); mpcWarm.Dispose();
            A.Dispose(); B.Dispose(); Q.Dispose(); R.Dispose(); uLo.Dispose(); uHi.Dispose();
        }

        // The persistent warm path (MPC.solve carries qpFactor/qpReduced across ticks, reusing them
        // whole when the working set has not moved) must land on the SAME optimum a from-scratch COLD
        // solve of the IDENTICAL condensed QP finds -- EVERY frame, across both the steady-state reuse
        // ticks and the working-set-changing (rebuild) ticks. The condensed Hessian is PD (R > 0), so
        // the minimizer is unique: cross-tick reuse cannot silently drift the answer. x0=(5,0) with a
        // tight [-0.5,0.5] input bound saturates early (working-set churn) then relaxes to steady state
        // -- exercising both code paths in one run.
        void WarmPersistentMatchesColdEachFrame()
        {
            var A = Mat2(1, 1, 0, 1); var B = ColVec2(0, 1); var Q = Eye(2); var R = R1(1);
            var uLo = Vec1((float)(-0.5)); var uHi = Vec1((float)0.5);
            var mpc = new floatMPCState(2, 1, 8, Allocator.Temp, in A, in B, in Q, in R, in uLo, in uHi);

            var x = Vec2(5, 0);
            var reference = new floatN(2, Allocator.Temp);   // track to the origin
            var u0 = new floatN(1, Allocator.Temp, true);

            int nz = mpc.nz;
            var xCold = new floatN(nz, Allocator.Temp, true);
            double objTol = 3e-3;
            float zTol = 5e-3f;

            for (int f = 0; f < 16; f++)
            {
                var info = MPC.solve(ref mpc, in x, in reference, ref u0);
                AssertTrue(info.status == MPCStatus.Optimal || info.status == MPCStatus.MaxIterations);

                // Independently COLD-solve the same condensed QP MPC just warm-solved: its gradient and
                // general RHS are still in cScratch/bScratch, the warm solution in z. Same optimum since
                // H is PD -- pins that the persistent reuse produced the true minimizer this frame.
                for (int i = 0; i < nz; i++) xCold[i] = (float)0;
                var coldInfo = QP.solve(in mpc.H, in mpc.cScratch, in mpc.Arows, in mpc.bScratch, in mpc.senses,
                                        in mpc.xl, in mpc.xu, ref xCold, out double objCold, 0);
                AssertTrue(coldInfo.status == QPStatus.Optimal);
                AssertCloseD(info.objective, objCold, objTol);
                for (int i = 0; i < nz; i++) AssertClose(mpc.z[i], xCold[i], zTol);

                float x0n = x[0] + x[1];
                float x1n = x[1] + u0[0];
                x[0] = x0n; x[1] = x1n;
            }

            xCold.Dispose();
            x.Dispose(); reference.Dispose(); u0.Dispose(); mpc.Dispose();
            A.Dispose(); B.Dispose(); Q.Dispose(); R.Dispose(); uLo.Dispose(); uHi.Dispose();
        }

        // ============================ (e) prestabilization correctness ============================

        // Prestabilization is a PURE change of coordinates, so it must reproduce the IDENTICAL
        // physical answer as solving the SAME (A,B,Q,R,uLo,uHi) problem without it. x0=(3,1.9) is
        // the SAME binding scenario as SaturatedMatchesOracle (u0 saturates at uLo=-2), reused here
        // as the discriminating case.
        void PrestabBindingBoundMatchesNonPrestab()
        {
            var A = Mat2(1, 1, 0, 1); var B = ColVec2(0, 1); var Q = Eye(2); var R = R1(1);
            var uLo = Vec1((float)(-2)); var uHi = Vec1((float)2);

            var Kstab = new floatMxN(1, 2, Allocator.Temp);
            var lqrInfo = LQR.lqr(in A, in B, in Q, in R, ref Kstab);
            AssertTrue(lqrInfo.status == RiccatiStatus.Converged);

            var mpcPlain = new floatMPCState(2, 1, 8, Allocator.Temp, in A, in B, in Q, in R, in uLo, in uHi);
            var mpcPrestab = new floatMPCState(2, 1, 8, Allocator.Temp, in A, in B, in Q, in R, in uLo, in uHi,
                                                default, default, default, default(floatN), (float)0, (float)0, in Kstab);

            var x0 = Vec2(3, (float)1.9);
            var reference = new floatN(2, Allocator.Temp);   // zero-initialized: track to the origin
            var u0Plain = new floatN(1, Allocator.Temp, true);
            var u0Prestab = new floatN(1, Allocator.Temp, true);

            var infoPlain = MPC.solve(ref mpcPlain, in x0, in reference, ref u0Plain);
            var infoPrestab = MPC.solve(ref mpcPrestab, in x0, in reference, ref u0Prestab);
            AssertTrue(infoPlain.status == MPCStatus.Optimal || infoPlain.status == MPCStatus.MaxIterations);
            AssertTrue(infoPrestab.status == MPCStatus.Optimal || infoPrestab.status == MPCStatus.MaxIterations);

            // (i) reconstruct u0 = -Kstab*x0 + v0 INDEPENDENTLY (reading mpcPrestab.Kstab/z directly,
            // not trusting MPC.solve's own internal ExtractU0 conversion) and check it respects the
            // physical input bound -- the off-by-one bug placed this wildly outside [-2,2].
            float uRecon = -(mpcPrestab.Kstab[0, 0] * x0[0] + mpcPrestab.Kstab[0, 1] * x0[1]) + mpcPrestab.z[0];
            AssertClose(uRecon, u0Prestab[0], OracleU0Tol());
            AssertTrue(uRecon >= uLo[0] - OracleU0Tol() && uRecon <= uHi[0] + OracleU0Tol());

            // (ii) same physical problem, different coordinates -- must agree to tight tolerance.
            AssertClose(u0Prestab[0], u0Plain[0], OracleU0Tol());
            AssertClose(u0Prestab[0], (float)(-2.0), OracleU0Tol());   // both saturate at uLo, per the prototype

            x0.Dispose(); reference.Dispose(); u0Plain.Dispose(); u0Prestab.Dispose();
            mpcPlain.Dispose(); mpcPrestab.Dispose(); Kstab.Dispose();
            A.Dispose(); B.Dispose(); Q.Dispose(); R.Dispose(); uLo.Dispose(); uHi.Dispose();
        }

        // ================================ helpers ================================

        static floatMxN Mat2(float a, float b, float c, float d)
        {
            var M = new floatMxN(2, 2, Allocator.Temp);
            M[0, 0] = a; M[0, 1] = b; M[1, 0] = c; M[1, 1] = d;
            return M;
        }

        static floatMxN ColVec2(float a, float b)
        {
            var M = new floatMxN(2, 1, Allocator.Temp);
            M[0, 0] = a; M[1, 0] = b;
            return M;
        }

        static floatMxN Eye(int n)
        {
            var M = new floatMxN(n, n, Allocator.Temp);
            for (int i = 0; i < n; i++) M[i, i] = (float)1;
            return M;
        }

        static floatMxN R1(float v)
        {
            var M = new floatMxN(1, 1, Allocator.Temp);
            M[0, 0] = v;
            return M;
        }

        static floatMxN WallRow()
        {
            var M = new floatMxN(1, 2, Allocator.Temp);
            M[0, 0] = (float)1; M[0, 1] = (float)0;
            return M;
        }

        static floatN Vec1(float v)
        {
            var vec = new floatN(1, Allocator.Temp, true);
            vec[0] = v;
            return vec;
        }

        static floatN Vec2(float a, float b)
        {
            var vec = new floatN(2, Allocator.Temp, true);
            vec[0] = a; vec[1] = b;
            return vec;
        }

        // ============================ (f) moving soft bounds ============================

        // Moving the bound must produce the SAME QP a rebuild at that bound produces. Both states here
        // are freshly constructed and solved once, so nothing but rowConstRHS can differ and the two u0
        // values must agree EXACTLY -- an inexact match means d leaked into the condensing.
        //
        // x0=(3,2.3) is SoftRowUnavoidableMinimalViolation's point, reused for its hand-derived slack:
        // 0.6 at a wall of 5, and ~0 at a wall of 9 (which the trajectory never reaches). The third
        // state, left at 9, is the non-vacuity probe -- without it the agreement above would also hold
        // if setSoftBound did nothing at all.
        void SetSoftBoundMatchesRebuild()
        {
            var A = Mat2(1, 1, 0, 1); var B = ColVec2(0, 1); var Q = Eye(2); var R = R1(1);
            var uLo = Vec1((float)(-2)); var uHi = Vec1((float)2);
            var C = WallRow();
            var dNear = Vec1((float)5); var dFar = Vec1((float)9);

            var built = new floatMPCState(2, 1, 8, Allocator.Temp, in A, in B, in Q, in R, in uLo, in uHi, in C, in dNear);
            var moved = new floatMPCState(2, 1, 8, Allocator.Temp, in A, in B, in Q, in R, in uLo, in uHi, in C, in dFar);
            var stale = new floatMPCState(2, 1, 8, Allocator.Temp, in A, in B, in Q, in R, in uLo, in uHi, in C, in dFar);
            MPC.setSoftBound(ref moved, in dNear);

            var x0 = Vec2(3, (float)2.3);
            var reference = new floatN(2, Allocator.Temp);   // zero-initialized: track to the origin
            var u0Built = new floatN(1, Allocator.Temp, true);
            var u0Moved = new floatN(1, Allocator.Temp, true);
            var u0Stale = new floatN(1, Allocator.Temp, true);

            var infoBuilt = MPC.solve(ref built, in x0, in reference, ref u0Built);
            var infoMoved = MPC.solve(ref moved, in x0, in reference, ref u0Moved);
            var infoStale = MPC.solve(ref stale, in x0, in reference, ref u0Stale);
            AssertTrue(infoBuilt.status == MPCStatus.Optimal || infoBuilt.status == MPCStatus.MaxIterations);
            AssertTrue(infoMoved.status == MPCStatus.Optimal || infoMoved.status == MPCStatus.MaxIterations);
            AssertTrue(infoStale.status == MPCStatus.Optimal || infoStale.status == MPCStatus.MaxIterations);

            AssertTrue(u0Moved[0] == u0Built[0]);
            AssertTrue(infoMoved.maxSlackViolation == infoBuilt.maxSlackViolation);
            AssertCloseD(infoMoved.maxSlackViolation, 0.6, SlackTol());

            // Non-vacuity: the wall this test moved away from must genuinely be a different problem.
            AssertLEd(infoStale.maxSlackViolation, SlackTol());

            x0.Dispose(); reference.Dispose(); u0Built.Dispose(); u0Moved.Dispose(); u0Stale.Dispose();
            built.Dispose(); moved.Dispose(); stale.Dispose(); C.Dispose(); dNear.Dispose(); dFar.Dispose();
            A.Dispose(); B.Dispose(); Q.Dispose(); R.Dispose(); uLo.Dispose(); uHi.Dispose();
        }

        // The per-stage form addresses one stage at a time, which the CONSTRUCTOR cannot express (its d
        // is one bound per row, shared by every stage). At x0=(3,2.3) a wall of 5 forces 0.3 of slack at
        // stage 0 and 0.6 at stage 1 (both hand-derived in SoftRowUnavoidableMinimalViolation), so
        // walling stage 0 alone and leaving the rest at 9 must report 0.3 -- a value NO constant bound
        // can produce, since 5 everywhere gives 0.6 and 9 everywhere gives 0.
        void SetSoftBoundPerStageBindsPerStage()
        {
            var A = Mat2(1, 1, 0, 1); var B = ColVec2(0, 1); var Q = Eye(2); var R = R1(1);
            var uLo = Vec1((float)(-2)); var uHi = Vec1((float)2);
            var C = WallRow(); var dFar = Vec1((float)9);
            var mpc = new floatMPCState(2, 1, 8, Allocator.Temp, in A, in B, in Q, in R, in uLo, in uHi, in C, in dFar);

            var x0 = Vec2(3, (float)2.3);
            var reference = new floatN(2, Allocator.Temp);
            var u0 = new floatN(1, Allocator.Temp, true);

            // A per-stage array whose entries are all equal must reproduce the held form exactly.
            var flat = new floatN(8, Allocator.Temp, true);
            for (int k = 0; k < 8; k++) flat[k] = (float)5;
            MPC.setSoftBound(ref mpc, in flat);
            var infoFlat = MPC.solve(ref mpc, in x0, in reference, ref u0);
            AssertTrue(infoFlat.status == MPCStatus.Optimal || infoFlat.status == MPCStatus.MaxIterations);
            AssertCloseD(infoFlat.maxSlackViolation, 0.6, SlackTol());

            var staged = new floatN(8, Allocator.Temp, true);
            staged[0] = (float)5;
            for (int k = 1; k < 8; k++) staged[k] = (float)9;
            MPC.setSoftBound(ref mpc, in staged);
            var infoStaged = MPC.solve(ref mpc, in x0, in reference, ref u0);
            AssertTrue(infoStaged.status == MPCStatus.Optimal || infoStaged.status == MPCStatus.MaxIterations);
            AssertCloseD(infoStaged.maxSlackViolation, 0.3, SlackTol());

            flat.Dispose(); staged.Dispose();
            x0.Dispose(); reference.Dispose(); u0.Dispose(); mpc.Dispose(); C.Dispose(); dFar.Dispose();
            A.Dispose(); B.Dispose(); Q.Dispose(); R.Dispose(); uLo.Dispose(); uHi.Dispose();
        }

        // A wall that moves every frame must not corrupt the carried factorization: the warm solve has
        // to land on the true minimizer of the QP it was actually handed. Same oracle as
        // WarmPersistentMatchesColdEachFrame -- cold-solve the identical condensed QP still sitting in
        // H/cScratch/Arows/bScratch -- but with the bound closing in on the vehicle each frame, so the
        // working set is forced to move on ticks the reuse path would otherwise skip.
        void SetSoftBoundSurvivesWarmStart()
        {
            var A = Mat2(1, 1, 0, 1); var B = ColVec2(0, 1); var Q = Eye(2); var R = R1(1);
            var uLo = Vec1((float)(-0.5)); var uHi = Vec1((float)0.5);
            var C = WallRow(); var d0 = Vec1((float)12);
            var mpc = new floatMPCState(2, 1, 8, Allocator.Temp, in A, in B, in Q, in R, in uLo, in uHi, in C, in d0);

            // Drive TOWARD the wall (setpoint at 10, starting at rest at the origin) while the wall
            // sweeps in at 1.0 per frame against an input bound of 0.5 -- so it necessarily overtakes
            // the vehicle rather than being outrun, and the soft rows are forced through activation,
            // saturation and release over one run.
            var x = Vec2(0, 0);
            var reference = Vec2(10, 0);
            var u0 = new floatN(1, Allocator.Temp, true);
            var wall = Vec1((float)12);

            int nz = mpc.nz;
            var xCold = new floatN(nz, Allocator.Temp, true);
            // RELATIVE, unlike WarmPersistentMatchesColdEachFrame's absolute bound: that run tracks to
            // the origin and its objective is O(1), whereas this one drives to a setpoint of 10 against
            // a barrier and reaches ~1.1e4, where an absolute 3e-3 is below the float noise floor.
            double objRelTol = 5e-6;
            float zTol = 5e-3f;

            bool everForced = false, everMoved = false;
            for (int f = 0; f < 16; f++)
            {
                wall[0] = (float)12 - (float)f;
                MPC.setSoftBound(ref mpc, in wall);

                var info = MPC.solve(ref mpc, in x, in reference, ref u0);
                AssertTrue(info.status == MPCStatus.Optimal || info.status == MPCStatus.MaxIterations);

                for (int i = 0; i < nz; i++) xCold[i] = (float)0;
                var coldInfo = QP.solve(in mpc.H, in mpc.cScratch, in mpc.Arows, in mpc.bScratch, in mpc.senses,
                                        in mpc.xl, in mpc.xu, ref xCold, out double objCold, 0);
                AssertTrue(coldInfo.status == QPStatus.Optimal);
                AssertRelCloseD(info.objective, objCold, objRelTol);
                for (int i = 0; i < nz; i++) AssertClose(mpc.z[i], xCold[i], zTol);

                if (info.maxSlackViolation > SlackTol()) everForced = true;
                if (info.activeSetChanges > 0) everMoved = true;

                float x0n = x[0] + x[1];
                float x1n = x[1] + u0[0];
                x[0] = x0n; x[1] = x1n;
            }

            // Non-vacuity, two ways. The working set must MOVE at some frame -- otherwise the run only
            // ever exercised the steady-state reuse path and says nothing about a rebuild under a
            // changed RHS. And the wall must at some point overtake the vehicle outright: an exact
            // penalty drives the violation to zero exactly when the constraint is doing its job, so a
            // nonzero slack is evidence the bound went somewhere no control could follow.
            AssertTrue(everMoved);
            AssertTrue(everForced);

            xCold.Dispose(); wall.Dispose();
            x.Dispose(); reference.Dispose(); u0.Dispose(); mpc.Dispose(); C.Dispose(); d0.Dispose();
            A.Dispose(); B.Dispose(); Q.Dispose(); R.Dispose(); uLo.Dispose(); uHi.Dispose();
        }

        // ---- Fail[0..3] diagnostic asserts (same shape as ControlLQRTests.float.cs) ----
        void AssertTrue(bool cond)
        {
            if (!cond && Fail[0] == (float)0) { Fail[0] = (float)1; Fail[1] = (float)0; Fail[2] = (float)1; Fail[3] = (float)0; }
            Assert.IsTrue(cond);
        }

        void AssertLE(int got, int limit)
        {
            if (!(got <= limit) && Fail[0] == (float)0) { Fail[0] = (float)1; Fail[1] = (float)got; Fail[2] = (float)limit; Fail[3] = (float)(got - limit); }
            Assert.IsTrue(got <= limit);
        }

        void AssertLEd(double got, double limit)
        {
            if (!(got <= limit) && Fail[0] == (float)0) { Fail[0] = (float)1; Fail[1] = (float)got; Fail[2] = (float)limit; Fail[3] = (float)(got - limit); }
            Assert.IsTrue(got <= limit);
        }

        void AssertClose(float a, float b, float precision)
        {
            float diff = math.abs(a - b);
            if (!(diff <= precision) && Fail[0] == (float)0) { Fail[0] = (float)1; Fail[1] = a; Fail[2] = b; Fail[3] = diff; }
            Assert.IsTrue(diff <= precision);
        }

        // Symmetric relative difference, 2|a-b|/(|a|+|b|). Scale-free by construction: no constant
        // floor, so it cannot silently become an absolute test on small values the way max(x, 1) does.
        // Both exactly zero is agreement.
        void AssertRelCloseD(double a, double b, double relTol)
        {
            double denom = math.abs(a) + math.abs(b);
            double rel = denom > 0 ? 2 * math.abs(a - b) / denom : 0;
            if (!(rel <= relTol) && Fail[0] == (float)0) { Fail[0] = (float)1; Fail[1] = (float)a; Fail[2] = (float)b; Fail[3] = (float)rel; }
            Assert.IsTrue(rel <= relTol);
        }

        void AssertCloseD(double a, double b, double precision)
        {
            double diff = math.abs(a - b);
            if (!(diff <= precision) && Fail[0] == (float)0) { Fail[0] = (float)1; Fail[1] = (float)a; Fail[2] = (float)b; Fail[3] = (float)diff; }
            Assert.IsTrue(diff <= precision);
        }
    }

    public static Array GetEnums() => Enum.GetValues(typeof(TestJob.TestType));

    [TestCaseSource("GetEnums")]
    public void MPCTests(TestJob.TestType type)
    {
        var fail = new NativeArray<float>(4, Allocator.TempJob);
        try
        {
            new TestJob() { Type = type, Fail = fail }.Run();
            if (fail[0] != (float)0)
                Assert.Fail($"got {fail[1]}, expected/limit {fail[2]}, diff/extra {fail[3]}");
        }
        catch (Exception e)
        {
            if (fail[0] != (float)0)
                Assert.Fail($"{type}: got {fail[1]}, expected/limit {fail[2]}, diff/extra {fail[3]} ({e.Message})");
            throw;
        }
        finally
        {
            fail.Dispose();
        }
    }

    // ---- managed-thread validation throws + disposal (not Burst-job-friendly: constructor exceptions
    // and repeated Dispose() are awkward inside a single IJob.Execute) ----

    [Test]
    public void SolveThrowsOnMismatchedX0()
    {
        var A = new floatMxN(2, 2, Allocator.Temp); A[0, 0] = 1; A[0, 1] = 1; A[1, 1] = 1;
        var B = new floatMxN(2, 1, Allocator.Temp); B[1, 0] = 1;
        var Q = new floatMxN(2, 2, Allocator.Temp); Q[0, 0] = 1; Q[1, 1] = 1;
        var R = new floatMxN(1, 1, Allocator.Temp); R[0, 0] = 1;
        var uLo = new floatN(1, Allocator.Temp, true); uLo[0] = (float)(-1);
        var uHi = new floatN(1, Allocator.Temp, true); uHi[0] = (float)1;
        var mpc = new floatMPCState(2, 1, 4, Allocator.Temp, in A, in B, in Q, in R, in uLo, in uHi);

        var badX0 = new floatN(3, Allocator.Temp, true);
        var reference = new floatN(2, Allocator.Temp);   // zero-initialized: track to the origin
        var u0 = new floatN(1, Allocator.Temp, true);
        Assert.Catch<ArgumentException>(() => MPC.solve(ref mpc, in badX0, in reference, ref u0));

        badX0.Dispose(); reference.Dispose(); u0.Dispose(); mpc.Dispose();
        A.Dispose(); B.Dispose(); Q.Dispose(); R.Dispose(); uLo.Dispose(); uHi.Dispose();
    }

    [Test]
    public void SolveThrowsOnBadReferenceLength()
    {
        var A = new floatMxN(2, 2, Allocator.Temp); A[0, 0] = 1; A[0, 1] = 1; A[1, 1] = 1;
        var B = new floatMxN(2, 1, Allocator.Temp); B[1, 0] = 1;
        var Q = new floatMxN(2, 2, Allocator.Temp); Q[0, 0] = 1; Q[1, 1] = 1;
        var R = new floatMxN(1, 1, Allocator.Temp); R[0, 0] = 1;
        var uLo = new floatN(1, Allocator.Temp, true); uLo[0] = (float)(-1);
        var uHi = new floatN(1, Allocator.Temp, true); uHi[0] = (float)1;
        var mpc = new floatMPCState(2, 1, 4, Allocator.Temp, in A, in B, in Q, in R, in uLo, in uHi);

        var x0 = new floatN(2, Allocator.Temp, true);
        var badReference = new floatN(3, Allocator.Temp, true);   // neither n(2) nor N*n(8)
        var u0 = new floatN(1, Allocator.Temp, true);
        Assert.Catch<ArgumentException>(() => MPC.solve(ref mpc, in x0, in badReference, ref u0));

        x0.Dispose(); badReference.Dispose(); u0.Dispose(); mpc.Dispose();
        A.Dispose(); B.Dispose(); Q.Dispose(); R.Dispose(); uLo.Dispose(); uHi.Dispose();
    }

    [Test]
    public void SetSoftBoundThrowsOnBadInput()
    {
        var A = new floatMxN(2, 2, Allocator.Temp); A[0, 0] = 1; A[0, 1] = 1; A[1, 1] = 1;
        var B = new floatMxN(2, 1, Allocator.Temp); B[1, 0] = 1;
        var Q = new floatMxN(2, 2, Allocator.Temp); Q[0, 0] = 1; Q[1, 1] = 1;
        var R = new floatMxN(1, 1, Allocator.Temp); R[0, 0] = 1;
        var uLo = new floatN(1, Allocator.Temp, true); uLo[0] = (float)(-1);
        var uHi = new floatN(1, Allocator.Temp, true); uHi[0] = (float)1;
        var C = new floatMxN(1, 2, Allocator.Temp); C[0, 0] = 1;
        var d = new floatN(1, Allocator.Temp, true); d[0] = (float)5;

        var plain = new floatMPCState(2, 1, 4, Allocator.Temp, in A, in B, in Q, in R, in uLo, in uHi);
        var soft = new floatMPCState(2, 1, 4, Allocator.Temp, in A, in B, in Q, in R, in uLo, in uHi, in C, in d);

        var ok = new floatN(1, Allocator.Temp, true); ok[0] = (float)3;
        var badLen = new floatN(3, Allocator.Temp, true);           // neither k(1) nor N*k(4)
        var nan = new floatN(1, Allocator.Temp, true); nan[0] = (float)double.NaN;

        Assert.Catch<ArgumentException>(() => MPC.setSoftBound(ref plain, in ok));      // no soft rows
        Assert.Catch<ArgumentException>(() => MPC.setSoftBound(ref soft, in badLen));
        Assert.Catch<ArgumentException>(() => MPC.setSoftBound(ref soft, in nan));

        ok.Dispose(); badLen.Dispose(); nan.Dispose();
        plain.Dispose(); soft.Dispose(); C.Dispose(); d.Dispose();
        A.Dispose(); B.Dispose(); Q.Dispose(); R.Dispose(); uLo.Dispose(); uHi.Dispose();
    }

    [Test]
    public void ConstructorThrowsOnUnstabilizablePair()
    {
        // A's mode with eigenvalue 2 is uncontrollable (B only actuates the second state), mirroring
        // ControlLQRTests.float.cs's own FailureUnstabilizable job -- the auto-DARE terminal cost
        // cannot be computed, so construction must fail rather than silently produce a bad terminal cost.
        var A = new floatMxN(2, 2, Allocator.Temp); A[0, 0] = 2; A[1, 1] = (float)0.5;
        var B = new floatMxN(2, 1, Allocator.Temp); B[1, 0] = 1;
        var Q = new floatMxN(2, 2, Allocator.Temp); Q[0, 0] = 1; Q[1, 1] = 1;
        var R = new floatMxN(1, 1, Allocator.Temp); R[0, 0] = 1;
        var uLo = new floatN(1, Allocator.Temp, true); uLo[0] = (float)(-1);
        var uHi = new floatN(1, Allocator.Temp, true); uHi[0] = (float)1;

        Assert.Catch<ArgumentException>(() => new floatMPCState(2, 1, 4, Allocator.Temp, in A, in B, in Q, in R, in uLo, in uHi));

        A.Dispose(); B.Dispose(); Q.Dispose(); R.Dispose(); uLo.Dispose(); uHi.Dispose();
    }

    [Test]
    public void DisposeIsIdempotent()
    {
        var A = new floatMxN(2, 2, Allocator.Temp); A[0, 0] = 1; A[0, 1] = 1; A[1, 1] = 1;
        var B = new floatMxN(2, 1, Allocator.Temp); B[1, 0] = 1;
        var Q = new floatMxN(2, 2, Allocator.Temp); Q[0, 0] = 1; Q[1, 1] = 1;
        var R = new floatMxN(1, 1, Allocator.Temp); R[0, 0] = 1;
        var uLo = new floatN(1, Allocator.Temp, true); uLo[0] = (float)(-1);
        var uHi = new floatN(1, Allocator.Temp, true); uHi[0] = (float)1;
        var mpc = new floatMPCState(2, 1, 4, Allocator.Temp, in A, in B, in Q, in R, in uLo, in uHi);

        Assert.IsTrue(mpc.IsCreated);
        mpc.Dispose();
        Assert.DoesNotThrow(() => mpc.Dispose());

        A.Dispose(); B.Dispose(); Q.Dispose(); R.Dispose(); uLo.Dispose(); uHi.Dispose();
    }

    // ---- Job-struct-copy regression: floatMPCState.populated must survive a plain .Run() by-value
    // copy -- same bug class as floatLQRState.populated / LPBasis.populated (see DemoSmokeTests.cs's
    // LqrWarmState_SurvivesRunByValueCopy, the established pattern this mirrors). `mpc` is the state
    // held as a field on PopulatedRunJob; the assertion below reads it back through `mpc` itself --
    // the ORIGINAL local that was value-copied INTO job.S at construction and never written to
    // directly again -- so a pass can only be explained by populated living behind native
    // (pointer-shared) storage, not by any incidental round-trip of the job struct as a whole.
    [BurstCompile(CompileSynchronously = true, FloatPrecision = FloatPrecision.High, FloatMode = FloatMode.Default)]
    public struct PopulatedRunJob : IJob
    {
        public floatMPCState S;
        public floatN X0;
        public floatN Reference;
        public floatN U0;
        public NativeArray<int> Out;   // [0] = 1 if the solve reached Optimal/MaxIterations

        public void Execute()
        {
            var info = MPC.solve(ref S, in X0, in Reference, ref U0);
            Out[0] = (info.status == MPCStatus.Optimal || info.status == MPCStatus.MaxIterations) ? 1 : 0;
        }
    }

    // Smallest valid MPC problem already used by this file's own managed throw tests above
    // (SolveThrowsOnMismatchedX0 / ConstructorThrowsOnUnstabilizablePair / DisposeIsIdempotent):
    // n=2, m=1, N=4, double integrator, input bound [-1,1].
    [Test]
    public void MPCState_Populated_SurvivesRunByValueCopy()
    {
        var A = new floatMxN(2, 2, Allocator.TempJob); A[0, 0] = 1; A[0, 1] = 1; A[1, 1] = 1;
        var B = new floatMxN(2, 1, Allocator.TempJob); B[1, 0] = 1;
        var Q = new floatMxN(2, 2, Allocator.TempJob); Q[0, 0] = 1; Q[1, 1] = 1;
        var R = new floatMxN(1, 1, Allocator.TempJob); R[0, 0] = 1;
        var uLo = new floatN(1, Allocator.TempJob, true); uLo[0] = (float)(-1);
        var uHi = new floatN(1, Allocator.TempJob, true); uHi[0] = (float)1;
        var mpc = new floatMPCState(2, 1, 4, Allocator.TempJob, in A, in B, in Q, in R, in uLo, in uHi);

        var x0 = new floatN(2, Allocator.TempJob, true);
        var reference = new floatN(2, Allocator.TempJob);   // zero-initialized: track to the origin
        var u0 = new floatN(1, Allocator.TempJob, true);
        var outFlag = new NativeArray<int>(1, Allocator.TempJob);

        var job = new PopulatedRunJob { S = mpc, X0 = x0, Reference = reference, U0 = u0, Out = outFlag };

        Assert.IsFalse(mpc.populated, "fresh MPC state should not be populated");
        job.Run();   // plain .Run() -> Execute runs on a by-value copy of `job`

        Assert.IsTrue(outFlag[0] == 1, "cold MPC solve did not reach Optimal/MaxIterations");
        Assert.IsTrue(mpc.populated,
            "floatMPCState.populated did not survive the .Run() by-value copy -- a native-backed flag is what makes this pass; a plain bool would be lost");

        x0.Dispose(); reference.Dispose(); u0.Dispose(); outFlag.Dispose();
        mpc.Dispose();
        A.Dispose(); B.Dispose(); Q.Dispose(); R.Dispose(); uLo.Dispose(); uHi.Dispose();
    }
}
