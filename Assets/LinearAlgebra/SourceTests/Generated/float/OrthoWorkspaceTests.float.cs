...
public class floatOrthoWorkspaceTests
{
    ...
    public struct WorkspaceEquivJob : IJob
    {
        ...
        void DecompEquiv(int M, int N)
        {
            ...
            var A = arena.floatRandomMatrix(M, N, -1f, 1f, 73101);
        ...
        void SolveQRSolve(int M, int N)
        {
            ...
            var A = arena.floatRandomMatrix(M, N, -1f, 1f, 33417);
            for (int d = 0; d < N; d++)   // well-conditioned columns
                A[d, d] += 5f;

            var xOrig = arena.floatRandomVector(N, -3f, 3f, 60221);
            var b = float_OP.dot(A, xOrig);   // consistent RHS; read-only in SolveQR, reusable
