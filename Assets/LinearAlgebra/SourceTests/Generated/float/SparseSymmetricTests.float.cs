...
// Symmetric lower-block-triangle storage test suite for floatBSR (BSR). Milestone A: prove that
// a matrix stored as its lower block-triangle ONLY (floatBSR.Symmetric == true, built via
...
public class floatSparseSymmetricTests
{
    ...
    public struct SparseSymmetricTestJob : IJob
    {
        ...
        static void AddDiag(int i, int BR, float strong, uint seed,
                            floatBSRBuilder full, floatBSRBuilder sym, ref floatMxN dense)
        {
            var Mi = GenerateOP.floatRandomMat(BR, BR, (float)(-1f), (float)1f, seed);
        ...
        static void AddOffDiag(int bi, int bj, int BR, uint seed,
                               floatBSRBuilder full, floatBSRBuilder sym, ref floatMxN dense)
        {
            var block = GenerateOP.floatRandomMat(BR, BR, (float)(-0.2f), (float)0.2f, seed);

            full.AddBlock(bi, bj, in block);

            var blockT = new floatMxN(BR, BR, Allocator.Temp);
            for (int r = 0; r < BR; r++)
                for (int c = 0; c < BR; c++)
        ...
        static void CrossCheck(in floatBSR sym, in floatBSR full,
                               in floatMxN dense, uint seedBase)
        {
            ...
            // ---- spMV direction ----
            var x = GenerateOP.floatRandomVec(n, (float)(-1f), (float)1f, seedBase);
            ...
            var xt = GenerateOP.floatRandomVec(n, (float)(-1f), (float)1f, seedBase + 1u);
        ...
        static void BuildSpdPair_BR1_Sparse(out floatBSR sym, out floatBSR full, out floatMxN dense)
        {
            const int BR = 1, nb = 5;
            int dim = BR * nb;
            float strong = (float)dim;
            dense = new floatMxN(dim, dim, Allocator.Temp);

            var f = new floatBSRBuilder(nb, nb, BR, BR, Allocator.Temp, nb + 2 * 2);
        ...
        static void BuildSpdPair_BR1_Dense(out floatBSR sym, out floatBSR full, out floatMxN dense)
        {
            const int BR = 1, nb = 5;
            int dim = BR * nb;
            float strong = (float)dim;
            ...
            var f = new floatBSRBuilder(nb, nb, BR, BR, Allocator.Temp, nb + 2 * 10);
            var s = new floatBSRBuilder(nb, nb, BR, BR, Allocator.Temp, nb + 10);
        ...
        static void BuildSpdPair_BR3_Sparse(out floatBSR sym, out floatBSR full, out floatMxN dense)
        {
            const int BR = 3, nb = 4;
            int dim = BR * nb;
            float strong = (float)dim;
            ...
            var f = new floatBSRBuilder(nb, nb, BR, BR, Allocator.Temp, nb + 2 * 3);
            var s = new floatBSRBuilder(nb, nb, BR, BR, Allocator.Temp, nb + 3);
        ...
        static void BuildSpdPair_BR3_Dense(out floatBSR sym, out floatBSR full, out floatMxN dense)
        {
            const int BR = 3, nb = 4;
            int dim = BR * nb;
            float strong = (float)dim;
            ...
            var f = new floatBSRBuilder(nb, nb, BR, BR, Allocator.Temp, nb + 2 * 6);
            var s = new floatBSRBuilder(nb, nb, BR, BR, Allocator.Temp, nb + 6);
        ...
        void CrossCheck_DiagonalOnly()
        {
            ...
            int dim = BR * nb;
            float strong = (float)dim;
            var dense = new floatMxN(dim, dim, Allocator.Temp);

            var f = new floatBSRBuilder(nb, nb, BR, BR, Allocator.Temp, nb);
            var s = new floatBSRBuilder(nb, nb, BR, BR, Allocator.Temp, nb);
        ...
        void CrossCheck_Empty()
        {
            ...
            int dim = BR * nb;
            var builder = new floatBSRBuilder(nb, nb, BR, BR, Allocator.Temp); // never AddBlock/AddValue
            ...
            var zero = new floatMxN(dim, dim, Allocator.Temp);
            AssertMatEq(in dense, in zero, Tol());

            // spMV of an empty BSR == the zero vector, for a random nonzero x.
            var x = GenerateOP.floatRandomVec(dim, (float)(-1f), (float)1f, 42000u);
            ...
            var xt = GenerateOP.floatRandomVec(dim, (float)(-1f), (float)1f, 42001u);
        ...
        void CgSymMatchesFull()
        {
            BuildSpdPair_BR3_Dense(out var sym, out var full, out var dense);
            int dim = sym.M_Rows;
            var b = GenerateOP.floatRandomVec(dim, (float)(-1f), (float)1f, 50000u);

            var xSym = new floatN(dim, Allocator.Temp);
            bool okSym = Krylov.cg(in sym, in b, ref xSym, 4 * dim, Consts.floatSqrtEps);
            Assert.IsTrue(okSym);

            var xFull = new floatN(dim, Allocator.Temp);
        ...
        void MinresSymMatchesFull()
        {
            BuildSpdPair_BR3_Dense(out var sym, out var full, out var dense);
            int dim = sym.M_Rows;
            var b = GenerateOP.floatRandomVec(dim, (float)(-1f), (float)1f, 51000u);
        ...
        void PcgBlockJacobiSymMatchesFull()
        {
            BuildSpdPair_BR3_Dense(out var sym, out var full, out var dense);
            int dim = sym.M_Rows;
            var b = GenerateOP.floatRandomVec(dim, (float)(-1f), (float)1f, 52000u);

            // Both preconditioners must construct without throwing (diagonal blocks ARE present in
            // symmetric lower storage).
            var mSym  = new floatBlockJacobi(in sym, Allocator.Temp);
            var mFull = new floatBlockJacobi(in full, Allocator.Temp);

            var xPcgSym = new floatN(dim, Allocator.Temp);
            bool okSym = Krylov.cg(in sym, in mSym, in b, ref xPcgSym, 4 * dim, Consts.floatSqrtEps);
            Assert.IsTrue(okSym);

            var xPcgFull = new floatN(dim, Allocator.Temp);
    ...
    public void ToBSRSymmetric_UpperTriangleTriplet_Throws()
    {
        ...
        builder.AddBlock(0, 1, GenerateOP.floatRandomMat(BR, BR, (float)(-1f), (float)1f, 60001)); // bc > br
    ...
    public void ToBSRSymmetric_NonSymmetricDiagonalBlock_Throws()
    {
        ...
        var builder = new floatBSRBuilder(1, 1, BR, BR, Allocator.Temp, 1);
    ...
    {
        var builder = new floatBSRBuilder(2, 2, 3, 2, Allocator.Temp); // BR=3 != BC=2, but square block grid
        Assert.Throws<ArgumentException>(() => builder.ToBSRSymmetric(Allocator.Temp));
    }
    ...
    {
        var builder = new floatBSRBuilder(2, 3, 3, 3, Allocator.Temp); // BlockRows=2 != BlockCols=3, BR==BC
        Assert.Throws<ArgumentException>(() => builder.ToBSRSymmetric(Allocator.Temp));
    }
    ...
    public void Ctor_SymmetricNonSquareBlock_Throws()
    {
        ...
        Assert.Throws<ArgumentException>(() => new floatBSR(2, 2, 3, 2, 1, Allocator.Temp, symmetric: true));
    }
    ...
    public void Ctor_SymmetricNonSquareGrid_Throws()
    {
        ...
        Assert.Throws<ArgumentException>(() => new floatBSR(2, 3, 3, 3, 1, Allocator.Temp, symmetric: true));
    }
}
