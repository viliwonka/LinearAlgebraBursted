...
// Symmetric upper-block-triangle storage test suite for floatBSR (BSR). Milestone A: prove that
// a matrix stored as its upper block-triangle ONLY (floatBSR.Symmetric == true, built via
// floatBSRBuilder.ToBSRSymmetric) behaves identically to the SAME matrix stored FULLY (every
...
//
// The correctness cases run inside a [BurstCompile] IJob (same pattern as floatSparseBSRTests /
...
public class floatSparseSymmetricTests
{
    ...
    public struct SparseSymmetricTestJob : IJob
    {
        ...
        // needs the looser bound, double is far tighter). Matches floatSparseBSRTests.Tol().
        ...
        static void CrossCheck(ref Arena arena, in floatBSR sym, in floatBSR full,
        ...
        static void BuildSpdPair_BR1_Sparse(ref Arena arena, out floatBSR sym, out floatBSR full, out floatMxN dense)
        {
            const int BR = 1, nb = 5;
            int dim = BR * nb;
            ...
            var f = arena.floatBSRBuilder(nb, nb, BR, BR, nb + 2 * 2);
            var s = arena.floatBSRBuilder(nb, nb, BR, BR, nb + 2);
        ...
        static void BuildSpdPair_BR1_Dense(ref Arena arena, out floatBSR sym, out floatBSR full, out floatMxN dense)
        {
            const int BR = 1, nb = 5;
            int dim = BR * nb;
            float strong = (float)dim;
            ...
            var f = arena.floatBSRBuilder(nb, nb, BR, BR, nb + 2 * 10);
            var s = arena.floatBSRBuilder(nb, nb, BR, BR, nb + 10);
        ...
        static void BuildSpdPair_BR3_Sparse(ref Arena arena, out floatBSR sym, out floatBSR full, out floatMxN dense)
        {
            ...
            var f = arena.floatBSRBuilder(nb, nb, BR, BR, nb + 2 * 3);
            var s = arena.floatBSRBuilder(nb, nb, BR, BR, nb + 3);
        ...
        static void BuildSpdPair_BR3_Dense(ref Arena arena, out floatBSR sym, out floatBSR full, out floatMxN dense)
        {
            const int BR = 3, nb = 4;
            int dim = BR * nb;
            float strong = (float)dim;
            ...
            var f = arena.floatBSRBuilder(nb, nb, BR, BR, nb + 2 * 6);
            var s = arena.floatBSRBuilder(nb, nb, BR, BR, nb + 6);
        ...
        void CrossCheck_DiagonalOnly()
        {
            ...
            var f = arena.floatBSRBuilder(nb, nb, BR, BR, nb);
            var s = arena.floatBSRBuilder(nb, nb, BR, BR, nb);
        ...
        void CrossCheck_Empty()
        {
            ...
            int dim = BR * nb;
            var builder = arena.floatBSRBuilder(nb, nb, BR, BR); // never AddBlock/AddValue
    ...
    public void ToBSRSymmetric_NonSymmetricDiagonalBlock_Throws()
    {
        ...
        try
        {
            ...
            var builder = arena.floatBSRBuilder(1, 1, BR, BR, 1);
    ...
    public void ToBSRSymmetric_NonSquareBlock_Throws()
    {
        ...
        try
        {
            var builder = arena.floatBSRBuilder(2, 2, 3, 2); // BR=3 != BC=2, but square block grid
            Assert.Throws<ArgumentException>(() => builder.ToBSRSymmetric(ref arena));
        }
        finally { arena.Dispose(); }
    }
    ...
    public void ToBSRSymmetric_NonSquareGrid_Throws()
    {
        ...
        try
        {
            var builder = arena.floatBSRBuilder(2, 3, 3, 3); // BlockRows=2 != BlockCols=3, BR==BC
            Assert.Throws<ArgumentException>(() => builder.ToBSRSymmetric(ref arena));
        }
        finally { arena.Dispose(); }
    }

    // The floatBSR CONSTRUCTOR's OWN symmetric guard (a different code path than the builder's):
    ...
    public void Ctor_SymmetricNonSquareBlock_Throws()
    {
        ...
        try
        {
            ...
            Assert.Throws<ArgumentException>(() => arena.floatBSR(2, 2, 3, 2, 1, symmetric: true));
        }
        finally { arena.Dispose(); }
    }
    ...
    public void Ctor_SymmetricNonSquareGrid_Throws()
    {
        ...
        try
        {
            ...
            Assert.Throws<ArgumentException>(() => arena.floatBSR(2, 3, 3, 3, 1, symmetric: true));
        }
        finally { arena.Dispose(); }
    }
}
