// <auto-generated>
//   Generated from Assets/LinearAlgebra/CodeGen/TemplateSourceTests/fProxy/SVDRandomizedWorkspaceTests.fProxy.cs
...
// floatSVDRandomizedCache (the standalone (m, n, k, oversample, Allocator) ctor and the
...
public class floatSVDRandomizedWorkspaceTests
{
    ...
    public struct WorkspaceJob : IJob
    {
        ...
        static floatMxN RankR(int m, int n, int r, uint seed)
        {
            var B = GenerateOP.floatRandomMat(m, r, (float)(-2f), (float)2f, seed, allocator: Allocator.Temp);
            var C = GenerateOP.floatRandomMat(r, n, (float)(-2f), (float)2f, seed + 13u, allocator: Allocator.Temp);
            return Blas.dot(B, C);
        }
        ...
        void ExplicitArgsEquiv()
        {
            ...
            var ws = new floatSVDRandomizedCache(m, n, k, oversample, Allocator.Temp);
        ...
        void DefaultFactoryEquiv()
        {
            ...
            var ws = new floatSVDRandomizedCache(m, n, k, Allocator.Temp);   // default oversample 10
            var UkW = new floatMxN(m, k, Allocator.Temp); var SkW = new floatN(k, Allocator.Temp); var VkW = new floatMxN(n, k, Allocator.Temp);
        ...
        void WorkspaceReuse()
        {
            ...
            var ws = new floatSVDRandomizedCache(m, n, k, oversample, Allocator.Temp);   // ONCE
            ...
            var SDiff = new floatN(in SW, Allocator.Temp); SDiff.subInPlace(SA);
            Assert.IsTrue(Analysis.isZero(SDiff, Tol()));
            var UDiff = new floatMxN(in UW, Allocator.Temp); UDiff.subInPlace(UA);
            Assert.IsTrue(Analysis.isZero(UDiff, Tol()));
            var VDiff = new floatMxN(in VW, Allocator.Temp); VDiff.subInPlace(VA);
            Assert.IsTrue(Analysis.isZero(VDiff, Tol()));
        }
    }
    ...
    public void Randomized_BadWorkspaceM_Throws()
    {
        ...
        var Uk = new floatMxN(m, k, Allocator.Temp); var Sk = new floatN(k, Allocator.Temp); var Vk = new floatMxN(n, k, Allocator.Temp);
        var ws = new floatSVDRandomizedCache(m + 1, n, k, oversample, Allocator.Temp);   // wrong m
    ...
    public void Randomized_BadWorkspaceOversample_Throws()
    {
        ...
        var ws = new floatSVDRandomizedCache(m, n, k, 0, Allocator.Temp);
        Assert.Throws<ArgumentException>(
            () => SVD.randomized(in A, ref Uk, ref Sk, ref Vk, k, 2, 2, 123u, 75, ref ws));
    }

    // Standalone floatSVDRandomizedCache(m, n, k, oversample, allocator) sizes everything by l = min(k+p, n).
    [Test]
    public void SvdRandomizedWorkspace_Factory_SizesCorrectly()
    {
        ...
        var wsDef = new floatSVDRandomizedCache(m, n, k, Allocator.Temp);
