...
// the sparse op against the DENSE reference: build a floatBSR via the builder, expand with
...
public class floatSparseBSRTests
{
    [BurstCompile]
    public struct SparseBSRTestJob : IJob
    {
        ...
        static floatBSR BuildRandom(ref Arena arena)
        {
            const int BR = 3, BC = 3;
        ...
        void RectangularBlocks()
        {
            ...
            const int BR = 2, BC = 3;
            var builder = arena.floatBSRBuilder(2, 3, BR, BC); // 4 x 9 dense
        ...
        void DuplicateSummation()
        {
            ...
            const int BR = 2, BC = 2;
            var builder = arena.floatBSRBuilder(2, 2, BR, BC); // 4 x 4 dense
        ...
        void OutOfOrderTriplets()
        {
            ...
            const int BR = 2, BC = 2;
            var builder = arena.floatBSRBuilder(2, 3, BR, BC); // 4 x 6 dense
        ...
        void OneByOneBlocks()
        {
            ...
            var builder = arena.floatBSRBuilder(4, 4, 1, 1); // 4 x 4 scalar sparse
        ...
        static floatBSR BuildDenseGrown(ref Arena arena, in floatMxN refDense, out int tripletCount)
        {
            int N = refDense.M_Rows;
            var builder = arena.floatBSRBuilder(N, N, 1, 1); // DEFAULT capacityHint = 8
            for (int r = 0; r < N; r++)
                for (int c = 0; c < N; c++)
        ...
        void EmptyBSRRoundTrip()
        {
            ...
            var oneBuilder = arena.floatBSRBuilder(1, 1, 1, 1);
    ...
    //
    // floatBSR.ToDense and floatBSRBuilder.ToBSR used to take `in Arena arena`, but both call a
    // MUTATING arena allocator method internally (arena.floatMat / arena.floatBSR). Since those
    ...
    // Blas.trans(dense).floatTempMat -- dereferenced the dangling pointer and threw
    ...
    public void ToDense_TransposeReference_Works()
    {
        ...
        try
        {
            ...
            var y = arena.floatVec(A.N_Cols);
            BSR.spMVT(in A, in x, ref y);
            Assert.IsTrue(Analysis.isZero(y - yRef, (float)1e-4f));
        }
    ...
    public void AddBlock_BlockRowOutOfBounds_Throws()
    {
        ...
        try
        {
            var builder = arena.floatBSRBuilder(2, 2, 3, 3);
    ...
    public void AddValue_GlobalIndexOutOfBounds_Throws()
    {
        ...
        try
        {
            var builder = arena.floatBSRBuilder(2, 2, 3, 3); // 6 x 6
    ...
    static floatBSR BuildSquare(ref Arena arena)
    {
        const int BR = 2, BC = 2;
        var builder = arena.floatBSRBuilder(2, 2, BR, BC);
