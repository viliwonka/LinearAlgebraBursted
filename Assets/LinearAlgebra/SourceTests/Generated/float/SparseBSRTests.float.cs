// <auto-generated>
//   Generated from Assets/LinearAlgebra/CodeGen/TemplateSourceTests/fProxy/SparseBSRTests.fProxy.cs
...
public class floatSparseBSRTests
{
    ...
    public struct SparseBSRTestJob : IJob
    {
        ...
        void HandBuiltSmall()
        {
            ...
            var expected = new floatMxN(6, 6, Allocator.Temp);
        ...
        static floatBSR BuildRandom()
        {
            const int BR = 3, BC = 3;
            var builder = new floatBSRBuilder(3, 3, BR, BC, Allocator.Temp);
            builder.AddBlock(0, 0, GenerateOP.floatRandomMat(BR, BC, (float)(-1f), (float)1f, 1001));
            builder.AddBlock(0, 2, GenerateOP.floatRandomMat(BR, BC, (float)(-1f), (float)1f, 1002));
            builder.AddBlock(1, 1, GenerateOP.floatRandomMat(BR, BC, (float)(-1f), (float)1f, 1003));
            builder.AddBlock(2, 0, GenerateOP.floatRandomMat(BR, BC, (float)(-1f), (float)1f, 1004));
            builder.AddBlock(2, 2, GenerateOP.floatRandomMat(BR, BC, (float)(-1f), (float)1f, 1005));
        ...
        void RandomSpMV()
        {
            var A = BuildRandom();
            var dense = A.ToDense(Allocator.Temp);
            var x = GenerateOP.floatRandomVec(A.N_Cols, (float)(-1f), (float)1f, 7777);
        ...
        void RandomSpMVT()
        {
            var A = BuildRandom();
            var dense = A.ToDense(Allocator.Temp);
            var x = GenerateOP.floatRandomVec(A.M_Rows, (float)(-1f), (float)1f, 8888);

            var y = new floatN(A.N_Cols, Allocator.Temp);
            BSR.spMVT(in A, in x, ref y);

            var yRef = new floatN(A.N_Cols, Allocator.Temp);
        ...
        {
            const int BR = 2, BC = 3;
            var builder = new floatBSRBuilder(2, 3, BR, BC, Allocator.Temp); // 4 x 9 dense
            builder.AddBlock(0, 0, GenerateOP.floatRandomMat(BR, BC, (float)(-1f), (float)1f, 2001));
            builder.AddBlock(0, 2, GenerateOP.floatRandomMat(BR, BC, (float)(-1f), (float)1f, 2002));
            builder.AddBlock(1, 1, GenerateOP.floatRandomMat(BR, BC, (float)(-1f), (float)1f, 2003));
            var A = builder.ToBSR(Allocator.Temp);
            ...
            var x = GenerateOP.floatRandomVec(A.N_Cols, (float)(-1f), (float)1f, 2100);
            var y = new floatN(A.M_Rows, Allocator.Temp);
            BSR.spMV(in A, in x, ref y);
            var diff = new floatN(in y, Allocator.Temp);
            ...
            var xt = GenerateOP.floatRandomVec(A.M_Rows, (float)(-1f), (float)1f, 2200);
        ...
        {
            const int BR = 2, BC = 2;
            var builder = new floatBSRBuilder(2, 2, BR, BC, Allocator.Temp); // 4 x 4 dense

            var blkA = GenerateOP.floatRandomMat(BR, BC, (float)(-1f), (float)1f, 3001);
            var blkB = GenerateOP.floatRandomMat(BR, BC, (float)(-1f), (float)1f, 3002);
            ...
            var expected = new floatMxN(4, 4, Allocator.Temp);
            var sum = new floatMxN(BR, BC, Allocator.Temp);
            for (int r = 0; r < BR; r++)
                for (int c = 0; c < BC; c++)
        ...
        {
            const int BR = 2, BC = 2;
            var builder = new floatBSRBuilder(2, 3, BR, BC, Allocator.Temp); // 4 x 6 dense

            // Scrambled (br,bc) insertion order across both rows.
            var b_1_2 = GenerateOP.floatRandomMat(BR, BC, (float)(-1f), (float)1f, 4001);
            var b_0_2 = GenerateOP.floatRandomMat(BR, BC, (float)(-1f), (float)1f, 4002);
            var b_1_0 = GenerateOP.floatRandomMat(BR, BC, (float)(-1f), (float)1f, 4003);
            var b_0_0 = GenerateOP.floatRandomMat(BR, BC, (float)(-1f), (float)1f, 4004);
            var b_0_1 = GenerateOP.floatRandomMat(BR, BC, (float)(-1f), (float)1f, 4005);
            ...
            var expected = new floatMxN(4, 6, Allocator.Temp);
        ...
        void OneByOneBlocks()
        {
            var builder = new floatBSRBuilder(4, 4, 1, 1, Allocator.Temp); // 4 x 4 scalar sparse
            ...
            var x = GenerateOP.floatRandomVec(4, (float)(-1f), (float)1f, 5050);
            var y = new floatN(4, Allocator.Temp);
            BSR.spMV(in A, in x, ref y);
            var diff = new floatN(in y, Allocator.Temp);
            floatComp.subInPlace(diff, Blas.dot(expected, x));
            Assert.IsTrue(Analysis.isZero(diff, Tol()));
        }
        ...
        static floatBSR BuildDenseGrown(in floatMxN refDense, out int tripletCount)
        {
            int N = refDense.M_Rows;
            var builder = new floatBSRBuilder(N, N, 1, 1, Allocator.Temp); // DEFAULT capacityHint = 8
            for (int r = 0; r < N; r++)
                for (int c = 0; c < N; c++)
        ...
        {
            const int N = 15;
            var refDense = GenerateOP.floatRandomMat(N, N, (float)(-1f), (float)1f, 74101);
            ...
            var x = GenerateOP.floatRandomVec(N, (float)(-1f), (float)1f, 74202);
            var y = new floatN(N, Allocator.Temp);
            BSR.spMV(in A, in x, ref y);
            var diff = new floatN(in y, Allocator.Temp);
            floatComp.subInPlace(diff, Blas.dot(refDense, x));
            Assert.IsTrue(Analysis.isZero(diff, Tol()));
        }
        ...
        void ClearThenReallocate()
        {
            ...
            // First grown builder.
            var refA = GenerateOP.floatRandomMat(N, N, (float)(-1f), (float)1f, 75101);
            ...
            var refB = GenerateOP.floatRandomMat(N, N, (float)(-1f), (float)1f, 75202);
            ...
            var x = GenerateOP.floatRandomVec(N, (float)(-1f), (float)1f, 75303);
            var y = new floatN(N, Allocator.Temp);
            BSR.spMV(in B, in x, ref y);
            var diff = new floatN(in y, Allocator.Temp);
            floatComp.subInPlace(diff, Blas.dot(refB, x));
            Assert.IsTrue(Analysis.isZero(diff, Tol()));
        }
        ...
        // codebase's established zero-length-vector pattern (new floatN(0, allocator) etc.). Also folds
        ...
        void EmptyBSRRoundTrip()
        {
            ...
            var ox = new floatN(1, Allocator.Temp);
            ox[0] = (float)3;
            var oy = new floatN(1, Allocator.Temp);
    ...
    public void ToDense_TransposeReference_Works()
    {
        var A = BuildSquare();
        var dense = A.ToDense(Allocator.Temp);
        var x = GenerateOP.floatRandomVec(A.M_Rows, (float)(-1f), (float)1f, 12321);
        ...
        var diff = new floatN(in y, Allocator.Temp);
        floatComp.subInPlace(diff, yRef);
        Assert.IsTrue(Analysis.isZero(diff, (float)1e-4f));
    }
    ...
    {
        var builder = new floatBSRBuilder(2, 2, 3, 3, Allocator.Temp);
        var block = new floatMxN(3, 3, Allocator.Temp);
    ...
    {
        var builder = new floatBSRBuilder(2, 2, 3, 3, Allocator.Temp);
        var wrong = new floatMxN(2, 3, Allocator.Temp); // not 3 x 3
    ...
    {
        var builder = new floatBSRBuilder(2, 2, 3, 3, Allocator.Temp); // 6 x 6
    ...
    public void SpMV_AliasingY_Throws()
    {
        ...
        var x = GenerateOP.floatRandomVec(A.N_Cols, (float)(-1f), (float)1f, 9001);
    ...
    {
        var A = BuildSquare();
        var x = GenerateOP.floatRandomVec(A.M_Rows, (float)(-1f), (float)1f, 9002);
        var yAlias = x;
    ...
    public void BlockJacobiApply_AliasingZ_Throws()
    {
        ...
        var r = GenerateOP.floatRandomVec(A.M_Rows, (float)(-1f), (float)1f, 9003);
    ...
    public void SpMV_DimensionMismatch_Throws()
    {
        ...
        var badX = new floatN(A.N_Cols + 1, Allocator.Temp);
        var y = new floatN(A.M_Rows, Allocator.Temp);
        Assert.Throws<ArgumentException>(() => BSR.spMV(in A, in badX, ref y));

        // wrong y length (must equal M_Rows).
        var x = new floatN(A.N_Cols, Allocator.Temp);
        var badY = new floatN(A.M_Rows + 1, Allocator.Temp);
    ...
    public void SpMVT_DimensionMismatch_Throws()
    {
        ...
        var badX = new floatN(A.M_Rows + 1, Allocator.Temp);
        var y = new floatN(A.N_Cols, Allocator.Temp);
        Assert.Throws<ArgumentException>(() => BSR.spMVT(in A, in badX, ref y));

        var x = new floatN(A.M_Rows, Allocator.Temp);
        var badY = new floatN(A.N_Cols + 1, Allocator.Temp);
    ...
    static floatBSR BuildSquare()
    {
        const int BR = 2, BC = 2;
        var builder = new floatBSRBuilder(2, 2, BR, BC, Allocator.Temp);
        builder.AddBlock(0, 0, GenerateOP.floatRandomMat(BR, BC, (float)(-1f), (float)1f, 6001));
        builder.AddBlock(1, 1, GenerateOP.floatRandomMat(BR, BC, (float)(-1f), (float)1f, 6002));
        return builder.ToBSR(Allocator.Temp);
    }
}
