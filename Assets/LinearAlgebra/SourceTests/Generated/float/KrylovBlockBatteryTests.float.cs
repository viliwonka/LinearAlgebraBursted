    ...
    public struct TestJob : IJob
    {
            ...
            where TScalar : struct, IfloatSquareSolverInvoker
        {
            if (inv.NeedsGeneralDenseOperator)
            {
                ...
                CheckDenseCore(inv, scalarInv, gm, in A, new floatDenseOperatorGeneral(in A), flags);
            }
            else
            {
                var A = floatKrylovBatteryGallery.Build(gm);
                CheckDenseCore(inv, scalarInv, gm, in A, new floatDenseOperator(in A), flags);
            }
        }

        void CheckDenseCore<TInvoker, TScalar, TOp>(TInvoker inv, TScalar scalarInv, GalleryDenseMatrix gm,
                                                     in floatMxN A, in TOp Aop, CheckFlags flags)
            where TInvoker : struct, IfloatBlockSolverInvoker
            where TScalar : struct, IfloatSquareSolverInvoker
            where TOp : struct, IfloatLinearOperator
        {
            ...
            var B = GenerateOP.floatRandomMat(flags.S, n, (float)(-1), (float)1, 0xD100u + (uint)gm);

            inv.Init(n, flags.S);
            scalarInv.Init(n);

            // 1. Converges: Solved or MaxIterations, AND the fresh (recomputed) residual bound holds.
            var X1 = new floatMxN(flags.S, n, Allocator.Temp);
            ...
            {
                var bj = floatKrylovBatteryOracles.Row(in B, j, n);
            ...
            var X3a = new floatMxN(flags.S, n, Allocator.Temp);
            var X3b = new floatMxN(flags.S, n, Allocator.Temp);
            ...
            var X4a = new floatMxN(flags.S, n, Allocator.Temp);
            var X4b = new floatMxN(flags.S, n, Allocator.Temp);
            ...
            {
                var M13 = new floatDenseSpdPreconditioner { Nmat = floatKrylovBatteryOracles.BuildDenseSpd(n, 0xD500u + (uint)gm, (float)(2 * n)) };
                var X13 = new floatMxN(flags.S, n, Allocator.Temp);
            ...
            where TScalar : struct, IfloatSquareSolverInvoker
        {
            ...
            var B = GenerateOP.floatRandomMat(flags.S, n, (float)(-1), (float)1, 0xB100u + (uint)gm);
            ...
            var X5 = new floatMxN(flags.S, n, Allocator.Temp);
            ...
            where TOp : struct, IfloatLinearOperator
        {
            ...
            // 6/7.
            var B6 = GenerateOP.floatRandomMat(flags.S, n, (float)(-1), (float)1, seedBase + 1u);
            var X6 = new floatMxN(flags.S, n, Allocator.Temp);
            BlockSolveInfo info6 = inv.Solve(in Aop, in B6, ref X6);
            int worstScalarIter = 0;
            for (int j = 0; j < flags.S; j++)
            {
                var bj = floatKrylovBatteryOracles.Row(in B6, j, n);
            ...
            var B8 = GenerateOP.floatRandomMat(flags.S, n, (float)(-1), (float)1, seedBase + 2u);
            for (int c = 0; c < n; c++) B8[dupB, c] = B8[dupA, c];
            var X8 = new floatMxN(flags.S, n, Allocator.Temp);
            ...
            var Xstar10 = GenerateOP.floatRandomMat(flags.S, n, (float)(-1), (float)1, seedBase + 3u);
            var Bwarm10 = new floatMxN(flags.S, n, Allocator.Temp);
            Aop.ApplyBlock(in Xstar10, ref Bwarm10, flags.S);
            var Xwarm10 = GenerateOP.floatRandomMat(flags.S, n, (float)(-1), (float)1, seedBase + 4u);
            ...
            int zeroRow11 = 0;
            var Bz11 = GenerateOP.floatRandomMat(flags.S, n, (float)(-1), (float)1, seedBase + 5u);
            for (int c = 0; c < n; c++) Bz11[zeroRow11, c] = (float)0;
            var Xz11 = new floatMxN(flags.S, n, Allocator.Temp);
            ...
            {
                var scratch11 = new floatN(n, Allocator.Temp);
                for (int j = 0; j < flags.S; j++)
                {
                    if (j == zeroRow11) continue;
                    var bj = floatKrylovBatteryOracles.Row(in Bz11, j, n);
                    var xj = floatKrylovBatteryOracles.Row(in Xz11, j, n);
