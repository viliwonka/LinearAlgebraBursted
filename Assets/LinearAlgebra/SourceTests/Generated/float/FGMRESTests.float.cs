...
public class floatFGMRESTests
{
    ...
    public struct FgmresTestJob : IJob
    {
        ...
        void MatchesGmresConstantPrecond()
        {
            ...
            var b = arena.floatRandomVec(n, -1f, 1f, 0xFA01u);
            ...
            for (int i = 0; i < n; i++) { xG[i] = (float)0; xF[i] = (float)0; }
            ...
            for (int i = 0; i < n; i++)
                Assert.IsTrue(math.abs(xF[i] - xG[i]) <= MatchTol() * ((float)1 + math.abs(xG[i])));
        ...
        void MatchesGmresIdentity()
        {
            ...
            var xTrue = arena.floatRandomVec(n, -1f, 1f, 0xFA12u);
            ...
            Assert.IsTrue(gi.status == IterativeSolveStatus.Converged);
            Assert.IsTrue(fi.status == IterativeSolveStatus.Converged);
            Assert.IsTrue(floatKrylovBatteryOracles.RelResidualDense(in A, in xF, in b) <= Tol());
        ...
        void KnownSolutionRecovery()
        {
            ...
            int n = 120;
            var A = ConvDiff1D(ref arena, n);
            var xTrue = arena.floatRandomVec(n, -1f, 1f, 0xFA21u);
        ...
        void RestartConverges()
        {
            ...
            int n = 120;
            var A = ConvDiff1D(ref arena, n);
            var b = arena.floatRandomVec(n, -1f, 1f, 0xFA31u);
        ...
        void VariableInnerGmresConverges()
        {
            ...
            int n = 150;
            var A = ConvDiff1D(ref arena, n);
            var b = arena.floatRandomVec(n, -1f, 1f, 0xFA41u);
            var op = new floatBSROperator(in A);
