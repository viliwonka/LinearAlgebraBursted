...
public class floatAMGGalerkinTests
{
    ...
    public struct GalerkinTestJob : IJob
    {
        ...
        static floatBSR BlockChain(ref Arena arena, int nb, int BR)
        {
            var b = arena.floatBSRBuilder(nb, nb, BR, BR, 3 * nb);
        ...
        void CheckIdentity(ref Arena arena, in floatBSR A, in floatBSR T, in floatBSR Ac, int ncoarse, uint seed)
        {
            var u = arena.floatRandomVec(ncoarse, -1f, 1f, seed);
            var v = arena.floatRandomVec(ncoarse, -1f, 1f, seed ^ 0x9E3779B9u);
        ...
        void IdentityBlock()
        {
            ...
            int n = nb * BR;
            var B = arena.floatRandomMat(n, m, -1f, 1f, 0x77A1u);
            var aggId = arena.Indices(nb);
            AMG.aggregate(in A, (float)0, ref aggId, out int numAgg);
        ...
        void CoarseIsSymmetric()
        {
            ...
            var A = BlockChain(ref arena, nb, BR);
            int n = nb * BR;
            var B = arena.floatRandomMat(n, m, -1f, 1f, 0x9001u);
            var aggId = arena.Indices(nb);
            AMG.aggregate(in A, (float)0, ref aggId, out int numAgg);
            var T = AMG.tentativeProlongator(in A, in aggId, numAgg, in B, ref arena, out _);
            var Ac = AMG.galerkinRAP(in A, in T, in aggId, numAgg, ref arena);
            ...
            var u = arena.floatRandomVec(ncoarse, -1f, 1f, 0xAAu);
            var v = arena.floatRandomVec(ncoarse, -1f, 1f, 0xBBu);
            var Acu = BSR.spMV(in Ac, in u);
        ...
        void Deterministic()
        {
            ...
            var A = BlockChain(ref arena, nb, BR);
            int n = nb * BR;
            var B = arena.floatRandomMat(n, m, -1f, 1f, 0xDEEDu);
            var aggId = arena.Indices(nb);
            AMG.aggregate(in A, (float)0, ref aggId, out int numAgg);
            var T = AMG.tentativeProlongator(in A, in aggId, numAgg, in B, ref arena, out _);
