...
// (bsmMatVecB{b} / bsmMatVecTB{b} / bsmMatVecSymB{b} in UnsafeOP.Sparse.float.cs), dispatched
// from Sparse_OP.spMV / spMVT (SparseOP.float.cs). Every case here proves ONE dispatch branch
// against the dense reference: build a floatBSM via the builder, expand with ToDense (already
// independently validated by floatSparseBSMTests), and assert spMV/spMVT agree with
// Linear_OP.dot on the dense expansion -- exactly the recipe floatSparseBSMTests.RandomSpMV /
...
// property is already covered separately by floatSparseSymmetricTests).
//
...
public class floatSparseUnrollTests
{
    ...
    public struct SparseUnrollTestJob : IJob
    {
        ...
        // Matches floatSparseBSMTests.Tol(): values live in [-1,1], dot products sum a handful
        ...
        // Linear_OP.trans), matching floatSparseBSMTests.DenseTransMatVec.
        ...
        static floatBSM BuildRandomSquare(ref Arena arena, int b, uint seedBase)
        {
            var builder = arena.floatBSMBuilder(4, 4, b, b);
        ...
        static floatBSM BuildRandomSymmetric(ref Arena arena, int b, uint seedBase)
        {
            var builder = arena.floatBSMBuilder(4, 4, b, b);
            builder.AddBlock(0, 0, arena.floatRandomMat(b, b, (float)(-1f), (float)1f, seedBase + 1u));
            builder.AddBlock(1, 1, arena.floatRandomMat(b, b, (float)(-1f), (float)1f, seedBase + 2u));
            builder.AddBlock(2, 2, arena.floatRandomMat(b, b, (float)(-1f), (float)1f, seedBase + 3u));
            builder.AddBlock(3, 3, arena.floatRandomMat(b, b, (float)(-1f), (float)1f, seedBase + 4u));
