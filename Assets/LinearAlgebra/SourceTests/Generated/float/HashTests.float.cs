...
// floatN / floatMxN). These are pure PROPERTY checks (determinism, seed sensitivity, avalanche,
...
public class floatHashTests
{
    ...
    public struct HashTestJob : IJob
    {
        ...
        floatN MakeVec(ref Arena arena, int n)
        {
            var v = arena.floatVec(n);
            for (int i = 0; i < n; i++)
                v[i] = (float)((i - 2) * 0.5f + 1.25f);
            return v;
        }

        floatMxN MakeMat(ref Arena arena, int m, int n)
        {
            var A = arena.floatMat(m, n);
            for (int r = 0; r < m; r++)
                for (int c = 0; c < n; c++)
                    A[r, c] = (float)((r * 3 - c * 2) * 0.25f + 0.5f);
            return A;
        }
        ...
        void AvalancheElement(ref Arena arena)
        {
            ...
            v[3] = v[3] + (float)1f; // perturb one element
        ...
        void AvalancheNegation(ref Arena arena)
        {
            ...
            v[0] = (float)1.5f; v[1] = (float)2.5f; v[2] = (float)(-3.5f); v[3] = (float)4.5f;
            uint before = Hash.hash(in v, 0u);
        ...
        {
            var v2 = arena.floatVec(2);
            v2[0] = (float)1f; v2[1] = (float)2f;

            var v3 = arena.floatVec(3);
            v3[0] = (float)1f; v3[1] = (float)2f; v3[2] = (float)0f;
        ...
        void RowHashesConsistency(ref Arena arena)
        {
            ...
            var row = arena.floatVec(A.N_Cols);
        ...
        void ColHashesConsistency(ref Arena arena)
        {
            ...
            var col = arena.floatVec(A.M_Rows);
            for (int c = 0; c < A.N_Cols; c++)
            {
                for (int r = 0; r < A.M_Rows; r++)
                    col[r] = A[r, c];
    ...
    public void ColHashesZeroColumnsReturnsEmpty()
    {
        ...
        try
        {
            var A = arena.floatMat(4, 0); // 4 rows, 0 cols
    ...
    public void RowHashesZeroRowsReturnsEmpty()
    {
        ...
        try
        {
            var A = arena.floatMat(0, 4); // 0 rows, 4 cols
