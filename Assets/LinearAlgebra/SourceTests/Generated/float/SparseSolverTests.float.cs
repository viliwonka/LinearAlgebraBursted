...
public class floatSparseSolverTests
{
    ...
    public struct SparseSolverTestJob : IJob
    {
        public enum TestType
        {
            ...
            // ---- LS diagnostics (floatLstsqInfo: rnorm/Arnorm/xnorm/iterations/converged) ----
        ...
        void LstsqInfoMatchesIndependentRecompute()
        {
            ...
            var xC = arena.floatVec(n);
            bool okC = Solvers.cgls(in A, in b, ref xC, maxIter, Consts.floatSqrtEps, (float)0, out var infoC);
            Assert.IsTrue(okC);
            ...
            var xL = arena.floatVec(n);
            bool okL = Solvers.lsqr(in A, in b, ref xL, maxIter, Consts.floatSqrtEps, (float)0, out var infoL);
            Assert.IsTrue(okL);
            ...
            var xM = arena.floatVec(n);
            bool okM = Solvers.lsmr(in A, in b, ref xM, maxIter, Consts.floatSqrtEps, (float)0, out var infoM);
            Assert.IsTrue(okM);
        ...
        void LstsqInfoDampedArnorm()
        {
            ...
            var xC = arena.floatVec(n);
            Assert.IsTrue(Solvers.cgls(in A, in b, ref xC, maxIter, Consts.floatSqrtEps, damp, out var infoC));
            AssertInfoMatches(in A, in b, in xC, damp, in infoC, LooseTol());

            var xL = arena.floatVec(n);
            Assert.IsTrue(Solvers.lsqr(in A, in b, ref xL, maxIter, Consts.floatSqrtEps, damp, out var infoL));
            AssertInfoMatches(in A, in b, in xL, damp, in infoL, LooseTol());

            var xM = arena.floatVec(n);
            Assert.IsTrue(Solvers.lsmr(in A, in b, ref xM, maxIter, Consts.floatSqrtEps, damp, out var infoM));
        ...
        void LstsqInfoBitIdenticalToPlainSolve()
        {
            ...
            int m = 12, n = 5;
            var A = arena.floatRandomMat(m, n, -1f, 1f, 51201);
            var b = arena.floatRandomVec(m, -1f, 1f, 51202);
            int maxIter = 8 * n;

            // cgls
            var xa = arena.floatVec(n);
            var xb = arena.floatVec(n);
            bool ra = Solvers.cgls(in A, in b, ref xa, maxIter, Consts.floatSqrtEps);
            bool rb = Solvers.cgls(in A, in b, ref xb, maxIter, Consts.floatSqrtEps, (float)0, out var ic);
            Assert.IsTrue(ra == rb && rb == ic.converged);
            for (int i = 0; i < n; i++) Assert.IsTrue(xa[i] == xb[i]);   // exact

            // lsqr
            var ya = arena.floatVec(n);
            var yb = arena.floatVec(n);
            bool sa = Solvers.lsqr(in A, in b, ref ya, maxIter, Consts.floatSqrtEps);
            bool sb = Solvers.lsqr(in A, in b, ref yb, maxIter, Consts.floatSqrtEps, (float)0, out var il);
            ...
            bool ta = Solvers.lsmr(in A, in b, ref za, maxIter, Consts.floatSqrtEps);
            bool tb = Solvers.lsmr(in A, in b, ref zb, maxIter, Consts.floatSqrtEps, (float)0, out var im);
        ...
        void LstsqInfoBSMMatchesDense()
        {
            ...
            Assert.IsTrue(Solvers.lsqr(in A, in b, ref xD, maxIter, Consts.floatSqrtEps, (float)0, out var infoD));

            var xB = arena.floatVec(n);
            Assert.IsTrue(Solvers.lsqr(in bsm, in b, ref xB, maxIter, Consts.floatSqrtEps, (float)0, out var infoB));
        ...
        void JacobiPreconditionerReducesIterations()
        {
            ...
            Assert.IsTrue(Solvers.lsqr(in A, in b, ref xPlain, maxIter, tol, (float)0, out var infoPlain));
            ...
            Assert.IsTrue(Solvers.lsqr(op, in b, ref y, ref u, ref v, ref w, ref tmpM, ref tmpN, maxIter, tol, (float)0, out floatLstsqInfo infoJac));
        ...
        void LstsqInfoStrangLineFitExact()
        {
            ...
            for (int which = 0; which < 3; which++)
            {
                ...
                floatLstsqInfo info;
                bool ok;
                if (which == 0)      ok = Solvers.cgls(in A, in b, ref x, 50, tol, (float)0, out info);
                else if (which == 1) ok = Solvers.lsqr(in A, in b, ref x, 50, tol, (float)0, out info);
                else                 ok = Solvers.lsmr(in A, in b, ref x, 50, tol, (float)0, out info);
