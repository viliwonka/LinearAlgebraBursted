...
public class floatBlockIDRTests
{
    ...
    public struct BlockIdrTestJob : IJob
    {
        ...
        static bool BlockResidualDenseOK(in floatMxN A, in floatMxN X, in floatMxN B, int m, int n, float tol)
        {
            var AX = new floatMxN(m, n, Allocator.Temp);
            new floatDenseOperatorGeneral(in A).ApplyBlock(in X, ref AX, m);
            for (int j = 0; j < m; j++)
            {
                float num = 0, den = 0;
        ...
        static bool BlockResidualBSROK(in floatBSR A, in floatMxN X, in floatMxN B, int m, int n, float tol)
        {
            var AX = new floatMxN(m, n, Allocator.Temp);
            new floatBSROperator(in A).ApplyBlock(in X, ref AX, m);
            for (int j = 0; j < m; j++)
            {
                float num = 0, den = 0;
                for (int c = 0; c < n; c++) { float d = AX[j, c] - B[j, c]; num += d * d; den += B[j, c] * B[j, c]; }
        ...
        void SolvesDenseNonsym()
        {
            ...
            var B = GenerateOP.floatRandomMat(m, n, (float)(-1f), (float)1f, 0x1B02u, Allocator.Temp);

            var X = new floatMxN(m, n, Allocator.Temp);                  // zero initial guess
        ...
        void SolvesBSRNonsym()
        {
            ...
            var A = ConvDiff1D(n);
            var B = GenerateOP.floatRandomMat(m, n, (float)(-1f), (float)1f, 0x1B12u, Allocator.Temp);

            var X = new floatMxN(m, n, Allocator.Temp);
            var info = Krylov.bidr(in A, in B, ref X, sDepth, 20 * n, ResTol());
        ...
        void MatchesScalarIdrPerColumn()
        {
            ...
            var B = GenerateOP.floatRandomMat(m, n, (float)(-1f), (float)1f, 0x1B22u, Allocator.Temp);
        ...
        void KnownSolutionRecovered()
        {
            ...
            var Xk = GenerateOP.floatRandomMat(m, n, (float)(-1f), (float)1f, 0x1B32u, Allocator.Temp);   // known solution

            var B = new floatMxN(m, n, Allocator.Temp);
            new floatDenseOperatorGeneral(in A).ApplyBlock(in Xk, ref B, m);           // B[j,:] = A Xk[j,:]
        ...
        void IdentityFoldBitIdentical()
        {
            ...
            var B = GenerateOP.floatRandomMat(m, n, (float)(-1f), (float)1f, 0x1B42u, Allocator.Temp);
            int maxIter = 12 * n;
            float tol = ResTol();
            ...
            var X1 = new floatMxN(m, n, Allocator.Temp);
            var info1 = Krylov.bidr<floatDenseOperatorGeneral, floatIdentityPreconditioner>(
                in opA, default(floatIdentityPreconditioner), in B, ref X1, sDepth, maxIter, tol, seed);

            // Unpreconditioned overload (folds to the identity core internally).
            var X2 = new floatMxN(m, n, Allocator.Temp);
            var info2 = Krylov.bidr<floatDenseOperatorGeneral>(
        ...
        void RankDeficientRHSBlockBreaksDownGracefully()
        {
            ...
            var B = GenerateOP.floatRandomMat(m, n, (float)(-1f), (float)1f, 0x1B52u, Allocator.Temp);
        ...
        void DeterminismExplicitSeed()
        {
            ...
            var A = ConvDiff1D(n);
            var B = GenerateOP.floatRandomMat(m, n, (float)(-1f), (float)1f, 0x1B62u, Allocator.Temp);
            uint seed = 0x1234ABCDu;
        ...
        void DeterminismDefaultSeed()
        {
            int n = 40, m = 3;
            var A = ConvDiff1D(n);
            var B = GenerateOP.floatRandomMat(m, n, (float)(-1f), (float)1f, 0x1B72u, Allocator.Temp);
        ...
        void PreconditionedILU0()
        {
            ...
            var A = ConvDiff1D(n);
            var B = GenerateOP.floatRandomMat(m, n, (float)(-1f), (float)1f, 0x1B82u, Allocator.Temp);
        ...
        void PreconditionedBlockJacobi()
        {
            int n = 120, m = 3, sDepth = 4;
            var A = ConvDiff1D(n);
            var B = GenerateOP.floatRandomMat(m, n, (float)(-1f), (float)1f, 0x1B92u, Allocator.Temp);
