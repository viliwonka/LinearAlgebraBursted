...
public class floatArenaBurstReadbackRegressionTests
{
    ...
    public struct ChecksumJob : IJob
    {
        ...
        public void Execute()
        {
            ...
            var A = arena.floatRandomMat(M, N, (float)(-1f), (float)1f, SeedA);
            var B = arena.floatRandomMat(S, M, (float)(-1f), (float)1f, SeedB);
            ...
            var ATB = arena.floatMat(S, N);
            for (int j = 0; j < S; j++)
            {
                var bj = arena.floatVec(M);
                for (int c = 0; c < M; c++) bj[c] = B[j, c];
                var atbj = arena.floatVec(N);
