...
public class floatLadFrischNewtonRankDeficientTests
{
    ...
    {
        int m = 5;
        var A = new floatMxN(m, 3, Allocator.Temp);
        var b = new floatN(m, Allocator.Temp);
        for (int i = 0; i < m; i++)
        {
            A[i, 0] = (float)1; A[i, 1] = (float)i; A[i, 2] = (float)i;
            b[i] = (float)i;
        }
        b[2] = (float)10;
        ...
        Assert.That(obj, Is.EqualTo(8.0).Within(1e-1),
    ...
    public void ZeroColumnStillReachesTheOptimum()
    {
        ...
        for (int i = 0; i < m; i++)
        {
            A[i, 0] = (float)1; A[i, 1] = (float)i; A[i, 2] = (float)0;
            b[i] = (float)i;
        }
        b[2] = (float)10;
        var x = new floatN(3, Allocator.Temp);
    ...
    public void IllConditionedVandermondeMatchesExactEngine()
    {
        ...
        for (int i = 0; i < m; i++)
        {
            ...
            double p = 1;
            for (int j = 0; j < n; j++) { A[i, j] = (float)p; p *= t; }
            b[i] = (float)(1.0 + 0.5 * t + rng.NextFloat(-0.2f, 0.2f));
        }
        b[7] = (float)(b[7] + 20f);   // one gross outlier, so the L1 and L2 fits differ
        ...
        Assert.That(rel, Is.LessThan(3e-4),
