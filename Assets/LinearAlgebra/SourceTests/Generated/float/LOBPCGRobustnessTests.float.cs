...
// convergence). Managed [Test]s (main thread), matching floatLOBPCGSmokeTests.
public class floatLOBPCGRobustnessTests
{
    static bool IsFloat => true;

    static double TolFactor() => 5e-2;
    ...
    static void AssertNoFalseCertificate(in LOBPCGInfo info, in floatN eig, in floatMxN vecs,
    ...
    static void AssertConvergedPairsGenuine(in floatBSR A, in floatLOBPCGCache ws, int k, int n,
                                             float tol, float normFloor, double[] dense, double tolFactor,
                                             in LOBPCGInfo info)
    {
    ...
    public void ZeroRowWarmStartNoFalseCertificate()
    {
        ...
        var info = Eigen.lobpcg(in A, ref ws, k, Consts.floatSqrtEps, 500);
    ...
    public void JobbedExactWarmStartCertifiesInCache()
    {
        ...
        var outp = new NativeArray<int>(3, Allocator.TempJob);
        var job = new JobbedLobpcgBSR { A = A, Cache = ws, K = k, Tol = Consts.floatSqrtEps, MaxIter = 500, Out = outp };
        job.Run();
