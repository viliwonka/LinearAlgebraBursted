...
// Unit battery for QP's PERSISTENT working-set factorization (floatQPFactorState + QP.TryAddToFactor /
...
// same InternalsVisibleTo route as QPActiveSetTests.float.cs; same Burst job + Fail[] pattern.
//
...
// optimum is interior, forcing 12 consecutive drops -- more than floatQPFactorState.DeadCap (8), so at
// least one RefactorWorkingSet fallback runs inside the loop; it also passes through k = 0 (empty
// working set) before converging.
//
// The PERSISTENT REDUCED SPACE (floatQPReducedState + QP.RebuildReduced / UpdateReducedOnAdd /
...
public class floatQPFactorStateTests
{
    // ================================================================================================
    ...
    public struct UpDownDateJob : IJob
    {
        ...
        static void BuildCol(in floatMxN A, int m, int n, int t, WorkingSetStatus st, ref floatN v)
        {
            float sign = st == WorkingSetStatus.ActiveUpper ? (float)(-1) : (float)1;
            if (t < m)
                for (int i = 0; i < n; i++) v[i] = sign * A[t, i];
            else
            {
                for (int i = 0; i < n; i++) v[i] = (float)0;
                v[t - m] = sign;
            }
        }

        void VerifyState(ref floatQPFactorState s, in floatMxN A, int m, int n,
        ...
        void Add(ref floatQPFactorState s, in floatMxN A, int m, int n, int t, WorkingSetStatus st,
                 NativeArray<byte> statusByT, float thr, int id)
        {
            H.AssertTrue(Fail, id, QP.TryAddToFactor(in A, m, n, t, st, ref s, thr));
            statusByT[t] = (byte)st;
        }

        void Drop(ref floatQPFactorState s, int col, NativeArray<byte> statusByT)
        {
        ...
        public void Execute()
        {
            ...
            var s = floatQPFactorState.Create(n);
            float thr = Consts.floatZeroThreshold * math.max(Norms.LInf(in A), (float)1);
    ...
    public struct CornerToInteriorJob : IJob
    {
        ...
        public void Execute()
        {
            ...
            var c = GenerateOP.floatVec(n, (float)(-2), Allocator.Temp);
            ...
            var xu = GenerateOP.floatVec(n, (float)10, Allocator.Temp);
            var x = new floatN(n, Allocator.Temp);                   // x0 = 0: every lower bound tight
    ...
    public struct IncrementalReducedJob : IJob
    {
        ...
        static void Sync(in floatMxN Q, ref floatQPFactorState s, ref floatQPReducedState red)
        {
            if (red.stale || red.changeCount >= floatQPReducedState.RebuildCap)
                QP.RebuildReduced(in Q, ref s, ref red);
        }

        void VerifyReduced(in floatMxN Q, ref floatQPFactorState s, ref floatQPReducedState red,
                           int baseId, double tol)
        {
            int n = s.n, k = s.k, nz = n - k;
            if (nz <= 0) return;

            var Zf = new floatMxN(n, nz, Allocator.Temp, true);
            QP.FormNullSpaceBasisFromFactor(ref s, ref Zf);
            var QZf = new floatMxN(n, nz, Allocator.Temp, true);
            Blas.dot(in Q, in Zf, ref QZf);
            var Hzf = new floatMxN(nz, nz, Allocator.Temp, true);
        ...
        void AddU(in floatMxN A, int m, int n, int t, WorkingSetStatus st, NativeArray<byte> statusByT,
                  ref floatQPFactorState s, ref floatQPReducedState red, float thr, int id)
        {
            H.AssertTrue(Fail, id, QP.TryAddToFactor(in A, m, n, t, st, ref s, thr));
            statusByT[t] = (byte)st;
        ...
        void DropU(in floatMxN Q, in floatMxN A, int m, int n, int T, int col, NativeArray<byte> statusByT,
                   ref floatQPFactorState s, ref floatQPReducedState red, float thr)
        {
            statusByT[s.rowOfCol[col]] = (byte)WorkingSetStatus.Inactive;
            QP.DropFromFactor(col, ref s);
            if (s.deadCount >= floatQPFactorState.DeadCap)
            {
        ...
        public void Execute()
        {
            ...
            for (int i = 0; i < n; i++)
                for (int j = 0; j < n; j++)
                    Q[i, j] = i == j ? (float)4 : (float)1 / (float)(1 + math.abs(i - j));

            var statusByT = new NativeArray<byte>(T, Allocator.Temp);
            var s = floatQPFactorState.Create(n);
            var red = floatQPReducedState.Create(n);
            float thr = Consts.floatZeroThreshold * math.max(Norms.LInf(in A), (float)1);
    ...
    public struct CachedRetryJob : IJob
    {
        ...
        public void Execute()
        {
            ...
            var Q = new floatMxN(n, n, Allocator.Temp);   // PSD rank 2: null(Q) = span(e2, e3)
            Q[0, 0] = 1f; Q[1, 1] = 1f;

            var s = floatQPFactorState.Create(n);
            var red = floatQPReducedState.Create(n);
            float thr = Consts.floatZeroThreshold;
            ...
            gz[0] = (float)1; gz[1] = (float)0.5; gz[2] = (float)(-0.25);
            var y = new floatN(nz, Allocator.Temp, false);

            var snap = new floatMxN(nz, nz, Allocator.Temp, true);
            for (int i = 0; i < nz; i++)
                for (int j = 0; j < nz; j++) snap[i, j] = red.Hz[i, j];

            var info = QP.SolveReducedNewtonStepCached(ref red, nz, ref gz, ref y, (float)1, out bool regularized);
    ...
    public struct FallbackEquivalenceJob : IJob
    {
        ...
        void SolveBoth(in floatMxN Q, in floatN c, in floatMxN A, in floatN b,
                       NativeArray<ConstraintSense> senses, in floatN xl, in floatN xu,
                       int baseId, double tol)
        {
            int n = Q.M_Rows;
            var xInc = new floatN(n, Allocator.Temp);   // zero-init: x0 = 0 (feasible by construction)
            var xBat = new floatN(n, Allocator.Temp);
        ...
        public void Execute()
        {
            ...
            {   // instance 1: n = 8, corner start, interior-leaning optimum
                ...
                var Q = new floatMxN(n, n, Allocator.Temp);
                for (int i = 0; i < n; i++)
                    for (int j = 0; j < n; j++) Q[i, j] = i == j ? (float)3 : (float)0.25;
                var c = new floatN(n, Allocator.Temp, false);
                for (int i = 0; i < n; i++) c[i] = (float)(-1.0 - 0.1 * i);
                var A = new floatMxN(m, n, Allocator.Temp);
                for (int j = 0; j < n; j++) { A[0, j] = 1f; if ((j & 1) == 0) A[1, j] = 1f; }
                var b = new floatN(m, Allocator.Temp, false);
                b[0] = (float)2.5; b[1] = (float)1.2;
                ...
                var xl = new floatN(n, Allocator.Temp);                            // 0
                var xu = new floatN(n, Allocator.Temp, false);
                for (int i = 0; i < n; i++) xu[i] = (float)2;
            ...
            {   // instance 2: n = 10, steep costs against the upper box + a tight sum row
                ...
                var Q = new floatMxN(n, n, Allocator.Temp);
                for (int i = 0; i < n; i++)
                    for (int j = 0; j < n; j++) Q[i, j] = i == j ? (float)2 : (float)0.1;
                var c = new floatN(n, Allocator.Temp, false);
                for (int i = 0; i < n; i++) c[i] = (float)(-4.0 + 0.3 * i);
                var A = new floatMxN(m, n, Allocator.Temp);
                for (int j = 0; j < n; j++) { A[0, j] = 1f; A[1, j] = (float)(0.2 * (j + 1)); }
                var b = new floatN(m, Allocator.Temp, false);
                b[0] = (float)4; b[1] = (float)6;
                var senses = new NativeArray<ConstraintSense>(m, Allocator.Temp);
                for (int i = 0; i < m; i++) senses[i] = ConstraintSense.LessEqual;   // (default enum is Equal)
                var xl = new floatN(n, Allocator.Temp);                            // 0
                var xu = new floatN(n, Allocator.Temp, false);
                for (int i = 0; i < n; i++) xu[i] = (float)1;
