// <auto-generated>
//   Generated from Assets/LinearAlgebra/CodeGen/TemplateSourceTests/fProxy/CompMathTests.fProxy.cs
//   DO NOT EDIT BY HAND - edit the template and run Tools/regen.ps1.
// </auto-generated>
using System;

using BULA;

using NUnit.Framework;
using Unity.Burst;
using Unity.Collections;
using Unity.Jobs;
using Unity.Mathematics;

// Tests for the floatComp elementwise math surface (OP.Component.float.cs forwarding to
// UnsafeMathOP). Every case uses Unity.Mathematics math.xxx on the SAME (pre-mutation) inputs as
// the oracle, so the expected value is computed by the exact function the kernel uses - the checks
// are therefore near-exact (a small type-scaled relative tolerance absorbs only reassociation).
//
// The unary math ops are batched: each is one enum value dispatched through a single Unary() body
// (input builder + apply switch + oracle switch), rather than 27 copy-paste methods. Binary/ternary
// ops get their own methods because each has a distinct mutation contract (which buffer is written,
// which must stay unchanged) that is itself under test - these are the classic forwarder failure
// modes (argument order, wrong mutation target).
public class floatCompMathTests
{
    [BurstCompile(CompileSynchronously = true, FloatPrecision = FloatPrecision.High, FloatMode = FloatMode.Default)]
    public struct TestsJob : IJob
    {
        public enum TestType
        {
            // ---- unary (oracle = the exact math.xxx the kernel calls) ----
            Abs, Sign, Sqrt, Rsqrt, Acos, Asin, Atan, Acosh,
            Ceil, Floor, Round, Cos, Cosh, Sin, Sinh, Tan, Tanh,
            Exp, Exp2, Exp10, Log, Log2, Log10, Saturate, Frac, Rcp, Relu,

            // ---- pow (int exponent) ----
            PowExponents,

            // ---- interpolation / edges / fused ----
            Lerp, Unlerp, SmoothstepBuffers, SmoothstepScalarEdges, Step,
            Mad, Remap, DegreesRadians, Atan2, MinBuf, MaxBuf, Sincos,
            AbsDiff, SqrDiff,

            // ---- matrix path (generic over IUnsafefloatArray) ----
            MatrixAbs, MatrixExp, MatrixMad,

            // ---- degenerate shapes ----
            SingleElement, EmptyBuffer,
        }

        public TestType Type;

        public void Execute()
        {
            switch (Type)
            {
                // unary ops all share one body
                case TestType.Abs: case TestType.Sign: case TestType.Sqrt:
                case TestType.Rsqrt: case TestType.Acos: case TestType.Asin:
                case TestType.Atan: case TestType.Acosh: case TestType.Ceil:
                case TestType.Floor: case TestType.Round: case TestType.Cos:
                case TestType.Cosh: case TestType.Sin: case TestType.Sinh:
                case TestType.Tan: case TestType.Tanh: case TestType.Exp:
                case TestType.Exp2: case TestType.Exp10: case TestType.Log:
                case TestType.Log2: case TestType.Log10: case TestType.Saturate:
                case TestType.Frac: case TestType.Rcp: case TestType.Relu:
                    Unary(Type);
                    break;

                case TestType.PowExponents: PowExponents(); break;

                case TestType.Lerp: LerpTest(); break;
                case TestType.Unlerp: UnlerpTest(); break;
                case TestType.SmoothstepBuffers: SmoothstepBuffersTest(); break;
                case TestType.SmoothstepScalarEdges: SmoothstepScalarEdgesTest(); break;
                case TestType.Step: StepTest(); break;
                case TestType.Mad: MadTest(); break;
                case TestType.Remap: RemapTest(); break;
                case TestType.DegreesRadians: DegreesRadiansTest(); break;
                case TestType.Atan2: Atan2Test(); break;
                case TestType.MinBuf: MinBufTest(); break;
                case TestType.MaxBuf: MaxBufTest(); break;
                case TestType.Sincos: SincosTest(); break;
                case TestType.AbsDiff: AbsDiffTest(); break;
                case TestType.SqrDiff: SqrDiffTest(); break;

                case TestType.MatrixAbs: MatrixAbsTest(); break;
                case TestType.MatrixExp: MatrixExpTest(); break;
                case TestType.MatrixMad: MatrixMadTest(); break;

                case TestType.SingleElement: SingleElementTest(); break;
                case TestType.EmptyBuffer: EmptyBufferTest(); break;

                default: throw new NotImplementedException();
            }
        }

        // true only when float expands to double (doubleEpsilon ~2.2e-16 < 1e-10).
        private bool IsDouble() => (double)Consts.floatEpsilon < 1e-10;

        // Type-scaled relative+absolute closeness. Oracle uses the same math.xxx as the kernel, so
        // the gap is essentially reassociation-only; float gets a looser band than double.
        private void AssertClose(float got, float expected)
        {
            float tol = IsDouble() ? (float)1e-11 : (float)1e-4;
            float diff = math.abs(got - expected);
            float scale = (float)1 + math.abs(expected);
            Assert.IsTrue(diff <= tol * scale);
        }

        // Domain-safe deterministic input per unary op (sqrt/log need x>0, acos/asin need [-1,1],
        // acosh needs x>=1, tan avoids +/-pi/2, exp10 kept small so 10^x doesn't overflow).
        private floatN MakeUnaryInput(TestType k, int n)
        {
            switch (k)
            {
                case TestType.Sqrt:
                case TestType.Log:
                case TestType.Log2:
                case TestType.Log10:
                    return GenerateOP.floatLinVec(n, (float)0.02, (float)9, Allocator.Temp);

                case TestType.Rsqrt:
                case TestType.Rcp:
                    return GenerateOP.floatLinVec(n, (float)0.05, (float)6, Allocator.Temp);

                case TestType.Acos:
                case TestType.Asin:
                    return GenerateOP.floatLinVec(n, (float)(-0.95), (float)0.95, Allocator.Temp);

                case TestType.Acosh:
                    return GenerateOP.floatLinVec(n, (float)1, (float)4, Allocator.Temp);

                case TestType.Tan:
                    return GenerateOP.floatLinVec(n, (float)(-1.2), (float)1.2, Allocator.Temp);

                case TestType.Exp10:
                    return GenerateOP.floatLinVec(n, (float)(-2), (float)3, Allocator.Temp);

                case TestType.Exp:
                case TestType.Exp2:
                    return GenerateOP.floatLinVec(n, (float)(-3), (float)4, Allocator.Temp);

                case TestType.Abs:
                case TestType.Sign:
                case TestType.Relu:
                {
                    // span negatives .. positives and pin an exact zero (sign/relu boundary).
                    var v = GenerateOP.floatLinVec(n, (float)(-3), (float)3, Allocator.Temp);
                    v[0] = (float)0;
                    return v;
                }

                default:
                    // ceil/floor/round/cos/cosh/sin/sinh/tanh/atan/saturate/frac: any real ok.
                    return GenerateOP.floatLinVec(n, (float)(-3.3), (float)3.7, Allocator.Temp);
            }
        }

        private void Unary(TestType k)
        {
            int n = 37; // odd, not a SIMD multiple -> exercises the scalar tail
            floatN x = MakeUnaryInput(k, n);
            floatN orig = new floatN(in x, Allocator.Temp);

            switch (k)
            {
                case TestType.Abs: x.absInPlace(); break;
                case TestType.Sign: x.signInPlace(); break;
                case TestType.Sqrt: x.sqrtInPlace(); break;
                case TestType.Rsqrt: x.rsqrtInPlace(); break;
                case TestType.Acos: x.acosInPlace(); break;
                case TestType.Asin: x.asinInPlace(); break;
                case TestType.Atan: x.atanInPlace(); break;
                case TestType.Acosh: x.acoshInPlace(); break;
                case TestType.Ceil: x.ceilInPlace(); break;
                case TestType.Floor: x.floorInPlace(); break;
                case TestType.Round: x.roundInPlace(); break;
                case TestType.Cos: x.cosInPlace(); break;
                case TestType.Cosh: x.coshInPlace(); break;
                case TestType.Sin: x.sinInPlace(); break;
                case TestType.Sinh: x.sinhInPlace(); break;
                case TestType.Tan: x.tanInPlace(); break;
                case TestType.Tanh: x.tanhInPlace(); break;
                case TestType.Exp: x.expInPlace(); break;
                case TestType.Exp2: x.exp2InPlace(); break;
                case TestType.Exp10: x.exp10InPlace(); break;
                case TestType.Log: x.logInPlace(); break;
                case TestType.Log2: x.log2InPlace(); break;
                case TestType.Log10: x.log10InPlace(); break;
                case TestType.Saturate: x.saturateInPlace(); break;
                case TestType.Frac: x.fracInPlace(); break;
                case TestType.Rcp: x.rcpInPlace(); break;
                case TestType.Relu: x.reluInPlace(); break;
                default: throw new NotImplementedException();
            }

            for (int i = 0; i < n; i++)
            {
                float o = orig[i];
                float expected;
                switch (k)
                {
                    case TestType.Abs: expected = math.abs(o); break;
                    case TestType.Sign: expected = math.sign(o); break;
                    case TestType.Sqrt: expected = math.sqrt(o); break;
                    case TestType.Rsqrt: expected = math.rsqrt(o); break;
                    case TestType.Acos: expected = math.acos(o); break;
                    case TestType.Asin: expected = math.asin(o); break;
                    case TestType.Atan: expected = math.atan(o); break;
                    // no math.acosh: kernel is log(x + sqrt(x^2 - 1)), domain x>=1.
                    case TestType.Acosh: expected = math.log(o + math.sqrt(o * o - (float)1)); break;
                    case TestType.Ceil: expected = math.ceil(o); break;
                    case TestType.Floor: expected = math.floor(o); break;
                    case TestType.Round: expected = math.round(o); break;
                    case TestType.Cos: expected = math.cos(o); break;
                    case TestType.Cosh: expected = math.cosh(o); break;
                    case TestType.Sin: expected = math.sin(o); break;
                    case TestType.Sinh: expected = math.sinh(o); break;
                    case TestType.Tan: expected = math.tan(o); break;
                    case TestType.Tanh: expected = math.tanh(o); break;
                    case TestType.Exp: expected = math.exp(o); break;
                    case TestType.Exp2: expected = math.exp2(o); break;
                    // kernel uses math.pow(10, x), not a math.exp10 (which does not exist).
                    case TestType.Exp10: expected = math.pow((float)10, o); break;
                    case TestType.Log: expected = math.log(o); break;
                    case TestType.Log2: expected = math.log2(o); break;
                    case TestType.Log10: expected = math.log10(o); break;
                    case TestType.Saturate: expected = math.saturate(o); break;
                    case TestType.Frac: expected = o - math.floor(o); break;
                    case TestType.Rcp: expected = math.rcp(o); break;
                    case TestType.Relu: expected = o < (float)0 ? (float)0 : o; break;
                    default: throw new NotImplementedException();
                }
                AssertClose(x[i], expected);
            }
        }

        // ---- pow: int exponent (0,1,2,3 and negatives). Positive bases keep math.pow well-defined
        //      for the reciprocal-power (negative-exponent) cases. ----
        private void PowExponents()
        {
            CheckPow(0);
            CheckPow(1);
            CheckPow(2);
            CheckPow(3);
            CheckPow(-1);
            CheckPow(-2);
        }

        private void CheckPow(int e)
        {
            int n = 16;
            floatN x = GenerateOP.floatLinVec(n, (float)0.3, (float)3.5, Allocator.Temp);
            floatN orig = new floatN(in x, Allocator.Temp);
            x.powInPlace(e);
            for (int i = 0; i < n; i++)
                AssertClose(x[i], math.pow(orig[i], (float)e)); // exponent 0 -> 1, negative -> reciprocal power
        }

        // ---- interpolation / edges / fused ----

        private void LerpTest()
        {
            int n = 24;
            floatN a = GenerateOP.floatLinVec(n, (float)(-2), (float)5, Allocator.Temp);
            floatN b = GenerateOP.floatLinVec(n, (float)3, (float)9, Allocator.Temp);
            floatN a0 = new floatN(in a, Allocator.Temp);
            floatN b0 = new floatN(in b, Allocator.Temp);
            float t = (float)0.35;

            a.lerpInPlace(b, t); // a[i] = lerp(a[i], b[i], t); a mutated, b untouched

            for (int i = 0; i < n; i++)
            {
                AssertClose(a[i], math.lerp(a0[i], b0[i], t));
                AssertClose(b[i], b0[i]);
            }
        }

        private void UnlerpTest()
        {
            int n = 24;
            floatN a = GenerateOP.floatLinVec(n, (float)(-2), (float)1, Allocator.Temp);
            floatN b = GenerateOP.floatLinVec(n, (float)6, (float)12, Allocator.Temp); // b != a everywhere (no /0)
            floatN a0 = new floatN(in a, Allocator.Temp);
            floatN b0 = new floatN(in b, Allocator.Temp);
            float t = (float)0.4;

            a.unlerpInPlace(b, t);

            for (int i = 0; i < n; i++)
            {
                AssertClose(a[i], math.unlerp(a0[i], b0[i], t));
                AssertClose(b[i], b0[i]);
            }
        }

        private void SmoothstepBuffersTest()
        {
            int n = 24;
            floatN a = GenerateOP.floatLinVec(n, (float)(-1), (float)1, Allocator.Temp); // edge0 buffer
            floatN b = GenerateOP.floatLinVec(n, (float)2, (float)4, Allocator.Temp);    // edge1 buffer (> edge0)
            floatN a0 = new floatN(in a, Allocator.Temp);
            floatN b0 = new floatN(in b, Allocator.Temp);
            float t = (float)0.5;

            a.smoothstepInPlace(b, t); // a[i] = smoothstep(a[i], b[i], t)

            for (int i = 0; i < n; i++)
            {
                AssertClose(a[i], math.smoothstep(a0[i], b0[i], t));
                AssertClose(b[i], b0[i]);
            }
        }

        private void SmoothstepScalarEdgesTest()
        {
            int n = 24;
            floatN x = GenerateOP.floatLinVec(n, (float)(-0.5), (float)1.5, Allocator.Temp); // spans below/above edges
            floatN x0 = new floatN(in x, Allocator.Temp);
            float edge0 = (float)0;
            float edge1 = (float)1;

            x.smoothstepInPlace(edge0, edge1); // x[i] = smoothstep(edge0, edge1, x[i])

            for (int i = 0; i < n; i++)
                AssertClose(x[i], math.smoothstep(edge0, edge1, x0[i]));
        }

        private void StepTest()
        {
            int n = 24;
            floatN x = GenerateOP.floatLinVec(n, (float)(-1), (float)2, Allocator.Temp);
            floatN x0 = new floatN(in x, Allocator.Temp);
            float edge = (float)0.5;

            x.stepInPlace(edge); // x[i] = step(edge, x[i]) == (x >= edge ? 1 : 0)

            for (int i = 0; i < n; i++)
                AssertClose(x[i], math.step(edge, x0[i]));
        }

        private void MadTest()
        {
            int n = 24;
            floatN a = GenerateOP.floatLinVec(n, (float)(-2), (float)2, Allocator.Temp);
            floatN b = GenerateOP.floatLinVec(n, (float)1, (float)3, Allocator.Temp);
            floatN c = GenerateOP.floatLinVec(n, (float)(-1), (float)1, Allocator.Temp);
            floatN a0 = new floatN(in a, Allocator.Temp);
            floatN b0 = new floatN(in b, Allocator.Temp);
            floatN c0 = new floatN(in c, Allocator.Temp);

            a.madInPlace(b, c); // a[i] = a*b + c ; ONLY a is mutated

            for (int i = 0; i < n; i++)
            {
                AssertClose(a[i], a0[i] * b0[i] + c0[i]);
                AssertClose(b[i], b0[i]);
                AssertClose(c[i], c0[i]);
            }
        }

        private void RemapTest()
        {
            int n = 24;
            floatN x = GenerateOP.floatLinVec(n, (float)1, (float)9, Allocator.Temp); // strictly inside [oldMin,oldMax]
            floatN x0 = new floatN(in x, Allocator.Temp);
            float oldMin = (float)0, oldMax = (float)10, newMin = (float)(-1), newMax = (float)1;

            x.remapInPlace(oldMin, oldMax, newMin, newMax);

            // Maps x from [oldMin,oldMax] onto [newMin,newMax]. Unity's math.remap takes the VALUE
            // as its LAST argument: remap(srcStart, srcEnd, dstStart, dstEnd, value).
            for (int i = 0; i < n; i++)
                AssertClose(x[i], math.remap(oldMin, oldMax, newMin, newMax, x0[i]));
        }

        private void DegreesRadiansTest()
        {
            int n = 20;

            // degreesInPlace vs math.degrees
            floatN r = GenerateOP.floatLinVec(n, (float)(-3), (float)3, Allocator.Temp);
            floatN r0 = new floatN(in r, Allocator.Temp);
            floatN d = new floatN(in r, Allocator.Temp);
            d.degreesInPlace();
            for (int i = 0; i < n; i++)
                AssertClose(d[i], math.degrees(r0[i]));

            // radians undoes degrees -> round trip back to original radians
            d.radiansInPlace();
            for (int i = 0; i < n; i++)
                AssertClose(d[i], r0[i]);

            // radiansInPlace vs math.radians on a degree-scale input
            floatN deg = GenerateOP.floatLinVec(n, (float)(-180), (float)180, Allocator.Temp);
            floatN deg0 = new floatN(in deg, Allocator.Temp);
            deg.radiansInPlace();
            for (int i = 0; i < n; i++)
                AssertClose(deg[i], math.radians(deg0[i]));
        }

        private void Atan2Test()
        {
            int n = 24;
            // receiver is y (numerator), argument is x (denominator): atan2InPlace(y, x) == atan2(y, x).
            floatN y = GenerateOP.floatLinVec(n, (float)(-3), (float)3, Allocator.Temp);
            floatN x = GenerateOP.floatLinVec(n, (float)0.5, (float)4, Allocator.Temp); // x > 0 (principal branch)
            floatN y0 = new floatN(in y, Allocator.Temp);
            floatN x0 = new floatN(in x, Allocator.Temp);

            y.atan2InPlace(x); // y[i] = atan2(y[i], x[i]) ; x untouched

            for (int i = 0; i < n; i++)
            {
                AssertClose(y[i], math.atan2(y0[i], x0[i]));
                AssertClose(x[i], x0[i]);
            }
        }

        private void MinBufTest()
        {
            int n = 24;
            floatN x = GenerateOP.floatLinVec(n, (float)(-2), (float)5, Allocator.Temp);
            floatN y = GenerateOP.floatLinVec(n, (float)4, (float)(-1), Allocator.Temp); // crosses x so both branches hit
            floatN x0 = new floatN(in x, Allocator.Temp);
            floatN y0 = new floatN(in y, Allocator.Temp);

            x.minInPlace(y); // x[i] = min(x[i], y[i]) ; y untouched

            for (int i = 0; i < n; i++)
            {
                AssertClose(x[i], math.min(x0[i], y0[i]));
                AssertClose(y[i], y0[i]);
            }
        }

        private void MaxBufTest()
        {
            int n = 24;
            floatN x = GenerateOP.floatLinVec(n, (float)(-2), (float)5, Allocator.Temp);
            floatN y = GenerateOP.floatLinVec(n, (float)4, (float)(-1), Allocator.Temp);
            floatN x0 = new floatN(in x, Allocator.Temp);
            floatN y0 = new floatN(in y, Allocator.Temp);

            x.maxInPlace(y); // x[i] = max(x[i], y[i]) ; y untouched

            for (int i = 0; i < n; i++)
            {
                AssertClose(x[i], math.max(x0[i], y0[i]));
                AssertClose(y[i], y0[i]);
            }
        }

        // ---- absDiff/sqrDiff: componentwise |a-b| / (a-b)^2 - these are NOT whole-vector Euclidean
        //      distances, each index is an independent scalar difference. Oracle computed directly,
        //      not via math.distance/math.distancesq, so the test doesn't just re-assert the kernel's
        //      own implementation. ----

        private void AbsDiffTest()
        {
            int n = 24;
            floatN x = GenerateOP.floatLinVec(n, (float)(-2), (float)5, Allocator.Temp);
            floatN y = GenerateOP.floatLinVec(n, (float)4, (float)(-1), Allocator.Temp); // crosses x so sign of (x-y) varies
            floatN x0 = new floatN(in x, Allocator.Temp);
            floatN y0 = new floatN(in y, Allocator.Temp);

            x.absDiffInPlace(y); // x[i] = |x[i] - y[i]| ; y untouched

            for (int i = 0; i < n; i++)
            {
                AssertClose(x[i], math.abs(x0[i] - y0[i]));
                AssertClose(y[i], y0[i]);
            }
        }

        private void SqrDiffTest()
        {
            int n = 24;
            floatN x = GenerateOP.floatLinVec(n, (float)(-2), (float)5, Allocator.Temp);
            floatN y = GenerateOP.floatLinVec(n, (float)4, (float)(-1), Allocator.Temp);
            floatN x0 = new floatN(in x, Allocator.Temp);
            floatN y0 = new floatN(in y, Allocator.Temp);

            x.sqrDiffInPlace(y); // x[i] = (x[i] - y[i])^2 ; y untouched

            for (int i = 0; i < n; i++)
            {
                AssertClose(x[i], (x0[i] - y0[i]) * (x0[i] - y0[i]));
                AssertClose(y[i], y0[i]);
            }
        }

        private void SincosTest()
        {
            int n = 30;
            floatN x = GenerateOP.floatLinVec(n, (float)(-3), (float)3, Allocator.Temp);
            floatN x0 = new floatN(in x, Allocator.Temp);
            floatN s = new floatN(n, Allocator.Temp);
            floatN c = new floatN(n, Allocator.Temp);

            x.sincos(s, c); // x is NOT mutated; s <- sin(x), c <- cos(x).

            for (int i = 0; i < n; i++)
            {
                AssertClose(x[i], x0[i]); // the one op here that does NOT mutate its receiver
                AssertClose(s[i], math.sin(x0[i]));
                AssertClose(c[i], math.cos(x0[i]));
            }
        }

        // ---- matrix path: proves the generic IUnsafefloatArray constraint covers floatMxN.
        //      Non-square (5x7) so the flat length, not a square dim, drives the loop. ----

        private void MatrixAbsTest()
        {
            int rows = 5, cols = 7;
            floatMxN m = new floatMxN(rows, cols, Allocator.Temp);
            for (int i = 0; i < m.Length; i++)
                m[i] = (float)(i - 15) * (float)0.5; // negatives and positives
            floatMxN m0 = new floatMxN(in m, Allocator.Temp);

            m.absInPlace();

            for (int i = 0; i < m.Length; i++)
                AssertClose(m[i], math.abs(m0[i]));
        }

        private void MatrixExpTest()
        {
            int rows = 5, cols = 7;
            floatMxN m = new floatMxN(rows, cols, Allocator.Temp);
            for (int i = 0; i < m.Length; i++)
                m[i] = (float)(i - 15) * (float)0.1;
            floatMxN m0 = new floatMxN(in m, Allocator.Temp);

            m.expInPlace();

            for (int i = 0; i < m.Length; i++)
                AssertClose(m[i], math.exp(m0[i]));
        }

        private void MatrixMadTest()
        {
            int rows = 5, cols = 7;
            floatMxN a = new floatMxN(rows, cols, Allocator.Temp);
            floatMxN b = new floatMxN(rows, cols, Allocator.Temp);
            floatMxN c = new floatMxN(rows, cols, Allocator.Temp);
            for (int i = 0; i < a.Length; i++)
            {
                a[i] = (float)(i - 15) * (float)0.2;
                b[i] = (float)(i) * (float)0.3 - (float)1;
                c[i] = (float)2 - (float)i * (float)0.1;
            }
            floatMxN a0 = new floatMxN(in a, Allocator.Temp);
            floatMxN b0 = new floatMxN(in b, Allocator.Temp);
            floatMxN c0 = new floatMxN(in c, Allocator.Temp);

            a.madInPlace(b, c); // a = a*b + c ; only a mutated

            for (int i = 0; i < a.Length; i++)
            {
                AssertClose(a[i], a0[i] * b0[i] + c0[i]);
                AssertClose(b[i], b0[i]);
                AssertClose(c[i], c0[i]);
            }
        }

        // ---- degenerate shapes ----

        private void SingleElementTest()
        {
            floatN v = GenerateOP.floatVec(1, (float)(-2.5), Allocator.Temp);
            v.absInPlace();
            AssertClose(v[0], (float)2.5);

            floatN a = GenerateOP.floatVec(1, (float)3, Allocator.Temp);
            floatN b = GenerateOP.floatVec(1, (float)4, Allocator.Temp);
            floatN c = GenerateOP.floatVec(1, (float)5, Allocator.Temp);
            a.madInPlace(b, c);
            AssertClose(a[0], (float)17); // 3*4 + 5

            floatN x = GenerateOP.floatVec(1, (float)0.7, Allocator.Temp);
            floatN s = new floatN(1, Allocator.Temp);
            floatN co = new floatN(1, Allocator.Temp);
            x.sincos(s, co);
            AssertClose(x[0], (float)0.7);
            AssertClose(s[0], math.sin((float)0.7));
            AssertClose(co[0], math.cos((float)0.7));
        }

        private void EmptyBufferTest()
        {
            // zero-length buffers: kernels loop zero times, must not touch memory / throw.
            floatN v = new floatN(0, Allocator.Temp);
            floatN y = new floatN(0, Allocator.Temp);
            v.absInPlace();
            v.sinInPlace();
            v.reluInPlace();
            v.minInPlace(y);
            v.powInPlace(2);
            Assert.AreEqual(0, v.N);
        }
    }

    public static Array GetEnums()
    {
        return Enum.GetValues(typeof(TestsJob.TestType));
    }

    [TestCaseSource("GetEnums")]
    public void Test(TestsJob.TestType type)
    {
        new TestsJob() { Type = type }.Run();
    }
}
