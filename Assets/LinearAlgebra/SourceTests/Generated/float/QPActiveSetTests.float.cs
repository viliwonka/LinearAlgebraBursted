// <auto-generated>
//   Generated from Assets/LinearAlgebra/CodeGen/TemplateSourceTests/fProxy/QPActiveSetTests.fProxy.cs
...
// (TemplateSource/AssemblyInfo.cs), the same route QPEqpTests.float.cs uses for QP.eqpSolve.
//
...
// back on the managed side -- the exact pattern QPEqpTests.float.cs / LPTests.float.cs use.
//
// ---- Acceptance oracles ----
//
...
public class floatQPActiveSetTests
{
    // ================================================================================================
    ...
    public struct HS21Job : IJob
    {
        ...
        public void Execute()
        {
            var Q = new floatMxN(2, 2, Allocator.Temp); Q[0, 0] = 0.02f; Q[1, 1] = 2f;
            var c = new floatN(2, Allocator.Temp);
            var A = new floatMxN(1, 2, Allocator.Temp); A[0, 0] = 10f; A[0, 1] = -1f;
            var b = new floatN(1, Allocator.Temp); b[0] = 10f;
            var senses = new NativeArray<ConstraintSense>(1, Allocator.Temp); senses[0] = ConstraintSense.GreaterEqual;
            var xl = new floatN(2, Allocator.Temp); xl[0] = 2f; xl[1] = -50f;
            var xu = new floatN(2, Allocator.Temp); xu[0] = 50f; xu[1] = 50f;
            var x = new floatN(2, Allocator.Temp); x[0] = 5f; x[1] = 10f;
    ...
    public struct HS35Job : IJob
    {
        ...
        public void Execute()
        {
            ...
            var c = new floatN(3, Allocator.Temp); c[0] = -8f; c[1] = -6f; c[2] = -4f;
            var A = new floatMxN(1, 3, Allocator.Temp); A[0, 0] = 1f; A[0, 1] = 1f; A[0, 2] = 2f;
            var b = new floatN(1, Allocator.Temp); b[0] = 3f;
            ...
            var xu = new floatN(3, Allocator.Temp); xu[0] = 1e30f; xu[1] = 1e30f; xu[2] = 1e30f;
            var x = new floatN(3, Allocator.Temp); x[0] = 0.5f; x[1] = 0.5f; x[2] = 0.5f;
    ...
    public struct HS52Job : IJob
    {
        ...
        public void Execute()
        {
            var Q = new floatMxN(5, 5, Allocator.Temp);
            Q[0, 0] = 32f; Q[0, 1] = -8f;
            Q[1, 0] = -8f; Q[1, 1] = 4f; Q[1, 2] = 2f;
            Q[2, 1] = 2f; Q[2, 2] = 2f;
            Q[3, 3] = 2f;
            Q[4, 4] = 2f;
            var c = new floatN(5, Allocator.Temp); c[1] = -4f; c[2] = -4f; c[3] = -2f; c[4] = -2f;
            ...
            var xl = new floatN(5, Allocator.Temp); for (int i = 0; i < 5; i++) xl[i] = -1e30f;
            var xu = new floatN(5, Allocator.Temp); for (int i = 0; i < 5; i++) xu[i] = 1e30f;
            var x = new floatN(5, Allocator.Temp);   // origin: all three equalities are homogeneous (RHS 0)
    ...
    public struct HS76Job : IJob
    {
        ...
        public void Execute()
        {
            ...
            Q[3, 2] = 1f; Q[3, 3] = 1f;
            var c = new floatN(4, Allocator.Temp); c[0] = -1f; c[1] = -3f; c[2] = 1f; c[3] = -1f;
            var A = new floatMxN(3, 4, Allocator.Temp);
            A[0, 0] = 1f; A[0, 1] = 2f; A[0, 2] = 1f; A[0, 3] = 1f;
            A[1, 0] = 3f; A[1, 1] = 1f; A[1, 2] = 2f; A[1, 3] = -1f;
            A[2, 1] = 1f; A[2, 2] = 4f;
            var b = new floatN(3, Allocator.Temp); b[0] = 5f; b[1] = 4f; b[2] = 1.5f;
            var senses = new NativeArray<ConstraintSense>(3, Allocator.Temp);
            senses[0] = ConstraintSense.LessEqual; senses[1] = ConstraintSense.LessEqual; senses[2] = ConstraintSense.GreaterEqual;
            var xl = new floatN(4, Allocator.Temp);
            var xu = new floatN(4, Allocator.Temp); for (int i = 0; i < 4; i++) xu[i] = 1e30f;
            var x = new floatN(4, Allocator.Temp); for (int i = 0; i < 4; i++) x[i] = 0.5f;
    ...
    public struct BruteForceJob : IJob
    {
        ...
        public void Execute()
        {
            ...
            var xEq = new floatN(n, Allocator.Temp, true);
            var Qcopy = new floatMxN(n, n, Allocator.Temp, true);
            var rhs = new floatN(n, Allocator.Temp, true);
            ...
            for (int combo = 0; combo < combos; combo++)
            {
                ...
                {
                    for (int i = 0; i < n; i++)
                        if ((double)xEq[i] < (double)xl[i] - 1e-4
                            || (double)xEq[i] > (double)xu[i] + 1e-4)
    ...
    public struct LpLimitJob : IJob
    {
        ...
        public void Execute()
        {
            ...
            var Ax0 = new floatN(m, Allocator.Temp);
            ...
            var Q = new floatMxN(n, n, Allocator.Temp);   // zero
            var xl = new floatN(n, Allocator.Temp);
            var xu = new floatN(n, Allocator.Temp); for (int i = 0; i < n; i++) xu[i] = 1e30f;

            var xQp = new floatN(n, Allocator.Temp);
            ...
            var xLp = new floatN(n, Allocator.Temp);
    ...
    public struct DegenerateJob : IJob
    {
        ...
        public void Execute()
        {
            ...
            for (int i = 0; i < n; i++) c[i] = rng.NextFloat(-1f, 1f);
            var xl = new floatN(n, Allocator.Temp);
            var xu = new floatN(n, Allocator.Temp); for (int i = 0; i < n; i++) xu[i] = 5f;
            var x0 = new floatN(n, Allocator.Temp); x0[0] = 2f; x0[1] = 2f; x0[2] = 2f;   // sum = 6, tight below

            // single copy
            var A1 = new floatMxN(1, n, Allocator.Temp); A1[0, 0] = 1f; A1[0, 1] = 1f; A1[0, 2] = 1f;
            var b1 = new floatN(1, Allocator.Temp); b1[0] = 6f;
            var senses1 = new NativeArray<ConstraintSense>(1, Allocator.Temp); senses1[0] = ConstraintSense.LessEqual;
            var x1 = new floatN(n, Allocator.Temp); x1[0] = x0[0]; x1[1] = x0[1]; x1[2] = x0[2];
            ...
            var A2 = new floatMxN(2, n, Allocator.Temp);
            A2[0, 0] = 1f; A2[0, 1] = 1f; A2[0, 2] = 1f;
            A2[1, 0] = 1f; A2[1, 1] = 1f; A2[1, 2] = 1f;
            var b2 = new floatN(2, Allocator.Temp); b2[0] = 6f; b2[1] = 6f;
            ...
            var x2 = new floatN(n, Allocator.Temp); x2[0] = x0[0]; x2[1] = x0[1]; x2[2] = x0[2];
    ...
    public struct UnboundedJob : IJob
    {
        ...
        public void Execute()
        {
            var Q = new floatMxN(2, 2, Allocator.Temp); Q[0, 0] = 1f; Q[1, 1] = 0f;
            var c = new floatN(2, Allocator.Temp); c[1] = -1f;
            var A = new floatMxN(0, 2, Allocator.Temp);
            var b = new floatN(0, Allocator.Temp);
            var senses = new NativeArray<ConstraintSense>(0, Allocator.Temp);
            var xl = new floatN(2, Allocator.Temp); xl[0] = -1e30f; xl[1] = 0f;
            var xu = new floatN(2, Allocator.Temp); xu[0] = 1e30f; xu[1] = 1e30f;
            var x = new floatN(2, Allocator.Temp);   // (0,0), feasible
    ...
    // ---- shared test-side helpers (Fail[]-array Burst diagnostic pattern, see QPEqpTests.float.cs) ----
    static class H
    {
