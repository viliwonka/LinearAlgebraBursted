...
public class floatSVDWorkspaceTests
{
    ...
    public struct WorkspaceEquivJob : IJob
    {
        ...
        void PinvEquiv(int m, int n)
        {
            ...
            var A0 = arena.floatRandomMatrix(m, n, -5f, 5f, 778231);
            for (int d = 0; d < k; d++)   // boost leading diagonal block for conditioning
                A0[d, d] += (float)10f;

            var b = arena.floatRandomVector(m, -5f, 5f, 9090);   // read-only in pinvSolve
            ...
            var M = arena.floatMat(k, k);
            floatMxN At = default;
            if (m < n)
                At = arena.floatMat(n, m);
            int rb = SVD.pinvSolve(ref Ab, in b, ref xb, out bool cb, (float)(-1), 30, ref S, ref M, ref At);
            ...
            var ws = arena.floatSvdWorkspace(m, n);
        ...
        void PseudoEquiv(int m, int n)
        {
            ...
            var A0 = arena.floatRandomMatrix(m, n, -5f, 5f, 314221);
            for (int d = 0; d < k; d++)
                A0[d, d] += (float)10f;
            ...
            // caller-scratch form
            var Ab = A0.Copy();
            var Pb = arena.floatMat(n, m);
            var S = arena.floatVec(k);
            var M = arena.floatMat(k, k);
            floatMxN At = default;
            if (m < n)
                At = arena.floatMat(n, m);
            int rb = SVD.pseudoInverse(ref Ab, ref Pb, out bool cb, (float)(-1), 30, ref S, ref M, ref At);
        ...
        void WorkspaceReuse()
        {
            ...
            var A0 = arena.floatRandomMatrix(m, n, -5f, 5f, 24681);
            for (int d = 0; d < n; d++)
                A0[d, d] += (float)10f;

            var ws = arena.floatSvdWorkspace(m, n);   // allocated ONCE, reused below

            for (int t = 0; t < 3; t++)
            {
                var b = arena.floatRandomVector(m, -5f, 5f, (uint)(1000 + t * 7));
    ...
    public void Pinv_BadScratchS_Throws()
    {
        ...
        try
        {
            var A = arena.floatMat(4, 3);     // tall, k = 3
            var b = arena.floatVec(4);
            var x = arena.floatVec(3);
            var S = arena.floatVec(2);        // must be length 3
            var M = arena.floatMat(3, 3);
            floatMxN At = default;
            Assert.Throws<ArgumentException>(
                () => SVD.pinvSolve(ref A, in b, ref x, out bool c, (float)(-1), 30, ref S, ref M, ref At));
        }
    ...
    public void Pinv_BadScratchM_Throws()
    {
        ...
        try
        {
            ...
            var M = arena.floatMat(3, 2);     // must be 3 x 3
            floatMxN At = default;
            Assert.Throws<ArgumentException>(
                () => SVD.pinvSolve(ref A, in b, ref x, out bool c, (float)(-1), 30, ref S, ref M, ref At));
        }
    ...
    public void Pinv_WideMissingAt_Throws()
    {
        ...
        try
        {
            var A = arena.floatMat(3, 5);     // wide, k = 3, needs At = 5 x 3
            var b = arena.floatVec(3);
            var x = arena.floatVec(5);
            var S = arena.floatVec(3);
            var M = arena.floatMat(3, 3);
    ...
    // Arena.floatSvdWorkspace(m, n) must size S (k), M (k x k), and At (n x m only when wide).
    [Test]
    ...
    public void PseudoInverse_WideBadAt_Throws()
    {
        ...
        {
            var A = arena.floatMat(3, 5);     // wide, k = 3, needs At = 5 x 3
            var Aplus = arena.floatMat(5, 3); // N_Cols x M_Rows
            var S = arena.floatVec(3);
            var M = arena.floatMat(3, 3);
            var At = arena.floatMat(3, 5);    // wrong shape (must be 5 x 3) -> must throw
            Assert.Throws<ArgumentException>(
                () => SVD.pseudoInverse(ref A, ref Aplus, out bool c, (float)(-1), 30, ref S, ref M, ref At));
        }
        finally { arena.Dispose(); }
    }
}
