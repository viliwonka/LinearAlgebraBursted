...
public class floatSVDWorkspaceTests
{
    ...
    public struct WorkspaceEquivJob : IJob
    {
        ...
        void PinvEquiv(int m, int n)
        {
            ...
            var A0 = GenerateOP.floatRandomMat(m, n, -5f, 5f, 778231, allocator: Allocator.Temp);
            for (int d = 0; d < k; d++)   // boost leading diagonal block for conditioning
                A0[d, d] += (float)10f;

            var b = GenerateOP.floatRandomVec(m, -5f, 5f, 9090, allocator: Allocator.Temp);   // read-only in pinvSolve
            ...
            var M = new floatMxN(k, k, Allocator.Temp);
            var U = new floatMxN(m < n ? n : m, k, Allocator.Temp);
            floatMxN At = default;
            if (m < n)
                At = new floatMxN(n, m, Allocator.Temp);
            ...
            Assert.IsTrue(ra == rb);
            Assert.IsTrue(ca == cb);
            var xDiffAB = new floatN(in xa, Allocator.Temp); xDiffAB.subInPlace(xb);
            ...
            var ws = new floatSVDCache(m, n, Allocator.Temp);
            ...
            Assert.IsTrue(rw == rb);
            Assert.IsTrue(cw == cb);
            var xDiffWB = new floatN(in xw, Allocator.Temp); xDiffWB.subInPlace(xb);
        ...
        void PseudoEquiv(int m, int n)
        {
            ...
            var A0 = GenerateOP.floatRandomMat(m, n, -5f, 5f, 314221, allocator: Allocator.Temp);
            for (int d = 0; d < k; d++)
                A0[d, d] += (float)10f;

            // allocating reference (Aplus is N_Cols x M_Rows = n x m)
            var Aa = A0.Copy();
            var Pa = new floatMxN(n, m, Allocator.Temp);
            RankInfo ia = SVD.pseudoInverse(ref Aa, ref Pa);
            int ra = ia.rank; bool ca = ia;

            // caller-scratch form
            var Ab = A0.Copy();
            var Pb = new floatMxN(n, m, Allocator.Temp);
            var S = new floatN(k, Allocator.Temp);
            var M = new floatMxN(k, k, Allocator.Temp);
            var U = new floatMxN(m < n ? n : m, k, Allocator.Temp);
            floatMxN At = default;
            ...
            Assert.IsTrue(ra == rb);
            Assert.IsTrue(ca == cb);
            var PDiffAB = new floatMxN(in Pa, Allocator.Temp); PDiffAB.subInPlace(Pb);
            Assert.IsTrue(Analysis.isZero(PDiffAB, Tol()));

            var Aw = A0.Copy();
            var Pw = new floatMxN(n, m, Allocator.Temp);
            var ws = new floatSVDCache(m, n, Allocator.Temp);
            ...
            Assert.IsTrue(rw == rb);
            Assert.IsTrue(cw == cb);
            var PDiffWB = new floatMxN(in Pw, Allocator.Temp); PDiffWB.subInPlace(Pb);
        ...
        void WorkspaceReuse()
        {
            ...
            var A0 = GenerateOP.floatRandomMat(m, n, -5f, 5f, 24681, allocator: Allocator.Temp);
            for (int d = 0; d < n; d++)
                A0[d, d] += (float)10f;

            var ws = new floatSVDCache(m, n, Allocator.Temp);   // allocated ONCE, reused below

            for (int t = 0; t < 3; t++)
            {
                var b = GenerateOP.floatRandomVec(m, -5f, 5f, (uint)(1000 + t * 7), allocator: Allocator.Temp);
                ...
                var xDiff = new floatN(in xa, Allocator.Temp); xDiff.subInPlace(xw);
    ...
    {
        var A = new floatMxN(4, 3, Allocator.Temp);     // tall, k = 3
        var b = new floatN(4, Allocator.Temp);
        var x = new floatN(3, Allocator.Temp);
        var S = new floatN(2, Allocator.Temp);        // must be length 3
        var M = new floatMxN(3, 3, Allocator.Temp);
        var U = new floatMxN(4, 3, Allocator.Temp);
        floatMxN At = default;
    ...
    public void Pinv_BadScratchM_Throws()
    {
        ...
        var M = new floatMxN(3, 2, Allocator.Temp);     // must be 3 x 3
        var U = new floatMxN(4, 3, Allocator.Temp);
        floatMxN At = default;
        Assert.Throws<ArgumentException>(
            () => SVD.pinvSolve(ref A, in b, ref x, (float)(-1), 30, ref S, ref M, ref U, ref At));
    ...
    {
        var A = new floatMxN(4, 3, Allocator.Temp);     // tall, k = 3, big = 4 -> U must be 4 x 3
        var b = new floatN(4, Allocator.Temp);
        var x = new floatN(3, Allocator.Temp);
        var S = new floatN(3, Allocator.Temp);
        var M = new floatMxN(3, 3, Allocator.Temp);
        var U = new floatMxN(3, 3, Allocator.Temp);     // wrong: must be 4 x 3
        floatMxN At = default;
        Assert.Throws<ArgumentException>(
            () => SVD.pinvSolve(ref A, in b, ref x, (float)(-1), 30, ref S, ref M, ref U, ref At));
    }
    ...
    {
        var A = new floatMxN(3, 5, Allocator.Temp);     // wide, k = 3, needs At = 5 x 3
        var b = new floatN(3, Allocator.Temp);
        var x = new floatN(5, Allocator.Temp);
        var S = new floatN(3, Allocator.Temp);
        var M = new floatMxN(3, 3, Allocator.Temp);
        var U = new floatMxN(5, 3, Allocator.Temp);
        floatMxN At = default;            // missing (0 x 0) -> must throw
        Assert.Throws<ArgumentException>(
            () => SVD.pinvSolve(ref A, in b, ref x, (float)(-1), 30, ref S, ref M, ref U, ref At));
    }

    // Standalone floatSVDCache(m, n, allocator) must size S (k), M (k x k), and At (n x m only when wide).
    [Test]
    public void SvdWorkspace_Factory_SizesCorrectly()
    {
        // tall: k = n, big = m; U = big x k = 7 x 4; At unused (left default).
        var wsTall = new floatSVDCache(7, 4, Allocator.Temp);
        ...
        var wsWide = new floatSVDCache(3, 8, Allocator.Temp);
    ...
    {
        var A = new floatMxN(3, 5, Allocator.Temp);     // wide, k = 3, needs At = 5 x 3
        var Aplus = new floatMxN(5, 3, Allocator.Temp); // N_Cols x M_Rows
        var S = new floatN(3, Allocator.Temp);
        var M = new floatMxN(3, 3, Allocator.Temp);
        var U = new floatMxN(5, 3, Allocator.Temp);
        var At = new floatMxN(3, 5, Allocator.Temp);    // wrong shape (must be 5 x 3) -> must throw
        Assert.Throws<ArgumentException>(
