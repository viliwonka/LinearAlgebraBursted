...
public class floatKrylovRound2Tests
{
    ...
    public struct KrylovRound2TestJob : IJob
    {
        ...
        void ApplyDotBSRSymExact()
        {
            ...
            builder.AddBlock(0, 1, arena.floatRandomMat(b, b, -1f, 1f, 70205));  // upper off-diagonals
            builder.AddBlock(1, 3, arena.floatRandomMat(b, b, -1f, 1f, 70206));
        ...
        static floatBSR BuildSymMultiBlock(ref Arena arena, int b, uint seed)
        {
            ...
            for (int i = 0; i < 5; i++)
                builder.AddBlock(i, i, SpdBlock(ref arena, b, seed + (uint)i + 1u));
            builder.AddBlock(0, 1, arena.floatRandomMat(b, b, -1f, 1f, seed + 11u));  // row0: diag + 3
            builder.AddBlock(0, 2, arena.floatRandomMat(b, b, -1f, 1f, seed + 12u));
            builder.AddBlock(0, 4, arena.floatRandomMat(b, b, -1f, 1f, seed + 13u));
            builder.AddBlock(1, 3, arena.floatRandomMat(b, b, -1f, 1f, seed + 14u));  // row1: diag + 2
            builder.AddBlock(1, 4, arena.floatRandomMat(b, b, -1f, 1f, seed + 15u));
            builder.AddBlock(2, 3, arena.floatRandomMat(b, b, -1f, 1f, seed + 16u));  // row2: diag + 1
