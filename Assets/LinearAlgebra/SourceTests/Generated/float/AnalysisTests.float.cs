...
public class floatAnalysisTests
{
    ...
    public struct AnalysisTestJob : IJob
    {
        ...
        void IsOrthogonal()
        {
            ...
            A = float_OP.dot(arena.floatPermutationMatrix(dim, 5, 13), A);

            Assert.IsTrue(Analysis_OP.IsOrthogonal(A, 0.00001f));

            A = float_OP.dot(arena.floatRotationMatrix(dim, 3, 15, math.PI/4f ), A);

            Assert.IsTrue(Analysis_OP.IsOrthogonal(A, 0.00001f));

            floatN reflect = arena.floatRandomVector(dim, -1f, 1f);

            A = float_OP.dot(arena.floatHouseholderMatrix(dim, reflect), A);
