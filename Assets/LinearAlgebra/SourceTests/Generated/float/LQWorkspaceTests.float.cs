...
public class floatLQWorkspaceTests
{
    ...
    public struct WorkspaceEquivJob : IJob
    {
        ...
        void LqDecompEquiv(int m, int n)
        {
            ...
            var Lb = new floatMxN(m, m, Allocator.Temp);
        ...
        {
            var A0 = GenerateOP.floatRandomMat(m, n, -5f, 5f, 61729);
            for (int d = 0; d < m; d++)
                A0[d, d] += (float)10f;

            var b = GenerateOP.floatRandomVec(m, -5f, 5f, 8080);
        ...
        void WorkspaceReuse()
        {
            ...
            var lqWs = new floatLQCache(m, n, Allocator.Temp);                     // allocated ONCE, reused below
            var solveWs = new floatLQMinNormCache(m, n, Allocator.Temp);      // allocated ONCE, reused below

            for (int t = 0; t < 3; t++)
            {
                var A0 = GenerateOP.floatRandomMat(m, n, -5f, 5f, (uint)(3000 + t * 13));
                for (int d = 0; d < m; d++)
                    A0[d, d] += (float)10f;
                ...
                var Aw = new floatMxN(in A0, Allocator.Temp);
                var Lw = new floatMxN(m, m, Allocator.Temp);
    ...
    {
        var A = new floatMxN(4, 8, Allocator.Temp);
        var L = new floatMxN(4, 4, Allocator.Temp);
        var Q = new floatMxN(4, 8, Allocator.Temp);
        var ws = new floatLQCache(4, 8, Allocator.Temp);
        ws.W = new floatMxN(3, 8, Allocator.Temp);   // wrong: must be m x n = 4 x 8
    ...
    public void LqDecomp_BadScratchV_Throws()
    {
        ...
        var Q = new floatMxN(4, 8, Allocator.Temp);
        var ws = new floatLQCache(4, 8, Allocator.Temp);
        ws.v = new floatN(5, Allocator.Temp);    // wrong: must be length n = 8
        Assert.Throws<ArgumentException>(() => LQ.decomp(in A, ref L, ref Q, ref ws));
    }
    ...
    public void LqMinNormSolve_BadScratchL_Throws()
    {
        ...
        var x = new floatN(8, Allocator.Temp);
        var ws = new floatLQMinNormCache(4, 8, Allocator.Temp);
        ws.L = new floatMxN(3, 3, Allocator.Temp);   // wrong: must be m x m = 4 x 4
        Assert.Throws<ArgumentException>(() => LQ.minNormSolve(in A, in b, ref x, ref ws));
    }

    // floatLQCache(m, n, allocator) / floatLQMinNormCache(m, n, allocator) must size every field for m x n.
    [Test]
    public void LQWorkspace_Factory_SizesCorrectly()
    {
        var lqWs = new floatLQCache(4, 8, Allocator.Temp);
        Assert.AreEqual(4, lqWs.W.M_Rows);
        Assert.AreEqual(8, lqWs.W.N_Cols);
        Assert.AreEqual(8, lqWs.v.N);

        var solveWs = new floatLQMinNormCache(4, 8, Allocator.Temp);
