...
// Milestone B test suite for the materialized-transpose optimization on floatBSM (block-CSR):
//   * Arena.floatBSMTranspose(in A)         -- builds A^T as its own compressed BSM
//   * floatBSMOperator(in A, in AT) two-arg  -- ApplyT forwards to spMV(AT, .) instead of spMVT(A, .)
//
...
public class floatSparseTransposeTests
{
    ...
    public struct SparseTransposeTestJob : IJob
    {
        ...
        // floatBSMTranspose. Only 4 of the 6 block positions are filled (leaving (1,0) and (2,1)
        // empty) so genuine block sparsity -- not a dense grid -- is transposed.
        static floatBSM BuildTallRectBSM(ref Arena arena)
        {
            const int blockRows = 3, blockCols = 2, BR = 3, BC = 2;
            var b = arena.floatBSMBuilder(blockRows, blockCols, BR, BC, 4);
            b.AddBlock(0, 0, arena.floatRandomMat(BR, BC, -1f, 1f, 70001));
            b.AddBlock(0, 1, arena.floatRandomMat(BR, BC, -1f, 1f, 70002));
            b.AddBlock(1, 1, arena.floatRandomMat(BR, BC, -1f, 1f, 70003));
            b.AddBlock(2, 0, arena.floatRandomMat(BR, BC, -1f, 1f, 70004));
        ...
        static floatBSM BuildWideRectBSM(ref Arena arena)
        {
            const int blockRows = 2, blockCols = 3, BR = 2, BC = 3;
            var b = arena.floatBSMBuilder(blockRows, blockCols, BR, BC, 4);
            b.AddBlock(0, 0, arena.floatRandomMat(BR, BC, -1f, 1f, 71001));
            b.AddBlock(0, 2, arena.floatRandomMat(BR, BC, -1f, 1f, 71002));
            b.AddBlock(1, 0, arena.floatRandomMat(BR, BC, -1f, 1f, 71003));
            b.AddBlock(1, 1, arena.floatRandomMat(BR, BC, -1f, 1f, 71004));
            return b.ToBSM(ref arena);
        }
        ...
        // outer dimensions.
        static void CrossCheckTranspose(ref Arena arena, in floatBSM A, uint seed)
        {
            var AT = arena.floatBSMTranspose(in A);
            ...
            var x = arena.floatRandomVec(A.M_Rows, -1f, 1f, seed);
        ...
        void OperatorApplyTParity()
        {
            ...
            var op1 = new floatBSMOperator(in A);        // one-arg: ApplyT == on-the-fly spMVT(A, .)
            var op2 = new floatBSMOperator(in A, in AT); // two-arg: ApplyT == spMV(AT, .)

            // ApplyT: y = A^T x, x length m = A.M_Rows, y length n = A.N_Cols. y must not alias x,
            // and each operator needs its own destination.
            var xT = arena.floatRandomVec(A.M_Rows, -1f, 1f, 73001);
            var yT1 = arena.floatVec(A.N_Cols);
            var yT2 = arena.floatVec(A.N_Cols);
            ...
            var xF = arena.floatRandomVec(A.N_Cols, -1f, 1f, 73002);
            var yF1 = arena.floatVec(A.M_Rows);
            var yF2 = arena.floatVec(A.M_Rows);
        ...
        //
        // For symmetric upper-block storage A == A^T by construction, so floatBSMTranspose returns
        ...
        void SymmetricNoOp()
        {
            ...
            for (int i = 0; i < nb; i++)
            {
                var Mi = arena.floatRandomMat(BR, BR, -1f, 1f, (uint)(74000 + i));
            ...
            var x = arena.floatRandomVec(dim, -1f, 1f, 74200);
