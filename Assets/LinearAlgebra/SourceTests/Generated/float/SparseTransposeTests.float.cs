...
//   * Arena.floatBSRTranspose(in A)         -- builds A^T as its own compressed BSR
...
public class floatSparseTransposeTests
{
    ...
    public struct SparseTransposeTestJob : IJob
    {
        ...
        static floatBSR BuildTallRectBSR(ref Arena arena)
        {
            const int blockRows = 3, blockCols = 2, BR = 3, BC = 2;
            var b = arena.floatBSRBuilder(blockRows, blockCols, BR, BC, 4);
        ...
        static floatBSR BuildWideRectBSR(ref Arena arena)
        {
        ...
        // outer dimensions.
        static void CrossCheckTranspose(ref Arena arena, in floatBSR A, uint seed)
        {
            var AT = arena.floatBSRTranspose(in A);
