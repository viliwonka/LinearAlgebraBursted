...
// ARENA-LEVEL wiring tests for the shortN/shortMxN -> ChunkedRecordTable migration. The table's
...
// ChunkedRecordTableTests.cs -- these tests prove the COMPOSITION: that Arena.shortVec/shortMat,
// shortN/shortMxN.Dispose()/Copy()/TempCopy(), and Arena.Clear()/ClearTemp()/Dispose() drive
...
public class shortArenaWiringTests
{
    ...
    public unsafe struct WiringTestJob : IJob
    {
        ...
        void MultiChunkVectors()
        {
            ...
            const int n = 300;
            const int len = 3;
            shortN* vecs = stackalloc shortN[n];

            for (int i = 0; i < n; i++)
            {
                shortN v = arena.shortVec(len, uninit: true);
                for (int j = 0; j < len; j++)
                    v[j] = (short)(i * len + j);   // distinct sentinel per (alloc, element)
                vecs[i] = v;
            }
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                // == against a (short)-cast expected, not Assert.AreEqual(int, (int)cell): the
                ...
                // same-typed values sidesteps it and matches the established short test idiom.
                for (int j = 0; j < len; j++)
                    Assert.IsTrue(vecs[i][j] == (short)(i * len + j));
            }
        ...
        // ---- multi-chunk: 300 persistent matrices, same argument on the shortMatRecords table ----
        void MultiChunkMatrices()
        {
            ...
            const int n = 300;
            const int R = 2, C = 2;
            shortMxN* mats = stackalloc shortMxN[n];

            for (int i = 0; i < n; i++)
            {
                shortMxN m = arena.shortMat(R, C, uninit: true);
                for (int r = 0; r < R; r++)
                    for (int c = 0; c < C; c++)
                        m[r, c] = (short)(i * (R * C) + r * C + c);
                mats[i] = m;
            }
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                for (int r = 0; r < R; r++)
                    for (int c = 0; c < C; c++)
                        Assert.IsTrue(mats[i][r, c] == (short)(i * (R * C) + r * C + c));
            }
        ...
        void TempRecyclingCycles()
        {
            ...
            const int N = 20;
            var seedV = arena.shortVec(4, (short)0);
            var seedM = arena.shortMat(3, 3, (short)0);
            ...
            for (int cycle = 0; cycle < 3; cycle++)
            {
                for (int i = 0; i < N; i++)
                {
                    ...
                    shortN tv = seedV.TempCopy();
                    tv[0] = (short)(cycle * 1000 + i);
                    Assert.IsTrue(tv[0] == (short)(cycle * 1000 + i));

                    shortMxN tm = seedM.TempCopy();
                    tm[0, 0] = (short)(cycle * 1000 + i);
                    Assert.IsTrue(tm[0, 0] == (short)(cycle * 1000 + i));
                }
        ...
        // shortN.Dispose(), and (b) that a following Clear()/Dispose() SKIPS the already-freed slot
        // (IsAlive guard) instead of double-freeing it -- no crash, no leak.
        void DirectDisposeCountsThenClearVectors()
        {
            var arena = new Arena(Allocator.Persistent);

            var a = arena.shortVec(4, (short)1);
            var b = arena.shortVec(4, (short)2);
            var c = arena.shortVec(4, (short)3);
            ...
            Assert.IsTrue(a[0] == (short)1);
            Assert.IsTrue(c[0] == (short)3);
        ...
        void DirectDisposeCountsThenClearMatrices()
        {
            ...
            var a = arena.shortMat(2, 2, (short)1);
            var b = arena.shortMat(2, 2, (short)2);
            var c = arena.shortMat(2, 2, (short)3);
            Assert.AreEqual(3, arena.AllocationsCount);

            b.Dispose();
            Assert.AreEqual(2, arena.AllocationsCount);

            Assert.IsTrue(a[0, 0] == (short)1);
            Assert.IsTrue(c[0, 0] == (short)3);
        ...
        void DisposeThenReallocateRecyclesSlot()
        {
            ...
            var v1 = arena.shortVec(4, uninit: true);
            for (int j = 0; j < 4; j++) v1[j] = (short)(100 + j);   // sentinel written into v1's buffer
            Assert.AreEqual(1, arena.AllocationsCount);

            v1.Dispose();                                            // frees the slot back to the free-list
            Assert.AreEqual(0, arena.AllocationsCount);

            var v2 = arena.shortVec(4, uninit: false);              // should recycle v1's freed slot
            Assert.AreEqual(1, arena.AllocationsCount);
            Assert.AreEqual(4, v2.N);
            for (int j = 0; j < 4; j++)
                Assert.IsTrue(v2[j] == (short)0);                    // zero-init: no stale sentinel bleed

            // And v2 is genuinely its own independent, writable buffer going forward.
            for (int j = 0; j < 4; j++) v2[j] = (short)(200 + j);
            for (int j = 0; j < 4; j++) Assert.IsTrue(v2[j] == (short)(200 + j));
    ...
    //
    // (An alias is REQUIRED to trip the guard: shortN.Dispose() nulls its own _rec, so calling
    ...
    public void AliasedDoubleDispose_Vector_Throws()
    {
        ...
        try
        {
            var v = arena.shortVec(8, (short)0);
            var alias = v;                 // struct copy -- shares v's shortVecRecord*
            Assert.AreEqual(1, arena.AllocationsCount);
    ...
    public void AliasedDoubleDispose_Matrix_Throws()
    {
        ...
        try
        {
            var m = arena.shortMat(3, 4, (short)0);
            var alias = m;
            Assert.AreEqual(1, arena.AllocationsCount);
    ...
    // InvalidOperationException instead of silently returning a dead/garbage buffer. shortN is
    // option (b) (Alive-only: the 32B struct has no spare padding for a generation stamp); shortMxN is
    ...
    // (b) VECTOR read-after-Dispose. shortN.Dispose() nulls the DISPOSER's OWN _rec, so the stale read
    ...
    public void Vector_ReadAfterDispose_Throws()
    {
        ...
        try
        {
            var v = arena.shortVec(4, (short)0);
    ...
    public void Vector_ReadAfterClearTemp_Throws()
    {
        ...
        try
        {
            var seed = arena.shortVec(4, (short)0);
    ...
    // shortMxN's Alive+generation guard. Same alias-for-dispose requirement (mutable struct).
    [Test]
    ...
    public void Matrix_ReadAfterClearTemp_Throws()
    {
        ...
        try
        {
            var seed = arena.shortMat(3, 3, (short)0);
    ...
    // again for the new occupant, but shortMxN's _gen (stamped at v1's construction) no longer matches
    ...
    public void Matrix_StaleGenerationAfterSlotRecycle_OldThrows_NewWorks()
    {
        ...
        try
        {
            var v1 = arena.shortMat(2, 3, (short)0);
            ...
            var v2 = arena.shortMat(2, 3, (short)0);  // recycles v1's EXACT slot; generation bumped
            Assert.AreEqual(1, arena.AllocationsCount);
