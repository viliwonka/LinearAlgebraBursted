...
// kernel (mathUnsafeshort.clamp) uses a `x > max ? max : x < min ? min : x` ternary chain
...
public class shortClampTests
{
    ...
    public struct ClampTestJob : IJob
    {
        ...
        void ClampVector()
        {
            ...
            shortOP.clampInpl(in v, (short)(-2), (short)5);
        ...
        void ClampMatrix()
        {
            ...
            shortOP.clampInpl(in A, (short)0, (short)10);
        ...
        void ClampNoOpInRange()
        {
            ...
            shortOP.clampInpl(in v, (short)0, (short)5);
            for (int i = 0; i < 4; i++)
    ...
    // regression test (the production fix is to make mathUnsafeshort.clamp use
    ...
    public void ClampLoGreaterThanHiCollapsesToLo()
    {
        ...
        shortOP.clampInpl(in v, (short)6, (short)(-1)); // lo > hi
        for (int i = 0; i < 3; i++)
            Assert.AreEqual((short)6, v[i], $"index {i}: lo>hi must collapse to lo (6)");
