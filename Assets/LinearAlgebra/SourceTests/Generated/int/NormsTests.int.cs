...
public class intNormsTests
{
    ...
    public struct NormsTestJob : IJob
    {
        ...
        // ABS-OVERFLOW EDGE: single element == int.MinValue. The kernel widens each element to
        ...
        //     WRONG answer; see NormsOP.int.cs's LInf doc comment), so it also surfaces ->
        ...
        void MinValueAbsOverflow(ref Arena arena)
        {
            ...
            v[0] = (int)int.MinValue;

            long expectedL1 = 2147483648L;
            long expectedLInf = 2147483648L;

            Assert.IsTrue(Norms.L1(in v) == expectedL1);
            Assert.IsTrue(Norms.LInf(in v) == expectedLInf);
        }

        // MIXED-INPUT contrast (see NormsOP.int.cs's L1/LInf doc comments for the full
