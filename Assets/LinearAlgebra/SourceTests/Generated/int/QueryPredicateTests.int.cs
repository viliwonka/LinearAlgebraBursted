...
// Tests for the integer scalar-predicate subset of the QueryOP extension (intQueryOP, Group A
...
public class intQueryPredicateTests
{
    struct GreaterThanInt : IfintPredicate
    {
        public int t;
        public bool Test(int x) => x > t;
    }
    ...
    public struct TestJob : IJob
    {
        ...
        void GroupAScalar()
        {
            ...
            var pass = new GreaterThanInt { t = (int)2 };
            AssertEqI(intQueryOP.findFirst(in v, ref pass), 2);
            AssertEqI(intQueryOP.count(in v, ref pass), 2);
            AssertTrue(intQueryOP.any(in v, ref pass));
            // not all > 2 (e.g. the -2 fails) -> all == false.
            AssertTrue(!intQueryOP.all(in v, ref pass));

            var idx = arena.Indices(6);
            int fc = intQueryOP.findAll(in v, ref pass, ref idx);
            AssertEqI(fc, 2);
            AssertEqI(idx[0], 2); AssertEqI(idx[1], 4);
            // findAll count == count.
            AssertEqI(fc, intQueryOP.count(in v, ref pass));

            // No element matches -> findFirst -1, count 0, any false, findAll 0.
            var none = new GreaterThanInt { t = (int)100 };
            AssertEqI(intQueryOP.findFirst(in v, ref none), -1);
            AssertEqI(intQueryOP.count(in v, ref none), 0);
            AssertTrue(!intQueryOP.any(in v, ref none));
            AssertEqI(intQueryOP.findAll(in v, ref none, ref idx), 0);

            // Every element passes -> all true, any true.
            var allPass = new GreaterThanInt { t = (int)(-10) };
            AssertTrue(intQueryOP.all(in v, ref allPass));
            AssertTrue(intQueryOP.any(in v, ref allPass));

            // Empty vector: findFirst -1, count 0, any false, all true (vacuous), findAll 0.
            var v0 = arena.intVec(0);
            AssertEqI(intQueryOP.findFirst(in v0, ref pass), -1);
            AssertEqI(intQueryOP.count(in v0, ref pass), 0);
            AssertTrue(!intQueryOP.any(in v0, ref pass));
            AssertTrue(intQueryOP.all(in v0, ref pass));
            var idx0 = arena.Indices(1);
            AssertEqI(intQueryOP.findAll(in v0, ref pass, ref idx0), 0);
            ...
            AssertEqI(intQueryOP.findFirst(in A, ref matPass), 1);
            AssertEqI(intQueryOP.count(in A, ref matPass), 2);
            var idxM = arena.Indices(4);
            int mc = intQueryOP.findAll(in A, ref matPass, ref idxM);
            AssertEqI(mc, 2);
    ...
    public void FindAllUndersizedThrows()
    {
        ...
        Assert.Throws<ArgumentException>(() => intQueryOP.findAll(in v, ref gt, ref small));
        arena.Dispose();
    }
}
