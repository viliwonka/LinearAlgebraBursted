...
// ARENA-LEVEL wiring tests for the intN/intMxN -> ChunkedRecordTable migration. The table's
...
public class intArenaWiringTests
{
    ...
    // InvalidOperationException instead of silently returning a dead/garbage buffer. intN is
    // option (b) (Alive-only: the 32B struct has no spare padding for a generation stamp); intMxN is
    ...
    // (b) VECTOR read-after-Dispose. intN.Dispose() nulls the DISPOSER's OWN _rec, so the stale read
    ...
    // intMxN's Alive+generation guard. Same alias-for-dispose requirement (mutable struct).
    [Test]
    ...
    public void Matrix_ReadAfterClearTemp_Throws()
    {
        ...
        try
        {
            var seed = arena.intMat(3, 3, (int)0);
    ...
    // again for the new occupant, but intMxN's _gen (stamped at v1's construction) no longer matches
    ...
    public void Matrix_StaleGenerationAfterSlotRecycle_OldThrows_NewWorks()
    {
        ...
        try
        {
            ...
            var v2 = arena.intMat(2, 3, (int)0);  // recycles v1's EXACT slot; generation bumped
            Assert.AreEqual(1, arena.AllocationsCount);
