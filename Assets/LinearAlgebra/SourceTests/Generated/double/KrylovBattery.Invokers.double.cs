...
namespace LinearAlgebra
{
    ...
    /// <summary>
    /// <see cref="IdoubleBlockSolverInvoker"/> for <see cref="Krylov.bgcrodr{TOp, TPre}"/> -- block
    ...
    /// as <see cref="doubleBgmresInvoker"/> (including <see cref="NeedsGeneralDenseOperator"/> and the
    /// IllConditioned exclusion -- it shares bgmres's block Arnoldi/Hessenberg machinery, same
    /// rationale). <see cref="ScalarCounterpart"/> is <see cref="doubleGcrodrInvoker"/> (same recycled-
    /// subspace family, not plain gmres).
    /// </summary>
    public struct doubleBgcrodrInvoker : IdoubleBlockSolverInvoker
    {
        public double TolValue;
        public int MaxIterMul;
        public int Restart;
        ...
        public IdoubleSquareSolverInvoker ScalarCounterpart() => new doubleGcrodrInvoker { TolValue = TolValue, MaxIterMul = MaxIterMul, Restart = Restart, Recycle = Recycle };
    }
    ...
    /// <summary>
    /// <see cref="IdoubleLstsqSolverInvoker"/> for <see cref="Krylov.lnlq{TOp}"/> -- least-NORM
    /// (Estrin-Orban-Saunders) solver for UNDERDETERMINED (Rows &lt;= Cols, full row rank) CONSISTENT
    /// systems. Returns the SAME minimum-‖x‖ iterate as <see cref="doubleCraigInvoker"/> (LNLQ's
    ...
    /// </summary>
    public struct doubleLnlqInvoker : IdoubleLstsqSolverInvoker
    {
        public double TolValue;
        public int MaxIterMul;
    ...
    /// <summary>
    /// <see cref="IdoubleBlockLstsqSolverInvoker"/> for <see cref="Krylov.bcraig{TOp}"/> -- block CRAIG
    ...
    /// Euclidean-norm one. Block min-NORM counterpart to <see cref="doubleBlsmrInvoker"/>/
    /// <see cref="doubleBcglsInvoker"/> (which are OVERDETERMINED / min-RESIDUAL) -- exactly the block
    /// analog of what <see cref="doubleCraigInvoker"/> is to <see cref="doubleLsmrInvoker"/>. Owns no
    /// scratch (bcraig allocates its whole workspace from Allocator.Temp, mirroring
    /// <see cref="doubleBlsmrInvoker"/>), so <see cref="Init"/> is a no-op. <see cref="ScalarCounterpart"/>
    /// is <see cref="doubleCraigInvoker"/> -- bcraig finds the same minimum-norm solution craig does, one
    /// column at a time. Requires Underdetermined and Forbids RankDeficient, mirroring
    /// <see cref="doubleCraigInvoker"/>.
    /// </summary>
    public struct doubleBcraigInvoker : IdoubleBlockLstsqSolverInvoker
    {
        public double TolValue;
        public int MaxIterMul;
