// <auto-generated>
//   Generated from Assets/LinearAlgebra/CodeGen/TemplateSourceTests/fProxy/SpecialConstructorsTests.fProxy.cs
//   DO NOT EDIT BY HAND - edit the template and run Tools/regen.ps1.
// </auto-generated>
using BULA;
using NUnit.Framework;

using System.Diagnostics;

using Unity.Mathematics;
using Unity.Burst;
using Unity.Collections;

using Unity.Jobs;

public class doubleSpecialConstructorsTests {

    [BurstCompile(CompileSynchronously = true, FloatPrecision = FloatPrecision.High)] 
    public struct TestJob : IJob
    {
        public enum TestType
        {
            BasisVec,
            IndexZeroVec,
            IndexOneVec,
            RandomUnitVec,
            RandomVec,
            LinVec,

            IdentityMat,
            DiagonalMat,
            RandomDiagonalMat,
            IndexZeroMat,
            IndexOneMat,
            RandomMat,
            RandomRangeMat,
            RotationMat,
            PermutationMat,
            HouseholderMat,

        }

        public TestType TType;

        public void Execute()
        {
            switch(TType)
            {
                case TestType.BasisVec:
                    BasisVec();
                    break;
                case TestType.IndexZeroVec:
                    IndexZeroVec();
                    break;
                case TestType.IndexOneVec:
                    IndexOneVec();
                    break;
                case TestType.RandomUnitVec:
                    RandomUnitVec();
                    break;
                case TestType.RandomVec:
                    RandomVec();
                    break;
                case TestType.LinVec:
                    LinVec();
                    break;

                case TestType.IdentityMat:
                    IdentityMat();
                    break;
                case TestType.DiagonalMat:
                    DiagonalMat();
                    break;
                case TestType.RandomDiagonalMat:
                    RandomDiagonalMat();
                    break;
                case TestType.IndexZeroMat:
                    IndexZeroMat();
                    break;
                case TestType.IndexOneMat:
                    IndexOneMat();
                    break;
                case TestType.RandomMat:
                    RandomMat();
                    break;
                case TestType.RandomRangeMat:
                    RandomRangeMat();
                    break;
                case TestType.RotationMat:
                    RotationMat();
                    break;
                case TestType.PermutationMat:
                    PermutationMat();
                    break;
                case TestType.HouseholderMat:
                    HouseholderMat();
                    break;
            }
        }

        public void BasisVec()
        {
            var v = GenerateOP.doubleBasisVec(10, 0);

            Assert.IsTrue(v[0] == (double)1);

            for(int i = 1; i < v.N; i++) {
                Assert.IsTrue(v[i] == (double)0);
            }

            v = GenerateOP.doubleBasisVec(10, 9);

            Assert.IsTrue(v[9] == (double)1);

            for(int i = 0; i < v.N - 1; i++) {
                Assert.IsTrue(v[i] == (double)0);
            }
        }

        public void IndexZeroVec()
        {
            var v = GenerateOP.doubleIndexZeroVec(16);

            for(int i = 0; i < v.N; i++) {
                Assert.IsTrue(v[i] == (double)i);
            }
        }

        public void IndexOneVec()
        {
            var v = GenerateOP.doubleIndexOneVec(16);

            for (int i = 0; i < v.N; i++) {
                Assert.IsTrue(v[i] == (double)i + 1);
            }
        }

        public void RandomUnitVec()
        {
            for (uint seed = 0; seed < 16; seed++)
            {
                var v = GenerateOP.doubleRandomUnitVec(16, 332*seed+17);

                var len = Norms.L2(in v);

                Assert.IsTrue(Unity.Mathematics.math.abs(len - (double)1) <= 0.00001f);
            }
        }

        public void RandomVec()
        {
            for (uint seed = 0; seed < 16; seed++)
            {
                var v = GenerateOP.doubleRandomVec(16, -3f, 3f, 351*seed+19);

                for (int i = 0; i < v.N; i++)
                    Assert.IsFalse(v[i] < -(double)3 || v[i] > (double)3);
            }
        }

        public void LinVec()
        {
            var v = GenerateOP.doubleLinVec(16, (double)0, (double)15);

            for (int i = 0; i < v.N; i++)
                Assert.IsTrue(math.abs(i- v[i]) < 0.0001f);

            v = GenerateOP.doubleLinVec(16, 15, 0);

            for (int i = 0; i < v.N; i++)
                Assert.IsTrue(math.abs((15f-i) - v[i]) < 0.0001f);
        }

        public void IndexZeroMat()
        {
            var m = GenerateOP.doubleIndexZeroMat(16, 16);

            for(int i = 0; i < m.Length; i++)
                Assert.IsTrue(m[i] == (double)i);
        }

        public void IndexOneMat()
        {
            var m = GenerateOP.doubleIndexOneMat(16, 16);

            for (int i = 0; i < m.Length; i++)
                Assert.IsTrue(m[i] == (double)i + 1);
        }

        public void IdentityMat()
        {
            var m = GenerateOP.doubleIdentityMat(16);

            Assert.IsTrue(Analysis.isDiagonal(in m));
            Assert.IsTrue(Analysis.isIdentity(in m));

            for (int i = 0; i < m.M_Rows; i++)
            for(int j = 0; j < m.N_Cols; j++)
            {
                if(i == j)
                    Assert.IsTrue(m[i, j] == (double)1);
                else
                    Assert.IsTrue(m[i, j] == (double)0);
            }
        }

        public void DiagonalMat()
        {
            var m = GenerateOP.doubleDiagonalMat(16, 2f);

            Assert.IsTrue(Analysis.isDiagonal(in m));

            for (int i = 0; i < m.M_Rows; i++)
            for (int j = 0; j < m.N_Cols; j++)
            {
                if (i == j)
                    Assert.IsTrue(m[i, j] == (double)2);
                else
                    Assert.IsTrue(m[i, j] == (double)0);
            }
        }

        public void RandomDiagonalMat()
        {
            var m = GenerateOP.doubleRandomDiagonalMat(16, -3f, 3f);

            Assert.IsTrue(Analysis.isDiagonal(in m));

            for (int i = 0; i < m.M_Rows; i++)
            for (int j = 0; j < m.N_Cols; j++)
            {
                if (i == j)
                    Assert.IsFalse(m[i, j] < -3f || m[i, j] > 3f);
                else
                    Assert.IsTrue(m[i, j] == (double)0);
            }
        }

        public void RandomMat()
        {
            var m = GenerateOP.doubleRandomMat(16, 16);

            for (int i = 0; i < m.M_Rows; i++)
            for (int j = 0; j < m.N_Cols; j++)
                Assert.IsFalse(m[i, j] < -1f || m[i, j] > 1f);
        }

        public void RandomRangeMat()
        {
            var m = GenerateOP.doubleRandomMat(16, 16, -6f, 6f);

            for (int i = 0; i < m.M_Rows; i++)
            for (int j = 0; j < m.N_Cols; j++)
                Assert.IsFalse(m[i, j] < -6f || m[i, j] > 6f);
        }

        public void RotationMat()
        {
            var m = GenerateOP.doubleRotationMat(16, 1, 14, math.PI/4f);

            Assert.IsTrue(Analysis.isOrthogonal(in m, 0.00001f));
            Assert.IsFalse(Analysis.isIdentity(in m, 0.00001f));

            var mTm = Blas.dot(m, m, true);
            Assert.IsTrue(Analysis.isIdentity(in mTm, 0.00001f));

            m = GenerateOP.doubleRotationMat(2, 0, 1, math.PI/4f);

            Assert.IsTrue(math.abs((double)0.70710678118654752440084436210485d - m[0, 0]) < 0.00001f);
            Assert.IsTrue(math.abs((double)0.70710678118654752440084436210485d - m[1, 1]) < 0.00001f);
            Assert.IsTrue(math.abs((double)(-0.70710678118654752440084436210485d) - m[0, 1]) < 0.00001f);
            Assert.IsTrue(math.abs((double)0.70710678118654752440084436210485d - m[1, 0]) < 0.00001f);
        }

        public void PermutationMat()
        {
            var m = GenerateOP.doublePermutationMat(16, 1, 14);

            Assert.IsTrue(Analysis.isOrthogonal(in m, 0.00001f));
            Assert.IsFalse(Analysis.isIdentity(in m, 0.00001f));

            var mTm = Blas.dot(m, m, true);
            Assert.IsTrue(Analysis.isIdentity(in mTm, 0.00001f));

            m = GenerateOP.doublePermutationMat(2, 0, 1);

            Assert.IsTrue(m[0, 0] == (double)0);
            Assert.IsTrue(m[1, 1] == (double)0);
            Assert.IsTrue(m[0, 1] == (double)1);
            Assert.IsTrue(m[1, 0] == (double)1);
        }

        public void HouseholderMat()
        {
            var v = GenerateOP.doubleRandomUnitVec(16);
            var m = GenerateOP.doubleHouseholderMat(16, v);

            Assert.IsTrue(Analysis.isOrthogonal(in m, 0.00001f));
            Assert.IsFalse(Analysis.isIdentity(in m, 0.00001f));

            var mTm = Blas.dot(m, m, true);
            Assert.IsTrue(Analysis.isIdentity(in mTm, 0.00001f));

            v = GenerateOP.doubleBasisVec(2, 0);
            m = GenerateOP.doubleHouseholderMat(2, v);
        }
    }

    [Test]
    public void BasisVec()
    {
        new TestJob() { TType = TestJob.TestType.BasisVec }.Run();
    }

    [Test]
    public void IndexZeroVec()
    {
        new TestJob() { TType = TestJob.TestType.IndexZeroVec }.Run();
    }
    [Test]
    public void IndexOneVec()
    {
        new TestJob() { TType = TestJob.TestType.IndexOneVec }.Run();
    }

    [Test]
    public void RandomUnitVec()
    {
        new TestJob() { TType = TestJob.TestType.RandomUnitVec }.Run();
    }

    [Test]
    public void RandomVec()
    {
        new TestJob() { TType = TestJob.TestType.RandomVec }.Run();
    }

    [Test]
    public void LinVec()
    {
        new TestJob() { TType = TestJob.TestType.LinVec }.Run();
    }

    [Test]
    public void IdentityMat()
    {
        new TestJob() { TType = TestJob.TestType.IdentityMat }.Run();
    }

    [Test]
    public void DiagonalMat()
    {
        new TestJob() { TType = TestJob.TestType.DiagonalMat }.Run();
    }

    [Test]
    public void RandomDiagonalMat()
    {
        new TestJob() { TType = TestJob.TestType.RandomDiagonalMat }.Run();
    }

    [Test]
    public void IndexZeroMat()
    {
        new TestJob() { TType = TestJob.TestType.IndexZeroMat }.Run();
    }

    [Test]
    public void IndexOneMat()
    {
        new TestJob() { TType = TestJob.TestType.IndexOneMat }.Run();
    }

    [Test]
    public void RandomMat()
    {
        new TestJob() { TType = TestJob.TestType.RandomMat }.Run();
    }

    [Test]
    public void RandomRangeMat()
    {
        new TestJob() { TType = TestJob.TestType.RandomRangeMat }.Run();
    }

    [Test]
    public void RotationMat()
    {
        new TestJob() { TType = TestJob.TestType.RotationMat }.Run();
    }

    [Test]
    public void PermutationMat()
    {
        new TestJob() { TType = TestJob.TestType.PermutationMat }.Run();
    }

    [Test]
    public void HouseholderMat()
    {
        new TestJob() { TType = TestJob.TestType.HouseholderMat }.Run();
    }

    
}
