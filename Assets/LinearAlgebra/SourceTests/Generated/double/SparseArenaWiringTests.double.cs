...
//     SAME-COPY double-dispose THROWS from the table's double-Free guard (unlike doubleN/doubleBSR
...
//   * doubleBSR IS non-readonly and its Dispose() DOES null _rec (doubleBSR.cs), so a BSR same-copy
//     re-dispose is a safe no-op like doubleN; only an ALIASED BSR double-dispose throws.
...
//     first (`var rp = A.RowPtr; rp[0] = ...;`), exactly as doubleBSRBuilder.ToBSRCore does.
//
...
public class doubleSparseArenaWiringTests
{
    ...
    public unsafe struct WiringTestJob : IJob
    {
        ...
        static void FillDiagBlock(ref doubleBSR A, int sz, double scale)
        {
            ...
            for (int r = 0; r < sz; r++)
                for (int c = 0; c < sz; c++)
                    vals[r * sz + c] = (r == c) ? scale : (double)0;
        }
        ...
        void MultiAllocBSRs()
        {
            ...
            const int n = 30;
            doubleBSR* mats = stackalloc doubleBSR[n];

            for (int i = 0; i < n; i++)
            {
                int sz = 1 + (i % 6);                          // block sizes 1..6
                var A = arena.doubleBSR(1, 1, sz, sz, 1, uninit: true);
                FillDiagBlock(ref A, sz, (double)(i + 1));     // distinct per-alloc scale sentinel
                mats[i] = A;
            }
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                var x = new doubleN(sz, Allocator.Temp, uninit: true);
                for (int j = 0; j < sz; j++) x[j] = (double)1;
                var y = new doubleN(sz, Allocator.Temp);       // zero-initialized
                BSR.spMV(in mats[i], in x, ref y);

                // Diagonal (i+1)*I applied to ones -> every entry == i+1.
                for (int j = 0; j < sz; j++)
                    Assert.IsTrue(math.abs(y[j] - (double)(i + 1)) < Tol());
        ...
        void DisposeThenReallocateRecyclesSlot()
        {
            ...
            const int sz = 2;
            var v1 = arena.doubleBSR(1, 1, sz, sz, 1, uninit: true);
            FillDiagBlock(ref v1, sz, (double)777);            // sentinel written into v1's Values
            ...
            var v2 = arena.doubleBSR(1, 1, sz, sz, 1, uninit: false); // should recycle v1's freed slot
            Assert.AreEqual(1, arena.AllocationsCount);
            ...
            FillDiagBlock(ref v2, sz, (double)3);              // 3*I
            var x = new doubleN(sz, Allocator.Temp, uninit: true);
            for (int j = 0; j < sz; j++) x[j] = (double)1;
            var y = new doubleN(sz, Allocator.Temp);
            BSR.spMV(in v2, in x, ref y);
            for (int j = 0; j < sz; j++)
                Assert.IsTrue(math.abs(y[j] - (double)3) < Tol());
        ...
        void StandaloneBSRAndBlockJacobi()
        {
            ...
            var A = new doubleBSR(2, 2, sz, sz, 2, Allocator.Temp, uninit: true);
            ...
            {
                int o = b * sz * sz;
                vals[o + 0] = (double)2; vals[o + 1] = (double)1;
                vals[o + 2] = (double)0; vals[o + 3] = (double)2;
            }
            ...
            var x = new doubleN(4, Allocator.Temp, uninit: true);
            x[0] = (double)1; x[1] = (double)2; x[2] = (double)3; x[3] = (double)4;
            var y = new doubleN(4, Allocator.Temp);
            BSR.spMV(in A, in x, ref y);
            Assert.IsTrue(math.abs(y[0] - (double)4) < Tol());
            Assert.IsTrue(math.abs(y[1] - (double)4) < Tol());
            Assert.IsTrue(math.abs(y[2] - (double)10) < Tol());
            Assert.IsTrue(math.abs(y[3] - (double)8) < Tol());
        ...
        void BuilderToBSRSeam()
        {
            ...
            const int sz = 2;
            var builder = arena.doubleBSRBuilder(2, 2, sz, sz);

            var d = arena.doubleMat(sz, sz);
            d[0, 0] = (double)2; d[0, 1] = (double)1;
            d[1, 0] = (double)0; d[1, 1] = (double)2;
    ...
    // ([[2,1],[1,2]] is SPD) -- so doubleBlockJacobi construction succeeds. Managed helper.
    static doubleBSR BuildSquareInvertible(ref Arena arena)
    {
        const int BR = 2, BC = 2;
        var builder = arena.doubleBSRBuilder(2, 2, BR, BC);
        var d = arena.doubleMat(BR, BC);
        d[0, 0] = (double)2; d[0, 1] = (double)1;
        d[1, 0] = (double)1; d[1, 1] = (double)2;
        builder.AddBlock(0, 0, in d);
        builder.AddBlock(1, 1, in d);
        return builder.ToBSR(ref arena);
    }

    // ---- 1. ALIASED double-dispose of an arena-tracked doubleBSR ---------------------------------
    // Two struct copies share the SAME doubleBSRRecord*. Disposing the first frees the slot AND
    ...
    public void AliasedDoubleDispose_BSR_Throws()
    {
        ...
        try
        {
            ...
            var alias = A;                             // struct copy -- shares A's doubleBSRRecord*
    ...
    // A BSR is NON-readonly and its Dispose() nulls _rec (doubleBSR.cs), so disposing the SAME
    // variable twice is a safe no-op (the second call takes the standalone branch and disposes
    // default UnsafeLists) -- exactly like doubleN. Pins that doubleBSR does NOT share
    // doubleBlockJacobi's readonly same-copy-throws behavior.
    [Test]
    ...
    // ---- 2. doubleBlockJacobi double-dispose: SAME-COPY throws (readonly struct) + ALIASED throws --
    ...
    // An ALIASED doubleBlockJacobi double-dispose throws for the same double-Free-guard reason (this
    // half matches doubleN/doubleBSR; the same-copy half above is where they diverge).
    [Test]
    ...
    // instead of silently returning a dead/garbage buffer. doubleBSR is option (c): its
    // RowPtr/ColInd/Values getters SHARE one AssertRecordValid() (Alive + a free `_gen` stamp in the
    // struct's internal padding hole), so it ADDITIONALLY catches the recycled-slot case. doubleBlockJacobi
    // is option (b) (Alive-only: no spare padding), guarding its DInv getter.
    //
    // NO ClearTemp case here for EITHER type: neither doubleBSR nor doubleBlockJacobi has a temp-pool
    // counterpart (no doubleTempBSR / no ClearTemp-tracked BlockJacobi pool -- see Arena.Sparse.double.cs),
    ...
    // (c) BSR read-after-Dispose. doubleBSR is a mutable struct and Dispose() nulls the DISPOSER's OWN
    ...
    public void BSR_ReadAfterDispose_Throws()
    {
        ...
        try
        {
            var A = arena.doubleBSR(1, 1, 2, 2, 1);
    ...
    // read true again for the new occupant, but doubleBSR's _gen (stamped at v1's construction) no
    ...
    public void BSR_StaleGenerationAfterSlotRecycle_OldThrows_NewWorks()
    {
        ...
        try
        {
            var v1 = arena.doubleBSR(1, 1, 2, 2, 1);
            ...
            var v2 = arena.doubleBSR(1, 1, 2, 2, 1);   // recycles v1's EXACT slot; generation bumped
            Assert.AreEqual(1, arena.AllocationsCount);
    ...
    // (b) BLOCKJACOBI read-after-Dispose. doubleBlockJacobi is a READONLY struct, so Dispose() CANNOT
    // null its _rec afterward -- the SAME variable's _rec still points at the freed slot, so reading its
    // DInv getter throws with NO alias needed (unlike the mutable doubleN/doubleBSR families). This is
