...
// the new powerIteration(in doubleBSR, ...) overloads forward through doubleBSROperator into the
...
public class doubleSparseEigenTests
{
    ...
    public struct SparseEigenTestJob : IJob
    {
        ...
        // doubleSparseSolverTests.DenseToBSR1x1). nnzHint bounds the known nonzero pattern purely as
        ...
        // dense (doubleMxN) and BSR (doubleBSR) forwarders. Same fixture philosophy as the
        ...
        void LanczosFullSpectrum()
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                AssertClose(eigBsr[i], (double)lamD, FullSpectrumTol() * scale);
        ...
        void LanczosVectorsClosedFormLaplacian()
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                double lamD = 2.0 - 2.0 * math.cos(k * math.PI_DBL / (n + 1));
                double scale = (double)1 + math.abs((double)lamD);
                AssertClose(eig[i], (double)lamD, VecTol() * scale);
                ...
                double inv = (double)(1.0 / math.sqrt(nrm));
    ...
    // dense (in doubleMxN) and BSR (in doubleBSR) entry points where practical, using the
