...
public class doubleSVDWorkspaceTests
{
    ...
    public struct WorkspaceEquivJob : IJob
    {
        ...
        void PinvEquiv(int m, int n)
        {
            ...
            var A0 = arena.doubleRandomMatrix(m, n, -5f, 5f, 778231);
            for (int d = 0; d < k; d++)   // boost leading diagonal block for conditioning
                A0[d, d] += (double)10f;

            var b = arena.doubleRandomVector(m, -5f, 5f, 9090);   // read-only in pinvSolve
            ...
            int rb = SVD.pinvSolve(ref Ab, in b, ref xb, out bool cb, (double)(-1), 30, ref S, ref M, ref U, ref At);
        ...
        void PseudoEquiv(int m, int n)
        {
            ...
            var A0 = arena.doubleRandomMatrix(m, n, -5f, 5f, 314221);
            for (int d = 0; d < k; d++)
                A0[d, d] += (double)10f;
            ...
            if (m < n)
                At = arena.doubleMat(n, m);
            int rb = SVD.pseudoInverse(ref Ab, ref Pb, out bool cb, (double)(-1), 30, ref S, ref M, ref U, ref At);
        ...
        void WorkspaceReuse()
        {
            ...
            var A0 = arena.doubleRandomMatrix(m, n, -5f, 5f, 24681);
            for (int d = 0; d < n; d++)
                A0[d, d] += (double)10f;
            ...
            for (int t = 0; t < 3; t++)
            {
                var b = arena.doubleRandomVector(m, -5f, 5f, (uint)(1000 + t * 7));
    ...
    public void Pinv_BadScratchS_Throws()
    {
        ...
        try
        {
            ...
            Assert.Throws<ArgumentException>(
                () => SVD.pinvSolve(ref A, in b, ref x, out bool c, (double)(-1), 30, ref S, ref M, ref U, ref At));
        }
    ...
    public void PseudoInverse_WideBadAt_Throws()
    {
        ...
        try
        {
            ...
            Assert.Throws<ArgumentException>(
                () => SVD.pseudoInverse(ref A, ref Aplus, out bool c, (double)(-1), 30, ref S, ref M, ref U, ref At));
        }
        finally { arena.Dispose(); }
    }
}
