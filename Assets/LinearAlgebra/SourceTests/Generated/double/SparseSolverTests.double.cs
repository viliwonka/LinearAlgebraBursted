...
// doubleBSROperator / doubleBlockJacobi / Solvers.cg&lt;TOp&gt; / Solvers.pcg&lt;TOp,TPre&gt;, plus
// the concrete cg/pcg convenience overloads for doubleMxN and doubleBSR. Every
// BSR system is cross-checked against the equivalent dense system (same pattern as
// doubleSparseBSRTests: build the SAME system in both forms and compare).
//
// Correctness cases run inside a [BurstCompile] IJob (matches doubleConjugateGradientTests /
// doubleSparseBSRTests). Guard/exception cases run on the managed test thread with
...
public class doubleSparseSolverTests
{
    ...
    public struct SparseSolverTestJob : IJob
    {
        ...
        void Laplacian1DBSRCGMatchesDenseCG()
        {
            ...
            bool okBsr = Solvers.cg(in bsm, in b, ref xBsr, 4 * dim, Consts.doubleSqrtEps);
            Assert.IsTrue(okBsr);
        ...
        void MinresIndefiniteDenseAndBSR()
        {
            ...
            var xBsr = arena.doubleVec(dim);
            bool okBsr = Solvers.minres(in bsm, in b, ref xBsr, 4 * dim, Consts.doubleSqrtEps);
            Assert.IsTrue(okBsr);
        ...
        void MinresSpdMatchesCG()
        {
            ...
            bool okMinBsr = Solvers.minres(in bsm, in b, ref xMinBsr, 4 * dim, Consts.doubleSqrtEps);
        ...
        void BiCGStabNonSymmetricMatchesLU()
        {
            ...
            bool okBcgBsr = Solvers.biCGStab(in bsm, in b, ref xBcgBsr, 4 * dim, Consts.doubleSqrtEps);
        ...
        void CglsOverdeterminedConsistentDenseAndBSR()
        {
            ...
            bool okBsr = Solvers.cgls(in bsm, in b, ref xBsr, 8 * n, Consts.doubleSqrtEps);
            Assert.IsTrue(okBsr);
        ...
        void LsqrOverdeterminedConsistentDenseAndBSR()
        {
            ...
            var bsm = DenseToBSR1x1(ref arena, in A, m * n);
            var xBsr = arena.doubleVec(n);
            bool okBsr = Solvers.lsqr(in bsm, in b, ref xBsr, 8 * n, Consts.doubleSqrtEps);
            Assert.IsTrue(okBsr);
            AssertVecEq(in x, in xBsr, LooseTol());
        ...
        void LsmrOverdeterminedConsistentDenseAndBSR()
        {
            ...
            var bsm = DenseToBSR1x1(ref arena, in A, m * n);
            var xBsr = arena.doubleVec(n);
            bool okBsr = Solvers.lsmr(in bsm, in b, ref xBsr, 8 * n, Consts.doubleSqrtEps);
            Assert.IsTrue(okBsr);
            AssertVecEq(in x, in xBsr, LooseTol());
        ...
        static void AssertResidualNormBSR(in doubleBSR A, in doubleN b, in doubleN x, double rnorm, double tol)
        {
