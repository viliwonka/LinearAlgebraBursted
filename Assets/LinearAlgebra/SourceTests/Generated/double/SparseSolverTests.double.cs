...
// doubleBSROperator / doubleBlockJacobi / Krylov.cg&lt;TOp&gt; / Krylov.pcg&lt;TOp,TPre&gt;, plus
...
public class doubleSparseSolverTests
{
    ...
    public struct SparseSolverTestJob : IJob
    {
        ...
        void PCGBeatsCGIllConditioned()
        {
            ...
            for (int budget = 1; budget <= maxBudget && (minCG < 0 || minPCG < 0); budget++)
            {
                ...
                {
                    var xPCG = arena.doubleVec(dim);
                    if (Krylov.pcg(in bsm, in M, in b, ref xPCG, budget, Consts.doubleSqrtEps))
                        minPCG = budget;
                }
            }
        ...
        void WarmStart()
        {
            ...
            var xWarm = x.Copy();
            bool okWarm = Krylov.pcg(in bsm, in M, in b, ref xWarm, 1, Consts.doubleSqrtEps);
        ...
        void PcgNonSpdPreconditionerBreaksDown()
        {
            ...
            var x = arena.doubleVec(dim);
            bool ok = Krylov.pcg(in op, in pre, in b, ref x, dim, Consts.doubleSqrtEps);
            Assert.IsFalse(ok);
        ...
        void CgneUnderdeterminedMinNormMatchesLsqr()
        {
            ...
            var xC = arena.doubleVec(n);
            Assert.IsTrue(Krylov.cgne(in A, in b, ref xC, 8 * n, Consts.doubleSqrtEps));
            var AxC = Blas.dot(A, xC);
            AssertVecEq(in AxC, in b, LooseTol());          // solves the consistent system

            var xL = arena.doubleVec(n);
            Assert.IsTrue(Krylov.lsqr(in A, in b, ref xL, 8 * n, Consts.doubleSqrtEps));
            AssertVecEq(in xC, in xL, LooseTol());          // both land on the unique min-norm solution

            var bsm = DenseToBSR1x1(ref arena, in A, m * n);
            var xCb = arena.doubleVec(n);
            Assert.IsTrue(Krylov.cgne(in bsm, in b, ref xCb, 8 * n, Consts.doubleSqrtEps));
        ...
        void CgneWarmStart()
        {
            ...
            var x = arena.doubleVec(n);
            Assert.IsTrue(Krylov.cgne(in A, in b, ref x, 8 * n, Consts.doubleSqrtEps));

            var xWarm = x.Copy();
            bool okWarm = Krylov.cgne(in A, in b, ref xWarm, 1, Consts.doubleSqrtEps);
        ...
        void CgneInconsistentDoesNotConverge()
        {
            ...
            var xC = arena.doubleVec(n);
            bool okCgne = Krylov.cgne(in A, in b, ref xC, 8 * n, Consts.doubleSqrtEps);
            ...
            Assert.IsTrue(Krylov.cgls(in A, in b, ref xLs, 8 * n, Consts.doubleSqrtEps));
        ...
        void SolveInfoRnormMatchesResidual()
        {
            ...
            var ip = Krylov.pcg(in bsm, in M, in bspd, ref xp, maxIter, Consts.doubleSqrtEps);
            ...
            var inx = Krylov.cgne(in Aun, in bun, ref xn, 4 * nn, Consts.doubleSqrtEps);
            ...
            var ihp = Krylov.pcg(in bsm, in M, in bspd, ref xhp, 1, Consts.doubleSqrtEps);
            ...
            var ihn = Krylov.cgne(in Aun, in bun, ref xhn, 1, Consts.doubleSqrtEps);
    ...
    public void Pcg_AliasingRAndX_Throws()
    {
        ...
        try
        {
            ...
            Assert.Throws<ArgumentException>(() =>
                Krylov.pcg(in A, in M, in b, ref x, ref rAlias, ref p, ref Ap, ref z, dim, Consts.doubleSqrtEps));
        }
        finally { arena.Dispose(); }
    }
    ...
    public void Pcg_AliasingZAndX_Throws()
    {
        ...
        try
        {
            ...
            Assert.Throws<ArgumentException>(() =>
                Krylov.pcg(in A, in M, in b, ref x, ref r, ref p, ref Ap, ref zAlias, dim, Consts.doubleSqrtEps));
        }
        finally { arena.Dispose(); }
    }
    ...
    public void Pcg_AliasingXAndB_Throws()
    {
        ...
        try
        {
            ...
            var z = arena.doubleVec(dim);
            Assert.Throws<ArgumentException>(() =>
                Krylov.pcg(in A, in M, in b, ref xAlias, ref r, ref p, ref Ap, ref z, dim, Consts.doubleSqrtEps));
        }
        finally { arena.Dispose(); }
    }
    ...
    public void Cgne_AliasingPAndTmpN_Throws()
    {
        ...
        try
        {
            ...
            Assert.Throws<ArgumentException>(() =>
                Krylov.cgne(in A, in b, ref x, ref r, ref p, ref q, ref tmpNAlias, n, Consts.doubleSqrtEps));
        }
        finally { arena.Dispose(); }
    }
}
