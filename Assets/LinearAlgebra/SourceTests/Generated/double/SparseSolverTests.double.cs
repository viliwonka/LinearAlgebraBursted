...
// Phase-2 sparse-solver test suite: IdoubleLinearOperator / doubleDenseOperator /
// doubleBSMOperator / doubleBlockJacobi / Solvers.cg&lt;TOp&gt; / Solvers.pcg&lt;TOp,TPre&gt;, plus
// the concrete cg/pcg convenience overloads for doubleMxN and doubleBSM. Every
// BSM system is cross-checked against the equivalent dense system (same pattern as
// doubleSparseBSMTests: build the SAME system in both forms and compare).
//
// Correctness cases run inside a [BurstCompile] IJob (matches doubleConjugateGradientTests /
// doubleSparseBSMTests). Guard/exception cases run on the managed test thread with
...
public class doubleSparseSolverTests
{
    ...
    public struct SparseSolverTestJob : IJob
    {
        ...
        // Consts.doubleSqrtEps, not machine epsilon) doesn't false-fail.
        ...
        // error can be a few times Consts.doubleSqrtEps*scale -- looser than the SPD-CG cases above.
        ...
        // all of them. See the growth regression tests in SparseBSMTests.double.cs, which build
        ...
        void Laplacian1DBSMCGMatchesDenseCG()
        {
            ...
            bool okDense = Solvers.cg(in A, in b, ref xDense, 4 * dim, Consts.doubleSqrtEps);
            Assert.IsTrue(okDense);

            var xBsm = arena.doubleVec(dim);
            bool okBsm = Solvers.cg(in bsm, in b, ref xBsm, 4 * dim, Consts.doubleSqrtEps);
            Assert.IsTrue(okBsm);
        ...
        void ThreeByThreeBlockSPDConverges()
        {
            ...
            var mb = arena.doubleBSMBuilder(nb, nb, BR, BR, nb * nb);
            ...
            var ab = arena.doubleBSMBuilder(nb, nb, BR, BR, nb * nb);
        ...
        void DenseForwardingUnchanged()
        {
            ...
            // Calling cg<TOp> directly via doubleDenseOperator must reproduce the identical
            ...
            bool okGeneric = Solvers.cg(in op, in b, ref xGeneric, ref r, ref p, ref Ap, dim, Consts.doubleSqrtEps);
        ...
        void PCGBeatsCGIllConditioned()
        {
            ...
            for (int budget = 1; budget <= maxBudget && (minCG < 0 || minPCG < 0); budget++)
            {
                ...
                {
                    var xCG = arena.doubleVec(dim);
                    if (Solvers.cg(in bsm, in b, ref xCG, budget, Consts.doubleSqrtEps))
                        minCG = budget;
                }
                if (minPCG < 0)
                {
                    var xPCG = arena.doubleVec(dim);
                    if (Solvers.pcg(in bsm, in M, in b, ref xPCG, budget, Consts.doubleSqrtEps))
                        minPCG = budget;
                }
            }
        ...
        void BlockJacobiApplyHandComputed()
        {
            ...
            const int BR = 2;
            var builder = arena.doubleBSMBuilder(2, 2, BR, BR, 2);
            ...
            double det0 = 11f;
            double z0 = (3f * r[0] - 1f * r[1]) / det0;
            double z1 = (-1f * r[0] + 4f * r[1]) / det0;
        ...
        void WarmStart()
        {
            ...
            // doubleConjugateGradientTests.AlreadyConverged's dense-CG counterpart).
            var xWarm = x.Copy();
            bool okWarm = Solvers.pcg(in bsm, in M, in b, ref xWarm, 1, Consts.doubleSqrtEps);
            ...
            bool okCGWarm = Solvers.cg(in bsm, in b, ref xCGWarm, 1, Consts.doubleSqrtEps);
        ...
        void PcgNonSpdPreconditionerBreaksDown()
        {
            ...
            var x = arena.doubleVec(dim);
            bool ok = Solvers.pcg(in op, in pre, in b, ref x, dim, Consts.doubleSqrtEps);
            Assert.IsFalse(ok);
        ...
        void MinresIndefiniteDenseAndBSM()
        {
            ...
            var xDense = arena.doubleVec(dim);
            bool okDense = Solvers.minres(in A, in b, ref xDense, 4 * dim, Consts.doubleSqrtEps);
            Assert.IsTrue(okDense);
            ...
            var xBsm = arena.doubleVec(dim);
            bool okBsm = Solvers.minres(in bsm, in b, ref xBsm, 4 * dim, Consts.doubleSqrtEps);
            Assert.IsTrue(okBsm);
        ...
        void MinresSpdMatchesCG()
        {
            ...
            var xCG = arena.doubleVec(dim);
            bool okCG = Solvers.cg(in A, in b, ref xCG, 4 * dim, Consts.doubleSqrtEps);
            Assert.IsTrue(okCG);

            var xMin = arena.doubleVec(dim);
            bool okMin = Solvers.minres(in A, in b, ref xMin, 4 * dim, Consts.doubleSqrtEps);
            Assert.IsTrue(okMin);
            ...
            var bsm = DenseToBSM1x1(ref arena, in A, dim * dim);
            var xMinBsm = arena.doubleVec(dim);
            bool okMinBsm = Solvers.minres(in bsm, in b, ref xMinBsm, 4 * dim, Consts.doubleSqrtEps);
        ...
        void BiCGStabNonSymmetricMatchesLU()
        {
            ...
            bool okBcg = Solvers.biCGStab(in A, in b, ref xBcg, 4 * dim, Consts.doubleSqrtEps);
            Assert.IsTrue(okBcg);
            ...
            var bsm = DenseToBSM1x1(ref arena, in A, dim * dim);
            var xBcgBsm = arena.doubleVec(dim);
            bool okBcgBsm = Solvers.biCGStab(in bsm, in b, ref xBcgBsm, 4 * dim, Consts.doubleSqrtEps);
        ...
        void CglsOverdeterminedConsistentDenseAndBSM()
        {
            ...
            bool okBsm = Solvers.cgls(in bsm, in b, ref xBsm, 8 * n, Consts.doubleSqrtEps);
            Assert.IsTrue(okBsm);
        ...
        void LsqrOverdeterminedConsistentDenseAndBSM()
        {
            ...
            var bsm = DenseToBSM1x1(ref arena, in A, m * n);
            var xBsm = arena.doubleVec(n);
            bool okBsm = Solvers.lsqr(in bsm, in b, ref xBsm, 8 * n, Consts.doubleSqrtEps);
            Assert.IsTrue(okBsm);
            AssertVecEq(in x, in xBsm, LooseTol());
        ...
        void CglsLsqrUnderdeterminedConsistent()
        {
            ...
            bool okC = Solvers.cgls(in A, in b, ref xC, 8 * n, Consts.doubleSqrtEps);
            Assert.IsTrue(okC);
        ...
        void LsmrMonotonicArnorm()
        {
            ...
            for (int k = 1; k <= n; k++)
            {
                ...
                Solvers.lsmr(in A, in b, ref x, k, (double)0);  // tol 0 -> runs exactly k iterations

                var res = Linear_OP.dot(A, x) - b;              // A x - b   (length m)
                var atr = Linear_OP.dot(res, A);                // A^T res   (length n)
                double nrm = math.sqrt(Linear_OP.dot(atr, atr));
        ...
        void TikhonovDampingMatchesAugmentedQR()
        {
            ...
            var xC = arena.doubleVec(n);
            Assert.IsTrue(Solvers.cgls(in A, in b, ref xC, 16 * n, Consts.doubleSqrtEps, damp));
            AssertVecEq(in xC, in xref, LooseTol());

            var xL = arena.doubleVec(n);
            Assert.IsTrue(Solvers.lsqr(in A, in b, ref xL, 16 * n, Consts.doubleSqrtEps, damp));
            AssertVecEq(in xL, in xref, LooseTol());

            var xM = arena.doubleVec(n);
            Assert.IsTrue(Solvers.lsmr(in A, in b, ref xM, 16 * n, Consts.doubleSqrtEps, damp));
            ...
            Assert.IsTrue(Solvers.cgls(in bsm, in b, ref xCb, 16 * n, Consts.doubleSqrtEps, damp));
            AssertVecEq(in xCb, in xref, LooseTol());

            var xLb = arena.doubleVec(n);
            Assert.IsTrue(Solvers.lsqr(in bsm, in b, ref xLb, 16 * n, Consts.doubleSqrtEps, damp));
            AssertVecEq(in xLb, in xref, LooseTol());

            var xMb = arena.doubleVec(n);
            Assert.IsTrue(Solvers.lsmr(in bsm, in b, ref xMb, 16 * n, Consts.doubleSqrtEps, damp));
        ...
        void CgneUnderdeterminedMinNormMatchesLsqr()
        {
            ...
            int m = 4, n = 10;
            var A = arena.doubleRandomMat(m, n, -1f, 1f, 44001);
            var xGen = arena.doubleRandomVec(n, -1f, 1f, 44002);
            var b = Linear_OP.dot(A, xGen);      // consistent (b in range(A))

            var xC = arena.doubleVec(n);
            Assert.IsTrue(Solvers.cgne(in A, in b, ref xC, 8 * n, Consts.doubleSqrtEps));
            var AxC = Linear_OP.dot(A, xC);
            AssertVecEq(in AxC, in b, LooseTol());          // solves the consistent system

            var xL = arena.doubleVec(n);
            Assert.IsTrue(Solvers.lsqr(in A, in b, ref xL, 8 * n, Consts.doubleSqrtEps));
            AssertVecEq(in xC, in xL, LooseTol());          // both land on the unique min-norm solution

            var bsm = DenseToBSM1x1(ref arena, in A, m * n);
            var xCb = arena.doubleVec(n);
            Assert.IsTrue(Solvers.cgne(in bsm, in b, ref xCb, 8 * n, Consts.doubleSqrtEps));
        ...
        void CgneWarmStart()
        {
            ...
            int m = 4, n = 10;
            var A = arena.doubleRandomMat(m, n, -1f, 1f, 44101);
            var xGen = arena.doubleRandomVec(n, -1f, 1f, 44102);
            var b = Linear_OP.dot(A, xGen);      // consistent

            var x = arena.doubleVec(n);
            Assert.IsTrue(Solvers.cgne(in A, in b, ref x, 8 * n, Consts.doubleSqrtEps));

            var xWarm = x.Copy();
            bool okWarm = Solvers.cgne(in A, in b, ref xWarm, 1, Consts.doubleSqrtEps);
        ...
        void LstsqInfoMatchesIndependentRecompute()
        {
            ...
            var xC = arena.doubleVec(n);
            var infoC = Solvers.cgls(in A, in b, ref xC, maxIter, Consts.doubleSqrtEps, (double)0);
            ...
            var xL = arena.doubleVec(n);
            var infoL = Solvers.lsqr(in A, in b, ref xL, maxIter, Consts.doubleSqrtEps, (double)0);
            ...
            var xM = arena.doubleVec(n);
            var infoM = Solvers.lsmr(in A, in b, ref xM, maxIter, Consts.doubleSqrtEps, (double)0);
        ...
        void LstsqInfoDampedArnorm()
        {
            ...
            var xC = arena.doubleVec(n);
            var infoC = Solvers.cgls(in A, in b, ref xC, maxIter, Consts.doubleSqrtEps, damp);
            Assert.IsTrue(infoC.Solved);
            AssertInfoMatches(in A, in b, in xC, damp, in infoC, LooseTol());

            var xL = arena.doubleVec(n);
            var infoL = Solvers.lsqr(in A, in b, ref xL, maxIter, Consts.doubleSqrtEps, damp);
            Assert.IsTrue(infoL.Solved);
            AssertInfoMatches(in A, in b, in xL, damp, in infoL, LooseTol());

            var xM = arena.doubleVec(n);
            var infoM = Solvers.lsmr(in A, in b, ref xM, maxIter, Consts.doubleSqrtEps, damp);
            Assert.IsTrue(infoM.Solved);
        ...
        void LsmrRnormMatchesExact()
        {
            ...
            var xc = arena.doubleVec(n);
            var ic = Solvers.lsmr(in A, in bCons, ref xc, maxIter, Consts.doubleSqrtEps, (double)0);
            Assert.IsTrue(ic.Solved);
            var exC = Solvers.lstsqResidual(new doubleDenseOperator(in A), in bCons, in xc, (double)0, ref rS, ref sS);
            AssertClose(ic.rnorm, exC.rnorm, LooseTol());

            var xi = arena.doubleVec(n);
            var ii = Solvers.lsmr(in A, in bInc, ref xi, maxIter, Consts.doubleSqrtEps, (double)0);
            Assert.IsTrue(ii.Solved);
            var exI = Solvers.lstsqResidual(new doubleDenseOperator(in A), in bInc, in xi, (double)0, ref rS, ref sS);
            Assert.IsTrue(exI.rnorm > (double)0.1);             // genuinely inconsistent
            ...
            var if2 = Solvers.lsmr(in A, in bInc, ref xf, 2, Consts.doubleSqrtEps, (double)0);
            Assert.IsTrue(!if2.Solved && if2.iterations == 2);   // genuinely stopped mid-flight
            var exF = Solvers.lstsqResidual(new doubleDenseOperator(in A), in bInc, in xf, (double)0, ref rS, ref sS);
        ...
        void LstsqInfoBSMMatchesDense()
        {
            ...
            var infoD = Solvers.lsqr(in A, in b, ref xD, maxIter, Consts.doubleSqrtEps, (double)0);
            Assert.IsTrue(infoD.Solved);

            var xB = arena.doubleVec(n);
            var infoB = Solvers.lsqr(in bsm, in b, ref xB, maxIter, Consts.doubleSqrtEps, (double)0);
        ...
        void JacobiPreconditionerReducesIterations()
        {
            ...
            var infoPlain = Solvers.lsqr(in A, in b, ref xPlain, maxIter, tol, (double)0);
            Assert.IsTrue(infoPlain.Solved);

            // preconditioned via the composable diagnostic path (to read the iteration count)
            var d2 = arena.doubleVec(n); Linear_OP.columnNormsSquared(in A, ref d2);
            var d  = arena.doubleVec(n); Linear_OP.buildJacobiScale(in d2, ref d);
            ...
            var infoJac = Solvers.lsqr(op, in b, ref y, ref u, ref v, ref w, ref tmpM, ref tmpN, maxIter, tol, (double)0);
        ...
        void LstsqInfoStrangLineFitExact()
        {
            ...
            for (int which = 0; which < 3; which++)
            {
                ...
                AssertClose(x[1], (double)(-3), LooseTol());
        ...
        void SolveInfoRnormMatchesResidual()
        {
            ...
            var ig = Solvers.cg(in Aspd, in bspd, ref xg, maxIter, Consts.doubleSqrtEps);
            ...
            var ih = Solvers.cg(in Aspd, in bspd, ref xh, 1, Consts.doubleSqrtEps);
            ...
            var iind = Solvers.cg(in Aind, in bind, ref xind, 10, Consts.doubleSqrtEps);
            ...
            var izero = Solvers.cg(in Aspd, in bzero, ref xzero, maxIter, Consts.doubleSqrtEps);
    ...
    public void BlockJacobi_NonSquareBSM_Throws()
    {
        ...
        {
            const int BR = 2, BC = 3;
            var builder = arena.doubleBSMBuilder(2, 2, BR, BC, 1); // BR != BC
    ...
    public void Cg_NonSquareDenseOperator_Throws()
    {
        ...
        try
        {
            ...
            Assert.Throws<ArgumentException>(() =>
                Solvers.cg(in op, in b, ref x, ref r, ref p, ref Ap, 4, Consts.doubleSqrtEps));
        }
        finally { arena.Dispose(); }
    }
    ...
    // so the operator matrix's contents are irrelevant -- a bare square doubleMat suffices for cg.
    ...
    public void Cg_AliasingRAndAp_Throws()
    {
        ...
        try
        {
            ...
            Assert.Throws<ArgumentException>(() =>
                Solvers.cg(in op, in b, ref x, ref rAlias, ref p, ref Ap, dim, Consts.doubleSqrtEps));
        }
        finally { arena.Dispose(); }
    }
    ...
    public void Pcg_AliasingRAndX_Throws()
    {
        ...
        try
        {
            ...
            Assert.Throws<ArgumentException>(() =>
                Solvers.pcg(in A, in M, in b, ref x, ref rAlias, ref p, ref Ap, ref z, dim, Consts.doubleSqrtEps));
        }
        finally { arena.Dispose(); }
    }
    ...
    public void Pcg_AliasingZAndX_Throws()
    {
        ...
        try
        {
            ...
            Assert.Throws<ArgumentException>(() =>
                Solvers.pcg(in A, in M, in b, ref x, ref r, ref p, ref Ap, ref zAlias, dim, Consts.doubleSqrtEps));
        }
        finally { arena.Dispose(); }
    }
    ...
    public void Cg_AliasingXAndB_Throws()
    {
        ...
        try
        {
            ...
            var Ap = arena.doubleVec(dim);
            Assert.Throws<ArgumentException>(() =>
                Solvers.cg(in op, in b, ref xAlias, ref r, ref p, ref Ap, dim, Consts.doubleSqrtEps));
        }
        finally { arena.Dispose(); }
    }
    ...
    public void Pcg_AliasingXAndB_Throws()
    {
        ...
        try
        {
            ...
            var z = arena.doubleVec(dim);
            Assert.Throws<ArgumentException>(() =>
                Solvers.pcg(in A, in M, in b, ref xAlias, ref r, ref p, ref Ap, ref z, dim, Consts.doubleSqrtEps));
        }
        finally { arena.Dispose(); }
    }
    ...
    public void Minres_NonSquareDense_Throws()
    {
        ...
        try
        {
            ...
            Assert.Throws<ArgumentException>(() =>
                Solvers.minres(in A, in b, ref x, ref y, ref r1, ref r2, ref v, ref w, ref w1, ref w2, 3, Consts.doubleSqrtEps));
        }
        finally { arena.Dispose(); }
    }
    ...
    public void Minres_AliasingR1AndR2_Throws()
    {
        ...
        try
        {
            ...
            Assert.Throws<ArgumentException>(() =>
                Solvers.minres(in A, in b, ref x, ref y, ref r1, ref r2Alias, ref v, ref w, ref w1, ref w2, dim, Consts.doubleSqrtEps));
        }
        finally { arena.Dispose(); }
    }
    ...
    public void BiCGStab_AliasingXAndB_Throws()
    {
        ...
        try
        {
            ...
            Assert.Throws<ArgumentException>(() =>
                Solvers.biCGStab(in A, in b, ref xAlias, ref r, ref rHat0, ref p, ref v, ref t, dim, Consts.doubleSqrtEps));
        }
        finally { arena.Dispose(); }
    }
    ...
    public void Cgls_AliasingRAndQ_Throws()
    {
        ...
        try
        {
            ...
            Assert.Throws<ArgumentException>(() =>
                Solvers.cgls(in A, in b, ref x, ref r, ref s, ref p, ref qAlias, n, Consts.doubleSqrtEps));
        }
        finally { arena.Dispose(); }
    }
    ...
    public void Cgne_AliasingPAndTmpN_Throws()
    {
        ...
        try
        {
            ...
            var A = arena.doubleMat(m, n);
            var b = arena.doubleRandomVec(m, -1f, 1f, 39501);
            var x = arena.doubleVec(n);
            var r = arena.doubleVec(m);
            var p = arena.doubleVec(n);
            ...
            Assert.Throws<ArgumentException>(() =>
                Solvers.cgne(in A, in b, ref x, ref r, ref p, ref q, ref tmpNAlias, n, Consts.doubleSqrtEps));
        }
        finally { arena.Dispose(); }
    }
}
