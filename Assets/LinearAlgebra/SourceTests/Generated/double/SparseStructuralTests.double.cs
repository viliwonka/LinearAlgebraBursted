...
public class doubleSparseStructuralTests
{
    ...
    public struct SparseStructuralTestJob : IJob
    {
        ...
        static void FillBlock(ref doubleMxN b, int br, int bc, double scale)
        {
            for (int r = 0; r < b.M_Rows; r++)
                for (int c = 0; c < b.N_Cols; c++)
                    b[r, c] = scale * (double)((1 + br * 31 + bc * 7 + r * b.N_Cols + c) * ((r + c) % 2 == 0 ? 1 : -1));
        }
        ...
        void TraceAndDiagonal()
        {
            ...
            var b = new doubleMxN(BR, BC, Allocator.Temp);

            FillBlock(ref b, 0, 1, (double)1); builder.AddBlock(0, 1, in b);
            FillBlock(ref b, 1, 1, (double)1); builder.AddBlock(1, 1, in b);
            FillBlock(ref b, 2, 0, (double)1); builder.AddBlock(2, 0, in b);
            FillBlock(ref b, 2, 2, (double)1); builder.AddBlock(2, 2, in b);
            ...
            var d = new doubleN(A.M_Rows, Allocator.Temp);
            ...
            Assert.IsTrue(d[0] == (double)0);
            Assert.IsTrue(d[1] == (double)0);
        }

        void DenseDiagonal()
        {
            var A = new doubleMxN(4, 6, Allocator.Temp);
            for (int i = 0; i < 4; i++)
                for (int j = 0; j < 6; j++)
                    A[i, j] = (double)(i * 10 + j);

            var d = new doubleN(4, Allocator.Temp);   // min(4, 6)
            Analysis.diagonal(in A, ref d);
            for (int i = 0; i < 4; i++)
        ...
        void TransposeRectangular()
        {
            ...
            const int BR = 2, BC = 3;
            var builder = new doubleBSRBuilder(2, 3, BR, BC, Allocator.Temp);
            var b = new doubleMxN(BR, BC, Allocator.Temp);

            FillBlock(ref b, 0, 0, (double)1); builder.AddBlock(0, 0, in b);
            FillBlock(ref b, 0, 2, (double)1); builder.AddBlock(0, 2, in b);
            FillBlock(ref b, 1, 1, (double)1); builder.AddBlock(1, 1, in b);
            FillBlock(ref b, 1, 2, (double)1); builder.AddBlock(1, 2, in b);
        ...
        void TransposeIntoReuse()
        {
            ...
            var A = builder.ToBSR(Allocator.Temp);
            var At = new doubleBSR(A.BlockCols, A.BlockRows, A.BC, A.BR, A.Nnzb, Allocator.Temp, true);
        ...
        {
            const int BR = 2;
            var builder = new doubleBSRBuilder(2, 2, BR, BR, Allocator.Temp);
            ...
            diag[0, 0] = (double)4; diag[0, 1] = (double)1;
            diag[1, 0] = (double)1; diag[1, 1] = (double)5;
            FillBlock(ref off, 0, 1, (double)1);
        ...
        void AssemblyRefill()
        {
            ...
            FillBlock(ref b, 0, 0, (double)1); builder.AddBlock(0, 0, in b);
            FillBlock(ref b, 1, 1, (double)1); builder.AddBlock(1, 1, in b);
            FillBlock(ref b, 1, 1, (double)2); builder.AddBlock(1, 1, in b);
            ...
            builder.Clear();
            FillBlock(ref b, 0, 0, (double)5); builder.AddBlock(0, 0, in b);
            FillBlock(ref b, 1, 1, (double)7); builder.AddBlock(1, 1, in b);
            FillBlock(ref b, 1, 1, (double)(-3)); builder.AddBlock(1, 1, in b);
    ...
    public void TransposeWrongDestinationThrows()
    {
        ...
        var wrongGrid = new doubleBSR(2, 3, BR, BC, 1, Allocator.Temp, true);
        Assert.Throws<ArgumentException>(() => BSR.transpose(in A, ref wrongGrid));

        // Transposed grid but wrong nnzb -> throw.
        var wrongNnzb = new doubleBSR(3, 2, BC, BR, 2, Allocator.Temp, true);
