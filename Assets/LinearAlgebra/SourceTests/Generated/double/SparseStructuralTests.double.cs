...
public class doubleSparseStructuralTests
{
    ...
    public struct SparseStructuralTestJob : IJob
    {
        ...
        void TraceAndDiagonal()
        {
            ...
            var d = arena.doubleVec(A.M_Rows);
        ...
        void DenseDiagonal()
        {
            ...
            var A = arena.doubleMat(4, 6);
            for (int i = 0; i < 4; i++)
                for (int j = 0; j < 6; j++)
                    A[i, j] = (double)(i * 10 + j);

            var d = arena.doubleVec(4);   // min(4, 6)
            Analysis.diagonal(in A, ref d);
            for (int i = 0; i < 4; i++)
        ...
        void TransposeRectangular()
        {
            ...
            const int BR = 2, BC = 3;
            var builder = arena.doubleBSRBuilder(2, 3, BR, BC);
            var b = arena.doubleMat(BR, BC);
        ...
        void TransposeIntoReuse()
        {
            ...
            var A = builder.ToBSR(ref arena);
            var At = arena.doubleBSR(A.BlockCols, A.BlockRows, A.BC, A.BR, A.Nnzb, true);
        ...
        void TransposeSymmetric()
        {
            ...
            const int BR = 2;
            var builder = arena.doubleBSRBuilder(2, 2, BR, BR);
    ...
    public void RefillTopologyChangeThrows()
    {
        ...
        try
        {
            const int BR = 2, BC = 2;
            var builder = arena.doubleBSRBuilder(2, 2, BR, BC);
            var block = arena.doubleMat(BR, BC, (double)1);
