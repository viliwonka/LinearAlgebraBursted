...
public class doubleLQWorkspaceTests
{
    ...
    public struct WorkspaceEquivJob : IJob
    {
        ...
        void LqDecompEquiv(int m, int n)
        {
            ...
            var A0 = arena.doubleRandomMat(m, n, -5f, 5f, 41221);
        ...
        void LqMinNormSolveEquiv(int m, int n)
        {
            ...
            var A0 = arena.doubleRandomMat(m, n, -5f, 5f, 61729);
            for (int d = 0; d < m; d++)
                A0[d, d] += (double)10f;

            var b = arena.doubleRandomVec(m, -5f, 5f, 8080);
        ...
        void WorkspaceReuse()
        {
            ...
            for (int t = 0; t < 3; t++)
            {
                var A0 = arena.doubleRandomMat(m, n, -5f, 5f, (uint)(3000 + t * 13));
                for (int d = 0; d < m; d++)
                    A0[d, d] += (double)10f;
                ...
                var b = arena.doubleRandomVec(m, -5f, 5f, (uint)(4000 + t * 17));
    ...
    public void LqDecomp_BadScratchT_Throws()
    {
        ...
        try
        {
            ...
            ws.T = arena.doubleMat(8, 3);   // wrong: must be n x m = 8 x 4
    ...
    public void LqDecomp_BadScratchQrU_Throws()
    {
        ...
        try
        {
            ...
            var Q = arena.doubleMat(4, 8);
            var ws = arena.doubleLQ_WS(4, 8);
            ws.qrU = arena.doubleVec(5);    // wrong: must be length n = 8
            Assert.Throws<ArgumentException>(() => LQ.lqDecomposition(ref A, ref L, ref Q, ref ws));
        }
        finally { arena.Dispose(); }
    }
