...
// Regression coverage for the doubleMxN(in doubleMxN source, int rows, int cols) reslice-view
...
public class doubleMatrixViewInvariantTests
{
    ...
    public struct TestJob : IJob
    {
        ...
        {
            int s = 6, m = 3;
            double sentinel = (double)12345;

            var buf = new doubleMxN(s, s, Allocator.Temp, true);
            buf.fillInPlace(sentinel);

            // Known, deterministic m*m pattern (flat 1..m*m) in the leading elements only.
            for (int i = 0; i < m * m; i++) buf[i] = (double)(i + 1);

            var view = new doubleMxN(in buf, m, m);
            ...
            double expL1 = (double)0, expSumSq = (double)0, expLInf = (double)0;
            for (int i = 0; i < m * m; i++)
            {
                double v = (double)(i + 1);
                expL1 += v;
            ...
            double expL2 = math.sqrt(expSumSq);
        ...
        static void FillRowDominant(ref doubleMxN mat, int rows, int cols)
        {
            for (int i = 0; i < rows; i++)
                for (int j = 0; j < cols; j++)
                    mat[i, j] = (double)(((i * 3 + j * 2 + 1) % 7) - 3) + (double)((i == j) ? 20 : 0);
        }
        ...
        {
            int s = 8, n = 5, sLive = 3;
            double sentinel = (double)(-99999);

            var wide = new doubleMxN(s, n, Allocator.Temp, true);
            wide.fillInPlace(sentinel);
            FillRowDominant(ref wide, sLive, n);

            var exact = new doubleMxN(sLive, n, Allocator.Temp);
            FillRowDominant(ref exact, sLive, n);

            var view = new doubleMxN(in wide, sLive, n);
            Assert.AreEqual(sLive * n, view.Length);
            Assert.AreEqual(sLive * n, view.Data.Length);

            var Lview = new doubleMxN(sLive, sLive, Allocator.Temp);
            var Qview = new doubleMxN(sLive, n, Allocator.Temp);
            var Pview = new Pivot(sLive, Allocator.Temp);
            var infoView = LQRP.decomp(in view, ref Lview, ref Qview, ref Pview);

            var Lexact = new doubleMxN(sLive, sLive, Allocator.Temp);
            var Qexact = new doubleMxN(sLive, n, Allocator.Temp);
        ...
        static void FillSymmetricDominantFlat(ref doubleMxN mat, int m)
        {
            for (int i = 0; i < m; i++)
                for (int j = 0; j < m; j++)
                    mat[i * m + j] = (double)(((i + j) % 3) + 1) + (double)((i == j) ? 20 : 0);
        }
        ...
        void SquareViewRetroactiveFix_QRMatchesExact()
        {
            ...
            var buf = new doubleMxN(s, s, Allocator.Temp, true);
            buf.fillInPlace((double)(-77777));
            FillSymmetricDominantFlat(ref buf, m);

            var exact = new doubleMxN(m, m, Allocator.Temp);
            ...
            var Rview = new doubleMxN(m, m, Allocator.Temp);
            var infoView = QR.decompInPlace(ref view, ref Rview);

            var Rexact = new doubleMxN(m, m, Allocator.Temp);
        ...
        void SquareViewRetroactiveFix_CHOMatchesExact()
        {
            ...
            var bView = new doubleN(m, Allocator.Temp);
            var bExact = new doubleN(m, Allocator.Temp);
            for (int i = 0; i < m; i++) { bView[i] = (double)(i + 1); bExact[i] = (double)(i + 1); }
