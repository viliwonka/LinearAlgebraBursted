...
// raw pointers inside Burst jobs -- see QRCPDowndateTests.double.cs / LUTests.double.cs calling
...
public class doubleUnsafeSortTests
{
    ...
    public unsafe struct TestJob : IJob
    {
        ...
        void RandomVariousN()
        {
            ...
            for (int s = 0; s < 5; s++)
            {
                ...
                var key = new doubleN(n, Allocator.Temp);
                var val = new NativeArray<int>(n, Allocator.Temp);
                for (int i = 0; i < n; i++) { key[i] = rng.NextDouble(-(double)1000, (double)1000); val[i] = i; }
        ...
        void DuplicateKeys()
        {
            ...
            var key = new doubleN(n, Allocator.Temp);
            var val = new NativeArray<int>(n, Allocator.Temp);
            for (int i = 0; i < n; i++) { key[i] = (double)rng.NextInt(0, 10); val[i] = i; }
        ...
        void AlreadySorted()
        {
            ...
            var key = new doubleN(n, Allocator.Temp);
            var val = new NativeArray<int>(n, Allocator.Temp);
            for (int i = 0; i < n; i++) { key[i] = (double)i; val[i] = i; }
        ...
        void ReverseSorted()
        {
            ...
            var key = new doubleN(n, Allocator.Temp);
            var val = new NativeArray<int>(n, Allocator.Temp);
            for (int i = 0; i < n; i++) { key[i] = (double)(n - i); val[i] = i; }
        ...
        {
            if (Fail[0] == (double)0) { Fail[0] = (double)1; Fail[1] = (double)code; Fail[2] = (double)a; Fail[3] = (double)b; }
        }
    }
