...
// Tests for the weighted discrete picker (doubleRandom_OP.weightedPick / weightedPickInpl).
...
public class doubleRandomWeightedTests
{
    [BurstCompile]
    public struct TestJob : IJob
    {
        ...
        void SingleElementAlwaysZero()
        {
            ...
            w[0] = (double)5;
            for (int i = 0; i < 200; i++)
                AssertTrue(doubleRandom_OP.weightedPick(in w, ref rng) == 0);
            arena.Dispose();
        }
        ...
        void ZeroWeightNeverPicked()
        {
            ...
            {
                int idx = doubleRandom_OP.weightedPick(in w, ref rng);
        ...
        void InplFillsInRange()
        {
            ...
            doubleRandom_OP.weightedPickInpl(in w, ref dest, ref rng);
            AssertTrue(dest.N == k);
            for (int i = 0; i < k; i++)
            {
    ...
    public void WeightedPickValidationThrows()
    {
        ...
        Assert.Throws<ArgumentException>(() => doubleRandom_OP.weightedPick(in empty, ref rng));

        // Any negative weight.
        var neg = arena.doubleVec(3);
        neg[0] = (double)1; neg[1] = (double)(-2); neg[2] = (double)1;
        Assert.Throws<ArgumentException>(() => doubleRandom_OP.weightedPick(in neg, ref rng));

        // NaN weight (0/0 at runtime; not a compile-time constant).
        var nan = arena.doubleVec(2);
        nan[0] = (double)1; nan[1] = (double)0 / (double)0;
        Assert.Throws<ArgumentException>(() => doubleRandom_OP.weightedPick(in nan, ref rng));

        // +Inf weight (1/0 at runtime).
        var inf = arena.doubleVec(2);
        inf[0] = (double)1; inf[1] = (double)1 / (double)0;
        Assert.Throws<ArgumentException>(() => doubleRandom_OP.weightedPick(in inf, ref rng));

        // All-zero total.
        var zero = arena.doubleVec(3);
        zero[0] = (double)0; zero[1] = (double)0; zero[2] = (double)0;
        Assert.Throws<ArgumentException>(() => doubleRandom_OP.weightedPick(in zero, ref rng));
    ...
    public void WeightedPickInplValidatesUpFrontEvenWhenDestEmpty()
    {
        ...
        Assert.Throws<ArgumentException>(() => doubleRandom_OP.weightedPickInpl(in bad, ref emptyDest, ref rng));
        ...
        Assert.Throws<ArgumentException>(() => doubleRandom_OP.weightedPickInpl(in zero, ref emptyDest2, ref rng));
