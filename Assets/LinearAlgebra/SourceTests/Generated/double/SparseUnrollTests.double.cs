...
// (bsmMatVecB{b} / bsmMatVecTB{b} / bsmMatVecSymB{b} in UnsafeOP.Sparse.double.cs), dispatched
// from Sparse_OP.spMV / spMVT (SparseOP.double.cs). Every case here proves ONE dispatch branch
// against the dense reference: build a doubleBSM via the builder, expand with ToDense (already
// independently validated by doubleSparseBSMTests), and assert spMV/spMVT agree with
// Linear_OP.dot on the dense expansion -- exactly the recipe doubleSparseBSMTests.RandomSpMV /
...
public class doubleSparseUnrollTests
{
    ...
    public struct SparseUnrollTestJob : IJob
    {
        ...
        // Matches doubleSparseBSMTests.Tol(): values live in [-1,1], dot products sum a handful
        ...
        // Linear_OP.trans), matching doubleSparseBSMTests.DenseTransMatVec.
        ...
        static doubleBSM BuildRandomSquare(ref Arena arena, int b, uint seedBase)
        {
        ...
        void CheckSquareSpMV(int b, uint seedBase)
        {
            ...
            var x = arena.doubleRandomVec(A.N_Cols, (double)(-1f), (double)1f, seedBase + 900u);
        ...
        void CheckSquareSpMVT(int b, uint seedBase)
        {
            ...
            var A = BuildRandomSquare(ref arena, b, seedBase);
            var dense = A.ToDense(ref arena);
            var xt = arena.doubleRandomVec(A.M_Rows, (double)(-1f), (double)1f, seedBase + 900u);
        ...
        void CheckRectangularSpMV()
        {
            ...
            builder.AddBlock(0, 0, arena.doubleRandomMat(BR, BC, (double)(-1f), (double)1f, 71001));
            builder.AddBlock(0, 2, arena.doubleRandomMat(BR, BC, (double)(-1f), (double)1f, 71002));
            builder.AddBlock(1, 1, arena.doubleRandomMat(BR, BC, (double)(-1f), (double)1f, 71003));
            builder.AddBlock(2, 0, arena.doubleRandomMat(BR, BC, (double)(-1f), (double)1f, 71004));
            var A = builder.ToBSM(ref arena);
            var dense = A.ToDense(ref arena);

            var x = arena.doubleRandomVec(A.N_Cols, (double)(-1f), (double)1f, 71100);
            var y = arena.doubleVec(A.M_Rows);
            Sparse_OP.spMV(in A, in x, ref y);
        ...
        void CheckRectangularSpMVT()
        {
            ...
            const int BR = 2, BC = 3;
            var builder = arena.doubleBSMBuilder(3, 3, BR, BC);
            builder.AddBlock(0, 0, arena.doubleRandomMat(BR, BC, (double)(-1f), (double)1f, 72001));
            builder.AddBlock(0, 2, arena.doubleRandomMat(BR, BC, (double)(-1f), (double)1f, 72002));
            builder.AddBlock(1, 1, arena.doubleRandomMat(BR, BC, (double)(-1f), (double)1f, 72003));
            builder.AddBlock(2, 0, arena.doubleRandomMat(BR, BC, (double)(-1f), (double)1f, 72004));
            var A = builder.ToBSM(ref arena);
            var dense = A.ToDense(ref arena);

            var xt = arena.doubleRandomVec(A.M_Rows, (double)(-1f), (double)1f, 72100);
            var yt = arena.doubleVec(A.N_Cols);
            Sparse_OP.spMVT(in A, in xt, ref yt);
