...
// kernels (the reconstruction products are recomputed in double from the double factors), so the
...
public class doubleAccuracySweepTests
{
    ...
    public struct AccuracyJob : IJob
    {
        ...
        double Eps() => (double)Consts.doubleEpsilon;
        ...
        void RunLQ(int m, int n, uint seed, int mode)
        {
            ...
            if (mode == 1)
            {
                ...
                for (int i = 0; i < m; i++)
                {
                    double s = (double)math.pow(10.0, -3.0 * i / (m - 1));
                    for (int j = 0; j < n; j++)
                        A[i, j] *= s;
                }
            }
        ...
        static double FrobA(in doubleMxN A)
        {
            double s = 0;
        ...
        static double ReconResidual2(in doubleMxN A, in doubleMxN Q, in doubleMxN R)
        {
        ...
        static double ReconResidualLQ(in doubleMxN A, in doubleMxN L, in doubleMxN Q)
        {
            int m = A.M_Rows, n = A.N_Cols;
            double num = 0, den = 0;
            for (int i = 0; i < m; i++)
        ...
        static double ReconResidualLLt(in doubleMxN A, in doubleMxN L)
        {
            int n = A.M_Rows;
            double num = 0, den = 0;
            for (int i = 0; i < n; i++)
        ...
        double ReconResidualPALU(in doubleMxN A, in doubleMxN L, in doubleMxN U, in Pivot P)
        {
            int n = A.M_Rows;
            var PA = A.Copy();
        ...
        static double OrthoErrorCols(in doubleMxN Q)
        {
        ...
        static double OrthoErrorRows(in doubleMxN Q)
        {
            int m = Q.M_Rows, n = Q.N_Cols;
            double s = 0;
            for (int i = 0; i < m; i++)
        ...
        static double DiagCond(in doubleMxN T, bool squared)
        {
        ...
        // Fail layout: [0]=flag, [1]=value, [2]=limit, [3]=excess. value/limit stored as double for
        ...
        void AssertLE(double value, double limit)
        {
            if (!(value <= limit) && Fail[0] == (double)0)
            {
                ...
                Fail[1] = (double)value;
                Fail[2] = (double)limit;
                Fail[3] = (double)(value - limit);
            }
            Assert.IsTrue(value <= limit);
        }
    }

    // true only when double expands to double.
    private static bool IsDouble() => (double)Consts.doubleEpsilon < 1e-10;
