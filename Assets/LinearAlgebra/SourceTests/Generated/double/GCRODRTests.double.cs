...
public class doubleGCRODRTests
{
    ...
    public struct GcrodrTestJob : IJob
    {
        ...
        static doubleMxN SmallIsolatedEig(int n, uint seed)
        {
            ...
            A[2, 2] = (double)0.06;
            for (int i = 3; i < n; i++)
                A[i, i] = (double)(4.0 + 6.0 * (i - 3) / (double)(n - 4));   // cluster in [4, 10]

            var rnd = new Unity.Mathematics.Random(seed);
            for (int i = 0; i < n; i++)
                for (int j = i + 1; j < n; j++)
                    A[i, j] = (double)(rnd.NextFloat(-1f, 1f) * 0.02f);   // mild nonsymmetry, near-normal
            return A;
        }
        ...
        void RecyclingBeatsGmres()
        {
            ...
            Assert.IsTrue(doubleKrylovBatteryOracles.RelResidualDense(in A, in xR, in b) <= tol);
        ...
        // doubleIdentityPreconditioner path share the exact IsIdentity-folded body, so x, iteration
