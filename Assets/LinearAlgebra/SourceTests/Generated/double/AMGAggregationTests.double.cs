...
public class doubleAMGAggregationTests
{
    ...
    public struct AggregateTestJob : IJob
    {
        ...
        static doubleBSR Chain(ref Arena arena, int nb, double offMag)
        {
            var b = arena.doubleBSRBuilder(nb, nb, 1, 1, 3 * nb);
            for (int i = 0; i < nb; i++)
            {
                b.AddValue(i, i, (double)2);
        ...
        void ChainPartitionsAndCoarsens()
        {
            ...
            int nb = 12;
            var A = Chain(ref arena, nb, (double)1);
        ...
        void DiagonalOnlyAllSingletons()
        {
            var arena = new Arena(Allocator.Persistent);
            int nb = 8;
            var b = arena.doubleBSRBuilder(nb, nb, 1, 1, nb);
            for (int i = 0; i < nb; i++) b.AddValue(i, i, (double)3);
        ...
        void HighThetaWeakLinksAllSingletons()
        {
            var arena = new Arena(Allocator.Persistent);
            int nb = 10;
            var A = Chain(ref arena, nb, (double)0.01);   // off-diagonals tiny vs diagonal 2
            var aggId = arena.Indices(nb);

            // threshold = theta*sqrt(2*2) = 1.0 >> 0.01 -> all links weak -> all singletons.
            AMG.aggregate(in A, (double)0.5, ref aggId, out int numAgg);
            ...
            AMG.aggregate(in A, (double)0, ref aggId0, out int numAgg0);
        ...
        void ZeroDiagonalNodeIsolated()
        {
            ...
            for (int i = 0; i < nb; i++)
            {
                b.AddValue(i, i, i == 2 ? (double)0 : (double)2);
                if (i > 0) b.AddValue(i, i - 1, (double)(-1));
                if (i < nb - 1) b.AddValue(i, i + 1, (double)(-1));
            }
            var A = b.ToBSR(ref arena);
            var aggId = arena.Indices(nb);
        ...
        void Deterministic()
        {
            ...
            AMG.aggregate(in A, (double)0, ref a1, out int n1);
            AMG.aggregate(in A, (double)0, ref a2, out int n2);
