...
public class doubleSparseTransposeTests
{
    ...
    public struct SparseTransposeTestJob : IJob
    {
        ...
        static doubleBSR BuildTallRectBSR()
        {
            const int blockRows = 3, blockCols = 2, BR = 3, BC = 2;
            var b = new doubleBSRBuilder(blockRows, blockCols, BR, BC, Allocator.Temp, 4);
            b.AddBlock(0, 0, GenerateOP.doubleRandomMat(BR, BC, -1f, 1f, 70001));
            b.AddBlock(0, 1, GenerateOP.doubleRandomMat(BR, BC, -1f, 1f, 70002));
            b.AddBlock(1, 1, GenerateOP.doubleRandomMat(BR, BC, -1f, 1f, 70003));
            b.AddBlock(2, 0, GenerateOP.doubleRandomMat(BR, BC, -1f, 1f, 70004));
        ...
        static doubleBSR BuildWideRectBSR()
        {
            const int blockRows = 2, blockCols = 3, BR = 2, BC = 3;
            var b = new doubleBSRBuilder(blockRows, blockCols, BR, BC, Allocator.Temp, 4);
            b.AddBlock(0, 0, GenerateOP.doubleRandomMat(BR, BC, -1f, 1f, 71001));
            b.AddBlock(0, 2, GenerateOP.doubleRandomMat(BR, BC, -1f, 1f, 71002));
            b.AddBlock(1, 0, GenerateOP.doubleRandomMat(BR, BC, -1f, 1f, 71003));
            b.AddBlock(1, 1, GenerateOP.doubleRandomMat(BR, BC, -1f, 1f, 71004));
            return b.ToBSR(Allocator.Temp);
        }
        ...
        // outer dimensions.
        static void CrossCheckTranspose(in doubleBSR A, uint seed)
        {
            ...
            var x = GenerateOP.doubleRandomVec(A.M_Rows, -1f, 1f, seed);
        ...
        void OperatorApplyTParity()
        {
            ...
            var xT = GenerateOP.doubleRandomVec(A.M_Rows, -1f, 1f, 73001);
            var yT1 = new doubleN(A.N_Cols, Allocator.Temp);
            var yT2 = new doubleN(A.N_Cols, Allocator.Temp);
            ...
            var xF = GenerateOP.doubleRandomVec(A.N_Cols, -1f, 1f, 73002);
            var yF1 = new doubleN(A.M_Rows, Allocator.Temp);
            var yF2 = new doubleN(A.M_Rows, Allocator.Temp);
        ...
        //
        // For symmetric lower-block storage A == A^T by construction, so doubleBSRTranspose returns
        ...
        void SymmetricNoOp()
        {
            ...
            for (int i = 0; i < nb; i++)
            {
                var Mi = GenerateOP.doubleRandomMat(BR, BR, -1f, 1f, (uint)(74000 + i));
            ...
            s.AddBlock(1, 0, GenerateOP.doubleRandomMat(BR, BR, -0.2f, 0.2f, 74100));
            s.AddBlock(2, 1, GenerateOP.doubleRandomMat(BR, BR, -0.2f, 0.2f, 74101));
            ...
            var x = GenerateOP.doubleRandomVec(dim, -1f, 1f, 74200);
