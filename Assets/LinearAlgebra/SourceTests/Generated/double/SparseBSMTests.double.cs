...
public class doubleSparseBSMTests
{
    ...
    public struct SparseBSMTestJob : IJob
    {
        ...
        static void Scatter(ref doubleMxN dense, int br, int bc, in doubleMxN block)
        {
        ...
        //
        // NOTE: doubleBSM.ToDense used to take `in Arena`, which forced a defensive copy of the
        // arena before its internal (mutating) arena.doubleMat(...) call -- the returned matrix's
        // _arenaPtr captured the address of that dead temporary, so Linear_OP.trans(dense)
        // (which allocates via dense.tempdoubleMat) dereferenced a dangling pointer and threw
        ...
        void HandBuiltSmall()
        {
            ...
            for (int r = 0; r < BR; r++)
                for (int c = 0; c < BC; c++)
                {
                    b00[r, c] = (double)(10 + r * BC + c);
                    b01[r, c] = (double)(20 + r * BC + c);
                    b11[r, c] = (double)(30 + r * BC + c);
                }
        ...
        void DuplicateSummation()
        {
            ...
            builder.AddValue(1, 0, (double)5);
            builder.AddValue(1, 0, (double)7);          // duplicate scalar -> summed
        ...
        void OneByOneBlocks()
        {
            ...
            builder.AddValue(0, 3, (double)(-1));
            builder.AddValue(2, 1, (double)4);
