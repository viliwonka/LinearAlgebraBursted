...
//   (a) IdoubleLinearOperator.ApplyDot on every shipped operator -- each composes (Apply, then
...
//   (b) doubleBlockJacobi.Apply's blockJacobiApplyB{1,2,3,4,6} unrolls vs the general runtime-BR
...
public class doubleKrylovRound2Tests
{
    ...
    public struct KrylovRound2TestJob : IJob
    {
        ...
        static void AssertExactVec(in doubleN got, in doubleN expected)
        {
        ...
        static void CheckApplyDotExact<TOp>(in TOp op, in doubleN x, ref Arena arena)
            where TOp : struct, IdoubleLinearOperator
        {
            int rows = op.Rows;
            var y1 = arena.doubleVec(rows);
            double d1 = op.ApplyDot(in x, ref y1);

            var y2 = arena.doubleVec(rows);
            op.Apply(in x, ref y2);
            double d2 = Blas.dot(x, y2);
        ...
        void ApplyDotBSRFullExact()
        {
            ...
            const int b = 3, nb = 4;
            int dim = b * nb;
            var builder = arena.doubleBSRBuilder(nb, nb, b, b, 8);
            builder.AddBlock(0, 0, arena.doubleRandomMat(b, b, -1f, 1f, 70101));
            builder.AddBlock(0, 2, arena.doubleRandomMat(b, b, -1f, 1f, 70102));
            builder.AddBlock(1, 1, arena.doubleRandomMat(b, b, -1f, 1f, 70103));
            builder.AddBlock(1, 3, arena.doubleRandomMat(b, b, -1f, 1f, 70104));
            builder.AddBlock(2, 0, arena.doubleRandomMat(b, b, -1f, 1f, 70105));
            builder.AddBlock(3, 3, arena.doubleRandomMat(b, b, -1f, 1f, 70106));
            var A = builder.ToBSR(ref arena);
            var x = arena.doubleRandomVec(dim, -1f, 1f, 70110);
            CheckApplyDotExact(new doubleBSROperator(in A), in x, ref arena);
            arena.Dispose();
        }
        ...
        void ApplyDotBSRSymExact()
        {
            ...
            builder.AddBlock(0, 1, arena.doubleRandomMat(b, b, -1f, 1f, 70205));  // upper off-diagonals
            builder.AddBlock(1, 3, arena.doubleRandomMat(b, b, -1f, 1f, 70206));
            var A = builder.ToBSRSymmetric(ref arena);
            var x = arena.doubleRandomVec(dim, -1f, 1f, 70210);
            CheckApplyDotExact(new doubleBSROperator(in A), in x, ref arena);
            arena.Dispose();
        }
        ...
        void ApplyDotIdentityExact()
        {
            var arena = new Arena(Allocator.Persistent);
            int n = 9;
            var x = arena.doubleRandomVec(n, -1f, 1f, 70301);
            CheckApplyDotExact(new doubleIdentityOperator(n), in x, ref arena);
            arena.Dispose();
        }
        ...
        void ApplyDotColScaledSquareExact()
        {
            ...
            var x = arena.doubleRandomVec(n, -1f, 1f, 70403);
        ...
        void ApplyDotNormalOperatorExact()
        {
            ...
            var scratch = arena.doubleVec(n);
            var op = new doubleNormalOperator<doubleBSROperator>(in inner, in d, in scratch, (double)0.3f);
            var x = arena.doubleRandomVec(m, -1f, 1f, 70503);   // M is m x m -> x length m
            CheckApplyDotExact(in op, in x, ref arena);
            arena.Dispose();
        }

        // ==============================================================================
        // (b) doubleBlockJacobi.Apply: unrolled kernels (b in {1,2,3,4,6}) vs the general loop,
        ...
        static doubleBSR BuildBlockDiagBSR(ref Arena arena, int nb, int b, uint seed)
        {
            var builder = arena.doubleBSRBuilder(nb, nb, b, b, nb);
            for (int i = 0; i < nb; i++)
        ...
        void CheckBlockJacobi(int b, uint seed, bool checkFold)
        {
            ...
            var r = arena.doubleRandomVec(dim, -1f, 1f, seed + 500u);
            ...
            if (checkFold)
            {
                ...
                // in doubleBlockJacobi.Apply), reading the SAME DInv the unroll reads. Same
                // left-to-right fold order => the unrolled result must be bit-identical.
                var dinv = M.DInv;
                int blockLen = b * b;
                var zRef = arena.doubleVec(dim);
                for (int i = 0; i < nb; i++)
                {
                    int rowBase = i * b;
        ...
        static doubleBSR BuildFullMultiBlock(ref Arena arena, int b, uint seed)
        {
            var builder = arena.doubleBSRBuilder(5, 5, b, b, 16);
            builder.AddBlock(0, 0, arena.doubleRandomMat(b, b, -1f, 1f, seed + 1u));   // row0: 4 (even)
            builder.AddBlock(0, 1, arena.doubleRandomMat(b, b, -1f, 1f, seed + 2u));
            builder.AddBlock(0, 2, arena.doubleRandomMat(b, b, -1f, 1f, seed + 3u));
            builder.AddBlock(0, 3, arena.doubleRandomMat(b, b, -1f, 1f, seed + 4u));
            builder.AddBlock(1, 0, arena.doubleRandomMat(b, b, -1f, 1f, seed + 5u));   // row1: 3 (odd)
            builder.AddBlock(1, 2, arena.doubleRandomMat(b, b, -1f, 1f, seed + 6u));
            builder.AddBlock(1, 4, arena.doubleRandomMat(b, b, -1f, 1f, seed + 7u));
            builder.AddBlock(2, 1, arena.doubleRandomMat(b, b, -1f, 1f, seed + 8u));   // row2: 2 (even)
            builder.AddBlock(2, 3, arena.doubleRandomMat(b, b, -1f, 1f, seed + 9u));
            builder.AddBlock(3, 3, arena.doubleRandomMat(b, b, -1f, 1f, seed + 10u));  // row3: 1 (odd)
        ...
        static doubleBSR BuildSymMultiBlock(ref Arena arena, int b, uint seed)
        {
            var builder = arena.doubleBSRBuilder(5, 5, b, b, 16);
            for (int i = 0; i < 5; i++)
                builder.AddBlock(i, i, SpdBlock(ref arena, b, seed + (uint)i + 1u));
            builder.AddBlock(0, 1, arena.doubleRandomMat(b, b, -1f, 1f, seed + 11u));  // row0: diag + 3
            builder.AddBlock(0, 2, arena.doubleRandomMat(b, b, -1f, 1f, seed + 12u));
            builder.AddBlock(0, 4, arena.doubleRandomMat(b, b, -1f, 1f, seed + 13u));
            builder.AddBlock(1, 3, arena.doubleRandomMat(b, b, -1f, 1f, seed + 14u));  // row1: diag + 2
            builder.AddBlock(1, 4, arena.doubleRandomMat(b, b, -1f, 1f, seed + 15u));
            builder.AddBlock(2, 3, arena.doubleRandomMat(b, b, -1f, 1f, seed + 16u));  // row2: diag + 1
        ...
        void PairedSpMVT()
        {
            ...
            for (int t = 0; t < PairedBs.Length; t++)
            {
                ...
                var dense = A.ToDense(ref arena);
                var xt = arena.doubleRandomVec(A.M_Rows, -1f, 1f, (uint)(92500 + b));
        ...
        void PairedSpMVSym()
        {
            ...
            for (int t = 0; t < PairedBs.Length; t++)
            {
                ...
                var dense = A.ToDense(ref arena);
                var x = arena.doubleRandomVec(A.N_Cols, -1f, 1f, (uint)(93500 + b));
    ...
    // ==============================================================================
    // (e) Rectangular doubleColScaledOperator.ApplyDot: Apply is fine (x length Cols, y length
    ...
    public void ColScaledRectangularApplyDotThrowsOnDimensionMismatch()
    {
        ...
        var A = arena.doubleRandomMat(m, n, -1f, 1f, 96001);
        var d = arena.doubleRandomVec(n, (double)0.5f, (double)2f, 96002);
        var scratch = arena.doubleVec(n);
        var op = new doubleColScaledOperator<doubleDenseOperator>(new doubleDenseOperator(in A), d, scratch);

        var x = arena.doubleRandomVec(n, -1f, 1f, 96003);   // length Cols = n
        var y = arena.doubleVec(m);                         // length Rows = m
