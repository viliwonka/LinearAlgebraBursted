...
public class doubleKrylovRound2Tests
{
    ...
    public struct KrylovRound2TestJob : IJob
    {
        ...
        static doubleMxN SpdBlock(int b, uint seed)
        {
            var M = GenerateOP.doubleRandomMat(b, b, -1f, 1f, seed);
        ...
        static void CheckApplyDotExact<TOp>(in TOp op, in doubleN x)
            where TOp : struct, IdoubleLinearOperator
        {
            int rows = op.Rows;
            var y1 = new doubleN(rows, Allocator.Temp);
            double d1 = op.ApplyDot(in x, ref y1);

            var y2 = new doubleN(rows, Allocator.Temp);
        ...
        {
            int n = 11;
            var A = doubleKrylovBatteryOracles.BuildDenseSpdSystem(n, 70001);      // square so dot(x, A x) is well-formed
            var x = GenerateOP.doubleRandomVec(n, -1f, 1f, 70002);
            CheckApplyDotExact(new doubleDenseOperator(in A), in x);
        }
        ...
        void ApplyDotBSRFullExact()
        {
            ...
            const int b = 3, nb = 4;
            int dim = b * nb;
            var builder = new doubleBSRBuilder(nb, nb, b, b, Allocator.Temp, 8);
            builder.AddBlock(0, 0, GenerateOP.doubleRandomMat(b, b, -1f, 1f, 70101));
            builder.AddBlock(0, 2, GenerateOP.doubleRandomMat(b, b, -1f, 1f, 70102));
            builder.AddBlock(1, 1, GenerateOP.doubleRandomMat(b, b, -1f, 1f, 70103));
            builder.AddBlock(1, 3, GenerateOP.doubleRandomMat(b, b, -1f, 1f, 70104));
            builder.AddBlock(2, 0, GenerateOP.doubleRandomMat(b, b, -1f, 1f, 70105));
            builder.AddBlock(3, 3, GenerateOP.doubleRandomMat(b, b, -1f, 1f, 70106));
            var A = builder.ToBSR(Allocator.Temp);
            var x = GenerateOP.doubleRandomVec(dim, -1f, 1f, 70110);
            CheckApplyDotExact(new doubleBSROperator(in A), in x);
        }
        ...
        void ApplyDotBSRSymExact()
        {
            ...
            builder.AddBlock(1, 0, GenerateOP.doubleRandomMat(b, b, -1f, 1f, 70205));  // lower off-diagonals
            builder.AddBlock(3, 1, GenerateOP.doubleRandomMat(b, b, -1f, 1f, 70206));
            var A = builder.ToBSRSymmetric(Allocator.Temp);
            var x = GenerateOP.doubleRandomVec(dim, -1f, 1f, 70210);
            CheckApplyDotExact(new doubleBSROperator(in A), in x);
        }
        ...
        {
            int n = 9;
            var x = GenerateOP.doubleRandomVec(n, -1f, 1f, 70301);
            CheckApplyDotExact(new doubleIdentityOperator(n), in x);
        }
        ...
        void ApplyDotColScaledSquareExact()
        {
            ...
            int n = 8;
            var A = doubleKrylovBatteryOracles.BuildDenseSpdSystem(n, 70401);
            var d = GenerateOP.doubleRandomVec(n, (double)0.5f, (double)2f, 70402);   // nonzero scale
            ...
            var x = GenerateOP.doubleRandomVec(n, -1f, 1f, 70403);
        ...
        void ApplyDotNormalOperatorExact()
        {
            ...
            int m = 6, n = 4;
            var Adense = GenerateOP.doubleRandomMat(m, n, -1f, 1f, 70501);
            var bsm = doubleKrylovBatteryOracles.DenseToBSR1x1(in Adense);
            var inner = new doubleBSROperator(in bsm);
            var d = GenerateOP.doubleRandomVec(n, (double)0.25f, (double)1.5f, 70502);  // length As.Cols
            var scratch = new doubleN(n, Allocator.Temp);
            var op = new doubleNormalOperator<doubleBSROperator>(in inner, in d, in scratch, (double)0.3f);
            var x = GenerateOP.doubleRandomVec(m, -1f, 1f, 70503);   // M is m x m -> x length m
            CheckApplyDotExact(in op, in x);
        }
        ...
        static doubleBSR BuildBlockDiagBSR(int nb, int b, uint seed)
        {
            var builder = new doubleBSRBuilder(nb, nb, b, b, Allocator.Temp, nb);
            for (int i = 0; i < nb; i++)
        ...
        void CheckBlockJacobi(int b, uint seed, bool checkFold)
        {
            ...
            var r = GenerateOP.doubleRandomVec(dim, -1f, 1f, seed + 500u);
            ...
            if (checkFold)
            {
                ...
                var dinv = M.DInv;
                int blockLen = b * b;
                var zRef = new doubleN(dim, Allocator.Temp);
                for (int i = 0; i < nb; i++)
                {
                    int rowBase = i * b;
        ...
        static doubleBSR BuildFullMultiBlock(int b, uint seed)
        {
            var builder = new doubleBSRBuilder(5, 5, b, b, Allocator.Temp, 16);
            builder.AddBlock(0, 0, GenerateOP.doubleRandomMat(b, b, -1f, 1f, seed + 1u));   // row0: 4 (even)
            builder.AddBlock(0, 1, GenerateOP.doubleRandomMat(b, b, -1f, 1f, seed + 2u));
            builder.AddBlock(0, 2, GenerateOP.doubleRandomMat(b, b, -1f, 1f, seed + 3u));
            builder.AddBlock(0, 3, GenerateOP.doubleRandomMat(b, b, -1f, 1f, seed + 4u));
            builder.AddBlock(1, 0, GenerateOP.doubleRandomMat(b, b, -1f, 1f, seed + 5u));   // row1: 3 (odd)
            builder.AddBlock(1, 2, GenerateOP.doubleRandomMat(b, b, -1f, 1f, seed + 6u));
            builder.AddBlock(1, 4, GenerateOP.doubleRandomMat(b, b, -1f, 1f, seed + 7u));
            builder.AddBlock(2, 1, GenerateOP.doubleRandomMat(b, b, -1f, 1f, seed + 8u));   // row2: 2 (even)
            builder.AddBlock(2, 3, GenerateOP.doubleRandomMat(b, b, -1f, 1f, seed + 9u));
            builder.AddBlock(3, 3, GenerateOP.doubleRandomMat(b, b, -1f, 1f, seed + 10u));  // row3: 1 (odd)
        ...
        static doubleBSR BuildSymMultiBlock(int b, uint seed)
        {
            var builder = new doubleBSRBuilder(5, 5, b, b, Allocator.Temp, 16);
            for (int i = 0; i < 5; i++)
                builder.AddBlock(i, i, SpdBlock(b, seed + (uint)i + 1u));
            builder.AddBlock(4, 3, GenerateOP.doubleRandomMat(b, b, -1f, 1f, seed + 11u));  // row4: diag + 3
            builder.AddBlock(4, 2, GenerateOP.doubleRandomMat(b, b, -1f, 1f, seed + 12u));
            builder.AddBlock(4, 0, GenerateOP.doubleRandomMat(b, b, -1f, 1f, seed + 13u));
            builder.AddBlock(3, 1, GenerateOP.doubleRandomMat(b, b, -1f, 1f, seed + 14u));  // row3: diag + 2
            builder.AddBlock(3, 0, GenerateOP.doubleRandomMat(b, b, -1f, 1f, seed + 15u));
            builder.AddBlock(2, 1, GenerateOP.doubleRandomMat(b, b, -1f, 1f, seed + 16u));  // row2: diag + 1
        ...
        void PairedSpMVFull()
        {
            for (int t = 0; t < PairedBs.Length; t++)
            {
                ...
                var dense = A.ToDense(Allocator.Temp);
                var x = GenerateOP.doubleRandomVec(A.N_Cols, -1f, 1f, (uint)(91500 + b));
        ...
        void PairedSpMVT()
        {
            for (int t = 0; t < PairedBs.Length; t++)
            {
                ...
                var dense = A.ToDense(Allocator.Temp);
                var xt = GenerateOP.doubleRandomVec(A.M_Rows, -1f, 1f, (uint)(92500 + b));
        ...
        void PairedSpMVSym()
        {
            for (int t = 0; t < PairedBs.Length; t++)
            {
                ...
                var dense = A.ToDense(Allocator.Temp);
                var x = GenerateOP.doubleRandomVec(A.N_Cols, -1f, 1f, (uint)(93500 + b));
        ...
        {
            int dim = 12;
            var A = doubleKrylovBatteryOracles.BuildDenseSpdSystem(dim, 94001);
            var bsm = doubleKrylovBatteryOracles.DenseToBSR1x1(in A);
            var b = GenerateOP.doubleRandomVec(dim, -1f, 1f, 94002);
        ...
        {
            int dim = 12;
            var A = doubleKrylovBatteryOracles.BuildDenseSpdSystem(dim, 95001);
            var bsm = doubleKrylovBatteryOracles.DenseToBSR1x1(in A);
            var M = new doubleBlockJacobi(in bsm, Allocator.Temp);
            var b = GenerateOP.doubleRandomVec(dim, -1f, 1f, 95002);
    ...
    public void ColScaledRectangularApplyDotThrowsOnDimensionMismatch()
    {
        ...
        var A = GenerateOP.doubleRandomMat(m, n, -1f, 1f, 96001);
        var d = GenerateOP.doubleRandomVec(n, (double)0.5f, (double)2f, 96002);
        var scratch = new doubleN(n, Allocator.Temp);
        var op = new doubleColScaledOperator<doubleDenseOperator>(new doubleDenseOperator(in A), d, scratch);

        var x = GenerateOP.doubleRandomVec(n, -1f, 1f, 96003);   // length Cols = n
        var y = new doubleN(m, Allocator.Temp);                  // length Rows = m
