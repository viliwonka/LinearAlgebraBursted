...
// ARENA-LEVEL wiring tests for the doubleN/doubleMxN -> ChunkedRecordTable migration. The table's
...
// ChunkedRecordTableTests.cs -- these tests prove the COMPOSITION: that Arena.doubleVec/doubleMat,
// doubleN/doubleMxN.Dispose()/Copy()/TempCopy(), and Arena.Clear()/ClearTemp()/Dispose() drive
// those tables correctly end to end.
//
// Split, mirroring ChunkedRecordTableTests.cs / ArenaHandleTests.double.cs:
...
public class doubleArenaWiringTests
{
    ...
    public unsafe struct WiringTestJob : IJob
    {
        ...
        void MultiChunkVectors()
        {
            ...
            const int n = 300;
            const int len = 3;
            doubleN* vecs = stackalloc doubleN[n];

            for (int i = 0; i < n; i++)
            {
                doubleN v = arena.doubleVec(len, uninit: true);
                for (int j = 0; j < len; j++)
                    v[j] = (double)(i * len + j);   // distinct sentinel per (alloc, element)
                vecs[i] = v;
            }
        ...
        // ---- multi-chunk: 300 persistent matrices, same argument on the doubleMatRecords table ----
        void MultiChunkMatrices()
        {
            ...
            const int n = 300;
            const int R = 2, C = 2;
            doubleMxN* mats = stackalloc doubleMxN[n];

            for (int i = 0; i < n; i++)
            {
                doubleMxN m = arena.doubleMat(R, C, uninit: true);
                for (int r = 0; r < R; r++)
                    for (int c = 0; c < C; c++)
                        m[r, c] = (double)(i * (R * C) + r * C + c);
                mats[i] = m;
            }
        ...
        // _core->doubleTempVecRecords.ChunkCount. ChunkedRecordTableTests.FreeListRecyclingDoesNot-
        ...
        void TempRecyclingCycles()
        {
            ...
            const int N = 20;
            var seedV = arena.doubleVec(4, (double)0);
            var seedM = arena.doubleMat(3, 3, (double)0);
            ...
            for (int cycle = 0; cycle < 3; cycle++)
            {
                for (int i = 0; i < N; i++)
                {
                    ...
                    doubleN tv = seedV.TempCopy();
                    tv[0] = (double)(cycle * 1000 + i);
                    Assert.AreEqual(cycle * 1000 + i, (int)tv[0]);

                    doubleMxN tm = seedM.TempCopy();
                    tm[0, 0] = (double)(cycle * 1000 + i);
        ...
        // doubleN.Dispose(), and (b) that a following Clear()/Dispose() SKIPS the already-freed slot
        // (IsAlive guard) instead of double-freeing it -- no crash, no leak.
        void DirectDisposeCountsThenClearVectors()
        {
            var arena = new Arena(Allocator.Persistent);

            var a = arena.doubleVec(4, (double)1);
            var b = arena.doubleVec(4, (double)2);
            var c = arena.doubleVec(4, (double)3);
        ...
        void DirectDisposeCountsThenClearMatrices()
        {
            ...
            var a = arena.doubleMat(2, 2, (double)1);
            var b = arena.doubleMat(2, 2, (double)2);
            var c = arena.doubleMat(2, 2, (double)3);
            Assert.AreEqual(3, arena.AllocationsCount);
        ...
        void DisposeThenReallocateRecyclesSlot()
        {
            ...
            var v1 = arena.doubleVec(4, uninit: true);
            for (int j = 0; j < 4; j++) v1[j] = (double)(100 + j);   // sentinel written into v1's buffer
            Assert.AreEqual(1, arena.AllocationsCount);

            v1.Dispose();                                            // frees the slot back to the free-list
            Assert.AreEqual(0, arena.AllocationsCount);

            var v2 = arena.doubleVec(4, uninit: false);              // should recycle v1's freed slot
            Assert.AreEqual(1, arena.AllocationsCount);
            ...
            for (int j = 0; j < 4; j++) v2[j] = (double)(200 + j);
    ...
    //
    // (An alias is REQUIRED to trip the guard: doubleN.Dispose() nulls its own _rec, so calling
    ...
    public void AliasedDoubleDispose_Vector_Throws()
    {
        ...
        try
        {
            var v = arena.doubleVec(8, (double)0);
            var alias = v;                 // struct copy -- shares v's doubleVecRecord*
            Assert.AreEqual(1, arena.AllocationsCount);
    ...
    public void AliasedDoubleDispose_Matrix_Throws()
    {
        ...
        try
        {
            var m = arena.doubleMat(3, 4, (double)0);
            var alias = m;
            Assert.AreEqual(1, arena.AllocationsCount);
    ...
    // InvalidOperationException instead of silently returning a dead/garbage buffer. doubleN is
    // option (b) (Alive-only: the 32B struct has no spare padding for a generation stamp); doubleMxN is
    ...
    // (b) VECTOR read-after-Dispose. doubleN.Dispose() nulls the DISPOSER's OWN _rec (reading the
    ...
    public void Vector_ReadAfterDispose_Throws()
    {
        ...
        try
        {
            var v = arena.doubleVec(4, (double)0);
    ...
    public void Vector_ReadAfterClearTemp_Throws()
    {
        ...
        try
        {
            var seed = arena.doubleVec(4, (double)0);
    ...
    // doubleMxN's Alive+generation guard. Same alias-for-dispose requirement (mutable struct nulls its
    // own _rec on Dispose).
    [Test]
    ...
    public void Matrix_ReadAfterClearTemp_Throws()
    {
        ...
        try
        {
            var seed = arena.doubleMat(3, 3, (double)0);
    ...
    // read true again for the new occupant and wave the stale read through, but doubleMxN's _gen was
    ...
    public void Matrix_StaleGenerationAfterSlotRecycle_OldThrows_NewWorks()
    {
        ...
        try
        {
            var v1 = arena.doubleMat(2, 3, (double)0);
            ...
            var v2 = arena.doubleMat(2, 3, (double)0);  // recycles v1's EXACT slot; generation bumped
            Assert.AreEqual(1, arena.AllocationsCount);
