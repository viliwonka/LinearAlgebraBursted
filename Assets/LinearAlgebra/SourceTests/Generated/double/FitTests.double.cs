...
public class doubleFitTests
{
    ...
    public void QuadricCoefficientsMatchTheGeneratingEllipsoid()
    {
        ...
        for (int i = 0; i < 8; i++)
            for (int j = 0; j < 12; j++)
            {
                ...
                pts[k++] = new double3((double)(cx + a * math.sin(u) * math.cos(v)),
                                       (double)(cy + b * math.sin(u) * math.sin(v)),
                                       (double)(cz + cc * math.cos(u)));
            }

        var got = new doubleN(10, Allocator.Temp);
    ...
    // by a finite combinatorial algorithm, and Fit.linear with doubleL1Loss approaches it by iterative
    // reweighting. doubleL1Loss's own doc points callers at LP.lad for exactness -- these tests are
    ...
    public void IrlsL1AgreesWithTheExactLadSolver()
    {
        ...
        b[3] = (double)40; b[9] = (double)(-30); b[12] = (double)55;    // gross, and asymmetric

        var xExact = new doubleN(2, Allocator.Temp);
        ...
        var xIrls = new doubleN(2, Allocator.Temp);
    ...
    public void LinearL1BeatsL2UnderResponseOutliers()
    {
        ...
        var l1 = new doubleL1Loss((double)1e-2);
        var xL1 = new doubleN(2, Allocator.Temp);
    ...
    public void PlaneL1BeatsL2UnderOutlier()
    {
        ...
        var l1 = new doubleL1Loss((double)1e-2);
        Assert.IsTrue(Fit.plane(pts, in l1, out _, out double3 nL1), "L1 plane fit failed");
    ...
    public void Line3DRobustBeatsL2UnderOutlier()
    {
        const int n = 21;
        var pts = new NativeArray<double3>(n, Allocator.Temp);
        for (int i = 0; i < 20; i++) pts[i] = new double3((double)i, (double)i, (double)i);
        pts[20] = new double3((double)10, (double)26, (double)10);

        var want = math.normalize(new double3((double)1, (double)1, (double)1));

        Assert.IsTrue(Fit.line(pts, out _, out double3 dL2), "3D line fit failed");
        double errL2 = AngleError(dL2, want);

        var huber = new doubleHuberLoss((double)1);
        Assert.IsTrue(Fit.line(pts, in huber, out _, out double3 dH), "3D robust line fit failed");
        double errH = AngleError(dH, want);
    ...
    {
        const int n = 16;
        var pts = new NativeArray<double4>(n, Allocator.Temp);
        for (int i = 0; i < 15; i++)
            pts[i] = new double4((double)i, (double)i, (double)i, (double)i);
        pts[15] = new double4((double)7, (double)20, (double)7, (double)7);

        var want = math.normalize(new double4((double)1, (double)1, (double)1, (double)1));

        Assert.IsTrue(Fit.line(pts, out double4 c, out double4 dL2), "4D line fit failed");
        ...
        var huber = new doubleHuberLoss((double)1);
        Assert.IsTrue(Fit.line(pts, in huber, out _, out double4 dH), "4D robust line fit failed");
    ...
    public void Sphere3DRobustBeatsAlgebraicUnderOutlier()
    {
        ...
        for (int i = 0; i < 6; i++)
            ...
            {
                double u = math.PI_DBL * (i + 0.5) / 6.0, v = 2.0 * math.PI_DBL * j / 7.0;
                pts[k++] = new double3((double)(4.0 * math.sin(u) * math.cos(v)),
                                       (double)(4.0 * math.sin(u) * math.sin(v)),
                                       (double)(4.0 * math.cos(u)));
            }
        pts[k++] = new double3((double)7, (double)0, (double)0);     // off the surface

        Assert.IsTrue(Fit.sphere(pts, out _, out double rA), "3D algebraic sphere fit failed");

        var huber = new doubleHuberLoss((double)0.5);
        Assert.IsTrue(Fit.sphere(pts, in huber, out _, out double rH), "3D robust sphere fit failed");
    ...
    public void EllipsoidHandlesFlatterThanTwoToOne()
    {
        ...
        Assert.IsTrue(Fit.ellipsoid(pts, out _, out double3 rad, out _, out _),
    ...
    public void EllipsoidSamplingIsUniformByAreaNotByDirection()
    {
        var e = new Fit.doubleEllipsoid
        {
            ...
            AxisA = new double3((double)1, (double)0, (double)0),
            AxisB = new double3((double)0, (double)1, (double)0),
            Radii = new double3((double)1, (double)1, (double)0.5),
        };
    ...
    public void CapsuleSamplingSplitsTubeAndCapsByArea()
    {
        ...
        var c = new Fit.doubleCapsule
        {
            A = default,
            B = new double3((double)0, (double)0, (double)len),
            Radius = (double)rad,
        };
    ...
    public void TotalIsScaleInvariantAtLargeMagnitude()
    {
        ...
        foreach (double s in new[] { 1.0, 1e4 })
        {
            ...
            for (int i = 0; i < m; i++)
            {
                ...
                A[i, 0] = (double)a0; A[i, 1] = (double)a1;
                b[i] = (double)(2.0 * a0 - 0.5 * a1);
            }
    ...
    public void ConicSurvivesAnOffOriginCloud()
    {
        foreach (double off in new[] { 0.0, 500.0 })
        {
            ...
            {
                double t = 2.0 * math.PI_DBL * i / n;
                pts[i] = new double2((double)(off + 3.0 * math.cos(t)), (double)(off + 2.0 * math.sin(t)));
            }

            Assert.IsTrue(Fit.ellipse(pts, out double2 c, out double2 rad, out _),
