...
public class doubleSparseSymmetricTests
{
    ...
    public struct SparseSymmetricTestJob : IJob
    {
        ...
        void CgSymMatchesFull()
        {
            ...
            bool okSym = Solvers.conjugateGradient(in sym, in b, ref xSym, 4 * dim, Consts.doubleSqrtEps);
            Assert.IsTrue(okSym);

            var xFull = arena.doubleVec(dim);
            bool okFull = Solvers.conjugateGradient(in full, in b, ref xFull, 4 * dim, Consts.doubleSqrtEps);
            Assert.IsTrue(okFull);

            var xDense = arena.doubleVec(dim);
            bool okDense = Solvers.conjugateGradient(in dense, in b, ref xDense, 4 * dim, Consts.doubleSqrtEps);
        ...
        void MinresSymMatchesFull()
        {
            ...
            var xSym = arena.doubleVec(dim);
            bool okSym = Solvers.minres(in sym, in b, ref xSym, 4 * dim, Consts.doubleSqrtEps);
            Assert.IsTrue(okSym);

            var xFull = arena.doubleVec(dim);
            bool okFull = Solvers.minres(in full, in b, ref xFull, 4 * dim, Consts.doubleSqrtEps);
            Assert.IsTrue(okFull);
        ...
        void PcgBlockJacobiSymMatchesFull()
        {
            ...
            bool okSym = Solvers.pcg(in sym, in mSym, in b, ref xPcgSym, 4 * dim, Consts.doubleSqrtEps);
            Assert.IsTrue(okSym);

            var xPcgFull = arena.doubleVec(dim);
            bool okFull = Solvers.pcg(in full, in mFull, in b, ref xPcgFull, 4 * dim, Consts.doubleSqrtEps);
            Assert.IsTrue(okFull);
