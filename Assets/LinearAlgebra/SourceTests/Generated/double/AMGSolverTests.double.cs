...
public class doubleAMGSolverTests
{
    ...
    public struct AMGSolverTestJob : IJob
    {
        ...
        void BsrMatchesPrebuilt()
        {
            var A = Poisson2D(16, 16, Allocator.Temp);
            int n = A.M_Rows;
            var b = GenerateOP.doubleRandomVec(n, -1f, 1f, 0xB17u);

            var amg = new doubleAMG(in A, out var pre, Allocator.Temp);
            Assert.IsTrue(pre.Solved);
            var xPre = new doubleN(n, Allocator.Temp);
            for (int i = 0; i < n; i++) xPre[i] = (double)0;
            var sPre = MG.solve(in amg, in b, ref xPre, 100, Tol());

            var xOne = new doubleN(n, Allocator.Temp);
            for (int i = 0; i < n; i++) xOne[i] = (double)0;
        ...
        void BsrDefaults()
        {
            ...
            int n = A.M_Rows;
            var b = GenerateOP.doubleRandomVec(n, -1f, 1f, 0x0D15u);
        ...
        void BsrOptionsDefaults()
        {
            ...
            var b = GenerateOP.doubleRandomVec(n, -1f, 1f, 0x0D16u);
        ...
        void BsrNotPositiveDefinite()
        {
            ...
            var b = GenerateOP.doubleRandomVec(nb, -1f, 1f, 0x0D17u);
            var x = new doubleN(nb, Allocator.Temp);
            for (int i = 0; i < nb; i++) x[i] = (double)7;
            ...
            for (int i = 0; i < nb; i++) Assert.IsTrue(x[i] == (double)7);
        }
    }
