...
public class doubleLadSparseDesignTests
{
    static doubleBSR ToBsr1x1(in doubleMxN dense, Allocator alloc)
    {
        int m = dense.M_Rows, n = dense.N_Cols;
        int nnz = 0;
        for (int i = 0; i < m; i++) for (int j = 0; j < n; j++) if (dense[i, j] != (double)0) nnz++;
        var builder = new doubleBSRBuilder(m, n, 1, 1, alloc, nnz);
        for (int i = 0; i < m; i++)
            for (int j = 0; j < n; j++)
                if (dense[i, j] != (double)0)
                {
                    var blk = new doubleMxN(1, 1, alloc);
                    blk[0, 0] = dense[i, j];
    ...
    static void BuildSparseDesign(int m, int n, out doubleMxN A, out doubleN b, uint seed)
    {
        ...
        for (int i = 0; i < m; i++)
        {
            for (int j = 0; j < n; j++) A[i, j] = (double)0;
            A[i, 0] = (double)1;
            int c1 = 1 + rng.NextInt(0, n - 1);
            int c2 = 1 + rng.NextInt(0, n - 1);
            A[i, c1] = rng.NextDouble(0.5f, 2f);
            A[i, c2] = rng.NextDouble(0.5f, 2f);
            b[i] = (double)(2f * (float)A[i, c1] - 1.5f * (float)A[i, c2] + 0.75f + rng.NextDouble(-0.3f, 0.3f));
        }
        for (int k = 0; k < m / 10; k++) b[k * 7 % m] = (double)((float)b[k * 7 % m] + 25f);
    }
    ...
    public void RaggedRowPatternMatchesDense()
    {
        ...
        for (int i = 0; i < m; i++)
        {
            ...
            for (int t = 0; t < cnt; t++)
                A[i, (i * 3 + t * 5) % n] = rng.NextDouble(0.5f, 2f);
            double acc = 0;
            for (int j = 0; j < n; j++) acc += (double)A[i, j] * (1.0 + 0.25 * j);
            b[i] = (double)(acc + rng.NextDouble(-0.2f, 0.2f));
        }
        b[5] = (double)((float)b[5] + 30f);
        b[61] = (double)((float)b[61] - 30f);
        ...
        Assert.That(objS, Is.EqualTo(objD).Within(1e-9 * (1.0 + objD)),
