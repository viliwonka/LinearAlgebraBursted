...
public class doubleNormalizeTests
{
    ...
    public struct TestJob : IJob
    {
        ...
        static double ZTol() => (double)16 * (double)Consts.doubleSqrtEps;
        ...
        void MinMaxColumns()
        {
            ...
            doubleStatsOP.normalizeColumns(ref A, NormalizeMode.MinMax);

            // col0: min entry -> 0, max entry -> 1, interior -> exact fraction 0.5
            AssertClose(A[0, 0], (double)0f, MinMaxTol);
            AssertClose(A[1, 0], (double)0.5f, MinMaxTol);
            AssertClose(A[2, 0], (double)1f, MinMaxTol);

            // col1 (negatives): min entry -> 0, max entry -> 1, interior -> 0.5
            AssertClose(A[0, 1], (double)0f, MinMaxTol);
            AssertClose(A[1, 1], (double)1f, MinMaxTol);
            AssertClose(A[2, 1], (double)0.5f, MinMaxTol);
        ...
        void MinMaxRows()
        {
            ...
            doubleStatsOP.normalizeRows(ref A, NormalizeMode.MinMax);

            AssertClose(A[0, 0], (double)0f, MinMaxTol);
            AssertClose(A[0, 1], (double)0.25f, MinMaxTol);
            AssertClose(A[0, 2], (double)1f, MinMaxTol);

            AssertClose(A[1, 0], (double)0f, MinMaxTol);
            AssertClose(A[1, 1], (double)0.25f, MinMaxTol);
            AssertClose(A[1, 2], (double)1f, MinMaxTol);
        ...
        void ZScoreColumns()
        {
            ...
            doubleStatsOP.normalizeColumns(ref A, NormalizeMode.ZScore);

            // Specific entry oracle: [0,0] = (1-3)/sqrt(8/3); [2,1] = (60-30)/sqrt(1400/3).
            double exp00 = ((double)1f - (double)3f) / math.sqrt((double)(8f / 3f));
            double exp21 = ((double)60f - (double)30f) / math.sqrt((double)(1400f / 3f));
            AssertClose(A[0, 0], exp00, ZTol());
            AssertClose(A[2, 1], exp21, ZTol());

            // Property: each normalized column has mean ~ 0 and population std ~ 1.
            var mean = doubleStatsOP.colMean(in A);
            var std = doubleStatsOP.colStdDev(in A);
            for (int c = 0; c < A.N_Cols; c++)
            {
                AssertClose(mean[c], (double)0f, ZTol());
                AssertClose(std[c], (double)1f, ZTol());
            }
        ...
        void ZScoreRows()
        {
            ...
            doubleStatsOP.normalizeRows(ref A, NormalizeMode.ZScore);

            double exp00 = ((double)1f - (double)3f) / math.sqrt((double)(8f / 3f));
            double exp12 = ((double)60f - (double)30f) / math.sqrt((double)(1400f / 3f));
            AssertClose(A[0, 0], exp00, ZTol());
            AssertClose(A[1, 2], exp12, ZTol());

            var mean = doubleStatsOP.rowMean(in A);
            var std = doubleStatsOP.rowStdDev(in A);
            for (int r = 0; r < A.M_Rows; r++)
            {
                AssertClose(mean[r], (double)0f, ZTol());
                AssertClose(std[r], (double)1f, ZTol());
            }
        ...
        void CheckConstantColumn(NormalizeMode mode)
        {
            ...
            doubleStatsOP.normalizeColumns(ref A, mode);
            ...
            AssertGreater(maxAbs, (double)0f);
        ...
        void CheckConstantRow(NormalizeMode mode)
        {
            ...
            doubleStatsOP.normalizeRows(ref A, mode);
        ...
        void SingleRowMatrix()
        {
            ...
            for (int m = 0; m < 2; m++)
            {
                ...
                doubleStatsOP.normalizeColumns(ref A, m == 0 ? NormalizeMode.MinMax : NormalizeMode.ZScore);
            ...
            var B = arena2.doubleMat(1, 3);
            B[0, 0] = 4f;  B[0, 1] = 9f;  B[0, 2] = 2f;

            doubleStatsOP.normalizeRows(ref B, NormalizeMode.MinMax);
            AssertClose(B[0, 0], (double)(2f / 7f), MinMaxTol);
            AssertClose(B[0, 1], (double)1f, MinMaxTol);
            AssertClose(B[0, 2], (double)0f, MinMaxTol);
            arena2.Dispose();
        }
        ...
        void SingleColumnMatrix()
        {
            for (int m = 0; m < 2; m++)
            {
                ...
                doubleStatsOP.normalizeRows(ref A, m == 0 ? NormalizeMode.MinMax : NormalizeMode.ZScore);
            ...
            var arena2 = new Arena(Allocator.Persistent);
            var B = arena2.doubleMat(3, 1);
            B[0, 0] = 4f;  B[1, 0] = 9f;  B[2, 0] = 2f;

            doubleStatsOP.normalizeColumns(ref B, NormalizeMode.MinMax);
            AssertClose(B[0, 0], (double)(2f / 7f), MinMaxTol);
            AssertClose(B[1, 0], (double)1f, MinMaxTol);
            AssertClose(B[2, 0], (double)0f, MinMaxTol);
            arena2.Dispose();
        }
        ...
        void InPlaceMutation()
        {
            ...
            AssertClose(alias[1, 0], (double)0.5f, MinMaxTol);
            AssertClose(alias[2, 0], (double)1f, MinMaxTol);
            AssertClose(alias[0, 1], (double)0f, MinMaxTol);
        ...
        void AssertExactZero(double a)
        {
            if (!(a == (double)0f) && Fail[0] == (double)0)
            {
                Fail[0] = (double)1;
                Fail[1] = a;
                Fail[2] = (double)0f;
            ...
            Assert.IsTrue(a == (double)0f);
        }

        void AssertNotNaN(double a)
        {
            bool nan = math.isnan(a);
            if (nan && Fail[0] == (double)0)
            {
                Fail[0] = (double)1;
                Fail[1] = a;
                Fail[2] = (double)(-999f);
        ...
        void AssertGreater(double a, double limit)
        {
            if (!(a > limit) && Fail[0] == (double)0)
            {
                Fail[0] = (double)1;
                Fail[1] = a;
                Fail[2] = limit;
    ...
    public void EmptyMatrixNormalizeColumnsThrows()
    {
        ...
        var A0 = arena.doubleMat(0, 3); // 0 rows
        var A1 = arena.doubleMat(3, 0); // 0 cols

        Assert.Throws<InvalidOperationException>(() => doubleStatsOP.normalizeColumns(ref A0, NormalizeMode.MinMax));
        Assert.Throws<InvalidOperationException>(() => doubleStatsOP.normalizeColumns(ref A0, NormalizeMode.ZScore));
        Assert.Throws<InvalidOperationException>(() => doubleStatsOP.normalizeColumns(ref A1, NormalizeMode.MinMax));
        Assert.Throws<InvalidOperationException>(() => doubleStatsOP.normalizeColumns(ref A1, NormalizeMode.ZScore));
    ...
    public void EmptyMatrixNormalizeRowsThrows()
    {
        ...
        Assert.Throws<InvalidOperationException>(() => doubleStatsOP.normalizeRows(ref A0, NormalizeMode.MinMax));
        Assert.Throws<InvalidOperationException>(() => doubleStatsOP.normalizeRows(ref A0, NormalizeMode.ZScore));
        Assert.Throws<InvalidOperationException>(() => doubleStatsOP.normalizeRows(ref A1, NormalizeMode.MinMax));
        Assert.Throws<InvalidOperationException>(() => doubleStatsOP.normalizeRows(ref A1, NormalizeMode.ZScore));
