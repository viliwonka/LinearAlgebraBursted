...
using LinearAlgebra;
using LinearAlgebra.ML;        // doublePCA_OP, doublePCAModel, PCAScaling
using LinearAlgebra.Stats;     // doubleStats_OP.colMean (oracle side-uses)
...
// PCA (LinearAlgebra.ML.doublePCA_OP) — the four fit routes (pcaCovariance / pcaSVD /
// pcaSVDTruncated / pcaRandomized), the doublePCAModel, PCAScaling, and pcaTransform. Tests mirror
...
//
// Tolerances scale with Consts.doubleSqrtEps (float ≈ 3.45e-4, double ≈ 1.49e-8) so ONE expression is
...
public class doublePCATests
{
    ...
    public struct TestJob : IJob
    {
        ...
        void CrossRoute(PCAScaling scaling)
        {
            ...
            var mCov = doublePCA_OP.pcaCovariance(ref arena, in X, scaling);
            var mSvd = doublePCA_OP.pcaSVD(ref arena, in X, scaling);
            ...
            double ctol = (double)100 * Consts.doubleSqrtEps;
            for (int c = 0; c < p; c++)
            {
        ...
        void KnownSpectrum()
        {
            ...
            var m = doublePCA_OP.pcaCovariance(ref arena, in X);   // Covariance
            AssertTrue(m.converged);
            RecordEq(m.k, p);

            // variances recovered, descending.
            AssertClose(m.explainedVariance[0], (double)9, EvTol((double)9));
            AssertClose(m.explainedVariance[1], (double)4, EvTol((double)4));
            AssertClose(m.explainedVariance[2], (double)1, EvTol((double)1));

            // components ≈ identity: |diag| ≈ 1, |off| ≈ 0.
            double ctol = (double)100 * Consts.doubleSqrtEps;
            for (int c = 0; c < p; c++)
                for (int r = 0; r < p; r++)
                    AssertClose(math.abs(m.components[r, c]), (r == c) ? (double)1 : (double)0, ctol);
        ...
        void VarianceRatioAndOrder()
        {
            ...
            var m = doublePCA_OP.pcaCovariance(ref arena, in X);   // full route
            AssertTrue(m.converged);

            double sumRatio = (double)0;
            for (int i = 0; i < p; i++) sumRatio += m.explainedVarianceRatio[i];
            AssertClose(sumRatio, (double)1, (double)200 * Consts.doubleSqrtEps);

            // ev[0] is the max, and the sequence is descending.
            double gapTol = (double)50 * Consts.doubleSqrtEps;
            for (int i = 0; i < p; i++)
        ...
        void CorrelationDegenerate()
        {
            ...
            var mCov = doublePCA_OP.pcaCovariance(ref arena, in X, PCAScaling.Correlation);
            var mSvd = doublePCA_OP.pcaSVD(ref arena, in X, PCAScaling.Correlation);
            AssertTrue(mCov.converged);
            AssertTrue(mSvd.converged);

            // zero-variance feature → scale exactly 1 (never a divide-by-zero).
            AssertExact(mCov.scale[deg], (double)1);
            AssertExact(mSvd.scale[deg], (double)1);
            ...
            double sumCov = (double)0;
            for (int i = 0; i < p; i++) sumCov += mCov.explainedVariance[i];
            AssertClose(sumCov, (double)5, (double)500 * Consts.doubleSqrtEps);
            ...
            AssertClose(mCov.explainedVariance[p - 1], (double)0, (double)200 * Consts.doubleSqrtEps);
            AssertTrue(math.abs(mCov.components[deg, p - 1]) > (double)0.99f);
            for (int r = 0; r < p; r++)
                if (r != deg)
                    AssertTrue(math.abs(mCov.components[r, p - 1]) < (double)0.05f);
        ...
        void TopKExactMatchesFull()
        {
            ...
            var full = doublePCA_OP.pcaSVD(ref arena, in X);              // k == p
            var trunc = doublePCA_OP.pcaSVDTruncated(ref arena, in X, k); // k == 3
            AssertTrue(full.converged);
            AssertTrue(trunc.converged);
            RecordEq(trunc.k, k);
            RecordEq(trunc.components.N_Cols, k);

            double ctol = (double)200 * Consts.doubleSqrtEps;
            for (int i = 0; i < k; i++)
        ...
        void TopKRandomizedApprox()
        {
            ...
            var full = doublePCA_OP.pcaSVD(ref arena, in X);
            var rnd = doublePCA_OP.pcaRandomized(ref arena, in X, k);
            AssertTrue(full.converged);
            ...
            for (int i = 0; i < k; i++)
            {
                double exact = full.explainedVariance[i];
                double rel = math.abs(rnd.explainedVariance[i] - exact) / (math.abs(exact) + (double)1E-6f);
                AssertTrue(rel <= (double)0.05f);
            }

            // leading component recovered (loose, gap-guarded magnitude).
            if (WellSeparated(in full.explainedVariance, 0, p))
                for (int r = 0; r < p; r++)
                    AssertClose(math.abs(rnd.components[r, 0]), math.abs(full.components[r, 0]), (double)0.1f);
        ...
        void TransformScores()
        {
            ...
            var m = doublePCA_OP.pcaCovariance(ref arena, in X);   // Covariance, scale == 1
            AssertTrue(m.converged);

            var scores = doublePCA_OP.pcaTransform(ref arena, in X, in m);
            ...
            for (int c = 0; c < m.k; c++)
            {
                ...
                double ss = (double)0;
                for (int r = 0; r < n; r++) { double d = scores[r, c] - mean; ss += d * d; }
                double variance = ss / (double)(n - 1);

                AssertClose(variance, m.explainedVariance[c],
                            (math.abs(m.explainedVariance[c]) + (double)1) * (double)200 * Consts.doubleSqrtEps);
            }

            // hand-computed ((X-mean)/scale)·components matches the returned scores.
            double mtol = (double)200 * Consts.doubleSqrtEps;
            for (int r = 0; r < n; r++)
                for (int c = 0; c < m.k; c++)
                {
                    double acc = (double)0;
        ...
        void SignDeterminismNegate()
        {
            ...
            var m1 = doublePCA_OP.pcaCovariance(ref arena, in X1);
            var m2 = doublePCA_OP.pcaCovariance(ref arena, in X2);
        ...
        void RandomizedBitwise()
        {
            ...
            var a = doublePCA_OP.pcaRandomized(ref arena, in X, k);
            var b = doublePCA_OP.pcaRandomized(ref arena, in X, k);
        ...
        void WideCovariance()
        {
            ...
            var m = doublePCA_OP.pcaCovariance(ref arena, in X);
            AssertTrue(m.converged);
            RecordEq(m.k, p);
            RecordEq(m.components.M_Rows, p);
            RecordEq(m.components.N_Cols, p);

            AssertTrue(m.explainedVariance[0] > (double)0);
        ...
        double EvTol(double v) => (math.abs(v) + (double)1) * (double)50 * Consts.doubleSqrtEps;
        ...
        bool WellSeparated(in doubleN ev, int c, int count)
        {
            double vc = math.abs(ev[c]);
            double gap = double.MaxValue;
            if (c > 0)         gap = math.min(gap, math.abs(ev[c] - ev[c - 1]));
            if (c + 1 < count) gap = math.min(gap, math.abs(ev[c] - ev[c + 1]));
            return gap > (double)0.05f * (vc + (double)1E-6f);
        }
    ...
    public void NLessThanTwoThrows()
    {
        ...
        Assert.Throws<ArgumentException>(() => doublePCA_OP.pcaCovariance(ref arena, in X));
        arena.Dispose();
    }
    ...
    public void SvdWideThrows()
    {
        ...
        Assert.Throws<ArgumentException>(() => doublePCA_OP.pcaSVD(ref arena, in X));
        arena.Dispose();
    }
    ...
    public void SvdTruncatedWideThrows()
    {
        ...
        var arena = new Arena(Allocator.Persistent);
        var X = arena.doubleMat(3, 5);   // wide: p > n
        Assert.Throws<ArgumentException>(() => doublePCA_OP.pcaSVDTruncated(ref arena, in X, 2));
        arena.Dispose();
    }
    ...
    public void RandomizedWideThrows()
    {
        var arena = new Arena(Allocator.Persistent);
        var X = arena.doubleMat(3, 5);   // wide: p > n
        Assert.Throws<ArgumentException>(() => doublePCA_OP.pcaRandomized(ref arena, in X, 2));
        arena.Dispose();
    }
    ...
    public void TruncatedKZeroThrows()
    {
        ...
        Assert.Throws<ArgumentException>(() => doublePCA_OP.pcaSVDTruncated(ref arena, in X, 0));   // k <= 0
        arena.Dispose();
    }
    ...
    public void TruncatedKTooLargeThrows()
    {
        var arena = new Arena(Allocator.Persistent);
        var X = arena.doubleMat(10, 4);
        Assert.Throws<ArgumentException>(() => doublePCA_OP.pcaSVDTruncated(ref arena, in X, 5));   // k > min(n,p)=4
        arena.Dispose();
    }
    ...
    public void RefModelWrongShapeThrows()
    {
        ...
        Assert.Throws<ArgumentException>(() => doublePCA_OP.pcaCovariance(in X, ref model));
        arena.Dispose();
    }
    ...
    public void TransformStaleKThrows()
    {
        var arena = new Arena(Allocator.Persistent);
        var X = arena.doubleMat(10, 4);
        var model = doublePCA_OP.pcaCovariance(ref arena, in X);   // k == 4 == components.N_Cols
        model.k = model.components.N_Cols + 1;                     // stale / hand-tampered
        Assert.Throws<ArgumentException>(() => doublePCA_OP.pcaTransform(ref arena, in X, in model));
        arena.Dispose();
    }
    ...
    public void TransformMisSizedScoresThrows()
    {
        var arena = new Arena(Allocator.Persistent);
        var X = arena.doubleMat(10, 4);
        var model = doublePCA_OP.pcaCovariance(ref arena, in X);   // k == 4
        var badScores = arena.doubleMat(10, 3);                    // wrong column count (should be k == 4)
        Assert.Throws<ArgumentException>(() => doublePCA_OP.pcaTransform(in X, in model, ref badScores));
        arena.Dispose();
    }
}
