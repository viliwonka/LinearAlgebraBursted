...
// ARENA-LEVEL wiring tests for the longN/longMxN -> ChunkedRecordTable migration. The table's
...
public class longArenaWiringTests
{
    ...
    // InvalidOperationException instead of silently returning a dead/garbage buffer. longN is
    // option (b) (Alive-only: the 32B struct has no spare padding for a generation stamp); longMxN is
    ...
    // (b) VECTOR read-after-Dispose. longN.Dispose() nulls the DISPOSER's OWN _rec, so the stale read
    ...
    public void Vector_ReadAfterClearTemp_Throws()
    {
        ...
        try
        {
            var seed = arena.longVec(4, (long)0);
    ...
    // longMxN's Alive+generation guard. Same alias-for-dispose requirement (mutable struct).
    [Test]
    ...
    public void Matrix_ReadAfterClearTemp_Throws()
    {
        ...
        try
        {
            var seed = arena.longMat(3, 3, (long)0);
    ...
    // again for the new occupant, but longMxN's _gen (stamped at v1's construction) no longer matches
    ...
    public void Matrix_StaleGenerationAfterSlotRecycle_OldThrows_NewWorks()
    {
        ...
        try
        {
            var v1 = arena.longMat(2, 3, (long)0);
            ...
            var v2 = arena.longMat(2, 3, (long)0);  // recycles v1's EXACT slot; generation bumped
            Assert.AreEqual(1, arena.AllocationsCount);
