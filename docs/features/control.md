# Control — discrete-time LQR (`Control`)
...
- **`Control.lqr(in A, in B, in Q, in R, ref K[, maxIter])`** — cold solve via SDA
  (structure-preserving doubling, quadratic convergence, ~10-25 steps typically). `A`/`Q` are `n×n`,
  `B` is `n×m`, `R` is `m×m`, `K` (output) is `m×n`.
- **`Control.lqr(in A, in B, in Q, in R, ref K, ref fProxyLQRState state[, maxIter])`** —
